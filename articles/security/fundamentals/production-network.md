...
**Firewall hypervisoru (filtr paketů)** : Tato brána firewall je implementovaná v hypervisoru a konfigurovaná agentem řadiče prostředků infrastruktury (FC). Tato brána firewall chrání klienta, který běží ve virtuálním počítači, před neoprávněným přístupem. Ve výchozím nastavení platí, že když se vytvoří virtuální počítač, zablokuje se veškerý provoz a Agent FC přidá do filtru pravidla a výjimky, které umožní autorizovaný provoz.
...
- **Pravidla konfiguračního souboru role**: Definuje příchozí seznamy ACL založené na modelu služby klientů. Například pokud má tenant webový front-end na portu 80 na určitém virtuálním počítači, otevře se port 80 na všech IP adresách. Pokud má virtuální počítač spuštěnou roli pracovního procesu, je role pracovního procesu otevřená jenom pro virtuální počítač v rámci stejného tenanta.

**Nativní hostitelská brána firewall**: Azure Service Fabric a Azure Storage běží na nativním operačním systému, který nemá žádný hypervisor a proto je brána Windows Firewall nakonfigurovaná s předchozími dvěma sadami pravidel.
...
**Brána firewall hosta**: Brána firewall systému Windows hostovaného operačního systému, kterou můžou zákazníci konfigurovat pro virtuální počítače a úložiště zákazníků.
