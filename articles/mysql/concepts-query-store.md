---
title: Query Store ve službě Azure Database for MySQL
description: Tento článek popisuje funkci Query Store ve službě Azure Database for MySQL
author: ajlam
ms.author: andrela
ms.service: mysql
ms.topic: conceptual
...
1. Přihlaste se k webu Azure portal a vyberte váš server Azure Database for MySQL.
...
Použití [webu Azure portal](howto-server-parameters.md) nebo [rozhraní příkazového řádku Azure](howto-configure-server-parameters-using-cli.md) získat nebo nastavit jinou hodnotu pro parametr.

## <a name="views-and-functions"></a>Zobrazení a funkce

Umožňuje zobrazit a spravovat Query Store pomocí následujících zobrazení a funkce. Kdokoli ve [vyberte oprávnění veřejné role](howto-create-users.md#how-to-create-additional-admin-users-in-azure-database-for-mysql) pomocí těchto zobrazení zobrazit data v dotazu Store. Tato zobrazení jsou dostupné jen **mysql** databáze.
...
- Pokud tento parametr je na serveru MySQL `default_transaction_read_only` , Query Store nejde zachytit data.
