...
V tomto kurzu vytvoříte aplikaci pro iOS od začátku a integrujete rozhraní API pro čtení a moderní čtečku s využitím sady moderní čtečky. Kompletní pracovní ukázka tohoto kurzu je k dispozici [zde](https://github.com/microsoft/immersive-reader-sdk/iOS/samples/picture-to-immersive-reader-swift).
...
3.  Přidejte CocoaPod do svého souboru podfile `pod 'immersive-reader-sdk', :path => 'https://github.com/microsoft/immersive-reader-sdk/iOS/immersive-reader-sdk'`přidáním. Váš souboru podfile by měl vypadat jako na následujícím obrázku s názvem vašeho cíle, který nahrazuje obrázkový a rychlý čtenář – SWIFT:
 ```ruby
  platform :ios, '9.0'
