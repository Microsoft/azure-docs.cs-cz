...
| Koncový bod modulu runtime | Zdarma | Basic | Standard | Poznámky |
|------------------|------|-------|----------|-------|
...
IP adresy, které Azure Logic Apps používá pro příchozí a odchozí volání, závisí na oblasti, ve které vaše aplikace logiky existuje. *Všechny* Logic Apps ve stejné oblasti používají stejné rozsahy IP adres. Některá volání [Automatizace](https://docs.microsoft.com/power-automate/getting-started) , jako jsou požadavky **http** a **http + openapi** , procházejí přímo prostřednictvím služby Azure Logic Apps a pocházejí z IP adres, které jsou zde uvedeny. Další informace o IP adresách, které používá Power automat, najdete v tématu [omezení a konfigurace v Power automatizuje](https://docs.microsoft.com/flow/limits-and-config#ip-address-configuration).

> [!TIP]
> Chcete-li při vytváření pravidel zabezpečení zjednodušit složitost, můžete místo určení Logic Appsch IP adres pro každou oblast použít [značky služby](../virtual-network/service-tags-overview.md), které jsou popsané dále v této části. Tyto značky fungují v oblastech, kde je služba Logic Apps k dispozici:
>
...
V této části jsou uvedeny příchozí IP adresy pouze pro službu Azure Logic Apps. Chcete-li při vytváření pravidel zabezpečení zjednodušit složitost, můžete místo zadání předpony IP adres příchozích Logic Apps pro každou oblast použít [značku služby](../virtual-network/service-tags-overview.md) **LogicAppsManagement**. Tato značka funguje v oblastech, kde je služba Logic Apps k dispozici. Pokud máte Azure Government, přečtěte si téma [IP adresy Azure Government příchozích připojení](#azure-government-inbound).
...
| Západní střed USA | 52.161.26.172, 52.161.8.128, 52.161.19.82, 13.78.137.247 |
...
V této části jsou uvedeny odchozí IP adresy pro službu Azure Logic Apps a spravované konektory. Chcete-li při vytváření pravidel zabezpečení zjednodušit složitost, můžete místo zadání předpony IP adresy odchozího Logic Apps pro každou oblast použít [značku služby](../virtual-network/service-tags-overview.md) **LogicApps**. Tato značka funguje v oblastech, kde je služba Logic Apps k dispozici. U spravovaných konektorů použijte IP adresy. Pokud máte Azure Government, přečtěte si téma [Azure Government-odchozí IP adresy](#azure-government-outbound).
...
| Kanada – střed | 52.233.29.92, 52.228.39.241, 52.228.39.244, 40.85.250.135, 40.85.250.212, 13.71.186.1, 40.85.252.47, 13.71.184.150 | 13.71.170.208 - 13.71.170.223, 52.228.33.76, 52.228.34.13, 52.228.42.205, 52.233.31.197, 52.237.24.126, 52.237.32.212 |
...
| Západní střed USA | 52.161.27.190, 52.161.18.218, 52.161.9.108, 13.78.151.161, 13.78.137.179, 13.78.148.140, 13.78.129.20, 13.78.141.75 | 13.71.195.32 - 13.71.195.47, 52.161.102.22, 13.78.132.82, 52.161.101.204 |
