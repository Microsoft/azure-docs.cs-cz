...
Nastavte tajný název na `AZURE_CREDENTIALS` a jeho hodnotu na řetězec JSON, který jste našli pod nadpisem *Nastavení úložiště GitHub a ověření*.
...
Příkaz `az spring-cloud app create` není aktuálně idempotentní.  Tento pracovní postup doporučujeme pro existující cloudové aplikace a instance Azure jaře.
...
Příkaz AZ `run` použije nejnovější verzi rozhraní příkazového řádku Azure CLI. Pokud dojde k zásadním změnám, můžete použít také specifickou verzi Azure CLI s Azure/CLI `action`. 
