...
Pomocí [ `docker login` příkazu](https://docs.docker.com/engine/reference/commandline/login/), jak je znázorněno v následujícím příkladu, se přihlaste `containerpreview.azurecr.io`k privátnímu registru kontejnerů pro kontejnery Cognitive Services. V poli uživatelské jméno a * \<heslo\> * * \<nahraďte heslem, které jste zadali v přihlašovacích údajích, které jste dostali od týmu Azure Cognitive Services.\> *
