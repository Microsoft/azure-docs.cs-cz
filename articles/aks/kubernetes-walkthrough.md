...
Pokud se rozhodnete nainstalovat a používat rozhraní příkazového řádku místně, musíte mít spuštěnou verzi Azure CLI 2.0.64 nebo novější. Pokud chcete zjistit verzi, spusťte `az --version`. Pokud potřebujete instalaci nebo upgrade, přečtěte si téma [instalace Azure CLI][azure-cli-install].
...
Pomocí příkazu [AZ AKS Create][az-aks-create] vytvořte cluster AKS. Následující příklad vytvoří cluster s názvem *myAKSCluster* s jedním uzlem. Azure Monitor pro kontejnery je také povoleno pomocí parametru *monitorování--Enable-addons* .  Dokončení této akce bude trvat několik minut.
...
V tomto rychlém startu jste nasadili cluster Kubernetes a do něj jste nasadili aplikaci s více kontejnery. K [webovému řídicímu panelu Kubernetes můžete získat přístup][kubernetes-dashboard] také pro svůj cluster AKS.
