...
`@BlobTrigger` Atribut slouží k poskytnutí přístupu k objektu blob, který funkci aktivoval. Podrobnosti najdete v [příkladu triggeru](#trigger---example) .
...
Přístup k datům objektů `context.bindings.<name from function.json>`BLOB pomocí.
...
`@BlobInput` Atribut poskytuje přístup k objektu blob, který funkci aktivoval. Použijete-li pole bajtů s atributem, nastavte `dataType` na `binary`. Podrobnosti najdete v [příkladu vstupu](#input---example) .
...
Přístup k datům objektu BLOB `context.bindings.<name from function.json>`pomocí.
...
`@BlobOutput` Atribut poskytuje přístup k objektu blob, který funkci aktivoval. Použijete-li pole bajtů s atributem, nastavte `dataType` na `binary`. Podrobnosti najdete v [příkladu výstupu](#output---example) .
