---
title: Definování technického profilu s vlastním uplatněním ve vlastních zásadách v Azure Active Directory B2C | Microsoft Docs
...
Element **OutputClaims** obsahuje seznam deklarací identity, které mají být prezentovány pro shromažďování dat od uživatele. Pokud chcete předem naplnit výstupní deklarace s některými hodnotami, použijte vstupní deklarace, které byly dříve popsané. Element může obsahovat také výchozí hodnotu. Pořadí deklarací v **OutputClaims** řídí pořadí, které Azure AD B2C vykreslí deklarace na obrazovce. Atribut **DefaultValue** se uplatní pouze v případě, že deklarace identity nebyla nikdy nastavena před. Ale pokud byl nastaven před předchozí krok orchestrace, i když uživatel opustí hodnotu prázdnou, výchozí hodnota se neprojeví. Chcete-li vynutit použití výchozí hodnoty, nastavte atribut **AlwaysUseDefaultValue** na `true`hodnotu. Chcete-li vynutit, aby uživatel poskytoval hodnotu pro konkrétní výstupní deklaraci identity, nastavte **požadovaný** atribut prvku **OutputClaims** na `true`.

Element **ClaimType** v kolekci **OutputClaims** musí nastavit element **UserInputType** na libovolný typ vstupu uživatele podporovaný Azure AD B2C, například `TextBox` nebo. `DropdownSingleSelect` Nebo element **OutputClaim** musí mít nastavenou hodnotu **DefaultValue**.
...
Následující výstupní deklarace identity je vždycky nastavená `live.com`na:
...
- **Nastavení výchozí hodnoty ve výstupní deklaraci** – bez shromažďování dat od uživatele nebo vrácení dat z technického profilu ověření. Technický profil s `true` vlastnímuplatněnímnastavujedeklaracispuštěnou`LocalAccountSignUpWithLogonEmail` na SelfAsserted-Input na.
...
V následujícím příkladu `LocalAccountSignUpWithLogonEmail` technický profil s vlastním uplatněním ukazuje použití výstupních deklarací identity a nastaví **rutiny SelfAsserted-Input** na. `true` Deklarace identity jsou`AAD-UserWriteUsingLogonEmail` výstupem technického profilu ověření a nezobrazují se uživateli. `objectId` `authenticationSource` `newUser`
...
Pokud chybí element **PersistedClaims** , technický profil s vlastním uplatněním neuchovává data Azure AD B2C. Místo toho je provedeno volání na technický profil ověření, který je zodpovědný za uchování dat. Například zásada registrace používá `LocalAccountSignUpWithLogonEmail` ke shromáždění nového profilu uživatele technický profil s vlastním uplatněním. `LocalAccountSignUpWithLogonEmail` Technický profil volá technický profil ověření pro vytvoření účtu v Azure AD B2C.
