...
- Přístup ke clusteru AKS pomocí příkazu `kubectl`. Další informace najdete v tématu [připojení ke clusteru](https://docs.microsoft.com/azure/aks/kubernetes-walkthrough#connect-to-the-cluster) .
...
Úpravou nasazení přidejte popisek Azure identity Selector. V části `/spec/template/metadata/labels`pokračujte na následující část. Měly by se zobrazit hodnoty jako `isazuremlapp: “true”`. Přidejte popisek AAD-pod-identity, jak je znázorněno níže.
...
[Přiřaďte spravovanou identitu Azure k příslušným rolím](https://docs.microsoft.com/azure/active-directory/managed-identities-azure-resources/how-to-manage-ua-identity-portal) pro přístup k dalším prostředkům Azure. Ujistěte se, že role, které přiřazujete, mají správné **akce s daty**. Například [role čtečky dat objektů BLOB úložiště](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#storage-blob-data-reader) bude mít oprávnění ke čtení pro objekt BLOB úložiště, zatímco [role obecného čtecího modulu](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#reader) nemusí.
...
Pokud jste vašemu přístupu k datům v **objektu BLOB úložiště**udělili přístup ke čtení identity Azure, k němu má `score.py` přístup pomocí následujícího kódu.
