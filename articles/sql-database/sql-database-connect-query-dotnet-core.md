...
2. V textovém editoru otevřete soubor **sqltest. csproj** a vložte mezi `<Project>` značky následující kód XML. Tento kód XML `System.Data.SqlClient` se přidá jako závislost.
...
- Přečtěte si, jak [navrhnout první databázi SQL Azure pomocí SSMS](sql-database-design-first-database.md) nebo [navrhnout databázi SQL Azure a připojit se pomocí C# a ADO.NET](sql-database-design-first-database-csharp.md).
