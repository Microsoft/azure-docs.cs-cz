...
#### <a name="version"></a>Verze
uživatelem definované verze řetězec pro dokument s manifestem clusteru.
|Atribut|Hodnota|
|---|---|
|jméno|Verze|
|Použití|povinné|
...
Pokud kontejner HEALTHCHECK povolená a docker sestavy health_status události pro tento kontejner, Service Fabric bude obsahovat jako součást systému hlášené stavu. Pokud není v pořádku health_status Service Fabric bude odesílat upozornění stavu. Ve výchozím nastavení je nastavena na hodnotu true.
|Atribut|Hodnota|
|---|---|
...
Pokud kontejner HEALTHCHECK povolená a docker sestavy health_status události pro tento kontejner, Service Fabric restartuje kontejner při health_status hlášené docker není v pořádku.
...
Režimu izolace pro kontejner. Platné hodnoty jsou výchozí, procesu nebo hostitele Hyper-v (podporuje se jenom pro Windows kontejnery) na Windows 10, je podporováno pouze režimu izolace technologie Hyper-V, tak kontejneru se spustí v režimu izolace technologie Hyper-V bez ohledu na jeho nastavení režimu izolace.
|Atribut|Hodnota|
|---|---|
|jméno|Izolace|
|Použití|nepovinné|
...
Zdrojové složky, které může být do složky ve virtuálním počítači, který je hostitelem kontejnery nebo trvalé vzdálené úložiště.
|Atribut|Hodnota|
|---|---|
|jméno|Zdroj|
|Použití|povinné|
...
Umístění, který zdroj je namapovaný na spuštěné kontejneru. Cíl proto nemůže být umístění, které již existuje v rámci vašeho kontejneru.
|Atribut|Hodnota|
|---|---|
|jméno|Cíl|
|Použití|povinné|
...
#### <a name="driver"></a>Ovladač
Název ovladače pro tento modul plug-in Azure Files svazek je "sfazurefile".
|Atribut|Hodnota|
|---|---|
|jméno|Ovladač|
|Použití|nepovinné|
...
Možnosti ovladače mají být předány ovladačů. Modul plug-in svazku soubory Azure podporuje následující možnosti ovladače: shareName (soubory Azure sdílené složky, která poskytuje svazku kontejneru), storageAccountName (Azure účet úložiště, který obsahuje sdílené složky Azure Files), storageAccountKey ( Přístupový klíč pro účet úložiště Azure, který obsahuje sdílené složky Azure Files).
...
Název koncového bodu, který musí být deklarován v oddílu prostředků služby manifestu.  Při použití protokolu HTTPS, nepoužívejte stejný port a certifikát pro instance různé služby (nezávisle na aplikaci), které jsou nasazené na stejném uzlu. Upgrade dvě různé služby pomocí stejný port na jinou aplikaci instancí povede selhání při upgradu.
|Atribut|Hodnota|
|---|---|
|jméno|EndpointRef|
...
Sestavení verzi operačního systému. Verze sestavení systému Windows Server verze. 1709 je například 16299.
|Atribut|Hodnota|
|---|---|
...
#### <a name="name"></a>Název
Jedinečný identifikátor pro metriku v rámci clusteru z hlediska správce prostředků clusteru.
|Atribut|Hodnota|
|---|---|
|jméno|Název|
|Použití|povinné|
...
Omezuje prostředky, které lze použít na hostiteli a deklaruje limitů prostředků pro balíček kódu služby.
...
Celkové množství paměti odkládacího souboru, které je možné, v MB. Musí být kladné celé číslo.
|Atribut|Hodnota|
|---|---|
...
Logicky limit (v MB) pro řízení paměti, která je vynucená jenom v případě, že je zjištěna kolizí paměti na uzlu. Musí být kladné celé číslo.
|Atribut|Hodnota|
|---|---|
...
Použít procento k dispozici procesory (jenom Windows). Musí být kladné celé číslo. Pokud procesor omezení nejsou určené pro balíček služby, je tento parametr efektivně ignorován.
|Atribut|Hodnota|
|---|---|
|jméno|CpuPercent|
|type|xs:String|
...
Maximální vstupně-výstupní operace rychlost přenosu (čtení a zápis) z hlediska IOps, která lze použít. Musí být kladné celé číslo.
|Atribut|Hodnota|
|---|---|
|jméno|MaximumIOps|
|type|xs:String|
...
Maximální vstupně-výstupní operace (v bajtech za sekundu), můžete použít (čtení a zápis). Musí být kladné celé číslo.
|Atribut|Hodnota|
|---|---|
...
Vstupně-výstupní operace bloku váhy, relativně k jiným balíčkům kódu. Musí být kladné celé číslo od 10 do 1000.
|Atribut|Hodnota|
|---|---|
|jméno|BlockIOWeight|
|type|xs:String|
...
Identifikátor manifest služby nestrukturovaných řetězec.
|Atribut|Hodnota|
|---|---|
|jméno|ID manifestu|
|Použití|nepovinné|
...
#### <a name="contentchecksum"></a>ContentChecksum
Součet tohoto ServicePackage obsahu.
|Atribut|Hodnota|
|---|---|
|jméno|ContentChecksum|
|type|xs:String|
...
Popisuje uživatelská nastavení ServiceComponent nebo aplikaci. Skládá se z jednoho nebo více oddílů páry klíč hodnota.
