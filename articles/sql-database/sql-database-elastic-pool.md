...
Dostupné úrovně služeb a omezení pro každý model prostředků najdete v tématu [nákupní model založený na DTU](sql-database-service-tiers-dtu.md) nebo v [nákupním modelu založeném na Vcore](sql-database-service-tiers-vcore.md).
...
> [!IMPORTANT]
> Pokud se počet databází ve fondu blíží maximální podporované hodnotě, nezapomeňte zvážit [správu prostředků v hustých elastických fondech](sql-database-elastic-pool-resource-management.md).
> 
...
- **Geografické obnovení**
  ...
  Geografické obnovení nabízí výchozí možnost obnovení, když databáze není k dispozici z důvodu incidentu v oblasti, ve které je databáze hostována. Viz [obnovení Azure SQL Database nebo převzetí služeb při selhání sekundárním](sql-database-disaster-recovery.md)
