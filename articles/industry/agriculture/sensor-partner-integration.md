...
Pomocí této součásti můžou partneři vyvíjet senzory, které se integrují s FarmBeats, aby mohli používat rozhraní API a posílat data a telemetrie zákaznických zařízení do FarmBeats DataHub. Data se provizuálů pomocí akcelerátoru FarmBeats. Data se dají použít pro fúze dat a vytváření modelů strojového učení a umělých logik.

## <a name="link-a-farmbeats-account"></a>Propojit účet FarmBeats

Po zakoupení a nasazení zařízení nebo senzorů můžete získat přístup k datům a telemetrii zařízení na portálu SaaS (software jako služba) partnerů zařízení. Partneři zařízení vám umožní propojit váš účet s instancí FarmBeats v Azure. Následující přihlašovací údaje musí být vyplněny vámi nebo vaším systémovým integrátorem:
...
Máte možnost odpojit stávající integraci FarmBeats. Zrušení propojení FarmBeats by nemělo odstraňovat všechna zařízení nebo metadata senzorů vytvořená v datovém centru. Zrušení propojení provede následující akce:
...
Nastavení integrace FarmBeats můžete upravit, pokud se změní tajný kód klienta nebo připojovací řetězec. V takovém případě můžete upravit pouze následující pole:
...
Můžete zobrazit časovou známku poslední odeslané telemetrie, která se nachází na základě **odeslané telemetrie**. To je čas, kdy se úspěšně poslala nejnovější telemetrie do FarmBeats.
...
Rozhraní API obsahují technickou dokumentaci k Swagger. Další informace o rozhraních API a jejich odpovídajících požadavcích nebo odpovědích najdete v tématu [Swagger](https://aka.ms/FarmBeatsDatahubSwagger).
...
Chcete-li vytvořit žádost o REST API, zkombinujete metodu HTTP (GET, POST nebo PUT), adresu URL služby API, identifikátor URI (Uniform Resource Identifier) k prostředku, který se má dotazovat, odeslat data do, aktualizovat nebo odstranit a jednu nebo více hlaviček požadavku HTTP. Adresa URL služby API je koncový bod rozhraní API, který zadáte. Tady je příklad: https://\<yourdatahub-website-název >. azurewebsites. NET
...
- /**SensorModel**: SensorModel odpovídá metadatům snímače, jako je výrobce, typ snímače, který je buď analogový, nebo digitální, a míru snímače, například okolní teplota a tlak.
...
FarmBeats podporuje přidávání nových typů měření senzorů a jednotek. Další informace o rozhraní/ExtendedType API najdete v tématu [Swagger](https://aka.ms/FarmBeatsDatahubSwagger).
...
Pokud nemůžete přijímat data zařízení nebo telemetrie v zadané instanci FarmBeats, partner zařízení by měl poskytovat podporu a mechanismus pro řešení potíží.
...
Pokud chyba ovlivňuje metadata zařízení nebo senzoru nebo tok dat pro integraci nebo telemetrie dat v partnerském systému zařízení, měli byste obdržet oznámení. Mechanismus pro řešení všech chyb by měl být také navržen a implementován.
