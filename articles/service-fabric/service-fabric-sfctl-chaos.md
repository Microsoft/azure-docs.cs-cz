...
| --disable-move-replica-faults | Zakáže přesunutí primární a sekundární chyby přesunout. |
| --max-cluster-stabilization | Maximální dobu čekání pro všechny entity, které jsou stabilní a v dobrém stavu clusteru.  Výchozí\: 60. <br><br> Chaos spustí v iterací a na začátku každé iterace ověřuje stav clusteru entit. Během ověřování Pokud entita clusteru není stabilní a v pořádku v rámci MaxClusterStabilizationTimeoutInSeconds, Chaos vygeneruje událost ověření se nezdařilo. |
| --max-concurrent-faults | Maximální počet souběžných chyby vyvolané na iteraci. Chaos spustí v iteracích a dvě po sobě následujících iterací jsou odděleny fázi ověřování. Čím vyšší souběžnosti, mnohem vyššími vkládání chyb--nevyvolá složitější řadu stavy k odhalení chyb. Doporučujeme začínat hodnotu 2 nebo 3 a při přesouvání postupujte opatrně.  Výchozí\: 1. |
