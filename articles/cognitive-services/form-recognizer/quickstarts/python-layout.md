...
Po volání funkce **analyzovat rozložení** rozhraní API zavoláte rozhraní API **[výsledků získat analýzu rozložení](https://westus2.dev.cognitive.microsoft.com/docs/services/form-recognizer-api-v2-preview/operations/GetAnalyzeLayoutResult)** , abyste získali stav operace a extrahovaná data. Do dolní části skriptu Pythonu přidejte následující kód. Použije se hodnota ID operace v novém volání rozhraní API. Tento skript volá rozhraní API v pravidelných intervalech, dokud nebudou k dispozici výsledky. Doporučujeme interval jednoho sekundy nebo více.
...
1. Uložte skript.
1. Znovu použijte příkaz `python` ke spuštění ukázky. Například, `python form-recognizer-layout.py`.

### <a name="examine-the-response"></a>Prozkoumání odpovědi

Skript vytiskne odpovědi na konzolu, dokud se nedokončí operace **analyzovat rozložení** . Pak bude ve formátu JSON tisknout extrahovaná data. Uzel `"readResults"` obsahuje všechny řádky textu s příslušným umístěním ohraničovacího rámečku na stránce. Pole `"pageResults"` zobrazuje všechny části textu v tabulkách, z nichž každá má souřadnici řádku sloupce.

Podívejte se na následující obrázek faktury a příslušný výstup JSON. Výstup byl zkrácen pro zjednodušení.

> [!div class="mx-imgBorder"]
> ![dokumentu faktury contoso s tabulkovým](../media/contoso-invoice.png)
