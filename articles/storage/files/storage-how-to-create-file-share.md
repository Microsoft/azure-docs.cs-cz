...
K vytvoření účtu úložiště pomocí PowerShellu použijeme rutinu `New-AzStorageAccount`. Tato rutina má mnoho možností; zobrazí se pouze požadované možnosti. Další informace o rozšířených možnostech najdete v [dokumentaci k rutině`New-AzStorageAccount`](/powershell/module/az.storage/new-azstorageaccount).
...
K vytvoření účtu úložiště, který podporuje ukládání standardních sdílených složek Azure, použijeme následující příkaz. Parametr `-SkuName` se vztahuje na typ redundance, který požadujete. Pokud si přejete, aby byl účet geograficky redundantního nebo geograficky redundantního účtu úložiště, musíte odebrat i parametr `-EnableLargeFileShare`.
...
K vytvoření účtu úložiště, který podporuje ukládání prémiových sdílených složek Azure, použijeme následující příkaz. Všimněte si, že parametr `-SkuName` se změnil tak, aby zahrnoval `Premium` a požadovanou úroveň redundance místně redundantního (`LRS`). Parametr `-Kind` je `FileStorage` namísto `StorageV2`, protože soubory úrovně Premium se musí vytvořit v účtu úložiště úložiště místo účtu úložiště GPv2.
...
K vytvoření účtu úložiště pomocí Azure CLI použijeme příkaz AZ Storage Account Create. Tento příkaz má mnoho možností; zobrazí se pouze požadované možnosti. Další informace o rozšířených možnostech najdete v dokumentaci k [příkazům`az storage account create`](/cli/azure/storage/account).
...
K vytvoření účtu úložiště, který podporuje ukládání standardních sdílených složek Azure, použijeme následující příkaz. Parametr `--sku` se vztahuje na typ redundance, který požadujete. Pokud si přejete, aby byl účet geograficky redundantního nebo geograficky redundantního účtu úložiště, musíte odebrat i parametr `--enable-large-file-share`.
...
K vytvoření účtu úložiště, který podporuje ukládání prémiových sdílených složek Azure, použijeme následující příkaz. Všimněte si, že parametr `--sku` se změnil tak, aby zahrnoval `Premium` a požadovanou úroveň redundance místně redundantního (`LRS`). Parametr `--kind` je `FileStorage` namísto `StorageV2`, protože soubory úrovně Premium se musí vytvořit v účtu úložiště úložiště místo účtu úložiště GPv2.
...
# <a name="powershell"></a>[PowerShell](#tab/azure-powershell)
Sdílenou složku Azure můžete vytvořit pomocí rutiny [`New-AzRmStorageShare`](/powershell/module/az.storage/New-AzRmStorageShare) . Následující příkazy PowerShellu předpokládají, že jste nastavili proměnné `$resourceGroupName` a `$storageAccountName` jak jsou definované výše v části Vytvoření účtu úložiště s Azure PowerShell. 

> [!Important]  
> Pro sdílené složky Premium parametr `-QuotaGiB` odkazuje na zřízenou velikost sdílené složky. Zřízená velikost sdílené složky je množství, které se vám bude účtovat bez ohledu na využití. Standardní sdílené složky se účtují na základě využití místo zřízené velikosti.
...
# <a name="azure-cli"></a>[Azure CLI](#tab/azure-cli)
Než budeme moct vytvořit sdílenou složku Azure pomocí Azure CLI, musíte získat klíč účtu úložiště, abyste mohli autorizovat operaci vytvoření sdílené složky pomocí. To lze provést pomocí příkazu [`az storage account keys list`](/cli/azure/storage/account/keys) :
...
Jakmile budete mít klíč účtu úložiště, můžete vytvořit sdílenou složku Azure pomocí příkazu [`az storage share create`](/cli/azure/storage/share) . 

> [!Important]  
> Pro sdílené složky Premium parametr `--quota` odkazuje na zřízenou velikost sdílené složky. Zřízená velikost sdílené složky je množství, které se vám bude účtovat bez ohledu na využití. Standardní sdílené složky se účtují na základě využití místo zřízené velikosti.
...
Tento příkaz selže, pokud je účet úložiště obsažen v rámci virtuální sítě a počítač, ze kterého tento příkaz vyvoláte, není součástí virtuální sítě. Toto omezení bodu v čase můžete obejít pomocí rutiny Azure PowerShell `New-AzRmStorageShare`, jak je popsáno výše, nebo spuštěním rozhraní příkazového řádku Azure CLI z počítače, který je součástí virtuální sítě, včetně připojení prostřednictvím sítě VPN.
