...
description: Přehled jazyka OData pro filtry, výběr a pořadí podle Azure Search dotazů.
ms.date: 06/13/2019
services: search
ms.service: search
ms.topic: conceptual
...
Výrazy OData jsou od jednoduchých až vysoce složité, ale všechny sdílejí společné prvky. Nejzákladnější části výrazu OData v Azure Search jsou:

- **Cesty k poli**, které odkazují na konkrétní pole indexu.
- **Konstanty**, které jsou literálové hodnoty určitého datového typu.

> [!NOTE]
> Terminologie v Azure Search se liší od [standardu OData](https://www.odata.org/documentation/) několika způsoby. Volání **pole** v Azure Search se nazývá **vlastnost** v OData a podobně pro **cestu pole** a **cestu k vlastnosti**. **Index** obsahující **dokumenty** v Azure Search je v OData obecně označován jako **sada entit** obsahující **entity**. V rámci tohoto odkazu se používá terminologie Azure Search.
...
> [Diagram syntaxe OData pro Azure Search](https://azuresearch.github.io/odata-syntax-diagram/#field_path)
...
| `Stores/Address/Country` | Odkazuje na `Country` dílčí pole `Address` dílčího pole komplexní kolekce v indexu. `Edm.ComplexType` je typu `Collection(Edm.ComplexType)` a`Address` je v tomto příkladu typu `Stores` |
...
| `store/Address/Country` | Odkazuje na `Country` dílčí pole `Address` dílčího pole proměnné rozsahu,napříkladvevýrazufiltru.`store``Stores/any(store: store/Address/Country eq 'Canada')` |
...
Vezměte v úvahu cestu `Address/City`k poli. Ve filtru to odkazuje na jedno město pro aktuální dokument, jako je například "San Francisco". Naopak `Rooms/Type` odkazuje`Type` na dílčí pole pro mnoho místností (například "Standard" pro první místnost, "Deluxe" pro druhou místnost atd.). Vzhledem `Rooms/Type` k tomu, že neodkazuje na *jednu instanci* dílčího pole `Type`, nelze ji použít přímo ve filtru. Místo toho byste měli použít [výraz lambda](search-query-odata-collection-operators.md) s proměnnou rozsahu, jako je například:
...
Cesty polí se používají v mnoha parametrech [rozhraní Azure Search API](https://docs.microsoft.com/rest/api/searchservice/). V následující tabulce jsou uvedena všechna místa, kde je lze použít, a veškerá omezení jejich používání:
...
| [Search](https://docs.microsoft.com/rest/api/searchservice/search-documents) | `search`Kdy `queryType` je`full` | Lze odkazovat pouze na pole s **možností prohledávání** . |
...
Konstanty v OData jsou literálové hodnoty daného typu [model EDM (Entity Data Model)](https://docs.microsoft.com/dotnet/framework/data/adonet/entity-data-model) (EDM). Seznam podporovaných typů v Azure Search najdete v tématu [podporované datové typy](https://docs.microsoft.com/rest/api/searchservice/supported-data-types) . Konstanty typů kolekcí nejsou podporovány.

V následující tabulce jsou uvedeny příklady konstant pro každý datový typ podporovaný Azure Search:
...
Následující EBNF ([Rozšířený Backus – formulář Naur](https://en.wikipedia.org/wiki/Extended_Backus–Naur_form)) definuje gramatiku pro většinu konstant zobrazených ve výše uvedené tabulce. Gramatika pro geografické typy lze nalézt v [geograficky funkčních funkcích OData v Azure Search](search-query-odata-geo-spatial-functions.md).
...
> [!div class="nextstepaction"]
> [Diagram syntaxe OData pro Azure Search](https://azuresearch.github.io/odata-syntax-diagram/#constant)
...
Cesty polí a konstanty jsou nejzákladnější částí výrazu OData, ale jsou již plnými výrazy samotné. Ve skutečnosti je parametr **$Select** v Azure Search Nothing, ale čárkami oddělený seznam cest polí a **$OrderBy** není mnohem složitější než **$Select**. Pokud budete mít pole typu `Edm.Boolean` v indexu, můžete dokonce napsat filtr, který není Nothing, ale cesta k tomuto poli. Konstanty `true` a `false` jsou stejně platnými filtry.
...
> [!div class="nextstepaction"]
> [Diagram syntaxe OData pro Azure Search](https://azuresearch.github.io/odata-syntax-diagram/#filter_expression)

> [!NOTE]
> Kompletní EBNF najdete v referenčních informacích k [syntaxi výrazu OData pro Azure Search](search-query-odata-syntax-reference.md) .

Parametry **$OrderBy** a **$Select** jsou čárkami oddělený seznam jednodušších výrazů. Parametr **$Filter** je logický výraz, který se skládá z jednodušších dílčích výrazů. Tyto dílčí výrazy jsou kombinovány pomocí logických operátorů [ `and`, `or`jako jsou `not`, ](search-query-odata-logical-operators.md) [ `eq` `lt` `gt`](search-query-odata-comparison-operators.md)a, operátory porovnání, například,, a tak dále, a kolekce operátory jako [ `any` a `all` ](search-query-odata-collection-operators.md).

Parametry **$Filter**, **$OrderBy**a **$Select** jsou podrobněji prozkoumání v následujících článcích:

- [Syntaxe $filter OData v Azure Search](search-query-odata-filter.md)
- [Syntaxe $orderby OData v Azure Search](search-query-odata-orderby.md)
- [Syntaxe $select OData v Azure Search](search-query-odata-select.md)
