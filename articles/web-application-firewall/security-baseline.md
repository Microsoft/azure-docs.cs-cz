...
Tato základní hodnota zabezpečení se týká pokynů z webu [Azure Security test 1,0](../security/benchmarks/overview-v1.md) na bránu firewall webových aplikací Azure. Azure Security Benchmark poskytuje doporučení, jak můžete zabezpečit svá cloudová řešení v Azure. Obsah se seskupuje podle **ovládacích prvků zabezpečení** , které definuje srovnávací test zabezpečení Azure, a souvisejících pokynů týkajících se brány firewall webových aplikací Azure. 

> [!NOTE]
> **Ovládací prvky** , které se nevztahují na bránu firewall webových aplikací Azure nebo pro které je odpovědnost od Microsoftu, jsou vyloučené. Pokud chcete zjistit, jak se Firewall webových aplikací Azure zcela mapuje na bezpečnostní srovnávací testy zabezpečení Azure, přečtěte si **[kompletní soubor mapování standardních hodnot zabezpečení služby Firewall webových aplikací Azure](https://github.com/MicrosoftDocs/SecurityBenchmarks/raw/master/Azure%20Offer%20Security%20Baselines/1.1/azure-web-application-firewall-security-baseline-v1.1.xlsx)**.
...
- [Režimy WAF na Application Gateway](https://docs.microsoft.com/azure/web-application-firewall/ag/ag-overview#waf-modes)

- [WAF režimy na předních dveřích](https://docs.microsoft.com/azure/web-application-firewall/afds/afds-overview#waf-modes)
...
- [Skupiny pravidel a pravidla pro počítačový rezervační Firewall webových aplikací](https://docs.microsoft.com/azure/web-application-firewall/ag/application-gateway-crs-rulegroups-rules?tabs=owasp31)
...
- [Zásady WAF na front-dveří](/cli/azure/network/front-door/waf-policy)
...
- [Přehled protokolování](https://docs.microsoft.com/azure/web-application-firewall/ag/ag-overview#logging)
...
- [Konfigurace monitorování pro účet úložiště](https://docs.microsoft.com/azure/storage/common/manage-storage-analytics-logs#configure-logging)
...
- [Postup povolení nastavení diagnostiky pro protokol aktivit Azure](/azure/azure-monitor/essentials/diagnostic-settings-legacy)
...
- [Naučte se vytvářet uživatele správců v Azure Database for PostgreSQL](https://docs.microsoft.com/azure/postgresql/howto-create-users#the-server-admin-account)
...
**Doprovodné** materiály: použití pracovní stanice s privilegovaným přístupem (privilegovaným přístupem) s nakonfigurovaným vícefaktorového ověřováním, které je nakonfigurované pro přihlášení ke službě Azure Web Application firewall (WAF) a souvisejících prostředků, a jejich konfigurace.
...
- [Jaká je podmínka umístění ve službě Azure Active Directory (Azure AD) podmíněný přístup](../active-directory/reports-monitoring/quickstart-configure-named-locations.md)
...
**Pokyny**: pomocí funkcí Azure Active Directory (Azure AD) a ochrany Identity Protection můžete nakonfigurovat automatizované odezvy na zjištěné podezřelé akce týkající se identit uživatelů. Ingestujte data do Azure Sentinel pro další šetření.
...
**Pokyny**: řízení přístupu k prostředkům Azure, jako je například firewall webových aplikací, pomocí řízení přístupu na základě role Azure (RBAC).
...
- [Postup konfigurace šifrovacích klíčů spravovaných zákazníkem](https://docs.microsoft.com/azure/storage/common/customer-managed-keys-configure-key-vault?tabs=portal)
...
- [Přehled integrace mezi Application Gateway a Azure Security Center](https://docs.microsoft.com/azure/security-center/security-center-partner-integration#overview)
...
- [Jak odepřít konkrétní typ prostředku pomocí Azure Policy](/azure/governance/policy/samples/built-in-policies#general)
...
Udělení nebo odmítnutí oprávnění konkrétním uživatelům, vestavěným skupinám zabezpečení nebo skupinám definovaným v Azure Active Directory (Azure AD), pokud jsou integrované s Azure DevOps nebo Azure AD, pokud jsou integrované s Team Foundation Server (TFS).
...
- [Dokumentace k Azure Policy](/azure/governance/policy)
...
- [Použití obnovitelného odstranění Azure Key Vault](https://docs.microsoft.com/azure/key-vault/general/key-vault-recovery?tabs=azure-powershell&amp;preserve-view=true)
