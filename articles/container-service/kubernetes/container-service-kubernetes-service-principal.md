...
Tento článek ukazuje různé možnosti nastavení instančního objektu pro cluster Kubernetes. Pokud jste například nainstalovali a nastavili [Azure CLI](/cli/azure/install-az-cli2), můžete spustit příkaz [`az acs create`](/cli/azure/acs) a vytvořit současně cluster Kubernetes i instanční objekt.
...
> [!IMPORTANT]
> Abyste mohli vytvořit instanční objekt, musíte mít oprávnění k registraci aplikace v tenantu Azure AD a přiřazení aplikace k roli v předplatném. Pokud chcete zjistit, jestli máte požadovaná oprávnění, [podívejte se na portál](../../active-directory/develop/howto-create-service-principal-portal.md#permissions-required-for-registering-an-app).
>
...
Zvýrazněné jsou **ID klienta** ( `appId` ) a **tajný kód klienta** ( `password` ), který používáte jako parametry instančního objektu pro nasazení clusteru.
...
Tyto parametry můžete zadat při nasazování clusteru Kubernetes pomocí [rozhraní příkazového řádku Azure (CLI)](container-service-kubernetes-walkthrough.md), webu [Azure Portal](../dcos-swarm/container-service-deployment.md) nebo jiné metody.

>[!TIP]
>Při zadávání **ID klienta** se ujistěte, že používáte `appId` instančního objektu, a nikoli `ObjectId` instančního objektu.
>

Následující příklad ukazuje jeden ze způsobů předání parametrů pomocí Azure CLI. Tento příklad používá [šablonu Kubernetes pro rychlý start](https://github.com/Azure/azure-quickstart-templates/tree/master/101-acs-kubernetes).
...
3. Spusťte následující příkaz a možnost `--parameters` použijte k nastavení cesty k souboru azuredeploy.parameters.json. Tento příkaz nasadí cluster ve vámi vytvořené skupině prostředků s názvem `myResourceGroup` v oblasti USA – západ.
...
Pokud spustíte [`az acs create`](/cli/azure/acs#az-acs-create) příkaz pro vytvoření clusteru Kubernetes, máte možnost automaticky vygenerovat instanční objekt.
