...
Aby bylo možné odstranit data, lze pomocí příkazu HTTP DELETE provést následující volání rozhraní API. Jedná se o autorizaci pomocí hlavičky `Authorization: Bearer <arm-token>` v žádosti, kde `<arm-token>` je přístupový token AAD pro `https://management.core.windows.net/` koncový bod.  

Informace o tom, jak tento token získat a zavolat koncovým bodům Azure, najdete v [dokumentaci k azure REST API](https://docs.microsoft.com/rest/api/azure/).  
...
### <a name="delete-images"></a>Odstranit obrázky
...
Aby bylo možné exportovat data, lze pomocí příkazu HTTP GET provést následující volání rozhraní API. Jedná se o autorizaci pomocí hlavičky `Authorization: Bearer <arm-token>` v žádosti, kde `<arm-token>` je přístupový token AAD pro koncový bod `https://management.core.windows.net/`  
...
1. Přejít k Návrháři
    ...
    ![Odstranit prostředky](media/how-to-export-delete-data.md/delete-experiment.png)
