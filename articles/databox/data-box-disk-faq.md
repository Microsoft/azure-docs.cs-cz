...
Dodejte [podporovaný disk](../import-export/storage-import-export-requirements.md?toc=%2fazure%2fstorage%2fblobs%2ftoc.json#supported-disks) obsahující data pomocí [služby importu a exportu Azure](../import-export/storage-import-export-service.md) ze zdrojového umístění v Kanadě do datového centra Azure západní USA.
...
- Ujistěte se, že během procesu kopírování používáte USB 3,0 nebo novější. Stáhněte a použijte [ `USBView` Nástroj](/windows-hardware/drivers/debugger/usbview) k identifikaci řadičů USB a zařízení USB připojených k počítači.
...
### <a name="q-can-i-use-data-box-disk-to-transfer-data-to-azure-files-and-then-use-the-data-with-azure-file-sync"></a>Otázka: Můžu použít Data Box Disk k přenosu dat do souborů Azure a pak data použít s Synchronizace souborů Azure? 
A. Soubory Azure jsou podporované v Data Box Disk, ale nebudou fungovat dobře s Synchronizace souborů Azure. Metadata se nezachovají, pokud se data souboru používají s Synchronizace souborů Azure.
...
### <a name="q-i-just-noticed-that-i-didnt-follow-the-azure-naming-requirements-for-my-containers-will-my-data-fail-to-upload-to-azure"></a>Otázka: Právě jsem si všiml, že nedodržujeme požadavky na pojmenování Azure pro moje kontejnery. Znamená to, že nahrání mých dat do Azure se nezdaří?
...
A. Ano. Vaše data se nahrála do Azure, ale Pokud nevidíte žádné spravované disky se zadanými skupinami prostředků, pravděpodobně z důvodu neplatnosti těchto dat. Pokud jsou objekty blob stránky, objekty blob bloku, soubory Azure nebo spravované disky neplatné, budou přejít do následujících složek:
