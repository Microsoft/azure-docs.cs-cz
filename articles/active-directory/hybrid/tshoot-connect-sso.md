...
- Ujistěte se, že adresa URL služby`https://autologon.microsoftazuread-sso.com`Azure AD () je součástí nastavení zóny intranetu uživatele.
...
- Zajistěte `AZUREADSSOACC` , aby byl účet počítače přítomen a povolen v každé doménové struktuře služby AD, u které chcete bezproblémové jednotné přihlašování povolit. Pokud byl účet počítače odstraněný nebo chybí, můžete k jejich opětovnému vytvoření použít [rutiny PowerShellu](#manual-reset-of-the-feature) .
- Vypíše existující lístky protokolu Kerberos na zařízení pomocí `klist` příkazu z příkazového řádku. Ujistěte se, že jsou k dispozici lístky vydané pro účet `AZUREADSSOACC` počítače. Lístky protokolu Kerberos uživatelů jsou obvykle platné po dobu 10 hodin. Ve službě Active Directory můžete mít různá nastavení.
...
- Pomocí `klist purge` příkazu Vyprázdnit existující lístky protokolu Kerberos ze zařízení a zkuste to znovu.
...
2. Volání `Disable-AzureADSSOForest -OnPremCredentials $creds`. Tento příkaz odebere účet `AZUREADSSOACC` počítače z místního řadiče domény pro tuto konkrétní doménovou strukturu služby Active Directory.
