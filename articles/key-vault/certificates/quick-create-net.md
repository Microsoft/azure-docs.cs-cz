...
Začněte s klientskou knihovnou certifikátů Azure Key Vault pro .NET. [Azure Key Vault](../general/overview.md) je cloudová služba, která poskytuje zabezpečené úložiště pro certifikáty. Můžete bezpečně ukládat klíče, hesla, certifikáty a další certifikáty. Trezory klíčů Azure můžete vytvářet a spravovat přes web Azure Portal. V tomto rychlém startu se dozvíte, jak vytvářet, načítat a odstraňovat certifikáty z trezoru klíčů Azure pomocí klientské knihovny .NET.
...
V tomto příkladu můžete pro jednoduchost použít certifikát podepsaný svým držitelem s výchozími zásadami vystavování. Pro tuto úlohu použijte metodu [StartCreateCertificateAsync](/dotnet/api/azure.security.keyvault.certificates.certificateclient.startcreatecertificateasync) . První parametr metody přijímá název certifikátu a[zásady certifikátu](https://docs.microsoft.com/dotnet/api/azure.security.keyvault.certificates.certificatepolicy)zásad certifikátů.
