---
title: 'Rychlý start: Překlad textu pomocí Pythonu – Translator Text API'
...
Vytvořte nový projekt v jazyce Python v oblíbeném integrovaném vývojovém prostředí nebo editoru. Pak do svého projektu, do souboru s názvem `translate-text.py`, zkopírujte tento fragment kódu.
...
V současné době je pro službu Translator Text dostupný jeden koncový bod, který je nastavený jako `base_url`. `path` nastaví trasu `translate` a určuje, že chceme cílit na rozhraní API verze 3.
