...
>[!NOTE]
>[Dotazy na více prostředků](../logs/cross-workspace-query.md) v upozorněních protokolu jsou podporovány pouze v aktuálním [rozhraní scheduledQueryRules API](/rest/api/monitor/scheduledqueryrules). Pokud používáte starší rozhraní API pro výstrahy Log Analytics, budete muset [Přepnout na aktuální rozhraní API](../alerts/alerts-log-api-switch.md). [Viz příklady šablon](../alerts/alerts-log-create-templates.md).
...
K zobrazení podrobných informací o aplikacích Application Insights použijte [hledání v protokolu](../logs/log-query-overview.md) .
