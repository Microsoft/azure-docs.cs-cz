---
title: 'Rychlý Start: použití jazyků Java a JDBC s Azure Database for PostgreSQLm flexibilním serverem'
description: V tomto rychlém startu se naučíte používat Java a JDBC s Azure Database for PostgreSQLm flexibilním serverem.
author: mksuni
ms.author: sumuth
...
Toto téma ukazuje, jak vytvořit ukázkovou aplikaci, která používá Java a [JDBC](https://en.wikipedia.org/wiki/Java_Database_Connectivity) k ukládání a načítání informací v [Azure Database for PostgreSQL flexibilním serveru](./index.yml).
...
> [!NOTE]
> Podrobnější informace o vytváření serverů PostgreSQL najdete v tématu [vytvoření serveru Azure Database for PostgreSQL pomocí Azure Portal](./quickstart-create-server-portal.md).
...
Server PostgreSQL, který jste vytvořili dříve, má prázdnou databázi **Postgres** . V tomto rychlém startu vytvoříme novou databázi nazvanou `demo` následující kroky:

1. Nainstalujte místní instanci [psql](https://www.postgresql.org/docs/current/static/app-psql.html) pro připojení k serveru Azure PostgreSQL. 
2. Spusťte následující příkaz psql pro připojení k databázi **Postgres** na serveru PostgreSQL.
...
3. Vytvořte novou databázi s názvem **demo** na příkazovém řádku zadáním následujícího příkazu:
