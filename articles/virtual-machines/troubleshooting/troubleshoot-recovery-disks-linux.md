...
1. SSH k vašemu VIRTUÁLNÍmu počítači pro řešení potíží pomocí příslušných přihlašovacích údajů. Pokud je tento disk prvním datovým diskem připojeným k vašemu VIRTUÁLNÍmu počítači pro řešení potíží, `/dev/sdc`disk je nejspíš připojený k. Použijte `dmesg` k zobrazení připojených disků:
