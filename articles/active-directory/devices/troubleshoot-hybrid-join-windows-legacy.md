...
- Pro zákazníky s federované doménami, pokud je spojovací bod služby (SCP) nakonfigurovaný tak, aby odkazoval na název spravované domény (například contoso.onmicrosoft.com místo contoso.com), pak se připojení hybridní služby Azure AD pro zařízení se starší verzí systému Windows nefunguje.
...
- Nesprávně nakonfigurované AD FS nebo Azure AD nebo síťové problémy
   ...
   - Vaše organizace používá bezproblémové jednotné přihlašování v Azure AD, `https://autologon.microsoftazuread-sso.com` nebo `https://aadg.windows.net.nsatc.net` nejsou k dispozici v nastavení intranetu v zařízení Internet Explorer a možnost **Povolit aktualizace stavového řádku prostřednictvím skriptu** není povolená pro zónu intranetu.
