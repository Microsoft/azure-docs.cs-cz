---
title: Webová aplikace Podrobný plán zabezpečení a dodržování předpisů Azure-IaaS pro PCI DSS
description: Webová aplikace Podrobný plán zabezpečení a dodržování předpisů Azure-IaaS pro PCI DSS
services: security
author: meladie
...
ms.locfileid: "71259683"
---
# <a name="azure-security-and-compliance-blueprint-iaas-web-application-for-pci-dss"></a>Podrobný plán zabezpečení a dodržování předpisů Azure: IaaS webová aplikace pro PCI DSS

## <a name="overview"></a>Přehled

Tato Podrobný plán zabezpečení a dodržování předpisů Azure poskytuje pokyny pro nasazení prostředí pro standardy zabezpečení dat v odvětví platební karty (PCI DSS 3,2), které je vhodné pro shromažďování, ukládání a načítání služby (IaaS). data držitele. Prezentuje společnou referenční architekturu a předvádí správné zpracování dat kreditních karet (včetně čísla karty, vypršení platnosti a ověřovacích dat) v zabezpečeném a kompatibilním prostředí s více vrstvami. Tento podrobný plán znázorňuje ucelené řešení pro splnění potřeb organizací, které hledají cloudový přístup k omezení zátěže a nákladů na nasazení.
...
-   [Always Encrypted sloupce](https://docs.microsoft.com/azure/sql-database/sql-database-always-encrypted-azure-key-vault) zajistí, že se citlivá data držitele v rámci databázového systému nezobrazí v podobě prostého textu. Po povolení šifrování dat budou mít přístup k datům ve formátu prostého textu jenom klientské aplikace nebo aplikační servery s přístupem ke klíčům.
- Funkci [rozšířených vlastností](https://docs.microsoft.com/sql/relational-databases/system-stored-procedures/sp-addextendedproperty-transact-sql) lze použít k vypnutí zpracování subjektů údajů, protože umožňuje uživatelům přidat vlastní vlastnosti do databázových objektů a označit data jako "ukončené", aby podporovaly aplikační logiku, aby nedocházelo ke zpracování přidružených data držitele.
...
- [SQL Database dynamické maskování dat](https://docs.microsoft.com/azure/sql-database/sql-database-dynamic-data-masking-get-started) omezuje vystavení citlivých dat držitele na základě maskování dat pro uživatele nebo aplikace bez oprávnění. Dynamické maskování dat může automaticky zjišťovat potenciálně citlivá data a navrhovat vhodné masky pro použití. Pomůže vám identifikovat a snížit přístup k datům držitele, aby nedošlo k ukončení databáze prostřednictvím neoprávněného přístupu. Zákazníci zodpovídají za úpravu nastavení dynamického maskování dat, aby odpovídaly schématu databáze.
...
Diagram toku dat (DFD) pro tuto referenční architekturu je k dispozici ke [stažení](https://aka.ms/pcidss-iaaswa-tm) nebo najdete níže. Tento model může zákazníkům usnadnit pochopení bodů potenciálního rizika v systémové infrastruktuře při provádění úprav.

![IaaS webová aplikace pro diagram referenční architektury PCI DSS](images/pcidss-iaaswa-threat-model.png "IaaS webová aplikace pro diagram referenční architektury PCI DSS")

## <a name="compliance-documentation"></a>Dokumentace k dodržování předpisů

V [matici podrobný plán zabezpečení a dodržování předpisů Azure-PCI DSS zodpovědnosti zákazníka](https://aka.ms/pcidss-crm) najdete seznam povinností pro všechny požadavky PCI DSS 3,2.

[IaaS-PCI DSS implementace webové aplikace](https://aka.ms/pcidss-iaaswa-cim) obsahuje informace o tom, které požadavky PCI DSS 3,2 jsou řešeny architekturou webové aplikace IaaS, včetně podrobných popisů toho, jak podrobný plán zabezpečení a dodržování předpisů Azure implementace splňuje požadavky každého zahrnutého článku.
