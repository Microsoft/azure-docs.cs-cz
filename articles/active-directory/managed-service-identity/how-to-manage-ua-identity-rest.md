...
K vytvoření spravované identity přiřazené uživateli, použijte následující požadavek CURL k rozhraní API Azure Resource Manageru. Minimálně je třeba přiřadit svůj účet [Přispěvatel spravovaných identit](/azure/role-based-access-control/built-in-roles#managed-identity-contributor) role pro vytvoření identity přiřazené uživateli. Nahradit `<SUBSCRIPTION ID>`, `<RESOURCE GROUP>`, `<USER ASSIGNED IDENTITY NAME>`,`<LOCATION>`, a `<ACCESS TOKEN>` hodnoty vlastními hodnotami:
...
Seznam spravovaných identit přiřazených uživateli, použijte následující požadavek CURL k rozhraní API Azure Resource Manageru. Minimálně je třeba přiřadit svůj účet [operátor spravovaných identit](/azure/role-based-access-control/built-in-roles#managed-identity-operator) role má seznam vlastností identity přiřazené uživateli. Nahradit `<SUBSCRIPTION ID>`, `<RESOURCE GROUP>`, a `<ACCESS TOKEN>` hodnoty vlastními hodnotami:
...
Pokud chcete odstranit spravované identity přiřazené uživateli, použijte následující požadavek CURL k rozhraní API Azure Resource Manageru. Minimálně je třeba přiřadit svůj účet [Přispěvatel spravovaných identit](/azure/role-based-access-control/built-in-roles#managed-identity-contributor) role k odstranění identity přiřazené uživateli. Nahradit `<SUBSCRIPTION ID>`, `<RESOURCE GROUP>`, a `<ACCESS TOKEN>` parametry hodnoty vlastními hodnotami:
