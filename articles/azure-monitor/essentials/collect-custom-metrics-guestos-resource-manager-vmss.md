...
Pomocí [rozšíření Azure monitor Windows Azure Diagnostics (WAD)](../agents/diagnostics-extension-overview.md)můžete shromažďovat metriky a protokoly z hostovaného operačního systému (hostovaného operačního systému), který běží jako součást virtuálního počítače, cloudové služby nebo clusteru Azure Service Fabric. Rozšíření může odesílat telemetrii do mnoha různých umístění uvedených v dříve propojeném článku.  
