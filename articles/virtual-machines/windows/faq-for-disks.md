---
title: Nejčastější dotazy týkající se disků virtuálních počítačů s Windows Azure IaaS
description: Nejčastější dotazy týkající se disků virtuálních počítačů s Windows azure IaaS a disků premium (spravované a nespravované).
author: roygara
ms.service: virtual-machines-windows
ms.workload: infrastructure
ms.topic: conceptual
ms.date: 08/15/2019
ms.author: rogarana
ms.subservice: disks
ms.openlocfilehash: 1b03037d65597314d910a899d43b6f395b97438b
ms.sourcegitcommit: af1cbaaa4f0faa53f91fbde4d6009ffb7662f7eb
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 04/22/2020
ms.locfileid: "81869651"
---
# <a name="frequently-asked-questions-about-azure-iaas-vm-disks-and-managed-and-unmanaged-premium-disks"></a>Nejčastější dotazy týkající se disků virtuálních počítačů Azure IaaS a spravovaných a nespravovaných disků Premium
[!INCLUDE [virtual-machines-faq-for-disks](../../../includes/virtual-machines-faq-for-disks.md)]
