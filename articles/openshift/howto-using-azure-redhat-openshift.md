...
- Aplikace AAD clusteru (ID klienta a tajný klíč) a instanční objekt nebo dostatečná oprávnění služby AAD pro `az aro create` vytvoření aplikace AAD a instančního objektu pro vás automaticky.

- Instanční objekt a instanční objekt služby RP musí mít roli Přispěvatel ve virtuální síti clusteru.  Pokud máte ve virtuální síti roli správce přístupu uživatele, `az aro create` nastavení přiřazení rolí se nastaví automaticky.
...
> [!Note] 
> `az aro` Rozšíření je aktuální ve verzi Preview. Může být v budoucí verzi změněno nebo odebráno.
> Pokud se chcete přihlásit k `az aro` rozšíření Preview, musíte zaregistrovat poskytovatele `Microsoft.RedHatOpenShift` prostředků.
> 
>    ```console
...
2. Spusťte následující příkaz pro instalaci `az aro` rozšíření:
...
Tajný klíč pro stažení Red Hat umožňuje vašemu clusteru přístup k registrům kontejnerů Red Hat a dalšímu obsahu. Použití tajného klíče pro vyžádání obsahu je volitelné, ale doporučuje se.

Získání tajného kódu pro vyžádání obsahu:

1. Přejděte do části https://cloud.redhat.com/openshift/install/azure/aro-provisioned (Soubor > Nový > Jiné).
1. Přihlaste se ke svému účtu Red Hat nebo vytvořte nový účet Red Hat pomocí vašeho podnikového e-mailu. přijměte podmínky a ujednání.
1. Vyberte **Stáhnout tajný klíč pro vyžádání**.

Uložte soubor *Pull-Secret. txt* někam do trezoru. Tento soubor budete používat při každém vytvoření clusteru.
...
K tomuto clusteru se můžete přihlásit pomocí `kubeadmin` uživatele.  Pro vyhledání hesla pro `kubeadmin` uživatele spusťte následující příkaz:
