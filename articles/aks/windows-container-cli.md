...
Pokud je stav registrace `Registered`, ukončete monitorování stavu stisknutím kombinace kláves CTRL + C.  Potom aktualizujte registraci poskytovatele prostředků *Microsoft. ContainerService* pomocí příkazu [AZ Provider Register][az-provider-register] :
...
Ve výchozím nastavení se cluster AKS vytvoří s fondem uzlů, který může spouštět kontejnery Linux. Pomocí příkazu `az aks nodepool add` přidejte další fond uzlů, který může používat kontejnery Windows serveru.
...
Vytvořte soubor s názvem `sample.yaml` a zkopírujte následující definici YAML. Pokud Azure Cloud Shell použijete, můžete tento soubor vytvořit pomocí `vi` nebo `nano` jako při práci na virtuálním nebo fyzickém systému:
