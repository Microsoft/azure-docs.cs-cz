...
Po nahrání dat do Azure v datovém centru se `_error.xml` a soubory `_verbose.xml` vygenerují pro každý účet úložiště. Tyto protokoly se nahrají do stejného účtu úložiště, který jste použili k nahrání dat. 
...
Pro stejné pořadí je ukázka `_error.xml` uvedena níže.
...
Soubor s chybou v tomto případě obsahuje oddíl `Summary` a další část, která obsahuje všechny chyby na úrovni souborů. 
...
|`MovedToOverflowShare` |Nahrálo se soubory do nové sdílené složky, protože původní velikost sdílené složky překročila maximální limit velikosti Azure. Název nového sdílení souborů má příponu `-2`.   |
...
|`ContainerRenamed` |Kontejner pro tyto soubory neodpovídal zásadám vytváření názvů Azure a je přejmenován. Nový název začíná na `databox-` a má příponu s hodnotou hash SHA1 původního názvu. |
|`ShareRenamed` |Sdílená složka pro tyto soubory nesplňovala zásady vytváření názvů Azure a je přejmenovaná. Nový název začíná na `databox-` a má příponu s hodnotou hash SHA1 původního názvu. |
|`BlobRenamed` |Tyto soubory nesplňovaly zásady vytváření názvů Azure a přejmenovaly se. V poli `BlobPath` vyhledejte nový název. |
|`FileRenamed` |Tyto soubory nesplňovaly zásady vytváření názvů Azure a přejmenovaly se. V poli `FileStoragePath` vyhledejte nový název. |
|`DiskRenamed` |Tyto soubory nesplňovaly zásady vytváření názvů Azure a přejmenovaly se. V poli `BlobPath` vyhledejte nový název. |
