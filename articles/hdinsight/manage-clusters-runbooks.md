...
1. Vyberte **Vytvořit**.
1. Stejný postup opakujte pro nové přihlašovací údaje `ssh-password` s uživatelským `sshuser` a heslem podle vlastního výběru. Vyberte **Vytvořit**. Tento přihlašovací údaj slouží k uložení hesla SSH pro váš cluster.
...
Odstraňte cluster výběrem `hdinsight-cluster-delete` sadu Runbook, kterou jste vytvořili. Vyberte **Start**, zadejte parametr **název_clusteru** a sselect **OK**.
