...
Poznamenejte si hodnotu v části **obory** pro obor `demo.read`, který se má použít v pozdějším kroku při konfiguraci desktopové aplikace. Úplná hodnota oboru je podobná `https://contosob2c.onmicrosoft.com/api/demo.read`.
...
1. V projektu **Active-Directory-B2C-WPF** otevřete soubor *App.XAML.cs* a vyhledejte následující definice proměnných.
    1. Nahraďte hodnotu proměnné `ApiScopes` hodnotou, kterou jste si poznamenali dříve, když jste definovali obor **demo. Read** .
...
1. Aktualizujte tyto definice proměnných s následujícími hodnotami. Změňte `<web-API-application-ID>` na **ID aplikace (klienta)** webového rozhraní API, které jste zaregistrovali dříve (*webapi1*). Změňte `<your-b2c-tenant>` na název vašeho tenanta Azure AD B2C.
    ...
    ```
1. Vzhledem k tomu, že používáte rozhraní API místně, aktualizujte cestu v trase, aby metoda GET `/` místo umístění `/hello`ukázkové aplikace:
...
![Zobrazovaný název zobrazený v horním podokně desktopové aplikace WPF](media/active-directory-b2c-tutorials-desktop-app-webapi/desktop-app-01-post-api-call.png)
