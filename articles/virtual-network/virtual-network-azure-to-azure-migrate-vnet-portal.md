...
12. Můžete také změnit jiné parametry v šabloně, pokud zvolíte možnost a jsou nepovinné v závislosti na vašich požadavcích:
    ...
    * **Adresní prostor** – adresní prostor virtuální sítě je možné v šabloně změnit před uložením úpravou části**addressSpace** **prostředků** > a změnou vlastnosti **addressPrefixes** v **template. JSON.** soubor:
    ...
    * **Podsíť** – název podsítě a adresní prostor podsítě lze změnit nebo přidat k úpravou části **podsítě** souboru **template. JSON** . Název podsítě lze změnit změnou vlastnosti **název** v souboru **template. JSON** .  Adresní prostor podsítě lze změnit změnou vlastnosti **addressPrefix** v souboru **template. JSON** :
