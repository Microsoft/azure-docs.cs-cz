...
1. Vytvořte skupinu, která obsahuje prostředek `virtualip` a `azure_load_balancer` :
...
1. Přihlaste se ke každému uzlu virtuálního počítače. Pomocí následujícího příkazu otevřete soubor Hosts a nastavte překlad názvů hostitelů pro `ag1-listener` na každém počítači.
...
1. Pomocí nástroje SQLCMD se přihlaste k primárnímu uzlu SQL Server pomocí názvu naslouchacího procesu skupiny dostupnosti:
    ...
    - Použijte přihlašovací jméno, které bylo dříve vytvořeno a `<YourPassword>` nahraďte správným heslem. Následující příklad používá `sa` přihlášení, které bylo vytvořeno pomocí SQL Server.
...
1. Ověřte název serveru, ke kterému jste se připojili. V SQLCMD spusťte následující příkaz:
    ...
    Výstup by měl zobrazovat aktuální primární uzel. To by mělo `VM1` být, pokud jste nikdy neotestovali převzetí služeb při selhání.

    Ukončete relaci SQL zadáním `exit` příkazu.

### <a name="test-a-failover"></a>Testování převzetí služeb při selhání

1. Spusťte následující příkaz k ručnímu převzetí služeb při selhání `<VM2>` primární repliky do nebo jiné repliky. Nahraďte `<VM2>` hodnotou názvu vašeho serveru.
...
1. Pomocí příkazu `sudo pcs resource`zkontrolujte prostředky clusteru a měli byste vidět, že je primární instance nyní `<VM2>`.
