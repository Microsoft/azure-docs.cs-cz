...
Než budeme pokračovat, přečtěte si o [základních konceptech trezoru klíčů](../general/basic-concepts.md).
...
Azure Key Vault můžou přihlašovací údaje ukládat bezpečně, takže nejsou ve vašem kódu, ale pokud je chcete načíst, abyste je mohli Azure Key Vault, budete je muset ověřit. K ověření Key Vault ale potřebujete přihlašovací údaje. Jedná se o klasický problém při zavádění. Pomocí Azure a Azure Active Directory (Azure AD) může Identita spravované služby (MSI) poskytovat identitu pro spuštění, která výrazně zjednodušuje zprovoznění vašich věcí.
...
Následující příklad vytvoří virtuální počítač **myVM** a přidá uživatelský účet **azureuser**. `--generate-ssh-keys` Parametr US použil k automatickému vygenerování klíče SSH a umístí ho do výchozího umístění klíče (**~/.ssh**). Pokud místo toho chcete použít konkrétní sadu klíčů, použijte možnost `--ssh-key-value`.
...
Poznamenejte si `publicIpAddress` ve výstupu svého virtuálního počítače. Tuto adresu použijete pro přístup k virtuálnímu počítači v pozdějších krocích.
