...
- Jeden nebo více virtuálních počítačů připojených k doméně [v Azure se spuštěným SQL Server 2016 (nebo novější) Enterprise Edition](./create-sql-vm-portal.md) ve *stejné* skupině dostupnosti nebo v *různých* zónách dostupnosti, které byly [zaregistrovány s rozšířením agenta SQL IaaS v režimu úplné správy](sql-agent-extension-manually-register-single-vm.md) a používají stejný účet domény pro službu SQL Server na každém virtuálním počítači.
...
Nakonfigurujte cluster pomocí Azure Portal. Můžete buď vytvořit nový cluster, nebo pokud už máte nějaký cluster, můžete ho připojit k rozšíření agenta SQL IaaS pro správu portálu.
...
1. Vyberte SQL Server virtuálních počítačů, které chcete přidat do clusteru. Všimněte si, zda je vyžadováno restartování, a postupujte opatrně. Budou viditelné pouze virtuální počítače, které jsou v režimu úplné správy registrovány s rozšířením agenta SQL IaaS a jsou ve stejném umístění, doméně a ve stejné virtuální síti jako primární virtuální počítač SQL Server. 
...
Odeberte všechny SQL Server virtuálních počítačů z clusteru, abyste je zničili, a pak odeberte metadata clusteru z rozšíření agenta SQL IaaS. Můžete to provést pomocí nejnovější verze rozhraní příkazového [řádku Azure CLI](/cli/azure/install-azure-cli) nebo PowerShellu. 
