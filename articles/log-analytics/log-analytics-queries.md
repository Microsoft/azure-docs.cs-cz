...
- **Rozhraní API služby log Analytics.**  [Protokolu v log Analytics rozhraní API pro vyhledávání](log-analytics-log-search-api.md) umožňuje jakéhokoli klienta REST API k načtení dat protokolu z pracovního prostoru.  Žádosti rozhraní API obsahuje dotaz, který je spuštěn proti Log Analytics k určení dat k načtení.
...
- Další informace o [prohledávání protokolů portály, které použijete k vytvoření a úprava](log-analytics-log-search-portals.md).
- Podívejte se [kurz o psaní dotazů](log-analytics-tutorial-viewdata.md) pomocí nového dotazovacího jazyka.
