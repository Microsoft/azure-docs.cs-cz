...
Přidejte tyto řádky do `Transliterate` třídy. Nejdřív se klíč předplatného a koncový bod čtou z proměnných prostředí. Pak si všimněte, že spolu s `api-version`, byly do nástroje připojeny dva další parametry. `url` Tyto parametry slouží k nastavení vstupního jazyka a ke skriptům pro předávání. V této ukázce je nastavená na japonštinu (`jpan`) a latinku`latn`(). 
