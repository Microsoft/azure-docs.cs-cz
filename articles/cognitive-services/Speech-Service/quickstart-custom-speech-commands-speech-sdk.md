...
- Povinné `using` příkazy pro `Speech` obory `Speech.Dialog` názvů a
...
1. Do těla `ListenButton_ButtonClicked` metody ve `MainPage` třídě přidejte následující fragment kódu.
