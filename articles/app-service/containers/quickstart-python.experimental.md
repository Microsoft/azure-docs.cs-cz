...
Ve složce *Python-docs-Hello-World* , která obsahuje vzorový kód, spusťte následující `az webapp up` příkaz. Nahraďte `<app-name>` globálně jedinečným názvem aplikace (*platné znaky jsou `a-z`, `0-9`a `-` *). Nahraďte `<location-name>` také oblastí Azure, například **centralus**, **eastasia**, **westeurope**, **koreasouth**, **brazilsouth**, **centralindia**a tak dále. (Seznam povolených oblastí pro váš účet Azure můžete načíst spuštěním [`az account locations-list`](/cli/azure/appservice?view=azure-cli-latest.md#az-appservice-list-locations) příkazu.)
...
Ve svém oblíbeném editoru kódu otevřete *Application.py* a změňte `return` příkaz na posledním řádku tak, aby odpovídal následujícímu kódu. Tento `print` příkaz je zde uveden, chcete-li generovat výstup protokolování, se kterým pracujete v následující části. 
...
Znovu nasaďte aplikaci pomocí následujícího `az webapp up` příkazu a použijte stejný příkaz, který jste použili k nasazení aplikace poprvé, nahraďte `<app-name>` a `<location-name>` stejnými názvy, které jste použili dříve. 
...
Nejprve zapněte protokolování kontejneru spuštěním následujícího příkazu v terminálu, nahraďte `<app-name>` názvem vaší aplikace a `<resource-group-name>` názvem skupiny prostředků uvedeným ve výstupu `az webapp up` příkazu, který jste použili (například "appsvc_rg_Linux_centralus"):
