...
Chcete-li vytvořit záznam DNS, použijte příkaz `az network dns record-set <record-type> add-record` (kde `<record-type>` je typ záznamu, tj. a, SRV, txt atd.) Nápovědu najdete v tématu `az network dns record-set --help`.
...
Sady záznamů se vytvářejí pomocí příkazu `az network dns record-set <record-type> create`. Nápovědu získáte příkazem `az network dns record-set <record-type> create --help`.
...
Následující příklad vytvoří prázdnou sadu záznamů typu A s 60 sekundou TTL pomocí parametru `--ttl` (krátký tvar `-l`):

```azurecli
az network dns record-set a create --resource-group myresourcegroup --zone-name contoso.com --name www --ttl 60
```

Následující příklad vytvoří sadu záznamů se dvěma položkami metadat, "Odděl = finance" a "prostředí = produkce", a to pomocí parametru `--metadata`:
...
Při vytváření [sady záznamů SRV](dns-zones-records.md#srv-records)zadejte v názvu sady záznamů *službu\_* a *protokol\_* . Při vytváření sady záznamů SRV v vrcholu zóny není nutné do názvu sady záznamů zahrnout "\@".
...
Pomocí příkazu `az network dns record-set list` můžete zobrazit seznam všech záznamů v zóně DNS. Nápovědu získáte příkazem `az network dns record-set list --help`.
...
`az network dns record-set <record-type> add-record` můžete použít k vytvoření záznamu v nové sadě záznamů nebo k přidání záznamu do existující sady záznamů.
...
Tento příkaz odstraní ze sady záznamů záznam DNS. Pokud se odstraní poslední záznam v sadě záznamů, odstraní se i samotná sada záznamů. Chcete-li místo toho ponechat prázdnou sadu záznamů, použijte možnost `--keep-empty-record-set`.
...
Na vrcholu zóny (`--Name "@"`, včetně uvozovek) nemůžete přidávat, odebírat ani upravovat záznamy v sadě záznamů o automatickém vytváření NS. V případě této sady záznamů jsou povoleny pouze změny, kterými je upravena hodnota TTL a metadata sady záznamů.
...
Sady záznamů lze odstranit pomocí příkazu `az network dns record-set <record-type> delete`. Nápovědu získáte příkazem `azure network dns record-set <record-type> delete --help`. Odstraněním sady záznamů dojde také k odstranění všech záznamů v rámci sady záznamů.

> [!NOTE]
> Sady záznamů SOA a NS nelze odstranit ve vrcholu zóny (`--name "@"`).  Ty se vytvoří automaticky při vytvoření zóny a automaticky se odstraní, když se zóna odstraní.
...
Zobrazí se výzva k potvrzení operace odstranění. Chcete-li potlačit tuto výzvu, použijte přepínač `--yes`.
