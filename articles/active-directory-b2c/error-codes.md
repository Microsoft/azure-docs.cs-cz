...
| `AADB2C90006` | Identifikátor URI přesměrování{0}zadaný v požadavku není registrovaný pro ID klienta{1}. |
...
| `AADB2C90011` | ID klienta{0}zadané v žádosti neodpovídá ID klienta zaregistrovanému v zásadě{1}. |
...
| `AADB2C90016` | Požadovaný typ kontrolního výrazu klienta{0}se neshoduje s očekávaným typem{1}. |
...
| `AADB2C90023` | Profil{0}neobsahuje požadovaný klíč metadat "{1}". |
...
| `AADB2C90039` | Požadavek obsahuje kontrolní výraz klienta, ale zadané zásadě{0}v tenantovi{1}chybí client_secret v U relyingpartypolicy. |
| `AADB2C90040` | Cesta uživatele{0}neobsahuje krok odeslat deklarace. |
...
| `AADB2C90044` | Překladač deklarací identity "{1}" nepodporuje deklaraci '{0}'. |
...
| `AADB2C90080` | Platnost poskytnutého udělení vypršela. Proveďte prosím znovu ověření a zkuste to znovu. Aktuální čas: {0}, čas vystavení přidělení: {1}, udělení posuvných oken čas vypršení platnosti: {2}. |
...
| `AADB2C90092` | Poskytnutá aplikace s ID '{0}' je zakázána pro klienta '{1}'. Povolte prosím aplikaci a zkuste to znovu. |
| `AADB2C90107` | Aplikace s ID '{0}' nemůže získat token ID buď proto, že v požadavku nebyl zadán obor OpenID, nebo aplikace není pro ni autorizována. |
...
| `AADB2C90114` | Váš účet je dočasně zamčený, aby nedocházelo k neoprávněnému použití. Opakujte akci později. |
...
| `AADB2C90120` | Parametr maximálního stáří{0}zadaný v požadavku je neplatný. Maximální stáří musí být celé číslo mezi "{1}" a "{2}" včetně. |
| `AADB2C90122` | Vstup pro '{0}' přijatý v požadavku má neúspěšné ověření požadavku HTTP. Ujistěte se, že vstup neobsahuje znaky jako < nebo &. |
...
| `AADB2C90146` | Obor{0}poskytnutý v požadavku určuje více než jeden prostředek pro přístupový token, což není podporováno. |
| `AADB2C90149` | Nepovedlo se načíst skript{0}. |
...
| `AADB2C90165` | Zpráva iniciace SAML s ID{0}nebyla nalezena ve stavu. |
...
| `AADB2C90188` | Technický profil SAML '{0}' určuje PartnerEntity adresu URL '{1}', ale načtení metadat se nezdařilo z důvodu '{2}'. |
...
| `AADB2C90232` | Zadaný id_token_hint parametr neobsahuje přijatý Vystavitel. Platné vystavitele: '{0}'. Zadejte prosím jiný token a zkuste to znovu. |
...
| `AADB2C90255` | Výměna deklarací identity zadaná v profilu Technical Profile '{0}' nebyla dokončena podle očekávání. Možná budete chtít zkusit spustit relaci od začátku. |
| `AADB2C90261` | Výměna deklarací identity{0}zadaná v kroku "{1}" vrátila chybovou odpověď HTTP, kterou nelze analyzovat. |
...
| `AADB2C90276` | Požadavek není konzistentní s nastavením ovládacího prvku{0}:{1}v technicalProfile{2}pro zásady{3}tenant{4}. |
| `AADB2C90277` | Krok orchestrace{0}cesty uživatele{1}zásady{2}neobsahuje odkaz na definici obsahu. |
| `AADB2C90279` | Zadané ID klienta{0}se neshoduje s ID klienta, které vydalo udělené oprávnění. |
...
| `AADB2C99011` | Hodnota metadat{0}nebyla zadána v TechnicalProfile '{1}' v zásadě '{2}'. |
| `AADB2C99013` | Zadaná kombinace grant_type [{0}] a token_type [{1}] není podporována. |
| `AADB2C99015` | U profilu{0}v zásadě{1}v tenantovi{2}chybí všechny InputClaims vyžadované pro tok přihlašovacích údajů k heslu vlastníka prostředku. |
