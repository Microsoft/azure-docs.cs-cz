...
>[!NOTE]
>*SessionThresholdPerCPU* neomezuje počet relací na virtuálním počítači. Tento parametr určuje, zda je nutné spustit nové virtuální počítače pro vyrovnávání zatížení připojení. Pokud chcete omezit počet relací, musíte podle pokynů [Update-AzWvdHostPool](configure-host-pool-load-balancing.md#configure-breadth-first-load-balancing) nakonfigurovat parametr *MaxSessionLimit* odpovídajícím způsobem.
...
- [Fond hostitelů virtuálních počítačů s Windows](create-host-pools-azure-marketplace.md)
...
6. Po dokončení procesu se v zadaném Azure Automation účtu vytvoří Asset s názvem **AzureRunAsConnection** . Vyberte **účet Spustit jako pro Azure**. Asset připojení obsahuje ID aplikace, ID tenanta, ID předplatného a kryptografický otisk certifikátu. Na stránce **připojení** můžete také najít stejné informace. Pokud chcete přejít na tuto stránku, v podokně na levé straně okna vyberte v části **sdílené prostředky** možnost **připojení** a klikněte na prostředek připojení s názvem **AzureRunAsConnection**.

## <a name="create-the-azure-logic-app-and-execution-schedule"></a>Vytvoření aplikace logiky Azure a plánu spuštění

Nakonec budete muset vytvořit aplikaci logiky Azure a nastavit plán spouštění pro nový nástroj pro škálování. Nejdřív Stáhněte a importujte [modul PowerShell Virtualization Desktop](powershell-module.md) pro použití v relaci PowerShellu, pokud jste to ještě neudělali.
...
4. Spuštěním následujícího skriptu PowerShellu vytvořte aplikaci Azure Logic a plán spuštění pro fond hostitelů.
