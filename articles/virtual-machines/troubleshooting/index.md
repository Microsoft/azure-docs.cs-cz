---
layout: LandingPage
description: Zjistěte, jak řešit potíže s nasazením virtuálních počítačů.
title: Dokumentace k řešení potíží pro Azure Virtual Machines | Microsoft Docs
services: virtual-machines
author: genlin
manager: jeconnoc
ms.assetid: ''
ms.service: virtual-machines
ms.tgt_pltfrm: na
ms.devlang: na
ms.topic: landing-page
ms.date: 10/3/2018
ms.author: genli
ms.openlocfilehash: fadff0e3cab98f7b0aa69a6fe29c3d9ace31face
ms.sourcegitcommit: 62759a225d8fe1872b60ab0441d1c7ac809f9102
ms.translationtype: HT
ms.contentlocale: cs-CZ
ms.lasthandoff: 10/19/2018
ms.locfileid: "49465487"
---
# <a name="troubleshooting-azure-virtual-machines"></a>Řešení potíží virtuálních počítačů Azure

- Selhání přidělení
    - [ Selhání přidělení   ](allocation-failure.md)
    - [ Selhání přidělení pro klasická nasazení  ](allocation-failure-classic.md)
- [Diagnostika spouštění](boot-diagnostics.md)
- Protokol RDP
    - [ Resetování protokolu RDP  ](reset-rdp.md)
    - [ Řešení potíží s protokolem RDP  ](troubleshoot-rdp-connection.md)
    - [ Podrobné řešení potíží s protokolem RDP  ](detailed-troubleshoot-rdp.md)
    - [ Řešení konkrétních chyb  ](troubleshoot-specific-rdp-errors.md)
- SSH 
    - [ Řešení potíží s protokolem SSH  ](troubleshoot-ssh-connection.md)
    - [ Podrobné řešení potíží s protokolem SSH  ](detailed-troubleshoot-ssh-connection.md)
    - [Běžné chybové zprávy](error-messages.md)
    - [Problémy s výkonem virtuálních počítačů Windows](performance-diagnostics.md  )
    - [ Použití PerfInsights  ](how-to-use-perfInsights.md)
    - [ Rozšíření Diagnostika výkonu  ](performance-diagnostics-vm-extension.md)
- [Offline instalace agenta virtuálního počítače s Windows](install-vm-agent-offline.md)
- Znovunasazení virtuálního počítače
    - [ Linux  ](redeploy-to-new-node-linux.md)
    - [ Windows  ](redeploy-to-new-node-windows.md)
- Resetování hesla pro virtuální počítač
    - [ Windows  ](reset-local-password-without-agent.md)
    - [ Linux  ](reset-password.md)
- [Resetování síťové karty](reset-network-interface.md)
- [Restartování nebo změna velikosti virtuálního počítače](restart-resize-error-troubleshooting.md)
- Použití sériové konzoly
    - [Virtuální počítač s Linuxem](serial-console-linux.md)
        - [Sériová konzola – GRUB / režim jednoho uživatele](serial-console-grub-single-user-mode.md)
        - [Sériová konzola – NMI/SysRq](serial-console-nmi-sysrq.md)
    - [Virtuální počítač s Windows](serial-console-windows.md)
        - [Příkazy PowerShellu a CMD](serial-console-cmd-ps-commands.md)
- [Chyby při odstraňování prostředků úložiště](storage-resource-deletion-errors.md      )
- [Neočekávaná restartování virtuálních počítačů s připojenými virtuálními pevnými disky  ](unexpected-reboots-attached-vhds.md)
- [Problémy s aktivací Windows](troubleshoot-activation-problems.md)
- [Potíže s přístupem k aplikacím](troubleshoot-app-connection.md)
- Řešení potíží s nasazeními
    - [Linux  ](troubleshoot-deploy-vm-linux.md)
    - [Windows  ](troubleshoot-deploy-vm-windows.md)
- [Změna názvů zařízení](troubleshoot-device-names-problems.md)
- Přístup k obnovení virtuálního počítače
    - Windows
        - [PowerShell](troubleshoot-recovery-disks-windows.md)
        - [Azure Portal](troubleshoot-recovery-disks-portal-windows.md)
    - Linux
        - [Rozhraní příkazového řádku](troubleshoot-recovery-disks-linux.md)
    - [Azure Portal](troubleshoot-recovery-disks-portal-linux.md)
- [Chyby spouštění](boot-error-troubleshoot.md)
- [Chyby nástroje BitLocker](troubleshoot-bitlocker-boot-error.md)
- [Kontrola chyb systému souborů  ](troubleshoot-check-disk-boot-error.md)
- [Chyby typu modrá obrazovka  ](troubleshoot-common-blue-screen-error.md)
- [Chyby způsobené omezením provozu](troubleshooting-throttling-errors.md)
- [Použití vnořené virtualizace](troubleshoot-vm-by-use-nested-virtualization.md)
- [Vysvětlení restartování systému](understand-vm-reboot.md)

