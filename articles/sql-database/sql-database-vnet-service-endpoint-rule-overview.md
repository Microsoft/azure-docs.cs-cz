...
*Error description:* The client is in a subnet that has virtual network server endpoints. Azure SQL Database ale nemá žádné pravidlo virtuální sítě, které dává podsíti oprávnění ke komunikaci s SQL Database.
...
*Error description:* The client is trying to connect from an IP address that is not authorized to connect to the Azure SQL Database server. Brána firewall serveru neobsahuje žádné pravidlo IP adres, které by klientovi umožňovalo komunikovat se službou SQL Database z dané IP adresy.

*Error resolution:* Enter the client's IP address as an IP rule. To provedete v podokně Brána firewall na webu Azure Portal.
