...
Naslouchací procesy, které se připojují k Hybrid Connections s podporou `request` protokolu HTTP, musí zpracovat gesto. Naslouchací proces, který nezpracovává `request` , a proto způsobuje opakované chyby vypršení časového limitu při připojení, může služba v budoucnu zakázaná.
...
3. Jenom v případě, že je zapnutá autorizace přenosu, a pokud hlavičky žádosti `Authorization` obsahují hlavičku a není k dispozici žádný z předchozích výrazů, bude tato hlavička vyhodnocena a odstraněna. `Authorization`V opačném případě se vždy předává jako.
...
##### <a name="rejecting-the-socket"></a>Zamítnutí soketu
 ...
 Aby byl soket zamítnut, klient převezme ze `accept` zprávy identifikátor URI adresy a připojí dva parametry řetězce dotazu k tomuto objektu, a to následujícím způsobem:
...
Skládá `request` se ze dvou částí: záhlaví a binárních rámců (ů).
Pokud není k dispozici žádné tělo, budou rámce textu vynechány. Indikátor, zda je text přítomen, je vlastnost Boolean `body` ve zprávě požadavku.
...
* řetězec **adresy** URI. Toto je adresa Rendezvous, která se má použít pro tento požadavek. Pokud je příchozí požadavek větší než 64 kB, zbývající část této zprávy je ponecháno prázdné a klient musí iniciovat hodnotu Rendezvous handshake odpovídající `accept` operaci popsanou níže. Služba pak na zavedeném webovém `request` soketu vloží dokončeno. Pokud je možné očekávat odpověď maximálně 64 kB, naslouchací proces musí taky iniciovat hodnotu Rendezvous handshake a následně přenést odpověď na zavedený webový soket.
* **ID** – řetězec. Jedinečný identifikátor pro tento požadavek.
* **requestHeaders hostitele** – tento objekt obsahuje všechny hlavičky HTTP, které byly dodány do koncového bodu odesílatelem, s výjimkou informací o autorizaci, jak je vysvětleno [výše](#request-operation), a záhlavími, které se striktně vztahují k připojení s bránou. Konkrétně jsou všechny hlavičky definované nebo vyhrazené v [RFC7230](https://tools.ietf.org/html/rfc7230), s `Via`výjimkou, jsou odstraněny a nepředávány:
...
* **requestTarget** – řetězec. Tato vlastnost obsahuje ["cíl žádosti" (RFC7230, část 5,3)](https://tools.ietf.org/html/rfc7230#section-5.3) žádosti. To zahrnuje část řetězce dotazu, která je odstraněna ze všech `sb-hc-` předpevněných parametrů.
...
Odpovědi mohou být zasílány v libovolném pořadí, ale každý požadavek musí odpovídat do 60 sekund nebo doručení bude Hlášeno jako neúspěšné. Konečný termín 60-Second se počítá až do `response` chvíle, kdy služba nepřijala rámec. Probíhající odpověď s více binárními snímky nemůže být nečinná po dobu delší než 60 sekund nebo se ukončí.
...
* **RequestId** – řetězec Požadovanou. Hodnota `id` vlastnosti `request` zprávy, na kterou reaguje.
...
Pro odpovědi, které překračují 64 kB, musí být odpověď doručena přes soket Rendezvous. Také Pokud požadavek překročí 64 kB a obsahuje `request` pouze pole adresa, je nutné vytvořit soket Rendezvous, který získá. `request` Po navázání typu Rendezvous Socket se musí odpovědi na příslušného klienta a následné požadavky z tohoto klienta doručovat přes soket Rendezvous, a to i v případě, že přetrvává.
...
Požadavek může obsahovat libovolné dodatečné hlavičky protokolu HTTP, včetně aplikací definovaných aplikací. Veškerý dodaný hlavičkový tok do naslouchacího procesu, který lze najít `connectHeader` v objektu zprávy o **přijetí** kontrolního programu.

Možnosti parametru řetězce dotazu jsou následující:

| Param          | Povinné? | Popis
| -------------- | --------- | -------------------------- |
| `sb-hc-action` | Ano       | V případě role odesílatele musí být `sb-hc-action=connect`parametr.
| `{path}`       | Ano       | (viz následující odstavec)
...
Požadavek může obsahovat libovolné dodatečné hlavičky protokolu HTTP, včetně aplikací definovaných aplikací. Všechna dodaná záhlaví, kromě těch, která jsou přímo definována v RFC7230 (viz [zpráva požadavku](#Request message)) toku k naslouchacímu procesu a `requestHeader` lze ji najít v objektu zprávy **požadavku** .
...
Token se dá také přenést buď v hlavičce, `ServiceBusAuthorization` nebo `Authorization` v hlavičce HTTP. Token se dá vynechat, pokud je hybridní připojení nakonfigurované tak, aby povolovalo anonymní požadavky.

Vzhledem k tomu, že služba efektivně funguje jako proxy, a to i v případě, že ne jako skutečný proxy `Via` Server http, buď přidá hlavičku `Via` nebo doplní existující záhlaví kompatibilní s [RFC7230, oddíl 5.7.1](https://tools.ietf.org/html/rfc7230#section-5.7.1).
Služba přidá název hostitele oboru názvů Relay do `Via`.

| kód | Zpráva  | Popis                    |
| ---- | -------- | ------------------------------ |
| 200  | OK       | Žádost byla zpracována alespoň jedním naslouchací proces.  |
| 202  | Accepted | Požadavek přijal alespoň jeden naslouchací proces. |
