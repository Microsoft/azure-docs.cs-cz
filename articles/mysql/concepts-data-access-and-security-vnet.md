---
title: Koncové body služby Virtuální sítě – databáze Azure pro MySQL
description: Popisuje, jak koncové body služby virtuální sítě fungují pro váš azure database pro server MySQL.
author: ajlam
ms.author: andrela
ms.service: mysql
ms.topic: conceptual
...
**Koncový bod služby Virtuální síť:** [Koncový bod služby Virtuální síť][vm-virtual-network-service-endpoints-overview-649d] je podsíť, jejíž hodnoty vlastností zahrnují jeden nebo více formálních názvů typů služby Azure. V tomto článku nás zajímá název typu **Microsoft.Sql**, který odkazuje na službu Azure s názvem SQL Database. Tato značka služby platí také pro azure databáze pro služby MySQL a PostgreSQL. Je důležité si uvědomit, při použití značky služby **Microsoft.Sql** na koncový bod služby virtuální sítě bude konfigurovat provoz koncového bodu služby pro všechny Azure SQL Database, Azure Database for MySQL a Azure Database for PostgreSQL servery v podsíti. 

**Pravidlo virtuální sítě:** Pravidlo virtuální sítě pro váš server Azure Database for MySQL je podsíť, která je uvedená v seznamu řízení přístupu (ACL) vašeho serveru Azure Database for MySQL. Chcete-li být v acl pro váš server Azure Database for MySQL, podsíť musí obsahovat název typu **Microsoft.Sql.**

Pravidlo virtuální sítě říká, že databáze Azure pro server MySQL může přijímat komunikaci z každého uzlu, který je v podsíti.
...
### <a name="c-cannot-yet-have-azure-database-for-mysql-on-a-subnet-without-defining-a-service-endpoint"></a>C. Zatím nelze mít Azure Database for MySQL v podsíti bez definování koncového bodu služby

Pokud váš **Microsoft.Sql** server byl uzel v podsíti ve vaší virtuální síti, všechny uzly ve virtuální síti může komunikovat s azure databáze pro mysql server. V takovém případě vaše virtuální počítače můžete komunikovat s Azure Database pro MySQL bez nutnosti jakékoli virtuální sítě pravidla nebo pravidla IP.

Nicméně od srpna 2018 služba Azure Database for MySQL ještě není mezi službami, které lze přiřadit přímo k podsíti.
...
Každé pravidlo virtuální sítě se vztahuje na celou databázi Azure pro server MySQL, nejen na jednu konkrétní databázi na serveru. Jinými slovy pravidlo virtuální sítě platí na úrovni serveru, nikoli na úrovni databáze.
...
- **Správce databáze:** &nbsp; Aktualizujte seznam řízení přístupu (ACL) a přidejte danou podsíť do databáze Azure pro server MySQL.
...
> [!NOTE]
> V některých případech databáze Azure pro MySQL a podsítě virtuální sítě jsou v různých předplatných. V těchto případech je nutné zajistit následující konfigurace:
...
Pro Azure Database for MySQL má funkce pravidel virtuální sítě následující omezení:
...
- V bráně firewall pro databázi Azure pro MySQL odkazuje každé pravidlo virtuální sítě na podsíť. Všechny tyto odkazované podsítě musí být hostované ve stejné geografické oblasti, která je hostitelem databáze Azure pro MySQL.

- Každá databáze Azure pro mysql server může mít až 128 položek ACL pro danou virtuální síť.
...
- Zapnutí koncových bodů virtuální síťové služby do Databáze Azure pro MySQL pomocí značky služby **Microsoft.SQL** také umožňuje koncové body pro všechny služby Azure Database: Azure Database for MySQL, Azure Database for PostgreSQL, Azure SQL Database a Azure SQL Data Warehouse.
...
Chcete-li povolit komunikaci z vašeho okruhu do databáze Azure pro MySQL, musíte vytvořit pravidla sítě IP pro veřejné IP adresy vašich okruhů. Chcete-li najít veřejné IP adresy okruhu ExpressRoute, otevřete lístek podpory s ExpressRoute pomocí portálu Azure.
...
- [Vytváření a správa pravidel Azure Database for MySQL VNet pomocí portálu Azure](howto-manage-vnet-using-portal.md)
- [Vytváření a správa pravidel Azure Database for MySQL VNet pomocí rozhraní příkazového příkazového příkazu Azure](howto-manage-vnet-using-cli.md)
