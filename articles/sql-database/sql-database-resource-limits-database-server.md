...
- Pro velikosti úložiště prostředků pro izolované databáze prosím použijte omezení [prostředků založené na DTU](sql-database-dtu-resource-limits-single-databases.md) nebo [omezení prostředků na základě Vcore](sql-database-vcore-resource-limits-single-databases.md) pro limity velikosti úložiště na cenovou úroveň.
...
Když využité místo v databázi dosáhne limitu maximální velikosti, vkládání a aktualizace databáze, které zvyšují velikost dat, selžou a klienti obdrží [chybovou zprávu](troubleshoot-connectivity-issues-microsoft-azure-sql-database.md). Databáze vybere a odstraní nadále úspěšné.
