...
1. [Spusťte kontejner](#run-the-container-with-docker-run) s požadovaným nastavením fakturace. K dispozici jsou [ další ](./face-resource-container-config.md#example-docker-run-commands)příklady`docker run` příkazu. 
...
K dispozici jsou [příklady](face-resource-container-config.md#example-docker-run-commands) `docker run` příkazu.
...
K dispozici jsou [ další ](./face-resource-container-config.md#example-docker-run-commands)příklady`docker run` příkazu. 
