...
1. Nainstalujte nejnovější stabilní verze 3. x následujících balíčků NuGet:
...
V ASP.NET Core jsou konfigurace hostitelů nastaveny voláním metod v [`HostBuilder`](/dotnet/api/microsoft.extensions.hosting.hostbuilder) instanci. Další informace najdete v tématu [obecný hostitel .NET](/aspnet/core/fundamentals/host/generic-host). Metoda `ConfigureWebJobs` rozšíření inicializuje hostitele WebJobs. V `ConfigureWebJobs`nástroji můžete inicializovat specifická rozšíření WebJobs a nastavovat vlastnosti těchto rozšíření.  
...
1. Nainstalujte nejnovější stabilní verzi následujících balíčků NuGet:

   * `Microsoft.Extensions.Logging`– Protokolovací rozhraní.
   * `Microsoft.Extensions.Logging.Console`– Poskytovatel konzoly, který odesílá protokoly do konzoly.

   Tady jsou příkazy **konzoly Správce balíčků** pro verzi 2.2.0:
...
1. Zavolejte metodu na [`HostBuilder`.](/dotnet/api/microsoft.extensions.hosting.hostbuilder) [`ConfigureLogging`](/dotnet/api/microsoft.aspnetcore.hosting.webhostbuilderextensions.configurelogging) [`AddConsole`](/dotnet/api/microsoft.extensions.logging.consoleloggerextensions.addconsole) Metoda přidá protokolování konzoly do konfigurace.
...
1. Nainstalujte nejnovější stabilní verzi balíčku NuGet [Microsoft. Azure. WebJobs. Extensions. Storage](https://www.nuget.org/packages/Microsoft.Azure.WebJobs.Extensions.Storage) , verze 3. x. 
    ...
    Tady je příkaz **konzoly Správce balíčků** pro verzi 3.0.3:

    ```powershell
    Install-Package Microsoft.Azure.WebJobs.Extensions.Storage -Version 3.0.3
    ```

2. V rozšiřující metodě `AddAzureStorage` zavolejte metodu na [`HostBuilder`](/dotnet/api/microsoft.extensions.hosting.hostbuilder) instanci pro inicializaci rozšíření úložiště. `ConfigureWebJobs` V tomto okamžiku `ConfigureWebJobs` metoda vypadá jako v následujícím příkladu:
...
1. Klikněte pravým tlačítkem myši na projekt, vyberte možnost **Přidat** > **novou položku...** , zvolte možnost **Třída**, pojmenujte nový C# soubor třídy *Functions.cs*a vyberte možnost **Přidat**.
...
1. V Functions.cs nahraďte vygenerovanou šablonu následujícím kódem:
   ...
   Atribut říká modulu runtime, aby tuto funkci volal při zápisu nové zprávy ve frontě Azure Storage s názvem `queue`. `QueueTrigger` Obsah zprávy fronty je k dispozici pro kód metody v `message` parametru. Tělo metody je místo, kde se zpracovávají data triggeru. V tomto příkladu kód pouze protokoluje zprávu.
...
1. Spusťte projekt znovu.
   ...
   Vzhledem k tomu, `QueueTrigger` že jste `ProcessQueueMessage` ve funkci použili atribut, modul runtime sady WeJobs SDK při spuštění naslouchá zprávám ve frontě. Najde novou zprávu fronty ve frontě s názvem *Queue* a zavolá funkci.
...
1. Nainstalujte nejnovější stabilní verzi balíčku NuGet 3. x pro poskytovatele protokolování Application Insights: `Microsoft.Azure.WebJobs.Logging.ApplicationInsights`.
