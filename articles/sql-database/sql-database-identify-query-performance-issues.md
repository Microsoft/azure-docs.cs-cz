...
### <a name="ParamSniffing"></a>Dotazy, které mají problémy s parametrem citlivostní plán (PSP)
...
- **Vstupně-výstupní problémy**
  ...
  Dotazy můžou čekat na zápis stránek do dat nebo souborů protokolu. V takovém případě se podívejte `INSTANCE_LOG_RATE_GOVERNOR`na DMV, `WRITE_LOG`nebo `PAGEIOLATCH_*` Statistika čekání v. V tématu Použití zobrazení dynamické správy k [identifikaci potíží s výkonem v/](sql-database-monitoring-with-dmvs.md#identify-io-performance-issues)v.
