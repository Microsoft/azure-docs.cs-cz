...
Prostředí Azure Powershell můžete použít ke konfiguraci [webové přenosy přesměrování](multiple-site-overview.md) při vytváření [Aplikační brána](overview.md). V tomto kurzu definujete fondu back-end pomocí škálovací sadu virtuálních počítačů. Nakonfigurujete naslouchací procesy a pravidla založená na domény, které vlastníte a ujistěte se, že web přenos dorazí na odpovídajícímu fondu adres. V tomto kurzu se předpokládá, že vlastníte více domén a používá příklady *www.contoso.com* a *www.contoso.org*.
...
Můžete cconfigure přesměrování pro naslouchací proces pomocí [přidat AzureRmApplicationGatewayRedirectConfiguration](/powershell/module/azurerm.network/add-azurermapplicationgatewayredirectconfiguration). 
...
![Testování serveru contoso v aplikační brány](./media/redirect-internal-site-powershell/application-gateway-iistest.png)
