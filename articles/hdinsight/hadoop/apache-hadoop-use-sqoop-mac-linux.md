...
1. Použijte následující příkaz pro import dat z **mobiledata** do tabulky ve službě SQL Database **wasb: / / / kurzy/usesqoop/importeddata** adresáře v HDInsight:
    ...
    > [!IMPORTANT]  
    > `wasb:///` Cesta pracuje s clustery, které používají jako výchozí úložiště clusteru služby Azure Storage. Pro clustery, které používají Azure Data Lake Storage Gen2, použijte `abfs:///` místo. Pro clustery, které používají Azure Data Lake Storage Gen1, použijte `adl:///` místo.
