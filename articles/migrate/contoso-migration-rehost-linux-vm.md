...
Tento dokument je jednou z řady článků, které dokumentují, jak fiktivní společnosti Contoso migraci svých místních prostředků do cloudu Microsoft Azure. Obsahuje základní informace a řada scénářů, která ukazuje, jak připravit infrastrukturu pro migraci a spustit různé druhy migrace. Scénáře jejich složitost v a budeme přidávat další články v čase.
...
[Článek 5: Změna hostitele na virtuálních počítačích Azure](contoso-migration-rehost-vm.md) | Ukazuje, jak společnosti Contoso migrovat své aplikace SmartHotel virtuální počítače na virtuální počítače Azure. | K dispozici.
...
Článek 7: Změna hostitele Linuxovou aplikaci do virtuálních počítačů Azure | Ukazuje, jak společnosti Contoso migruje jejich osService Linuxové aplikace pomocí Azure Site Recovery. | Tento článek.
[Článek 8: Změna hostitele Linuxovou aplikaci na virtuálních počítačích Azure a Azure serveru MySQL](contoso-migration-rehost-linux-vm-mysql.md) | Ukazuje, jak společnosti Contoso migruje osService Linuxovou aplikaci, pomocí Site Recovery pro migraci virtuálního počítače a aplikace MySQL Workbench k migraci (na instanci serveru Azure MySQL. | K dispozici.
...
- **Rozšíření**: nebudete chtít změnit aplikaci hned teď. Jednoduše chtějí se ujistěte, že se jedná o stabilní.
...
2. Po přípravě Azure a místní komponenty, jejich nastavení a povolíte replikaci pro virtuální počítače.
3. Po funguje replikace se migrovat virtuální počítače po převzetí služeb při selhání je do Azure.
...
Tady je můžete vy (a Contoso) potřebovat pro tento scénář.
...
> * **Krok 2: Příprava VMware v místním prostředí pro Site Recovery**: Příprava účty, které má být použit pro virtuální počítač zjišťování a instalaci agenta a příprava připojení k virtuálním počítačům Azure po převzetí služeb při selhání.
> * **Krok 3: Replikace virtuálních počítačů**: nastavení zdrojového a cílového prostředí migrace, vytvoření zásady replikace a spustit replikaci virtuálních počítačů do Azure storage.
...
1. Contoso se vytvoří účet úložiště Azure (contosovmsacc20180528) v oblasti USA – východ 2.

    - Účet úložiště musí být ve stejné oblasti jako trezor služby Recovery Services.
    - Jejich používáte účet general purpose s storage úrovně standard a replikací LRS.

    ![Site Recovery úložiště](./media/contoso-migration-rehost-linux-vm/asr-storage.png)

2. Pomocí účtu sítě a úložiště v místě společnosti Contoso vytvořit trezor (ContosoMigrationVault) a umístit ji **ContosoFailoverRG** skupinu prostředků, v primární oblasti USA – východ 2.
...
- Vytvoření účtu na vCenter serveru nebo hostiteli vSphere ESXi, pro automatizaci zjišťování virtuálních počítačů.
- Vytvořte účet, který umožňuje automatickou instalaci služby Mobility na virtuální počítače VMware, který chcete replikovat.
- Příprava místních virtuálních počítačů, tak, aby se mohly připojit k virtuálním počítačům Azure po jejich vytvoření po migraci.
...
Musíte nainstalovat službu Mobility na virtuální počítače Linux společnosti Contoso migrace:
...
- Pro automatické nabízené instalace musíte připravit účet, který Site Recovery použije pro přístup k virtuálním počítačům.
...
Po převzetí služeb při selhání do Azure Contoso chce mít možnost se připojit k replikované virtuální počítače v Azure. K tomuto účelu se několik věcí, které potřebují provést:
...
- Po spuštění migrace (převzetí služeb při selhání), by měl zkontrolovat **Diagnostika spouštění** zobrazíte snímek obrazovky virtuálního počítače.
- Pokud to nepomůže, by měl zkontrolovat, že virtuální počítač běží a přečtěte si tyto [tipy pro řešení potíží](http://social.technet.microsoft.com/wiki/contents/articles/31666.troubleshooting-remote-desktop-connection-after-failover-using-asr.aspx).
...
Nyní Contoso nakonfiguruje nastavení cíle replikace.
...
Po zdroje a cíle jsou nastavené, Contoso je připravena vytvořit zásady replikace.
...
3. Vybere contoso **OSTICKETWEB** pro replikaci. 
    ...
    - V této fázi Contoso vybere pouze **OSTICKETWEB** protože musí být vybraná virtuální síť a podsíť a virtuální počítače nejsou ve stejné podsíti.
...
4. Ve vlastnostech virtuálního počítače vybere Contoso účet, který se používá procesový server při automatické instalaci služby Mobility na počítači.
...
Nyní můžete Contoso zahájení replikace **OSTICKETMYSQL**.
...
3. Společnost Contoso použije stejných zásad replikace, který se použil pro OSTICKETWEB a umožňuje replikaci.  
...
### <a name="create-and-customize-a-recovery-plan"></a>Vytvořit a přizpůsobit plán obnovení
 ...
 Po ověření, zda testovací převzetí služeb fungovala podle očekávání, Contoso vytvořit plán obnovení pro migraci. 
...
4.  V **+ skupina** > **přidat chráněné položky**, přidávají **OSTICKETWEB** k **2. skupina: Start**.  Contoso potřebuje tyto ve dvou různých skupin.
...
Contoso je připraven ke spuštění převzetí služeb při selhání v plánu obnovení, migrace virtuálních počítačů.
...
Posledním krokem v procesu migrace Contoso aktualizovat připojovací řetězec aplikace tak, aby odkazoval na databázi aplikace běžící na **OSTICKETMYSQL** virtuálního počítače. 
...
Nyní musí si udělat trochu pořádek Contoso:  

- Místní virtuální počítače odeberou z inventáře vCenter.
- Odstraňují místních virtuálních počítačů z místní úlohy zálohování.
- Aktualizují své interní dokumentaci zobrazíte nové umístění a IP adres pro OSTICKETWEB a OSTICKETMYSQL.
...
- Contoso používá služba Azure Migrate s mapováním závislostí pro vyhodnocení virtuálních počítačů pro migraci. Microsoft Monitoring Agent a agenta závislostí, které jsou instalovány pro tento účel, z virtuálního počítače by měly odebrat.
...
- Skupiny zabezpečení sítě (Nsg), zkontrolujte u virtuálních počítačů pro řízení přístupu. Skupiny zabezpečení sítě se používají k zajištění, že lze předat pouze provoz povolený pro aplikaci.
- Jsou také zvážení, zabezpečení dat na discích virtuálních počítačů pomocí šifrování disku a Azure Key Vaultu.
...
Contoso zálohovat data na virtuálních počítačích pomocí služby Azure Backup. [Další informace](https://docs.microsoft.com/azure/backup/backup-introduction-to-azure-backup?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json).
...
- Contoso nemá žádné licenční problémy s jejich servery se systémem Ubuntu.
...
V dalším článku v této sérii vám ukážeme, jak migrovat Contoso stejné helpdesku app service do Azure. Tentokrát Contoso využívá Site Recovery k migraci front-endový virtuální počítač pro aplikaci a migrovat databázi aplikace pomocí zálohování a obnovení do služby Azure Database for MySQL pomocí nástroje MySQL workbench. [Začínáme](contoso-migration-rehost-linux-vm-mysql.md).
