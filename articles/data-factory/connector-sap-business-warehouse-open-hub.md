---
title: Kopírování dat z SAP Business Warehouse přes Open hub pomocí Azure Data Factory
...
ADF SAP BW Open hub Connector nabízí dvě volitelné vlastnosti: `excludeLastRequest` a `baseRequestId`, které se dají použít ke zpracování rozdílového zatížení z otevřeného centra. 
...
| Jmen | Jméno uživatele, který má přístup k serveru SAP. | Ano |
...
Pokud jste nacházeli `excludeLastRequest` a `baseRequestId` v datové sadě, je tato možnost stále podporovaná tak, jak je, ale při návrhu používání nového modelu ve zdroji aktivity.
...
Pro urychlení načítání dat můžete nastavit [`parallelCopies`](copy-activity-performance.md#parallel-copy) aktivity kopírování, aby se data načetla v SAP BW otevřeném centru paralelně. Pokud jste například nastavili `parallelCopies` na čtyři, Data Factory souběžně provede čtyři volání RFC a každé volání RFC načte část dat z vaší SAP BW otevřenou tabulku hub dělenou ID žádosti DTP a ID balíčku. To platí v případě, že počet jedinečných ID žádosti DTP + ID balíčku je větší než hodnota `parallelCopies`. Při kopírování dat do úložiště dat založeného na souborech je také znovu zaškrtnuto, aby bylo možné zapisovat do složky jako více souborů (pouze název složky). v takovém případě je výkon lepší než zápis do jednoho souboru.
...
|:--- |:--- |
| C (řetězec) | Řetězec |
| I (celé číslo) | Uvedena |
| F (float) | Klepat |
| D (datum) | Řetězec |
| T (čas) | Řetězec |
| P (komprimovaná BCD, měna, desetinné číslo, množství) | Notaci |
| N (NUMC) | Řetězec |
