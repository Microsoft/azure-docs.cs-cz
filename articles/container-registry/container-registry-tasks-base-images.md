...
Fázemi definující většinu imagí kontejnerů určuje nadřazený obrázek, ze kterého je obrázek založen, často se označuje jako *základní obrázek*. Základní image typicky obsahují operační systém, například [Alpine Linux][base-alpine] nebo [Windows nano Server][base-windows], na kterém se používají zbývající vrstvy kontejneru. Můžou taky zahrnovat aplikační architektury, jako je [Node. js][base-node] nebo [.NET Core][base-dotnet]. Tyto základní image jsou obvykle založené na veřejných nadřazených obrázcích. Několik imagí vaší aplikace může sdílet společný základní obrázek.
...
Pokud se základní bitová kopie zadaná v příkazu `FROM` nachází v jednom z těchto umístění, úloha ACR přidá vidlici, aby se zajistilo, že se obrázek znovu sestaví, kdykoli se aktualizuje jeho základ.
