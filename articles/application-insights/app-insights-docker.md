...
1. Přihlaste se do [portálu Microsoft Azure](https://azure.com) a otevřete prostředek Application Insights pro vaši aplikaci; nebo [vytvořte novou](app-insights-create-new-resource.md). 
   
    *Který prostředek by měl použít?* Pokud aplikace, které jsou spuštěné v hostiteli vytvořené někým jiným, pak budete muset [vytvořit nový prostředek Application Insights](app-insights-create-new-resource.md). To je, kde můžete zobrazit a analyzovat telemetrická data. (Vyberte obecné pro typ aplikace.)
   
    Ale pokud jste vývojář aplikací a Věříme, že jste [přidat sadu Application Insights SDK](app-insights-java-live.md) u každého z nich. Pokud jsou všechny skutečně komponenty jednu obchodní aplikace, pak můžete nakonfigurovat všechny z nich k odesílání telemetrie na jeden prostředek a použijete tento stejný prostředek pro zobrazení údajů o Docker životního cyklu a výkonu. 
...
Pokud je vaše aplikace používaná s [Application Insights SDK pro Javu](app-insights-java-get-started.md), přidejte následující řádek do souboru ApplicationInsights.xml ve vašem projektu, v části `<TelemetryInitializers>` element:
...
K prozkoumání jednotlivé události, klikněte na tlačítko [hledání](app-insights-diagnostic-search.md). Vyhledávání a filtrování událostí, které chcete najít. Kliknutím na libovolnou událost zobrazíte další podrobnosti.
...
* [Export telemetrie](app-insights-export-telemetry.md) k další analýze do databáze, Power BI nebo jiný řídicí panel.

*Jak získat telemetrická data z sama aplikace?*

* Nainstalujte službu Application Insights SDK v aplikaci. Zjistěte, jak pro: [webové aplikace v Javě](app-insights-java-get-started.md), [Windows webové aplikace](app-insights-asp-net.md).
...
* [Application Insights pro Java](app-insights-java-get-started.md)
