...
Pro velké balíčky aplikací můžete zadat možnost `--show-progress` pro zobrazení průběhu nahrávání.
...
Chcete-li načíst všechny parametry, které byly zadány dříve, můžete použít příkaz `sfctl application info`.

Když probíhá upgrade aplikace, můžete stav načíst pomocí příkazu `sfctl application upgrade-status`.

Nakonec, pokud probíhá upgrade a je potřeba ho zrušit, můžete k vrácení upgradu použít `sfctl application upgrade-rollback`.
