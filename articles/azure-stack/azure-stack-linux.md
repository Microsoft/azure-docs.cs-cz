...
Azure Linux Agent (obvykle nazývá `WALinuxAgent` nebo `walinuxagent`) je nutné použít, a ne všechny verze agenta bude fungovat ve službě Azure Stack. Pokud vytvoříte vlastní image by měl používat nejnovější verze WALA nebo 2.2.20. Všimněte si, že verze mezi 2.2.20 a 2.2.35.1 (výhradní) nebudou fungovat ve službě Azure Stack. Použití agenta verze mezi 2.2.20 a 2.2.35.1, použijte rychlou záplatu opravu hotfix/1902 1901 nebo aktualizace služby Azure Stack na verzi 1903 (nebo vyšší). Všimněte si, že [cloud-init](https://cloud-init.io/) není v tuto chvíli nepodporuje v Azure stacku.
