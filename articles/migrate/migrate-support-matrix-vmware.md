---
title: Matice podpory Azure Migrate pro vyhodnocení a migraci VMware
description: Shrnuje nastavení podpory a omezení pro účely posouzení a migrace virtuálních počítačů VMware do Azure pomocí služby Azure Migrate.
services: backup
...
ms.locfileid: "68372445"
---
# <a name="support-matrix-for-vmware-assessment-and-migration"></a>Matice podpory pro vyhodnocení a migraci VMware

[Službu Azure Migrate](migrate-overview.md) můžete použít k vyhodnocení a migraci počítačů do Microsoft Azure cloudu. Tento článek shrnuje nastavení podpory a omezení pro vyhodnocení a migraci místních virtuálních počítačů VMware.
...
**Migrace virtuálních počítačů VMware** | Migraci můžete migrovat pomocí migrace bez agenta s některými omezeními nebo pomocí migrace založené na agentech. [Víc se uč](server-migrate-overview.md)
...
Omezení VMware  | Vyhodnoťte až 35 000 virtuálních počítačů VMware v jednom projektu.
...
## <a name="assessment-vmware-server-requirements"></a>Posouzení – požadavky na server VMware
...
Pouze pro posouzení potřebujete účet jen pro čtení vCenter Server.
...
- Pokud používáte proxy server brány firewall založený na adrese URL, povolte přístup k těmto adresám URL a ujistěte se, že proxy překládá všechny záznamy CNAME přijaté při vyhledávání adres URL.
...
**zařízení** | **připojení**
--- | ---
Zařízení | Příchozí připojení na portu TCP 3389 umožňující připojení ke vzdálené ploše zařízení.<br/> Příchozí připojení na portu 44368 pro vzdálený přístup k aplikaci pro správu zařízení pomocí adresy URL: https://< zařízení-IP-nebo-name >: 44368 <br/>Odchozí připojení na portu 443, která odesílají metadata zjišťování a výkonu Azure Migrate.
Server vCenter | Příchozí připojení na portu TCP 443, aby zařízení mohla shromažďovat metadata o konfiguraci a výkonu pro posouzení. <br/> Ve výchozím nastavení se zařízení připojuje ke vCenter na portu 443. Pokud Server vCenter naslouchá na jiném portu, můžete změnit port při nastavení zjišťování.
...
**Podpora** | **Podrobnosti**
--- | ---
**Server vCenter** | Virtuální počítače VMware, které migrujete pomocí migrace bez agenta, se musí spravovat na jednom nebo víc serverech vCenter, na kterých běží 5,5, 6,0, 6,5 nebo 6,7.
...
**Cílový disk** | Virtuální počítače se dají migrovat jenom na spravované disky (Standard HHD, Premium SSD) v Azure.
...
**Azure Migrate projekt** | Zařízení může být přidruženo k jednomu projektu.
**vCenter Server** | Zařízení může na vCenter Server zjistit až 10 000 virtuálních počítačů VMware.<br/> Zařízení se může připojit k jednomu vCenter Server.
**VDDK** | Pokud provádíte migraci bez agentů s migrací Azure Migrate serveru, musí být na virtuálním počítači zařízení nainstalovaná VMware vSphere VDDK (Virtual disk Development Kit).
...
**zařízení** | **připojení**
--- | ---
Zařízení | Odchozí port TCP 3389 pro nahrání replikovaných dat do Azure a komunikaci s Azure Migrate pro replikaci a migraci.
Server vCenter | Příchozí připojení na portu TCP 443, aby zařízení mohla orchestrovat replikaci – vytvářet snímky, kopírovat data, snímky verzí
...
**Podpora** | **Podrobnosti**
--- | ---
**Server vCenter/ESXI** | Virtuální počítače VMware, které migrujete, musí být spravované jedním nebo více servery vCenter se systémem 5,5, 6,0, 6,5 nebo 6,7 nebo spuštěny na hostiteli ESXI s vSphere verze 5,5, 6,0, 6,5 nebo 6,7.

### <a name="agent-based-migration-vcenter-server-permissions"></a>Migrace založená na agentech – vCenter Server oprávnění

**Oprávnění** | **Podrobnosti**
--- | ---
Úložiště dat. AllocateSpace | Povolí přidělování místa v úložišti dat pro virtuální počítač, snímek, klon nebo virtuální disk.
Datastore.Browse | Povolí procházení souborů protokolu virtuálních počítačů, aby bylo možné řešit problémy při vytváření a odstraňování snímků.
Datastore.LowLevelFileOperations | Povolí operace čtení, zápisu, odstranění a přejmenování v prohlížeči úložiště dat, aby bylo možné řešit potíže s vytvořením nebo odstraněním snímku.
Datastore.UpdateVirtualMachineFiles | Povolení aktualizace cest k souborům virtuálního počítače v úložišti dat po přepodepsání úložiště dat
Network.AssignNetwork | Povolte přiřazení sítě k prostředku virtuálního počítače.
AssignVirtualMachineToResourcePool | Umožňuje přiřadit virtuálnímu počítači fond zdrojů.
Resource.MigratePoweredOffVirtualMachine | Povolte migraci vypnutého virtuálního počítače do jiného fondu zdrojů nebo hostitele.
Resource.MigratePoweredOnVirtualMachine | Povolte migraci pomocí vMotion na virtuálním počítači s jiným fondem zdrojů nebo na hostitele.
Tasks.CreateTask | Umožňuje rozšíření vytvořit uživatelem definovanou úlohu.
Tasks.UpdateTask | Umožňuje rozšíření aktualizovat uživatelsky definovaný úkol.
VirtualMachine. Configuration. | Umožňuje konfigurovat možnosti virtuálních počítačů a zařízení.
Virtuální počítač. interakce. AnswerQuestion | Povolí řešení problémů s přechody stavu virtuálního počítače nebo chybami za běhu.
Virtuální počítač. interakce. DeviceConnection | Povolí změnu stavu připojení virtuálních zařízení, která se nepřipojují k virtuálnímu počítači.
Virtuální počítač. interakce. ConfigureCDMedia | Povolí konfiguraci virtuálního DVD nebo zařízení CD-ROM.
Virtuální počítač. interakce. ConfigureFloppyMedia | Povolí konfiguraci virtuální disketové zařízení.
Virtuální počítač. interakce. stavu poweroff | Umožňuje, aby se virtuální počítač při migraci do Azure vypnul.
Virtuální počítač. interakce. PowerOn | Povolte zapnutí u vypnutého virtuálního počítače a obnovování pozastaveného virtuálního počítače.
Virtuální počítač. interakce. VMwareToolsInstall | Povolte připojení a odpojení instalačního programu CD nástrojů VMware jako disk CD-ROM hostovaného operačního systému.
VirtualMachine.Inventory.CreateNew | Umožňuje vytvořit virtuální počítač a přidělení požadovaných prostředků.
VirtualMachine. Inventory. Register | Umožňuje přidat existující virtuální počítač do inventáře vCenter Server nebo hostitele.
VirtualMachine. Inventory. Unregister | Povolí zrušení registrace VMe z vCenter Server nebo inventáře hostitele.
VirtualMachine.Provisioning.AllowVirtualMachineFilesUpload | Povoluje operace zápisu u souborů přidružených k virtuálnímu počítači, včetně VMX, disků, protokolů a paměti NVRAM.
VirtualMachine.Provisioning.AllowVirtualMachineDownload | Povoluje operace čtení souborů přidružených k virtuálnímu počítači ke stažení protokolů pro řešení potíží.
VirtualMachine. SnapshotManagement. RemoveSnapshot | Povoluje odebrání snímku z historie snímku.
...
Jiné aplikace | Na zařízení replikace byste neměli spouštět jiné aplikace.
...
Nestahovat online | Umístěte aplikaci instalačního programu MySQL do složky C:\Temp\ASRSetup. Když nainstalujete zařízení a kliknete na stažení a instalaci MySQL, instalační program použije instalační program, který jste přidali.
Stáhnout z Azure Migrate | Po instalaci zařízení a zobrazení výzvy k MySQL vyberte **Stáhnout a nainstalovat**.
...
**zařízení** | **připojení**
--- | ---
virtuálních počítačů | Služba mobility spuštěná na virtuálních počítačích komunikuje s místním konfiguračním serverem na portu HTTPS 443 příchozí, pro správu replikací.<br/><br/> Virtuální počítače odesílají data replikace na procesový Server (spuštěný na počítači konfiguračního serveru) na portu HTTPS 9443 příchozí. Tento port lze změnit.
Replikační zařízení | Zařízení replikace orchestruje replikaci pomocí Azure přes odchozí port HTTPS 443.
Procesový server | Procesový server přijímá data replikace, optimalizuje je a šifruje a odesílá je do Azure Storage přes odchozí port 443.<br/> Ve výchozím nastavení běží na zařízení replikace procesový Server.
...
--- | --- | ---
Hostovaný operační systém | Ověřte podporované operační systémy pro [virtuální počítače VMware pomocí replikace](#agentless-migration-vmware-vm-requirements)bez agentů a pro [virtuální počítače VMware pomocí replikace založené na agentech](#agent-based-migration-vmware-vm-requirements).<br/> Můžete migrovat libovolné úlohy běžící v podporovaném operačním systému. | Pokud je tato operace Nepodporovaná, ověřte chybu.
...
Připojit po migraci – Windows | Připojení k virtuálním počítačům Azure s Windows po migraci:<br/> – Před migrací povolte RDP na místním virtuálním počítači. Ujistěte se, že jsou přidaná pravidla TCP a UDP pro **Veřejný** profil a že v části **Brána Windows Firewall** > **Povolené aplikace** je pro všechny profily povolený protokol RDP.<br/> V případě přístupu typu Site-to-site k síti VPN Povolte protokol RDP a Povolte protokol RDP v **bráně Windows Firewall** -> **povolené aplikace a funkce** pro **domény a privátní** sítě. Dále ověřte, že je zásada SAN operačního systému nastavená na **OnlineAll**. [Další informace](https://support.microsoft.com/kb/3031135). |
