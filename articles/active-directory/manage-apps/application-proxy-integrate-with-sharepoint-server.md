...
> [!NOTE]
> Postupujte podle následujících doporučení pro interní adresa URL:
...
1. Klikněte na tlačítko **přidat** tlačítko, klikněte na tlačítko **uživatele nebo počítače**a vyhledejte účet fondu aplikací služby SharePoint, například _demo\spAppPoolAccount_.
...
1. Po publikování aplikace konfigurujte nastavení jednotného přihlašování pomocí následujících kroků:
   ...
   1. Vnitřní hlavní název služby aplikace nastavte na hodnotu, která jste nastavili dříve. V tomto příkladu, který by **HTTP/SharePoint**.
   1. V "Delegovaná identita přihlášení" Vyberte nejvhodnější možnosti pro konfiguraci doménové struktury služby Active Directory. Například pokud máte v doménové struktuře, vyberte jednu doménu AD **název účtu SAM On-premises** (uvedeno níže), ale v případě, že uživatelé nejsou ve stejné doméně jako Sharepointu a pak vyberte servery konektoru Proxy aplikace  **Hlavní název uživatele v místním** (nezobrazení).
...
1. Spusťte následující skript pro rozšíření webové aplikace do sítě Extranet zóny a zapněte ověřování protokolem Kerberos:
...
1. Vyberte svou lokalitu, například **SharePoint - 80**. Pro tuto chvíli nemá extranetu zóny interní adresa URL správně nastavit ještě:

   ![Zobrazí pole mapování alternativních adres URL](./media/application-proxy-integrate-with-sharepoint-server/alternate-access1.png)

1. Klikněte na tlačítko **přidejte interní adresy URL**.
1. V **protokol adresy URL, hostitele a port** textové pole, typ **interní adresa URL** nakonfigurované v proxy služby Azure AD, například <https://SharePoint/>.
1. Vyberte zónu **Extranet** v rozevíracím seznamu.
1. Klikněte na **Uložit**.
1. Mapování alternativních adres URL by teď měl vypadat takto:
...
1. Spusťte následující skript, který vygenerovat certifikát podepsaný svým držitelem a přidejte ho do počítače úložišti:
...
1. Rozbalte položku serveru ve stromovém zobrazení rozbalte "Lokalit", vyberte lokalitu, "Proxy AAD – SharePoint" a klikněte na **vazby**.
1. Vyberte vazbu https a klikněte na tlačítko **upravit...** .
1. V poli certifikátu protokolu SSL, zvolte **SharePoint** certifikátu a klikněte na tlačítko OK.
