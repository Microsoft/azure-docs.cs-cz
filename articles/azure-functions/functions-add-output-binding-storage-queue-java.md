...
Vzhledem k tomu, že Archetype také vytvoří sadu testů, je nutné aktualizovat tyto testy pro zpracování nového parametru `msg` v signatuře metody `run`.  

Přejděte do umístění kódu testu v části _Src/test/Java_, otevřete soubor projektu *Function. Java* a nahraďte řádek kódu v rámci `//Invoke` následujícím kódem.
