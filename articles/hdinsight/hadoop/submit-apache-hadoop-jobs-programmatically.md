---
title: Odesílání úloh Hadoop v HDInsight | Microsoft Docs
description: Zjistěte, jak k odesílání úloh Hadoop v prostředí Azure HDInsight.
editor: cgronlun
manager: jhubbard
services: hdinsight
documentationcenter: ''
tags: azure-portal
author: mumian
ms.assetid: 50430b96-2329-4775-9713-19c5795b775f
ms.service: hdinsight
ms.custom: hdinsightactive
ms.devlang: na
ms.topic: conceptual
ms.date: 05/16/2018
ms.author: jgao
ms.openlocfilehash: 2263201b0caf543db7167294f346863d7dcbf383
ms.sourcegitcommit: eb75f177fc59d90b1b667afcfe64ac51936e2638
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 05/16/2018
ms.locfileid: "34200424"
---
# <a name="submit-hadoop-jobs-in-hdinsight"></a>Odesílání úloh Hadoopu do HDInsight

Můžete odeslat úloh Hadoop v prostředí Azure HDInsight pomocí rozhraní Microsoft .NET SDK pro HDInsight, curl nebo Azure PowerShell.

## <a name="resources"></a>Zdroje a prostředky

- **.NET SDK**:

  - [Vytvoření neinteraktivního ověřování aplikace .NET](../hdinsight-create-non-interactive-authentication-dotnet-applications.md)
  - [Spouštění dotazů Hive pomocí sady .NET SDK HDInsight](apache-hadoop-use-hive-dotnet-sdk.md)
  - [Spuštění úlohy Pig pomocí .NET SDK pro Hadoop v HDInsight](apache-hadoop-use-pig-dotnet-sdk.md)
  - [Spuštění úloh Sqoop pomocí .NET SDK pro Hadoop v HDInsight](apache-hadoop-use-sqoop-dotnet-sdk.md)
  - [Spuštění úloh MapReduce pomocí .NET SDK služby HDInsight](apache-hadoop-use-mapreduce-dotnet-sdk.md)

- **curl**:

  - [Spouštění dotazů Hive s Hadoop v HDInsight pomocí curl](apache-hadoop-use-hive-curl.md)
  - [Spuštění úlohy Pig s Hadoop v HDInsight pomocí curl](apache-hadoop-use-pig-curl.md)
  - [Spuštění úloh Sqoop s Hadoop v HDInsight pomocí curl](apache-hadoop-use-sqoop-curl.md)
  - [Spuštění úloh MapReduce s Hadoop v HDInsight pomocí curl](apache-hadoop-use-mapreduce-curl.md)

- **Prostředí PowerShell**:

  - [Spouštění dotazů Hive pomocí prostředí PowerShell](apache-hadoop-use-hive-powershell.md)
  - [Spuštění úlohy Pig pomocí prostředí PowerShell](apache-hadoop-use-pig-powershell.md)
  - [Použití nástroje Sqoop se systémem Hadoop v HDInsight](apache-hadoop-use-sqoop-powershell.md)
  - [Spuštění úloh MapReduce s Hadoop v HDInsight pomocí prostředí PowerShell](apache-hadoop-use-mapreduce-powershell.md)

## <a name="see-also"></a>Další informace najdete v tématech

- [Dokumentace k Azure HDInsight](https://docs.microsoft.com/azure/hdinsight/)
