...
1. Obsah automaticky generovaného `ViewController.m` souboru nahraďte následujícím kódem:
...
1. Přidejte žádost o přístup k mikrofonu. Klikněte pravým tlačítkem `Info.plist` myši na položku stromové struktury projektu a vyberte **Otevřít jako** > **zdrojový kód**. Do `<dict>` oddílu přidejte následující řádky a potom soubor uložte.
