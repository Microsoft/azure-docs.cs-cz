...
Každá metrika obsahuje dvě verze. Jedna metrika měří výkon celé mezipaměti a pro mezipaměti, které používají [clusteringu](cache-how-to-premium-clustering.md), druhá verze metriky, která zahrnuje `(Shard 0-9)` v názvu, měří výkon pro jeden horizontálních oddílů v mezipaměti. Například pokud má mezipaměť 4 horizontálních oddílů, `Cache Hits` je celková velikost přístupů pro celou mezipaměť a `Cache Hits (Shard 3)` je pouze počet přístupů k této horizontálních oddílů mezipaměti.
...
| Metrika | Popis |
| --- | --- |
| Přístupy do mezipaměti |Počet úspěšných vyhledání klíčů během zadaného intervalu generování sestav. To se mapuje na `keyspace_hits` z příkazu Redis [info](https://redis.io/commands/info) . |
| Latence mezipaměti (Preview) | Latence mezipaměti se vypočítala na základě latence v mezipaměti pro uzel. Tato metrika se měří v mikrosekundách a má tři dimenze: "průměr", "min" a "Max", což představuje průměrnou, minimální a maximální latenci mezipaměti v tomto pořadí v zadaném intervalu generování sestav. |
| Neúspěšné přístupy do mezipaměti |Počet neúspěšných vyhledání klíčů během zadaného intervalu generování sestav. To se mapuje na `keyspace_misses` z příkazu Redis INFO. Neúspěšné přístupy do mezipaměti nutně znamenají, že došlo k potížím s mezipamětí. Například při použití modelu programování s mezipamětí aplikace vypadá jako první v mezipaměti pro položku. Pokud položka není tam (nenalezena mezipaměť), bude položka načtena z databáze a přidána do mezipaměti pro další čas. Neúspěšné přístupy do mezipaměti představují normální chování pro model programování s mezipamětí. Pokud počet neúspěšných přístupů do mezipaměti je vyšší, než se očekávalo, Projděte si logiku aplikace, která naplní a přečte z mezipaměti. Pokud se položky z mezipaměti vyloučí z důvodu přetížení paměti, může dojít k nějakým neúspěšným výjimkám, ale lepší metrika monitorování pro tlak paměti by `Used Memory` nebo `Evicted Keys`. |
...
| Připojení klienti |Počet připojení klientů k mezipaměti během zadaného intervalu generování sestav. To se mapuje na `connected_clients` z příkazu Redis INFO. Po dosažení [limitu připojení](cache-configure.md#default-redis-server-configuration) dojde k selhání následného pokusu o připojení do mezipaměti. Mějte na paměti, že i když neexistují žádné aktivní klientské aplikace, může být v důsledku interních procesů a připojení stále několik instancí připojených klientů. |
...
| Vyřazení klíčů |Počet položek vyřazených z mezipaměti během zadaného intervalu vytváření sestav kvůli limitu `maxmemory`. To se mapuje na `evicted_keys` z příkazu Redis INFO. |
...
| Celkem klíčů  | Maximální počet klíčů v mezipaměti během uplynulých časových období generování sestav. To se mapuje na `keyspace` z příkazu Redis INFO. Z důvodu omezení základního systému metrik pro ukládání do mezipaměti s povoleným clusteringem vrátí funkce Total Keys maximální počet klíčů horizontálních oddílů, který měl maximální počet klíčů během intervalu generování sestav.  |
| Celkem operací |Celkový počet příkazů zpracovaných serverem mezipaměti během zadaného intervalu generování sestav. Tato hodnota se mapuje na `total_commands_processed` z příkazu Redis INFO. Všimněte si, že pokud je mezipaměť Azure pro Redis používána čistě pro Pub/sub, nebudou žádné metriky pro `Cache Hits`, `Cache Misses`, `Gets`nebo `Sets`, ale budou `Total Operations` metriky, které odrážejí využití mezipaměti pro operace Pub/sub. |
| Využitá paměť |Velikost paměti mezipaměti použitá pro páry klíč-hodnota v mezipaměti v MB během zadaného intervalu generování sestav. Tato hodnota se mapuje na `used_memory` z příkazu Redis INFO. Nezahrnuje metadata ani fragmentace. |
| Procento využité paměti | % Celkové paměti, která se používá během zadaného intervalu generování sestav.  To odkazuje na hodnotu "used_memory" z příkazu Redis INFO pro výpočet procenta. |
