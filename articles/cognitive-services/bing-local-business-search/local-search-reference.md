...
|Hlavička|Popis|  
|------------|-----------------|  
|vyjádřit|Nepovinná hlavička požadavku.<br /><br /> Výchozí typ média je Application/JSON. Chcete-li určit, že odpověď používá [JSON-ld](https://json-ld.org/), nastavte hlavičku Accept na Application/ld + JSON.|  
...
|----------|-----------|----------|--------------|
|počet <a name="count" />|Počet výsledků, které se mají vrátit, počínaje indexem určeného parametrem `offset`|Řetězec|Ne|   
...
|posun <a name="offset"/>|Index pro zahájení výsledků zadaných parametrem `count`.|Integer|Ne|  
...
|Adresáře|Poštovní adresa místa, kde se entita nachází.|PostalAddress|  
|entityPresentationInfo|Další informace o entitě, jako jsou například tipy, které můžete použít k určení typu entity. Například bez ohledu na to, jestli se jedná o restaurace nebo Hotel. Pole `entityScenario` je nastaveno na ListItem.|EntityPresentationInfo|  
