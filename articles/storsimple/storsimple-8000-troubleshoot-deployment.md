...
| 7 |Invoke-HcsSetupWizard: z mapovače koncových bodů nejsou k dispozici žádné další koncové body. (Výjimka z HRESULT: 0x800706D9) |Funkce clusteru nefunguje. |O dalším postupu se [poraďte s podporou Microsoftu](storsimple-8000-contact-microsoft-support.md). |

## <a name="errors-during-the-optional-web-proxy-settings"></a>Chyby při nastavení volitelného webového proxy serveru
| Ne. | Chybová zpráva | Možné příčiny | Doporučená akce |
| --- | --- | --- | --- |
| 1 |Invoke-HcsSetupWizard: neplatný parametr (výjimka z HRESULT: 0x80070057) |Jeden z parametrů poskytnutých pro nastavení proxy serveru není platný. |Identifikátor URI není k dispozici ve správném formátu. Použijte následující formát: http:// *\<IP adresa nebo plně kvalifikovaný název domény proxy server webu >* : *\<číslo portu TCP >* |
...
| 5 |Invoke-HcsSetupWizard: prostředek clusteru není online (výjimka z HRESULT: 0x8007138c) |Prostředky clusteru nejsou online. |O dalším postupu se [poraďte s podporou Microsoftu](storsimple-8000-contact-microsoft-support.md). |
...
| 5 |Chyba 350031: zařízení je už zaregistrované. | |Není nutná žádná akce. |
...
* `Test-Connection`: pomocí této rutiny můžete ověřit připojení k síti uvnitř i vně sítě.
...
V následujících částech jsou uvedeny ukázky výstupu z rutiny `Get-NetAdapter`.
...
Pomocí rutiny `Test-Connection` můžete určit, jestli se zařízení StorSimple může připojit k vnější síti. Pokud jsou všechny parametry sítě, včetně DNS, správně nakonfigurované v Průvodci instalací nástroje, můžete použít rutinu `Test-Connection` k otestování známé adresy mimo síť, například outlook.com.

Pokud je parametr pøíkazového testu zakázaný, měli byste povolit příkazy pro řešení potíží s připojením pomocí příkazů if.

Podívejte se na následující ukázky výstupu z rutiny `Test-Connection`.
...
Podívejte se na následující ukázky výstupu z rutiny `Test-HcsmConnection`.
...
Pokud spustíte rutinu `Get-HcsRoutingTable` bez zadání parametrů (jak ukazuje následující příklad), rutina vytvoří výstup směrovacích tabulek IPv4 i IPv6. Případně můžete zadat `Get-HcsRoutingTable -IPv4` nebo `Get-HcsRoutingTable -IPv6` k získání příslušné směrovací tabulky.
...
1. Ověřte konfiguraci zařízení: na aktivním řadiči spusťte `Invoke-HcsSetupWizard`.
