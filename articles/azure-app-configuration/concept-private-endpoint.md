...
Vlastníci účtu služby můžou spravovat žádosti o souhlas a soukromé koncové body prostřednictvím karty `Private Endpoints` v úložišti konfigurace v [Azure Portal](https://portal.azure.com).
...
Azure při směrování připojení z virtuální sítě do úložiště konfigurace prostřednictvím privátního propojení spoléhá na překlad DNS. Řetězce připojení můžete rychle najít v Azure Portal tak, že vyberete úložiště konfigurace aplikace a pak vyberete **nastavení** > **přístup k klíčům**.  

> [!IMPORTANT]
> Použijte stejný připojovací řetězec pro připojení k úložišti konfigurace aplikace pomocí privátních koncových bodů, protože byste použili pro veřejný koncový bod. Nepřipojujte se k účtu úložiště pomocí adresy URL subdomény `privatelink`.

## <a name="dns-changes-for-private-endpoints"></a>Změny DNS u privátních koncových bodů

Při vytváření privátního koncového bodu se záznam prostředku CNAME DNS pro úložiště konfigurace aktualizuje na alias v subdoméně s předponou `privatelink`. Azure také vytvoří [privátní ZÓNU DNS](../dns/private-dns-overview.md) , která odpovídá `privatelink` subdomény, a záznamy prostředků DNS pro privátní koncové body.
...
Pokud ve vaší síti používáte vlastní server DNS, klienti musí být schopni přeložit plně kvalifikovaný název domény (FQDN) pro koncový bod služby na IP adresu privátního koncového bodu. Nakonfigurujte server DNS tak, aby delegoval subdoménu privátního propojení do privátní zóny DNS pro virtuální síť, nebo nakonfigurujte záznamy A pro `AppConfigInstanceA.privatelink.azconfig.io` s IP adresou privátního koncového bodu.

> [!TIP]
> Pokud používáte vlastní nebo místní server DNS, měli byste server DNS nakonfigurovat tak, aby přeložil název úložiště v subdoméně `privatelink` na IP adresu privátního koncového bodu. To můžete provést delegováním `privatelink` subdomény do privátní zóny DNS virtuální sítě nebo konfigurací zóny DNS na serveru DNS a přidáním záznamů DNS.
