...
---
# <a name="azure-signalr-service-authentication"></a>Ověřování služby signalizace Azure
...
4. Do třídy `GetUserCompanyInfoAsync` přidejte pomocnou metodu `Startup`.
...
4. Spusťte okno prohlížeče a přejděte na adresu `http://localhost:5000`. Kliknutím na odkaz **tady** v horní části se přihlaste s využitím GitHubu.
    ...
    ![Identifikace účtu](media/signalr-concept-authenticate-oauth/signalr-oauth-account-identified.png)
...
> [Ukázky rozhraní příkazového řádku pro službu Azure SignalR](./signalr-reference-cli.md)