...
Spusťte následující příkaz [az account alias create](/cli/azure/account/alias#az_account_alias_create) a zadejte v něm údaje `billing-scope` a `id` některého z vašich účtů `enrollmentAccounts`. 
