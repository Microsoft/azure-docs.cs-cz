...
[Azure Resource Manager](../../azure-resource-manager/management/overview.md) vám umožní pracovat s prostředky ve vašem řešení jako se skupinou. Všechny prostředky pro vaše řešení můžete nasadit, aktualizovat nebo odstranit v rámci jediné koordinované operace. Pro nasazení použijete [šablonu Azure Resource Manager](../../azure-resource-manager/templates/overview.md) a tato šablona může fungovat v různých prostředích, jako je testování, příprava a produkce. Resource Manager poskytuje funkce zabezpečení, auditování a označování, které vám po nasazení pomohou prostředky spravovat.
...
[Application Insights](../../azure-monitor/app/app-insights-overview.md) je rozšiřitelná služba správy výkonu aplikací (APM) pro webové vývojáře. Pomocí Application Insights můžete monitorovat živé webové aplikace a automaticky zjišťovat anomálie výkonu. Obsahuje výkonné analytické nástroje, které vám pomůžou diagnostikovat problémy a porozumět tomu, co uživatelé skutečně dělají s vašimi aplikacemi. Monitoruje vaši aplikaci pokaždé, když je spuštěná, jak během testování, tak i po jeho publikování nebo nasazení.
...
[Azure monitor](/azure/monitoring-and-diagnostics/) nabízí vizualizaci, dotaz, směrování, upozorňování, automatické škálování a automatizaci dat jak z předplatného Azure ([Protokol aktivit](../../azure-monitor/essentials/platform-logs-overview.md)), tak z každého jednotlivého prostředku Azure ([protokoly prostředků](../../azure-monitor/essentials/platform-logs-overview.md)). Pomocí Azure Monitor můžete upozorňovat na události související se zabezpečením, které jsou generovány v protokolech Azure.

### <a name="azure-monitor-logs"></a>Protokoly služby Azure Monitor

[Protokoly Azure monitor](../../azure-monitor/logs/log-query-overview.md) – poskytuje kromě prostředků Azure i řešení pro správu IT pro místní i cloudovou infrastrukturu (například AWS). Data z Azure Monitor můžete směrovat přímo do protokolů Azure Monitor, abyste viděli metriky a protokoly pro celé prostředí na jednom místě.
...
Neprovádíme [testování průniku](./pen-testing.md) vaší aplikace za vás, ale chápeme, že chcete a potřebujete provádět testování na vašich vlastních aplikacích. To je dobré, protože když vylepšujete zabezpečení vašich aplikací, pomůžete zajistit vyšší zabezpečení celého ekosystému Azure. Zatímco oznamování společnosti Microsoft pro testování perem již nevyžadují zákazníky, musí stále splňovat [pravidla pro testování průniku v rámci zapojení Microsoft Cloud](https://www.microsoft.com/msrc/pentest-rules-of-engagement).

### <a name="web-application-firewall"></a>Firewall webových aplikací

Firewall webových aplikací (WAF) v [Azure Application Gateway](../../application-gateway/features.md#web-application-firewall) pomáhá chránit webové aplikace před běžnými webovými útoky, jako jsou vkládání SQL, útoky skriptování mezi weby a zneužití relace. Obsahuje předem nakonfigurovanou ochranu před hrozbami, které identifikoval [projekt OWASP (Open Web Application Security) jako prvních 10 běžných ohrožení zabezpečení](https://www.owasp.org/index.php/Category:OWASP_Top_Ten_Project).
...
[App Service webové aplikace](../../app-service/troubleshoot-diagnostic-logs.md) poskytují diagnostické funkce pro protokolování informací z webového serveru i z webové aplikace. Tyto jsou logicky rozděleny na diagnostiku webových serverů a Application Diagnostics. Webový server obsahuje dva hlavní pokrok při diagnostice a řešení potíží s weby a aplikacemi.
...
[Skupina zabezpečení sítě (NSG)](../../virtual-network/virtual-network-vnet-plan-design-arm.md#security) je základní bránou firewall pro filtrování paketů a umožňuje řídit přístup na základě 5 řazených kolekcí členů. Skupin zabezpečení sítě neposkytuje kontrolu aplikační vrstvy ani ověřování řízení přístupu. Dají se použít k řízení přesunu provozu mezi podsítěmi v rámci Azure Virtual Network a přenosem mezi Virtual Network Azure a internetem.
...
[Trasy definované uživatelem](../../virtual-network/virtual-networks-udr-overview.md#custom-routes) umožňují přizpůsobit příchozí a odchozí cesty pro provoz, který se přesouvá do a z jednotlivých virtuálních počítačů nebo podsítí, aby se zajistila nejvyšší možná úroveň zabezpečení trasy. [Vynucené tunelování](../../vpn-gateway/vpn-gateway-forced-tunneling-rm.md) je mechanismus, který můžete použít k zajištění toho, aby vaše služby neumožňovaly iniciovat připojení k zařízením v Internetu.
...
Transparentní šifrování dat (TDE) a šifrování na úrovni sloupce (CLE) jsou funkce šifrování SQL serveru. Tato forma šifrování vyžaduje, aby zákazníci spravovali a ukládali kryptografické klíče, které používáte pro šifrování.
...
- [Azure Active Directory připojená](../../active-directory/devices/overview.md) umožňují rozšiřování cloudových možností do zařízení s Windows 10 pro centralizovanou správu. Umožňuje uživatelům připojit se k podnikovému nebo organizačnímu cloudu prostřednictvím Azure Active Directory a zjednodušit přístup k aplikacím a prostředkům.
...
- Přečtěte si, jak [Azure Security Center](../../security-center/security-center-introduction.md) vám pomůžou zabránit hrozbám, detekovat je a reagovat na ně se zvýšenou viditelností a kontrolou zabezpečení vašich prostředků Azure.