...
Blok **Properties. policyRule. then. Details** oznamuje Azure Policy, co hledat v souvislosti s vytvořeným nebo aktualizovaným prostředkem v bloku **Properties. policyRule. if** . V tomto příkladu musí být sledovací proces sítě ve skupině prostředků **networkWatcherRG** s **polem** `location` rovný umístění nového nebo aktualizovaného prostředku. Použití funkce `field()` umožňuje **existenceCondition** získat přístup k vlastnostem nového nebo aktualizovaného prostředku, konkrétně k vlastnosti `location`.
