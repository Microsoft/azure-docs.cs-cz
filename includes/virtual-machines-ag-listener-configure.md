...
3. Přepněte roli clusteru skupiny dostupnosti do režimu offline. V **Správce clusteru převzetí služeb při selhání** pod **role**, klikněte pravým tlačítkem na roli a vyberte **zastavit roli**.

4. <a name="congroup"></a>Konfigurace IP prostředku pro skupinu dostupnosti.
...
5. <a name = "dependencyGroup"></a>Ujistěte se, prostředek skupiny dostupnosti SQL serveru závisí na klientský přístupový bod.
...
6. <a name="listname"></a>Zkontrolujte přístup k bodu klienta závislá na IP adresu prostředku.
...
7. <a name="setparam"></a>Nastavení parametrů clusteru v prostředí PowerShell.
...
8. Přeneste online role clusteru skupiny dostupnosti. V **Správce clusteru převzetí služeb při selhání** pod **role**, klikněte pravým tlačítkem myši klikněte na roli a vyberte **Role spuštění**.
...
2. Klikněte pravým tlačítkem na **IP adresu**a vyberte **vlastnosti**.

3. Kopírovat **název** IP adresy. To může být `Cluster IP Address`. 

4. <a name="setwsfcparam"></a>Nastavení parametrů clusteru v prostředí PowerShell.
