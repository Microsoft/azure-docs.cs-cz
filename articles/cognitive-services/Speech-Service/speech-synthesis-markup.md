...
V závislosti na jazyku sady Speech SDK nastavíte vlastnost `"SpeechServiceResponse_Synthesis_WordBoundaryEnabled"` na `false` na instanci objektu `SpeechConfig`.
...
Ve výchozím nastavení služba pro převod textu na řeč syntetizuje text pomocí neutrálního mluveného stylu pro hlasy Standard i neuronové. S neuronové hlasy můžete upravit styl speakování na Express cheerfulness, soucit nebo mínění pomocí elementu `<mstts:express-as>`. Toto je volitelný element jedinečný pro službu Speech Service.
...
| Atribut | Popis | Povinné / volitelné |
|-----------|-------------|---------------------|
| `type` | Určuje styl speaking. V současné době jsou styly mluvené řeči specifické pro hlas. | Vyžaduje se, když se upraví styl speakování pro neuronové hlas. Pokud používáte `mstts:express-as`, je nutné zadat typ. Pokud je zadána neplatná hodnota, bude tento prvek ignorován. |
...
| `pt-BR-FranciscaNeural` | `type="cheerful"` | Vyjadřuje, že emoce jsou pozitivní a veselé |
...
| Atribut | Popis | Povinné / volitelné |
|-----------|-------------|---------------------|
| `alphabet` | Určuje fonetickou abecedu, která se použije při syntetizování výslovnosti řetězce v atributu `ph`. Řetězec určující abecedu musí být zadán malými písmeny. Níže jsou uvedené možné abecedy, které můžete zadat.<ul><li>`ipa` &ndash; mezinárodní fonetická abecedou</li><li>fonetická abeceda služby `sapi` &ndash; řeči</li><li>Univerzální telefonní sada `ups` &ndash;</li></ul><br>Abeceda se vztahuje pouze na `phoneme` v elementu. Další informace najdete v referenčních informacích o [fonetické abecedě](https://en.wikipedia.org/wiki/International_Phonetic_Alphabet). | Volitelné |
...
| Atribut | Popis | Povinné / volitelné |
|-----------|-------------|---------------------|
| `uri` | Adresa externího dokumentu jiných pracovních prostorů | Požadováno. |
...
`sapi` můžete použít jako hodnotu pro atribut `alphabet` s vlastními lexikony, jak je znázorněno níže:
