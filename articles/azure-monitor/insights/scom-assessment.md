...
## <a name="system-center-operations-manager-health-check-data-collection-details"></a>Podrobnosti o System Center Operations Manager Health Check shromažďování dat
...
### <a name="sql-script-to-grant-granular-permissions-to-the-run-as-account"></a>Skript SQL pro udělení podrobných oprávnění k účtu Spustit jako
...
### <a name="configure-the-health-check-rule"></a>Konfigurovat pravidlo kontroly stavu
...
Ve výchozím nastavení je pravidlo kontroly stavu spouštění Microsoft System Center Operations Manager zakázané. Chcete-li spustit kontrolu stavu, je nutné povolit pravidlo na management server. Použijte následující postup.

#### <a name="enable-the-rule-for-a-specific-management-server"></a>Povolit pravidlo pro konkrétní management server
...
3. Klikněte pravým tlačítkem na pravidlo a potom klikněte na možnost **přepsání** > **pro konkrétní objekt třídy: Server pro správu**.
...
5.  Zajistěte, aby byla hodnota přepsání změněna na hodnotu **true** pro hodnotu **povoleného** parametru.<br><br> parametr přepsat ![](./media/scom-assessment/rule.png)

    Stále v tomto okně nakonfigurujte četnost spouštění pomocí následujícího postupu.

#### <a name="configure-the-run-frequency"></a>Konfigurace četnosti spuštění
