---
title: Řešení úložiště Azure pro ML služby v HDInsight - Azure | Microsoft Docs
description: Informace o možnostech jiného úložiště k dispozici službou ML v HDInsight
services: hdinsight
documentationcenter: ''
author: nitinme
manager: jhubbard
...
ms.locfileid: "37052959"
---
# <a name="azure-storage-solutions-for-ml-services-on-azure-hdinsight"></a>Řešení úložiště Azure pro ML služby v Azure HDInsight

ML služby v HDInsight můžete použít celou řadu řešení úložiště k uchování dat, kódu nebo objektů, které obsahují výsledky z analýzy. Mezi ně patří následující možnosti:
...
## <a name="use-azure-blob-storage-accounts-with-ml-services-cluster"></a>Účty úložiště objektů Blob v Azure pomocí služby ML clusteru

Pokud jste zadali více než jeden účet úložiště při vytváření clusteru služby ML, následující pokyny popisují, jak používat sekundární účet pro přístup k datům a operace v ML služby clusteru. Předpokládejme následující účty úložiště a kontejneru: **storage1** výchozí kontejner s názvem **container1**, a **storage2** s **container2**.
...
### <a name="use-the-default-storage-with-ml-services-on-hdinsight"></a>Použít výchozí úložiště službou ML v HDInsight
...
### <a name="use-the-additional-storage-with-ml-services-on-hdinsight"></a>Použití dodatečné úložiště se službou ML Services v HDInsight
...
## <a name="use-an-azure-data-lake-store-with-ml-services-cluster"></a>Pomocí Azure Data Lake Store s clusterem služby ML 
...
### <a name="how-to-access-the-data-lake-store-from-ml-services-on-hdinsight"></a>Jak získat přístup k úložišti Data Lake ze služeb ML v HDInsight

Jakmile jste dali přístup do Data Lake Store, můžete použít úložiště v clusteru služby ML v HDInsight způsob, jakým byste účet sekundární úložiště Azure. Jediným rozdílem je, že předpona **wasb: / /** změny **adl: / /** následujícím způsobem:
...
## <a name="use-azure-file-storage-with-ml-services-on-hdinsight"></a>Používání Azure File storage s ML služby v HDInsight
...
* [Začínáme s clusterem služby ML systému Hadoop](r-server-get-started.md)
