...
Příklady scénářů přístupu k zabezpečeným prostředkům z ACR úlohy pomocí spravované identity najdete v těchto tématech:

* [Ověřování mezi registry](container-registry-tasks-cross-registry-authentication.md)
...
Spravovaná identita pro prostředky Azure poskytuje vybrané služby Azure s automaticky spravovanou identitou v Azure Active Directory (Azure AD). Pomocí spravované identity můžete nakonfigurovat úlohu ACR, aby tato úloha mohla přistupovat k jiným zabezpečeným prostředkům Azure bez předání přihlašovacích údajů v krocích úkolu.
...
Pokud plánujete použít uživatelem přiřazenou identitu, můžete použít existující identitu. Nebo vytvořte identitu pomocí rozhraní příkazového řádku Azure CLI nebo dalších nástrojů Azure. Použijte například příkaz [AZ identity Create][az-identity-create] . 

Pokud máte v úmyslu používat jenom identitu přiřazenou systémem, tento krok přeskočte. Při vytváření úlohy ACR můžete vytvořit identitu přiřazenou systémem.

### <a name="2-enable-identity-on-an-acr-task"></a>2. povolení identity v úloze ACR

Když vytvoříte úlohu ACR, můžete volitelně povolit uživatelsky přiřazenou identitu, identitu přiřazenou systémem nebo obojí. Například parametr `--assign-identity` předejte při spuštění příkazu [AZ ACR Task Create][az-acr-task-create] v rozhraní příkazového řádku Azure CLI.

Pokud chcete povolit identitu přiřazenou systémem, předejte `--assign-identity` bez hodnoty nebo `assign-identity [system]`. Následující příkaz vytvoří úlohu pro Linux z veřejného úložiště GitHub, které vytvoří image `hello-world` pomocí triggeru potvrzení Git a pomocí spravované identity přiřazené systémem:
...
Pokud chcete povolit uživatelsky přiřazenou identitu, předejte `--assign-identity` s hodnotou *ID prostředku* identity. Následující příkaz vytvoří úlohu pro Linux z veřejného úložiště GitHub, které vytvoří image `hello-world` pomocí triggeru potvrzení Git a pomocí spravované identity přiřazené uživatelem:
...
Ke správě přístupu na základě rolí k prostředkům použijte rozhraní příkazového [řádku Azure CLI](../role-based-access-control/role-assignments-cli.md) nebo jiné nástroje Azure. Například spuštěním příkazu [AZ role Assignment Create][az-role-assignment-create] přiřaďte identitu roli. 
...
Pokud úloha vyžádá nebo nahraje image do jiného služby Azure Container Registry, přidejte přihlašovací údaje k úloze pro ověření identity. Spuštěním příkazu [AZ ACR Task Credential Add přidejte][az-acr-task-credential-add] do úlohy přihlašovací údaje identity a předejte parametr `--use-identity`. 

Pokud například chcete přidat pověření pro identitu přiřazenou systémem k ověření pomocí registru *targetregistry*, předejte `use-identity [system]`:
...
Chcete-li přidat pověření pro uživatelem přiřazenou identitu k ověření pomocí registru *targetregistry*, předejte `use-identity` s hodnotou *ID klienta* identity. Příklad:
