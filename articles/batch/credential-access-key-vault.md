---
title: Zabezpečený přístup k Key Vault s využitím Batch-Azure Batch
...
ms.locfileid: "77463099"
---
# <a name="securely-access-key-vault-with-batch"></a>Zabezpečený přístup k Key Vault pomocí Batch
...
## <a name="obtain-a-certificate"></a>Získejte certifikát

Pokud certifikát ještě nemáte, nejjednodušší způsob, jak ho získat, je vytvořit certifikát podepsaný svým držitelem pomocí nástroje pro `makecert` příkazového řádku.

Obvykle můžete najít `makecert` v této cestě: `C:\Program Files (x86)\Windows Kits\10\bin\<arch>`. Otevřete příkazový řádek jako správce a přejděte na `makecert` pomocí následujícího příkladu.

```console
cd C:\Program Files (x86)\Windows Kits\10\bin\x64
```

Dále použijte nástroj `makecert` k vytvoření souborů certifikátů podepsaných svým držitelem nazvaných `batchcertificate.cer` a `batchcertificate.pvk`. Použitý běžný název (CN) není pro tuto aplikaci důležitý, ale je vhodné ho využít k tomu, abyste si využívali, k čemu se certifikát používá.

```console
makecert -sv batchcertificate.pvk -n "cn=batch.cert.mydomain.org batchcertificate.cer -b 09/23/2019 -e 09/23/2019 -r -pe -a sha256 -len 2048
```

Batch vyžaduje soubor `.pfx`. Pomocí nástroje [Pvk2Pfx](https://docs.microsoft.com/windows-hardware/drivers/devtest/pvk2pfx) Převeďte soubory `.cer` a `.pvk` vytvořené `makecert` na jeden soubor `.pfx`.
...
Dál je potřeba přiřadit k účtu Batch certifikát. Když k účtu přiřadíte certifikát, umožníme mu přiřadit fondy a pak uzly. Nejjednodušší způsob, jak to provést, je přejít na účet Batch na portálu, přejít na **certifikáty**a vybrat **Přidat**. Nahrajte soubor `.pfx`, který jsme vygenerovali v části [získání certifikátu](#obtain-a-certificate) , a zadejte heslo. Po dokončení se certifikát přidá do seznamu a tento kryptografický otisk můžete ověřit.
...
Pokud plánujete přístup k Key Vault pomocí skriptů PowerShellu na uzlech, budete potřebovat nainstalovanou knihovnu Azure PowerShell. Existuje několik způsobů, jak to provést, pokud jsou v uzlech nainstalované rozhraní Windows Management Framework (WMF) 5, můžete ho stáhnout pomocí příkazu install-Module. Pokud používáte uzly, které nemají WMF 5, nejjednodušší způsob, jak ji nainstalovat, je třeba, abyste soubor Azure PowerShell `.msi` nastavili pomocí dávkových souborů a pak jako první část spouštěcího skriptu Batch volali instalační program. Podrobnosti najdete v tomto příkladu:
