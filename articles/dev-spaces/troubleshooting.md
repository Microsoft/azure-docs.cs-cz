...
Azure Dev prostory poskytuje nativní podporu pro C# nebo Node.js. Když pracujete `azds prep` v adresáři s kódem napsaným v jednom z těchto jazyků, Azure dev Spaces pro vás automaticky vytvoří odpovídající souboru Dockerfile.
...
### <a name="error-release-azds-identifier-spacename-servicename-failed-services-servicename-already-exists-or-pull-access-denied-for-servicename-repository-does-not-exist-or-may-require-docker-login"></a>Chyba "Release azds-\<identifikátor\>-\<SpaceServiceNamesenezdařil-: služby"\<ServiceName\>\<\> \>' už existuje "nebo" přístup k získání byl \<odepřen\>pro ServiceName, úložiště neexistuje nebo může vyžadovat "Docker login"
...
Chcete-li tento problém vyřešit, `launch.json` aktualizujte soubor `.vscode` v podadresáři složky projektu. Změnit `configurations->cwd` směrnice tak, aby odkazoval do stejného adresáře jako `WORKDIR` definované ve vašem projektu `Dockerfile`. Budete muset taky aktualizovat `configurations->program` také směrnice.
...
Pokud chcete tento problém vyřešit, pomocí účtu s přístupem *vlastníka* nebo *přispěvatele* k předplatnému Azure `Microsoft.DevSpaces` ručně zaregistrujte obor názvů:
...
1. `azds.yaml` Upravte soubor a nastavte kontext sestavení na úroveň řešení.
2. Upravte soubory `Dockerfile.develop` `.csproj`a tak, aby odkazovaly na soubory projektu, například správně vzhledem k novému kontextu sestavení. `Dockerfile`
3. Přidejte do stejného adresáře `.sln` jako soubor. `.dockerignore`
