...
Azure AD B2C podporuje [protokoly OAuth 2,0 a OpenID Connect](active-directory-b2c-reference-protocols.md), které využívají tokeny pro ověřování a zabezpečený přístup k prostředkům. Všechny tokeny používané v Azure AD B2C jsou [webové tokeny JSON (JWTs)](https://self-issued.info/docs/draft-ietf-oauth-json-web-token.html) , které obsahují kontrolní výrazy informací o nosiči a předmětu tokenu.
...
Při použití Azure AD B2C máte jemně odstupňovanou kontrolu nad obsahem vašich tokenů. Můžete nakonfigurovat [toky uživatelů](active-directory-b2c-reference-policies.md) a [vlastní zásady](active-directory-b2c-overview-custom.md) , které odesílají určité sady uživatelských dat v deklaracích identity, které jsou nutné pro vaši aplikaci. Tyto deklarace můžou zahrnovat standardní vlastnosti, jako jsou **DisplayName** a **EmailAddress**. Vaše aplikace můžou tyto deklarace identity použít k bezpečnému ověření uživatelů a požadavků.

Deklarace identity v tokenech ID se nevrací v žádném konkrétním pořadí. Nové deklarace identity je možné kdykoli zavést v tokenech ID. Vaše aplikace by se neměla porušit při zavedení nových deklarací. V deklaracích můžete také zahrnout [vlastní atributy uživatele](active-directory-b2c-reference-custom-attr.md) .
...
| Name | Deklarace identity | Příklad hodnoty | Popis |
| ---- | ----- | ------------- | ----------- |
...
| Scope | `scp` | `Read`| Oprávnění udělená prostředku pro přístupový token. Vícenásobná udělená oprávnění jsou oddělená mezerou. |
...
Po zahájení cesty uživatele Azure AD B2C obdrží přístupový token od poskytovatele identity. Azure AD B2C používá tento token k načtení informací o uživateli. [V toku uživatele povolíte deklaraci identity](idp-pass-through-user-flow.md) nebo [ve svých vlastních zásadách definujete deklaraci identity](idp-pass-through-custom.md) , abyste mohli token předat do aplikací, které zaregistrujete v Azure AD B2C. Aby bylo možné využít výhod předávání tokenu jako deklarace identity, musí vaše aplikace používat [uživatelský tok v2](user-flow-versions.md) .
...
Dokument metadat pro `B2C_1_signupsignin1` zásadu `contoso.onmicrosoft.com` v tenantovi se nachází v těchto umístěních:
