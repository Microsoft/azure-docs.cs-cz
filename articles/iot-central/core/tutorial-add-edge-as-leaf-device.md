...
1. Vyberte **+** pro přidání nového zařízení ze šablony. Na stránce **vytvořit nové zařízení** vyberte **vytvořit**.
...
1. Stisknutím klávesy **ENTER** zobrazíte `login:`ovou výzvu. Zadejte své uživatelské jméno a heslo pro přihlášení.
...
1. Pomocí editoru `nano` otevřete soubor IoT Edge config. yaml:
...
1. Nahraďte `{scope_id}` **rozsahem ID** , kde jste si poznamenali předchozí poznámku.
...
1. Uložte změny (**CTRL-O**) a ukončete Editor `nano` (**CTRL-X**).
