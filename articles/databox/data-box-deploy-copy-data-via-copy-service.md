...
3. V **konfigurace a spuštění** dialogovém okně zadejte následující vstupy.
    ...
    |Uživatelské jméno                       |Uživatelské jméno pro přístup ke zdroji dat.        |
    ...
    |Optimalizace souboru              |Když je povoleno, soubory jsou zkomprimována na ingestování. Tím se urychlí kopírování dat pro malé soubory.        |
...
5. Vytvoření úlohy se zadanými nastaveními. Zaškrtněte políčko a potom můžete pozastavit a obnovit, zrušit nebo restartujte úlohu.
    ...
    - Je-li Optimalizace souborů na, jsou zkomprimována malých souborů na ingestování můžete zvýšit výkon kopírování. V těchto případech se zobrazí komprimovaný soubor (jako název GUID), jak je znázorněno na následujícím snímku obrazovky.

        ![Příklad zkomprimovaný soubor](media/data-box-deploy-copy-data-via-copy-service/packed-file-on-ingest.png)
