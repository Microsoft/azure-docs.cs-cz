...
* [SQL Server na virtuálních počítačích Azure:](https://azure.microsoft.com/services/virtual-machines/sql-server/) SQL Server nainstalovaný a hostovaný v cloudu na virtuálních počítačích s Windows Serverech běžící v Azure označovaný také jako IaaS.
  SQL Server na virtuálních počítačích Azure je stejného softwaru SQL Server, který by nainstalovat na serveru místní - jediným rozdílem je, že tento server je teď virtuální počítač hostovaný v rámci Azure. SQL Server v Azure je optimalizovaná pro migraci existujících aplikací systému SQL Server a všechny verze a edice systému SQL Server jsou k dispozici. Umožňuje hostování jako mnoho databáze podle potřeby, podporuje provádění mezidatabázové transakce a nabízí plnou kontrolu nad instance systému SQL Server a operačního systému Windows.
...
|  | Týmy, které mají Microsoft ke správě jejich databáze upgrady, vysokou dostupnost a zotavení po havárii. |Týmy, které mohou konfigurovat a spravovat vysokou dostupnost, zotavení po havárii a použití dílčích oprav systému SQL Server. Některé poskytované automatizované funkce to značně zjednodušují. | |
