...
Agent Log Analytics pro Linux se skládá z více balíčků. Soubor verze obsahuje následující balíčky, které jsou k dispozici spuštěním sady prostředků prostředí s parametrem `--extract`:
...
* Soubor *zahrnutí* sudoers se vytvoří v `/etc/sudoers.d/omsagent`. To zmocňuje `omsagent` k restartování procesu démona syslog a omsagent. Pokud direktivy *include* sudo nejsou podporovány v nainstalované verzi sudo, budou tyto položky zapsány do `/etc/sudoers`.
...
2. Nainstalujte sadu prostředků pomocí argumentu `--install`. Chcete-li se připojit k pracovnímu prostoru Log Analytics během instalace, zadejte parametry `-w <WorkspaceID>` a `-s <workspaceKey>`, které jste zkopírovali dříve.
    ...
    >[!NOTE]
    >Pokud jsou nainstalované všechny závislé balíčky, jako je OMI, SCX, omsconfig nebo jejich starší verze, je potřeba použít argument `--upgrade`, jako by to byl případ, kdy už je nainstalovaný Agent nástroje System Center Operations Manager pro Linux. 
...
Pokud počítač se systémem Linux potřebuje komunikovat prostřednictvím proxy server k Log Analytics, lze tuto konfiguraci zadat na příkazovém řádku, a to vložením `-p [protocol://][user:password@]proxyhost[:port]`. Vlastnost *protokolu* přijímá `http` nebo `https`a vlastnost *ProxyHost* přijímá plně kvalifikovaný název domény nebo IP adresu proxy server. 
...
1. Chcete-li počítač se systémem Linux nakonfigurovat pro připojení k Log Analyticsmu pracovnímu prostoru, spusťte následující příkaz, který poskytuje ID pracovního prostoru a primární klíč. Tento příkaz stáhne agenta, ověří jeho kontrolní součet a nainstaluje ho.
    ...
    Následující příkaz obsahuje parametr `-p` proxy a ukázkovou syntaxi, pokud proxy server vyžaduje ověření:
...
Upgrade z předchozí verze, počínaje verzí 1.0.0-47, je podporován v každé vydané verzi. Chcete-li upgradovat všechny součásti agenta na nejnovější verzi, proveďte instalaci pomocí parametru `--upgrade`.
