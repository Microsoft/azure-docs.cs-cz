...
## <a id="logout-adal"></a>Implementace správného odhlášení pomocí metod ADAL při použití Azure AD
...
## <a id="finite-tokens"></a>Použití konečných životností pro vygenerované tokeny SaS
...
## <a id="resource-tokens"></a>Použití minimální životnosti tokenů pro vygenerované tokeny prostředků
...
## <a id="wsfederation-logout"></a>Implementace správného odhlášení pomocí metod WsFederation při použití ADFS
...
## <a id="proper-logout"></a>Implementace správného odhlášení při použití serveru identity
...
## <a id="https-secure-cookies"></a>Aplikace dostupné přes HTTPS musí používat zabezpečené soubory cookie.
...
## <a id="cookie-definition"></a>Všechny aplikace založené na protokolu HTTP by měly určovat pouze http pro definici souboru cookie.
...
| **Kroky** | Hodnota vlastnosti vlastnost RequireSSL je nastavena v konfiguračním souboru pro aplikaci ASP.NET pomocí atributu vlastnost requireSSL konfiguračního elementu. Můžete zadat v souboru Web. config pro aplikaci ASP.NET, zda je vyžadován protokol SSL (SSL (Secure Sockets Layer)) pro vrácení souboru cookie ověřování formulářů na server nastavením atributu vlastnost requireSSL.|
...
## <a id="csrf-asp"></a>Zmírnění útoků proti útokům přes CSRF (site-to Request) na webových stránkách ASP.NET
...
| **Atribut**              | Nevztahuje se  |
| **Odkazy**              | [Prevence XSRF/CSRF v ASP.NET MVC a webových stránkách](https://www.asp.net/mvc/overview/security/xsrfcsrf-prevention-in-aspnet-mvc-and-web-pages) |
| **Kroky** | Anti-CSRF a ASP.NET MVC Forms – použití pomocné metody `AntiForgeryToken` v zobrazeních; Vložte `Html.AntiForgeryToken()` do formuláře, například|
...
### <a name="example"></a>Příklad:
Ve stejnou dobu HTML. AntiForgeryToken () přiřadí návštěvníkovi soubor cookie s názvem __RequestVerificationToken a stejnou hodnotou jako náhodná Skrytá hodnota uvedená výše. Dále pro ověření příchozího příspěvku formuláře přidejte filtr [ValidateAntiForgeryToken] do metody cíle akce. Například:
```
[ValidateAntiForgeryToken]
...
* Příchozí požadavek obsahuje položku `Request.Form` s názvem __RequestVerificationToken
* Tyto soubory cookie a `Request.Form` se shodují s předpokladem, že jsou všechny správné, požadavek prochází běžným způsobem. Ale v takovém případě nedošlo k chybě autorizace se zprávou "vyžadovaný token proti padělání se nezadal nebo byl neplatný". 
...
## <a id="inactivity-lifetime"></a>Nastavte relaci pro dobu neaktivity.
...
| **Kroky** | Když je webová aplikace předávající stranou a služba AD FS je STS, doba života souborů cookie ověřování – FedAuth tokeny-lze nastavit pomocí následující konfigurace v souboru Web. config:|
...
## <a id="proper-app-logout"></a>Implementace správného odhlášení z aplikace
...
## <a id="csrf-api"></a>Zmírnění útoků na webové rozhraní API v ASP.NET proti útokům přes lokalitu (CSRF)
...
| **Fáze SDL**               | Build |  
| **Použitelné technologie** | MVC5, MVC6 |
| **Atribut**              | Zprostředkovatel identity – ADFS, zprostředkovatel identity – Azure AD |
...
### <a name="example"></a>Příklad:
Webové rozhraní API je potřeba informovat, aby se mohlo spoléhat jenom na tokeny nosiče a ne na soubory cookie. To lze provést pomocí následující konfigurace v `WebApiConfig.Register` metody:
