...
4. Zadáním: `Get-HcsSystem` ověřte, zda je protokol HTTP povolený.
...
5. Spusťte relaci prostředí Windows PowerShell na zařízení zadáním tohoto příkazu:
   ...
   > [!NOTE]
   > Pokud chcete vytvořit relaci Windows PowerShellu pro použití s virtuálním zařízením StorSimple, přidejte parametr `–Port` a zadejte veřejný port, který jste nakonfigurovali v části Vzdálená komunikace pro virtuální zařízení StorSimple.
...
6. Zkopírujte informace v certifikátu z **-----začít s certifikátem-----** do textového editoru, jako je Poznámkový blok **----------** , a uložte ho jako soubor. cer. (Tento soubor budete kopírovat na svého vzdáleného hostitele při přípravě hostitele.)
   ...
   > [!NOTE]
   > Pokud chcete vygenerovat nový certifikát, použijte rutinu `Set-HcsRemoteManagementCert`.
...
Pomocí prostředí Windows PowerShell a protokolu SSL zadejte do zařízení relaci SSAdmin ze vzdáleného hostitele nebo klienta. Relace SSAdmin se mapuje na možnost 1 v nabídce [konzoly sériového portu](storsimple-8000-windows-powershell-administration.md#connect-to-windows-powershell-for-storsimple-via-the-device-serial-console) vašeho zařízení.

V počítači, ze kterého chcete vzdálené připojení Windows PowerShell, proveďte následující postup.

#### <a name="to-enter-an-ssadmin-session-on-the-device-by-using-windows-powershell-and-ssl"></a>Zadání relace SSAdmin do zařízení pomocí Windows PowerShellu a protokolu SSL
1. Spusťte relaci Windows PowerShellu jako správce. Pokud používáte klienta Windows 10, je ve výchozím nastavení služba Vzdálená správa systému Windows nastavena na ruční. Službu možná budete muset spustit zadáním:
...
6. Budete muset několik minut počkat a pak se k zařízení připojíte přes HTTPS přes SSL. Zobrazí se zpráva s informacemi o tom, že jste připojení k vašemu zařízení.
   
    ![Vzdálená komunikace PowerShellu pomocí protokolu HTTPS a SSL](./media/storsimple-remote-connect/HCS_PSRemotingUsingHTTPSAndSSL.png)
