...
1. V VS Code otevřete `CMakeLists.txt` souboru ve složce `azure-iot-sdk-c`. Ujistěte se, že jste otevřeli soubor `CMakeLists.txt` ve složce `azure-iot-sdk-c`, nikoli ve složce `devkit_device`.

1. Do dolní části `CMakeLists.txt` souboru přidejte řádek, který bude při kompilování zahrnovat složku se zástupnými procedurami kódu zařízení:

    ```txt
    add_subdirectory(central_app/devkit_device)
    ```

1. Vytvořte složku `cmake` ve složce `azure-iot-sdk-c` a přejděte do této složky z příkazového řádku:
