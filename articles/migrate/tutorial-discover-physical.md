---
title: Zjištění fyzických serverů pomocí posouzení serveru Azure Migrate
description: Naučte se zjišťovat místní fyzické servery s Azure Migrate posouzení serveru.
ms.topic: tutorial
ms.date: 09/14/2020
ms.custom: mvc
...
V tomto kurzu se dozvíte, jak zjišťovat místní fyzické servery pomocí nástroje Azure Migrate: Nástroj pro vyhodnocení serveru s využitím odlehčeného Azure Migrateho zařízení. Zařízení nasadíte jako fyzický server, abyste mohli průběžně zjišťovat metadata o počítači a výkonu.
...
- V případě serverů s Linuxem musíte na serverech s Linuxem, které chcete vyhledat, mít účet superuživatele. Případně nastavte přístup následujícím způsobem:
    - setcap CAP_DAC_READ_SEARCH + EIP/usr/sbin/Fdisk
    - setcap CAP_DAC_READ_SEARCH + EIP/sbin/fdisk (Pokud není k dispozici/usr/sbin/Fdisk)<br/> -setcap "cap_dac_override, cap_dac_read_search, cap_fowner, cap_fsetid, cap_setuid, cap_setpcap, cap_net_bind_service, cap_net_admin, cap_sys_chroot, cap_sys_admin, cap_sys_resource, cap_audit_control, cap_setfcap = + EIP"/sbin/LVM
...
Pokud chcete zařízení nastavit na fyzickém počítači, Stáhněte a spusťte skript instalačního programu na [vhodném počítači](#prerequisites). Po vytvoření zařízení ho nastavte poprvé a zaregistrujte ho v Azure Migrate projektu.
...
1. V **Azure Migrate registrovat v**vyberte **Přihlásit**. Pokud se nezobrazí, ujistěte se, že jste v prohlížeči zakázali blokování automaticky otevíraných oken.
    ...
    ![Kliknutím na přihlašovací jméno spusťte registraci zařízení.](./media/tutorial-discover-physical/register.png)

2. Na **přihlašovací** stránce se přihlaste pomocí uživatelského jména a hesla Azure. Přihlášení pomocí PIN kódu se nepodporuje.
3. Po úspěšném přihlášení se vraťte do aplikace.
4. V části **zaregistrovat v Azure Migrate**vyberte předplatné, ve kterém byl vytvořen Azure Migrate projekt, a pak vyberte projekt.
5. Zadejte název zařízení. Název by měl být alfanumerický a nesmí obsahovat více než 14 znaků.
6. Vyberte **Zaregistrovat**. Pak klikněte na **pokračovat**. Zpráva zobrazuje registraci jako úspěšnou.

    ![Vyplňte předplatné, projekt a název zařízení a zaregistrujte zařízení.](./media/tutorial-discover-physical/success-register.png)

## <a name="start-continuous-discovery"></a>Spustit průběžné zjišťování

Zařízení se připojí k fyzickému serveru pro zjišťování.
