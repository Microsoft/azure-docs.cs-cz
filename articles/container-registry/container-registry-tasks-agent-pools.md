...
- **Škálujte podle potřeby** – zvyšte počet instancí ve fondu agentů pro úlohy náročné na výpočetní výkon nebo nastavte škálování na nulu. Fakturace je založena na přidělení.
...
- Fondy agentů úloh jsou k dispozici ve verzi Preview v následujících oblastech: Západní USA 2, Střed USA – jih, Východní USA 2 a Východní USA.
- Pro každý registr je výchozí celková kvóta vCPU (jádro) pro všechny fondy agentů 16. Otevřete [žádost o podporu][open-support-ticket] pro další přidělení.
...
V následujících příkladech se předpokládá, že jste nastavili výchozí registr. V takovém případě předejte `--registry <registryName>` parametr v `az acr` každém příkazu.
...
Dokončení vytváření fondu agentů a dalších operací správy fondů bude trvat několik minut.
...
| Směr | Protocol (Protokol) | Zdroj         | Zdrojový port | Cíl          | Cílový port | Pro    |
|-----------|----------|----------------|-------------|----------------------|-----------|---------|
...
> [!NOTE]
> Pokud vaše úkoly vyžadují další prostředky z veřejného Internetu, přidejte odpovídající pravidla. Tato pravidla jsou například nutná, pokud spustíte úlohu sestavení Docker, která potřebuje načíst základní image z Docker Hub nebo obnovit balíček NuGet.
...
### <a name="quick-task-run"></a>Spuštění rychlého úkolu

Zařadit do fronty rychlý běh fondu agentů pomocí příkazu [AZ ACR Build][az-acr-build] a předat `--agent-pool` parametr:
