...
Tento atribut je jen pro interní použití.
|Atribut|Hodnota|
|---|---|
|jméno|IsActivator|
|type|xs:Boolean|
...
Název oddílu Settings.xml složce ConfigPackage.
|Atribut|Hodnota|
|---|---|
|jméno|SectionName|
|Použití|povinné|
...
|Atribut|Hodnota|
|---|---|
|content|prvky wsdl: 10, 8 atributy|
|Definice|Globálně|
...
Použijte výchozí úložiště přihlašovacích údajů zadaných v clusterManifest.xml místo přihlašovací údaje zadané ve značce "RepositoryCredentials" v souboru ApplicationManifest.xml.
...
Použití MSI ověřování pomocí tokenu (nebo vlastní zadaný koncový bod) k získání tokenu používaného k ověřování.
...
Režim izolace pro kontejner. Platné hodnoty jsou výchozí, procesu nebo Hyper-v (podporováno pouze pro kontejnery windows).
|Atribut|Hodnota|
|---|---|
|jméno|Izolace|
|Použití|nepovinné|
...
Příznak označující, zda se automaticky provedou odstranění kontejnerů (pravda) nebo ne (false), pokud kontejner je ukončen.  Pokud false SF ručně odebrat kontejnery.
|Atribut|Hodnota|
|---|---|
|jméno|AutoRemove|
|Použití|nepovinné|
...
NetworkType. Aktuálně podporované typy jsou "Otevřená" a "Izolovaný režim".
|Atribut|Hodnota|
|---|---|
|jméno|NetworkType|
|Použití|povinné|
...
|content|prvky wsdl: 0, 1 atributy|
|Definice|Globálně|
|jméno|ContainerNetworkPolicyEndpointBindingType|
...
Popisuje přidružení mezi balíček služby a kontejner sítě a volitelně přidružení mezi koncovými body balíčku služby a sítě kontejneru.

|Atribut|Hodnota|
|---|---|
|content|prvky wsdl: 1, 1 atributy|
|Definice|Globálně|
|jméno|ContainerNetworkPolicyType|
...
Název kontejneru sítě (velká a malá písmena). Musí to být název kontejneru sítě již vytvořené nebo vyhrazené názvy, včetně "Otevřít" nebo "NAT".
|Atribut|Hodnota|
|---|---|
|jméno|NetworkRef|
|Použití|povinné|
...
|Atribut|Hodnota|
|---|---|
|content|4 elementy, atributy 10|
|Definice|Globálně|
...
|type|xs:String|
|Použití|nepovinné|
|default|Ve formátu prostého textu|
...
#### <a name="name"></a>Název
Název koncového bodu pro přepsání. Řetězec.
|Atribut|Hodnota|
|---|---|
|jméno|Název|
|Použití|povinné|
...
Protokol pro koncový bod. Přepíše hodnotu v manifestu služby. Koncové body HTTPS, musíte mít také EndpointCertificate a EndpointBindingPolicy deklarované v manifestu aplikace. Protokol nelze změnit později v upgrade aplikace. 
|Atribut|Hodnota|
|---|---|
...
#### <a name="type"></a>Typ
Typ koncového bodu. Přepíše hodnotu v manifestu služby. Vstupní koncové body se používají k vystavení portů na vnější koncovým bodům s interním se používají ke komunikaci uvnitř aplikace.
|Atribut|Hodnota|
|---|---|
|jméno|Typ|
|type|xs:String|
...
#### <a name="protocol"></a>Protocol (Protokol)
Protokol pro koncový bod: http, https, tcp nebo udp. Koncové body HTTPS, musíte mít také EndpointCertificate a EndpointBindingPolicy deklarované v manifestu aplikace. Protokol nelze změnit později v upgrade aplikace. 
|Atribut|Hodnota|
|---|---|
|jméno|Protocol (Protokol)|
|Použití|nepovinné|
...
|Atribut|Hodnota|
|---|---|
|content|21. elementy, atributy 0|
|Definice|Globálně|
...
Popisuje zásady sítě, včetně kontejneru síťové zásady pro balíček služby.

|Atribut|Hodnota|
|---|---|
|content|prvky wsdl: 1, 0 atributy|
|Definice|Globálně|
|jméno|NetworkPoliciesType|
...
#### <a name="passwordencrypted"></a>PasswordEncrypted
Určuje, jestli je heslo v zašifrované nebo v podobě prostého textu. Tento atribut je zastaralý. Použijte k označení šifrované heslo atributu 'Type'.
|Atribut|Hodnota|
|---|---|
|jméno|PasswordEncrypted|
|type|xs:Boolean|
...
|Atribut|Hodnota|
|---|---|
|content|0 elementy, atributy 12|
|Definice|Globálně|
...
Jádra omezení paměti v bajtech.  Musí být kladné celé číslo.  Všimněte si, že toto je konkrétní Linuxu a dockeru v systému windows skončí chybou, pokud je nastavené.
|Atribut|Hodnota|
|---|---|
...
Velikost v bajtech shm/dev /. Pokud tento parametr vynechán, systém použije 64MB.  Musí být kladné celé číslo.  Všimněte si, že jde o Linuxu konkrétní, ale docker se NEVYGENERUJE chybu, pokud zadaný.  Jednoduše je ignorován.
|Atribut|Hodnota|
|---|---|
|jméno|ShmSizeInMB|
|type|xs:String|
...
Udělení oprávnění: Čtení, změna nebo úplná. Výchozí hodnota je pro čtení.
|Atribut|Hodnota|
|---|---|
|jméno|GrantRights|
|default|Čtení|
...
Typ prostředku definován v oddíle prostředků v manifestu služby. Koncový bod nebo certifikát.
|Atribut|Hodnota|
|---|---|
|jméno|ResourceType|
|Použití|nepovinné|
...
Označuje, pokud modul Runtime Fabric konkrétní informace by měly být vystaveny balíčky kódu v balíčku služby.
|Atribut|Hodnota|
|---|---|
|jméno|RemoveServiceFabricRuntimeAccess|
|type|xs:Boolean|
|default|false (nepravda)|

#### <a name="useservicefabricreplicatedstore"></a>UseServiceFabricReplicatedStore
Označuje, pokud Service Fabric replikované blockstore by měla sloužit k poskytnutí svazku pro kontejnery v balíčku služby.
|Atribut|Hodnota|
|---|---|
|jméno|UseServiceFabricReplicatedStore|
|type|xs:Boolean|
|default|false (nepravda)|
...
|Atribut|Hodnota|
|---|---|
|content|11. elementy, atributy 0|
|Definice|Globálně|
...
#### <a name="isolation"></a>Izolace
Režim izolace pro kontejner. Platné hodnoty jsou "Výchozí", "proces" nebo "Hyper-v".
|Atribut|Hodnota|
|---|---|
|jméno|Izolace|
|Použití|nepovinné|
...
|Atribut|Hodnota|
|---|---|
|content|11. elementy, atributy 4|
|Definice|Globálně|
...
Určuje maximální přípustný procento služby není v pořádku, než se aplikace se považoval za poškozený. Výchozí procentuální hodnotu: 0.
|Atribut|Hodnota|
|---|---|
...
Určuje maximální přípustný procento poškozené oddíly, než se služba se považuje za není v pořádku. Výchozí procentuální hodnotu: 0.
|Atribut|Hodnota|
|---|---|
...
Určuje maximální přípustný procento poškozené repliky, než se oddíl se považoval za poškozený. Výchozí procentuální hodnotu: 0.
|Atribut|Hodnota|
|---|---|
