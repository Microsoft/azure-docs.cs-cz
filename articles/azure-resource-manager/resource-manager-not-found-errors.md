---
title: Nenašly se chyby prostředku Azure | Microsoft Docs
...
Použijete-li funkci [reference](resource-group-template-functions-resource.md#reference) nebo [klíče listkey](resource-group-template-functions-resource.md#listkeys) s prostředkem, který nelze vyřešit, dojde k následující chybě:
...
Ale chcete se vyhnout nastavení závislostí, které nepotřebujete. Pokud máte zbytečné závislosti, prodloužete dobu trvání nasazení tím, že zabráníte souběžnému nasazení prostředků, které na sobě nejsou závislé. Kromě toho můžete vytvořit cyklické závislosti, které zablokují nasazení. Funkce [reference](resource-group-template-functions-resource.md#reference) a [list *](resource-group-template-functions-resource.md#list) vytvoří implicitní závislost na odkazovaném prostředku, pokud je tento prostředek nasazený ve stejné šabloně a odkazuje na jeho název (nikoli ID prostředku). Proto je možné, že budete mít více závislostí než závislosti zadané ve vlastnosti **dependsOn** . Funkce [ResourceID](resource-group-template-functions-resource.md#resourceid) nevytvoří implicitní závislost nebo ověří, zda prostředek existuje. Funkce [reference](resource-group-template-functions-resource.md#reference) a [list *](resource-group-template-functions-resource.md#list) nevytvoří implicitní závislost, pokud je prostředek odkazován podle jeho ID prostředku. Chcete-li vytvořit implicitní závislost, předejte název prostředku, který je nasazen ve stejné šabloně.
...
1. Vyberte historii nasazení pro skupinu prostředků.
   ...
   ![Vybrat historii nasazení](./media/resource-manager-not-found-errors/select-deployment.png)

2. Vyberte nasazení z historie a vyberte **události**.

   ![vybrat události nasazení](./media/resource-manager-not-found-errors/select-deployment-events.png)
...
Pokud prostředek existuje v jiné skupině prostředků než ta, která je nasazena do, použijte [funkci ResourceID](resource-group-template-functions-resource.md#resourceid) k získání plně kvalifikovaného názvu prostředku.
