---
title: Kurz volání aplikace LUIS (Language Understanding Intelligent Service) pomocí Javy | Microsoft Docs
...
ms.locfileid: "36263716"
---
# <a name="tutorial-call-a-luis-endpoint-using-java"></a>Kurz: Volání koncového bodu služby LUIS pomocí Javy
...
4. Vraťte se na web [LUIS](luis-reference-regions.md) a přihlaste se pomocí svého účtu Azure. 
    ...
    [![](media/luis-get-started-java-get-intent/app-list.png "Snímek obrazovky se seznamem aplikací")](media/luis-get-started-java-get-intent/app-list.png)
...
2. Vložte adresu URL do okna prohlížeče a stiskněte Enter. V prohlížeči se zobrazí výsledek JSON, který značí, že služba LUIS rozpoznala záměr `HomeAutomation.TurnOn` a entitu `HomeAutomation.Room` s hodnotou `bedroom`.
    ...
    ![Výsledek JSON s rozpoznaným záměrem TurnOn (Zapnout)](./media/luis-get-started-java-get-intent/turn-on-bedroom.png)
3. Změňte hodnotu parametru `q=` v adrese URL na `turn off the living room light` a stiskněte Enter. Výsledek teď značí, že služba LUIS rozpoznala záměr `HomeAutomation.TurnOff` a entitu `HomeAutomation.Room` s hodnotou `living room`. 
    ...
    ![Výsledek JSON s rozpoznaným záměrem TurnOff (Vypnout)](./media/luis-get-started-java-get-intent/turn-off-living-room.png)


## <a name="consume-a-luis-result-using-the-endpoint-api-with-java"></a>Využití výsledku ze služby LUIS pomocí rozhraní API pro koncové body a Javy 

Pomocí Javy můžete získat přístup ke stejným výsledkům, jako jste viděli v okně prohlížeče v předchozím kroku. 
1. Zkopírujte následující kód a vytvořte ve svém integrovaném vývojovém prostředí (IDE) třídu:
...
2. Nahraďte hodnotu proměnné `SubscriptionKey` klíčem vašeho předplatného LUIS.

3. V integrovaném vývojovém prostředí (IDE) přidejte odkazy na knihovny `httpclient` a `httpcore`.
