...
1. In the **Basics** tab, use the VM settings as specified in the table below the image:
    ...
    | ------------ | ---------------- | ---------------- |
    | **Předplatné** | Vaše předplatné | The subscription under which your resources are created. | 
    ...
    | **[Region](https://azure.microsoft.com/regions/)** | (Evropa) Evropa – západ | Choose a region near you or near the functions that access the VM. |
    | **Velikost** | B1s | Choose **Change size** and then select the B1s standard image, which has 1 vCPU and 1 GB of memory. |
    | **Authentication type** | Heslo | To use password authentication, you must also specify a **Username**, a secure **Password**, and then **Confirm password**. For this tutorial, you won't need to sign in to the VM unless you need to troubleshoot. |
...
1. In **Create virtual network**, use the settings in the table below the image:
    ...
    | Nastavení      | Navrhovaná hodnota  | Popis      |
    | ------------ | ---------------- | ---------------- |
    | **Název** | myResourceGroup-vnet | You can use the default name generated for your virtual network. |
    | **Rozsah adres** | 10.10.0.0/16 | Use a single address range for the virtual network. |
    | **Název podsítě** | Tutorial-Net | Name of the subnet. |
...
1. Vyberte **Zkontrolovat a vytvořit**. After validation completes, select **Create**. The VM create process takes a few minutes. The created VM can only access the virtual network.
...
1. In **Network Feature Status**, use the settings in the table below the image:
    ...
    | **Podsíť** | Create New Subnet | Create a subnet in the virtual network for your function app to use. VNet Integration must be configured to use an empty subnet. It doesn't matter that your functions use a different subnet than your VM. The virtual network automatically routes traffic between the two subnets. |
    ...
    | **Rozsah adres** | 10.10.2.0/24   | The subnet size restricts the total number of instances that your Premium plan function app can scale out to. This example uses a `/24` subnet with 254 available host addresses. This subnet is over-provisioned, but easy to calculate. |
...
1. In your function app, select  **Proxies** >  **+** , then use the proxy settings in the table below the image:
    ...
    | Nastavení  | Navrhovaná hodnota  | Popis      |
    | -------- | ---------------- | ---------------- |
    | **Název** | Plant | The name can be any value. It's used to identify the proxy. |
