...
- [Pracovní prostor analýzy Azure synapse](../get-started-create-workspace.md). Ujistěte se, že má následující účet úložiště nakonfigurovaný jako výchozí úložiště: Azure Data Lake Storage Gen2. U Data Lake Storage Gen2 systému souborů, se kterými pracujete, se ujistěte, že jste **přispěvatelem dat objektů BLOB úložiště**.
- Fond Apache Spark ve vašem pracovním prostoru Azure synapse Analytics. Podrobnosti najdete v tématu [rychlý Start: vytvoření vyhrazeného fondu SQL pomocí Azure synapse Analytics studia](../quickstart-create-sql-pool-studio.md).
...
1. Naimportujte Poznámkový blok do Azure synapse Analytics studia.
![Snímek obrazovky Azure synapse Analytics s zvýrazněnou možností import](media/tutorial-automl-wizard/tutorial-automl-wizard-00a.png)

1. Vyberte fond Spark, který chcete použít, a vyberte **Spustit vše**. Tím se získá data New York taxislužby z otevřené datové sady a uloží se do vaší výchozí databáze Spark.
...
![Snímek karty data Azure synapse Analytics s zvýrazněnou novou tabulkou](media/tutorial-automl-wizard/tutorial-automl-wizard-00c.png)

## <a name="launch-automated-machine-learning-wizard"></a>Spustit Průvodce automatickým Machine Learningem

Jak na to:

1. Klikněte pravým tlačítkem na tabulku Spark, kterou jste vytvořili v předchozím kroku. Průvodce otevřete tak, že vyberete **Machine Learning**  >  **obohacení s novým modelem**.
...
1. Pak můžete zadat podrobnosti o konfiguraci pro vytvoření automatizovaného experimentu Machine Learningu v Azure Machine Learning. Tato akce spustí více modelů a nejlepší model z úspěšného spuštění je zaregistrován v registru Azure Machine Learningho modelu.

   ![Snímek obrazovky s obohacením s novými specifikacemi konfigurace modelu](media/tutorial-automl-wizard/tutorial-automl-wizard-configure-run-00a.png)
    ...
    - **Fond Spark**: fond Spark, který chcete použít k automatizovanému spuštění experimentu. Výpočty se spouští ve fondu, který zadáte.
...
Vyberte typ modelu strojového učení pro experiment na základě otázky, na kterou se snažíte odpovědět. Vzhledem k tomu, že `fareAmount` je cílový sloupec a jedná se o číselnou hodnotu, vyberte možnost **regrese** . Potom vyberte **Pokračovat**.
...
Pokud vyberete možnost **regrese** nebo **klasifikace** jako typ modelu v předchozí části, jsou k dispozici následující konfigurace:

- **Primární metrika**: metrika používaná k měření, jak dobře model pracuje. Toto je metrika, která slouží k porovnání různých modelů vytvořených v rámci automatizovaného spuštění a určení modelu, který se vykonal nejlépe.

- **Čas úlohy školení (hodiny)**: maximální množství času (v hodinách), ve kterém se má experiment spouštět a naučit modely. Všimněte si, že můžete také poskytnout hodnoty menší než 1 (například `0.5` ).

- **Maximální počet souběžných iterací**: představuje maximální počet paralelně spuštěných iterací.
...
![Snímek obrazovky s obohacením nového modelu o další konfigurace](media/tutorial-automl-wizard/tutorial-automl-wizard-configure-run-00c.png)

Po dokončení všech požadovaných konfigurací můžete spustit automatizovaný běh. Můžete zvolit **vytvořit běh**, který spustí přímo, bez kódu. Případně, pokud dáváte přednost kódu, můžete vybrat **otevřít v poznámkovém bloku**. Tato možnost umožňuje zobrazit kód, který vytvoří běh a spustí Poznámkový blok.

>[!NOTE]
>Pokud v předchozí části vyberete možnost **prognózování časových řad** jako typ modelu, je nutné provést další konfigurace. Prognózy také nepodporují kompatibilitu modelu ONNX.

### <a name="create-run-directly"></a>Vytvořit přímo spustit

Pokud chcete spustit automatický běh machine learningu přímo, vyberte **Spustit spustit**. Zobrazí se oznámení, které indikuje, že spuštění probíhá. Pak se zobrazí další oznámení oznamující úspěch. Můžete také zjistit stav v Azure Machine Learning tím, že vyberete odkaz v oznámení.
...
Pokud chcete vygenerovat Poznámkový blok, vyberte **otevřít v poznámkovém bloku**. Pak vyberte **Spustit vše**. Získáte také možnost přidat další nastavení do automatizovaného spuštění machine learningu.
