...
1. Vytvořte kontejner úložiště objektů blob pro přílohy e-mailů.
   ...
   1. V části **Nový kontejner**zadejte jako název kontejneru `attachments`. V části **úroveň veřejného přístupu**vyberte **kontejner (anonymní přístup pro čtení pro kontejnery a objekty BLOB)**  > **OK**.
...
1. Potvrďte informace o připojení a pak vyberte **připojit**.
   ...
   Průzkumník služby Storage vytvoří připojení a zobrazí účet úložiště v okně Průzkumníka v části **místní & připojené** > **účty úložiště**.
...
1. Do vyhledávacího pole v Návrháři zadejte jako filtr `when new email arrives`. Vyberte pro svého poskytovatele e-mailu tento trigger: **Při přijetí nového e-mailu – <*váš_poskytovatel_e-mailu*>** .
...
1. Zkontrolujte, jestli aplikace logiky uložila e-mail do správného kontejneru úložiště.
   ...
   1. V Průzkumník služby Storage rozbalte **místní & připojené** > **účty úložiště** > **Attachmentstorageacct (klíč)**  > **kontejnery objektů BLOB** > **přílohy**.
...
1. Do vyhledávacího pole zadejte jako filtr `create blob` a pak vyberte tuto akci: **vytvořit objekt BLOB**
...
1. Do vyhledávacího pole zadejte jako filtr `send email` a pak pro vašeho poskytovatele e-mailu vyberte akci "Odeslat e-mail".
