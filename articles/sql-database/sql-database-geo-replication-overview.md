...
* **Podpora fondu elastické databáze**: každou repliku můžete samostatně účastnit fondu elastické databáze nebo nesmí být v každém fondu elastické vůbec. Volba fondu pro každou repliku je oddělená a není závislá na konfiguraci další repliky (jestli je primární nebo sekundární). Každý elastického fondu je obsažena v jedné oblasti, proto může sdílet více replik ve stejné topologie nikdy fondu elastické databáze.
