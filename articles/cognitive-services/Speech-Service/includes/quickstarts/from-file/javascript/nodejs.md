...
```
## <a name="load-the-file-into-an-pushaudioinputstream"></a>Načtení souboru do PushAudioInputStream

Pro NodeJS sada Speech SDK nedokáže nativně podporovat přímý přístup k souborům, takže soubor načteme a vložíme do souboru `PushAudioInputStream` . To bude dobře fungovat i pro menší soubory, ale pro větší využití `PullAudioInputStream` paměti by byl větší výkon.
...
Než budete moct inicializovat `SpeechRecognizer` objekt, musíte vytvořit konfiguraci, která používá váš klíč předplatného a oblast předplatného. Vložte tento kód dál.
...
Nyní je třeba vytvořit `AudioConfig` objekt, který odkazuje na vaše `PushAudioInputStream` . Vložte tento kód přímo pod konfiguraci řeči.
...
Nyní vytvoříme `SpeechRecognizer` objekt pomocí `SpeechConfig` `AudioConfig` dříve vytvořených objektů a.
...
Do konzoly také napíšeme vrácený text, všechny chyby a nakonec Nástroj pro rozpoznávání zavřete.
```JavaScript
