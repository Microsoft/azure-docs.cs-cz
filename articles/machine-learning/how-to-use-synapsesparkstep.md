...
description: Připojte svůj pracovní prostor Synapse k vašemu kanálu služby Azure Machine Learning, abyste mohli používat Spark pro manipulaci s daty.
...
ms.locfileid: "101662550"
---
# <a name="how-to-use-apache-spark-in-your-machine-learning-pipeline-with-azure-synapse-preview"></a>Jak používat Apache Spark v kanálu Machine Learning s Azure synapse (Preview)

V tomto článku se dozvíte, jak používat fondy Apache Spark zajištěné synapse jako výpočetní cíl pro krok přípravy dat v kanálu Azure Machine Learning. Naučíte se, jak může jeden kanál využívat výpočetní prostředky, které jsou vhodné pro konkrétní krok, jako je třeba Příprava dat nebo školení. Uvidíte, jak se připravují data pro krok Spark a jak se předává k dalšímu kroku. 
...
* Vytvořte pracovní prostor synapse a fond Apache Spark (Další informace najdete v tématu [rychlý Start: Vytvoření fondu Apache Spark bez serveru pomocí synapse studia](../synapse-analytics/quickstart-create-apache-spark-pool-studio.md)). 

## <a name="link-your-machine-learning-workspace-and-synapse-workspace"></a>Propojení pracovního prostoru Machine Learning a pracovního prostoru synapse 

Fondy Apache Spark můžete vytvářet a spravovat v pracovním prostoru synapse. Pokud chcete fond Spark integrovat s Azure Machine Learning pracovním prostorem, musíte se připojit k pracovnímu prostoru synapse. 

Fond synapse Spark můžete připojit prostřednictvím uživatelského rozhraní Azure Machine Learning studia pomocí stránky **propojených služeb** . Můžete to provést také prostřednictvím **výpočetní** stránky s možností **připojit COMPUTE** .

Můžete také připojit fond synapse Spark prostřednictvím sady SDK (jak je vypracované níže) nebo prostřednictvím šablony ARM (viz příklad této [šablony ARM](https://github.com/Azure/azure-quickstart-templates/blob/master/101-machine-learning-linkedservice-create/azuredeploy.json)). 

Pomocí příkazového řádku můžete dodržet šablonu ARM, přidat propojenou službu a připojit fond Synapse k následujícímu kódu:
...
> [!Important]
> Pokud chcete úspěšně propojit pracovní prostor synapse, musíte mít roli vlastníka v prostředku pracovního prostoru synapse. Ověřte přístup v Azure Portal.
...
## <a name="create-or-retrieve-the-link-between-your-synapse-workspace-and-your-azure-machine-learning-workspace"></a>Vytvoření nebo načtení propojení mezi pracovním prostorem synapse a pracovním prostorem Azure Machine Learning
...
## <a name="attach-your-synapse-spark-pool-as-a-compute-target-for-azure-machine-learning"></a>Připojte svůj synapse Spark jako cíl výpočetní služby pro Azure Machine Learning

Pokud chcete použít fond synapse Spark k napájení kroku v kanálu Machine Learning, musíte ho připojit jako `ComputeTarget` Krok kanálu, jak je znázorněno v následujícím kódu.
...
Kromě dat může krok kanálu mít závislosti v jazyce Python vázané na krok. Jednotlivé `SynapseSparkStep` objekty mohou přesně určovat svou přesnou konfiguraci synapse. Tento kód je zobrazen v následujícím kódu, který určuje, že `azureml-core` verze balíčku musí být alespoň `1.20.0` . (Jak už bylo zmíněno dříve, tento požadavek pro `azureml-core` je nutný k použití `FileDataset` jako vstupu.)
...
Další sada argumentů pro `SynapseSparkStep` řízení konstruktoru Apache Spark. `compute_target`Je ta `'link1-spark01'` , kterou jsme dříve připojili jako cíl výpočtů. Ostatní parametry určují paměť a jádra, které bychom chtěli použít.
...
Tento skript pro přípravu dat neprovádí žádnou skutečnou transformaci dat, ale znázorňuje, jak data načíst, převést na Spark dataframe a jak provést základní manipulaci s Sparkem. Výstup můžete najít v Azure Machine Learning Studio otevřením podřízeného běhu, výběrem karty **výstupy + protokoly** a otevřením `logs/azureml/driver/stdout` souboru, jak je znázorněno na následujícím obrázku.
