...
* Služba A – má omezení umístění "NodeType = = typ1", hlásí zatížení 10
* Služba B – má omezení umístění "NodeType = = typ1", hlásí zatížení 10
* Služba C – má omezení umístění "NodeType = = typ2", oznamuje zatížení 100
* Služba D – má omezení umístění "NodeType = = typ2", oznamuje zatížení 100
* A máme čtyři uzly. Dva z nich mají jako "typ1" nastavenou hodnotu NodeType a druhá dvě jsou "typ2".
...
Příklad][Image1]
![v podclusterovém umístění </center>
...
* Služba B: omezení umístění "NodeType = = typ1"
* Služba C: omezení umístění "NodeType = = typ2"

Tato konfigurace vytvoří vztah nadmnožiny mezi skupinami uzlů pro různé služby.

<center>

podclustery nadmnožinou ![jsou nadmnožinou][Image2]
</center>

V takové situaci je pravděpodobné, že se vyrovnává z optimálního zůstatku.

Správce prostředků rozpozná tuto situaci a vytvoří zprávu o stavu, která vám pomůže rozdělit službu A na dvě služby – Service a1, kterou lze umístit na uzly typ1 a službu a2, kterou lze umístit na typ2 uzly. Tím se vrátíme k první situaci kategorie, kterou je možné optimálně vyvážit.
