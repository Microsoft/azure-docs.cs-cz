...
3. Jako strategii nasazení můžete zadat eithor `runOnce` nebo `rolling`. 

   `runOnce` je nejjednodušší strategii nasazení, která zaznamená všechny zavěšení životního cyklu, zejména `preDeploy` `deploy`, `routeTraffic`a `postRouteTraffic`se spustí jednou. Pak se spustí buď `on:` `success` nebo `on:` `failure`.
...
4. Níže je uveden příklad fragmentu YAML, který můžete použít k definování postupné strategie pro virtuální počítače aktualizace až 5 cílů v každé iteraci. `maxParallel` určí počet cílů, které mohou být nasazeny paralelně. Účty výběru pro absolutní číslo nebo procento cílů, které musí zůstat dostupné kdykoli, kromě cílů, které se nasazují na. Slouží také k určení stavu úspěch a selhání během nasazování.
   ...
   Při každém spuštění této úlohy se historie nasazení zaznamená do prostředí `<environment name>`, které jste vytvořili a zaregistrovali pro virtuální počítače.
