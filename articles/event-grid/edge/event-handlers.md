...
Pokud chcete publikovat na koncový bod Webhooku, nastavte `endpointType` na `WebHook` a zadejte:
...
Pokud chcete publikovat do koncového bodu cloudu Azure Event Grid, nastavte `endpointType` na `eventGrid` a zadejte:
...
Pokud chcete publikovat do modulu Edge hub, nastavte `endpointType` na `edgeHub` a zadejte:
...
## <a name="service-bus-topics"></a>Témata Service Bus
...
## <a name="storage-queues"></a>Fronty úložiště
