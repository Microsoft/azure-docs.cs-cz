...
- Pokud `options.uploadMode` je nastavená **file** na soubor `options.openMode` a je nastavená na **Binary**, výstup je kódovaný v kódování Base64.
- `options.encoding`Určuje kódování, které má být použito při čtení souboru. Výchozí hodnota je **UTF-8**a používá se jenom v případě, `options.openMode` že je nastavená na **text**.
