...
Při přidání Resource Manageru došlo ke zpětnému přidání všech prostředků do výchozích skupin prostředků. Pokud nyní vytvoříte prostředek prostřednictvím klasického nasazení, prostředek se automaticky vytvoří v rámci výchozí skupiny prostředků pro tuto službu, i když jste tuto skupinu prostředků při nasazení nezadali. Pouze existující v rámci skupiny prostředků však neznamená, že prostředek byl převeden na model Správce prostředků.

## <a name="understand-support-for-the-models"></a>Vysvětlení podpory modelů

Je potřeba vzít v úvahu tři scénáře:

1. Cloudové služby nepodporují model nasazení Resource Manageru.
...
Rutina Správce prostředků **Get-AzVM** však vrátí pouze virtuální počítače nasazené prostřednictvím Správce prostředků. Následující příkaz nevrátí virtuální počítač vytvořený prostřednictvím klasického nasazení.

```powershell
Get-AzVM -ResourceGroupName ExampleGroup
```

Značky jsou podporované jenom u prostředků vytvořených pomocí Resource Manageru. Značky nelze použít na klasické prostředky.
...
* Virtuální počítač odkazuje na konkrétní kartu síťového rozhraní definovanou v poskytovateli síťových prostředků (povinné) a na sadu dostupnosti definovanou ve zprostředkovateli výpočetních prostředků (volitelné).
* Karta síťového rozhraní odkazuje na přiřazenou IP adresu virtuálního počítače (povinnou), podsíť virtuální sítě pro virtuální počítač (povinné) a na skupinu zabezpečení sítě (volitelně).
* Podsíť virtuální sítě odkazuje na skupinu zabezpečení sítě (volitelné).
* Instance nástroje pro vyrovnávání zatížení odkazuje na back-endový fond adres IP, které obsahují kartu síťového rozhraní virtuálního počítače (volitelné) a odkazují na veřejnou nebo soukromou IP adresu nástroje pro vyrovnávání zatížení (volitelně).
...
* Povinná cloudová služba, která funguje jako kontejner pro hostování virtuálních počítačů (výpočetní prostředky). Virtuální počítače jsou automaticky k dispozici s kartou síťového rozhraní a IP adresou přiřazenou Azure. Kromě toho cloudová služba obsahuje instanci externího nástroje pro vyrovnávání zatížení, veřejnou IP adresu a výchozí koncové body, což umožňuje datový provoz ze vzdálené plochy a vzdáleného prostředí PowerShell u virtuálních počítačů s Windows a datový provoz SSH (Secure Shell) u virtuálních počítačů s Linuxem.
* Požadovaný účet úložiště, který ukládá virtuální pevné disky pro virtuální počítač, včetně operačního systému, dočasné a další datové disky (úložiště).
* Volitelná virtuální síť, která funguje jako další kontejner, ve kterém můžete vytvořit podsíť a zvolit podsíť, ve které se virtuální počítač nachází (síť).
...
| Virtuální sítě |Virtuální síť je pro virtuální počítač volitelná. Pokud je zahrnuta, virtuální síť nelze nasadit pomocí Správce prostředků. |Virtuální počítač vyžaduje virtuální síť nasazenou pomocí Resource Manageru. |
...
| Skupiny vztahů |Skupiny vztahů byly nezbytné k vytváření služeb Virtual Network. Nicméně, se zavedením regionálních virtuálních sítí, které nebyly nutné už. |Abychom to zjednodušili, koncept skupin vztahů neexistuje v rozhraních API, které se vystavují prostřednictvím správce Azure Resource Manager. |
...
| Síťová rozhraní |Primární a sekundární síťové rozhraní a jeho vlastnosti byly definované jako síťová konfigurace virtuálního počítače. |Síťové rozhraní je prostředek vystavený poskytovatelem Microsoft.Network. Životní cyklus síťového rozhraní není vázán na virtuální počítač. Odkazuje na přiřazenou IP adresu virtuálního počítače (povinné), podsíť virtuální sítě pro daný virtuální počítač (povinné) a skupinu zabezpečení sítě (volitelné). |
...
Pokud jste připraveni migrovat prostředky z klasického nasazení do nasazení Správce prostředků, přečtěte si:
...
Tato konfigurace není podporována. Správce prostředků nelze použít k nasazení virtuálního počítače do virtuální sítě, která byla vytvořena pomocí klasického nasazení.

**Můžu pomocí Resource Manageru vytvořit virtuální počítač z uživatelské image vytvořené pomocí modelu nasazení Classic?**

Tato konfigurace není podporována. Můžete však zkopírovat soubory virtuálního pevného disku z účtu úložiště, který byl vytvořen pomocí klasického modelu nasazení, a přidat je do nového účtu vytvořeného prostřednictvím Správce prostředků.
...
Komplexní sadu počátečních šablon najdete na [úvodních šablonách Nástroje pro rychlé spuštění Nástroje pro Azure.](https://azure.microsoft.com/documentation/templates/)
