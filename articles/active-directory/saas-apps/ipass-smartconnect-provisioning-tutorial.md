...
5. V části **přihlašovací údaje správce** zadejte `https://openmobile.ipass.com/moservices/scim/v1` na **adrese URL tenanta**. Zadejte token nosiče načtený dříve v **tajném tokenu**. Klikněte na **Test připojení** a ujistěte se, že se služba Azure AD může připojit k IPASS SmartConnect. Pokud se připojení nepovede, ujistěte se, že účet iPass SmartConnect má oprávnění správce, a zkuste to znovu.
