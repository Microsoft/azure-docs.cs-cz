...
* SAP součást: jednotlivé SAP aplikace, jako je například ECC, BW, správci řešení nebo S/4HANA.  Komponenty SAP může být založen na tradičních technologií ABAP a Java nebo jiných NetWeaver na základě aplikace, například pro obchodní objekty.
...
* Mezi různými místy a hybridní: Popisuje scénář, ve které jsou nasazené virtuální počítače s předplatným Azure, který má site-to-site a multi-Site, připojení ExpressRoute mezi místní nápomocni a Azure. Dokumentace v běžných Azure a tyto druhy nasazení jsou také popsány jako mezi různými místy a hybridní scénáře. Důvod pro připojení je k rozšíření místních domén, v místním Active Directory/OpenLDAP a místní DNS do Azure. V místním prostředí je rozšířit na prostředky předplatného Azure. S touto příponou, virtuální počítače můžou být součástí místní domény. Domény uživatele z místní domény můžete přistupovat k serverům a spuštěním služby na těchto virtuálních počítačů (například systému DBMS služby). Komunikace a název řešení mezi virtuální počítače nasazené v místním a nasazených virtuálních počítačích Azure není možné. Toto je nejběžnější a skoro výhradně případ nasazení SAP prostředky do Azure. Další informace najdete v tématu [to][vpn-gateway-cross-premises-options] article and [this][vpn-gateway-site-to-site-create].

> [!NOTE]
> Mezi různými místy a hybridní nasazení systémů SAP, kde Azure Virtual Machines systémy SAP jsou členy místní domény jsou podporovány pro produkční systémy SAP. Mezi různými místy a hybridní konfigurace jsou podporovány pro nasazení částí nebo dokončení prostředí SAP do Azure. Dokonce i kompletní prostředí SAP běžící v Azure vyžaduje s tyto virtuální počítače se zapojil místní domény a reklamy/OpenLDAP. 
>
>
...
Vstupní bod pro úlohy SAP na dokumentaci k Azure se nachází [tady](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/get-started). Počínaje tento vstupní bod najít mnoho články týkající se témata:

- SAP NetWeaver a Business jeden v Azure
- Příručky k SAP DBMS pro různé systémy DBMS v Azure
- Vysoká dostupnost a zotavení po havárii pro úlohy SAP v Azure
- Konkrétní pokyny pro spuštění SAP HANA v Azure
- Pokyny specifické pro velké instance Azure HANA pro SAP HANA DBMS 


> [!IMPORTANT]
> Všude, kde se používá možné odkaz na odkazující průvodců instalací SAPU nebo další dokumentaci k SAPU (referenční dokumentace InstGuide-01, naleznete v tématu <http://service.sap.com/instguides>). Při rozhodování o požadavky, proces instalace nebo podrobnosti o konkrétní funkce SAP, SAP dokumentace a příručky by měl vždy být pečlivě si přečtěte, jako Microsoft dokumenty pokrývá jenom konkrétní úkoly pro SAP software nainstalovat a provozovat v Virtuální počítač Microsoft Azure.
>
>
...
Podniky mají tedy přemýšlet pečlivě o které poskytovatelem cloudu, zvolte možnost pro spuštění těchto obchodních kritické obchodní procesy na. Azure je ideální veřejná Cloudová platforma pro klíčové firemní aplikace SAP a obchodních procesů. Zadaný celou řadu infrastrukturu Azure, téměř všechny existující SAP NetWeaver a S/4HANA systémy je možné hostovat v Azure ještě dnes. Azure poskytuje virtuální počítače se po mnoho terabajtů paměti a víc než 200 procesory. Mimo ně, které Azure nabízí [velkých instancích HANA](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-overview-architecture), které umožňují nasazení HANA horizontální navýšení kapacity až na 24 TB a ANA horizontální navýšení kapacity nasazení až 120 TB. 


Pokud chcete úspěšně nasadit systémy SAP do Azure IaaS nebo IaaS obecně platí, je důležité porozumět významných rozdílů mezi nabídky tradiční outsourcers nebo hostitelé a nabídky IaaS. Zatímco tradiční hostitele nebo pro subdodavatele přizpůsobí infrastruktury (typ sítě, úložiště a server) pro úlohy, které zákazník chce hostovat, zodpovídá za místo toho zákazníka nebo partnera charakterizují zatížení a zvolit správnou Azure součásti virtuální počítače, úložiště a síť pro nasazení IaaS.
...
Většina těchto dat můžete najít [tady (Linux)][virtual-machines-sizes-linux] and [here (Windows)][virtual-machines-sizes-windows].
...
Platforma Microsoft Azure je platforma pro více tenantů. V důsledku úložiště, sítě a další prostředky se sdílejí mezi tenanty. Inteligentní logiky omezení a kvóty se dá zabránit jednoho tenanta vliv na výkon jiného tenanta (hlučného souseda) závažný způsobem. Hlavně pro platformy Azure certifikaci pro SAP HANA, společnost Microsoft potřebuje k prokázání izolaci prostředků pro případy, kde lze spustit více virtuálních počítačů v rámci stejného hostitele v pravidelných intervalech s SAP. Když logiky v Azure se pokusí zajistit odchylky v šířky pásma došlo malé, vysoce sdílené platformy mají tendenci zavést větší odchylky dostupnost prostředků nebo šířka pásma než zákazníků může dojít v jejich místní nasazení. Pravděpodobnost, že systém SAP v Azure se může setkat s větší odchylky než v místním systému je potřeba vzít v úvahu.

Posledním krokem je vyhodnotit požadavky na dostupnost. Může se stát, že základní infrastrukturu Azure je potřeba aktualizovat a vyžaduje hostitelích spuštěných virtuálních počítačů, třeba restartovat. Microsoft dokumenty v různých případech [Údržba pro virtuální počítače v Azure](https://docs.microsoft.com/azure/virtual-machines/windows/maintenance-and-updates). Ke zmírnění výjimečných případech kdy virtuální počítače se musí restartovat, ale ještě důležitější pro případy, budete muset opravy hostovaného operačního systému nebo komponenty DBMS, potřebujete pro vývoj koncepty platný vysoká dostupnost pro vaše produkční systémy SAP. Tento požadavek není žádná jiná než požadavky, že kterými se setkáváte místní. Microsoft neustále posiluje Azure platforma snížit prostoje způsobené změn platformy. 
...
Velikost a omezení různých velikostí různé virtuální počítače nabízí, můžete vidět v tabulce v [(Linux) v tomto článku][virtual-machines-sizes-linux] and [this article (Windows)][virtual-machines-sizes-windows].
...
* [Nasazení a správa virtuálních počítačů pomocí šablon Azure Resource Manageru a Azure CLI](../../linux/create-ssh-secured-vm-from-template.md)
...
Další informace o vytváření bitové kopie z virtuálních počítačů najdete v [(Linux) v tomto článku][virtual-machines-linux-capture-image-resource-manager] and [this article (Windows)][virtual-machines-windows-capture-image-resource-manager].
...
Existují různé možnosti konfigurace název a IP řešení v Azure. Je také služba Azure DNS, který je možné použít místo nastavení serveru DNS. Další informace najdete v [v tomto článku][virtual-networks-manage-dns-in-vnet] a na [na této stránce](https://azure.microsoft.com/services/dns/).

Mezi různými místy a hybridní scénáře, jsme závislí vychází ze skutečnosti, které místní AD/OpenLDAP/DNS bylo rozšířeno přes VPN nebo privátní připojení k Azure. U určitých scénářů, jak je zde uvedeno, může být nutné mít repliku AD/OpenLDAP nainstalované v Azure.
...
Při plánování a změny velikosti SAP na řešení Azure, musí být zvážen kvóty pro jednotlivé velikosti virtuálního počítače. Kvóty virtuálních počítačů jsou popsány [tady (Linux)][virtual-machines-sizes-linux] and [here (Windows)][virtual-machines-sizes-windows].
...
* [Použití Azure classic CLI pro Mac, Linux a Windows pomocí Azure Resource Manageru][xplat-cli-azure-resource-manager]

Také přečíst kapitolu [rozhraní příkazového řádku Azure pro virtuální počítače s Linuxem][deployment-guide-4.5.2] in the [Deployment Guide][planning-guide] o tom, jak pomocí Azure CLI k nasazení rozšíření monitorování Azure pro SAP.
...
> ![Windows][Logo_Windows] Windows
>
> Ujistěte se, že tuto jednotku D:\ není používané automatického připojení Sada disků připojených disků, jak je popsáno v kapitole [nastavení automatického připojení pro připojené disky][planning-guide-5.5.3] v tomto dokumentu.
>
> ![Linux][Logo_Linux] Linux
>
...
K nahrání virtuálního pevného disku nebo existujícího virtuálního počítače z místní sítě, aby bylo možné ho použít jako image virtuálního počítače Azure virtuální počítač nebo virtuální pevný disk musí splňovat požadavky uvedené v kapitole [přípravy pro nasazení virtuálního počítače pomocí bitové kopie zákaznického pro SAP][planning-guide-5.2.2] z Tento dokument.
...
---
Počet disků pro DBMS datové soubory a typ jsou tyto disky hostované na Azure Storage byste měli určit podle požadavků na vstupně-výstupních operací a latence vyžaduje. Přesné kvóty jsou popsané v [(Linux) v tomto článku][virtual-machines-sizes-linux] and [this article (Windows)][virtual-machines-sizes-windows].
...
> * [Tajné klíče Azure Storage a optimalizace Linux vstupně-výstupních operací](https://blogs.msdn.com/b/igorpag/archive/2014/10/23/azure-storage-secrets-and-linux-i-o-optimizations.aspx)
>
>
...
> * [Automatického připojení](https://technet.microsoft.com/library/cc753703.aspx)
>
...
Pro scénáře, ve které chcete připojit k těmto systémům SAP přes veřejný internet pomocí grafického uživatelského rozhraní SAP je potřeba použít následující postupy.
...
1. Pro podsíť nebo síťové rozhraní není definována žádná skupina zabezpečení sítě. Síťový provoz do virtuálních počítačů Azure můžete zabezpečit prostřednictvím takzvané "skupiny zabezpečení sítě". Další informace najdete v článku [Skupina zabezpečení sítě][virtual-networks-nsg].
...
Mezi různými místy a hybridní scénář mohou být zhruba popsány jako v grafice níže:
...
Podrobnější informace o tom porty vyžadované pro různé produkty SAP nebo služby v jednotlivých produktů SAP najdete tady <https://scn.sap.com/docs/DOC-17124>.
...
> ![Windows][Logo_Windows] Windows
>
> Další podrobnosti o tisk s Windows najdete tady: <https://technet.microsoft.com/library/jj590748.aspx>.
>
>
...
Konfigurace domény přenosu systému jste uvedli jako řadič domény přenosu, jak je popsáno v [konfigurací řadiče domény přenosu](https://help.sap.com/erp2005_ehp_04/helpdata/en/44/b4a0b47acc11d1899e0000e829fbbd/content.htm). Vygeneruje se uživatel systému, vytvoří se TMSADM a požadované cílové RFC. Můžete vyhledat tato připojení RFC pomocí SM59 transakce. Překlad názvů hostitelů musí být povolena napříč doménou přenosu.
...
* VÝVOJ systému v Azure přejděte k systému pro přenos (klient 000) a volání transakcí moduly STM. Vyberte další konfiguraci z dialogových oken a pokračujte zahrnují systému v doméně. Zadejte řadič domény jako cílový hostitel ([včetně systémů SAP v doméně přenosu](https://help.sap.com/erp2005_ehp_04/helpdata/en/44/b4a0c17acc11d1899e0000e829fbbd/content.htm?frameset=/en/44/b4a0b47acc11d1899e0000e829fbbd/frameset.htm)). Systém čeká nyní mají být zahrnuty v doméně přenosu.
...
Pokračovat v konfiguraci vašeho systému přenosu jako obvykle, jak je popsáno v dokumentaci k [změn a systému pro přenos](https://help.sap.com/saphelp_nw70ehp3/helpdata/en/48/c4300fca5d581ce10000000a42189c/content.htm?frameset=/en/44/b4a0b47acc11d1899e0000e829fbbd/frameset.htm).
...
Pokud chcete obejít tato čekací doba a mít systémy pracovat rychle čtení a zápis do nebo z adresáře přenosu, můžete nastavit dvěma doménami moduly STM přenosu (jeden pro místní a jeden u systémů v Azure a propojte přenos domén. Zkontrolujte, zda tato dokumentace, která vysvětluje principy za tento koncept v SAP TMS: <https://help.sap.com/saphelp_me60/helpdata/en/c4/6045377b52253de10000009b38f889/content.htm?frameset=/en/57/38dd924eb711d182bf0000e829fbfe/frameset.htm>.

Postup:

* Nastavení domény přenosu na jednotlivých umístěních (místní a Azure) pomocí transakce moduly STM <https://help.sap.com/saphelp_nw70ehp3/helpdata/en/44/b4a0b47acc11d1899e0000e829fbbd/content.htm>
...
* [Změna adresy URL portálu](https://wiki.scn.sap.com/wiki/display/EP/Change+Portal+URL)
* [Změnit výchozí čísla portů, čísla portů pro portál](https://wiki.scn.sap.com/wiki/display/NWTech/Change+Default++port+numbers%2C+Portal+port+numbers)
...
Zajištění vysoké dostupnosti SAP v Azure má několik rozdílů ve srovnání s vysokou dostupností pro SAP v místním fyzickém nebo virtuálním prostředí. Následující dokument od SAPU. popisuje standardní konfigurace vysoké dostupnosti SAP ve virtualizovaných prostředích ve Windows: <https://scn.sap.com/docs/DOC-44415>. Není žádná integrované sapinst SAP-HA konfigurace pro Linux jako existuje pro Windows. O SAP HA on-premises pro Linux najdete další informace tady: <https://scn.sap.com/docs/DOC-8541>.
...
* [Spuštění/zastavení SAP spolu s vaší spuštění/zastavení serveru systému Unix](https://scn.sap.com/community/unix/blog/2012/08/07/startstop-sap-along-with-your-unix-server-startstop)
...
Blog s podrobnostmi o tom, jak nasadit toto řešení je zdokumentované tady: <https://blogs.msdn.com/b/saponsqlserver/archive/2014/11/19/protecting-sap-solutions-with-azure-site-recovery.aspx>.
