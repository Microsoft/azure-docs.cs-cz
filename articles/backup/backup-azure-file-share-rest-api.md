...
Identifikátor URI příspěvku má `{subscriptionId}`, `{vaultName}`, `{vaultresourceGroupName}`a parametry `{fabricName}`. V našem příkladu bude hodnota různých parametrů následující:
