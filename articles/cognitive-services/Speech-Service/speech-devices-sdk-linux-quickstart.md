---
title: 'Rychlý start: Spuštění zařízení řeči sady SDK v Linuxu – hlasové služby'
titleSuffix: Azure Cognitive Services
description: Požadavky a pokyny, jak začít sadou SDK pro Linux řeč zařízení.
...
ms.locfileid: "67723388"
---
# <a name="quickstart-run-the-speech-devices-sdk-sample-app-on-linux"></a>Rychlý start: Spuštění ukázkové aplikace sadou SDK pro řeč zařízení v Linuxu

V tomto rychlém startu se dozvíte jak vytvářet produkt podporou řeči nebo jej použít jako pomocí řeči Devices SDK pro Linux [konverzace určené k transkripci](conversation-transcription-service.md) zařízení. Momentálně se podporuje jenom [Azure Kinect DK](https://azure.microsoft.com/services/kinect-dk/) je podporována.
...
* Operační systém: 64bitová verze systému Linux (Ubuntu 16.04, Ubuntu 18.04, Debian 9)
...
* Stáhněte si nejnovější verzi [sadou SDK pro řeč zařízení](https://aka.ms/sdsdk-download-JRE) pro Javu a extrahování ZIP do pracovního adresáře.
   > [!NOTE]
   > Soubor prostředí JRE. ukázka Release.zip obsahuje ukázkovou aplikaci v prostředí JRE a v tomto rychlém startu se předpokládá, že aplikace je extrahován do /home/wcaltest/JRE-Sample-Release

Ujistěte se, že jsou tyto závislosti nainstalovány před zahájením Eclipse.
...
1. V **Průzkumníku balíčků**, klikněte pravým tlačítkem na projekt. Zvolte **vlastnosti**, pak **spustit/ladit nastavení** > **nové...** > **Aplikace v Javě**. 

1. **Upravit konfiguraci** zobrazí se okno. V **název** zadat **hlavní**a použijte **hledání** pro **hlavní třída** najděte a vyberte  **com.microsoft.cognitiveservices.speech.samples.FunctionsList**.

   ![Snímek obrazovky upravit konfiguraci spuštění](media/speech-devices-sdk/eclipse-edit-launch-configuration-linux.png)

1. Také z **upravit konfiguraci** vyberte okno **prostředí** stránky a **nový**. **Nové proměnné prostředí** zobrazí se okno. V **název** zadat **LD_LIBRARY_PATH** a **hodnotu** pole zadat složku, která obsahuje soubory *.so například   **/home/wcaltest / Prostředí JRE. ukázkové verzi**

1. Kopírování `kws.table` a `participants.properties` do složky projektu **cílové/třídy**
...
1. Výchozí probuzení word (klíčové slovo) je "Počítač". Můžete také zkusit jednu z nich k dispozici funkce slova, třeba "Počítač" nebo "Assistant". Soubory prostředků pro tato slova alternativní probuzení se v zařízení sadou SDK pro řeč, ve složce – klíčové slovo. Například `/home/wcaltest/JRE-Sample-Release/keyword/Computer` obsahuje soubory používané pro probuzení slovo "Počítač".
...
1. Na panelu nabídek Eclipse **spustit** > **spuštění** 
...
1. Vyzkoušejte novou **konverzace určené k transkripci** ukázku. Spustit zdlouhavého přepisování pořídili s **relace** > **Start**. Ve výchozím nastavení všichni uživatelé jsou hosta. Nicméně, pokud máte podpisů hlasové účastníka, můžou být přepnuté do `participants.properties` ve složce projektu **cílové/třídy**. Na vygenerovat signaturu hlasové, podívejte se na [přepisy konverzace (SDK)](how-to-use-conversation-transcription-service.md).

   ![Ukázkové aplikace určené k transkripci konverzace](media/speech-devices-sdk/cts-sample-app-linux.png)
...
1. **Spustitelných JAR soubor exportu** zobrazí se okno. Zvolte **cíl exportu** pro aplikaci a pak vyberte **Dokončit**.
   ...
   ![Snímek obrazovky s pro spustitelný soubor JAR souboru exportu](media/speech-devices-sdk/eclipse-export-jar-linux.png)

1. Prosím vložte `kws.table` a `participants.properties` v cílové složce zvoleno výše, protože tyto soubory jsou potřebné pro aplikaci.
