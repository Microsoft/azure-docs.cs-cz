...
Seznamte se s komponentami a verzemi prostředí [Apache Hadoop](https://hadoop.apache.org/) v Microsoft Azure HDInsight a balíčku enterprise security. Přečtěte si také, jak zkontrolovat verze komponent Hadoop v HDInsight.
...
Azure HDInsight podporuje několik verzí clusteru Hadoop, které lze kdykoli nasadit. 4. dubna 2017 je výchozí verze clusteru používaná službou Azure HDInsight 3.6.
...
**Vypršení platnosti podpory** znamená, že společnost Microsoft již nebude poskytovat podporu pro zadanou verzi HDInsight. A už nebude k dispozici prostřednictvím portálu Azure pro vytváření clusterů. Tyto verze však stále lze vytvořit pomocí azure CLI nebo různé sady SDK.
...
- Integrace se službou Active Directory pro ověřování.
    ...
    V minulosti jste vytvořili clustery HDInsight s místním uživatelem správce a místním uživatelem SSH. Místní správce může přistupovat ke všem souborům, složkám, tabulkám a sloupcům.  Pomocí balíčku Enterprise Security Package povolíte řízení přístupu na základě rolí integrací hdinsightu se službou Active Directory. Který zahrnuje místní službu Active Directory, službu Azure Active Directory Domain Services. Nebo služba Active Directory na virtuálním počítači IaaS. Správce domény v clusteru může uživatelům udělit, aby používali vlastní firemní (doménové) uživatelské jméno a heslo.
...
### <a name="pricing-and-service-level-agreement-sla"></a>Cenová smlouva a smlouva o úrovni služeb (SLA)
...
Smlouva o úrovni služeb (SLA) je definována jako _okno podpory_. Okno podpory je časové období, které `Microsoft Customer Service and Support`podporuje verze HDInsight . Pokud má verze _datum vypršení platnosti_předané podpory , cluster HDInsight je mimo okno podpory. Vypršení platnosti podpory pro HDInsight verze X (po k dispozici novější verze X +1) je pozdější:  
