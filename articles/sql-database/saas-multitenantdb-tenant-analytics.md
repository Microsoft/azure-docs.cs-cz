...
Teď, když jste nasadili aplikaci a nastavili ji s zajímavými daty tenanta, pomocí [SQL Server Management Studio (SSMS)](https://docs.microsoft.com/sql/ssms/download-sql-server-management-studio-ssms) připojte **tenants1\<-\> MT-User** a **Catalog\<-\> MT-User** servery pomocí přihlašovacích údajů = *Developer*, Password = *P\@ssword1*.
