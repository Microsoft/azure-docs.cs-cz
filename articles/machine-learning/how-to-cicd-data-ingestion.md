...
V tomto přístupu se školicí data ukládají do úložiště objektů BLOB v Azure. Kanál Azure Data Factory načte data ze vstupního kontejneru objektů blob, transformuje ho a uloží data do výstupního kontejneru objektů BLOB. Tento kontejner slouží jako [úložiště dat](https://docs.microsoft.com/azure/machine-learning/concept-data#access-data-in-storage) pro službu Azure Machine Learning. Když se dopravují data, kanál Data Factory vyvolá školicí Machine Learning kanál pro výuku modelu. V tomto konkrétním příkladu se transformace dat provádí pomocí poznámkového bloku Pythonu, který běží v clusteru Azure Databricks. 
