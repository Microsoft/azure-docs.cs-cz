...
Maximální velikost záznamu musí být 50 000 znaků měřenou [`String.Length`](https://docs.microsoft.com/dotnet/api/system.string.length). Pokud potřebujete data před odesláním do dovednosti rozdělit, zvažte použití [dovednosti rozdělení textu](cognitive-search-skill-textsplit.md).
...
|--------------------|-------------|
| defaultLanguageCode | Kód jazyka vstupního textu V současné době se podporuje jenom `en`. |
| minimumPrecision | Hodnota mezi 0,0 a 1,0. Pokud je hodnocení spolehlivosti (ve výstupu `piiEntities`) nižší než hodnota nastavená `minimumPrecision`, entita se nevrátí ani nemaskuje. Výchozí hodnota je 0,0. |
| maskingMode | Parametr, který poskytuje různé způsoby, jak maskovat zjištěné PII ve vstupním textu. Podporovány jsou následující možnosti: <ul><li>`none` (výchozí): to znamená, že se neprovede žádné maskování a `maskedText` výstup nebude vrácen. </li><li> `redact`: Tato možnost odebere zjištěné entity ze vstupního textu a nenahradí je cokoli. Všimněte si, že v tomto případě se posun ve výstupu `piiEntities` bude ve vztahu k původnímu textu, a ne na maskovaný text. </li><li> `replace`: Tato možnost nahradí zjištěné entity znakem uvedeným v parametru `maskingCharacter`.  Znak se zopakuje na délku zjištěné entity, takže posuny budou správně odpovídat vstupnímu textu i výstupnímu `maskedText`.</li></ul> |
| maskingCharacter | Znak, který bude použit k maskování textu, je-li parametr `maskingMode` nastaven na hodnotu `replace`. Podporovány jsou následující možnosti: `*` (výchozí), `#``X`. Tento parametr může být `null` pouze v případě, že `maskingMode` není nastavena na `replace`. |
...
| maskedText | Pokud je `maskingMode` nastaveno na jinou hodnotu než `none`, bude tento výstup výsledný řetězec maskování provedeného na vstupním textu, jak je popsáno ve vybraném `maskingMode`.  Pokud je `maskingMode` nastaveno na `none`, tento výstup nebude k dispozici. |
...
Pokud dovednost vrátí upozornění, výstupní `maskedText` může být prázdný.  To znamená, že pokud očekáváte, že výstup bude existovat pro vstup do pozdějších dovedností, nebude fungovat podle očekávání. Mějte na paměti, že při psaní definice dovednosti.
