...
| `broker__defaultMaxEventsPerBatch` | Maximální počet událostí, které mají být přidány do dávky, `MaxBatchSizeInBytes` Pokud je určena pouze hodnota. Výchozí hodnota je `10`.
...
| Název vlastnosti | Popis |
| ---------------- | ------------ |
| `metrics__reporterType` | Typ zpravodaje pro metriky enpoint. Výchozí hodnota `none` je a zakáže metriky. Nastavením této `prometheus` možnost povolíte metriky ve formátu Prometheus Exposition.