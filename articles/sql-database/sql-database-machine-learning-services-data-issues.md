...
- Chcete-li spustit vzorový kód v těchto cvičeních, musíte nejprve mít [Azure SQL Database s povolenou Machine Learning Services (s R)](sql-database-machine-learning-services-overview.md) .
...
Odpověď se obvykle dá najít pomocí příkazu R `str()` . Přidejte funkci `str(object_name)` kamkoli do skriptu jazyka r, aby bylo schéma dat zadaného objektu R vráceno jako informační zpráva. Zprávy můžete zobrazit na kartě **zprávy** v SSMS.

Chcete-li zjistit, proč příklad 1 a příklad 2 obsahují takové různé výsledky, vložte `str(OutputDataSet)` řádek na konci definice `@script` proměnné v každém příkazu, například takto:
...
> [!TIP]
> Můžete také použít funkce R identity, například `is.matrix`, `is.vector`, a získat informace o vnitřních datových strukturách.
...
Například následující skript definuje číselné pole o délce 6 a uloží jej do proměnné `df1`R. Číselné pole je pak kombinováno s celými čísly tabulky RTestData (vytvořené výše), která obsahuje tři (3) hodnoty, k vytvoření nového datového rámce `df2`.
