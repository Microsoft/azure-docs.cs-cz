...
Pomocí následujícího příkazu vytvořte v oblasti skupinu `<resourceGroupName>` prostředků. Nahraďte `<resourceGroupName>` názvem, který zvolíte. Pro tento kurz `East US 2` používáme. Další informace najdete v následujícím [rychlém](../quick-create-cli.md)startu.
...
- `<ApplicationID>` Nahraďte hodnotu ID z registrace vaší aplikace.
- `<servicePrincipalPassword>` Nahraďte hodnotu hodnotou z tajného klíče klienta.
...
Vzhledem k tomu, že jsme už přidali pravidlo k naší bráně firewall,`--add-service=high-availability`aby bylo možné službu ha (), nemusíte na všech uzlech otevírat následující porty brány firewall: 2224, 3121, 21064, 5405. Pokud ale máte nějaký typ potíží s připojením s HA, otevřete pomocí následujícího příkazu porty, které jsou spojené s HA.
...
1. Připojte se ke **všem uzlům** pomocí SQL Server Management Studio (SSMS) nebo SQL cmd. Spuštěním následujících příkazů povolte relaci AlwaysOn_health a vytvořte hlavní klíč:
    ...
    - `<Master_Key_Password>` Nahraďte vlastními hesly.
...
1. Připojte se k primární replice pomocí SSMS nebo SQL CMD. V následujících příkazech se vytvoří certifikát na `/var/opt/mssql/data/dbm_certificate.cer` primárním SQL Server replice a `var/opt/mssql/data/dbm_certificate.pvk` privátní klíč:

    - `<Private_Key_Password>` Nahraďte vlastními hesly.
...
1. Zkopírujte dva soubory, které byly vytvořeny do stejného umístění na všech serverech, které budou hostovat repliky dostupnosti.
    ...
    - `<username>` Nahraďte `<VM2>` a názvem uživatelského jména a cílového virtuálního počítače, který používáte.
...
Pokud je `synchronization_state_desc` seznam synchronizovaný pro `db1`, znamená to, že repliky se synchronizují. Sekundární replika se `db1` zobrazuje v primární replice.
...
1. Pomocí následujícího příkazu vytvořte prostředek `ag_cluster` ve skupině `ag1`dostupnosti.
...
1. K vytvoření prostředku virtuální IP adresy použijte dostupnou statickou IP adresu ze sítě. Můžete ji najít pomocí nástroje `nmap`příkazového řádku.
...
1. Spusťte následující příkaz pro ruční převzetí služeb při selhání primární `<VM2>`replikou na. Nahraďte `<VM2>` hodnotou názvu vašeho serveru.
