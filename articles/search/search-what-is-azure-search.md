...
+ Vytvořte vyhledávací index obsahující pouze data, zdrojem je různé typy obsahu a platformy. 

+ Využijte AI obohacení a extrahování textu a funkce z obrazových souborů, nebo entity a klíčové fráze z nezpracovaný text.

+ Vytvoření vyhledávací intuitivní prostředí s analýzou omezující vlastnost navigace a filtry, synonyma, automatické dokončování a text pro "neměli jste na mysli" autocorrected hledané termíny. Získejte relevance ladění prostřednictvím funkce a zvýšení skóre logiku.

+ Vytvářejte aplikace hledání pro zvláštní případy použití. Geografické vyhledávání podporuje možnosti "najít poblíž". Vícejazyčné hledání je podporované prostřednictvím jazykové analyzátory pro jiné než anglické jazykové fulltextové vyhledávání.
...
|----------------------------------|----------|
| Zdroje dat | Indexy Azure Search přijímají data z jakéhokoliv zdroje, pokud je odeslaný jako datová struktura JSON. <br/><br/> [**Indexery** ](search-indexer-overview.md) automatizovat příjem dat pro podporované zdroje dat Azure a popisovač serializace JSON. Připojte se k [Azure SQL Database](search-howto-connecting-azure-sql-database-to-azure-search-using-indexers.md), [služby Azure Cosmos DB](search-howto-index-cosmosdb.md), nebo [úložiště objektů Blob v Azure](search-howto-indexing-azure-blob-storage.md) k extrahování prohledávatelného obsahu v úložištích dat primární. Indexery Azure Blob můžou *pronikat do dokumentů* za účelem [extrahování textu z hlavních formátů souborů](search-howto-indexing-azure-blob-storage.md), včetně dokumentů Microsoft Office, PDF a HTML. |
| Hierarchické a vnořené datové struktury | [**Komplexní typy** ](search-howto-complex-data-types.md) a kolekce umožňují vám modelovat téměř libovolný typ struktury JSON jako index Azure Search. 1 n a n n Kardinalita lze vyjádřit nativně pomocí kolekcí, komplexní typy a kolekce komplexních typů.|
...
### <a name="step-1-provision-service"></a>Krok 1: Zřízení služby
