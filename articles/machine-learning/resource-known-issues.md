...
Další podrobnosti o monitorování koncových bodů webové služby najdete v [tomto článku](./how-to-enable-app-insights.md#query-logs-for-deployed-models).
...
* **ModuleErrors (žádný modul s názvem)**: Pokud při odesílání experimentů ve službě Azure ml používáte systém ModuleErrors, znamená to, že školicí skript očekává, že je balíček nainstalovaný, ale nepřidá se. Po zadání názvu balíčku nainstaluje Azure ML balíček do prostředí používaného pro váš školicí běh. 
    ...
    Závislosti specifické pro rozhraní jsou uvedeny v dokumentaci k příslušnému rozhraní – [chainer](/python/api/azureml-train-core/azureml.train.dnn.chainer?preserve-view=true&view=azure-ml-py#&preserve-view=trueremarks), [PyTorch](/python/api/azureml-train-core/azureml.train.dnn.pytorch?preserve-view=true&view=azure-ml-py#&preserve-view=trueremarks), [TensorFlow](/python/api/azureml-train-core/azureml.train.dnn.tensorflow?preserve-view=true&view=azure-ml-py#&preserve-view=trueremarks), [skriptu sklearn](/python/api/azureml-train-core/azureml.train.sklearn.sklearn?preserve-view=true&view=azure-ml-py#&preserve-view=trueremarks).
...
Celou řadu selhání webových služeb ve službě Azure Kubernetes Service je možné ladit připojením ke clusteru pomocí `kubectl`. `kubeconfig.json`Cluster služby Azure Kubernetes můžete získat spuštěním
...
* [Použití Application Insights k ladění kanálů strojového učení](./how-to-log-pipelines-application-insights.md)
