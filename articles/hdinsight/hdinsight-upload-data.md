...
description: Zjistěte, jak nahrát a přistupovat k datům pro úlohy Apache Hadoop ve službě HDInsight pomocí klasického rozhraní PŘÍKAZOVÉHO ŘÁDKU Azure, Průzkumníka úložiště Azure, Azure PowerShellu, příkazového řádku Hadoop nebo Sqoop.
...
Azure HDInsight poskytuje plnohodnotný distribuovaný souborový systém Hadoop (HDFS) přes Azure Storage a Azure Data Lake Storage (Gen1 a Gen2). Azure Storage a Data Lake Storage Gen1 a Gen2 jsou navržené jako rozšíření HDFS, která zákazníkům poskytují bezproblémové prostředí. Umožňují úplnou sadu komponent v ekosystému Hadoop pracovat přímo na datech, která spravuje. Azure Storage, Data Lake Storage Gen1 a Gen2 jsou odlišné systémy souborů, které jsou optimalizované pro ukládání dat a výpočtů na těchto datech. Informace o výhodách používání Azure Storage najdete v [tématech Použití Azure Storage s HDInsight](hdinsight-hadoop-use-blob-storage.md), [Použití data lake storage Gen1 s HDInsight](hdinsight-hadoop-use-data-lake-store.md)a Použití data lake storage [Gen2 s HDInsight](hdinsight-hadoop-use-data-lake-storage-gen2.md).
...
## <a name="hadoop-command-line"></a><a id="commandline"></a>Příkazový řádek Hadoop
...
| [Průzkumník úložišť Azure](../storage/blobs/storage-quickstart-blobs-storage-explorer.md) |✔ |✔ |✔ |
...
Služba Azure Data Factory je plně spravovaná služba pro vytváření služeb pro ukládání dat, zpracování dat a přesun dat do zjednodušených, škálovatelných a spolehlivých kanálů pro tvorbu dat.
...
### <a name="apache-sqoop"></a><a id="sqoop"></a>Apačský sqoop

Sqoop je nástroj určený k přenosu dat mezi Hadoopem a relačními databázemi. Můžete ji použít k importu dat ze systému správy relačních databází (RDBMS), jako je SQL Server, MySQL nebo Oracle do distribuovaného souborového systému Hadoop (HDFS), transformovat data v Hadoop u MapReduce nebo Hive a potom exportovat data zpět do RDBMS.
...
### <a name="storage-exception-for-write-on-blob"></a><a id="storageexception"></a>Výjimka úložiště pro zápis do objektu blob
...
**Rozlišení**: `fs.azure.write.request.size` Slouží k určení větší velikosti bloku. Můžete to provést na základě použití pomocí `-D` parametru. Následující příkaz je příkladem pomocí tohoto `hadoop` parametru s příkazem:
...
1. V prohlížeči přejděte do webového uživatelského uživatelského prostředí Ambari pro váš cluster. Toto `https://CLUSTERNAME.azurehdinsight.net`je `CLUSTERNAME` , kde je název clusteru.

    Po zobrazení výzvy zadejte název správce a heslo clusteru.
2. Na levé straně obrazovky vyberte **HDFS**a pak vyberte kartu **Configs.**
3. Do pole **Filtr...** `fs.azure.write.request.size`zadejte . Zobrazí se pole a aktuální hodnota uprostřed stránky.
...
Nyní, když chápete, jak získat data do HDInsight, přečtěte si následující články, kde se dozvíte, jak provádět analýzu:
...
* [Použijte Apache Pig s HDInsight](hadoop/hdinsight-use-pig.md)
