...
`true` means disable the default landing page that is shown for the root URL of a function app. Výchozí hodnota je `false`.
...
`true` means use Release mode when compiling .NET code; `false` means use Debug mode. Výchozí hodnota je `true`.
...
|Klíč|Ukázková hodnota|
|---|------------|
|WEBSITE\_RUN\_FROM\_PACKAGE|1\. místo|
...
|AZURE_FUNCTION_PROXY_DISABLE_LOCAL_CALL|false|Toto je výchozí hodnota. Calls with a  backend url pointing to a function in the local Function App will be forwarded directly to that Function|
