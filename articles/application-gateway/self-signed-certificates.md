...
1. Po zobrazení výzvy zadejte heslo pro kořenový klíč a informace o organizaci vlastní certifikační autority, jako je například země, stát, org, OU a plně kvalifikovaný název domény (Jedná se o doménu vystavitele).
...
## <a name="configure-the-certificate-in-your-web-servers-ssl-settings"></a>Konfigurace certifikátu v nastavení SSL vašeho webového serveru

Na webovém serveru nakonfigurujte SSL pomocí souborů Fabrikam. CRT a Fabrikam. Key. Pokud váš webový server nemůže převzít dva soubory, můžete je zkombinovat do jednoho souboru. pem nebo. pfx pomocí příkazů OpenSSL.
...
Následující konfigurace je příkladem [Nginx serveru](https://nginx.org/docs/http/configuring_https_servers.html) s konfigurací SSL:

![NGINX s protokolem SSL](media/self-signed-certificates/nginx-ssl.png)
