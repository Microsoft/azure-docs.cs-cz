...
Vytvořte aplikaci Node. js s názvem *Queues-Start-V12*.
...
1. Vytvořte nový textový soubor s názvem *Package. JSON*. Tento soubor definuje projekt Node. js. Uložte tento soubor v adresáři *Queues-Starter-V12* . Tady je obsah souboru:
...
Pořád v adresáři *Queues-Start-V12* nainstalujte knihovnu klienta Azure Queue Storage pro balíček JavaScriptu pomocí příkazu `npm install`.

```console
npm install
```

 Tento příkaz přečte soubor *Package. JSON* a nainstaluje balíček klientské knihovny Azure Queue Storage V12 for JavaScript a všechny knihovny, na kterých závisí.
...
1. Uložte nový soubor jako *Queues-Quickstart-V12. js* v adresáři *Queues-Starter-V12* .
...
* [QueueServiceClient](https://docs.microsoft.com/javascript/api/@azure/storage-queue/queueserviceclient): `QueueServiceClient` umožňuje správu všech front v účtu úložiště.
* [QueueClient](https://docs.microsoft.com/javascript/api/@azure/storage-queue/queueclient): třída `QueueClient` umožňuje spravovat a manipulovat s jednotlivou frontou a jejími zprávami.
* [QueueMessage](https://docs.microsoft.com/javascript/api/@azure/storage-queue/queuemessage): třída `QueueMessage` představuje jednotlivé objekty vracené při volání [receiveMessages](https://docs.microsoft.com/javascript/api/@azure/storage-queue/queueclient?view=azure-node-latest#receivemessages-queuereceivemessageoptions-) ve frontě.
...
Vytvořte instanci třídy [QueueClient](https://docs.microsoft.com/javascript/api/@azure/storage-queue/queueclient) . Pak zavolejte metodu [Create](https://docs.microsoft.com/javascript/api/@azure/storage-queue/queueclient#create-queuecreateoptions-) a vytvořte ve svém účtu úložiště frontu.
...
Následující fragment kódu přidá zprávy do fronty voláním metody [SendMessage](https://docs.microsoft.com/javascript/api/@azure/storage-queue/queueclient#sendmessage-string--queuesendmessageoptions-) . Ukládá také [QueueMessage](https://docs.microsoft.com/javascript/api/@azure/storage-queue/queuemessage) vrácené z třetího volání `sendMessage`. Vrácený `sendMessageResponse` slouží k aktualizaci obsahu zprávy později v programu.
...
Podívejte se na zprávy ve frontě voláním metody [peekMessages](https://docs.microsoft.com/javascript/api/@azure/storage-queue/queueclient#peekmessages-queuepeekmessagesoptions-) . Metoda `peekMessages` načítá jednu nebo více zpráv před frontou fronty, ale nemění viditelnost zprávy.
...
Aktualizujte obsah zprávy voláním metody [updateMessage](https://docs.microsoft.com/javascript/api/@azure/storage-queue/queueclient#updatemessage-string--string--string--undefined---number--queueupdatemessageoptions-) . Metoda `updateMessage` může změnit časový limit a obsah viditelnosti zprávy. Obsah zprávy musí být řetězec kódovaný v kódování UTF-8, který má velikost až 64 KB. Společně s novým obsahem předejte `messageId` a `popReceipt` z odpovědi, která byla uložena dříve v kódu. Vlastnosti `sendMessageResponse` určují, která zpráva se má aktualizovat.
...
Stáhněte dříve přidané zprávy voláním metody [receiveMessages](https://docs.microsoft.com/javascript/api/@azure/storage-queue/queueclient#receivemessages-queuereceivemessageoptions-) .  V poli `numberOfMessages` předejte maximální počet zpráv, které mají být pro toto volání přijímány.
...
Odstraňte zprávy voláním metody [deleteMessage](https://docs.microsoft.com/javascript/api/@azure/storage-queue/queueclient#deletemessage-string--string--queuedeletemessageoptions-) . Všechny zprávy, které se explicitně neodstraní, se později stanou viditelnými ve frontě, aby se mohly zpracovat další šance na jejich zpracování.
...
Přidejte tento kód na konec funkce `main` a uložte soubor:
...
V okně konzoly přejděte do adresáře, který obsahuje soubor *Queues-Quickstart-V12. js* , a poté spusťte následující příkaz `node` pro spuštění aplikace.
