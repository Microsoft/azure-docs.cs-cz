...
1. Po otevření **Marketplace** zadejte do vyhledávacího pole `logic apps management`a vyberte **Logic Apps Správa**.
...
1. V nabídce pracovního prostoru vyberte možnost **Souhrn pracovního prostoru** > **Logic Apps Správa**.
...
1. Pokud chcete zobrazit všechna spuštění konkrétní aplikace logiky nebo stav, vyberte řádek pro tuto aplikaci logiky nebo stav.
   ...
   > [!NOTE]
   > Sledované vlastnosti nebo dokončené události můžou při zobrazení Log Analytics pracovního prostoru zaznamenat zpoždění 10-15 minut.
...
* `clientTrackingId`: Pokud není k dispozici, Azure automaticky vygeneruje toto ID a koreluje události napříč spuštěním aplikace logiky, včetně všech vnořených pracovních postupů, které jsou volány z aplikace logiky. Toto ID můžete zadat ručně v triggeru tak, že v žádosti triggeru zadáte hlavičku `x-ms-client-tracking-id` s vaší hodnotou vlastního ID. Můžete použít Trigger žádosti, Trigger HTTP nebo Trigger Webhooku.

* `trackedProperties`: Chcete-li sledovat vstupy nebo výstupy v diagnostických datech, můžete přidat oddíl `trackedProperties` do akce buď pomocí návrháře aplikace logiky, nebo přímo v definici JSON vaší aplikace logiky. Sledované vlastnosti mohou sledovat pouze vstupy a výstupy jedné akce, ale můžete použít `correlation` vlastností událostí pro korelaci mezi akcemi v běhu. Chcete-li sledovat více než jednu vlastnost, jednu nebo více vlastností, přidejte část `trackedProperties` a vlastnosti, které chcete do definice akce.
...
Tento příklad ukazuje, jak událost `ActionCompleted` zahrnuje atributy `clientTrackingId` a `trackedProperties`:
