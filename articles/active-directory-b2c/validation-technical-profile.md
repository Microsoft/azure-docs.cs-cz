...
| Atribut | Požaduje se | Popis |
| --------- | -------- | ----------- |
| `Type` | Ano | Typ kontroly nebo dotazu, který má být proveden pro předběžnou podmínku. Buď `ClaimsExist` je zadáno, aby bylo zajištěno, že akce budou provedeny v případě, že zadané deklarace existují v aktuální sadě deklarací `ClaimEquals` uživatele, nebo zda je zadána akce, pokud zadaná deklarace identity existuje a její hodnota je rovna zadané hodnotě. |
...
| Prvek | Výskytů | Popis |
| ------- | ----------- | ----------- |
| Hodnota | 1: n | Data, která se používají při kontrole. Pokud je `ClaimsExist`typ této kontroly, toto pole určuje ClaimTypeReferenceId k dotazování na. Pokud je `ClaimEquals`typ kontroly, toto pole určuje ClaimTypeReferenceId k dotazování na. Zatímco jiný element Value obsahuje hodnotu, která má být kontrolována.|
...
2. Další technický profil ověření se neprovede, pokud deklarace userType neexistuje nebo pokud je `Partner`hodnota usertype. Technický profil ověření se pokusí přečíst profil uživatele z interní databáze zákazníka a pokračovat, pokud dojde k chybě, například služba REST API není k dispozici nebo došlo k vnitřní chybě.
3. Poslední technický profil ověření se neprovede, pokud deklarace userType neexistovala nebo pokud je `Customer`hodnota usertype. Technický profil ověření se pokusí přečíst profil uživatele z interní databáze partnerů a pokračuje v případě výskytu chyby, například REST API služba není k dispozici nebo došlo k vnitřní chybě.
