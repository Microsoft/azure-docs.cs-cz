...
Tento program vytvoří soubor **Security World** v adresáři %NFAST_KMDATA%\local\world, který odpovídá složce C:\ProgramData\nCipher\Key Management Data\local. Pro kvorum můžete použít různé hodnoty, ale v našem příkladu zadáte pro každé z nich tři prázdné karty a kódy PIN. Pak, všechny dvě karty poskytují plný přístup do světa bezpečnosti. Tyto karty tvoří sadu karet správce **Administrator Card Set** pro novou architekturu Security World.
...
> [!NOTE]
> Pokud chcete stažený balíček ověřit, musí být modul HSM připojený, zapnutý a musí mít v sobě architekturu Security World (třeba tu, kterou jste právě vytvořili).
