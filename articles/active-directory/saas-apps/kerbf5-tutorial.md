...
1. Certifikát metadat je třeba importovat do nástroje F5 (Kerberos), který bude použit později v procesu instalace. Do **> seznamu certifikátů protokolu SSL vyberte System > Certificate management > Traffic Certificate management >** . Klikněte na **Import** pravého rohu.

    ![F5 (Kerberos) – konfigurace](./media/kerbf5-tutorial/configure01.png) 

1. Navíc také potřebujete **certifikát SSL** pro název hostitele (`Kerbapp.superdemo.live`), v tomto příkladu jsme použili certifikát se zástupnými znaky.

    ![F5 (Kerberos) – konfigurace](./media/kerbf5-tutorial/configure02.png) 
 
1. Přejděte na – **F5 Big-IP klikněte na přístup > s asistencí konfigurace > federaci > poskytovatel služeb SAML**.
...
1. Vytvořte nový virtuální server a zadejte **cílovou adresu**. Vyberte **certifikát Nezástupných karet** (nebo **certifikát** , který jste nahráli pro aplikaci), který jsme nahráli dříve a **přidružený privátní klíč**.
...
1. V části **nastavení jednotného přihlašování**zvolte **Kerberos** a vyberte **Upřesnit nastavení**. Požadavek je třeba vytvořit v `user@domain.suffix`.

1. V části **zdroj uživatelského jména** zadejte `session.saml.last.attr.name. http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname`. Úplný seznam proměnných a hodnot najdete v příloze.
...
1. Podrobnosti o dokumentaci k nástroji pro **kontrolu koncového bodu** najdete v [týmu podpory](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45) pro službu Kerberos pro klienty.

    ![F5 (Kerberos) – konfigurace](./media/kerbf5-tutorial/configure10.png) 

1. Informace o dokumentaci k vlastnostem pro **správu relací** získáte v [týmu podpory pro správce klientských služeb (Kerberos)](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45) .
