...
1. V aplikaci logiky, kde chcete vytvořit výstup, postupujte podle jednoho z následujících kroků: 
   ...
   * Pokud chcete přidat akci mezi kroky, přesuňte ukazatel myši na šipku připojení, aby se zobrazilo znaménko plus ( **+** ). Vyberte znaménko plus a pak vyberte **přidat akci**.
...
Chcete-li vrátit hodnoty z pole, můžete použít [funkci`item()`](../logic-apps/workflow-definition-language-functions-reference.md#item) s akcí **vytvořit tabulku CSV** . Ve smyčce `For_each` můžete použít [funkci`items()`](../logic-apps/workflow-definition-language-functions-reference.md#items).
...
V definici JSON akce v poli `columns` nastavte vlastnost `header` na prázdný řetězec. Pro každou vlastnost `value` odkažte na každou vlastnost pole, kterou chcete.

1. Na panelu nástrojů návrháře vyberte **zobrazení kódu**.

1. V editoru kódu v poli `columns` akce přidejte prázdnou vlastnost `header` a tento výraz `value` pro každý sloupec hodnot pole, který chcete použít:
...
Chcete-li vrátit hodnoty z pole, můžete použít [funkci`item()`](../logic-apps/workflow-definition-language-functions-reference.md#item) s akcí **vytvořit tabulku HTML** . Ve smyčce `For_each` můžete použít [funkci`items()`](../logic-apps/workflow-definition-language-functions-reference.md#items).
...
1. Pro podmínku zadejte položky pole, které chcete porovnat, vyberte operátor porovnání a zadejte hodnotu porovnání.
   ...
   V tomto příkladu se používá funkce `item()` pro přístup k jednotlivým položkám v poli, zatímco akce **pole filtru** vyhledává položky pole, jejichž hodnota je větší než jedna:
...
1. V levém sloupci v poli **Mapa** zadejte název vlastnosti, který chcete přiřadit každé hodnotě ve zdrojovém poli. Do sloupce na pravé straně zadejte výraz, který představuje hodnotu, kterou chcete přiřadit vlastnost.
   ...
   Tento příklad určuje "Product_ID" jako název vlastnosti pro přiřazení každé hodnoty v poli Integer pomocí funkce `item()` ve výrazu, který přistupuje k jednotlivým položkám pole. 
...
1. V takovém případě klikněte na libovolné místo, kde chcete zobrazit výsledky. Po otevření seznamu dynamický obsah vyberte **výraz**. Chcete-li získat výstup pole z akce **Select** , zadejte tento výraz, který obsahuje název akce **výběru** :
   ...
   V tomto příkladu se používá **e-mailová** akce Office 365 Outlooku odeslat e-mail a obsahuje výstupy z `@actionBody('Select')` výrazu v těle e-mailu:
