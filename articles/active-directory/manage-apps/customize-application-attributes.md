...
1. Rozbalte **mapování** zobrazit a upravit atributy uživatele, které budou plout mezi službami Azure AD a cílovou aplikací. Pokud je cílová aplikace podporuje, tato část umožňuje volitelně konfigurovat zřizování uživatelských účtů a skupin.
   ...
   ![Pomocí mapování můžete zobrazit a upravit atributy uživatele](./media/customize-application-attributes/21.png)
...
1. Vyberte **mapování** konfigurace otevřete související **mapování atributů** obrazovky. Některé mapování atributů jsou vyžadované SaaS aplikace fungovat správně. Pro požadované atributy **odstranit** funkce není k dispozici.
   ...
   ![Použijte ke konfiguraci mapování atributů pro aplikace pro mapování atributů](./media/customize-application-attributes/22.png)
...
1. Vyberte existující **mapování atributů** otevřít **Upravit atribut** obrazovky. Tady můžete upravit atributy uživatele, které budou plout mezi službami Azure AD a cílovou aplikací.
   ...
   ![Upravit atributy uživatele pomocí úpravy atributu](./media/customize-application-attributes/23.png)

### <a name="understanding-attribute-mapping-types"></a>Principy typů mapování atributů

S mapování atributů můžete řídit, jak se atributy vyplní v aplikaci SaaS třetí strany.
Existují čtyři jiné typy mapování podporovány:
...
![Příklad ukazuje ServiceNow s zřízené objekty, skupiny a uživatele](./media/customize-application-attributes/24.png)
...
- Atribut IsSoftDeleted je často součástí výchozí mapování pro aplikaci. IsSoftdeleted může mít hodnotu true v jednom ze scénářů čtyři (uživatel je mimo rozsah z důvodu se Nepřiřazení z aplikace, uživatel je mimo rozsah z důvodu nesplnění podmínek smluv filtr oborů, uživatel byl lehce odstraněn ve službě Azure AD, nebo vlastnost AccountEnabled je nastavena na hodnotu false  na uživatele). 
