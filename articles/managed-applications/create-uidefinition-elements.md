---
title: Prvek definice uživatelského rozhraní Azure Create | Microsoft Docs
...
| nastavení | Ne | Další vlastnosti, které přizpůsobují chování prvku. Podobně jako u `constraints`se podporované vlastnosti liší podle typu prvku. |
| Zobrazeny | Ne | Určuje, zda je prvek zobrazen. Pokud `true`, zobrazí se element a příslušné podřízené prvky. Výchozí hodnota je `true`. K dynamickému řízení hodnoty této vlastnosti použijte [logické funkce](create-uidefinition-functions.md#logical-functions) .
