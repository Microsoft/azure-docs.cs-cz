...
| **Atributy**              | EnvironmentType – OnPrem |
| **Odkazy**              | Není k dispozici  |
| **Kroky** | Když je webová aplikace předávající stranou a IDP je server ADFS, zabezpečený atribut tokenu FedAuth lze nakonfigurovat nastavením vlastnost requireSSL na hodnotu true v `system.identityModel.services` sekci Web. config:|
...
| **Kroky** | CSRF a ASP.NET MVC Forms – použijte `AntiForgeryToken` pomocnou metodu pro zobrazení; `Html.AntiForgeryToken()` vložte do formuláře, například|
...
### <a name="example"></a>Příklad
Ve stejnou dobu HTML. AntiForgeryToken () přiřadí návštěvníkovi soubor cookie s názvem __RequestVerificationToken a stejnou hodnotou jako náhodná Skrytá hodnota uvedená výše. Dále pro ověření příchozího příspěvku formuláře přidejte filtr [ValidateAntiForgeryToken] do metody cíle akce. Příklad:
```
[ValidateAntiForgeryToken]
