...
| Prostřednictvím           | Nepodporuje se | Podporováno          |
...
1. Vytvořte řetězec, který bude podepsán zřetězením příkazu HTTP (např. `GET` nebo `PUT` ), cesty požadavku HTTP a hodnot `Date` hlaviček a `Host` `x-ms-content-sha256` protokolu HTTP v následujícím formátu:
    ...
    ```
1. Vygenerujte podpis HMAC-256 řetězce kódované v kódování UTF-8, který jste vytvořili v předchozím kroku. Potom zakódovat výsledky jako base64. Všimněte si, že budete také potřebovat kódování Base64 klíče účtu úložiště. Použijte následující formát (zobrazený jako pseudo kód):
    ```
