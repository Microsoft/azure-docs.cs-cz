...
Pokud parametr **response_type** v žádosti `/authorize` obsahuje `token`, musí parametr **Scope** zahrnovat alespoň jeden obor prostředků, než `openid` a `offline_access`, které budou uděleny. V opačném případě se `/authorize` požadavek nezdařil.

## <a name="request-a-token"></a>Požádat o token

K vyžádání přístupového tokenu potřebujete autorizační kód. Níže je příklad požadavku na koncový bod `/authorize` pro autorizační kód. Vlastní domény se nepodporují pro použití s přístupovými tokeny. V adrese URL žádosti použijte svoji tenant-name.onmicrosoft.com doménu.

V následujícím příkladu nahradíte tyto hodnoty:

- `<tenant-name>` – název vašeho tenanta Azure AD B2C.
- `<policy-name>` – název vašich vlastních zásad nebo toku uživatele.
...
Při použití https://jwt.ms k prohlédnutí vráceného přístupového tokenu by se měla zobrazit podobný příklad jako v následujícím příkladu:
