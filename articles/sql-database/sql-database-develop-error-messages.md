...
* @No__t_1 &lt;db_name databáze na serveru &lt;Azure_instance &gt; není aktuálně k dispozici. Zkuste prosím připojení znovu později. Pokud se problém opakuje, obraťte se na zákaznickou podporu a sdělte mu ID trasování relace &lt;session_id &gt;
* @No__t_1 &lt;db_name databáze na serveru &lt;Azure_instance &gt; není aktuálně k dispozici. Zkuste prosím připojení znovu později. Pokud se problém opakuje, obraťte se na zákaznickou podporu a sdělte mu ID trasování relace &lt;session_id &gt;. (Microsoft SQL Server, chyba: 40613)
...
| ---:| ---:|:--- |
| 4060 |16 |Databázi% nelze otevřít. &#x2a;LS vyžadovaná přihlášením Přihlášení se nezdařilo. Další informace najdete v tématu [chyby 4000 až 4999](https://docs.microsoft.com/sql/relational-databases/errors-events/database-engine-events-and-errors#errors-4000-to-4999) .|
...
| 40501 |20 |Služba je momentálně zaneprázdněna. Požadavek opakujte po 10 sekundách. ID incidentu:% ls. Kód:% d. Další informace: <br/>[omezení prostředků databázového serveru](sql-database-resource-limits-database-server.md) &bull; &nbsp;<br/>&bull; &nbsp;[omezení založené na DTU pro izolované databáze](sql-database-service-tiers-dtu.md)<br/>[omezení pro elastické fondy](sql-database-dtu-resource-limits-elastic-pools.md) &bull; &nbsp; na DTU<br/>&bull; &nbsp;[limity založené na Vcore pro jednotlivé databáze](sql-database-vcore-resource-limits-single-databases.md)<br/>[omezení pro elastické fondy](sql-database-vcore-resource-limits-elastic-pools.md) &bull; &nbsp; na Vcore<br/>&bull;[omezení prostředků spravované instance](sql-database-managed-instance-resource-limits.md)&nbsp;.|
...
| Kód chyby | Závažnost | Popis | 
| ---:| ---:|:---|
|  1104 |17 |V databázi TEMPDB došlo během nalití mimo prostor. Uvolněte místo vyřazením objektů nebo přepsáním dotazu pro využití méně řádků. Pokud problém stále přetrvává, zvažte upgrade na vyšší cíl na úrovni služby.|
| 40020 |16 |Databáze je v přechodu a operace se ukončuje.|
| 40143 |16 |Replika, kterou hostuje datový uzel pro požadovaný oddíl, není primární.|
| 40166 |16 |Probíhá rekonfigurace databáze clouddb a všechny nové uživatelské transakce budou přerušeny.|
| 40540 |16 |Transakce byla přerušena, protože databáze byla přesunuta do režimu jen pro čtení. Jedná se o dočasnou situaci a zkuste operaci zopakovat.|
...
| Kód chyby | Závažnost | Popis |
| ---:| ---:|:--- |
| 40635 |16 |Klient s IP adresou%. &#x2a;LS je dočasně zakázané. |
...
Následující chyby jsou způsobeny nadměrným využitím prostředků při práci s Azure SQL Database. Například:
...
| 10929 |20 |ID prostředku:% d. Minimální záruka% s je% d, maximální limit je% d a aktuální využití databáze je% d. Server je však v současnosti příliš zaneprázdněn, aby podporoval žádosti větší než% d pro tuto databázi. ID prostředku indikuje prostředek, který dosáhl limitu. Pro pracovní vlákna, ID prostředku = 1. Pro relace, ID prostředku = 2. Další informace: <br/>[omezení prostředků databázového serveru](sql-database-resource-limits-database-server.md) &bull; &nbsp;<br/>&bull; &nbsp;[omezení založené na DTU pro izolované databáze](sql-database-service-tiers-dtu.md)<br/>[omezení pro elastické fondy](sql-database-dtu-resource-limits-elastic-pools.md) &bull; &nbsp; na DTU<br/>&bull; &nbsp;[limity založené na Vcore pro jednotlivé databáze](sql-database-vcore-resource-limits-single-databases.md)<br/>[omezení pro elastické fondy](sql-database-vcore-resource-limits-elastic-pools.md) &bull; &nbsp; na Vcore<br/>&bull;[omezení prostředků spravované instance](sql-database-managed-instance-resource-limits.md)&nbsp;. <br/>V opačném případě zkuste to prosím znovu později. |
...
| 40844 | 16 |Databáze% ls na serveru% LS je databáze edice% LS v elastickém fondu a nemůže mít relaci průběžného kopírování.  |Nevztahuje se |
...
| [18452](https://docs.microsoft.com/sql/relational-databases/errors-events/database-engine-events-and-errors#errors-18000-to-18999) |14 |Přihlášení se nezdařilo. Přihlašovací jméno pochází z nedůvěryhodné domény a nedá se použít s ověřováním systému Windows .%. &#x2a;LS (přihlášení systému Windows nejsou v této verzi SQL Server podporována.) |
...
| [18470](https://docs.microsoft.com/sql/relational-databases/errors-events/database-engine-events-and-errors#errors-18000-to-18999) |14 |Přihlášení uživatele% se nezdařilo. &#x2a;ls. Důvod: účet je zakázaný .%. &#x2a;LS |
...
| 40507 |16 |'%. &#x2a;LS nelze vyvolat s parametry v této verzi SQL Server. |
| 40508 |16 |Příkaz USE není podporován pro přepínání databází. Pro připojení k jiné databázi použijte nové připojení. |
| 40510 |16 |Příkaz%. &#x2a;ls se v této verzi SQL Server nepodporuje. |
| 40511 |16 |Integrovaná funkce%. &#x2a;ls není v této verzi SQL Server podporován. |
| 40512 |16 |Zastaralá funkce% ls není v této verzi SQL Server podporována. |
| 40513 |16 |Proměnná serveru '%. &#x2a;ls není v této verzi SQL Server podporován. |
| 40514 |16 |% ls není v této verzi SQL Server podporován. |
| 40515 |16 |Odkaz na název databáze nebo serveru v%. &#x2a;ls není v této verzi SQL Server podporován. |
| 40516 |16 |Globální dočasné objekty nejsou v této verzi SQL Server podporovány. |
| 40517 |16 |Klíčová slova nebo možnost příkazu '%. &#x2a;ls není v této verzi SQL Server podporován. |
| 40518 |16 |Příkaz DBCC '%. &#x2a;ls není v této verzi SQL Server podporován. |
...
| 40522 |16 |Objekt zabezpečení databáze%. &#x2a;typ ls není v této verzi SQL Server podporován. |
| 40523 |16 |Implicitní uživatel% &#x2a;vytváření ls není v této verzi SQL Server podporováno. Před použitím uživatele explicitně vytvořte. |
| 40524 |16 |Datový typ%. &#x2a;ls není v této verzi SQL Server podporován. |
| 40525 |16 |S '%. ls ' není v této verzi SQL Server podporován. |
| 40526 |16 |'%. &#x2a;zprostředkovatel sady řádků ls není v této verzi SQL Server podporován. |
...
| 40529 |16 |Integrovaná funkce%. &#x2a;LS v kontextu zosobnění není v této verzi SQL Server podporován. |
| 40532 |11 |Nejde otevřít server%. &#x2a;LS vyžadovaná přihlášením Přihlášení se nezdařilo. |
| 40553 |16 |Relace byla ukončena z důvodu nadměrného využití paměti. Zkuste upravit dotaz pro zpracování méně řádků.<br/><br/> Snížení počtu `ORDER BY` a `GROUP BY` operací v kódu Transact-SQL pomáhá snižovat nároky na paměť dotazu. |
...
| 40615 |16 |Nelze otevřít server ' {0} ' požadovaný přihlášením. Klient s IP adresou {1} nemá povolený přístup k serveru.<br /><br />Chcete-li povolit přístup, použijte portál SQL Database nebo spusťte \_rule SP \_set \_firewall v hlavní databázi vytvořte pravidlo brány firewall pro tuto IP adresu nebo rozsah adres. Tato změna může trvat až pět minut. |
...
| 40620 |16 |Přihlášení uživatele "%" se nezdařilo. &#x2a;ls. Změna hesla se nezdařila. Změna hesla během přihlášení není v této verzi SQL Server podporována. |
| 40627 |20 |Probíhá operace na serveru {0} a v databázi {1}. Počkejte prosím několik minut, než to zkusíte znovu. |
...
| 40636 |16 |Nelze použít rezervovaný název databáze%. &#x2a;LS v této operaci. |
...
| 40642 |17 |Server je aktuálně zaneprázdněný. Zkuste to prosím znova později. |
...
| 40852 |16 |Databázi% nelze otevřít. \*ls na serveru%. \*ls požadováno přihlášením. Přístup k databázi je povolen pouze pomocí připojovacího řetězce s povoleným zabezpečením. Chcete-li získat přístup k této databázi, upravte připojovací řetězce tak, aby obsahovaly hodnotu Secure v plně kvalifikovaném názvu domény serveru – název serveru. Database. Windows. NET by se měly změnit na název serveru. Database. `secure`.windows.net. |
...
| 45168 |16 |SQL Azure systém je zatížen a umísťuje horní limit souběžných operací databáze CRUD pro jeden SQL Database Server (např. vytvořit databázi). Server uvedený v chybové zprávě překročil maximální počet souběžných připojení. Zkuste to znovu později. |
| 45169 |16 |Systém SQL Azure je zatížen a umísťuje horní limit počtu souběžných operací operace CRUD serveru pro jedno předplatné (např. vytvořit server). Předplatné zadané v chybové zprávě překročilo maximální počet souběžných připojení a žádost byla zamítnuta. Zkuste to znovu později. |
