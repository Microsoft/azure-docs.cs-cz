---
title: Azure Stream Analytics – poznámky k verzi
description: Tento článek popisuje historie verzí Azure Stream Analytics a odpovídající nástrojů Visual Studio.
...
Tato aktualizace je pro Visual Studio Tools. Tato verze obsahuje následující nové funkce:
...
Tato aktualizace je pro řešení potíží dokumentaci k verzi.

[Průvodce odstraňováním potíží s](stream-analytics-troubleshooting-guide.md) a byly vydané jiné dokumenty. Zkontrolujte tato příručka a vaše zpětná vazba je úvodní.
...
| Zobrazení chybové zprávy pro vstupy/výstupy | Pokud žádné běhová chyba související s vstupů nebo výstupů úlohy, je zobrazena na diagramu úlohy, kde můžete podržet zobrazit podrobnou chybovou zprávu.|
...
| Integrace Azure Machine Learning |Tato verze obsahuje podporu pro Azure Machine Learning uživatelsky definované funkce. Další informace najdete v tématu [kurzu](stream-analytics-machine-learning-integration-tutorial.md) a [obecné blog oznámení](http://blogs.technet.com/b/machinelearning/archive/2015/12/10/apply-azure-ml-as-a-function-in-azure-stream-analytics.aspx). |
...
| Přidání POSUNUTÍ pro Přeskakujícího a přepínání windows |Ve výchozím nastavení [Přeskakujícího](https://msdn.microsoft.com/library/azure/dn835055.aspx) a [Hopping](https://msdn.microsoft.com/library/azure/dn835041.aspx) windows jsou v souladu s přechodu do režimu (1/1/0001 12:00:00 AM UTC). Nový parametr (volitelné) `offsetsize` umožňuje zadat vlastní posun (nebo zarovnání). |
...
| Datové typy odvozené ze vstupního zdroje |Pokud příkaz CREATE TABLE se nepoužívá, Vstupní typ je odvozen od formát vstupu, například všechna pole ze souboru CSV jsou řetězec. Pole explicitně převeďte na správný typ. pomocí funkce CAST, aby se zabránilo chybám Neshoda typu. |
...
| Omezení svazku událostí databáze SQL |Při použití SQL Database jako cíl výstupu, může způsobit velký objem výstupních dat úlohy služby Stream Analytics vypršení časového limitu. Chcete-li tento problém vyřešit, snižte objem výstup pomocí agregací nebo operátory filtru, nebo zvolte úložiště objektů Blob v Azure nebo Event Hubs jako cíl výstupu místo toho. |
...
O další pomoc, vyzkoušení [fóru Azure Stream Analytics](https://social.msdn.microsoft.com/Forums/azure/home?forum=AzureStreamAnalytics)
