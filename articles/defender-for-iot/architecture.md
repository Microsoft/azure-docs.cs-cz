...
Defender pro senzory IoT se připojí k portu rozpětí nebo v síti a hned zahájí provádění DPI na síti IoT a v síťovém provozu. 
...
Detekce senzorů se zobrazují v konzole senzorů, kde je můžete zobrazit, prošetřit a analyzovat v mapě sítě, inventáři prostředků a v rozsáhlých sestavách, například v sestavách hodnocení rizik, dotazech dolování dat a vektorech útoků. Konzolu můžete také použít k zobrazení a zpracování hrozeb zjištěných moduly senzorů, předávajících informace do systémů třetích stran, správě uživatelů a dalších.

![Defender pro konzolu IoT snímač](./media/architecture/sensor-console.png)
...
Konzolu pro správu lze nasadit pro konfiguraci s vysokou dostupností, která zajišťuje konzolu zálohování, která pravidelně přijímá zálohy všech konfiguračních souborů vyžadovaných pro obnovení. Pokud hlavní konzola selže, zařízení pro správu místní lokality se automaticky převezmou při synchronizaci s konzolou zálohování, aby se zachovala dostupnost bez přerušení.

#### <a name="azure-portal"></a>portál Azure

Portál Defender pro IoT v Azure vám umožňuje: ·   Koupit zařízení řešení ·   Instalace a aktualizace softwaru ·   Připojit senzory do Azure ·   Aktualizace balíčků pro analýzu hrozeb
