...
3. V části **Přidat vCenter**zadejte popisný název hostitele vSphere nebo vCenter Server.
4. Zadejte IP adresu nebo plně kvalifikovaný název domény serveru.
5. Pokud vaše servery VMware nejsou konfigurované k naslouchání požadavkům na jiném portu, ponechte port 443.
6. Vyberte účet, který se používá pro připojení k serveru VMware vCenter nebo vSphere ESXi. Pak klikněte na **OK**.
...
1. Přihlaste se ke konfiguraci.
2. Pomocí zástupce na ploše otevřete nástroj konfiguračního serveru (cspsconfigtool. exe).
2. Na kartě **Spravovat účet** klikněte na **Přidat účet** .

   ![Přidat účet](./media/vmware-azure-manage-vcenter/addaccount.png)
   
3. Zadejte podrobnosti o novém účtu a klikněte na **OK**. Účet potřebuje oprávnění uvedená [výše](#account-permissions).
4. V trezoru > **Site Recovery infrastruktuře** > **konfiguračních severů**, otevřete konfigurační server.
5. V **podrobnostech**klikněte na **aktualizovat server**.
6. Po dokončení úlohy aktualizovat server vyberte vCenter Server.
7. V části **Souhrn**vyberte nově přidaný účet ve **službě Center Server/účet hostitele vSphere**a klikněte na **Uložit**.
