---
title: Sami otestovat klienta předběžně ověřit virtuálního počítače – Azure Marketplace | Dokumentace Microsoftu
description: Postup vytvoření samoobslužné testovacího klienta pro předběžné ověření image virtuálního počítače pro Azure Marketplace.
...
|  DNSName           |  Název DNS virtuálního počítače k testování    |
...
3. Zadejte parametry textu ve formátu JSON.
...
Následující snímek obrazovky ukazuje příklad výsledků testů JSON v online prohlížeč JSON (například [kód Beautify](https://codebeautify.org/jsonviewer) nebo [prohlížeč JSON](https://jsonformatter.org/json-viewer)).
...
Pro volání rozhraní API pomocí cURL, postupujte podle těchto kroků:
...
20. Vytvoření tajného klíče výběrem klíč **popis** textového pole. Vyplňte následující pole:
    ...
    - V části **hodnota**, klíč si zkopírujte.
     ...
     >[!Important]
     >Nebudete moci zobrazit hodnota klíče po ukončení **klíče** formuláře.
...
Jakmile úspěšně otestujete vašeho virtuálního počítače Azure, můžete [publikování nabídky](./cpp-publish-offer.md).
