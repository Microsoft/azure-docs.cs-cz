...
Pro **odchozí připojení se vynutilo tls 1,2**: v lednu 2020 Microsoft vynutila 1,2 TLS pro provoz uvnitř služby ve všech službách Azure. Pro Azure SQL Database Managed instance to vedlo k vymáhání TLS 1,2 na odchozích připojeních, která se používají pro replikaci a připojení serveru k SQL Server. Pokud používáte verze SQL Server starších než 2016 se správou instance, zajistěte, aby byly použity [specifické aktualizace TLS 1,2](https://support.microsoft.com/help/3135244/tls-1-2-support-for-microsoft-sql-server) .

Pro spravovanou instanci nejsou aktuálně podporovány následující funkce virtuální sítě:
- **Partnerský vztah Microsoftu**: povolení [partnerského vztahu Microsoftu](../expressroute/expressroute-faqs.md#microsoft-peering) na okruhech Express Route, na které se vztahuje přímý nebo průjezd s virtuální sítí, kde se spravovaná instance nachází, ovlivňuje tok přenosů mezi komponentami spravované instance v rámci virtuální sítě a služeb, které závisí na příčině potíží s dostupností. Nasazení spravovaných instancí do virtuální sítě s partnerským vztahem Microsoftu, které už je povolené, se očekává jako neúspěšné.
- **Globální partnerské vztahy virtuálních sítí**: připojení [partnerských vztahů virtuálních sítí](../virtual-network/virtual-network-peering-overview.md) napříč oblastmi Azure nefunguje pro spravovanou instanci z důvodu [dokumentovaných omezení nástroje pro vyrovnávání zatížení](../virtual-network/virtual-networks-faq.md#what-are-the-constraints-related-to-global-vnet-peering-and-load-balancers).
- **AzurePlatformDNS**: použití [značky služby](../virtual-network/service-tags-overview.md) AZUREPLATFORMDNS k blokování překladu DNS platformy by nedostupné spravované instance. I když spravovaná instance podporuje DNS definovaný zákazníkem pro překlad DNS v rámci motoru, existuje závislost na platformě DNS platformy pro operace platforem.
- **Brána NAT**: použití [Virtual Network NAT](../virtual-network/nat-overview.md) k řízení odchozího připojení s konkrétní veřejnou IP adresou by vygenerovalo spravovanou instanci nedostupnou. Služba Managed instance je aktuálně omezená na použití základního nástroje pro vyrovnávání zatížení, který neposkytuje korelace příchozích a odchozích toků s Virtual Network překladem adres (NAT).
