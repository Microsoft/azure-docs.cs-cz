...
* [MXNet](https://mxnet.io/): Flexibilní a efektivní knihovna hloubkového učení s mnoha jazykovými vazbami.
...
* [Torch](http://torch.ch/): Vědecky výpočetní architektura s rozsáhlou podporou algoritmů strojového učení.
* CUDA, cuDNN a ovladač NVIDIA.
* Mnoho ukázkových poznámkových bloků Jupyter
...
* Microsoft Machine Learning Server s Microsoft R Open.
* Distribuce Pythonu Anaconda (verze 2,7 a 3,5), včetně oblíbených knihoven analýz dat.
* JuliaPro, uspořádaná distribuce jazyka Helena s oblíbenými vědeckými a analytickými knihovnami pro analýzu dat.
* Samostatná instance Spark a Hadoop s jedním uzlem (HDFS, příze).
...
* Rozhraní příkazového řádku Azure pro správu prostředků Azure.
...
* Vývojové nástroje a editory (RStudio, PyCharm, IntelliJ, (Emacs), vim).
...
Odborníci přes data pomocí různých nástrojů k provedení těchto úloh. Vyhledání odpovídajících verzí softwaru může být časově náročné a pak je můžete stáhnout, zkompilovat a nainstalovat.

Virtuální počítač pro datové vědy pro Linux můžete podstatně usnadňují tato zatížení. Použijte ho k nastartování analytického projektu. Umožňuje vám pro práci na úkolech v různých jazycích, včetně R, Python, SQL, Java a C++. Zahrnuté ve virtuálním počítači Azure SDK můžete vytvářet své aplikace s použitím různých služeb v Linuxu pro cloudovou platformu Microsoft. Kromě toho máte přístup k dalším jazykům, jako jsou Ruby, Perl, PHP a Node. js, které jsou také předem nainstalovány.

Pro tuto DSVM image se neúčtují žádné poplatky za software. Platíte jenom Azure hardware poplatky za využití, které jsou vyhodnocené na základě velikosti virtuálního počítače, který zřídíte. Další informace o poplatkůch za výpočetní prostředky najdete na [stránce se seznamem virtuálních počítačů v Azure Marketplace](https://azure.microsoft.com/marketplace/partners/microsoft-ads/linux-data-science-vm/).

## <a name="other-versions-of-the-data-science-virtual-machine"></a>Jiné verze z virtuální počítač pro datové vědy

A [CentOS](linux-dsvm-intro.md) image je taky dostupná s řadu stejných nástrojů jako Ubuntu image. K dispozici je také bitová kopie [systému Windows](provision-vm.md) .

## <a name="prerequisites"></a>Požadavky

Než vytvoříte virtuální počítač pro datové vědy pro Linux, musíte mít předplatné Azure. Můžete [získat bezplatnou zkušební verzi Azure](https://azure.microsoft.com/free/).
...
1. Přejít na virtuální počítač výpisu na [Azure Portal](https://portal.azure.com/#create/microsoft-dsvm.linux-data-science-vm-ubuntulinuxdsvmubuntu). Může se zobrazit výzva, abyste se přihlásili ke svému účtu Azure, pokud ještě nejste přihlášení. 
1. Kliknutím na **vytvořit** otevřete průvodce.
    ![Průvodce vytvořením virtuálního počítače](./media/dsvm-ubuntu-intro/configure-data-science-virtual-machine.png)
1. Zadáním následujících informací nakonfigurujte jednotlivé kroky průvodce:
    ...
    1. **Základy**:
       ...
       * **Název virtuálního počítače**: Zadejte název serveru pro datové vědy, který vytváříte.
       * **Oblast:** Vyberte odpovídající datové centrum. Obvykle se jedná o datové centrum, které má většinu vašich dat, nebo je nejblíže vašemu fyzickému umístění pro nejrychlejší přístup k síti.
       * **Možnosti dostupnosti**: Tuto hodnotu nastavte, pokud chcete tento virtuální počítač použít v sadách nebo zónách dostupnosti. V opačném případě ponechte výchozí nastavení.
       * **Obrázek**: Nechte zadanou výchozí hodnotu.
       * **Velikost**: Vyberte typ serveru, který splňuje vaše funkční požadavky a omezení nákladů. Vyberte virtuální počítač NC nebo ND-Series pro instance virtuálních počítačů na bázi GPU. 
       * **Uživatelské jméno**: Zadejte uživatelské jméno správce.
       * **Veřejný klíč SSH**: Zadejte veřejný klíč RSA ve formátu single-line. (Místo klíče SSH můžete použít heslo.)
    ...
    1. Pro zbývající nastavení můžete použít výchozí hodnoty. Pokud chcete použít jiné než výchozí hodnoty, najeďte na informační odkaz pro nápovědu. Až budete hotovi, vyberte **zkontrolovat + vytvořit**.
    
    1. Jakmile virtuální počítač projde ověřením, ověřte, že jsou všechny zadané informace správné. Odkaz vás přesměruje na podmínek použití. Virtuální počítač nemá žádné další poplatky nad výpočetní kapacitu pro velikost serveru, kterou jste zvolili ve vstupu **velikosti** . Pokud chcete zahájit zřizování, vyberte **vytvořit**.
...
K Ubuntu DSVM můžete přistupovat pomocí tří metod:
...
1. Spusťte klienta X2Go a vyberte **novou relaci**. Otevře se okno Konfigurace s více karet. Zadejte následující parametry konfigurace:
   * **Karta relace**:
     * **Hostitel**: Zadejte název hostitele nebo IP adresu Data Science Virtual Machine pro Linux.
...
Po přihlášení k virtuálnímu počítači pomocí klienta SSH nebo grafické plochy desktop Xfce prostřednictvím klienta X2Go jste připraveni začít používat nástroje, které jsou nainstalované a nakonfigurované na virtuálním počítači. V desktop Xfce se můžete podívat na zástupce nabídky aplikace a ikony na ploše pro mnohé z těchto nástrojů.

### <a name="jupyterhub-and-jupyterlab"></a>JupyterHub a JupyterLab

Ubuntu DSVM spouští [JupyterHub](https://github.com/jupyterhub/jupyterhub), víceuživatelského Jupyter serveru. Pokud se chcete připojit, přejděte na\/přenosné počítače nebo desktopu na https:/your-VM-IP: 8000. Zadejte uživatelské jméno a heslo, které jste použili k vytvoření virtuálního počítače, a přihlaste se. Mnoho ukázkové poznámkové bloky jsou k dispozici pro procházení a vyzkoušejte si.

JupyterLab, generace poznámkové bloky Jupyter a JupyterHub, je také k dispozici. Abyste k němu měli přístup, přihlaste se k JupyterHub a pak přejděte na adresu\/URL https:/your-VM-IP: 8000/User/Your-username/Lab. Můžete nastavit JupyterLab jako výchozí Poznámkový Server přidáním tohoto řádku do `/etc/jupyterhub/jupyterhub_config.py`:
