...
Pokud chcete přesunout Recovery Services trezor do jiné skupiny prostředků, použijte rutinu `Move-AzureRMResource`. `Move-AzureRMResource` vyžaduje název prostředku a typ prostředku. Z rutiny `Get-AzureRmRecoveryServicesVault` můžete získat obojí.
