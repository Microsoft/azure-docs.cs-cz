...
Clustery HDInsight využívají [monohttps://mono-project.com) (](https://mono-project.com) ke spouštění aplikací .NET). Mono verze 4.2.1 je součástí HDInsight verze 3,6. Další informace o verzi mono, která je součástí služby HDInsight, najdete v tématu [Apache Hadoop komponenty dostupné s různými verzemi služby HDInsight](../hdinsight-component-versioning.md#apache-hadoop-components-available-with-different-hdinsight-versions).
...
* Schéma identifikátoru URI pro primární úložiště clusterů. Toto schéma bude `wasb://` Azure Storage `abfs://` pro Azure Data Lake Storage Gen2 nebo `adl://` pro Azure Data Lake Storage Gen1. Pokud je pro Azure Storage nebo Data Lake Storage Gen2 povolený zabezpečený přenos, identifikátor URI `wasbs://` by `abfss://`byl nebo v uvedeném pořadí.
...
1. Pomocí jednoho z následujících příkazů spusťte úlohu MapReduce:
   ...
   * Pokud je výchozí úložiště **Data Lake Storage Gen2**:
   ...
   |– soubory|Určuje soubory *Mapper. exe* a *snižoval. exe* pro tuto úlohu. Deklarace `wasbs:///`protokolu `adl:///`, nebo `abfs:///` předtím, než je každý soubor cestou k kořenovému adresáři výchozího úložiště pro cluster.|
