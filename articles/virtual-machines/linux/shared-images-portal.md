---
title: Vytváření sdílených ibi virtuálních počítačů Azure Linux pomocí portálu
description: Přečtěte si, jak pomocí portálu Azure vytvářet a sdílet ibionty virtuálních automatů na Linuxu.
author: cynthn
...
Chcete-li dokončit příklad v tomto článku, musíte mít existující spravované image generalizovaného virtuálního počítače nebo snímek specializovaného virtuálního počítače. Můžete postupovat [podle kurzu: Vytvořte vlastní image virtuálního počítače Azure s Azure PowerShell](tutorial-custom-images.md) vytvořit spravovanou image nebo [vytvořit snímek](../windows/snapshot-copy-managed-disk.md) pro specializovaný virtuální počítač. Pro spravované bitové kopie a snímky velikost datového disku nesmí být větší než 1 TB.
...
Teď můžete vytvořit jeden nebo více nových virtuálních ms. Tento příklad vytvoří virtuální ho s názvem *myVMfromImage*, v *myResourceGroup* v *usa východní* datového centra.
...
1. V **části Účet správce**, pokud byl zdrojový virtuální počítač zobecněn, zadejte své uživatelské **jméno** a veřejný **klíč SSH**. Pokud byl zdrojový virtuální virtuální soud specializovaný, budou tyto možnosti zašedlé, protože se používají informace ze zdrojového virtuálního soudu.
1. Pokud chcete povolit vzdálený přístup k virtuálnímu virtuálnímu serveru, v části **Veřejné příchozí porty**zvolte **Povolit vybrané porty** a pak z rozevíracího souboru vyberte **SSH (22).** Pokud nechcete povolit vzdálený přístup k virtuálnímu virtuálnímu serveru, ponechejte **možnost Žádný** pro veřejné **příchozí porty**.
