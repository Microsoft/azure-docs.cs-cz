---
title: Funkce stroje pravidel Azure CDN od Verizonu Premium | Dokumentace Microsoftu
...
Tento článek obsahuje podrobný popis funkcí dostupných pro Azure Content Delivery Network (CDN) [stroj pravidel](cdn-verizon-premium-rules-engine.md).
...
[Ignore Origin No-Cache](#ignore-origin-no-cache) | Určuje, zda CDN ignoruje některé direktivy ze zdrojového serveru.
...
## <a name="azure-cdn-from-verizon-premium-rules-engine-features-reference"></a>Azure CDN od funkce referenční informace ke stroji pravidel Verizon Premium
...
**Výchozí chování:** Pouze odpovědi GET jsou uložené v mezipaměti.
...
**Účel:** Určuje generování `Cache-Control` záhlaví POP, když je aktivní externí funkce Max-Age.
...
Předávání|Zajišťuje, `Cache-Control` záhlaví vytvořený pomocí funkce externí Max-Age se nikdy přidá do odpovědi. <br/> Pokud zdrojový server vytvoří `Cache-Control` záhlaví, prochází koncovému uživateli. <br/> Pokud zdrojový server se nevytvoří `Cache-Control` záhlaví, pak se tato možnost může způsobit, že hlavička odpovědi, aby neobsahovaly `Cache-Control` záhlaví.
...
--|--
 ...
 Vyloučení | Označuje, že pouze zadané parametry je vyloučena z klíče mezipaměti. Všechny další parametry řetězce dotazu jsou součástí klíče mezipaměti.
...
- **Typ:** Vyloučení
- **Parametry:** sessioned ID uživatele
...
**Účel:** Určuje generování `Expires` záhlaví POP, když je aktivní funkce externí Max-Age.
...
Hodnota|Výsledek
-|-
Enabled|Požadavky můžete přesměrovat.
...
**Výchozí chování:** Ve výchozím nastavení podporuje HTTP progresivního stahování MP4 a F4V média.
...
**Účel:** Určuje, zda požadavky klienta HTTP bez mezipaměti se předají ke zdrojovému serveru.
...
- Sada platný stavové kódy pro tuto funkci je: 200, 203, 300, 301, 302, 305, 307, 400, 401, 402, 403, 404, 405, 406, 407, 408, 409, 410, 411, 412, 413, 414, 415, 416, 417, 500, 501, 502, 503, 504 a 505.
...
**Účel:** Určuje, který bude vrácen pro klienty, pokud žádost vygeneruje 416 požadovaný rozsah nelze uspokojit stavový kód odpovědi.
...
Hodnota|Výsledek
-|-
Enabled|Zabrání bodů POP reagovat na požadavek neplatný rozsah bajtů se 416 požadovaný rozsah nelze uspokojit stavovým kódem. Servery se místo toho poskytovat požadované asset a 200 OK vraťte se do klienta.
...
**Účel:** Ovládací prvky, jak dlouho po čase normální vypršení platnosti mezipaměti asset může obsluhovat z místní nabídky, když lokalita POP není schopen se znovu ověřit v mezipaměti asset s původním serveru.
...
Hodnota|Výsledek
-|-
Enabled|Při nahrávání adresy URL v protokolu přístup, umožňuje ukládání řetězce dotazu. Pokud adresa URL neobsahuje řetězec dotazu, tato možnost nebude mít vliv.
...
**Účel:** Definuje maximální počet požadavků pro zachování připojení předtím, než je uzavřený.
...
Hodnota|Výsledek
-|-
Enabled|Požadavky můžete vygenerovat částečně uložené v mezipaměti obsahu.
...
**Výchozí chování:** Off. Opětovné ověření může provádět pouze po vypršení hodnoty TTL obsah uložený v mezipaměti.
...
**Účel:** Určuje, jak se žádost klienta HTTP pro prostředek 0 bajtů mezipaměti zařizuje služba bodů POP.

Platné hodnoty jsou:

Hodnota|Výsledek
--|--
Enabled|Způsobí, že POP, znovu načíst prostředek ze zdrojového serveru.
Zakázáno|Obnoví výchozí chování. Výchozí chování je poskytovat platnou mezipaměti prostředky na vyžádání.

Tato funkce není vyžadován pro správný ukládání do mezipaměti a doručování obsahu, ale může být užitečné jako alternativní řešení. Například generátory dynamického obsahu v původních serverů může způsobit neúmyslně odesílají do bodů POP odpovědi 0 bajtů. Tyto typy odpovědí jsou obvykle ukládají do mezipaměti podle bodů POP. Pokud víte, že je 0 bajtů odpověď nikdy platné odpovědi pro takový obsah, tuto funkci můžete zabránit tyto typy prostředků používané pro služby vašim klientům.
...
- Sada platný stavové kódy pro tuto funkci je: 203, 300, 301, 302, 305, 307, 400, 401, 402, 403, 404, 405, 406, 407, 408, 409, 410, 411, 412, 413, 414, 415, 416, 417, 500, 501, 502, 503, 504 a 505.
- Tuto funkci nelze použít k zakázání ukládání do mezipaměti pro odpovědi, které generují stavový kód 200 OK kódu.

**Výchozí chování:** Ukládání do mezipaměti je povoleno pouze pro odpovědi, které generují stavový kód 200 OK kódu.
...
- Chování této funkce se liší podle vybraného časovou jednotku.
    - **Časová jednotka:** Zadejte dobu a vyberte časovou jednotku (například několika sekund, minut, hodin, atd.) umožňující zastaralé doručování obsahu. Tento typ instalačního programu umožňuje CDN prodloužit dobu, která může doručovat obsah před vyžadování ověření podle následující vzorec: **Hodnota TTL** + **zastaralé při Revalidate čas**
    - **Vypnuto:** Vyberte "Off" vyžadovat revalidaci předtím, než se služba může poskytovat požadavek na starý obsah.
        - Nezadávejte časový úsek, protože je nepoužitelný a budou ignorovány.

**Výchozí chování:** Off. Opětovné ověření musí dojít předtím, než je možné dodávat požadovaný obsah.
...
302|Najít|Tímto stavovým kódem přesměruje na adresu URL zadat v hlavičce Location neoprávnění uživatelé. Tento kód stavu je standardní způsob provedení přesměrování.
...
404|Soubor nebyl nalezen|Stavový kód označuje, že klient HTTP byl schopen komunikovat se serverem, ale nenašel se požadovaný obsah.
...
Hodnota|Výsledek
---|----
Enabled|Způsobí, že POP, ignorovat velikost písmen při porovnávání adres URL pro ověřování pomocí tokenu parametry.
...
Hodnota|Výsledek
----|----
Enabled|Možnost Hodnota definuje název parametru řetězce dotazu, přes který by měla být definována tokeny.
...
Tato adresa URL přesměrování lze dosáhnout pomocí následující konfigurace: ![Adresa URL pro přesměrování](./media/cdn-rules-engine-reference/cdn-rules-engine-redirect.png)
...
Tato adresa URL přesměrování lze dosáhnout pomocí následující konfigurace: ![Adresa URL pro přesměrování](./media/cdn-rules-engine-reference/cdn-rules-engine-rewrite.png)

**Ukázkový scénář 2**

Tento příklad ukazuje, jak přesměrovat okraj CNAME adresu URL z velká písmena na malá písmena pomocí regulárních výrazů.

Tato adresa URL přesměrování lze dosáhnout pomocí následující konfigurace: ![Adresa URL pro přesměrování](./media/cdn-rules-engine-reference/cdn-rules-engine-to-lowercase.png)
