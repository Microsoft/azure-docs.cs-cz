...
1. Spuštěním tohoto příkazu PowerShellu Vygenerujte certifikát podepsaný svým držitelem. Upravte argument `-Subject` podle potřeby pro vaši aplikaci a Azure AD B2C název tenanta. Můžete také upravit datum `-NotAfter` a zadat pro certifikát jiné vypršení platnosti.
...
1. Otevřete okno **Správa uživatelských certifikátů** > **aktuální uživatel** > **osobních** > **certifikátů** > *YourAppName.yourtenant.onmicrosoft.com*
1. Vyberte **akci** > certifikátu > **všechny úlohy** > **exportu** .
...
1. Vyberte **Přidat**a potom vyberte **Možnosti** > **nahrát**.
...
Vyhledejte část `<ClaimsProviders>` a přidejte následující fragment kódu XML.

Můžete změnit hodnotu metadat `IssuerUri`. Toto je identifikátor URI vystavitele vrácený v odpovědi SAML z Azure AD B2C. Aplikace předávající strany by měla být nakonfigurovaná tak, aby přijímala identifikátor URI vystavitele během ověřování kontrolního výrazu SAML.
...
1. Změňte `PolicyId` a `PublicPolicyUri` zásady na _B2C_1A_signup_signin_saml_ a `http://tenant-name.onmicrosoft.com/B2C_1A_signup_signin_saml`, jak vidíte níže.
...
1. Nahraďte celý `<TechnicalProfile>` prvek v `<RelyingParty>` elementu následujícím technickým profilem XML.
...
1. Aktualizujte `tenant-name` s názvem vašeho tenanta Azure AD B2C.
...
Pro účely tohoto kurzu, ve kterém používáte aplikaci testu SAML, nastavte vlastnost `url` `replyUrlsWithType` na hodnotu zobrazenou v následujícím fragmentu kódu JSON.
...
Pro účely tohoto kurzu, který používá testovací aplikaci SAML, nechte `logoutUrl` nastavenou na `https://samltestapp2.azurewebsites.net/logout`:
...
* **Certifikát**: toto je *B2C_1A_SamlIdpCert*, ale bez privátního klíče. Získání veřejného klíče certifikátu:
    ...
    1. Zkopírujte hodnotu v prvku `<X509Certificate>`.
...
Vyberte **přihlášení** a měli byste se dodávat s přihlašovací obrazovkou pro koncové uživatele. Po přihlášení je kontrolní výraz SAML vydán zpět do ukázkové aplikace.
