...
| --- | --- |
| **Skupiny dostupnosti** |Repliky dostupnosti běžící na virtuálních počítačích Azure ve stejné oblasti poskytují vysokou dostupnost. Pro zajištění vyšší redundance a dostupnosti můžete virtuální počítače Azure nasadit v různých [zónách dostupnosti](https://docs.microsoft.com/en-us/azure/availability-zones/az-overview) , jak je popsáno [zde](https://docs.microsoft.com/en-us/azure/virtual-machines/windows/sql/virtual-machines-windows-portal-sql-availability-group-overview). Pokud jsou virtuální počítače s SQL Server ve skupině dostupnosti nasazené v zónách dostupnosti, pak použijte službu [Load Balancer úrovně Standard](https://docs.microsoft.com/en-us/azure/load-balancer/load-balancer-standard-overview) pro naslouchací proces, jak je uvedeno v těchto článcích – [Azure SQL VM CLI](https://docs.microsoft.com/en-us/azure/virtual-machines/windows/sql/virtual-machines-windows-sql-availability-group-cli) & [šablony pro rychlý Start Azure](https://docs.microsoft.com/en-us/azure/virtual-machines/windows/sql/virtual-machines-windows-sql-availability-group-quickstart-template). Musíte nakonfigurovat virtuální počítač řadiče domény, protože Clustering s podporou převzetí služeb při selhání systému Windows vyžaduje doménu služby Active Directory.<br/> @no__t – 0Availability skupiny @ no__t-1<br/>Další informace najdete v tématu [Konfigurace skupin dostupnosti v Azure (GUI)](virtual-machines-windows-portal-sql-alwayson-availability-groups.md). |
| **Instance clusteru převzetí služeb při selhání** |Instance clusterů s podporou převzetí služeb při selhání (FCI), které vyžadují sdílené úložiště, se dají vytvořit 3 různými způsoby.<br/><br/>1. cluster s podporou převzetí služeb při selhání se dvěma uzly, který běží na virtuálních počítačích Azure s připojeným úložištěm pomocí [Windows serveru 2016 Prostory úložiště s přímým přístupem \(S2D @ no__t-2](virtual-machines-windows-portal-sql-create-failover-cluster.md) pro poskytování softwarově vycházejících virtuálních sítí San<br/><br/>2. cluster s podporou převzetí služeb při selhání se dvěma uzly běžícím na virtuálních počítačích Azure s úložištěm podporovaným řešením clusteringu třetí strany. Konkrétní příklad, který používá s DataKeeper, najdete v tématu [Vysoká dostupnost sdílené složky pomocí clusteringu s podporou převzetí služeb při selhání a softwaru třetích stran s Datakeepers](https://azure.microsoft.com/blog/high-availability-for-a-file-share-using-wsfc-ilb-and-3rd-party-software-sios-datakeeper/).<br/><br/>3. cluster s podporou převzetí služeb při selhání se dvěma uzly, který běží na virtuálních počítačích Azure s sdíleným blokovým úložištěm cíle iSCSI přes ExpressRoute. Například NetApp Private Storage (NPS) zveřejňuje cíl iSCSI prostřednictvím ExpressRoute s Equinix do virtuálních počítačů Azure.<br/><br/>Pro řešení sdíleného úložiště a replikace dat třetích stran byste se měli obrátit na dodavatele s případnými problémy souvisejícími s přístupem k datům při převzetí služeb při selhání.<br/><br/>Všimněte si, že použití FCI na [Azure File Storage](https://azure.microsoft.com/services/storage/files/) ještě není podporované, protože toto řešení nevyužívá Premium Storage. Pracujeme na podpoře tohoto brzo. |
...
| **Replikace a převzetí služeb při selhání SQL Server do Azure s využitím Azure Site Recovery** |Produkční SQL Server jednoho datacentra Azure se replikují přímo do Azure Storage jiného datacentra Azure pro zotavení po havárii.<br/>@no__t – 0Replicate pomocí Azure Site Recovery @ no__t-1<br/>Další informace najdete v tématu [ochrana SQL Server pomocí SQL Server zotavení po havárii a Azure Site Recovery](../../../site-recovery/site-recovery-sql.md). |
...
| **Replikace a převzetí služeb při selhání SQL Server do Azure s využitím Azure Site Recovery** |Místní produkční SQL Server replikované přímo do Azure Storage pro zotavení po havárii.<br/>@no__t – 0Replicate pomocí Azure Site Recovery @ no__t-1<br/>Další informace najdete v tématu [ochrana SQL Server pomocí SQL Server zotavení po havárii a Azure Site Recovery](../../../site-recovery/site-recovery-sql.md). |
