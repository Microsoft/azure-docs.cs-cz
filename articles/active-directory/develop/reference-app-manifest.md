...
|---------|---------|---------|---------|
| `accessTokenAcceptedVersion` | Hodnota Int32 s možnou hodnotou null | Určuje verzi přístupového tokenu, kterou prostředek očekával. Tím se změní verze a formát tokenu JWT vyprodukovaný nezávisle na koncovém bodu nebo klientovi, který se použil k vyžádání přístupového tokenu.<br/><br/>Koncový bod použitý v 1.0 nebo v 2.0 je vybraný klientem a má vliv jenom na verzi id_tokens. Prostředky musí explicitně nakonfigurovat `accesstokenAcceptedVersion`, aby označovaly formát podporovaného přístupového tokenu.<br/><br/>Možné hodnoty pro `accesstokenAcceptedVersion` jsou 1, 2 nebo null. Pokud je hodnota null, je výchozí hodnota 1, což odpovídá koncovému bodu v 1.0. <br/><br/>Pokud je `signInAudience` `AzureADandPersonalMicrosoftAccount`, musí být hodnota `2`.  | `2` |
...
| `groupMembershipClaims` | Řetězec | Nakonfiguruje deklaraci identity `groups` vydanou v uživatelském nebo přístupovém tokenu OAuth 2,0, který aplikace očekává. Chcete-li nastavit tento atribut, použijte jednu z následujících platných řetězcových hodnot:<br/><br/>- `"None"`<br/>-  @ no__t-1 (pro skupiny zabezpečení a role Azure AD)<br/>-  @ no__t-1 (zobrazí se všechny skupiny zabezpečení, distribuční skupiny a role adresáře Azure AD, kterých je přihlášený uživatel členem. | `"SecurityGroup"` |
...
| `identifierUris` | Pole řetězců | Uživatelsky definované identifikátory URI, které jedinečně identifikují webovou aplikaci v rámci jejího tenanta Azure AD, nebo v ověřené vlastní doméně, pokud je aplikace více tenantů. | <code>[<br>&nbsp;&nbsp;"https://MyRegisteredApp"<br>]</code> |
...
| `parentalControlSettings` | Řetězec | `countriesBlockedForMinors` určuje země, ve kterých je aplikace blokovaná pro nezletilé.<br>`legalAgeGroupRule` Určuje pravidlo platné pro věkovou skupinu, které se vztahuje na uživatele aplikace. Dá se nastavit na `Allow`, `RequireConsentForPrivacyServices`, `RequireConsentForMinors`, `RequireConsentForKids` nebo `BlockMinors`.  | <code>{<br>&nbsp;&nbsp;&nbsp;"countriesBlockedForMinors":[],<br>&nbsp;&nbsp;&nbsp;"legalAgeGroupRule":"Allow"<br>} </code> |
...
| `replyUrlsWithType` | Kolekce | Tato vlastnost s více hodnotami obsahuje seznam registrovaných hodnot redirect_uri, které služba Azure AD přijme jako cílová umístění při vracení tokenů. Každá hodnota identifikátoru URI by měla obsahovat přidruženou hodnotu typu aplikace. Podporované hodnoty typu jsou: <ul><li>`Web`</li><li>`InstalledClient`</li></ul><br> Přečtěte si další informace o [omezeních a](https://docs.microsoft.com/azure/active-directory/develop/reply-url)omezeních replyUrl. | <code>"replyUrlsWithType":&nbsp;[<br>&nbsp;&nbsp;{<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"url":&nbsp;"https://localhost:4400/services/office365/redirectTarget.html",<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"type":&nbsp;"InstalledClient"&nbsp;&nbsp;&nbsp;<br>&nbsp;&nbsp;}<br>]</code> |
| `requiredResourceAccess` | Kolekce | V případě dynamického souhlasu `requiredResourceAccess` jednotky a prostředí pro vyjádření souhlasu správce s uživatelským souhlasem pro uživatele, kteří používají statický souhlas. To však nezpůsobuje uživatelské prostředí pro vyjádření souhlasu uživatele pro obecný případ.<br>`resourceAppId` je jedinečný identifikátor pro prostředek, ke kterému aplikace vyžaduje přístup. Tato hodnota by měla být stejná jako appId deklarované v cílové aplikaci prostředků.<br>`resourceAccess` je pole, ve kterém jsou uvedeny obory oprávnění OAuth 2.0 a aplikační role, které aplikace vyžaduje ze zadaného prostředku. Obsahuje hodnoty `id` a `type` zadaných prostředků. | <code>[<br>&nbsp;&nbsp;{<br>&nbsp;&nbsp;&nbsp;&nbsp;"resourceAppId":"00000002-0000-0000-c000-000000000000",<br>&nbsp;&nbsp;&nbsp;&nbsp;"resourceAccess":[<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"id":"311a71cc-e848-46a1-bdf8-97ff7156d8e6",<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"type":"Scope"<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}<br>&nbsp;&nbsp;&nbsp;&nbsp;]<br>&nbsp;&nbsp;}<br>] </code> |
...
| `tags` | Pole řetězců | Vlastní řetězce, které lze použít ke kategorizaci a identifikaci aplikace. | <code>[<br>&nbsp;&nbsp;"ProductionApp"<br>]</code> |
