...
3. V instalačním programu na **úvodní** klikněte na tlačítko **Další**. Na stránce **Licenční podmínky** kliknutím na **Souhlasím** přijměte licenci.
...
Nepodporovaný operační systém Windows | Azure podporuje pouze [vybrané verze operačního systému Windows](https://aka.ms/WSosstatement), zvažte upgradování operačního systému na počítač před migrací do Azure.
Podmíněně schválený operační systém Linux | Azure nezodpovídá pouze [vybrané verze operačního systému Linux](../virtual-machines/linux/endorsed-distros.md), zvažte upgradování operačního systému na počítač před migrací do Azure.
Neschválený operační systém Linux | Tento počítač může spustit v Azure, ale žádná podpora operačního systému je k dispozici v Azure, zvažte upgradování operačního systému a [schválené pro Linux verze](../virtual-machines/linux/endorsed-distros.md) před migrací do Azure
...
| 754       | NoPerfDataAvaialable           | Nejsou k dispozici žádné údaje o výkonu.                                               | Zkontrolujte úroveň statistiky v systému vCenter Server. Musí být nastavená na 3 pro data výkonu k dispozici. | Změňte úroveň statistiky na 3 (pro intervaly o délkách 5 minut, 30 minut a 2 hodiny), počkejte aspoň jeden den a zkuste to znovu.                   |
...
| 758       | GetPerfDataTimeout             | Vypršel časový limit požadavku VCenter. Zpráva % Message;                                  | Přihlašovací údaje k vCenter Serveru nejsou správné.                                                              | Zkontrolujte přihlašovací údaje serveru vCenter a ujistěte se, že tento Server vCenter dostupný. Opakujte operaci. Pokud potíže potrvají, obraťte se na podporu. |
| 759       | VmwareDllNotFound              | Nenašla se knihovna DLL VMWare.Vim.                                                     | PowerCLI není správně nainstalovaný.                                                                   | Zkontrolujte, zda je správně nainstalován PowerCLI. Opakujte operaci. Pokud potíže potrvají, obraťte se na podporu.                               |
...
| 801       | PowerCLIError                  | Nepovedlo se nainstalovat VMware PowerCLI.                                          | Nepovedlo se nainstalovat VMware PowerCLI.                                                                  | Opakujte operaci. Pokud potíže potrvají, nainstalujte ručně a operaci opakujte.                                                   |
...
| 703       | OMSHttpRequestException        | Při odesílání žádosti došlo k chybě. Zpráva % Message;                                | Zkontrolujte ID a klíč projektu a ujistěte se, že je koncový bod dostupný.                                       | Opakujte operaci. Pokud potíže potrvají, kontaktujte prosím podporu Microsoftu.                                                                     |
