---
title: Přehled back-endu s více klienty, jako je Azure App Service, s využitím Azure Application Gateway
...
- Možnost nastavit název hostitele na pevnou hodnotu explicitně zadaný v nastavení HTTP. Tato možnost zajistí, že se hlavička hostitele přepíše na tuto hodnotu pro veškerý provoz do fondu back-end, kde se používají konkrétní nastavení protokolu HTTP. Při použití koncového šifrování protokolu SSL se přepsaný název hostitele použije v rozšíření SNI. Tato možnost umožňuje scénářům, kdy skupina back-end fondu očekává hlavičku hostitele, která se liší od hlavičky příchozího hostitele zákazníka.

- Schopnost odvodit název hostitele z IP adresy nebo plně kvalifikovaného názvu domény členů fondu back-end. Nastavení HTTP taky nabízí možnost dynamicky vybrat název hostitele z plně kvalifikovaného názvu domény člena fondu back-endu, pokud je nakonfigurovaný parametr s možností odvodit název hostitele od jednotlivých členů fondu back-end. Při použití koncového šifrování protokolu SSL se tento název hostitele odvodí z plně kvalifikovaného názvu domény a použije v rozšíření SNI. Tato možnost umožňuje scénářům, kdy fond back-endu může mít dvě nebo víc PaaS služeb pro více tenantů, jako jsou webové aplikace Azure, a záhlaví hostitele žádosti pro každého člena obsahuje název hostitele odvozený z jeho plně kvalifikovaného názvu domény. V případě implementace tohoto scénáře používáme v nastavení protokolu HTTP přepínač s názvem [Vybrat název hostitele z back-endu adresy](https://docs.microsoft.com/azure/application-gateway/configuration-overview#pick-host-name-from-back-end-address) , ve kterém bude dynamicky přepsána Hlavička hostitele v původní žádosti na tu, která je uvedena ve fondu back-end.  Pokud Váš plně kvalifikovaný název domény ve fondu back-end obsahuje například "contoso11.azurewebsites.net" a "contoso22.azurewebsites.net", Hlavička hostitele původní žádosti, která je contoso.com, bude přepsána na contoso11.azurewebsites.net nebo contoso22.azurewebsites.net Když se požadavek pošle na příslušný back-end Server. 
...
### <a name="ssl-termination-and-end-to-end-ssl-with-multi-tenant-services"></a>Ukončení SSL a koncové šifrování protokolu SSL pomocí víceklientské služby

Pro víceklientské služby se podporuje ukončení protokolu SSL i koncové šifrování protokolu SSL. V případě ukončení protokolu SSL ve službě Application Gateway bude certifikát SSL i nadále vyžadován k přidání do naslouchacího procesu služby Application Gateway. V případě koncového protokolu SSL ale důvěryhodné služby Azure, jako například Azure App Service Web Apps, nevyžadují seznam povolených back-endu ve službě Application Gateway. Proto není nutné přidávat žádné ověřovací certifikáty. 

![koncové šifrování protokolu SSL](./media/application-gateway-web-app-overview/end-to-end-ssl.png)
