...
V tomto článku se dozvíte, jak upgradovat mikroslužbu v rámci aplikace. V tomto příkladu změníme službu `WebFrontEnd` tak, aby zobrazila kategorii úlohy a zvýšila množství procesoru, které je mu přiděleno. Pak upgraduje nasazenou službu.
...
Aktualizujeme parametr `WebFrontEnd_cpu`, aby se aktualizovaly prostředky procesoru tak, aby se `1.5` při předvídání, že se služba **webendu** bude intenzivně používat.

1. V projektu **todolistapp** otevřete v části **prostředí** > **cloudu**soubor **Parameters. yaml** . Upravte `WebFrontEnd_cpu`, hodnotu na `1.5`. Název parametru je s názvem služby `WebFrontEnd_` jako osvědčený postup pro odlišení od parametrů stejného názvu, které platí pro různé služby.
...
2. Otevřete soubor **Service. yaml** projektu **webendu** v části **prostředky služby** **webendu** > .
    ...
    Všimněte si, že v části `resources:` `cpu:` je nastavená na `"[parameters('WebFrontEnd_cpu')]"`. Pokud projekt sestavíte pro Cloud, hodnota `'WebFrontEnd_cpu` bude pocházet z **prostředí** >  > souboru **parametrů** **cloudu** a bude `1.5`. Pokud je projekt sestaven tak, aby běžel místně, hodnota bude provedena z **prostředí** > **místní** > souboru **Parameters. yaml** a bude "0,5".
...
Chcete-li zavést změnu kódu, přidejte do `ToDoItem` třídy v souboru `ToDoItem.cs` vlastnost `Category`.
...
Pak aktualizujte metodu `Load()` ve stejném souboru, chcete-li nastavit kategorii na výchozí řetězec:
...
`WebFrontEnd` projekt je ASP.NET Core aplikace s webovou stránkou, která zobrazuje položky seznamu úkolů. V projektu `WebFrontEnd` otevřete `Index.cshtml` a přidejte následující dva řádky, které jsou uvedené níže, aby se zobrazila Kategorie úkolu:
...
Pomocí rozevíracího seznamu **cílový profil** vyberte soubor Profile. yaml, který chcete použít pro toto nasazení. Upgradujete aplikaci v cloudu, takže v rozevíracím seznamu vyberete **Cloud. yaml** , ve kterém se použije hodnota `WebFrontEnd_cpu` 1,0 definovaná v tomto souboru.
