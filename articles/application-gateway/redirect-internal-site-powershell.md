...
Prostředí Azure PowerShell můžete použít ke konfiguraci [přesměrování webového provozu](multiple-site-overview.md) při vytváření [aplikační brány](overview.md). V tomto článku definujete back-end fond pomocí sady škálování virtuálních počítačů. Pak můžete nakonfigurovat naslouchací procesy a pravidla na základě domén, které vlastníte, aby se zajistilo, že webový provoz přijde do příslušného fondu. V tomto článku se předpokládá, že vlastníte více domén a používáte příklady *webových \. contoso.com* a *www \. contoso.org*.
...
Naslouchací proces je potřeba k tomu, aby brána Application Gateway mohla správně směrovat provoz na back-endový fond. V tomto článku vytvoříte dva naslouchací procesy pro vaše dvě domény. V tomto příkladu se pro domény *webových \. contoso.com* a *www \. contoso.org* vytvoří naslouchací procesy.
...
- [Přehled přesměrování Application Gateway](redirect-overview.md)
