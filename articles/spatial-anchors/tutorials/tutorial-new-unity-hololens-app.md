---
title: Kurz – podrobné pokyny k vytvoření nové aplikace Unity pro HoloLens pomocí prostorových kotev Azure | Microsoft Docs
...
7. Uložte prázdnou výchozí scénu do nového souboru pomocí: Soubor > **uložte jako**.
...
Chcete-li otestovat, že vše funguje, sestavte aplikaci v **Unity** a nasaďte ji ze sady **Visual Studio**. Postupujte podle kapitoly 6 ze [ **základů Mr 100: Začínáme s kurzem** ](https://docs.microsoft.com/windows/mixed-reality/holograms-100#chapter-6---build-and-deploy-to-device-from-visual-studio) Unity. Měli byste vidět úvodní obrazovku Unity a pak Vymazat displej.
...
Nyní byste měli mít nastavenou **koule** jako Prefab na vašem skriptu. Sestavte z **Unity** a pak znovu otevřete výsledné řešení sady **Visual Studio** , stejně jako jste [to](#trying-it-out)právě zkoušeli. 

V **aplikaci Visual Studio**otevřete `AzureSpatialAnchorsScript.cs` znovu. Do své `Start()` metody přidejte následující kód. Tento kód se `GestureRecognizer`zaplní, který se detekuje, když dojde k klepnutí vzduchu a volání `HandleTap`.
...
V novém okně pro **Import balíčku Unity** , které se zobrazí, vyberte v levém dolním rohu **žádné** . Pak v části**moduly plug-in** **AzureSpatialAnchorsPlugin** > vyberte **Common**, **Editor**a **HoloLens**. V pravém dolním rohu klikněte na **importovat** .
...
Nyní zapojte `Start()` metodu do vaší metody. `initializeSession()`
...
Jakmile budete mít identifikátor účtu a klíč prostorových ukotvení Azure, přejdete do části `Account Id` `Account Key` `SpatialAnchorsAccountKey`a `SpatialAnchorsAccountId` vložíte do.

Nakonec připojovat všechno dohromady. `SpawnNewAnchoredObject()` Do metody přidejte následující kód. Vyvolá `CreateAnchorAsync()` metodu hned po vytvoření vaší koule. Jakmile se metoda vrátí, kód níže provede jednu poslední aktualizaci vaší koule, přičemž změna barvy na modrou.
...
> [!NOTE]
> Pokud získáte "DllNotFoundException –: Nepovedlo se načíst DLL ' AzureSpatialAnchors ': Zadaný modul se nepovedlo najít. ", měli byste řešení **vyčistit** a znovu **sestavit** .
...
* Volání `ResetSession()`, které `CloudSpatialAnchorSession` zastaví a odebere z obrazovky existující modrou koule.
