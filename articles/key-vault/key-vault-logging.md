...
> [!NOTE]
> Tento kurz neobsahuje pokyny k vytvoření trezorů klíčů, klíčů nebo tajných klíčů. Tyto informace najdete v tématu [co je Azure Key Vault?](key-vault-overview.md). Další pokyny pro Azure CLI pro různé platformy najdete v [tomto ekvivalentním kurzu](key-vault-manage-with-cli2.md).
>
...
## <a id="connect"></a>Připojení k předplatnému trezoru klíčů
...
## <a id="storage"></a>Vytvoření účtu úložiště pro vaše protokoly
...
Pro další snadné řízení používáme také stejnou skupinu prostředků jako ta, která obsahuje Trezor klíčů. V [úvodním kurzu](key-vault-get-started.md)se tato skupina prostředků jmenuje **ContosoResourceGroup**a budeme dál používat východní Asie umístění. Tyto hodnoty nahraďte vlastními, a to podle potřeby:
...
V [úvodním kurzu](key-vault-get-started.md)se název trezoru klíčů **ContosoKeyVault**. Budeme dál používat tento název a uložíme podrobnosti do proměnné s názvem **KV**:
...
## <a id="loganalytics"></a>Použití protokolů Azure Monitor
...
Kurz týkající se střídání klíčů a auditování protokolů pomocí Azure Key Vault najdete v tématu [nastavení Key Vault s koncovým otočením a auditováním klíčů](key-vault-key-rotation-log-monitoring.md).
