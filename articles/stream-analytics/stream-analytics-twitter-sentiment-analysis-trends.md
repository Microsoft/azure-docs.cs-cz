...
* Nainstalujte [.NET Core CLI](https://docs.microsoft.com/dotnet/core/tools/?tabs=netcore2x).
...
2. Pomocí textového editoru otevřete soubor *App. config* . Proveďte následující změny `<appSettings>` elementu:
   ...
   * Nastavte `oauth_consumer_secret` na příjemce Twitteru (tajný klíč rozhraní API).
...
3. Otevřete příkazový řádek a přejděte do adresáře, kde se nachází vaše aplikace TwitterClientCore. K sestavení projektu použijte `dotnet build` příkazu. Pak použijte příkaz `dotnet run` ke spuštění aplikace. Aplikace pošle tweety do centra událostí.
...
3. Vyberte **Create** (Vytvořit). Až se nasazení dokončí, přejděte k vaší úloze.
...
2. Vyberte **+&nbsp;přidat vstup streamu** > **centrum událostí**. Vyplňte **nový vstupní** formulář s následujícími informacemi:

   |**Nastavení**  |**Navrhovaná hodnota**  |**Popis**  |
   |---------|---------|---------|
   |Alias pro vstup| *TwitterStream* | Zadejte alias pro vstup. |
...
2. Na stránce **výstupy** klikněte na **+&nbsp;přidat** a **BLOB Storage/Data Lake Storage Gen2**:
   ...
   * **Kontejner**. Vyberte **vytvořit novou** a zadejte `socialtwitter`.
