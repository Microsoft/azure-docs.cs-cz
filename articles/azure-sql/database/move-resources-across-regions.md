...
description: Naučte se přesunout Azure SQL Database nebo spravovanou instanci Azure SQL do jiné oblasti.
...
Tento článek vás seznámí s obecným pracovním postupem, jak přesunout vaši Azure SQL Database nebo spravovanou instanci Azure SQL do nové oblasti.

## <a name="overview"></a>Přehled

Existují různé scénáře, ve kterých byste chtěli přesunout existující Azure SQL Database nebo spravovanou instanci z jedné oblasti do druhé. Například rozšíříte svou firmu do nové oblasti a chcete ji optimalizovat pro novou zákaznickou základnu. Nebo je nutné operace přesunout do jiné oblasti z důvodu dodržování předpisů. Nebo Azure vydala značku – novou oblast, která poskytuje lepší blízkost a zlepšuje prostředí pro zákazníky.  
...
## <a name="move-azure-sql-database"></a>Přesunout Azure SQL Database
...
1. Vytvořte [skupinu převzetí služeb při selhání](failover-group-add-single-database-tutorial.md#2---create-the-failover-group) mezi serverem zdroje na serveru cíle.  
...
1. Ověřte, že se příkaz úspěšně dokončil pomocí příkazu `nslook up <fog-name>.secondary.database.windows.net` , aby se ověřilo, že záznam DNS CNAME odkazuje na IP adresu cílové oblasti. Pokud příkaz switch selže, záznam CNAME se nebude aktualizovat.
...
1. Pro audity na úrovni serveru zajistěte, aby:
    - Kontejner úložiště, Log Analytics nebo centrum událostí s existujícími protokoly auditu se přesunou do cílové oblasti.
    - Konfigurace auditu je nakonfigurovaná na cílovém serveru. Další informace najdete v tématu [auditování služby SQL Database](../../azure-sql/database/auditing-overview.md).
1. Pokud má vaše instance dlouhodobé zásady uchovávání informací (LTR), existující zálohy LTR zůstanou spojené s aktuálním serverem. Vzhledem k tomu, že cílový server je jiný, budete mít přístup k starším zálohám LTR ve zdrojové oblasti pomocí zdrojového serveru, i když se server odstraní.
...
## <a name="move-azure-sql-managed-instance"></a>Přesunout spravovanou instanci Azure SQL

### <a name="verify-prerequisites"></a>Ověření požadavků

1. U každé zdrojové spravované instance SQL vytvořte cílovou instanci SQL spravované instance se stejnou velikostí v cílové oblasti.  
1. Nakonfigurujte síť pro spravovanou instanci SQL. Další informace najdete v tématu [Konfigurace sítě](../managed-instance/how-to-content-reference-guide.md#network-configuration).
...
1. Pokud jsou vaše databáze šifrované pomocí TDE a používají vlastní šifrovací klíč v Azure Key Vault, zajistěte, aby Azure Key Vault se stejnými šifrovacími klíči existovaly ve zdrojové i cílové oblasti. Další informace najdete v tématu [TDE s klíči spravovanými zákazníky v Azure Key Vault](transparent-data-encryption-byok-overview.md).
1. Pokud je pro spravovanou instanci SQL povolený audit, ujistěte se, že:
    - Kontejner úložiště nebo centrum událostí s existujícími protokoly se přesune do cílové oblasti.
    - V cílové instanci je nakonfigurovaný audit. Další informace najdete v tématu [auditování pomocí spravované instance](../managed-instance/auditing-configure.md).
...
Vytvořte skupinu převzetí služeb při selhání mezi jednotlivými zdroji spravované instance SQL a odpovídající cílovou instancí SQL.
...
1. Připojte se k cílové instanci SQL spravované pomocí sekundárního koncového bodu `<fog-name>.secondary.database.windows.net` .
...
Po migraci Azure SQL Database [Spravovat](manage-data-after-migrating-to-database.md) .
