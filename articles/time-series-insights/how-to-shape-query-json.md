...
ms.locfileid: "65790068"
---
# <a name="how-to-shape-json-to-maximize-query-performance"></a>Jak tvarovat JSON pro zajištění maximálního výkonu dotazů 

Tento článek obsahuje pokyny pro úpravu JSON pro maximalizaci efektivity vašich dotazů Azure Time Series Insights.
...
1. Zajistěte omezení maximální vlastnost Time Series Insights není přístupů:
...
1. Neposílat zbytečné vlastnosti. Pokud vlastnost dotazu není povinné, doporučujeme odeslat, a vyhněte se omezení úložiště.
...
1. Nepoužívejte vnoření hloubkové pole. Time Series Insights podporuje až dvě úrovně vnořená pole, které obsahují objekty. Time Series Insights pole ve zprávách, sloučí do více událostí pomocí dvojice vlastnost.
...
> [!TIP]
> **Doporučení**: odeslat každý měření/signál jako samostatný vlastnost nebo sloupec.
...
* Referenční data tabulky (klíčovou vlastnost **deviceId**):
...
- Referenční data slouží k omezení počtu bajtů přenesených v síti. Dva atributy **messageId** a **deviceLocation**, jsou spojeny pomocí klíčová vlastnost **deviceId**. Tato data je spojen s telemetrická data v době příchozího přenosu dat a následně uložená v Time Series Insights pro dotazování.
...
* Referenční Data (jsou klíčové vlastnosti **deviceId** a **series.tagId**):
