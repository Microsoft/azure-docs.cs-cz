...
* Zřiďte aplikaci pomocí starší verze portálu pro Azure AD. Další informace najdete v tématu [Registrace aplikace](quickstart-register-app.md) .
...
App1 URI pro přesměrování: `x-msauth-mytestiosapp://com.myapp.mytestapp`

App2 URI pro přesměrování: `x-msauth-mytestiosapp://com.myapp.mytestapp2`

APP3 URI pro přesměrování: `x-msauth-mytestiosapp://com.myapp.mytestapp3`
...
Jakmile budete mít v každé aplikaci povolený nárok na řetězce klíčů a jste připraveni použít jednotné přihlašování, řekněte sadě identity SDK o vašem řetězci pomocí následujícího nastavení v `ADAuthenticationSettings` s následujícím nastavením:
...
```
Nastavení `AD_CREDENTIALS_AUTO` umožní sadě SDK, aby se pokusila zavolat službě Broker, `AD_CREDENTIALS_EMBEDDED` zabrání sadě SDK v volání do zprostředkovatele.
...
Tento identifikátor URI pro přesměrování musí být zadaný v registraci vaší aplikace pomocí [Azure Portal](https://portal.azure.com/). Další informace o registraci aplikace služby Azure AD najdete v tématu [integrace s Azure Active Directory](active-directory-how-to-integrate.md).
...
ADAL používá – canOpenURL: ke kontrole, jestli je zprostředkovatel nainstalovaný na zařízení. V systému iOS 9 on se společnost Apple zablokuje v tom, v jakých schématech může aplikace dotazovat. Do části LSApplicationQueriesSchemes v `info.plist file`budete muset přidat "msauth".
