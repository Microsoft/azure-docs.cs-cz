...
Oprávnění OAuth2 jsou obory oprávnění, které aplikace Azure Active Directory (Azure AD) pro vývojáře webového rozhraní API (v 1.0) zpřístupňuje klientským aplikacím. Tyto obory oprávnění se můžou klientským aplikacím udělit během souhlasu. Informace najdete `oauth2Permissions` v části v tématu [Azure Active Directory Reference k manifestu aplikace](reference-app-manifest.md#manifest-reference).
...
Chcete-li získat tokeny pro konkrétní obory aplikace v 1.0 (například rozhraní Microsoft Graph API, což https://graph.microsoft.com)je, vytvořte obory zřetězením požadovaného identifikátoru prostředku s požadovaným oprávněním OAuth2 pro daný prostředek.
