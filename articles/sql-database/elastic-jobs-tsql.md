...
Ve výchozím nastavení vytvoří Agent úlohy výstupní tabulku pro uložení vrácených výsledků. Proto musí objekt `CREATE TABLE` zabezpečení databáze přidružený k výstupnímu přihlašovacímu údaji splňovat alespoň následující oprávnění: v databázi, `ALTER` `SELECT`,, `INSERT`, `DELETE` ve výstupní tabulce nebo schématu a `SELECT` v zobrazení katalogu [Sys. indexs](https://docs.microsoft.com/sql/relational-databases/system-catalog-views/sys-indexes-transact-sql) .
