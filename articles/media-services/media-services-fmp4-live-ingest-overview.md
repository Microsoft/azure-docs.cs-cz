...
1. Kodér nesmí používat `Events()` substantivum, jak je popsáno v 9,2 v [1] pro živou příjem dat do Media Services.
...
1. Nový kodér by se měl pokusit minimalizovat ztrátu dat. `fragment_absolute_time` a `fragment_index` fragmentů média by se měly zvýšit od bodu, ve kterém se kodér naposledy zastavil. `fragment_absolute_time` a `fragment_index` by se měly narůstat průběžně, ale je přípustné, aby v případě potřeby zavedly nekontinuitu. Media Services ignoruje fragmenty, které již byly přijaty a zpracovány, takže je lepší se na straně opětovného odesílání fragmentů, než je zavedení nekontinuity na časové ose médií, poznamenat. 
