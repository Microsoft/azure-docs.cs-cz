...
1. Volitelně můžete na stránce **oprávnění rozhraní API** vybrat **udělit souhlas správce pro \<vaše jméno-tenanta>** pro udělení souhlasu jménem všech uživatelů v tomto adresáři. 
...
1. Pokud používáte koncové body **v2** , použijte obor, který jste vytvořili pro back-end aplikaci, do pole **výchozí obor** . Ujistěte se také, že jste [`accessTokenAcceptedVersion`](/azure/active-directory/develop/reference-app-manifest#accesstokenacceptedversion-attribute) `2` v [manifestu aplikace](/azure/active-directory/develop/reference-app-manifest)nastavili hodnotu pro vlastnost.
...
4. Po úspěšném přihlášení se k žádosti `Authorization` přidá hlavička s přístupovým tokenem z Azure AD. Následuje ukázkový token (kódovaný v kódování Base64):
...
```
> [!NOTE]
> Tato `openid-config` adresa URL odpovídá koncovému bodu v1. Pro koncový bod `openid-config`v2 použijte `https://login.microsoftonline.com/common/v2.0/.well-known/openid-configuration`.
