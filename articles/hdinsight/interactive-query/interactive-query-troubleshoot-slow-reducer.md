...
Při spuštění dotazu, jako je například `insert into table1 partition(a,b) select a,b,c from table2` plánu dotazu, se spustí svazek reduktorů, ale data z každého oddílu přecházejí do jednoho zmenšení. Tím dojde k tomu, že dotaz bude pomalý jako čas, který zmenší největší oddíl.

## <a name="cause"></a>Příčina

Otevřete [Beeline](../hadoop/apache-hadoop-use-hive-beeline.md) a ověřte hodnotu nastavit `hive.optimize.sort.dynamic.partition`.

Hodnota této proměnné má být nastavena na hodnotu true nebo false na základě povaze dat.

Pokud jsou oddíly ve vstupní tabulce menší (tzn. méně než 10), a proto je počet výstupních oddílů a proměnná je nastavená na `true`, způsobí to, že se data globálně seřadí a napíší pomocí jednoho zmenšení na oddíl. I v případě, že počet dostupných reduktorů je větší, může být několik reduktorů zpožděných na začátku z důvodu zešikmení dat a maximální paralelismus nelze dosáhnout. Když se změní na `false`, může více než jeden redukce zpracovat jeden oddíl a několik menších souborů se bude zapisovat, což bude mít za následek rychlejší vložení. To může mít vliv na další dotazy, i když z důvodu přítomnosti menších souborů.
