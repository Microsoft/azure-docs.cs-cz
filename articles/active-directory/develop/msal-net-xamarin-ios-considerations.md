...
- Přepište a implementujte funkci `OpenUrl` v `AppDelegate`.
...
Přepište metodu `OpenUrl` `FormsApplicationDelegate` odvozené třídy a volání `AuthenticationContinuationHelper.SetAuthenticationContinuationEventArgs`. Tady je příklad:
...
Pokud chcete povolit přístup pomocí řetězce klíčů, ujistěte se, že má vaše aplikace přístupovou skupinu pro řetězce klíčů. Přístupovou skupinu pro řetězce klíčů můžete nastavit při vytváření aplikace pomocí rozhraní `WithIosKeychainSecurityGroup()` API.
...
V souboru `Entitlements.plist` taky povolte přístup k řetězci klíčů. Použijte buď následující skupinu přístupu, nebo vlastní přístupovou skupinu.
...
Když použijete rozhraní `WithIosKeychainSecurityGroup()` API, MSAL automaticky připojí skupinu zabezpečení na konec *ID týmu* aplikace (`AppIdentifierPrefix`). MSAL přidá vaši skupinu zabezpečení, protože když sestavíte aplikaci v Xcode, bude to mít stejnou. To je důvod, proč oprávnění v souboru `Entitlements.plist` musí zahrnovat `$(AppIdentifierPrefix)` před přístupovou skupinou pro řetězce klíčů.
...
Pokud chcete povolit toto sdílení mezipaměti, pomocí metody `WithIosKeychainSecurityGroup()` nastavte přístupovou skupinu pro řetězce klíčů na stejnou hodnotu ve všech aplikacích, které sdílejí stejnou mezipaměť. První příklad kódu v tomto článku ukazuje, jak použít metodu.

Dříve v tomto článku jste zjistili, že MSAL přidává `$(AppIdentifierPrefix)` vždy, když použijete rozhraní `WithIosKeychainSecurityGroup()` API. MSAL přidá tento prvek, protože ID týmu `AppIdentifierPrefix` zajišťuje, aby přístup k řetězci klíčů mohl sdílet jenom aplikace, které provádí tentýž Vydavatel.

> [!NOTE]
> Vlastnost `KeychainSecurityGroup` je zastaralá.
> 
> Počínaje MSAL 2. x byly vývojáři nuceni při použití vlastnosti `KeychainSecurityGroup` zahrnout předponu `TeamId`. Ale počínaje verzí MSAL 2.7. x se při použití vlastnosti New `iOSKeychainSecurityGroup` vyřeší MSAL předponu `TeamId` během běhu. Pokud použijete tuto vlastnost, nezahrnujte předponu `TeamId` do hodnoty. Předpona není povinná.
>
> Vzhledem k tomu, že vlastnost `KeychainSecurityGroup` je zastaralá, použijte vlastnost `iOSKeychainSecurityGroup`.
