...
| |Spravovaná instance | 8, 16, 24, 32, 40, 64, 80 virtuální jádra | – | 8, 16, 24, 32, 40, 64, 80 virtuální jádra |
...
| | Spravovaná instance | Závisí na velikosti souboru | – | 1375 IOPS/vCore |
...
|**Zálohování**|Všechny|RA-GRS, 7-35 dnů uchování (ve výchozím nastavení 7 dní)| RA-GRS, 7 dní uchovávání, konstantní doba obnovení v čase (PITR) | RA-GRS, 7-35 dnů uchování (ve výchozím nastavení 7 dní) |
...
- **Horizontální navýšení nebo zmenšení kapacity**
  ...
  Zřizování každé z těchto dalších výpočetních replik se dá provést v konstantním čase a je to online operace. K těmto dalším výpočetním replikám, které jsou jen pro čtení, se můžete `ApplicationIntent` připojit nastavením argumentu v připojovacím řetězci na `ReadOnly`. Všechna připojení k záměru `ReadOnly` aplikace jsou automaticky směrována na jednu z dalších výpočetních replik jen pro čtení.
...
Vaše `tempdb` databáze je umístěná v místním úložišti SSD a je úměrná velikosti pro výpočetní velikost, kterou zřizujete. Vaše `tempdb` aplikace je optimalizovaná tak, aby poskytovala maximální výkonnostní výhody. `tempdb`velikost není konfigurovatelná a je spravovaná pro vás.
...
### <a name="does-the-size-of-my-tempdb-database-also-grow-as-the-compute-is-scaled-up"></a>Zvětšuje se velikost `tempdb` databáze i v případě, že se škálovat výpočetní výkon

Ano. Při `tempdb` nárůstu výpočetního prostředí se databáze automaticky narůstá.  
...
K těmto dalším výpočetním replikám, které jsou jen pro čtení, se můžete `ApplicationIntent` připojit nastavením argumentu v připojovacím řetězci na `ReadOnly`. Všechna připojení označená `ReadOnly` pomocí jsou automaticky směrována na jednu z dalších výpočetních replik jen pro čtení.  
