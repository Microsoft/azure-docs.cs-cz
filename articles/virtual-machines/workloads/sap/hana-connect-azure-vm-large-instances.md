...
![Virtuální síť Azure připojení k SAP HANA v Azure (velké instance) a místní](./media/hana-overview-architecture/image1-architecture.png)

Hledání blíže na straně virtuální síť Azure, je potřeba pro:
...
Při odkazování na **adresní prostor** v této dokumentaci, do adresního prostoru, který může používat virtuální síť Azure. Tento adresní prostor je také požadovaný rozsah adres virtuální sítě používá pro šíření tras BGP. To **adresní prostor** uvedené tady:
...
Ve dvou předchozí grafiky **adresní prostor virtuální sítě** zahrnuje obě **rozsah IP adres podsítě virtuálního počítače Azure** a zda brána virtuální sítě.
...
Můžete použít všechny standardní pojmenování, který rádi používáte pro tyto podsítě tenanta (podsítí virtuálních počítačů). Ale **musí vždy obsahovat jeden a pouze jeden, podsíť brány pro každou virtuální síť** , která se připojuje k SAP HANA v okruhu ExpressRoute Azure (velké instance). **Tato podsíť brány musí mít název "GatewaySubnet"** abyste měli jistotu, že je správně umístěny bránu ExpressRoute.
...
- Musíte odeslat **adresní prostor virtuální sítě** společnosti Microsoft, když provádíte velké instance HANA je počáteční nasazení. 
...
Některé rozsahy IP adres, které jsou nezbytné pro nasazení velkých instancích HANA máte již zaveden. Ale existují další rozsahy IP adres, které jsou také důležité. Ne všechny tyto rozsahy IP adres potřebovat k odeslání společnosti Microsoft. Však nutné definovat před odesláním požadavku pro počáteční nasazení:

- **Adresní prostor virtuální sítě**: **Adresní prostor virtuální sítě** se rozsahy IP adres, které můžete přiřadit parametr místo adres ve virtuálních sítích Azure. Tyto sítě připojit k prostředí velkou instanci SAP HANA. Doporučujeme vám, že tento parametr místo adresy je Víceřádkový hodnotu. Měla by obsahovat rozsahu podsítě virtuálních počítačů Azure a rozsahy podsítě brány Azure. Rozsah této podsítě se zobrazilo v předchozí grafiky. Musíte nepřekrývá s vaší on-premises nebo server fond IP adres nebo rozsahy adres ER P2P. Jak získat tyto rozsahy IP adres? Vaší podnikové síti týmu nebo poskytovali služeb, musí zadat jeden nebo více IP adresu, rozsah(y), které nejsou použity uvnitř vaší sítě. Například 10.0.1.0/24 je podsíť virtuálního počítače Azure a podsíť brány Azure podsítě je 10.0.2.0/28.  Doporučujeme vám, že adresní prostor vaší virtuální síti Azure je definována jako: 10.0.1.0/24 a 10.0.2.0/28. I když hodnoty místo adres se dají agregovat, doporučujeme, abyste zjišťování shody s rozsahy adres podsítě. Tímto způsobem můžete nechtěně vyhnout opětovné použití nevyužité rozsahy IP adres v rámci větší adresní prostory jinde ve vaší síti. **Adresní prostor virtuální sítě je rozsah IP adres. Musí být odeslána společnosti Microsoft, když požádáte pro počáteční nasazení**.
- **Azure VM IP rozsah adres podsítě:** Tento rozsah IP adres je ten, který přiřadíte k parametru podsítě virtuální sítě Azure. Tento parametr je ve službě Azure virtual network a připojí k prostředí velkou instanci SAP HANA. Tento rozsah IP adres se používá k přiřazování IP adres pro virtuální počítače Azure. IP adresy mimo tento rozsah mohou připojit na vašich serverech velké Instance SAP HANA. V případě potřeby můžete použít několik podsítí virtuálních počítačů Azure. Doporučujeme, abyste/24 je blok CIDR pro každou podsíť virtuálních počítačů Azure. Tento rozsah adres musí být součástí hodnoty, které se používají v adresním prostoru virtuální sítě Azure. Jak získáte tento rozsah IP adres Vaší podnikové síti týmu nebo poskytovali služeb, by měla poskytnout rozsah IP adres, který se nepoužívá uvnitř vaší sítě.
...
Volitelné rozsahy IP adres, který časem budete muset odeslat společnosti Microsoft:

- Pokud se rozhodnete použít [ExpressRoute globální dosah](https://docs.microsoft.com/azure/expressroute/expressroute-global-reach) umožňuje přímé směrování z místní jednotky velká Instance HANA je nutné rezervovat minimální velikostí/29 jiný rozsah IP adres. Tato oblast se nemohou překrývat s žádným z jiné rozsahy IP adres, které jste definovali před.
- Pokud se rozhodnete použít [ExpressRoute globální dosah](https://docs.microsoft.com/azure/expressroute/expressroute-global-reach) umožňuje přímé směrování z tenanta velká Instance HANA v jedné oblasti Azure do jiného tenanta velká Instance HANA v jiné oblasti Azure, je nutné rezervovat minimální velikostí/29 jiný rozsah IP adres . Tato oblast se nemohou překrývat s žádným z jiné rozsahy IP adres, které jste definovali před.

Další informace o ExpressRoute globální dosah a využití velkých instancích HANA najdete v dokumentech:
...
- [Připojení virtuální sítě pro velké instance HANA](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-connect-vnet-express-route)
...
Obrázek nezobrazuje další IP adresu, rozsah(y), které jsou požadovány pro nepovinným použitím ExpressRoute globální dosah.
...
V příkladu, místo dvě menší rozsahy definované adresní prostor virtuální sítě Azure máme jeden větší rozsah, který zahrnuje 4096 IP adresy. Velké definice adresního prostoru znamená některé docela rozsáhlá rozsahy nevyužitá. Vzhledem k tomu, že hodnoty místo adres virtuální sítě se používají pro šíření tras protokolu BGP, využití nevyužité oblasti v místním nebo jinde v síti může způsobit problémů se směrováním. Obrázek nezobrazuje další IP adresu, rozsah(y), které jsou požadovány pro nepovinným použitím ExpressRoute globální dosah.

Doporučujeme, abyste adresní prostor naprosto v souladu s skutečné podsítě adresní prostor, který používáte. V případě potřeby, aniž by došlo k výpadku ve virtuální síti, můžete vždy přidat nové hodnoty místo adres později.
