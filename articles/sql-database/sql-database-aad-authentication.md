...
description: Přečtěte si, jak používat Azure Active Directory k ověřování pomocí SQL Database, spravované instance a Azure synapse
...
- Azure AD podporuje připojení z aplikace SQL Server Management Studio využívající univerzální ověřování Active Directory, což zahrnuje službu Multi-Factor Authentication (MFA).  MFA zahrnuje silné ověřování s využitím celé řady možností ověření – telefonických hovorů, textových zpráv, čipových karet s kódem PIN nebo oznámení mobilní aplikace. Další informace najdete v tématu [Podpora SSMS pro vícefaktorové ověřování Azure AD s využitím SQL Database a Azure synapse](sql-database-ssms-mfa-authentication.md).
...
3. Vytvořte správce Azure Active Directory pro Azure SQL Database Server, spravovanou instanci nebo [fond SQL](https://azure.microsoft.com/services/sql-data-warehouse/).
...
> [!NOTE]
> Informace o tom, jak vytvořit a naplnit Azure AD a jak nakonfigurovat Azure AD pomocí Azure SQL Database, spravované instance a fondu SQL, najdete v tématu [konfigurace Azure AD s Azure SQL Database](sql-database-aad-authentication-configure.md).

## <a name="trust-architecture"></a>Architektura důvěryhodnosti

Následující diagram vysoké úrovně shrnuje architekturu řešení pro použití ověřování Azure AD s Azure SQL Database. Stejné koncepty platí i pro Azure synapse. Aby bylo možné podporovat heslo nativního uživatele služby Azure AD, bude zvážena pouze část cloudu a Azure AD/Azure SQL Database. Aby bylo možné podporovat federované ověřování (nebo uživatele/heslo pro přihlašovací údaje systému Windows), je vyžadována komunikace s blokem ADFS. Šipky označují cesty komunikace.
...
Pokud chcete vytvořit uživatele databáze s omezením v Azure SQL Database, spravované instanci nebo Azure synapse, musíte se připojit k databázi nebo instanci pomocí identity Azure AD. Chcete-li vytvořit prvního uživatele databáze s omezením, je nutné se připojit k databázi pomocí správce Azure AD (který je vlastníkem databáze). To je znázorněno v [konfiguraci a správě Azure Active Directory ověřování pomocí SQL Database nebo Azure synapse](sql-database-aad-authentication-configure.md). Jakékoli ověřování Azure AD je možné pouze v případě, že byl vytvořen správce Azure AD pro Azure SQL Database nebo Azure synapse Server. Pokud byl správce Azure Active Directory odebraný ze serveru, stávající uživatelé Azure Active Directory dříve vytvořeni dříve v SQL Server se již nebudou moci připojit k databázi pomocí příslušných přihlašovacích údajů Azure Active Directory.
...
- Uživatelé Azure AD, kteří jsou součástí skupiny, která má `db_owner` role serveru, nemůžou používat syntaxi **[CREATE DATABASE s oborem pověření](/sql/t-sql/statements/create-database-scoped-credential-transact-sql)** proti Azure SQL Database a Azure synapse. Zobrazí se následující chyba:
...
- Jenom správce Azure AD pro SQL Server se může zpočátku připojit k serveru Azure SQL Database, spravované instanci nebo Azure SQL synapse pomocí účtu Azure Active Directory. Správce služby Active Directory může nakonfigurovat další uživatele databáze služby Azure AD.   
