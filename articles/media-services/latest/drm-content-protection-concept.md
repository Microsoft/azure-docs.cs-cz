...
V Media Services V3 je klíč obsahu přidružený k lokátoru streamování (viz [Tento příklad](drm-playready-license-template-concept.md)). Pokud používáte službu doručování klíčů Media Services, můžete nechat Azure Media Services vygenerovat klíč obsahu. Klíč obsahu by se měl vygenerovat sami, pokud používáte vlastní službu pro doručování klíčů, nebo pokud potřebujete, aby byl scénář s vysokou dostupností, kdy potřebujete stejný klíč obsahu ve dvou datových centrech.
...
1. Vytvořte a nakonfigurujte [zásady klíče obsahu](drm-content-key-policy-concept.md).
   ...
   * Nakonfigurujte licence [PlayReady](drm-playready-license-template-concept.md), [Widevine](drm-widevine-license-template-concept.md)a [Fairplay](drm-fairplay-license-overview.md) . Šablony vám umožní nakonfigurovat práva a oprávnění pro jednotlivé několikanásobnou.
...
2. Vytvořte [Lokátor streamování](stream-streaming-locators-concept.md) , který je nakonfigurovaný pro streamování šifrovaného prostředku.
  
   Lokátor streamování musí být přidružený k [zásadě streamování](stream-streaming-policy-concept.md). V tomto příkladu nastavíme `StreamingLocator.StreamingPolicyName` zásadu "Predefined_MultiDrmCencStreaming".
