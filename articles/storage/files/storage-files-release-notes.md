...
|----|----------------------|--------------|------------------|
| Kumulativní aktualizace června | 3.0.13.0 | 29. června 2018 | Podporovaná (doporučená verze) |
| Aktualizace 2 | 3.0.12.0 | 22. května 2018 | Podporováno |
| Kumulativní aktualizace duben | 2.3.0.0 | 8 může 2018 | Podporováno |
| Kumulativní aktualizace března | 2.2.0.0 | 12. březen 2018 | Podporováno |
| Kumulativní aktualizace února | 2.1.0.0 | 28. února 2018 | Podporováno |
| Aktualizujte 1 | 2.0.11.0 | 8. února 2018 | Podporováno |
| Leden kumulativní aktualizace | 1.4.0.0 | 8 leden 2018 | Podporováno |
| Kumulativní aktualizace listopadu | 1.3.0.0 | 30. listopadu 2017 | Podporováno |
| Říjen kumulativní aktualizace | 1.2.0.0 | 31. října 2017 | Podporováno |
| Počáteční verze Preview | 1.1.0.0 | 26. září 2017 | Podporováno |
...
Následující poznámky k verzi jsou pro verzi agenta Azure souboru Sync vydané 29. června 2018 3.0.13.0. Tyto poznámky jsou kromě poznámky k verzi pro verzi 3.0.12.0 uvedené.

Tato verze obsahuje následující potíže:
- Body rozboru synchronizace nezdaří, pokud server se přidá do existující skupiny sync, pokud existují v umístění koncového bodu serveru na serveru.

## <a name="agent-version-30120"></a>Verze agenta 3.0.12.0
Následující poznámky k verzi jsou pro verzi 3.0.12.0 agenta synchronizace souboru Azure (vydané 22 může 2018).
...
- Instalační balíček agenta musí být nainstalován s oprávněními zvýšenými (správce).
- Agent není podporována v systému Windows Server Core nebo Nano Server možnosti nasazení.
- Agent se podporuje pouze ve Windows Serveru 2016 a Windows Serveru 2012 R2.
- Agent vyžaduje alespoň 2 GB fyzické paměti.
- Služba agenta synchronizace úložiště (FileSyncSvc) nepodporuje koncové body serveru umístěn ve svazku, který má komprimované systémového svazku informace (SVI) adresáře. Tato konfigurace bude vést k neočekávaným výsledkům.
...
- Spuštěním programu sysprep na serveru, který má nainstalovaného agenta Azure souboru synchronizace není podporována a může vést k neočekávaným výsledkům. Registrace agenta se instalace a server by měl následovat po nasazení bitové kopie serveru a dokončení zkrácené instalace nástroje sysprep.
...
- Cloud vrstvení není podporována na systémovém svazku. Chcete-li vytvořit koncový bod serveru na systémovém svazku, zakažte vrstvení při vytváření serveru koncového bodu cloudu.
...
- Vrstvený soubory se stane nepoužitelný, pokud nejsou soubory připomenout před odstraněním serveru koncového bodu.
...
## <a name="agent-version-2300"></a>Verze agenta 2.3.0.0
Následující poznámky k verzi jsou pro verzi agenta Azure souboru Sync vydané 8 může 2018 2.3.0.0. Tyto poznámky doplňují uvedené poznámky k verzi 2.0.11.0.

Tato verze obsahuje následující opravy:
- Aktualizace agenta může přestat reagovat, pokud ovladač filtru vrstvení cloudu neuvolní.
- Při synchronizaci velké množství souborů může snížit výkon synchronizace.
