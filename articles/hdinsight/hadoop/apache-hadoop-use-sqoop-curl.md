...
Pro příkazy v této části nahraďte `USERNAME` uživateli, který se má ověřit v clusteru, a nahraďte `PASSWORD` heslem pro uživatelský účet. Nahraďte `CLUSTERNAME` názvem svého clusteru.
...
1. K odeslání úlohy Sqoop použijte následující postup:
   ...
   * **-d** – vzhledem k tomu, že se `-G` nepoužívá, je požadavek nastaven na výchozí metodu post. `-d` určuje hodnoty dat, které se odesílají spolu s požadavkem.
...
1. Chcete-li zjistit stav úlohy, použijte následující příkaz. Nahraďte `JOBID` hodnotou vrácenou v předchozím kroku. Pokud byla například návratová hodnota `{"id":"job_1415651640909_0026"}`, `JOBID` `job_1415651640909_0026`. Podle potřeby upravte umístění `jq`.
...
1. Jakmile se stav úlohy změní na **úspěch**, můžete načíst výsledky úlohy z úložiště objektů BLOB v Azure. Parametr `statusdir` předaný s dotazem obsahuje umístění výstupního souboru; v tomto případě `wasb:///example/data/sqoop/curl`. Tato adresa ukládá výstup úlohy do adresáře `example/data/sqoop/curl` ve výchozím kontejneru úložiště, který používá cluster HDInsight.
...
* Dávkování – se systémem Linux HDInsight při použití přepínače `-batch` při provádění operací vložení provede Sqoop vícenásobné vkládání místo dávkování operací vložení.
