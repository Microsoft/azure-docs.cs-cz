...
ms.locfileid: "68774863"
---
# <a name="tutorial-deploy-virtual-machine-extensions-with-azure-resource-manager-templates"></a>Kurz: Nasazení rozšíření virtuálních počítačů pomocí šablon Azure Resource Manager
...
* [Visual Studio Code](https://code.visualstudio.com/) s rozšířením Nástroje Resource Manageru Viz [Instalace rozšíření](./resource-manager-quickstart-create-templates-use-visual-studio-code.md#prerequisites).
* Pro zlepšení zabezpečení použijte pro účet správce virtuálního počítače vygenerované heslo. Tady ukázka generování hesla:
    ...
    Služba Azure Key Vault je určená k ochraně kryptografických klíčů a dalších tajných klíčů. Další informace najdete v tématu [kurz: Integruje Azure Key Vault do Správce prostředků](./resource-manager-tutorial-use-key-vault.md)Template Deployment. Doporučujeme vám také aktualizovat heslo každé tři měsíce.

## <a name="prepare-a-powershell-script"></a>Příprava skriptu PowerShellu

PowerShellový skript s následujícím obsahem se sdílí z účtu služby [Azure Storage s veřejným přístupem](https://armtutorials.blob.core.windows.net/usescriptextensions/installWebServer.ps1):

```azurepowershell
Install-WindowsFeature -name Web-Server -IncludeManagementTools
```

Pokud se rozhodnete publikovat soubor ve svém vlastním umístění, musíte aktualizovat `fileUri` element v šabloně později v tomto kurzu.
...
* **název**: Vzhledem k tomu, že prostředek rozšíření je podřízeným prostředkem objektu virtuálního počítače, název musí mít předponu názvu virtuálního počítače. Viz [Nastavení názvu a typu pro podřízené prostředky](child-resource-name-type.md).
...
Postup nasazení najdete v části ["nasazení šablony" v kurzu: Vytvořte šablony Azure Resource Manager se závislými](./resource-manager-tutorial-create-templates-with-dependent-resources.md#deploy-the-template)prostředky. Doporučujeme použít vygenerované heslo pro účet správce virtuálního počítače. Viz část [požadavky](#prerequisites) tohoto článku.

## <a name="verify-the-deployment"></a>Ověření nasazení

1. V Azure Portal vyberte virtuální počítač.
1. V přehledu virtuálních počítačů zkopírujte IP adresu tak, že **kliknete na tlačítko Kopírovat**a pak ho vložíte na kartu prohlížeče.  
   Otevře se výchozí úvodní stránka Internetová informační služba (IIS):
