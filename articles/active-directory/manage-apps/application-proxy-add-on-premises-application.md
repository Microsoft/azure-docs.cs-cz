...
Můžete povolit připojení k \*. msappproxy.net a \*. servicebus.windows.net Pokud brána firewall nebo proxy server umožňuje DNS na seznam povolených. Pokud ne, je potřeba povolit přístup k [rozsahy IP adres Azure DataCenter](https://www.microsoft.com/download/details.aspx?id=41653). Rozsahy IP adres se aktualizují každý týden.
...
2. V pravém horním rohu vyberte své uživatelské jméno. Ověřte, že jste přihlášeni do adresáře, který používá Proxy aplikací. Pokud potřebujete změnit adresář, vyberte **přepnout adresář** a vyberte adresář, který používá Proxy aplikací.
...
5. Vyberte **stáhnout službu konektoru**.
    ...
    ![Stáhnout službu konektoru](./media/application-proxy-add-on-premises-application/application-proxy-download-connector-service.png)

6. Přečtěte si podmínky služby.  Jakmile budete připraveni, vyberte **přijmout podmínky a stáhnout**.
7. V dolní části okna vyberte **spustit** k instalaci konektoru. Otevře se Průvodce instalací. 
8. Postupujte podle pokynů v průvodci pro instalaci služby. Po zobrazení výzvy k registraci konektoru Proxy aplikace pro vašeho tenanta Azure AD, zadejte svoje přihlašovací údaje správce aplikace.
...
3. Pokud není stav služby **systémem**, klikněte pravým tlačítkem a vyberte jednotlivé služby a zvolte **Start**. 
...
3. Vyberte **podnikové aplikace**a pak vyberte **novou aplikaci**.
4. Vyberte **On-premises application**.  
5. V **přidat vlastní místní aplikaci** části, zadejte následující informace o vaší aplikaci:
...
3. V části **uživatelů a skupin**vyberte **přidat uživatele**.
4. V části **přidat přiřazení**vyberte **uživatelů a skupin**. **Uživatele a skupiny** se zobrazí část. 
5. Vyberte účet, který chcete přidat. 
6. Zvolte **vyberte**a pak vyberte **přiřadit**.
...
2. Přihlaste se jako uživatel, který jste vytvořili v předchozí části.
