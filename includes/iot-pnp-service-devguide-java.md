...
ms.locfileid: "92521366"
---
K dispozici jsou také tyto prostředky:

- [Referenční dokumentace k sadě Java SDK](/java/api/com.microsoft.azure.sdk.iot.service.devicetwin.devicetwindevice?preserve-view=true&view=azure-java-stable)
- [Ukázky klienta služby](https://github.com/Azure/azure-iot-sdk-java/blob/master/service/iot-service-samples/pnp-service-sample)
- [Vzorky digitálních vláken](https://github.com/Azure/azure-iot-sdk-java/tree/master/service/iot-service-samples/digitaltwin-service-samples)

## <a name="iot-hub-service-client-examples"></a>Příklady klienta služby IoT Hub

V této části jsou uvedeny příklady jazyka Java s použitím klienta služby IoT Hub a tříd **DeviceTwin** a **DeviceMethod** z oboru názvů **com. Microsoft. Azure. SDK. IoT. Service. DeviceTwin** . Třídu **DeviceTwin** použijete k interakci se stavem zařízení pomocí nevláken zařízení. Můžete také použít třídu **DeviceTwin** k [dotazování registrace zařízení](../articles/iot-hub/iot-hub-devguide-query-language.md) v IoT Hub. Třídu **DeviceMethod** použijete k volání příkazů na zařízení. Model [DTDL](../articles/iot-pnp/concepts-digital-twin.md) pro zařízení definuje vlastnosti a příkazy, které zařízení implementuje. V fragmentech kódu `deviceId` proměnná obsahuje ID zařízení technologie Plug and Play IoT, které je zaregistrované ve službě IoT Hub.
...
Následující fragment kódu ukazuje, jak aktualizovat `targetTemperature` vlastnost na zařízení. Před tím, než ho aktualizujete, musíte získat dvojitou dvojici. Vlastnost je definována ve výchozí součásti zařízení:
...
Následující fragment kódu ukazuje, jak aktualizovat `targetTemperature` vlastnost pro komponentu. Před tím, než ho aktualizujete, musíte získat dvojitou dvojici. Vlastnost je definována v komponentě **thermostat1** :
...
Třídu **DigitalTwinAsyncClient** použijete v oboru názvů **com. Microsoft. Azure. SDK. IoT. Service. digitaltwin** pro interakci se stavem zařízení pomocí digitálních vláken. Následující příklady používají také třídy **UpdateOperationUtility** a **BasicDigitalTwin** ze stejného oboru názvů. Model [DTDL](../articles/iot-pnp/concepts-digital-twin.md) pro zařízení definuje vlastnosti a příkazy, které zařízení implementuje.
...
Následující fragment kódu ukazuje, jak číst telemetrii z výchozího koncového bodu Event Hubs. Kód v tomto fragmentu kódu je pořízen z rychlého startu IoT Hub [Odeslat telemetrii ze zařízení do služby IoT Hub a přečíst si ho pomocí back-endové aplikace](../articles/iot-hub/quickstart-send-telemetry-java.md):
...
Kód zobrazený v předchozím fragmentu kódu Java generuje následující výstup, když IoT Hub vygeneruje oznámení o zdvojení zařízení pro zařízení termostatu bez komponent. Vlastnosti aplikace `iothub-message-schema` a `opType` poskytují informace o typu oznámení o změně:
...
Kód zobrazený v předchozím fragmentu kódu Java generuje následující výstup, když IoT Hub pro zařízení s komponentami generuje oznámení o zdvojených změnách zařízení. Tento příklad ukazuje výstup, když zařízení snímače teploty s termostatovou komponentou generuje oznámení. Vlastnosti aplikace `iothub-message-schema` a `opType` poskytují informace o typu oznámení o změně:
...
Kód zobrazený v předchozím fragmentu kódu Java generuje následující výstup, když IoT Hub generuje pro zařízení termostatu bez komponenty žádné oznámení o změně digitálního vlákna. Vlastnosti aplikace `iothub-message-schema` a `opType` poskytují informace o typu oznámení o změně:
...
Kód zobrazený v předchozím fragmentu kódu Java generuje následující výstup, když IoT Hub generuje upozornění na digitální dvojitou změnu pro zařízení s komponentami. Tento příklad ukazuje výstup, když zařízení snímače teploty s termostatovou komponentou generuje oznámení. Vlastnosti aplikace `iothub-message-schema` a `opType` poskytují informace o typu oznámení o změně:
