...
![Část souboru UsersData. JSON zobrazující bloky JSON dvou uživatelů](./media/user-migration/pre-migration-data-file.png)
...
![Okno příkazového řádku zobrazující výstup příkazu UserMigration. exe](./media/user-migration/pre-migration-demo.png)
...
- Pokud chcete načíst uživatele podle přihlašovací e-mailové adresy, použijte ukázkovou aplikaci:
   ...
   1. Otevřete soubor UserProfile. JSON v editoru JSON a podívejte se na informace o uživateli.
      ...
      ![Soubor UserProfile. JSON otevřený v editoru Visual Studio Code](./media/user-migration/pre-migration-get-by-email2.png)
...
Pokud chcete získat odkaz na zásady pro resetování hesla, postupujte podle těchto kroků. Tento postup předpokládá, že jste dříve vytvořili [vlastní zásadu](custom-policy-get-started.md)pro resetování hesla.
...
1. Zkopírujte adresu URL zobrazenou v textovém poli **Spustit nyní** a odešlete ji uživatelům.
    ...
    ![Stránka zásad resetování hesel s zvýrazněným koncovým bodem spustit](./media/user-migration/pre-migration-policy-uri.png)
...
Po definování technického profilu pro rozhraní API RESTful nakonfigurujte stávající technický profil `SelfAsserted-LocalAccountSignin-Email` tak, aby dál volal REST API technický profil přepsáním v souboru *TrustFrameworkExtensions. XML* :
...
Pak změňte `Id` Technical profilu `LocalAccountSignIn` na `LocalAccountUserMigration`.
...
1. Zadejte přihlašovací údaje jednoho ze migrovaných uživatelů a pak vyberte **Přihlásit**se. Váš REST API by měl vyvolat následující chybovou zprávu:
    ...
    ![Přihlašovací stránka pro přihlášení ukazující chybovou zprávu změnit heslo](./media/user-migration/pre-migration-error-message.png)
...
1. Vyberte **Uložit**.
    ...
    ![Stránka Konfigurace diagnostických protokolů v Azure Portal](./media/user-migration/pre-migration-diagnostic-logs.png)
