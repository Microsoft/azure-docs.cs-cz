...
Tento článek popisuje, jak spravovat sdílené složky na Azure Stack Edge pro. Azure Stack Edge pro můžete spravovat přes Azure Portal nebo prostřednictvím místního webového uživatelského rozhraní. Pomocí Azure Portal můžete přidat, odstranit, aktualizovat sdílené složky nebo synchronizovat klíč úložiště pro účet úložiště, který je přidružený ke sdíleným složkám. Tento článek se týká Azure Stackch PROCESORů Edge pro, Azure Stack Edge pro R a Azure Stack hraničních zařízení R.
...
1. V Azure Portal přejdete do svého prostředku Azure Stack Edge a pak přejdete do **cloudové služby úložiště > sdílené složky**. Na panelu příkazů vyberte **+ Přidat sdílenou složku** .
    ...
    ![Vyberte přidat sdílenou složku](media/azure-stack-edge-gpu-manage-shares/add-share-1.png)
...
6. Postup v tomto kroku závisí na tom, jestli vytváříte sdílenou složku SMB nebo systému souborů NFS.
    - **Pokud vytváříte sdílenou složku SMB** – v poli **Místní uživatel se všemi oprávněními** zvolte **Vytvořit nový** nebo **Použít existující**. Pokud vytváříte nového místního uživatele, zadejte **uživatelské jméno**, **heslo** a pak potvrďte heslo. Tím se místnímu uživateli přiřadí oprávnění. Jakmile tady přiřadíte oprávnění, můžete pak tato oprávnění upravit pomocí Průzkumníka souborů.
        ...
        ![Přidání sdílené složky SMB](media/azure-stack-edge-gpu-manage-shares/add-smb-share.png)
    ...
    - **Pokud vytváříte sdílenou složku systému souborů NFS** – je nutné zadat **IP adresy klientů s povoleným přístupem** ke sdílené složce.
        ...
        ![Přidání sdílené složky systému souborů NFS](media/azure-stack-edge-gpu-manage-shares/add-nfs-share.png)
...
1. V Azure Portal přejdete do svého prostředku Azure Stack Edge a pak přejdete do **cloudové služby úložiště > sdílené složky**. Na panelu příkazů vyberte **+ Přidat sdílenou složku** .
    ...
    ![Vyberte přidat sdílenou složku 2.](media/azure-stack-edge-gpu-manage-shares/add-local-share-1.png)
...
7. Vyberte **Vytvořit**. 
    ...
    ![Vytvořit místní sdílenou složku](media/azure-stack-edge-gpu-manage-shares/add-local-share-2.png)
    ...
    ![Okno zobrazit aktualizace pro sdílené složky](media/azure-stack-edge-gpu-manage-shares/add-local-share-3.png)
    ...
    ![Zobrazit podrobnosti o místní sdílené složce](media/azure-stack-edge-gpu-manage-shares/add-local-share-4.png)
...
1. V Azure Portal přejdete do svého prostředku Azure Stack Edge a pak přejdete do **cloudové služby úložiště > sdílené složky**. V seznamu sdílených složek vyberte sdílenou složku, kterou chcete připojit. Ve sloupci **použito pro výpočty** se zobrazí stav **zakázáno** pro vybranou sdílenou složku.
    ...
    ![Vyberte sdílenou složku](media/azure-stack-edge-gpu-manage-shares/mount-share-1.png)

2. Vyberte **připojit**.

    ![Vybrat připojení](media/azure-stack-edge-gpu-manage-shares/mount-share-2.png)

3. Po zobrazení výzvy k potvrzení vyberte **Ano**. Tím se sdílená složka připojí.

    ![Potvrdit připojení](media/azure-stack-edge-gpu-manage-shares/mount-share-3.png)
...
4. Po připojení sdílené složky přejdete do seznamu sdílených složek. Uvidíte, že se ve sloupci **použito pro výpočetní** prostředky zobrazuje stav sdílení jako **povolený**.
    ...
    ![Sdílet připojené](media/azure-stack-edge-gpu-manage-shares/mount-share-4.png)
...
5. Pokud chcete zobrazit místní přípojný bod pro sdílenou složku, vyberte ji znovu. Modul COMPUTE pro Edge používá pro sdílenou složku Tento místní přípojný bod.
    ...
    ![Místní přípojný bod pro sdílenou složku](media/azure-stack-edge-gpu-manage-shares/mount-share-5.png)
...
1. V Azure Portal přejdete do svého prostředku Azure Stack Edge a pak přejdete do **cloudové služby úložiště > sdílené složky**. V seznamu sdílených složek vyberte sdílenou složku, kterou chcete odpojit. Chcete se ujistit, že se sdílená složka nepoužívá v žádných modulech. Pokud je sdílená složka používána modulem, zobrazí se problémy s odpovídajícím modulem.

    ![Vybrat sdílenou složku 2](media/azure-stack-edge-gpu-manage-shares/unmount-share-1.png)

2.  Vyberte **Odpojit**.

    ![Vybrat odpojit](media/azure-stack-edge-gpu-manage-shares/unmount-share-2.png)

3. Po zobrazení výzvy k potvrzení vyberte **Ano**. Tím se sdílená složka odpojí.

    ![Potvrdit odpojení](media/azure-stack-edge-gpu-manage-shares/unmount-share-3.png)
...
4. Až bude sdílená složka odpojená, otevřete seznam sdílených složek. Uvidíte, že se **používá pro výpočetní** sloupec zobrazuje stav sdílení jako **zakázaný**.
    ...
    ![Sdílet nepřipojené](media/azure-stack-edge-gpu-manage-shares/unmount-share-4.png)
...
1. V seznamu sdílených složek vyberte a klikněte na sdílenou složku, kterou chcete odstranit.
    ...
    ![Vybrat sdílenou složku 3](media/azure-stack-edge-gpu-manage-shares/delete-share-1.png)

2. Klikněte na **Odstranit**.

    ![Kliknutím odstraňte](media/azure-stack-edge-gpu-manage-shares/delete-share-2.png)

3. Po zobrazení výzvy k potvrzení klikněte na **Ano**.

    ![Potvrzení odstranění](media/azure-stack-edge-gpu-manage-shares/delete-share-3.png)
...
1.  Na webu Azure Portal přejděte na **Sdílené složky**. Vyberte a klikněte na sdílenou složku, kterou chcete aktualizovat.
    ...
    ![Vybrat sdílenou složku 4](media/azure-stack-edge-gpu-manage-shares/refresh-share-1.png)

2.  Klikněte na **Aktualizovat**. 

    ![Kliknutím aktualizujte](media/azure-stack-edge-gpu-manage-shares/refresh-share-2.png)
 
3.  Po zobrazení výzvy k potvrzení klikněte na **Ano**. Spustí se úloha, která zaktualizuje obsah místní sdílené složky.

    ![Potvrďte aktualizaci](media/azure-stack-edge-gpu-manage-shares/refresh-share-3.png)
...
![Aktualizované časové razítko](media/azure-stack-edge-gpu-manage-shares/refresh-share-4.png)
...
2. Pokud chcete vytvořit kontejner, klikněte na **kontejnery** a vyberte **+ kontejner** . Pojmenujte tento kontejner jako *newcontainer*. Nastavte **úroveň veřejného přístupu** na Container.
    ...
    ![Automatizovaná synchronizace pro připnuté soubory 1](media/azure-stack-edge-gpu-manage-shares/image-1.png)
...
3. Vyberte název kontejneru a nastavte následující metadata:  
    ...
    ![Automatizovaná synchronizace pro připnutý soubor 2](media/azure-stack-edge-gpu-manage-shares/image-2.png)
...
4. Vytvořte na svém zařízení novou sdílenou složku. Namapujte ho k připnutému kontejneru výběrem možnosti existující kontejner. Označí sdílenou složku jako jen pro čtení. Vytvořte nového uživatele a zadejte uživatelské jméno a odpovídající heslo pro tuto sdílenou složku.  
    ...
    ![Automatizovaná synchronizace připnutých souborů 3](media/azure-stack-edge-gpu-manage-shares/image-3.png)
...
6. Vyberte možnost **aktualizovat data** v Azure Portal, aby zařízení stahoval zásady připnutí pro daný kontejner Azure Storage.  
    ...
    ![Automatizovaná synchronizace pro připnuté soubory 4](media/azure-stack-edge-gpu-manage-shares/image-4.png)
...
1. Ve svém prostředku přejděte na **Přehled**. V seznamu sdílených složek zvolte a klikněte na sdílenou složku, která je přidružená k účtu úložiště, které potřebujete synchronizovat.
    ...
    ![Vyberte sdílet s relevantním účtem úložiště.](media/azure-stack-edge-gpu-manage-shares/sync-storage-key-1.png)

2. Klikněte na **Synchronizovat klíč úložiště**. Po zobrazení výzvy k potvrzení klikněte na **Ano**.

     ![Vybrat synchronizační klíč úložiště](media/azure-stack-edge-gpu-manage-shares/sync-storage-key-2.png)
...
- Přečtěte si, jak [spravovat uživatele pomocí webu Azure Portal](azure-stack-edge-gpu-manage-users.md).