---
title: Zálohování virtuálních počítačů Azure na trezor služby Recovery Services s využitím Azure Backup
description: Popisuje, jak zálohovat virtuální počítače Azure v trezoru služby Recovery Services s využitím Azure Backup
services: backup
...
Obecně platí není nutné výslovně povolit odchozí síťový přístup pro virtuální počítač Azure, tak může komunikovat s Azure Backup.
Pokud se nemůže připojit virtuální počítače, a pokud se zobrazí chyba **ExtensionSnapshotFailedNoNetwork**, byste měli explicitně povolit přístup. Rozšíření zálohování může komunikovat s Azure veřejné IP adresy pro provoz zálohování.


#### <a name="explicitly-allow-outbound-access"></a>Výslovně povolit odchozí přístup

Pokud virtuální počítač nemůže připojit ke službě Backup, výslovně povolit odchozí přístup pomocí jedné z metod, které jsou uvedené v tabulce.

**Možnost** | **Akce** | **Podrobnosti** 
--- | --- | --- 
**Nastavení pravidla skupiny zabezpečení sítě** | Povolit [rozsahy IP adres datacentra Azure](https://www.microsoft.com/download/details.aspx?id=41653). | Místo povolení a správa každý rozsah adres, můžete přidat pravidlo skupiny (NSG) zabezpečení sítě, která umožňuje přístup ke službě Azure Backup s využitím [značka služby](backup-azure-arm-vms-prepare.md#set-up-an-nsg-rule-to-allow-outbound-access-to-azure). [Další informace](../virtual-network/security-overview.md#service-tags).<br/><br/> Nejsou spojené žádné další náklady.<br/><br/> Pravidla se dají snadno spravovat pomocí značky služeb.
**Nasazení proxy serveru** | Nasazení proxy server HTTP pro směrování provozu. | Tato metoda poskytuje přístup k celé Azure, a ne jenom úložiště.<br/><br/> Podrobnou kontrolu nad adresy URL úložiště je povolený.<br/><br/> Existuje jeden bod Internetu přístup pro virtuální počítače.<br/><br/> Existují další náklady pro proxy server.
**Nastavení brány Firewall Azure** | Povolte přenos přes bránu Firewall služby Azure ve virtuálním počítači, pomocí značky plně kvalifikovaný název domény pro službu Azure Backup. |  Tato metoda se snadno používá, pokud máte brány Firewall Azure nastavit v podsíti virtuální sítě.<br/><br/> Nejde vytvořit vlastní značky plně kvalifikovaný název domény nebo upravit plně kvalifikovaných názvů domén ve značce.<br/><br/> Pokud používáte Azure Managed Disks, může být nutné počáteční další portu (port 8443) na bránu firewall.
...
### <a name="allow-firewall-access-by-using-an-fqdn-tag"></a>Povolit přístup přes bránu firewall pomocí značky plně kvalifikovaný název domény
...
Ve výchozím nastavení, má váš trezor nastavené [geograficky redundantní úložiště (GRS)](https://docs.microsoft.com/azure/storage/common/storage-redundancy-grs). Doporučujeme účet úložiště GRS pro vaši primární zálohu. Můžete použít [místně redundantní úložiště (LRS)](https://docs.microsoft.com/azure/storage/common/storage-redundancy-lrs?toc=%2fazure%2fstorage%2fblobs%2ftoc.json) pro levnější možnost.

Změňte typ replikace úložiště následujícím způsobem:

1. Na portálu vyberte nový trezor. V části **nastavení**vyberte **vlastnosti**.
2. V **vlastnosti**v části **konfigurace zálohování**vyberte **aktualizace**.
...
1. V trezoru > **přehled**vyberte **+ zálohování**.

   ![Zálohování tlačítko](./media/backup-azure-arm-vms-prepare/backup-button.png)

   **Zálohování** a **cíl zálohování** otevření podokna.

2. V **cíl zálohování** > **ve kterém je spuštěná vaše úloha?** vyberte **Azure**. V **co chcete zálohovat?** vyberte **virtuálního počítače** >  **OK**.
...
3. V **zásady zálohování**, vyberte zásadu, kterou chcete přidružit k trezoru. Pak vyberte **OK**.
    - Podrobnosti výchozí zásady jsou uvedené pod rozevírací nabídkou.
    - Vyberte **vytvořit nový** můžete vytvořit zásadu. [Další informace](backup-azure-arm-vms-prepare.md#configure-a-backup-policy) o definování zásad.
...
4. V **výběr virtuálních počítačů** podokně, vyberte virtuální počítače, které budou používat zadaných zásadách zálohování > **OK**.
   ...
   Vybraný virtuální počítač se ověří. Virtuální počítače můžete vybrat pouze ve stejné oblasti jako trezor. Virtuální počítače můžete zálohovat jenom v jednom trezoru.
...
5. V **zálohování**vyberte **povolit zálohování**.
   ...
   V tomto kroku nasadíte tuto zásadu do trezoru a virtuálním počítačům. Také nainstaluje rozšíření zálohování na virtuální počítač agenta spuštěného na virtuálním počítači Azure.
...
Spuštěný virtuální počítač poskytuje největší šanci získání bodu obnovení, který je konzistentní v rámci aplikace. Ale i v případě, že je vypnutý a rozšíření nejde nainstalovat je zálohování virtuálního počítače. To se označuje jako offline virtuálního počítače. V takovém případě bude bod obnovení konzistentní při selhání.
...
1. V nabídce trezoru, vyberte **zálohování položek**.
2. V **zálohování položek**vyberte **virtuálního počítače Azure**.
3. V **zálohování položek** seznamu, vyberte tři tečky (**...** ).
...
6. Sledujte oznámení o portálu. Můžete sledovat průběh úlohy v řídicím panelu trezoru > **úlohy zálohování** > **probíhá**. V závislosti na velikosti virtuálního počítače může vytváření prvotní zálohy nějakou dobu.
