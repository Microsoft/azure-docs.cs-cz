...
description: Tato specifikace popisuje protokol a formát fragmentace živého streamování založeného na MP4 pro Azure Media Services. Pomocí Azure Media Services můžete streamovat živé události a obsah všesměrového vysílání v reálném čase s využitím Azure jako cloudové platformy. Tento dokument také popisuje osvědčené postupy pro vytváření vysoce redundantních a robustních mechanismů živého přijímání.
...
## <a name="3-bitstream-format--iso-14496-12-fragmented-mp4"></a>3. Bitstream Format – ISO 14496-12 fragmentované MP4
...
1. Časová razítka a indexy pro fragmenty `fragment_index`MP4 (**TrackFragmentExtendedHeaderBox** `fragment_ absolute_ time` a) by měly dorazit ve vzestupném pořadí. I když Media Services je odolný vůči duplicitním fragmentům, má omezená schopnost změnit pořadí fragmentů podle časové osy médií.

## <a name="4-protocol-format--http"></a>4. Formát protokolu – HTTP
ISO fragmentované živé ingestování na bázi MP4 pro Media Services používá standardní dlouhodobou žádost HTTP POST k přenosu kódovaných mediálních dat, která jsou zabalená do služby ve formátu fragmentů MP4. Každý HTTP POST pošle kompletní fragment Bitstream MP4 ("Stream") od začátku do polí záhlaví (**ftyp**, **Live Server manifest**a **Moov** box) a pokračuje v sekvenci fragmentů (**Moof** a  **mdat** pole). Syntaxi URL požadavku HTTP POST najdete v části 9,2 v [1]. Příklad adresy URL příspěvku: 
...
## <a name="6-definition-of-stream"></a>6. Definice typu "Stream"
...
### <a name="option-3-bundle-audio-track-with-the-lowest-bitrate-video-track-into-one-stream"></a>Možnost 3: Sledování zvukového záznamu s nejnižší přenosovou rychlostí videa do jednoho streamu
