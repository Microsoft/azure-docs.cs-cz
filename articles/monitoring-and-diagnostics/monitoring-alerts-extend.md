...
- Můžete omezit přístup pouze k monitorování a upozorňování s využitím uživatelům [role Azure Monitor](monitoring-roles-permissions-security.md).
- Ve výstrahách Azure, můžete použít [skupiny akcí](monitoring-action-groups.md). To umožňuje mít více než jednu akci pro každou výstrahu. Zprávy SMS, můžete odeslat hlasový hovor, volání runbooku Azure Automation, vyvolat webhook a nakonfigurujete konektor správy služeb IT (ITSM). 
...
Můžete pokračovat k vytvoření výstrahy [rozhraní API služby Log Analytics](../log-analytics/log-analytics-api-alerts.md) a [šablony prostředku Log Analytics](../monitoring/monitoring-solutions-resources-searches-alerts.md). Pokud tak učiníte, musíte zahrnout skupiny akcí.
