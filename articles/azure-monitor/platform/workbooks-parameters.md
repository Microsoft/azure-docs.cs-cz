---
title: Vytváření interaktivních sestav pomocí Azure Monitorch sešitů | Vytváření parametrů | Dokumentace Microsoftu
...
4. V podokně nového parametru, které se objeví, zadejte:
    ...
    2. Zobrazovaný název: `Time Range` *(ale __zobrazované názvy__ můžou obsahovat mezery, speciální znaky, Emoji atd.)*
...
2. Otevřete rozevírací seznam _časový rozsah_ a v dolní části parametry vyberte možnost `Time Range`.
...
Oddíl _in text_ používá `label` parametru namísto jeho hodnoty. Parametry zveřejňují různé možnosti v závislosti na typu, např. výběry časových rozsahů umožňují hodnotu, popisek, dotaz, začátek, konec a zrnitost.

Pomocí oddílu `Previews` v podokně _Upravit parametr_ můžete zobrazit možnosti rozšíření pro váš parametr:
