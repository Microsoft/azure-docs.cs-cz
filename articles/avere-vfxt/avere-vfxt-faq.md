...
ms.locfileid: "50633854"
---
# <a name="avere-vfxt-for-azure-faq"></a>Avere vFXT pro Azure – nejčastější dotazy

Tento článek obsahuje odpovědi na otázky, které vám můžou pomoct rozhodnout, jestli řešení vFXT Avere je vhodná pro vaše potřeby. Poskytuje základní informace o možnostech Avere vFXT a vysvětluje, jak to funguje s dalšími komponentami Azure a produkty od jiných dodavatelů. 

## <a name="general"></a>Obecné 

### <a name="what-is-avere-vfxt-for-azure"></a>Co je Avere vFXT pro Azure?

Avere vFXT for Azure je systém souborů vysoce výkonné, který ukládá do mezipaměti aktivní data v Azure Compute pro efektivní zpracování důležité úlohy.

### <a name="is-the-avere-vfxt-a-storage-solution"></a>Je Avere vFXT řešení úložiště?

Ne. Systém souborů je Avere vFXT **mezipaměti** , připojí se k prostředí, úložiště, jako je vaše EMC nebo NetApp NAS nebo kontejner objektů Blob. VFXT zjednodušuje dat přicházejících od klientů a ukládá do mezipaměti, data, která slouží ke zlepšení výkonu ve velkém měřítku a v čase. VFXT samotný neukládá data. Nemá žádné informace o objemu dat uložených za ní.

### <a name="is-the-avere-vfxt-a-tiering-solution"></a>Je Avere vFXT vrstvení řešení?

Avere vFXT dělá automaticky úroveň dat mezi horkou a studenou úrovní.  

### <a name="how-do-i-know-if-an-environment-is-right-for-the-avere-vfxt"></a>Jak poznám, jestli je vhodná pro Avere vFXT prostředí?

Nejlepší způsob, jak uvažovat o tuto otázku, je zeptat, "Je možné ukládat do mezipaměti pracovního vytížení?" To znamená zatížení mít vysokou pro čtení k zápisu poměr – například 80/20 nebo 70/30 čtení/zápisu.
...
* Celkově je pomalý nebo nekonzistentní z důvodu dlouhé časy přístupu (desítky milisekund nebo sekund v závislosti na požadavcích). Latence klesne na nepřijatelnou pro koncové zákazníky.
...
* Zákazník chce spouštět své aktuální kanál "jako-je" v Azure virtual machines a potřebám POSIX základě sdíleného úložiště (nebo ukládání do mezipaměti) řešení pro zajištění škálovatelnosti. Když použijete Avere vFXT pro Azure, nemáte úprava architektury pracovních kanálu pro nativní volání do úložiště objektů Blob v Azure.

* Prostředí HPC aplikace je založená na NFSv3 klientů. (V některých případech klientů SMB 2.1 je možné, ale výkon je omezený.)

   Na obrázku níže se pokouší zjednodušit odpověď na tuto otázku. Čím blíž pracovního postupu je vpravo nahoře, tím je pravděpodobnější, že Avere řešení ukládání do mezipaměti je nejvhodnější pro vaše prostředí.
...
Řešení vFXT mezipaměti je určený pro zpracování stovky, tisíce nebo desítek tisíců výpočetních jader. Pokud máte několik počítače, které běží lehké práce, není Avere vFXT má to pravé řešení.

Typické Avere vFXT zákazníci spouštět náročných úloh, počínaje přibližně 1 000 jader procesoru. Tato prostředí můžou být velké až 50 000 jader nebo více. Vzhledem k tomu, vFXT je škálovatelná, můžete přidat uzly pro podporu těchto úloh s růstem tak, aby vyžadovala větší propustnost nebo Další vstupně-výstupních operací.

### <a name="how-much-data-can-an-avere-vfxt-environment-store"></a>Kolik dat můžete ukládat prostředí vFXT Avere?

Mezipaměť je Avere vFXT, nejsou uloženy konkrétně data. Kombinace paměti RAM a vlastnosti jednotek SSD používá k ukládání dat uložených v mezipaměti. Data se trvale uložena v back endové úložné systému (například NetApp NAS systému nebo kontejner objektů Blob). Systém vFXT Avere nemá informace o objemu dat uložených za. vFXT ukládá do mezipaměti pouze podmnožinu dat, který klienti požadují.  

### <a name="what-regions-are-supported"></a>Jaké oblasti jsou podporovány?

Od 1. listopadu 2018 se podporuje Avere vFXT pro Azure ve všech oblastech kromě suverénních oblastech (Čína, Německo) a oblasti státní správy. Ujistěte se, že oblast, kterou chcete použít může podporovat velký počet výpočetních jader, stejně jako instance virtuálních počítačů, které jsou potřeba k vytvoření clusteru vFXT Avere. Suverénní oblasti a ve státních cloudech se zatím nepodporují.

### <a name="how-do-i-get-help-with-the-avere-vfxt"></a>Jak získat pomoc s Avere vFXT?
...
### <a name="is-the-avere-vfxt-highly-available"></a>Je Avere vFXT s vysokou dostupností?
...
Ano, zákazníci mohou pomocí více než jednoho poskytovatele cloudu s clusterem vFXT Avere. Podporuje kontejnery standardní AWS S3 a standard kontejnerů Google Cloud Services, stejně jako kontejnery objektů Blob v Azure. 
...
### <a name="what-kind-of-azure-virtual-machines-does-the-avere-vfxt-run-on"></a>Jaký druh virtuální počítače Azure se Avere vFXT spouštět?  
...
Avere vFXT clusteru může být malá jako tři uzly virtuálních počítačů nebo velké až 24 uzly. Požádejte technickou podporu Azure o pomoc, plánování, pokud se domníváte, že potřebujete více než devíti uzlů clusteru. Větší počet uzlů vyžaduje větší architektury nasazení.
...
### <a name="can-i-run-the-vfxt-cluster-as-a-virtual-machine-scale-set"></a>Můžete spustit vFXT cluster jako škálovací sada virtuálních počítačů?

Avere vFXT nepodporuje nasazování virtuálního počítače škálovací sady (VMSS). Několik mechanismů dostupnosti integrované podpory jsou určeny pouze pro atomické virtuální počítače součástí clusteru.  

### <a name="can-i-run-the-vfxt-cluster-on-low-priority-vms"></a>Můžete spustit vFXT clusteru na virtuální počítače s nízkou prioritou

Ne, bude systém vyžadovat základní spouští stabilní sada virtuálních počítačů.

### <a name="can-i-run-the-vfxt-cluster-in-containers"></a>Můžete spustit vFXT clusteru v kontejnerech?
...
Ne. Model pro vysokou dostupnost v Avere vFXT v současné době nepoužívá nepodporuje členů clusteru jednotlivé vFXT umístěné v různých zón dostupnosti.
...
Ne, je systém nabízí jako clusterové zařízení a otestovali na typy konkrétního virtuálního počítače. Toto omezení pomáhá zákazníkům Vyhněte se vytváření systému, který nepodporuje požadavky na vysoký výkon pracovního postupu typické vFXT Avere. 

## <a name="technical---disks"></a>Technické – disky

### <a name="what-type-of-disks-are-supported-for-the-azure-vms"></a>Jaký typ disky jsou podporovány pro virtuální počítače Azure?
...
Data jsou rozdělená mezi disky, ale nejsou šifrována. Disky se však mohou být šifrována. Další informace najdete [tady](https://docs.microsoft.com/azure/virtual-machines/linux/security-policy#encryption).

## <a name="technical---networking"></a>Technické - sítě

### <a name="what-network-is-recommended"></a>Jaké sítě je doporučeno?

Pokud pomocí místního úložiště s Avere vFXT, měli byste mít 1 GB/s nebo vyšší síťové připojení. Pokud máte malé množství dat a jste ochotni ke zkopírování dat do cloudu před spuštěním úlohy, může být připojení k síti VPN dostatečná. 
...
### <a name="can-i-run-the-avere-vfxt-in-a-different-virtual-network-than-my-compute-cluster"></a>Můžu spustit Avere vFXT v jiné virtuální sítě než výpočetní cluster?
...
### <a name="does-the-avere-vfxt-require-its-own-subnet"></a>Vyžaduje Avere vFXT jeho vlastní podsíti?
...
### <a name="can-i-run-the-avere-vfxt-on-infiniband"></a>Můžete spustit Avere vFXT na Infiniband?

Ne, Avere vFXT používá jenom Ethernet/IP.

### <a name="how-do-i-access-my-on-premises-nas-environment-from-the-avere-vfxt"></a>Jak získám přístup Moje NAS v místním prostředí z Avere vFXT?

Prostředí vFXT Avere se nijak neliší od jakýkoli jiný virtuální počítač Azure, vyžaduje směrované přístup přes bránu sítě nebo VPN do datového centra zákazníka (a zpět). Zvažte použití připojení Azure ExpressRoute, pokud je k dispozici ve vašem prostředí.

### <a name="what-are-the-bandwidth-requirements-for-the-avere-vfxt"></a>Jaké jsou požadavky na šířku pásma pro Avere vFXT?
...
### <a name="can-i-run-the-vfxt-with-public-ip-addresses"></a>Můžete spustit vFXT pomocí veřejné IP adresy?

Ne, vFXT slouží k provozu v rámci prostředí sítě zabezpečený pomocí osvědčených postupů.  

## <a name="technical---backend-storage-core-filers"></a>Technické - back-Endového úložiště (křížového jader)
...
### <a name="how-does-the-avere-environment-store-data"></a>Jak prostředí Avere ukládání dat?

Avere vFXT není úložiště. Je mezipaměť, která čte a zapisuje data z více cílů úložiště volá základní filtrech. Data uložená na discích SSD Avere vFXT premium je přechodná a nakonec se vyprázdní na back-endu jádro souborového úložiště.
...
* Kontejnery objektů Blob v Azure (pouze LRS) 
* AWS S3 intervalů 
* Google Cloud intervalů

### <a name="why-doesnt-the-avere-vfxt-support-all-nfs-filers"></a>Proč Avere vFXT nepodporuje všechny křížového systému souborů NFS?
...
Podpora je založena na množství poptávky v poli. Pokud nejsou k dispozici dostatek výnosy podle požadavků na podporu dané řešení NAS, bude se považovat za. Proveďte požadavky prostřednictvím podpory Azure.

### <a name="can-i-use-azure-blob-storage-as-a-core-filer"></a>Můžete použít úložiště objektů Blob v Azure jako filtr core?

Ano, Avere vFXT pro Azure můžete použít kontejner objektů Blob bloku jako filer core cloudu.  
...
Musí být váš účet úložiště pro obecné účely v2 (GPv2) účtu a nakonfigurována pro místně redundantní úložiště (LRS). GRS a ZRS se nepodporují.

### <a name="can-i-use-archive-blob-storage"></a>Můžete použít úložiště objektů Blob v archivní?

Ne. Smlouva SLA pro archiv služby storage není kompatibilní s adresáři v reálném čase a přístup k souborům musí vFXT systému. 
...
Můžete buď nakonfigurovat šifrování objektů Blob v Azure (upřednostňováno) nebo na úrovni souborového vFXT core.  
...
Ve výchozím nastavení data se šifrují pomocí Microsoft Managed Keys pro Azure BLOB, tabulky, soubory a fronty. Můžete přineste si vlastní klíč k šifrování pro soubory a objekty BLOB Azure. Pokud se rozhodnete používat vFXT šifrování, musíte použít generovaný Avere klíč a uložte ho místně. 
...
Začínáme Avere vFXT licenci Azure je snadné v Tržišti Azure Marketplace. Zaregistrujte si účet Azure a pak postupujte podle pokynů v [nasazení clusteru vFXT](avere-vfxt-deploy.md) k vytvoření clusteru vFXT Avere. 

### <a name="how-much-does-the-avere-vfxt-cost"></a>Kolik stojí Avere vFXT?
...
Abyste mohli začít s Avere vFXT pro Azure, přečtěte si tyto odkazy na informace o plánování a nasazení vlastních systému:

* [Plánování Avere vFXT systému](avere-vfxt-deploy-plan.md)
* [Přehled nasazení](avere-vfxt-deploy-overview.md)
* [Příprava k tvorbě Avere vFXT](avere-vfxt-prereqs.md)
* [Nasazení clusteru vFXT](avere-vfxt-deploy.md)

Další informace o možnostech Avere vFXT a případy použití, navštivte [Avere vFXT pro Azure (preview)](https://azure.microsoft.com/services/storage/avere-vfxt/).
