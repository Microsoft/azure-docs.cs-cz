...
Získat odpovídající tokeny SAS a provádět volání do úložiště. `-SecretName` je vytvořený pomocí vstupu z `AccountName` a `Name` parametry při provedení [Set-AzureKeyVaultManagedStorageSasDefinition](https://docs.microsoft.com/en-us/powershell/module/AzureRM.KeyVault/Set-AzureKeyVaultManagedStorageSasDefinition).
