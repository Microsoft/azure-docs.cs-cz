...
| Monitorování a diagnostika | [**Umožněte funkcím monitorování**](search-monitor-usage.md) , abyste mohli přesáhnout metriky, které jsou vždy viditelné na portálu. Metriky pro dotazy za sekundu, latenci a omezování se zachycují a uvádějí na stránkách portálu bez další nezbytné konfigurace. <br/><br/>[**Prohledat analýzu provozu**](search-traffic-analytics.md) je další alternativa monitorování, kde se shromažďují a analyzují data na straně serveru a na straně klienta, aby se vyodemkly informace o tom, co uživatelé zapíší do vyhledávacího pole. |
...
### <a name="step-1-provision-service"></a>Krok 1: Zřídit službu
...
3. Pokračujte s kódem s využitím .NET nebo rozhraní REST API:
   ...
   + [Začínáme s rozhraním API REST](https://github.com/Azure-Samples/search-rest-api-getting-started) ukazuje stejný postup pomocí rozhraní REST API. Tento rychlý Start můžete také použít k volání rozhraní REST API z post nebo Fiddler: [Prozkoumejte Azure Search rozhraní REST API](search-get-started-postman.md).
