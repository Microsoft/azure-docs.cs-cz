...
A [cluster HDInsight Kafka s balíčkem Enterprise Security Package](./apache-domain-joined-configure-using-azure-adds.md).
...
2. Vyberte  **\<Název_clusteru > _kafka** pod **Kafka**. Může být uvedena jedna předem nakonfigurovaná zásada.
...
4. Vyberte **přidat** zásadu uložte.

5. Vyberte **přidat nové zásady** a potom zadejte následující hodnoty:
...
6. Vyberte **přidat** zásadu uložte.
...
1. Navažte připojení SSH ke clusteru použitím následujícího příkazu:
   ...
   Nahraďte `DOMAINADMIN` s uživatele s rolí správce pro cluster nakonfigurovaná v průběhu [vytvoření clusteru](./apache-domain-joined-configure-using-azure-adds.md#create-a-hdinsight-cluster-with-esp)a nahraďte `CLUSTERNAME` s názvem vašeho clusteru. Pokud se zobrazí výzva, zadejte heslo pro uživatelský účet správce. Další informace o použití `SSH` se službou HDInsight najdete v tématu [Použití SSH se službou HDInsight](../../hdinsight/hdinsight-hadoop-linux-use-ssh-unix.md).
...
3. Chcete-li získat hostiteli zprostředkovatele Kafka použijte následující příkazy. Po zobrazení výzvy zadejte heslo účtu správce clusteru.
   ...
   Než budete pokračovat, budete muset nastavit vývojové prostředí, pokud jste tak již neučinili. Budete potřebovat komponent, jako jsou Java JDK, Apache Maven a klienta SSH s spojovací bod služby. Další informace najdete v tématu [instalační pokyny](https://github.com/Azure-Samples/hdinsight-kafka-java-get-started/tree/master/DomainJoined-Producer-Consumer).
...
1. Postupujte podle kroků 2 a 3 v části **sestavovat a nasazovat v příkladu** v [kurzu: Použití Apache Kafka Producer and Consumer API](../kafka/apache-kafka-producer-consumer-api.md#build-and-deploy-the-example)
...
4. Postupujte podle kroku 3 v části **sestavovat a nasazovat v příkladu** v [kurzu: Použití Apache Kafka Producer and Consumer API](../kafka/apache-kafka-producer-consumer-api.md#build-and-deploy-the-example) zajistit, aby `kafka-producer-consumer.jar` dostupná je i **sales_user**.
...
Pokud nebudete tuto aplikaci používat, odstraňte clusteru Kafka, kterou jste vytvořili pomocí následujících kroků:
