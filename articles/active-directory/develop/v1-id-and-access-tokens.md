...
| `appidacr` |Reference třídy kontextu ověřování aplikace |Určuje, jak došlo k ověření klienta. Pro veřejné klienta hodnota je 0. Pokud používáte ID klienta a tajný kód klienta, hodnota je 1. Pokud certifikát klienta byla použita pro ověřování, hodnota je 2. <br><br> **Příklad hodnoty tokenů JWT**: <br> `"appidacr": "0"` |
...
| Okamžik ověření |Zaznamenává data a času, kdy došlo k ověřování. <br><br> **Příklad hodnoty SAML**: <br> `<AuthnStatement AuthnInstant="2011-12-29T05:35:22.000Z">` | |
...
| `unique_name` |Název |Poskytuje lidsky čitelnou hodnotu, která identifikuje subjekt tokenu. Tato hodnota nemusí být jedinečný v rámci tenanta a je určený pouze pro účely zobrazení. <br><br> **Příklad hodnoty SAML**: <br> `<Attribute Name=”http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name”>`<br>`<AttributeValue>frankm@contoso.com<AttributeValue>` <br><br> **Příklad hodnoty tokenů JWT**: <br> `"unique_name": "frankm@contoso.com"` |
