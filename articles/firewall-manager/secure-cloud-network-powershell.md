...
Pokud chcete zobrazit protokoly pro pravidla použití (s popisem povolených a zakázaných připojení HTTP) nebo změnit způsob zobrazení protokolů, můžete zkusit použít jiné dotazy KQL. Některé příklady najdete v [protokolech Azure Monitor Azure firewall](../firewall/log-analytics-samples.md).
