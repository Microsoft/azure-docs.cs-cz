...
## <a name="provision-a-centos-linux-virtual-machine"></a>Zřízení virtuálního počítače CentOS Linux
...
Zřízení virtuálního počítače CentOS v Azure. Další úlohou je vzdáleně připojit k virtuálnímu počítači pomocí účtu místního správce vytvořené při zřizování virtuálních počítačů.
...
```
"Contoso100.com" následuje název domény DNS vaší spravované domény. "contoso-centos" je název hostitele virtuálního počítače CentOS, ke které se připojujete ke spravované doméně.
...
Ověřte, zda je počítač byl úspěšně připojen ke spravované doméně. Připojení k doméně CentOS virtuálního počítače pomocí různých připojení SSH. Použít účet uživatele domény a pak zaškrtněte, pokud chcete zobrazit, pokud je uživatelský účet správně přeložit.

1. Vaše SSH terminálu zadejte následující příkaz pro připojení k doméně připojený CentOS virtuálnímu počítači pomocí SSH. Použít účet domény, který patří ke spravované doméně (například "bob@CONTOSO100.COM" v tomto případě.)
    ```
