...
0.9.0 verze obsahuje podporu pro komprimaci výstupu. To je ekvivalentní se spuštěným `-bgzip` následovaným `-tabix` ve výstupu VCF nebo gvcf. Další informace najdete v článku [Nejčastější dotazy](frequently-asked-questions-genomics.md). 
...
Verze 0.7.4 zahrnuje podporu pro přijetí tokenů SAS namísto klíčů účtu ve `config.txt` vstupu. Další informace najdete v tématu [rychlý Start – vstupní tokeny SAS](quickstart-input-sas.md). 
