...
description: Šifrovat úložiště objektů Blob v Azure, soubory Azure, Azure Queue storage a Azure Table storage na straně služby při ukládání dat pomocí funkce služby Azure Storage a šifrování a dešifrování se při načítání dat pomocí klíčů spravovaných zákazníkem.
services: storage
author: lakasa
manager: jeconnoc
ms.service: storage
...
Používat klíče spravované zákazníkem SSE, musíte přiřadit identitu účtu úložiště do účtu úložiště. Spuštěním následujícího příkazu Powershellu můžete nastavit identitu:

```powershell
Set-AzureRmStorageAccount -ResourceGroupName \$resourceGroup -Name \$accountName -AssignIdentity
```

Povolit obnovitelné odstranění a proveďte není vymazat spuštěním následujících příkazů Powershellu:
