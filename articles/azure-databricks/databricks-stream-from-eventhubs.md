...
description: Zjistěte, jak pomocí Azure Databricks se službou Event Hubs ingestovat streamovaná data z Twitteru a jak tato data číst v reálném čase.
...
V tomto kurzu s Azure Databricks propojíte systém pro příjem dat, abyste mohli streamovat data v reálném čase do clusteru Apache Spark. Pomocí služby Azure Event Hubs nastavíte systém pro příjem dat v reálném čase a pak ho připojíte k Azure Databricks, aby zpracovával přicházející zprávy. Pro přístup k datovému proudu v reálném čase použijete rozhraní Twitter API umožňující ingestování tweetů do služby Event Hubs. Jakmile budete mít data v Azure Databricks, můžete je dále analyzovat spouštěním analytických úloh. V tomto kurzu extrahujete tweety, které obsahují výraz Azure.

Následující snímek obrazovky ukazuje běh aplikace:
...
Pro příjem datového proudu tweetů v reálném čase vytvoříte aplikaci v Twitteru. Postupujte podle pokynů k vytvoření aplikace Twitter a poznamenejte si hodnoty, které potřebujete k dokončení tohoto kurzu.
...
Pokud chcete poznámkový blok spustit, stiskněte **SHIFT + ENTER**. Zobrazí se výstup podobný následujícímu fragmentu kódu. Každá událost ve výstupu představuje tweet v reálném čase, který obsahuje výraz Azure a ingestuje se do služby Event Hubs. 
...
A to je vše! Pomocí Azure Databricks jste úspěšně streamovali data v reálném čase do služby Azure Event Hubs. Následně jste streamovaná data použili pomocí konektoru služby Event Hubs pro Apache Spark.
...
>[Analýza mínění na streamovaných datech pomocí Azure Databricks](databricks-stream-from-eventhubs.md)
