...
## <a id="disable-xslt"></a>Zakázat skriptování XSLT pro všechny transformace pomocí nedůvěryhodných šablon stylů
...
## <a id="out-sniffing"></a>Zajistěte, aby každá stránka, která by mohla obsahovat uživatelsky ovladatelného obsahu, výslovný z automatického sledování MIME.
...
## <a id="xml-resolution"></a>Posílení nebo zakázání rozlišení entit XML
...
## <a id="app-verification"></a>Aplikace, které využívají http. sys, provádějí ověřování pomocí kanonického zpracování adresy URL.
...
## <a id="typesafe"></a>Zajistěte, aby se pro přístup k datům používaly parametry bezpečné pro typ ve webové aplikaci.
...
## <a id="binding-mvc"></a>Použití samostatných tříd vazeb modelů nebo seznamů filtru vazeb k zamezení ohrožení zabezpečení pro hromadné přiřazení MVC
...
| **Atributy**              | Není k dispozici  |
| **Odkazy**              | [Jak zabránit skriptování mezi weby v ASP.NET](https://msdn.microsoft.com/library/ms998274.aspx), [skriptování mezi weby](https://cwe.mitre.org/data/definitions/79.html), skriptování [XSS (skriptování mezi weby) prevence tahák listů](https://www.owasp.org/index.php/XSS_(Cross_Site_Scripting)_Prevention_Cheat_Sheet) |
...
## <a id="typemodel"></a>Provede ověřování vstupu a filtrování u všech vlastností modelu řetězcového typu.
...
## <a id="richtext"></a>Pro pole formuláře, která přijímají všechny znaky, například Editor formátovaného textu, by měla být použita upravená část.
...
## <a id="inbuilt-encode"></a>Nepřiřazujte elementy modelu DOM k jímky, které nemají sestavené kódování.
...
## <a id="redirect-safe"></a>Ověření všech přesměrování v rámci aplikace je uzavřeno nebo provedeno bezpečně.
...
## <a id="string-method"></a>Implementovat ověřování vstupu pro všechny parametry řetězcového typu akceptované metodami řadiče
...
## <a id="dos-expression"></a>Nastavte časový limit horní meze pro zpracování regulárního výrazu, aby se zabránilo DoS v důsledku chybných regulárních výrazů.
...
## <a id="html-razor"></a>Nepoužívejte HTML. Raw v zobrazeních Razor
...
## <a id="validation-api"></a>Ujistěte se, že se ověřování modelu provádí na metodách webového rozhraní API.
...
## <a id="string-api"></a>Implementovat ověřování vstupu pro všechny parametry řetězcového typu akceptované metodami webového rozhraní API
...
## <a id="typesafe-api"></a>Zajistěte, aby se pro přístup k datům používaly parametry bezpečné pro typ ve webovém rozhraní API.
...
## <a id="sql-docdb"></a>Použití parametrizovaných dotazů SQL pro Cosmos DB
...
## <a id="schema-binding"></a>Ověřování vstupu WCF prostřednictvím vazby schématu
...
## <a id="parameters"></a>WCF – ověřování vstupu prostřednictvím inspektorů parametrů
