...
Azure nabízí také obrázky Red Hat Gold (`rhel-byos`). Tyto obrázky můžou být užitečné pro zákazníky, kteří mají existující předplatné Red Hat a chtějí je používat v Azure. Abyste mohli povolit cloudový přístup Red Hat do Azure, budete si muset povolit svoje stávající předplatné Red Hat. Přístup k těmto obrázkům se uděluje automaticky, pokud jsou pro cloudový přístup povolená předplatná Red Hat a splňují požadavky na způsobilost. Použití těchto imagí umožňuje zákazníkovi vyhnout se dvojímu vyúčtování, které by mohlo vést k používání imagí s průběžnými platbami.
...
RHEL image připojené k RHUI aktualizací ve výchozím nastavení na nejnovější podverzi RHEL při spuštění `yum update`. Toto chování znamená, že virtuální počítač s RHEL 7,4 se může upgradovat na RHEL 7,7, pokud se na něm spustí operace `yum update`. Toto chování je záměrné pro RHUI. Pokud chcete toto chování upgradu zmírnit, přepínejte z běžných úložišť RHEL do [úložišť podpory rozšířené aktualizace](./redhat-rhui.md#rhel-eus-and-version-locking-rhel-vms).
