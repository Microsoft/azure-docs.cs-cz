...
V prostředí založeném na bash na MacOS použijte `curl` ke stažení linkerované verze následujícím způsobem:
...
`linkerd`Binární soubor klienta běží na klientském počítači a umožňuje interakci s propojenou sítí služby. Použijte následující příkazy pro instalaci linkeru `linkerd` binárního klienta v prostředí založeném na bash na MacOS. Tyto příkazy kopírují `linkerd` binární soubor klienta do umístění standardního uživatelského programu v `PATH` .
