...
1. Název pracovní skupiny změňte na něco smysluplného, například `AGWORKGROUP`: 
   ...
   ![Změnit název pracovní skupiny](media/virtual-machines-windows-sql-workgroup-availability-group/1-change-workgroup-name.png)
...
1. Na konci souboru přidejte položku pro každý uzel, skupinu dostupnosti a naslouchací proces ve formě `IP Address, DNS Suffix #comment` jako: 
   ...
   ![Přidat položky pro IP adresu, cluster a naslouchací proces do souboru hostitele](media/virtual-machines-windows-sql-workgroup-availability-group/4-host-file.png)
...
Po vytvoření clusteru přiřaďte IP adresu statického clusteru. Chcete-li to provést, postupujte takto:
...
1. Na jednom z uzlů otevřete **Správce clusteru s podporou převzetí služeb při selhání**, vyberte cluster, klikněte pravým tlačítkem na **Název: \<ClusterNam >** v části **základní prostředky clusteru** a pak vyberte **vlastnosti**. 
   ...
   ![Spustit vlastnosti pro název clusteru](media/virtual-machines-windows-sql-workgroup-availability-group/5-launch-cluster-name-properties.png)
...
1. Vyberte **použít statické**, zadejte IP adresu clusteru a pak vyberte **OK**: 
   ...
   ![Zadejte statickou IP adresu clusteru.](media/virtual-machines-windows-sql-workgroup-availability-group/6-provide-static-ip-for-cluster.png)

1. Ověřte, zda nastavení vypadá správně, a pak vyberte **OK** a uložte je:

   ![Ověřit vlastnosti clusteru](media/virtual-machines-windows-sql-workgroup-availability-group/7-verify-cluster-properties.png)

## <a name="create-a-cloud-witness"></a>Vytvoření určujícího cloudu 

V tomto kroku nakonfigurujte určující sdílenou složku v cloudu. Pokud nejste obeznámeni s postupem, Projděte si [kurz clusteru s podporou převzetí služeb při selhání](virtual-machines-windows-portal-sql-create-failover-cluster.md#create-a-cloud-witness). 

## <a name="enable-availability-group-feature"></a>Povolit funkci skupiny dostupnosti 

V tomto kroku povolíte funkci Skupina dostupnosti. Pokud nejste obeznámeni s postupem, Projděte si [kurz skupiny dostupnosti](virtual-machines-windows-portal-sql-availability-group-tutorial.md#enable-availability-groups). 
...
V tomto kroku Nakonfigurujte skupinu dostupnosti a přidejte do ní své databáze. V tuto chvíli nevytvářejte naslouchací proces. Pokud nejste obeznámeni s postupem, Projděte si [kurz skupiny dostupnosti](virtual-machines-windows-portal-sql-availability-group-tutorial.md#create-the-availability-group). Nezapomeňte zahájit převzetí služeb při selhání a navrácení služeb po obnovení, abyste ověřili, že vše funguje tak, jak by mělo. 

   > [!NOTE]
   > Pokud během procesu synchronizace dojde k selhání, bude pravděpodobně nutné udělit `NT AUTHORITY\SYSTEM` oprávnění sysadmin k vytváření prostředků clusteru v prvním uzlu, například `AGNode1` dočasně. 

## <a name="configure-load-balancer"></a>Konfigurace nástroje pro vyrovnávání zatížení

V tomto posledním kroku nakonfigurujte nástroj pro vyrovnávání zatížení pomocí [Azure Portal](virtual-machines-windows-portal-sql-alwayson-int-listener.md) nebo [PowerShellu](virtual-machines-windows-portal-sql-ps-alwayson-int-listener.md) .


## <a name="next-steps"></a>Další kroky

Ke konfiguraci skupiny dostupnosti můžete použít taky [příkaz AZ SQL VM CLI](virtual-machines-windows-sql-availability-group-cli.md) . 
