...
Po ověření `id_token`můžete zahájit relaci s uživatelem a pomocí deklarací v `id_token` získat informace o uživateli ve vaší aplikaci. Tyto informace lze použít pro zobrazení, záznamy, přizpůsobení atd. Pokud chcete získat další informace o `id_tokens` a deklaracích, přečtěte si [Id_tokens AAD](../develop/id-tokens.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json).
...
* Přečtěte si další informace o [`id_token` a deklaracích](../develop/id-tokens.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json).
