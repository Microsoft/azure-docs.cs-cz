...
[Referenční dokumentace k rozhraní API](https://docs.microsoft.com/python/api/azure-storage-queue/index) |  | balíčku [zdrojového kódu knihovny](https://github.com/Azure/azure-sdk-for-python/tree/master/sdk/storage/azure-storage-queue) [(Python Package index)](https://pypi.org/project/azure-storage-queue/) | [ukázky](https://github.com/Azure/azure-sdk-for-python/tree/master/sdk/storage/azure-storage-queue/samples)
...
Pomocí příkazu `pip install` nainstalujte klientskou knihovnu služby Azure Blob Storage pro balíček python.
...
* [QueueServiceClient](https://docs.microsoft.com/python/api/azure-storage-queue/azure.storage.queue.queueserviceclient): `QueueServiceClient` umožňuje správu všech front v účtu úložiště.
* [QueueClient](https://docs.microsoft.com/python/api/azure-storage-queue/azure.storage.queue.queueclient): třída `QueueClient` umožňuje spravovat a manipulovat s jednotlivou frontou a jejími zprávami.
* [QueueMessage](https://docs.microsoft.com/python/api/azure-storage-queue/azure.storage.queue.queuemessage): třída `QueueMessage` představuje jednotlivé objekty vracené při volání [receive_messages](https://docs.microsoft.com/python/api/azure-storage-queue/azure.storage.queue.queueclient#receive-messages---kwargs-) ve frontě.
...
Následující fragment kódu přidá zprávy do fronty voláním metody [send_message](https://docs.microsoft.com/python/api/azure-storage-queue/azure.storage.queue.queueclient#send-message-content----kwargs-) . Ukládá také [QueueMessage](https://docs.microsoft.com/python/api/azure-storage-queue/azure.storage.queue.queuemessage) vrácené z třetího volání `send_message`. `saved_message` slouží k aktualizaci obsahu zprávy později v programu.
...
Podívejte se na zprávy ve frontě tak, že zavoláte metodu [peek_messages](https://docs.microsoft.com/python/api/azure-storage-queue/azure.storage.queue.queueclient#peek-messages-max-messages-none----kwargs-) . Metoda `peek_messages` načítá jednu nebo více zpráv před frontou fronty, ale nemění viditelnost zprávy.
...
Aktualizujte obsah zprávy voláním metody [update_message](https://docs.microsoft.com/python/api/azure-storage-queue/azure.storage.queue.queueclient#update-message-message--pop-receipt-none--content-none----kwargs-) . Metoda `update_message` může změnit časový limit a obsah viditelnosti zprávy. Obsah zprávy musí být řetězec kódovaný v kódování UTF-8, který má velikost až 64 KB. Společně s novým obsahem předejte hodnoty z zprávy, která byla uložena dříve v kódu. Hodnoty `saved_message` určují, která zpráva se má aktualizovat.
...
V okně konzoly přejděte do adresáře obsahujícího soubor *Queues-Quickstart-V12.py* a spusťte aplikaci spuštěním následujícího příkazu `python`.
