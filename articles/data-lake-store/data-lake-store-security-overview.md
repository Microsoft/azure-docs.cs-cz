...
Každé předplatné Azure lze přidružit k instanci služby Azure Active Directory. K vašemu účtu Data Lake Storage Gen1 mají přístup pouze uživatelé a identity služeb, které jsou definovány ve vaší službě Azure Active Directory, pomocí portálu Azure Portal, nástrojů příkazového řádku nebo prostřednictvím klientských aplikací, které vaše organizace vytváří pomocí datového jezera. Úložiště Gen1 SDK. Hlavní výhody použití služby Azure Active Directory jako centralizovaného mechanismu řízení přístupu jsou:
...
| --- | --- | --- | --- |
| Nebyla přiřazena žádná role. |Žádný |Řídí se acl |Uživatel nemůže používat portál Azure nebo rutiny Azure PowerShell k procházení Gen1 úložiště datového jezera. Uživatel může používat pouze nástroje příkazového řádku. |
