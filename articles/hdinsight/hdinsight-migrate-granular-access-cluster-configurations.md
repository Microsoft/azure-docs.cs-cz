...
description: Další informace o změny potřebné k migraci do granulární přístup na základě rolí pro konfigurace clusteru.
author: tylerfox
ms.author: tyfox
ms.reviewer: jasonh
...
ms.locfileid: "60006229"
---
# <a name="migrate-to-granular-role-based-access-for-cluster-configurations"></a>Migrace na granulární přístup na základě rolí pro konfigurace clusteru
...
Od této chvíle, těchto tajných kódů nebude přístupný pro uživatele k roli Čtenář. Nepovedlo se také zavádět nové Role 'Operátor clusteru Hdinsight', který se bude moct načíst tajné kódy bez udělení oprávnění správce, Přispěvatel nebo vlastník. Shrnutí:
...
- [Azure HDInsight Tools for Visual Studio Code](#azure-hdinsight-tools-for-visual-studio-code) ___ verzi a níže.
- [Sada Azure Toolkit pro IntelliJ](#azure-toolkit-for-intellij) __ verzi a níže.
- [Sada Azure Toolkit pro Eclipse](#azure-toolkit-for-eclipse) __ verzi a níže.
- [Sada SDK pro .NET](#sdk-for-net)
    ...
    - [verze 3.x a až](#versions-3x-and-up): Uživatele, kteří používají `EnableHttp`, `DisableHttp`, `Update`, nebo `Get` metody ze `ConfigurationsOperationsExtensions` třídy.
- [Sada SDK pro Python](#sdk-for-python): Uživatele, kteří používají `get` nebo `update` metody ze třídy ConfigurationsOperations.
- [Sada SDK pro Javu](#sdk-for-java): Uživatele, kteří používají `update` nebo `get` metody ze třídy ConfigurationsInner.
- [Sada SDK for Go](#sdk-for-go): Uživatele, kteří používají `Get` nebo `Update` metody ze struktury ConfigurationsClient.

Viz níže uvedené části (nebo použijte výše uvedené odkazy) zobrazíte migrace kroky pro váš scénář.

### <a name="api"></a>Rozhraní API

Následující rozhraní API se změnil nebo zastaralé:

- [**/Configurations/ GET {configurationName}** ](https://docs.microsoft.com/rest/api/hdinsight/hdinsight-cluster#get-configuration) (odebrání citlivých informací) https://management.azure.com/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.HDInsight/clusters/{clusterName}/configurations/{configurationName}?api-version={api-version}
    - Dříve se používá k získání individuální konfigurace typy (včetně tajné kódy).
    - Toto volání rozhraní API nyní vrátí typy individuální konfigurace s tajnými kódy vynechán. Získat všechny konfigurace, včetně tajné klíče, použít novou [příspěvek /configurations]() volání. Získat jenom nastavení brány, pomocí nové [příspěvek /getGatewaySettings]() volání.
- [**GET /configurations** ](https://docs.microsoft.com/rest/api/hdinsight/hdinsight-cluster#get-configurations) (zastaralé) https://management.azure.com/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.HDInsight/clusters/{clusterName}/configurations?api-version={api-version}
    - Dříve se používá k získání všech konfigurací (včetně tajné kódy)
    - Toto volání rozhraní API se už nebude podporovat. Pokud chcete získat všechny konfigurace do budoucna, pomocí nové [příspěvek /configurations]() volání. Chcete-li získat konfigurace s citlivé parametry tento parametr vynechán, použijte [/configurations/ GET {configurationName}]() volání.
- [**PŘÍSPĚVEK /configurations/ {configurationName}** ](https://docs.microsoft.com/rest/api/hdinsight/hdinsight-cluster#change-connectivity-settings) (zastaralé) https://management.azure.com/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.HDInsight/clusters/{clusterName}/configurations/{configurationName}?api-version={api-version}
    - Dříve se používá k aktualizaci přihlašovacích údajů brány.
    - Toto volání rozhraní API bude zastaralé a už není podporovaná. Pomocí nové [příspěvek /updateGatewaySettings]() místo.

Následující nahrazení, který jste přidali rozhraní API:</span>

- **/Configurations příspěvku** https://management.azure.com/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.HDInsight/clusters/{clusterName}/configurations?api-version={api-version}
    - Pomocí tohoto rozhraní API k získání všechny konfigurace, včetně tajných klíčů.
- **/GetGatewaySettings příspěvku** https://management.azure.com/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.HDInsight/clusters/{clusterName}/getGatewaySettings?api-version={api-version}
    - Pomocí tohoto rozhraní API k získání nastavení brány.
- **/UpdateGatewaySettings příspěvku** https://management.azure.com/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.HDInsight/clusters/{clusterName}/updateGatewaySettings?api-version={api-version}
...
Pokud používáte verze 1.1.1 nebo starší, aktualizujte prosím [nejnovější verzi Azure HDInsight Tools for Visual Studio Code](https://marketplace.visualstudio.com/items?itemName=mshdinsight.azure-hdinsight&ssr=false) vyhnout se přerušením.

### <a name="azure-toolkit-for-intellij"></a>Azure Toolkit pro IntelliJ

Pokud používáte verzi 3.21.0 nebo starší, aktualizujte prosím [nejnovější verzi sady Azure Toolkit pro IntelliJ plugin](https://plugins.jetbrains.com/plugin/8053-azure-toolkit-for-intellij) vyhnout se přerušením.

### <a name="azure-toolkit-for-eclipse"></a>Azure Toolkit pro Eclipse

Pokud používáte verzi 2019-03-29 let, aktualizujte prosím na nejnovější verzi sady Azure Toolkit pro Eclipse vyhnout se přerušením.

### <a name="sdk-for-net"></a>SDK pro .NET

#### <a name="versions-1x-and-2x"></a>Verze 1.x a 2.x

Aktualizujte prosím [verze 2.1.0](https://www.nuget.org/packages/Microsoft.Azure.Management.HDInsight/2.1.0) sady HDInsight SDK pro .NET. Změny jen minimum kódu mohou být vyžadovány, pokud používáte metodu těmito změnami ovlivněny:
...
Aktualizujte prosím [verze 5.0.0](https://www.nuget.org/packages/Microsoft.Azure.Management.HDInsight/5.0.0) sady HDInsight SDK pro .NET. Změny jen minimum kódu mohou být vyžadovány, pokud používáte metodu těmito změnami ovlivněny:
...
Aktualizujte prosím [verze 1.0.0](https://pypi.org/project/azure-mgmt-hdinsight/1.0.0/) sady HDInsight SDK pro Python. Změny jen minimum kódu mohou být vyžadovány, pokud používáte metodu těmito změnami ovlivněny:
...
Aktualizujte prosím [verze 1.0.0](https://search.maven.org/artifact/com.microsoft.azure.hdinsight.v2018_06_01_preview/azure-mgmt-hdinsight/) sady HDInsight SDK pro Javu. Změny jen minimum kódu mohou být vyžadovány, pokud používáte metodu těmito změnami ovlivněny:
...
- [`ConfigurationsInner.get`](https://docs.microsoft.com/java/api/com.microsoft.azure.management.hdinsight.v2018__06__01__preview.implementation._configurations_inner.get) bude **už nevracel citlivé parametry** , jako je úložiště klíčů (základní web) nebo přihlašovací údaje protokolu HTTP (brány).
    - Pokud chcete načíst všechny konfigurace, včetně citlivé parametrů, použijte `ConfigurationsInner.list` do budoucna.  Všimněte si, že uživatelé s rolí 'Čtečky' nebude možné použít tuto metodu. Díky tomu umožňuje zajistit detailní kontrolu nad tím, které mohou uživatelé citlivých informací pro cluster. 
    - Chcete-li načíst jenom přihlašovací údaje brány protokolu HTTP, použijte `ConfigurationsOperations.get_gateway_settings`.
- [`ConfigurationsInner.update`](https://docs.microsoft.com/java/api/com.microsoft.azure.management.hdinsight.v2018__06__01__preview.implementation._configurations_inner.update) je nyní zastaralá a nahradila ji `ClusterOperationsExtensions.update_gateway_settings`.

### <a name="sdk-for-go"></a>Sada SDK For Go

Aktualizujte prosím [verze 27.1.0](https://github.com/Azure/azure-sdk-for-go/tree/master/services/preview/hdinsight/mgmt/2018-06-01-preview/hdinsight) sady HDInsight SDK for Go. Změny jen minimum kódu mohou být vyžadovány, pokud používáte metodu těmito změnami ovlivněny:
...
Uživatel s [Přispěvatel](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#contributor) nebo [vlastníka](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#owner) role můžete udělit role operátora clusteru HDInsight pro uživatele, že chcete mít přístup pro čtení a zápis k tajným kódům clusteru HDInsight jako brány přihlašovacích údajů clusteru a klíče účtu úložiště.

### <a name="using-the-azure-cli"></a>Použití Azure CLI

Pomocí následujícího příkazu v rozhraní příkazového řádku Azure je nejjednodušší způsob, jak přidat tato přiřazení rolí:
...
Výše uvedený příkaz uděluje této role na úrovni předplatného. Místo toho pouze udělit tuto roli na úrovni skupiny prostředků, příkaz můžete upravit takto:
