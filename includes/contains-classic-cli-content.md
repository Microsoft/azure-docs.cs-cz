---
ms.topic: include
ms.date: 06/05/2019
author: dbradish-microsoft
ms.author: dbradish
manager: barbkess
ms.openlocfilehash: 93a7e7994727012cd27f2901f75f23d1b130784d
ms.sourcegitcommit: 877491bd46921c11dd478bd25fc718ceee2dcc08
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 07/02/2020
ms.locfileid: "85378440"
---
> [!IMPORTANT]
> Pro obsah v tomto článku se vyžaduje Azure Classic CLI. Jedná se o starší verzi Azure CLI, která by se měla používat pouze s modelem nasazení Azure Classic.
>
> Informace o instalaci klasického rozhraní příkazového řádku najdete v tématu [instalace rozhraní příkazového řádku Azure Classic](/cli/azure/install-classic-cli) a instalace aktuálního prostředí Azure cli pro Azure Resource Manager nasazení najdete v tématu Instalace rozhraní příkazového [řádku Azure CLI](/cli/azure/install-azure-cli).
