...
Při načítání poznámkových **`Error loading notebook`** bloků, které mají větší velikost, se může zobrazit chyba.  
...
* Je důležité zachovat malou velikost poznámkového bloku. Veškerý výstup úloh Sparku, který se pošle zpátky do Jupyter, je uložený v poznámkovém bloku.  Osvědčeným postupem je použití Jupyter obecně k tomu, abyste se `.collect()` vyhnuli spouštění ve velkých RDD nebo datarámečcích; místo toho, pokud chcete prohlížet obsah RDD, zvažte spuštění `.take()` nebo `.sample()` , aby váš výstup nezískal příliš velký.
