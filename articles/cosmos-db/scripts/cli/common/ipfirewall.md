...
> [!NOTE]
> Tato ukázka předvádí použití účtu rozhraní API SQL (Core). Chcete-li použít tuto ukázku pro jiná rozhraní API, použijte `ip-range-filter` parametr ve skriptu níže na `az cosmosdb account create` příkaz pro skript specifický pro rozhraní API.
