---
title: Kurz – připojení zařízení IoT technologie Plug and Play (Preview) k Azure IoT Central
...
> [!div class="checklist"]
> * Pomocí Visual Studio Code můžete pomocí DCM vytvořit zařízení technologie Plug and Play IoT (Preview).
...
1. Na příkazovém řádku vytvořte `cmake` podadresář ve `devkit_device` složce a přejděte do této složky:
...
1. Spuštěním následujících příkazů Sestavte zástupné procedury generovaného kódu. Nahraďte `<directory of your Vcpkg repo>` zástupný text cestou k kopii úložiště **Vcpkg** :
...
V tomto kurzu jste zjistili, jak připojit zařízení IoT technologie Plug and Play (Preview), které bylo vygenerováno z DCM v úložišti veřejného modelu.
