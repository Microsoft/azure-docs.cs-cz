...
Azure App Service umožňuje vytvářet a hostovat webové aplikace s použitím jazyků, jako jsou Python, Ruby, C# a Java. Azure podporuje také vlastní kontejnery, které umožňují spouštět prakticky všechny programovací jazyky na platformě Azure App Service.
...
1. V další nabídce vyberte virtuální síť vytvořenou v nasazení, které začíná na `aad-vnet`. Můžete buď vytvořit novou podsíť, nebo vybrat některou z existujících.
...
3.  Na kartě ověřování v části identifikátory URI přesměrování vyberte možnost Web v poli se seznamem a přidejte následující identifikátory URI pro přesměrování.
    ...
    o v části Rozšířená nastavení nastavte URL pro odhlášení na`https://WebApp-OpenIDConnect-DotNet-code-v2-contoso.azurewebsites.net/signout-oidc`
