---
title: Základní hodnoty zabezpečení Azure pro ExpressRoute
description: Základní ExpressRoute zabezpečení poskytuje pokyny a materiály k postupu pro implementaci doporučení zabezpečení, která jsou určená v srovnávacím testu zabezpečení Azure.
author: msmbaldwin
...
ms.locfileid: "101740303"
---
# <a name="azure-security-baseline-for-expressroute"></a>Základní hodnoty zabezpečení Azure pro ExpressRoute

Tato základní hodnota zabezpečení se vztahuje na pokyny od [verze Azure Security test 1,0](../security/benchmarks/overview-v1.md) až ExpressRoute. Azure Security Benchmark poskytuje doporučení, jak můžete zabezpečit svá cloudová řešení v Azure.
Obsah se seskupuje podle **ovládacích prvků zabezpečení** , které definuje srovnávací test zabezpečení Azure, a souvisejících pokynů, které platí pro ExpressRoute. **Ovládací prvky** , které se nevztahují na ExpressRoute, jsou vyloučené.

 
Pokud chcete zjistit, jak ExpressRoute kompletně mapuje srovnávací test zabezpečení Azure, přečtěte si [celý soubor mapování standardních hodnot ExpressRoute Security](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines).
...
**Doprovodné** materiály: nepoužitelné; jednotné přihlašování (SSO) přináší zabezpečení a pohodlí, když se uživatelé přihlásí k vlastním aplikacím v Azure Active Directory (Azure AD). Přístup k rovině ovládacího prvku Azure ExpressRoute (například Azure Portal) je už integrovaný s Azure AD a je přístupný prostřednictvím Azure Portal a Azure Resource Manager REST API.
...
**Pokyny**: použití pracovní stanice s privilegovaným přístupem (privilegovaným přístupem) se službou Azure Active Directory (Azure AD) Multi-Factor Authentication povolené pro přihlášení a konfiguraci prostředků souvisejících s Azure Sentinel. 
...
**Pokyny**: použití Azure Active Directory (Azure AD) jako centrálního ověřování a autorizačního systému pro prostředky Azure ExpressRoute. Azure AD chrání data pomocí silného šifrování pro neaktivní a tranzitní data. Azure AD také nasolete, hodnoty hash a bezpečně ukládají přihlašovací údaje uživatele.
...
- [Principy rolí správy v ExpressRoute](https://docs.microsoft.com/azure/expressroute/expressroute-howto-linkvnet-portal-resource-manager#connect-a-vnet-to-a-circuit---different-subscription)
