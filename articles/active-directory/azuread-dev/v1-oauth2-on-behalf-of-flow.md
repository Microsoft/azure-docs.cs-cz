...
Tyto kroky představují tok spouštěný jménem: ![zobrazuje kroky v toku za běhu OAuth 2.0 za běhu](./media/v1-oauth2-on-behalf-of-flow/active-directory-protocols-oauth-on-behalf-of-flow.png)
...
1. Zvolte **vybrat oprávnění** a pak vyberte **Access \<Service Name >** .
...
1. Vyhledejte vlastnost pole `knownClientApplications` a přidejte ID klienta klientské aplikace jako prvek.
...
| Parametr |  | Popis |
| --- | --- | --- |
| grant_type |požadováno | Typ požadavku tokenu Požadavek OBO používá JSON Web Token (JWT), takže hodnota musí být **urn: IETF: params: OAuth: Grant-Type: JWT-nosič**. |
| Neplatný |požadováno | Hodnota přístupového tokenu použitého v žádosti |
| client_id |požadováno | ID aplikace přiřazené volající službě během registrace ve službě Azure AD. Chcete-li najít ID aplikace v Azure Portal, vyberte možnost **Active Directory**, zvolte adresář a pak vyberte název aplikace. |
| client_secret |požadováno | Klíč zaregistrovaný pro volající službu ve službě Azure AD. Tato hodnota by se měla poznamenat v době registrace. |
| prostředek |požadováno | Identifikátor URI ID aplikace přijímající služby (zabezpečeného prostředku) Identifikátor URI ID aplikace v Azure Portal najdete tak, že vyberete **Active Directory** a zvolíte adresář. Vyberte název aplikace, zvolte **všechna nastavení**a pak vyberte **vlastnosti**. |
| requested_token_use |požadováno | Určuje, jak se má požadavek zpracovat. V toku musí být hodnota **on_behalf_of**. |
| obor |požadováno | Mezerou oddělený seznam oborů pro požadavek na token. Pro OpenID Connect se musí zadat obor **OpenID** .|

#### <a name="example"></a>Příklad

Následující příspěvek HTTP požaduje přístupový token pro https://graph.windows.net webové rozhraní API. `client_id` identifikuje službu, která žádá o přístupový token.
...
Následující příspěvek HTTP požaduje přístupový token pro https://graph.windows.net webové rozhraní API s certifikátem. `client_id` identifikuje službu, která žádá o přístupový token.
...
| obor |Rozsah přístupu uděleného v tokenu. |
...
Následující příklad ukazuje odpověď na úspěch na žádost o přístupový token pro https://graph.windows.net webové rozhraní API.
...
Služba střední vrstvy může pomocí získaného přístupového tokenu provádět ověřené požadavky webového rozhraní API pro příjem dat, a to nastavením tokenu v hlavičce `Authorization`.
...
| Parametr |  | Popis |
| --- | --- | --- |
| grant_type |požadováno | Typ požadavku tokenu Pro požadavek, který používá JWT, musí být hodnota **urn: IETF: params: OAuth: Grant-Type: JWT-nosič**. |
| Neplatný |požadováno | Hodnota přístupového tokenu použitého v žádosti|
| client_id |požadováno | ID aplikace přiřazené volající službě během registrace ve službě Azure AD. Chcete-li najít ID aplikace v Azure Portal, vyberte možnost **Active Directory**, zvolte adresář a pak vyberte název aplikace. |
| client_secret |požadováno | Klíč zaregistrovaný pro volající službu ve službě Azure AD. Tato hodnota by se měla poznamenat v době registrace. |
| prostředek |požadováno | Identifikátor URI ID aplikace přijímající služby (zabezpečeného prostředku) Toto je prostředek, který bude cílovou skupinou tokenu SAML. Identifikátor URI ID aplikace v Azure Portal najdete tak, že vyberete **Active Directory** a zvolíte adresář. Vyberte název aplikace, zvolte **všechna nastavení**a pak vyberte **vlastnosti**. |
| requested_token_use |požadováno | Určuje, jak se má požadavek zpracovat. V toku musí být hodnota **on_behalf_of**. |
| requested_token_type | požadováno | Určuje typ požadovaného tokenu. Hodnota může být **urn: IETF: params: OAuth: token-Type: typu Saml2** nebo **urn: IETF: parametr: OAuth: token-Type: saml1** v závislosti na požadavcích na prostředek, který je k dispozici. |
...
| refresh_token |Token obnovení. Volající služba může tento token použít k vyžádání jiného přístupového tokenu po vypršení platnosti aktuálního kontrolního výrazu SAML. |
...
Veřejné klienty s adresami URL odpovědí se zástupnými znaky nemůžou pro OBO toky použít `id_token`. Důvěrného klienta však stále můžou uplatnit **přístupové** tokeny získané prostřednictvím implicitního toku udělení, i když má veřejný klient registrovaný identifikátor URI pro přesměrování zástupného znaku.
