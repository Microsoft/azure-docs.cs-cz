...
1. Pokud chcete najít uživatele v chybovém stavu v konkrétní skupině, otevřete podokno pro skupinu. V části **licence**se zobrazí oznámení, pokud je v chybovém stavu nějaký uživatel.
...
**Řešení** Pro jeden z produktů, které jsou ve skupině zadané, není k dispozici dostatek licencí. Musíte si buď koupit další licence na produkt, nebo uvolnit nepoužívané licence od jiných uživatelů nebo skupin.
...
**Řešení** Jeden z produktů uvedených ve skupině obsahuje plán služby, který je v konfliktu s jiným plánem služby, který je už přiřazený k uživateli přes jiný produkt. Některé plány služeb jsou nakonfigurovány způsobem, že je nelze přiřadit stejnému uživateli jako jiný související plán služby.
...
**Řešení** Jeden z produktů uvedených ve skupině obsahuje plán služby, který musí být povolen pro jiný plán služby, v jiném produktu, aby fungoval. K této chybě dochází, když se Azure AD pokusí odebrat základní plán služby. K tomu může dojít například při odebrání uživatele ze skupiny.
...
**Řešení** Některé služby společnosti Microsoft nejsou k dispozici ve všech umístěních z důvodu místních zákonů a předpisů. Než budete moct přiřadit licenci k uživateli, musíte zadat vlastnost **umístění využití** pro uživatele. Umístění můžete zadat v části**Nastavení** **profilu** >  **uživatele** > v Azure Portal.
...
Chcete-li tento problém vyřešit, odeberte uživatele z nepodporovaného umístění z licencované skupiny. Případně, pokud aktuální hodnoty umístění použití nezpůsobí skutečné umístění uživatele, můžete je upravit tak, aby byly licence správně přiřazeny (Pokud je nové umístění podporováno).
...
**Řešení** Když aktualizujete přiřazení licence pro uživatele nebo skupinu, může se stát, že se změní atribut Azure AD mail a ProxyAddresses některých uživatelů.
...
## <a name="what-happens-when-theres-more-than-one-product-license-on-a-group"></a>Co se stane, když skupina obsahuje více než jednu licenci na produkt?
...
## <a name="what-happens-when-a-group-with-licenses-assigned-is-deleted"></a>Co se stane, když se odstraní skupina s přiřazenými licencemi?
...
## <a name="how-do-you-manage-licenses-for-products-with-prerequisites"></a>Jak spravovat licence na produkty s požadavky?
...
Pokud se pokusíme tento produkt přiřadit vlastní skupině, portál vrátí chybu. Po výběru oznámení o chybách se zobrazí následující podrobnosti:

![Skupina, chybějící požadovaná součást](./media/licensing-groups-resolve-problems/group-prerequisite-required.png)

Když vybereme podrobnosti, zobrazí se tato chybová zpráva:

>Operace licence se nezdařila. Před přidáním nebo odebráním závislé služby se ujistěte, že skupina obsahuje potřebné služby. **Služba Microsoft Workplace Analytics vyžaduje, aby byla povolená taky Exchange Online (plán 2).**
...
Je také možné vytvořit samostatnou skupinu, která bude obsahovat pouze minimální požadované produkty k tomu, aby doplněk pracoval. Dá se použít k licencování jenom vybraných uživatelů pro doplněk. V tomto příkladu jsme přiřadili následující produkty ke stejné skupině:
- Office 365 Enterprise E3 s povoleným plánem služby Exchange Online (plán 2)
- Microsoft Workplace Analytics

![Skupina, včetně požadovaných součástí](./media/licensing-groups-resolve-problems/group-addon-with-prerequisite.png)
...
## <a name="how-do-you-force-license-processing-in-a-group-to-resolve-errors"></a>Jak vynutíte zpracování licencí ve skupině za účelem vyřešení chyb?
...
## <a name="how-do-you-force-license-processing-on-a-user-to-resolve-errors"></a>Jak vynutíte zpracování licencí pro uživatele při řešení chyb?
