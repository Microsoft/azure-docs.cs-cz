...
|Protocol     | TCP, UDP nebo všechny, což zahrnuje (ale bez omezení) TCP, UDP a ICMP. Samotný protokol ICMP nejde zadat, takže pokud potřebujete protokol ICMP, použijte možnost Všechny. |
...
Následující značky služeb jsou k dispozici pro použití v [pravidla skupin zabezpečení sítě](https://docs.microsoft.com/azure/virtual-network/security-overview#security-rules). Značky služeb hvězdičkou na konci (to znamená AzureCloud *) můžete také použít v [pravidla brány Firewall Azure sítě](https://docs.microsoft.com/azure/firewall/service-tags). 

* **Virtuální síť** (Resource Manager) (**VIRTUAL_NETWORK** v případě klasického modelu): Tato značka zahrnuje adresní prostor virtuální sítě (všechny rozsahy CIDR definované pro virtuální síť), všechny připojené místní adresní prostory [v partnerském vztahu](virtual-network-peering-overview.md) virtuální sítě nebo virtuální sítě připojené k [virtuální Brána sítě](../vpn-gateway/vpn-gateway-about-vpngateways.md?toc=%2fazure%2fvirtual-network%2ftoc.json) a použít u předpony adres [trasy definované uživatelem](virtual-networks-udr-overview.md). Mějte na paměti, že tato značka může obsahovat výchozí trasu. 
...
* **Úložiště*** (pouze Resource Manager): Tato značka označuje adresní prostor IP adres pro službu Azure Storage. Pokud jako hodnotu zadáte *Storage*, provoz směřující do úložiště se povolí nebo zakáže. Pokud chcete pouze povolit přístup k úložišti v konkrétní [oblasti](https://azure.microsoft.com/regions), můžete zadat tuto oblast v následujícím formátu úložiště. [ Název oblasti]. Značka představuje službu, ale ne konkrétní instance služby. Značka například představuje službu Azure Storage, ale ne konkrétní účet služby Azure Storage. Tato značka se doporučuje pro odchozí pravidlo zabezpečení. 
* **SQL*** (pouze Resource Manager): Tato značka označuje předpony adres, Azure SQL Database, Azure Database for MySQL – Azure Database for PostgreSQL a služby Azure SQL Data Warehouse. Pokud jako hodnotu zadáte *SQL*, provoz směřující do SQL se povolí nebo zakáže. Pokud chcete pouze pro povolení přístupu k Sql v konkrétní [oblasti](https://azure.microsoft.com/regions), můžete zadat tuto oblast v následujícím formátu Sql. [název oblasti]. Značka představuje službu, ale ne konkrétní instance služby. Značka například představuje službu Azure SQL Database, ale ne konkrétní server nebo databázi SQL. Tato značka se doporučuje pro odchozí pravidlo zabezpečení. 
...
* **Služby App Service*** (pouze Resource Manager): Tato značka označuje předpony adres služby Azure AppService. Pokud jako hodnotu zadáte *AppService*, provoz směřující do služby App Service se povolí nebo zakáže. Pokud chcete povolit přístup ke službě App Service pouze v konkrétní [oblasti](https://azure.microsoft.com/regions), můžete oblast zadat v následujícím formátu: AppService.[název_oblasti]. Tato značka se doporučuje pro odchozí pravidlo zabezpečení webové aplikace front-endů.  
* **AppServiceManagement*** (pouze Resource Manager): Tato značka označuje předpony adres provozu správy pro nasazení služby App Service Environment dedicated. Pokud jako hodnotu zadáte *AppServiceManagement*, provoz směřující do služby pro správu služby App Service se povolí nebo zakáže. Tato značka se doporučuje pro příchozí/odchozí pravidlo zabezpečení. 
* **ApiManagement*** (pouze Resource Manager): Tato značka označuje předpony adres provozu správy pro služby APIM vyhrazené nasazení. Pokud jako hodnotu zadáte *ApiManagement*, provoz směřující do služby API Management se povolí nebo zakáže. Tato značka se doporučuje pro příchozí/odchozí pravidlo zabezpečení. 
* **AzureConnectors*** (pouze Resource Manager): Tato značka označuje předpony adres konektory Logic Apps pro test paměti/back-end připojení. Pokud jako hodnotu zadáte *AzureConnectors*, provoz směřující do konektorů Azure se povolí nebo zakáže. Pokud chcete povolit přístup ke konektorům Azure pouze v konkrétní [oblasti](https://azure.microsoft.com/regions), můžete oblast zadat v následujícím formátu: AzureConnectors.[název_oblasti]. Tato značka se doporučuje pro příchozí pravidlo zabezpečení. 
* **GatewayManager** (pouze Resource Manager): Tato značka označuje předpony adres provozu správy pro vyhrazené nasazení brány sítě VPN nebo aplikace. Pokud jako hodnotu zadáte *GatewayManager*, provoz směřující do služby Správce brány se povolí nebo zakáže. Tato značka se doporučuje pro příchozí pravidlo zabezpečení. 
* **AzureDataLake*** (pouze Resource Manager): Tato značka označuje předpony adres služby Azure Data Lake. Pokud jako hodnotu zadáte *AzureDataLake*, provoz směřující do služby Azure Data Lake se povolí nebo zakáže. Tato značka se doporučuje pro odchozí pravidlo zabezpečení. 
* **AzureActiveDirectory*** (pouze Resource Manager): Tato značka označuje předpony adres AzureActiveDirectory služby. Pokud jako hodnotu zadáte *AzureActiveDirectory*, provoz směřující do služby Azure Active Directory se povolí nebo zakáže. Tato značka se doporučuje pro odchozí pravidlo zabezpečení.
* **AzureMonitor*** (pouze Resource Manager): Tato značka označuje předpony adres služby Log Analytics, App Insights, AzMon a vlastní metriky (4gigový koncových bodů). Pokud zadáte *AzureMonitor* pro hodnotu, je povolené nebo zakázané AzureMonitor přenosy. Tato značka pro Log Analytics má závislost **úložiště** značky. Tato značka se doporučuje pro odchozí pravidlo zabezpečení.
* **ServiceFabric*** (pouze Resource Manager): Tato značka označuje předpony adres služby ServiceFabric. Pokud zadáte *ServiceFabric* pro hodnotu, je povolené nebo zakázané ServiceFabric přenosy. Tato značka se doporučuje pro odchozí pravidlo zabezpečení. 
* **AzureMachineLearning*** (pouze Resource Manager): Tato značka označuje předpony adres AzureMachineLearning služby. Pokud zadáte *AzureMachineLearning* pro hodnotu, je povolené nebo zakázané AzureMachineLearning přenosy. Tato značka se doporučuje pro odchozí pravidlo zabezpečení. 
* **BatchNodeManagement*** (pouze Resource Manager): Tato značka označuje předpony adres provozu správy pro nasazení služby Azure Batch dedicated. Pokud zadáte *BatchNodeManagement* pro hodnotu provoz je povolený nebo zakázaný ze služby Batch do výpočetních uzlů. Tato značka se doporučuje pro příchozí/odchozí pravidlo zabezpečení. 
* **AzureBackup*** (pouze Resource Manager): Tato značka označuje předpony adres služby AzureBackup. Pokud zadáte *AzureBackup* pro hodnotu provoz je povolený nebo zakázaný na AzureBackup. Tato značka nemá závislost **úložiště** a **AzureActiveDirectory** značky. Tato značka se doporučuje pro odchozí pravidlo zabezpečení. 
* **AzureActiveDirectoryDomainServices*** (pouze Resource Manager): Tato značka označuje předpony adres provozu správy pro vyhrazené nasazení Azure Active Directory Domain Services. Pokud zadáte *AzureActiveDirectoryDomainServices* pro hodnotu, je povolené nebo zakázané AzureActiveDirectoryDomainServices přenosy. Tato značka se doporučuje pro příchozí/odchozí pravidlo zabezpečení.  
* **SqlManagement*** (pouze Resource Manager): Tato značka označuje předpony adres provozu správy SQL vyhrazené nasazení. Pokud zadáte *SqlManagement* pro hodnotu, je povolené nebo zakázané SqlManagement přenosy. Tato značka se doporučuje pro příchozí/odchozí pravidlo zabezpečení. 
* **CognitiveServicesManagement** (pouze Resource Manager): Tato značka označuje předpony adres provozu pro služby Cognitive Services. Pokud zadáte *CognitiveServicesManagement* pro hodnotu, je povolené nebo zakázané CognitiveServicesManagement přenosy. Tato značka se doporučuje pro odchozí pravidlo zabezpečení.  
* **Dynamics365ForMarketingEmail** (pouze Resource Manager): Tato značka označuje předpony adres marketingových e-mailové služby Dynamics 365. Pokud zadáte *Dynamics365ForMarketingEmail* pro hodnotu, je povolené nebo zakázané Dynamics365ForMarketingEmail přenosy. Pokud chcete pouze chcete povolit přístup k Dynamics365ForMarketingEmail v konkrétní [oblasti](https://azure.microsoft.com/regions), můžete zadat tuto oblast v následujícím formátu Dynamics365ForMarketingEmail. [ Název oblasti].
* **AzurePlatformDNS** (pouze Resource Manager): Tato značka označuje DNS, což je základní infrastruktury služby. Pokud zadáte *AzurePlatformDNS* pro hodnotu můžete zakázat výchozí [platformy Azure zvážení](https://docs.microsoft.com/azure/virtual-network/security-overview#azure-platform-considerations) pro službu DNS. Věnujte prosím opatrní při použití této značky. Testování se doporučuje před použitím této značky. 
* **AzurePlatformIMDS** (pouze Resource Manager): Tato značka označuje IMDS, což je základní infrastruktury služby. Pokud zadáte *AzurePlatformIMDS* pro hodnotu můžete zakázat výchozí [platformy Azure zvážení](https://docs.microsoft.com/azure/virtual-network/security-overview#azure-platform-considerations) pro IMDS. Věnujte prosím opatrní při použití této značky. Testování se doporučuje před použitím této značky. 
* **AzurePlatformLKM** (pouze Resource Manager): Tato značka označuje licencování Windows nebo službu správy klíčů. Pokud zadáte *AzurePlatformLKM* pro hodnotu můžete zakázat výchozí [platformy Azure zvážení](https://docs.microsoft.com/azure/virtual-network/security-overview#azure-platform-considerations) pro licencování. Věnujte prosím opatrní při použití této značky. Testování se doporučuje před použitím této značky. 
...
### <a name="service-tags-in-on-premises"></a>Značky služeb v místních  
Můžete stáhnout a integrovat s místní branou seznam značek služeb s předponou podrobnosti o následující týdenní publikace pro Azure [veřejné](https://www.microsoft.com/download/details.aspx?id=56519), [státní správy USA](https://www.microsoft.com/download/details.aspx?id=57063), [Čína ](https://www.microsoft.com/download/details.aspx?id=57062), a [Německo](https://www.microsoft.com/download/details.aspx?id=57064) cloudy.

Můžete také programově načítat pomocí informací **rozhraní API zjišťování značku služby** (Public Preview) – [REST](https://aka.ms/discoveryapi_rest), [prostředí Azure PowerShell](https://aka.ms/discoveryapi_powershell), a [ Azure CLI](https://aka.ms/discoveryapi_cli). 

> [!NOTE]
> Následující publikace týdenní (starší verze) pro Azure [veřejné](https://www.microsoft.com/en-us/download/details.aspx?id=41653), [Čína](https://www.microsoft.com/en-us/download/details.aspx?id=42064), a [Německo](https://www.microsoft.com/en-us/download/details.aspx?id=54770) cloudů se přestanou používat podle 30. června 2020. Spusťte prosím pomocí aktualizované publikací, jak je popsáno výše. 
...
|---|---|---|---|---|---|---|
|65000|VirtualNetwork|0-65535|VirtualNetwork|0-65535|Vše|Allow|

#### <a name="allowazureloadbalancerinbound"></a>AllowAzureLoadBalancerInBound

|Priorita|Source|Zdrojové porty|Cíl|Cílové porty|Protocol|Access|
|---|---|---|---|---|---|---|
|65001|AzureLoadBalancer|0-65535|0.0.0.0/0|0-65535|Vše|Allow|
...
|Priorita|Source|Zdrojové porty| Cíl | Cílové porty | Protocol | Access |
|---|---|---|---|---|---|---|
| 65001 | 0.0.0.0/0 | 0-65535 | Internet | 0-65535 | Vše | Allow |
...
Ve sloupcích **Zdroj** a **Cíl** jsou hodnoty *VirtualNetwork*, *AzureLoadBalancer* a *Internet* [značky služeb](#service-tags), a nikoli IP adresy. Hodnota **Všechny** ve sloupci Protokol zahrnuje protokoly TCP, UDP a ICMP. Při vytváření pravidla můžete zadat protokol TCP, UDP nebo Všechny, ale nemůžete zadat samotný protokol ICMP. Proto pokud vaše pravidlo vyžaduje protokol ICMP, vyberte jako protokol hodnotu *Všechny*. Hodnota *0.0.0.0/0* ve sloupcích **Zdroj** a **Cíl** představuje všechny adresy. Klienti, jako je Azure portal, rozhraní příkazového řádku Azure, nebo můžete použít Powershell * nebo pro tento výraz.
...
> [!NOTE]
> Skupiny zabezpečení sítě jsou přidružené k podsítím nebo virtuálním počítačům a cloudovým službám, které jsou nasazené v modelu nasazení classic a k podsítím nebo síťovým rozhraním v modelu nasazení Resource Manager. Další informace o modelech nasazení Azure najdete v článku [Vysvětlení modelů nasazení Azure](../azure-resource-manager/resource-manager-deployment-model.md?toc=%2fazure%2fvirtual-network%2ftoc.json).
