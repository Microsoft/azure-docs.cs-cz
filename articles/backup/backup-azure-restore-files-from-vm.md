...
5. Pokud si chcete stáhnout software, který se používá ke kopírování souborů z bodu obnovení, klikněte na **stáhnout spustitelný soubor** (pro virtuální počítač s Windows Azure) nebo **Stáhněte skript** (pro virtuální počítač se systémem Linux Azure se vygeneruje skript Pythonu).
    ...
    Chcete-li spustit spustitelný soubor nebo skript jako správce, je doporučeno uložit stahování do počítače.
...
> * Adresa URL by byla "https:\//pod01-REC2.wcus.Backup.windowsazure.com"
   ...
   Přístup k download.microsoft.com se vyžaduje ke stažení komponent používaných k vytvoření zabezpečeného kanálu mezi počítačem, na kterém se skript spouští, a daty v bodu obnovení.
...
V systému Linux se po navázání připojení k bodu obnovení neodstraní příslušné cesty připojení automaticky. Cesty pro připojení existují jako "osamocené" svazky a jsou viditelné, ale při přístupu k souborům nebo jejich zápisu vyvolají chybu. Je možné je odebrat ručně. Skript při spuštění identifikuje všechny takové svazky existující z předchozích bodů obnovení a vyčistí je na základě souhlasu.
...
K vypsání názvů skupin svazků v rámci fyzického svazku.
...
> [!Note]
> Zjistili jsme některé problémy s spuštěním skriptu pro obnovení souboru na počítačích s operačním systémem SLES 12 SP4. Zkoumání pomocí týmu SLES.
...
Tato část popisuje různé bezpečnostní opatření, která se provádí při implementaci obnovení souborů ze záloh virtuálních počítačů Azure, takže uživatelé znají aspekty zabezpečení této funkce.

### <a name="feature-flow"></a>Tok funkcí

Tato funkce byla sestavena pro přístup k datům virtuálních počítačů, aniž by bylo nutné obnovovat celý virtuální počítač nebo disky virtuálních počítačů a minimální postup. Přístup k datům virtuálních počítačů se poskytuje pomocí skriptu (který připojí svazek pro obnovení, jak je vidět níže), a proto vytvoří základ pro všechny implementace zabezpečení.
...
Vygenerovaný skript je podepsaný oficiálním certifikátem Microsoftu pro službu Azure Backup. Jakékoli manipulace se skriptem znamená, že signatura je poškozená a všechny pokusy o spuštění skriptu jsou v operačním systému zvýrazněné jako potenciální riziko.
...
Jakýkoli seznam Access Control souborů (ACL), který se nachází v nadřazeném nebo zálohovaném virtuálním počítači, se zachová také v připojeném systému souborů.

Skript poskytuje přístup jen pro čtení k bodu obnovení a je platný jenom 12 hodin. Pokud si uživatel přeje odebrat přístup dříve, přihlaste se k portálu Azure Portal, PowerShellu nebo rozhraní příkazového řádku a proveďte **odpojování disků** pro daný bod obnovení. Tento skript se okamžitě zruší.
