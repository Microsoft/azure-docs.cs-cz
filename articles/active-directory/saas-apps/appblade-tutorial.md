...
2. **[Nakonfigurujte jednotné přihlašování AppBlade](#configure-appblade-single-sign-on)** – ke konfiguraci nastavení jednotného přihlašování na straně aplikace.
...
Pokud má uživatel e-mailovou adresu končící doménou nakonfigurovanou AppBlade pro váš účet, pak se uživatel automaticky připojí k účtu jako člen s vámi zadanou úrovní oprávnění, což je jedna z "Basic" (základní uživatel, který může instalovat jenom aplikace). , "Člen týmu" (uživatel, který může nahrávat nové verze aplikací a spravovat projekty), nebo "správce" (oprávnění správce k účtu s oprávněními správce). Obvykle byste si měli vybrat základní a pak ručně zvýšit úroveň uživatelů prostřednictvím přihlašovacího jména správce (AppBlade musí nakonfigurovat přihlašovací jméno správce na základě e-mailu předem nebo povýšit uživatele jménem zákazníka po přihlášení).
