...
> [!NOTE]
> -   Funkce Azure Sentinel CMK je k dispozici pouze pro zákazníky, kteří jsou **noví** a mají přístup k této funkci, a to prostřednictvím registrace funkcí Azure. Můžete požádat o přístup kontaktováním azuresentinelCMK@microsoft.coma jakmile bude dostupná kapacita, schválí se žádosti, které čekají na vyřízení.
...
1.  [Zapnout možnosti obnovení:](../key-vault/key-vault-best-practices.md#turn-on-recovery-options)

    -   Ujistěte se, že je zapnuté [obnovitelné odstranění](../key-vault/key-vault-ovw-soft-delete.md) .

    -   Zapněte funkci [vyprázdnění ochrany](../key-vault/key-vault-ovw-soft-delete.md#purge-protection) pro ochranu proti vynucenému odstranění tajného nebo trezoru i po obnovitelném odstranění.
