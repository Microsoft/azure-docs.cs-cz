...
- [Uložení vlastních imagí a distribuce do více cvičení](image-factory-save-distribute-custom-images.md)
...
Než nakonfigurujete kroky čištění, definujte, kolik historických imagí si v DevTest Labs chcete zachovat. Když jste postupovali podle článku [spuštění objektu pro vytváření imagí z Azure DevOps](image-factory-set-up-devops-lab.md) , nakonfigurovali jste různé proměnné sestavení. Jedna z nich byla **ImageRetention**. Tuto proměnnou nastavíte na `1`, což znamená, že DevTest Labs nebude udržovat historii vlastních imagí. K dispozici budou pouze nejnovější distribuované obrázky. Pokud tuto proměnnou změníte na `2`, zachová se nejnovější distribuovaná Image plus předchozí. Tuto hodnotu můžete nastavit tak, aby definovala počet historických imagí, které chcete zachovat v DevTest Labs.
...
Teď máte spuštěnou továrnu imagí, která může na vyžádání generovat a distribuovat vlastní image do vašich cvičení. V tuto chvíli je to jen pro to, aby se vaše image správně nastavily a identifikovaly cílové cvičení. Jak je uvedeno v předchozím článku, soubor **Labs. JSON** umístěný ve složce **Konfigurace** určuje, které Image by se měly zpřístupnit v každé cílové laboratoři. Když do své organizace přidáte další DevTest Labs, stačí pro nové testovací prostředí přidat položku v Labs. JSON.
...
4. Vytvořte [samostatné sestavení/vydání](/azure/devops/pipelines/overview?view=azure-devops-2019) , ke kterému se má skript **DistributeImages** spustit samostatně. Tento skript můžete spustit, když provedete změny v testovacích prostředích Labs a nakopírujete obrázky zkopírované do cílové laboratoře bez nutnosti opětovného vytváření všech imagí.
