...
description: Příklad Prostředí PowerShell, který uvádí aplikace proxy aplikací Azure Active Directory (Azure AD) spolu s ID aplikace (AppId), názvem (DisplayName) a ID objektu (ObjId).
...
---
# <a name="get-all-application-proxy-apps-and-list-basic-information"></a>Získejte všechny aplikace Proxy aplikace a seznam základních informací

Tento příklad skriptu PowerShellu uvádí informace o všech aplikacích proxy aplikací Azure Active Directory (Azure AD), včetně ID aplikace (AppId), názvu (DisplayName) a ID objektu (ObjId).
