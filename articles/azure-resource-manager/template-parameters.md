...
V šabloně odkazujete na hodnotu parametru pomocí funkce [Parameters](resource-group-template-functions-deployment.md#parameters) . V následujícím příkladu se hodnota parametru používá k nastavení SKU pro účet úložiště.
...
V části Parameters (parametry) nemůžete použít [odkazovou](resource-group-template-functions-resource.md#reference) funkci ani žádnou z funkcí [seznamu](resource-group-template-functions-resource.md#list) . Tyto funkce získávají běhový stav prostředku a nelze jej provést před nasazením, když jsou parametry vyřešeny.
...
* Další informace o dostupných vlastnostech parametrů naleznete v tématu [pochopení struktury a syntaxe šablon Azure Resource Manager](resource-group-authoring-templates.md).
* Další informace o předávání hodnot parametrů jako souboru naleznete v tématu [Create správce prostředků Parameter File](resource-manager-parameter-files.md).
