...
Language Understanding (LUIS) poskytuje *předem sestavené domény, které jsou předem vybudovány*modely [záměrů](luis-how-to-add-intents.md) a [entit](luis-concept-entity-types.md) , které pracují společně pro domény nebo běžné kategorie klientských aplikací. 
...
Může se stát, že předem sestavená doména obsahuje záměr podobný záměru, který chcete mít v aplikaci LUIS, ale chcete, aby se chovala jinak. Například předem sestavená doména poskytuje `MakeReservation` záměr vytvořit rezervaci v rámci restaurace, ale chcete, aby aplikace použila tento záměr k použití rezervací hotelu. V takovém případě můžete chování tohoto záměru upravit přidáním příkladu projevy do záměru o provedení rezervací hotelu. Pak se aplikace znovu rozhlasuje. 
...
LUIS poskytuje předem sestavené záměry a jejich projevy z předem připravených domén. Záměry je možné přidat bez přidání celé domény. Přidání záměru je proces přidání záměru a jeho projevy do aplikace. Název záměru i seznam utterance lze upravit.  
