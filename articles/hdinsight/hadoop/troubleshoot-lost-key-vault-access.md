...
Pro clustery Bring Your Own Key (BYOK) se zobrazí výstraha centra Resource Health (RHC) `The HDInsight cluster is unable to access the key for BYOK encryption at rest`, ve které uzly clusteru ztratily přístup ke zákazníkům Key Vault (KV). Podobné výstrahy je také možné zobrazit v uživatelském rozhraní Apache Ambari.
...
Další podrobnosti najdete v [Azure Key Vault dostupnosti a redundanci](../../key-vault/key-vault-disaster-recovery-guidance.md) a na stránce stavu Azure https://status.azure.com/
