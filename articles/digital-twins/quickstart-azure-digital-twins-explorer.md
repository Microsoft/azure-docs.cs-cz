...
:::image type="content" source="media/quickstart-azure-digital-twins-explorer/graph-view-full.png" alt-text="Zobrazení grafu tvořeného čtyřmi kruhovými uzly, které jsou připojeny šipkami. Kruh označený jako ' Floor1 ' je připojen šipkou, která je označena ' Contains ' na kroužek označený ' Room1 '. Kruh označený jako ' Floor0 ' je připojen šipkou, která je označena ' Contains ' na kroužek označený ' Room0 '. ' Floor1 ' a ' Floor0 ' nejsou připojeny.":::
...
Nakonec také budete muset stáhnout ukázku, která se má použít při rychlém startu. Ukázková aplikace je **Průzkumník digitálních vláken Azure**. Tato ukázka obsahuje aplikaci, kterou používáte v rychlém startu k načtení a prozkoumání scénáře digitálních vláken Azure. Obsahuje také ukázkové soubory scénáře. Ukázku získáte tak, že přejdete na [Průzkumník digitálních vláken Azure](/samples/azure-samples/digital-twins-explorer/digital-twins-explorer/). Vyberte tlačítko Procházet kód pod nadpisem, které vás **přesměruje** do úložiště GitHub pro ukázku. Vyberte tlačítko **kód** a Stáhněte si soubor **zip** a stáhněte ukázku jako *. Soubor ZIP* . 

:::image type="content" source="media/quickstart-azure-digital-twins-explorer/download-repo-zip.png" alt-text="Snímek obrazovky s úložištěm Digital-zdvojené – Průzkumník na GitHubu Je vybráno tlačítko kód a vytvoří se malé dialogové okno, ve kterém je zvýrazněno tlačítko Stáhnout ZIP." lightbox="media/quickstart-azure-digital-twins-explorer/download-repo-zip.png":::

Rozbalte složku **digital-twins-explorer-main.zip** a extrahujte soubory.
...
1. Pomocí [tohoto odkazu na instalaci](/cli/azure/install-azure-cli) dokončete instalaci, která odpovídá vašemu operačnímu systému.
...
1. Přejdete do složky Downloaded a unzip **Digital-revlákened-Explorer-Main** .
Otevřete okno konzoly do umístění složky **Digital-zdvojené-Explorer-Main/Client/src**.
...
1. Spusťte aplikaci spuštěním `npm run start` .
   ...
   :::image type="content" source="media/quickstart-azure-digital-twins-explorer/explorer-blank.png" alt-text="Okno prohlížeče zobrazující aplikaci spuštěnou na localhost: 3000. Aplikace se nazývá Průzkumník digitálních vláken Azure a obsahuje pole pro Průzkumníka dotazů, zobrazení modelu, zobrazení grafu a Průzkumník vlastností. Ještě nejsou k dispozici žádná data na obrazovce." lightbox="media/quickstart-azure-digital-twins-explorer/explorer-blank.png":::
...
1. V pravém horním rohu okna vyberte tlačítko **Přihlásit** , jak je znázorněno na následujícím obrázku, abyste mohli nakonfigurovat Průzkumníka digitálních vláken Azure pro práci s instancí, kterou jste nastavili.
   ...
   :::image type="content" source="media/quickstart-azure-digital-twins-explorer/sign-in.png" alt-text="Průzkumník digitálních vláken Azure zvýrazňuje ikonu přihlášení v horní části okna. Ikona zobrazuje jednoduché Silhouette osoby překrývající Silhouette klíče." lightbox="media/quickstart-azure-digital-twins-explorer/sign-in.png":::

1. Zadejte adresu URL instance digitálních vláken Azure, kterou jste shromáždili dříve v části [Nastavení instance digitálních vláken Azure](#set-up-an-azure-digital-twins-instance) , ve formátu *https://<instance-název hostitele>*.
...
V dalším kroku naimportujete vzorový scénář a graf do Průzkumníka digitálních vláken Azure. Vzorový scénář je také umístěný ve složce **Digital-zdvojené – Průzkumník – Hlavní** složka, kterou jste si stáhli dříve.
...
1. V poli **zobrazení modelu** vyberte ikonu **nahrát model** .
   ...
   :::image type="content" source="media/quickstart-azure-digital-twins-explorer/upload-model.png" alt-text="V poli zobrazení modelu je zvýrazněna střední ikona. Zobrazuje šipku ukazující na Cloud." lightbox="media/quickstart-azure-digital-twins-explorer/upload-model.png":::
 
1. V zobrazeném poli pro výběr souboru přejdete do složky **Digital-zdvojené – Průzkumník – hlavní/klient/příklady** ve staženém úložišti.
...
>[!NOTE]
>Pokud se zobrazí následující chybová zpráva: :::image type="content" source="media/quickstart-azure-digital-twins-explorer/error-models-popup.png" alt-text="automaticky otevírané okno čte chybu: Chyba při načítání modelů: ClientAuthError: Chyba při otevírání překryvného okna. K tomu může dojít, pokud používáte Internet Explorer nebo pokud jsou automaticky otevíraná okna blokována v prohlížeči. tlačítko s tlačítkem Zavřít v dolní části." border="false"::: 
...
:::row:::
    :::column:::
        :::image type="content" source="media/quickstart-azure-digital-twins-explorer/model-info.png" alt-text="Zobrazení pole zobrazení modelu se dvěma definicemi modelů, které jsou uvedeny uvnitř, Floor (dtmi: priklad: Floor; 1) a místnost (dtmi: Příklad: místnost; 1). Pro každý model se zvýrazní ikona zobrazení informací o modelu ukazující písmeno &quot;i&quot; v kruhu." lightbox="media/quickstart-azure-digital-twins-explorer/model-info.png":::
    :::column-end:::
    :::column:::
    :::column-end:::
    :::column:::
...
1. V poli **zobrazení grafu** vyberte ikonu **Import grafu** .
   ...
   :::image type="content" source="media/quickstart-azure-digital-twins-explorer/import-graph.png" alt-text="V poli zobrazení grafu je zvýrazněna ikona. Zobrazuje šipku ukazující na Cloud." lightbox="media/quickstart-azure-digital-twins-explorer/import-graph.png":::

2. V poli pro výběr souboru přejdete do složky **Digital-zdvojené – Průzkumník – hlavní/klient/příklady** a vyberte **buildingScenario.xlsx** soubor tabulky. Tento soubor obsahuje popis ukázkového grafu. Vyberte **OK**.
...
3. Pokud chcete potvrdit nahrávání grafu, vyberte ikonu **Uložit** v pravém horním rohu pole **zobrazení grafu** .
   ...
   :::row:::
    :::column:::
        :::image type="content" source="media/quickstart-azure-digital-twins-explorer/graph-preview-save.png" alt-text="Zvýraznění ikony uložit v podokně Náhled grafu" lightbox="media/quickstart-azure-digital-twins-explorer/graph-preview-save.png":::
    :::column-end:::
    :::column:::
    :::column-end:::
   :::row-end:::
...
4. Průzkumník digitálních vláken Azure teď pomocí nahraného souboru vytvoří požadované zdvojení a vztahy mezi nimi. Po dokončení se zobrazí dialogové okno. Vyberte **Zavřít**.
   ...
   :::row:::
    :::column:::
        :::image type="content" source="media/quickstart-azure-digital-twins-explorer/import-success.png" alt-text="Dialogové okno s oznámením o úspěchu importu grafu Čte import úspěšně. 4 naimportovány zdvojené. 2 importované relace. '" lightbox="media/quickstart-azure-digital-twins-explorer/import-success.png":::
    :::column-end:::
    :::column:::
    :::column-end:::
    :::column:::
...
5. Graf se teď nahrál do Průzkumníka digitálních vláken Azure. Chcete-li zobrazit graf, vyberte tlačítko **Spustit dotaz** v poli **Průzkumník grafu** v horní části okna Průzkumníka digitálních vláken Azure.
   ...
   :::image type="content" source="media/quickstart-azure-digital-twins-explorer/run-query.png" alt-text="Zvýrazní se tlačítko spustit dotaz v pravém horním rohu okna." lightbox="media/quickstart-azure-digital-twins-explorer/run-query.png":::
...
:::image type="content" source="media/quickstart-azure-digital-twins-explorer/graph-view-full.png" alt-text="Zobrazení pole zobrazení grafu s dvojitým grafem v. Kruh označený jako ' floor1 ' je připojen šipkou, která je označena ' Contains ' na kroužek označený ' Room1 '. Kruh označený jako ' floor0 ' je připojen šipkou, která je označena ' Contains ' na kroužek označený ' room0 '.":::
...
:::row:::
    :::column:::
        :::image type="content" source="media/quickstart-azure-digital-twins-explorer/properties-room0.png" alt-text="Zvýrazněte kolem okna Průzkumník vlastností, které zobrazuje vlastnosti pro Room0, které zahrnují (mimo jiné) $dtId pole Room0, pole teploty 70 a pole vlhkosti 30." lightbox="media/quickstart-azure-digital-twins-explorer/properties-room0.png":::
    :::column-end:::
    :::column:::
    :::column-end:::
:::row-end:::
...
:::row:::
    :::column:::
        :::image type="content" source="media/quickstart-azure-digital-twins-explorer/properties-room1.png" alt-text="Zvýrazněte kolem okna Průzkumník vlastností, které zobrazuje vlastnosti pro Room1, které zahrnují (mimo jiné) $dtId pole Room1, pole teploty 80 a pole vlhkosti 60." lightbox="media/quickstart-azure-digital-twins-explorer/properties-room1.png":::
    :::column-end:::
    :::column:::
    :::column-end:::
:::row-end:::
...
:::image type="content" source="media/quickstart-azure-digital-twins-explorer/result-query-property-before.png" alt-text="Výsledky dotazu vlastnosti zobrazující pouze Room1." lightbox="media/quickstart-azure-digital-twins-explorer/result-query-property-before.png":::
...
:::row:::
    :::column:::
        :::image type="content" source="media/quickstart-azure-digital-twins-explorer/new-properties-room0.png" alt-text="Okno Průzkumník vlastností zobrazující vlastnosti pro Room0. Hodnota teploty je upravitelné pole zobrazující 76 a zvýrazní se ikona Uložit." lightbox="media/quickstart-azure-digital-twins-explorer/new-properties-room0.png":::
    :::column-end:::
    :::column:::
    :::column-end:::
:::row-end:::
...
:::image type="content" source="media/quickstart-azure-digital-twins-explorer/result-query-property-after.png" alt-text="Výsledky dotazu vlastnosti zobrazující Room0 i Room1." lightbox="media/quickstart-azure-digital-twins-explorer/result-query-property-after.png":::
