...
## <a name="examples"></a>Příklady
  ...
  Následující příklad ukazuje, jak se `ToString` chová napříč různými typy.   
...
```    
 Následující příklad ukazuje, jak lze `ToString` použít s jinými řetězcovými funkcemi jako `CONCAT`.   
...
```
Následující příklad ukazuje, jak lze `ToString` použít s jinými řetězcovými funkcemi jako `REPLACE`.   
```sql
SELECT 
    n.id AS nutrientID,
