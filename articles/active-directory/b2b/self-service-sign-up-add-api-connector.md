---
title: Přidání konektoru API do toku uživatele
...
Vlastní atributy lze pro uživatele vytvořit pomocí formátu **extension_ \<aad-extensions-app-id> _ \<AttributeName> ** . Rozhraní API by mělo očekávat deklarace identity v tomto stejném serializovaném formátu. Vaše rozhraní API může vracet deklarace ve stejném formátu, ale bez `<aad-extensions-app-id>` . <!--TODO: update once fix is rolled out--> Další informace o vlastních atributech najdete v tématu [definování vlastních atributů pro vlastní toky podepisování](user-flow-add-custom-attributes.md).
...
- Přečtěte si, [jak by mělo vaše rozhraní API reagovat](self-service-sign-up-add-api-connector.md#expected-response-types-from-the-web-api)
- Zjistěte [, kde můžete povolit konektor API](api-connectors-overview.md#where-you-can-enable-an-api-connector-in-a-user-flow) .
