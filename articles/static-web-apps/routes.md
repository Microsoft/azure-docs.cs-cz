...
ms.locfileid: "107313636"
---
# <a name="routes-in-azure-static-web-apps-preview"></a>Trasy ve službě Azure static Web Apps Preview

> [!IMPORTANT]
> Funkce definované v _routes.jsv_ souboru jsou teď zastaralé a v [konfiguračním souboru](./configuration.md#routes)Azure static Web Apps se implementují lépe.

Směrování v Azure static Web Apps definuje pravidla směrování back-endu a autorizační chování pro statický obsah a rozhraní API<sup>1</sup>. Pravidla jsou definována jako pole pravidel v _staticwebapp.config.jsv_ souboru.

- _staticwebapp.config.js_ souboru musí existovat v kořenu složky artefaktů sestavení aplikace.
...
- Role jsou definované v _staticwebapp.config.jspro_ soubor a uživatelé jsou k rolím přidruženi prostřednictvím [pozvánk](authentication-authorization.md).
...
_staticwebapp.config.js_ souboru musí existovat v kořenu složky artefaktů sestavení aplikace. Pokud vaše webová aplikace zahrnuje krok sestavení, který kopíruje sestavené soubory z konkrétní složky do vaší složky artefaktů sestavení, musí _staticwebapp.config.jsv_ souboru existovat v této konkrétní složce.

Následující tabulka uvádí vhodné umístění pro vložení _staticwebapp.config.js_ do souboru pro několik rozhraní front-end a knihoven.
...
Trasy jsou definovány v _staticwebapp.config.jsv_ souboru jako pole pravidel směrování pro danou `routes` vlastnost. Každé pravidlo se skládá ze vzoru směrování spolu s jednou nebo více volitelnými vlastnostmi pravidla. Příklady použití najdete v [ukázkovém souboru směrování](#example-route-file) .
...
Uživatelé mohou narazit na řadu různých situací, které mohou mít za následek chybu. Pomocí `platformErrorOverrides` pole můžete v reakci na tyto chyby poskytnout vlastní prostředí. Chcete-li umístit pole do _staticwebapp.config.js_ souboru, přečtěte si [ukázkový soubor směrování](#example-route-file) .
...
- Hlavičky definované v _staticwebapp.config.js_ platí jenom pro statický obsah. V kódu funkce můžete přizpůsobit hlavičky odpovědí koncového bodu rozhraní API.
...
Následující příklad ukazuje, jak vytvořit pravidla směrování pro statický obsah a rozhraní API v _staticwebapp.config.js_ v souboru. Některé trasy používají [systémovou složku _/.auth_](authentication-authorization.md) , která přistupuje k koncovým bodům souvisejícím s ověřováním.
...
| _/customers/contoso_               | Ověřeným uživatelům, kteří patří do rolí _správců_ nebo _customers_contoso_ , je/Customers/contoso/soubor <sup>2</sup>. _1 index.html_ . Ověřeným uživatelům, kteří nejsou v rolích _Administrators_ nebo _customers_contoso_ , se doplní chyba 401. Neověření uživatelé přesměrováni na _/Login_. |
...
- _staticwebapp.config.jsv_ souboru nemůže být větší než 100 kB.
- _staticwebapp.config.jsv_ souboru podporuje maximálně 50 jedinečných rolí.
