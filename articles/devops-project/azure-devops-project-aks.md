---
title: Nasazení aplikace ASP.NET Core do služby Azure Kubernetes Service (AKS) pomocí služby Azure DevOps Project | Kurz VSTS
description: DevOps Project usnadňuje začátek práce v Azure. Projekt Azure DevOps umožňuje snadno a v několika rychlých krocích nasadit aplikaci ASP.NET Core s využitím služby Azure Kubernetes Service (AKS).
ms.author: mlearned
ms.manager: douge
ms.prod: devops
...
ms.locfileid: "37967330"
---
# <a name="tutorial--deploy-your-aspnet-core-app-to-azure-kubernetes-service-aks-with-the-azure-devops-project"></a>Kurz: Nasazení aplikace ASP.NET Core do služby Azure Kubernetes Service (AKS) pomocí služby Azure DevOps Project
...
Azure DevOps Project vytvoří kanál CI/CD ve VSTS.  Můžete vytvořit **nový účet VSTS** nebo použít **existující účet**.  Azure DevOps Project také vytvoří **prostředky Azure**, jako je cluster AKS, v **předplatném Azure** podle vašeho výběru.
...
1. Dokončení procesu bude trvat několik minut.  Ukázková aplikace ASP.NET Core se nastaví v úložišti ve vašem účtu VSTS, vytvoří se cluster AKS, spustí se sestavení a vydání a vaše aplikace se nasadí do Azure.  
    ...
    Tento řídicí panel poskytuje přehled o vašem **úložišti kódu** VSTS, **kanálu CI/CD VSTS** a **clusteru AKS**.  Ve VSTS můžete dále nakonfigurovat další možnosti.  Na pravé straně řídicího panelu vyberte **Procházet** a zobrazte vaši spuštěnou aplikaci.
...
1. Na pravé straně řídicího panelu projektu DevOps vyberte **službu Kubernetes**.
...
1. Napravo od ikony **Zahodit** vyberte **Trigger průběžného nasazování** (**ikona** blesku).  Tato definice verze má povolený trigger CD.  Tento trigger vytvoří nasazení pokaždé, když bude k dispozici nový artefakt sestavení.  Volitelně můžete trigger zakázat, aby pak vaše nasazení vyžadovala ruční spuštění. 
...
> [Používání řídicího panelu Kubernetes](https://docs.microsoft.com/vsts/pipelines/release/define-multistage-release-process?view=vsts)
