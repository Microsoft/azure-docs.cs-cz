...
Schéma pro protokoly diagnostiky prostředků se liší v závislosti na typu prostředku, který je definován `resourceId` vlastností) `category` a vlastnostech. V tomto seznamu jsou uvedeny všechny služby Azure, které podporují protokoly prostředků, s odkazy na schéma specifické pro službu a kategorii, kde je k dispozici.
