...
Azure Site Recovery aktivuje událost, pokud je frekvence změny dat na zdrojovém virtuálním počítači vyšší než podporované limity. Pokud chcete zjistit, jestli je problém z důvodu vysoké četnosti změn, přečtěte si části **replikované položky** > **VM** > **události – posledních 72 hodin**.
...
V Azure Site Recovery platí omezení četnosti změn dat v závislosti na typu disku. Pokud chcete zjistit, jestli je tento problém opakovaný nebo chvilku, najděte rychlost změny dat ovlivněného virtuálního počítače. Přejít na zdrojový virtuální počítač, najít metriky v části **monitorování**a přidat metriky, jak je znázorněno na tomto snímku obrazovky:
...
* **Vylučte disk, který způsobuje vysokou frekvenci změn dat**: disk můžete vyloučit pomocí [PowerShellu](./azure-to-azure-exclude-disks.md). Pokud chcete vyloučit disk, musíte nejdřív zakázat replikaci.
...
**Oprava** : Azure Site Recovery nemůže vytvořit bod obnovení konzistentní vzhledem k aplikacím pro konfiguraci prostorů úložiště s přímým přístupem. Pro správné [nakonfigurování zásad replikace](https://docs.microsoft.com/azure/site-recovery/azure-to-azure-how-to-enable-replication-s2d-vms) prosím přečtěte článek.
