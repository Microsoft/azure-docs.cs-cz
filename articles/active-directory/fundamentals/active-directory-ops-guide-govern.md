...
Pokud ve vaší organizaci neexistují žádné procesy pro správu privilegovaných účtů nebo máte v současnosti správci, kteří používají své běžné uživatelské účty ke správě služeb a prostředků, měli byste hned začít používat samostatné účty, například jeden pro běžný den v den. soutěž druhý pro privilegovaný přístup a nakonfigurovaný s MFA. V případě, že vaše organizace má Azure AD Premium předplatné P2, je ještě lepší, měli byste hned nasadit [Azure AD Privileged Identity Management](https://docs.microsoft.com/azure/active-directory/privileged-identity-management/pim-configure#license-requirements) (PIM). Ve stejném tokenu byste si měli projít také tyto privilegované účty a v případě potřeby [přiřadit méně privilegované role](https://docs.microsoft.com/azure/active-directory/users-groups-roles/directory-admin-roles-secure) .
