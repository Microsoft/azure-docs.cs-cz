...
ms.locfileid: "73467893"
---
# <a name="perform-sentiment-analysis-with-azure-stream-analytics-and-azure-machine-learning-studio-classic-preview"></a>Provedení analýzy mínění pomocí Azure Stream Analytics a Azure Machine Learning Studio (Classic) (Preview)
...
2. Zadejte název (`samldemo` v příkladu). Název může obsahovat jenom malá písmena a číslice a musí být jedinečný v rámci Azure. 
...
4. V Azure Portal vyberte účet úložiště. V okně účet úložiště klikněte na **kontejnery** a potom kliknutím na **+&nbsp;kontejner** vytvořte úložiště objektů BLOB.
...
2. Klikněte na **vytvořit prostředek** > **Internet věcí** > **úlohu Stream Analytics**. 
...
2. V okně **vstupy** klikněte na **Přidat vstup streamu** >**úložiště objektů BLOB** .
...
3. Vyplňte okno **BLOB Storage** s těmito hodnotami:
   ...
   |**Vnitřního**  | Vyberte kontejner, který jste vytvořili dříve (`azuresamldemoblob`).        |
...
2. V okně **výstupy** klikněte na **Přidat** >**BLOB Storage**a pak přidejte výstup s aliasem `datamloutput`. 
...
Dříve jste publikovali Machine Learning model pro webovou službu. Pokud se v tomto scénáři spustí úloha analýzy streamu, pošle Každá ukázka z vstupu do webové služby pro analýzu mínění. Webová služba Machine Learning vrací mínění (`positive`, `neutral`nebo `negative`) a pravděpodobnost, že se na něj klade pozitivní. 
...
2. Přejděte do okna úlohy > **functions** >  **+ Přidat** > **AzureML** .

3. Vyplňte okno **Azure Machine Learning funkce** s těmito hodnotami:

   |Pole  |Hodnota  |
   |---------|---------|
   | **Alias funkce** | Použijte název `sentiment` a vyberte možnost **zadat Azure Machine Learning nastavení funkce ručně** , která vám umožní zadat adresu URL a klíč.      |
...
2. Pokud máte nástroj, který běžně používáte k prohlédnutí obsahu úložiště objektů blob, použijte tento nástroj k zkontrolování kontejneru `azuresamldemoblob`. Případně proveďte následující kroky v Azure Portal:

    1. Na portálu Najděte `samldemo` účet úložiště a v rámci účtu Najděte kontejner `azuresamldemoblob`. V kontejneru vidíte dva soubory: soubor obsahující ukázkovou tweety a soubor CSV vygenerovaný úlohou Stream Analytics.
