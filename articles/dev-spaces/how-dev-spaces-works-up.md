...
1. Pomocí *předávaného portu* poskytne přístup ke koncovému bodu aplikace http://localhostpomocí.
...
Soubory a adresáře, ve kterých budou synchronizovány změny, jsou uvedeny ve vlastnosti *configurations. vývoj. Container. Sync* . Tyto adresáře jsou při spuštění `up` příkazu synchronizovány na začátku a také při zjištění změn. Pokud máte další nebo různé adresáře, které byste chtěli synchronizovat do vývojového prostoru, můžete tuto vlastnost změnit.
...
Další informace o použití Azure Dev Spaces pro rychlé iterace a vývoj najdete v tématu [Jak připojit vývojový počítač k vašemu][how-it-works-connect] vývojovému prostoru a [jak vzdálené ladění kódu pomocí Azure dev Spaces funguje][how-it-works-remote-debugging].
