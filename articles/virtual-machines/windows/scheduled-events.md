...
description: Naplánované události, které využívají Azure Metadata Service pro virtuální počítače s Windows.
...
> [!NOTE]
> Scheduled Events pro všechny virtuální počítače v tenantovi řadiče prostředků infrastruktury (FC) se doručují na všechny virtuální počítače v tenantovi FC. Tenant FC se rovná samostatnému virtuálnímu počítači, celé cloudové službě, celé sadě dostupnosti a skupině umístění pro sadu VM Scale set (VMSS) bez ohledu na využití zóny dostupnosti. 
...
| Zdroje informací| Seznam prostředků, které tato událost ovlivňuje V seznamu je zaručeno, že bude obsahovat počítače z jedné [aktualizační domény](../manage-availability.md), ale nemusí obsahovat všechny počítače v ud. <br><br> Příklad: <br><ul><li> ["FrontEnd_IN_0", "BackEnd_IN_0"] |
...
- Přečtěte si o [plánované údržbě virtuálních počítačů s Windows v Azure](../maintenance-and-updates.md?bc=/azure/virtual-machines/windows/breadcrumb/toc.json&toc=/azure/virtual-machines/windows/toc.json).
