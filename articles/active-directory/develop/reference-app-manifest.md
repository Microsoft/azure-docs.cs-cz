...
Manifest aplikace obsahuje více atributů, které jsou označovány jako kolekce například approles keycredentials, knownClientApplications, identifierUris, rediretUris, requiredResourceAccess, oauth2Permissions atd. V manifestu aplikace dokončena a pro každou aplikaci byl celkový počet položek ve všech kolekcích kombinovat limitována 1200. Pokud už máte 100 redirect URI zadaný v manifestu aplikace, pak jsou jen vlevo se zbývajícími 1100 položky přes všechny další kolekce kombinovat, které tvoří manifestu.

> [!NOTE]
> V případě pokusu o přidání více než 1200 položky v manifestu aplikace. Může se zobrazit chyba **"nepovedlo se aktualizovat xxxxxx aplikace. Podrobnosti o chybě: Velikost manifestu překročily limit. Snížit počet hodnot a opakujte žádost.** "
