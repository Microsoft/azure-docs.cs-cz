...
> [!NOTE]
> V jazyce JavaScript `customStatus` bude pole nastaveno při `yield` `return` Plánování akce další nebo.
...
> [!NOTE]
> V Pythonu se `custom_status` pole nastaví při `yield` `return` plánování další akce nebo.
