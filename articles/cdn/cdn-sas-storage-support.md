...
Další informace o nastavení parametrů najdete v tématu [informace o parametrech SAS](#sas-parameter-considerations) a [parametry signatury sdíleného přístupu](https://docs.microsoft.com/azure/storage/common/storage-sas-overview#how-a-shared-access-signature-works).
...
1. Pomocí [modulu pravidel](cdn-rules-engine.md) vytvořte pravidlo pro přepsání adresy URL. Rozšiřování nových pravidel trvá až 4 hodiny.
...
### <a name="option-3-using-cdn-security-token-authentication-with-a-rewrite-rule"></a>Možnost 3: Použití ověřování pomocí tokenu zabezpečení CDN s pravidlem pro přepsání
...
1. [Vytvořte Azure CDN token zabezpečení](https://docs.microsoft.com/azure/cdn/cdn-token-auth#setting-up-token-authentication) a aktivujte ho pomocí modulu pravidel pro koncový bod CDN a cestu, kde mají vaši uživatelé k souboru přístup.
...
2. Pomocí [modulu pravidel](cdn-rules-engine.md) vytvořte pravidlo pro přepsání adresy URL, které povolí přístup k tokenům SAS všem objektům blob v kontejneru. Rozšiřování nových pravidel trvá až 4 hodiny.
...
| Povolené IP adresy | Volitelný parametr. Pokud používáte **Azure CDN z Verizon**, můžete tento parametr nastavit na rozsahy definované v [Azure CDN z rozsahů IP adres serveru Verizon Edge](/azure/cdn/cdn-pop-list-api). Pokud používáte **Azure CDN z Akamai**, nemůžete nastavit parametr rozsahy IP adres, protože IP adresy nejsou statické.|
...
- [Signatury sdíleného přístupu, část 2: Vytvoření a použití SAS s úložištěm objektů BLOB](https://docs.microsoft.com/azure/storage/blobs/storage-dotnet-shared-access-signature-part-2)

Další informace o nastavení ověřování tokenu najdete v tématu [zabezpečení prostředků Azure Content Delivery Network s ověřováním tokenu](https://docs.microsoft.com/azure/cdn/cdn-token-auth).
