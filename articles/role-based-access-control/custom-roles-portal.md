...
1. Chcete-li odebrat oprávnění, klikněte na ikonu Odstranit na konci řádku. Vzhledem k tomu, že uživatel nebude potřebovat možnost vytvářet lístky podpory, může být v tomto příkladu možné odstranit `Microsoft.Support/*` oprávnění.
...
Pokud má vaše role zástupné oprávnění (\*) a chcete vyloučit nebo odečíst konkrétní oprávnění od tohoto zástupného znaku, můžete je vyloučit. Řekněme například, že máte následující oprávnění zástupných znaků:
...
Pokud oprávnění vyloučíte, přidá se jako `NotActions` nebo `NotDataActions`. Skutečná oprávnění pro správu jsou vypočítána přidáním všech `Actions` a následným odečtením všech `NotActions`. Platná oprávnění k datům jsou vypočítána přidáním všech `DataActions` a následným odečtením všech `NotDataActions`.
...
Na kartě **JSON** uvidíte vlastní roli formátovanou ve formátu JSON. Pokud chcete, můžete JSON přímo upravit. Pokud chcete přidat oprávnění zástupného znaku (\*), musíte použít tuto kartu.
