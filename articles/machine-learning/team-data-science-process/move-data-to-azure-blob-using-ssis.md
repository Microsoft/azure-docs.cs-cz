...
Jakmile si zákazníci přesunou místní data do cloudu, budou k nim mít přístup z libovolné služby Azure, aby využili plnou sílu sady technologií Azure. Dá se použít například v Azure Machine Learning nebo v clusteru HDInsight.

Obvykle se jedná o první krok v návodech k [SQL](sql-walkthrough.md) a [HDInsight](hive-walkthrough.md) .
...
K provedení úkolů popsaných v tomto článku musíte mít nastavené předplatné Azure a účet úložiště Azure. Aby bylo možné nahrávat nebo stahovat data, musíte znát název svého účtu služby Azure Storage a klíč účtu.
...
* **Balíček funkcí integračních služeb Microsoft SQL Server 2014 nebo 2016 pro Azure**: můžete je stáhnout ze stránky [SQL Server 2014 integrační služby](https://www.microsoft.com/download/details.aspx?id=47366) a [SQL Server služby 2016 Integration Services](https://www.microsoft.com/download/details.aspx?id=49492) .
...
| Pole | Popis |
| --- | --- |
| **AzureStorageConnection** |Určuje existujícího Správce připojení Azure Storage nebo vytvoří nový, který odkazuje na účet služby Azure Storage, který odkazuje na místo, kde jsou hostované soubory objektů BLOB. |
| **BlobContainer** |Určuje název kontejneru objektů blob, ve kterém jsou nahrané soubory uloženy jako objekty blob. |
