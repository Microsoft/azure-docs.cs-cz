...
|Podskupina|Popis|
| --- | --- |
| [schedule](service-fabric-sfctl-chaos-schedule.md) | Získejte a nastavte plán chaos. |
...
| --max-cluster-stabilization | Maximální doba, po kterou se má čekat, až budou všechny entity clusteru stabilní a v dobrém stavu.  Výchozí\: 60. <br><br> Chaos se spustí v iteracích a na začátku každé iterace ověří stav entit clusteru. Při ověřování, pokud entita clusteru není stabilní a v pořádku v rámci MaxClusterStabilizationTimeoutInSeconds, chaos vygeneruje událost neúspěšného ověření. |
