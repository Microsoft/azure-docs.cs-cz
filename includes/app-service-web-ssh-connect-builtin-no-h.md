---
title: zahrnout soubor
description: zahrnout soubor
services: app-service
author: cephalin
ms.service: app-service
ms.topic: include
ms.date: 03/29/2019
ms.author: cephalin
ms.custom: include file
ms.openlocfilehash: 7b82fa97d4f68d33335360b2a9b04a94dfea2646
ms.sourcegitcommit: eb6bef1274b9e6390c7a77ff69bf6a3b94e827fc
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 10/05/2020
ms.locfileid: "88078593"
---
[!INCLUDE [Open SSH session in browser](app-service-web-ssh-connect-no-h.md)]

> [!NOTE]
> Všechny změny provedené mimo adresář */home* se uloží ve vlastním kontejneru a po restartování aplikace se neuchovají.
>

Pokud chcete otevřít vzdálenou relaci SSH z místního počítače, projděte si téma věnované [otevření relace SSH ze vzdáleného prostředí](../articles/app-service/configure-linux-open-ssh-session.md#open-ssh-session-from-remote-shell).