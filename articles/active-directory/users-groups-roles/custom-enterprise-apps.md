---
title: Oprávnění vlastní role pro přiřazení přístupu k podnikové aplikaci – Azure Active Directory | Microsoft Docs
...
Další podrobnosti najdete v tématu [Vytvoření a přiřazení vlastní role](roles-create-custom.md) a [přiřazení vlastních rolí k oboru prostředků pomocí PowerShellu](roles-assign-powershell.md).
...
Pomocí poskytnutého příkladu v rozhraní Microsoft Graph API vytvořte vlastní roli. Další podrobnosti najdete v tématu [Vytvoření a přiřazení vlastní role](roles-create-custom.md) a [přiřazení vlastních rolí správce pomocí rozhraní Microsoft Graph API](roles-assign-graph.md).
...
Přiřazení role kombinuje ID objektu zabezpečení (může to být uživatel nebo instanční objekt), ID definice role a obor prostředku služby Azure AD. Další informace o prvcích přiřazení role najdete v tématu [Přehled vlastních rolí](roles-custom-overview.md) .
