...
1. Zkopírujte následující text a uložte ho jako ***devicecert.ps1***.
   ...
   Write-Host "$Message"
   ```
1. Zkopírujte následující text a uložte ho jako ***VPNProfile.xml** _ ve stejné složce jako _ * devicecert.ps1 * *. Upravte následující text tak, aby odpovídal vašemu prostředí.
