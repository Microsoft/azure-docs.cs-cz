...
| --- | --- | --- | --- | --- |
| ed0e4960-d9c5-11e6-85dc-d7996816aad3 | počítač | Microsoft.Compute/virtualMachines | 827f8aaa-ab72-437c-ba48-d8917a7336a3 | 1/13/2017 11:24:37 AM |
...
Předpokládejme, že ukázková datová sada se používá jako výstup aktivity kopírování, aktivita kopírování komprimuje výstupní data pomocí kodeku GZIP pomocí optimálního poměru a pak zapisuje komprimovaná data do souboru s názvem pagecounts. csv. gz v Azure Blob Storage.
...
Když v JSON vstupní datové sady zadáte vlastnost `compression`, kanál může číst komprimovaná data ze zdroje; a když zadáte vlastnost ve formátu JSON výstupní datové sady, aktivita kopírování může do cíle zapisovat komprimovaná data. Tady je několik ukázkových scénářů:

* Čtení GZIP komprimovaná data z objektu blob Azure by jej dekomprimovat a zápis Výsledná data do služby Azure SQL database. Můžete definovat vstupní datovou sadu Azure BLOB pomocí vlastnosti `compression` `type` JSON jako GZIP.
* Čtení dat ze souboru prostého textu v místním systému souborů, je ve formátu GZip komprimovat a zapíše komprimovaných dat do objektu blob Azure. Nadefinujete výstupní datovou sadu Azure Blob s vlastností `compression` `type` JSON jako GZip.
* Přečtěte si soubor. zip ze serveru FTP, dekomprimujte ho, aby se soubory dostaly do všech souborů, a Azure Data Lake Store. Nadefinujete vstupní datovou sadu FTP s vlastností `compression` `type` JSON jako ZipDeflate.
* Čtení GZIP komprimovaných dat z objektu blob Azure, by jej dekomprimovat, je pomocí BZIP2 komprimovat a zápis Výsledná data do objektu blob Azure. V tomto případě definujete vstupní datovou sadu Azure Blob s `compression` `type` nastavenou na GZIP a výstupní datovou sadu s `compression` `type` nastavenou na BZIP2.   
