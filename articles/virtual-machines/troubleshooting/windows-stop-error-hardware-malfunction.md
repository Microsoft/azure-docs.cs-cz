---
title: Chyba zastavení Windows – selhání hardwaru
...
3. Klíč registru NMI nastavte pomocí následujícího příkazu:
    ...
    ```
    [Zobrazit další informace o příkazu REG ADD](/windows-server/administration/windows-commands/reg-add)
...
5. *(Volitelné)* Nastavit přístup ke konzole sériového portu:
    ...
    ```
    [Zobrazit další informace o příkazu BCDEDIT](/windows-server/administration/windows-commands/bcdedit)
6. Restartujte virtuální počítač pomocí následujícího příkazu:

    ```
    SHUTDOWN /r /t 0 /f 
    ```
    [Zobrazit další informace o příkazu pro vypnutí](/windows-server/administration/windows-commands/shutdown)
