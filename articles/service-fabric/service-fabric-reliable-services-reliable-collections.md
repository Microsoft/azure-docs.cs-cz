...
* Žádné výstupní parametry: Používá `ConditionalValue<T>` vrátit hodnotu a hodnotu místo výstupní parametry. `ConditionalValue<T>` je třeba `Nullable<T>` ale nevyžaduje T být struktura.
