---
title: Zabezpečení a ochrana osobních údajů – Azure Search
description: Azure Search je kompatibilní s SOC 2, HIPAA a dalšími certifikacemi. Připojení a šifrování dat, ověřování a přístup k identitám prostřednictvím identifikátorů zabezpečení uživatelů a skupin v Azure Search filtrů.
author: HeidiSteen
manager: nitinme
services: search
ms.service: search
...
Komplexní funkce zabezpečení a řízení přístupu jsou integrované do Azure Search, aby se zajistilo, že soukromý obsah zůstane v tomto případě. Tento článek obsahuje výčet funkcí zabezpečení a dodržování standardů integrovaných v Azure Search.

Azure Search Architektura zabezpečení zahrnuje fyzické zabezpečení, šifrované přenosy, šifrované úložiště a dodržování standardů pro celou platformu. Operace Azure Search akceptuje pouze ověřené požadavky. Volitelně můžete přidat řízení přístupu pro jednotlivé uživatele k obsahu prostřednictvím filtrů zabezpečení. Tento článek se dotýká zabezpečení na jednotlivých vrstvách, ale primárně se zaměřuje na to, jak jsou data a operace zabezpečené v Azure Search.

## <a name="standards-compliance-iso-27001-soc-2-hipaa"></a>Dodržování standardů: ISO 27001, SOC 2, HIPAA

Azure Search je certifikováno pro následující standardy, jak je uvedeno [v červnu 2018](https://azure.microsoft.com/blog/azure-search-is-now-certified-for-several-levels-of-compliance/):
...
Šifrování se nachází v celém kanálu indexování: z připojení, prostřednictvím přenosu a dolů k indexovaným datům uloženým v Azure Search.
...
K dispozici je několik mechanismů zabezpečení, které jsou dostupné pro Azure, a tak automaticky dostupné pro Azure Search prostředky, které vytvoříte.
...
I když Azure Search dědí zabezpečení platformy Azure, poskytuje také vlastní ověřování založené na klíčích. Klíč rozhraní API je řetězec tvořený náhodně generovanými čísly a písmeny. Typ klíče (správce nebo dotaz) určuje úroveň přístupu. Odeslání platného klíče se považuje za důkaz, že požadavek pochází z důvěryhodné entity. 
...
V Azure Search není jednotlivý index zabezpečitelným objektem. Místo toho je přístup k indexu určen na úrovni služby (přístup pro čtení nebo zápis) spolu s kontextem operace.
...
Přístup správců a vývojářů k indexům není odlišený: musí mít přístup pro zápis k vytváření, odstraňování a aktualizaci objektů spravovaných službou. Kdokoli s klíčem správce ke službě může číst, upravovat nebo odstraňovat libovolný index ve stejné službě. Pro ochranu proti náhodnému nebo škodlivému odstranění indexů je vaše interní Správa zdrojového kódu pro assety kódů nápravou pro vrácení nechtěného odstranění nebo změny indexu. Azure Search má v rámci clusteru převzetí služeb při selhání, aby se zajistila dostupnost, ale neukládá ani neprovádí vlastní kód používaný k vytváření nebo načítání indexů.

Pro řešení s více architekturami, které vyžadují hranice zabezpečení na úrovni indexu, taková řešení obvykle zahrnují střední úroveň, kterou zákazníci používají ke zpracování izolace indexu. Další informace o případu použití s více klienty najdete v tématu [vzory návrhu pro víceklientské aplikace SaaS a Azure Search](search-modeling-multitenant-saas-applications.md).

## <a name="admin-access"></a>Přístup správce

[Přístup na základě role (RBAC)](https://docs.microsoft.com/azure/role-based-access-control/overview) určuje, jestli máte přístup k ovládacím prvkům přes službu a její obsah. Pokud jste vlastníkem nebo přispěvatelem Azure Search služby, můžete k vytváření, aktualizaci nebo odstraňování objektů ve službě použít portál nebo modul PowerShellu **AZ. Search.** Můžete také použít [REST API správy Azure Search](https://docs.microsoft.com/rest/api/searchmanagement/search-howto-management-rest-api).
...
## <a name="table-permissioned-operations"></a>Stolní Operace s oprávněním

Následující tabulka shrnuje operace, které jsou povolené v Azure Search a které klíč odemkne přístup k určité operaci.
...
| Spravovat klíče dotazů |  Klíč správce, vlastník RBAC nebo přispěvatel na prostředku  |
