...
- Schopnost převzetí služeb při selhání z jednoho datového centra do jiného
...
- Patched servery, které jsou tak pořád aktuální
...
|Integrace služby Active directory (RBAC)| Můžete také zadat nebo omezení přístupu k účtu Cosmos, databáze, kontejner a nabídky (propustnost) pomocí řízení přístupu (IAM) na webu Azure Portal. IAM poskytuje řízení přístupu na základě rolí a integruje se službou Active Directory. Pro jednotlivce a skupiny můžete použít integrované role nebo vlastních rolí. Zobrazit [integrace služby Active Directory](role-based-access-control.md) najdete další informace.|
...
|Místní replikaci|Dokonce i v rámci jednoho datového centra, Azure Cosmos DB automaticky replikuje data pro zajištění vysoké dostupnosti, dává vám možnost z [úrovně konzistence](consistency-levels.md). Tato replikace zaručuje 99,99 % [smlouva SLA o dostupnosti](https://azure.microsoft.com/support/legal/sla/cosmos-db) pro všechny účty v jedné oblasti a všechny účty ve více oblastech s mírnější konzistencí a 99,999 % dostupnosti pro všechny účty databáze pro více oblastí pro čtení.|
|Automatizované zálohování online|Databáze Azure Cosmos DB jsou pravidelně zálohovány a uložené v geograficky redundantní úložiště. <br><br>Další informace najdete v [automatické online zálohování a obnovení pomocí služby Azure Cosmos DB](online-backup-and-restore.md).|
...
|Šifrování protokolu HTTPS a SSL/TLS|Všechny interakce klienta služby Azure Cosmos DB jsou schopné SSL/TLS 1.2. Všechny uvnitř datového centra a různé replikace datacenter jsou také vynucuje SSL/TLS 1.2.|
...
|Zabezpečení a data protection certifikace| Nejaktuálnější seznam certifikací naleznete v tématu celkové [lokality dodržování předpisů Azure](https://www.microsoft.com/en-us/trustcenter/compliance/complianceofferings) a také nejnovější [dokumentu dodržování předpisů Azure](https://gallery.technet.microsoft.com/Overview-of-Azure-c1be3942) s všechny certifikace (vyhledejte Cosmos). Podívejte se na příspěvek 25. dubna 2018 přesněji zaměřené najdete [Azure #CosmosDB: Bezpečné, privátní a vyhovující, který obsahuje Soc 1/2 typ 2, HITRUST, PCI DSS úrovně 1, ISO 27001, HIPAA, FedRAMP High a mnoha dalších.
...
Další informace o hlavních klíčů a tokeny prostředků najdete v tématu [zabezpečení přístupu k datům služby Azure Cosmos DB](secure-access-to-data.md).
