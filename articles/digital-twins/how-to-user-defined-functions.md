...
1. **objectID** (`YOUR_USER_DEFINED_FUNCTION_ID`) bude uživatelsky definované ID funkce, které jste vytvořili dříve.
...
1. Vložte vrácenou `spacePaths` hodnotu do **cesty** , abyste vytvořili uživatelem definovanou funkci přiřazení role, a to provedením ověřené žádosti HTTP post:
    ...
    | YOUR_USER_DEFINED_FUNCTION_TYPE_ID | ID určující uživatelem definovaný typ funkce (`UserDefinedFunctionId`) |
