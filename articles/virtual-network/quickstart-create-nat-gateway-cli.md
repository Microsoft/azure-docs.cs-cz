---
title: 'Rychlý Start: Vytvoření brány NAT – Azure CLI'
...
ms.locfileid: "72376396"
---
# <a name="quickstart-create-a-nat-gateway-using-azure-cli"></a>Rychlý Start: Vytvoření brány NAT pomocí Azure CLI

V tomto rychlém startu se dozvíte, jak používat službu Azure NAT a vytvořit bránu NAT pro poskytování odchozího připojení pro virtuální počítač v Azure. 
...
Tento kurz můžete dokončit pomocí Azure Cloud shellu nebo místně spustit příslušné příkazy.  Pokud jste službu Azure Cloud Shell nikdy nepoužívali, [Přihlaste se hned](https://shell.azure.com) a Projděte Úvodní nastavení.
...
Pomocí příkazu [az network vnet create](https://docs.microsoft.com/cli/azure/network/vnet) vytvořte virtuální síť **myVnet** s podsítí **mySubnet** ve skupině prostředků **myResourceGroup**.  Adresní prostor IP adres pro virtuální síť je **192.168.0.0/16** a podsíť ve virtuální síti je **192.168.0.0/24**.
...
Nakonfigurujeme **mySubnet** zdrojové podsítě ve službě Virtual Network **myVnet** tak, aby používala konkrétní prostředek brány NAT **myNAT** pomocí [AZ Network VNet Subnet Update](https://docs.microsoft.com/cli/azure/network/vnet/subnet).  Tento příkaz aktivuje službu NAT v zadané podsíti.
...
Počkejte, až se virtuální počítač dokončí nasazování, a pak pokračujte zbývajícími kroky.
...
V tomto kurzu jste vytvořili bránu NAT a virtuální počítač pro použití služby NAT. Další informace o službě Azure NAT najdete v dalších kurzech pro službu Azure NAT.
