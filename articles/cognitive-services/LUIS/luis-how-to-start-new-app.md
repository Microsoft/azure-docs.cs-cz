...
* Importujte aplikaci LUIS ze souboru `.lu` nebo `.json` , který už obsahuje záměry, projevy a entity.
...
1. Zvolte jazykovou verzi vaší aplikace a potom vyberte **Hotovo**. Popis a předpověď prostředku jsou v tuto chvíli volitelné. V části **Spravovat** portálu můžete kdykoli nastavit.
    ...
    Po vytvoření aplikace se na portálu LUIS zobrazí seznam **záměrů** s již vytvořeným `None` záměrem. Teď máte prázdnou aplikaci. 
...
* Export protokolů koncového bodu, `.csv`jako je, pro offline analýzu
* Odstranit aplikaci
