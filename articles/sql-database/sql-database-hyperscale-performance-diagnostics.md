---
title: Azure SQL Database diagnostiky řešení potíží s výkonem s škálovatelným výkonem | Microsoft Docs
description: Tento článek popisuje, jak řešit problémy s výkonem s výkonným škálováním v SQL Database.
...
|RBIO_RG_REPLICA        | Vyvolá se v případě, že je v důsledku omezené spotřeby protokolu pro čitelné uzly sekundární repliky omezována rychlost generování protokolu databáze výpočetního uzlu.         |
...
Pokud je ve výpočetní replice vydaný objekt pro čtení, pokud data ve fondu vyrovnávací paměti nebo v místní mezipaměti RBPEX nejsou, volání funkce GetPage (pageId, LSN) a stránka se načte z odpovídajícího serveru stránky. Čtení ze stránkovacích serverů je vzdálené čtení a proto je pomalejší než čtení z místní RBPEX. Při řešení potíží s výkonem souvisejících s vstupně-výstupních operací potřebujeme vědět, kolik IOs bylo provedeno prostřednictvím poměrně pomalejšího čtení vzdáleného serveru stránky.

Přidali jsme do sady zobrazení dynamické správy a rozšířené události, které vám pomůžou určit, co čtení jsou vzdáleně čtena z logických čtení stránky serveru v/s.
...
- ActualPageServerReads/ActualPageServerReadAheads se přidají do XML plánu dotazů pro skutečné plány.
...
- Na primární výpočetní prostředky se za zápis protokolu používá file_id 2 v sys. DM _io_virtual_file_stats. Zápis do protokolu v primárním výpočetním prostředí je zápis do zóny pro odpočívadlo protokolu, což je vzdálené úložiště Azure Premium.
- U sekundární repliky nejsou záznamy protokolu u sekundární repliky na základě potvrzení zpřísněny, protokol je aplikován službou xlog na vzdálené repliky. Zadané zápisy do protokolu se ve skutečnosti nevyskytují na sekundárních replikách a slouží pouze k účelům sledování.

## <a name="additional-resources"></a>Další materiály

- Pro omezení prostředků vCore pro izolovanou databázi s jedním škálováním najdete v tématu [limity Vcore úrovně služby škálování na úrovni služeb](sql-database-vcore-resource-limits-single-databases.md#hyperscale-service-tier-for-provisioned-compute) .
