...
* Vysvětlení sítě TCP/IP. Pokud nejste obeznámeni s prací v síti TCP/IP, by měla spolupracovat s někým, kdo je před provedením změny do produkčních sítích.
...
3. Používáte skupiny zabezpečení sítě, tras definovaných uživatelem nebo virtuální síťová zařízení k omezení provozu do nebo ven z virtuální sítě?
    ...
    * Skupiny zabezpečení sítě
        ...
        Nahraďte `RESOURCEGROUP` s názvem skupiny prostředků, která obsahuje virtuální síť a potom zadejte příkaz:
...
1. Vždy musí povolit provoz z následujících IP adres:
    ...
    | Zdrojová IP adresa | Cílový port | Direction |
    | ---- | ----- | ----- |
...
2. Pokud váš cluster HDInsight je v jednom z následujících oblastí, musíte povolit přenosy z IP adresy uvedené pro oblast:
    ...
    | Země | Oblast | Povolené zdrojové IP adresy | Povolený cílový port | Direction |
    | ---- | ---- | ---- | ---- | ----- |
...
1. Pomocí následujícího příkazu vytvořte novou skupinu zabezpečení sítě s názvem `hdisecure`. Nahraďte `RESOURCEGROUP` s vybranou skupinou prostředků, která obsahuje virtuální síť Azure. Nahraďte `LOCATION` umístěním (oblastí), která byla skupina vytvořena v.
