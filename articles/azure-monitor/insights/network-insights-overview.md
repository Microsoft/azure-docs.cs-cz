...
description: Přehled Azure Monitor Network Insights, který poskytuje komplexní přehled o stavu a metrikách pro všechny nasazené síťové prostředky bez jakékoli konfigurace.
ms.topic: conceptual
author: KumudD
ms.author: kumud
ms.date: 11/25/2020
...
ms.locfileid: "105968250"
---
# <a name="azure-monitor-network-insights"></a>Azure Monitor Network Insights

Azure Monitor Network Insights poskytuje komplexní přehled o [stavu](../../service-health/resource-health-checks-resource-types.md) a [metrikách](../essentials/metrics-supported.md) pro všechny nasazené síťové prostředky, a to bez nutnosti žádné konfigurace. Poskytuje taky přístup k funkcím monitorování sítě, jako je [monitorování připojení](../../network-watcher/connection-monitor-overview.md), [protokolování toku pro skupiny zabezpečení sítě (skupin zabezpečení sítě)](../../network-watcher/network-watcher-nsg-flow-logging-overview.md), a [Analýza provozu](../../network-watcher/traffic-analytics.md). A poskytuje další funkce [diagnostiky](../../network-watcher/network-watcher-monitoring-overview.md#diagnostics) sítě.

Azure Monitor přehledy sítě jsou členěné kolem těchto klíčových komponent monitorování:
...
Stránka **přehled** Azure monitor Network Insights nabízí snadný způsob vizualizace inventáře síťových prostředků spolu se stavem prostředků a výstrahami. Je rozdělená do čtyř klíčových funkčních oblastí: hledání a filtrování, stav prostředku a metriky, výstrahy a zobrazení závislostí.
...
[![Snímek obrazovky, který ukazuje Azure Monitor výsledky hledání v síti pro přehled.](media/network-insights-overview/search.png)](media/network-insights-overview/search.png#lightbox)
...
![Snímek obrazovky zobrazující stav prostředku a metriky v Azure Monitor Network Insights](media/network-insights-overview/resource-health.png)

Pokud vyberete nedostupná připojení ER a VPN, zobrazí se zobrazení metriky: 

![Snímek obrazovky zobrazující zobrazení metriky v Azure Monitor Network Insights](media/network-insights-overview/metric-view.png)
...
![Sreenshot zobrazující Application Gateway zobrazení v Azure Monitor Network Insights.](media/network-insights-overview/application-gateway.png)
...
[![Snímek obrazovky zobrazující zobrazení závislostí v Azure Monitor Network Insights](media/network-insights-overview/dependency-view.png)](media/network-insights-overview/dependency-view.png#lightbox)
...
![Snímek obrazovky, který zobrazuje nabídku zobrazení závislosti v Azure Monitor Network Insights.](media/network-insights-overview/dependency-view-menu.png)
...
![Snímek obrazovky, který ukazuje příklad hledání v Azure Monitor Network Insights.](media/network-insights-overview/search-example.png)
...
![Snímek obrazovky zobrazující kartu připojení v Azure Monitor Network Insights](media/network-insights-overview/azure-monitor-for-networks-connectivity-tab.png)
...
[![Snímek obrazovky zobrazující testy připojení ve službě Azure Monitor Network Insights](media/network-insights-overview/azure-monitor-for-networks-connectivity-tests.png)](media/network-insights-overview/azure-monitor-for-networks-connectivity-tests.png#lightbox)

Můžete vybrat libovolnou dlaždici zdroje nebo cíle a otevřít tak zobrazení metriky:

[![Snímek obrazovky zobrazující metriky připojení v Azure Monitor Network Insights](media/network-insights-overview/azure-monitor-for-networks-connectivity-metrics.png)](media/network-insights-overview/azure-monitor-for-networks-connectivity-metrics.png#lightbox)
...
[![Snímek obrazovky zobrazující kartu přenos v Azure Monitor Network Insights](media/network-insights-overview/azure-monitor-for-networks-traffic-view.png)](media/network-insights-overview/azure-monitor-for-networks-traffic-view.png#lightbox)
...
[![Snímek obrazovky zobrazující zobrazení oblasti provozu v Azure Monitor Network Insights](media/network-insights-overview/azure-monitor-for-networks-traffic-region-view.png)](media/network-insights-overview/azure-monitor-for-networks-traffic-region-view.png#lightbox)
...
Tato část vám pomůže diagnostikovat a řešit některé běžné problémy, se kterými se můžete setkat při používání Azure Monitor Network Insights. 

### <a name="how-do-i-resolve-performance-problems-or-failures"></a>Návody vyřešit problémy s výkonem nebo chyby?

Další informace o řešení potíží souvisejících se sítí, které identifikujete pomocí Azure Monitor Network Insights, najdete v dokumentaci k řešení potíží pro prostředek, který nefunguje. 
...
Azure Monitor Network Insights může zobrazit prostředky jenom pro pět předplatných. 

### <a name="how-do-i-make-changes-or-add-visualizations-to-azure-monitor-network-insights"></a>Návody udělat změny nebo přidat vizualizace do Azure Monitor Network Insights?
...
Azure Monitor Network Insights používá **automaticky** zrnitost, takže časový interval je založený na vybraném časovém rozsahu.
...
### <a name="what-if-i-want-to-see-other-data-or-make-my-own-visualizations-how-can-i-make-changes-to-azure-monitor-network-insights"></a>Co když chci zobrazit další data nebo vytvořit vlastní vizualizace? Jak mohu provádět změny Azure Monitor Network Insights?
