---
title: Argumenty příkazového řádku pro Microsoft OPC Publisher
description: Tento článek poskytuje přehled argumentů příkazového řádku pro OPC vydavatele.
author: jehona-m
ms.author: jemorina
...
ms.locfileid: "104787532"
---
# <a name="command-line-arguments"></a>Argumenty příkazového řádku

V následující části jsou popsány některé argumenty příkazového řádku, které lze použít k nastavení globálního nastavení pro vydavatele OPC.

## <a name="opc-publisher-command-line-arguments-for-version-25-and-below"></a>Argumenty příkazového řádku pro OPC vydavatele pro verzi 2,5 a nižší

* Použití: opcpublisher.exe \<applicationname> [ \<iothubconnectionstring> ] [ \<options> ]
    
* ApplicationName: název aplikace OPC UA, která se má použít. název aplikace se používá také k registraci vydavatele pod tímto názvem v registru IoT Hub zařízení.
    
* iothubconnectionstring: IoT Hub ConnectionString vlastníka, volitelné. Obvykle zadáváte ConnectionString IoTHub Owner pouze při prvním spuštění aplikace. Připojovací řetězec je zašifrovaný a uložený v úložišti certifikátů platforem.
Při následném volání se z ní načte a znovu použije. Pokud při každém spuštění zadáte ConnectionString, zařízení, které se vytvoří pro aplikaci v registru IoT Hub zařízení, se odeberou a znovu vytvoří.
    
K dispozici je několik proměnných prostředí, které lze použít k řízení aplikace:
```
...
> [!NOTE] 
> Parametry příkazového řádku: nastavení proměnné prostředí je v souladu s pravidly.
...
## <a name="opc-publisher-command-line-arguments-for-version-26-and-above"></a>Argumenty příkazového řádku pro OPC vydavatele pro verzi 2,6 a vyšší
```
      --pf, --publishfile=VALUE
                                       the filename to configure the nodes  to publish.
...
Další zdroje najdete v úložištích GitHubu:
