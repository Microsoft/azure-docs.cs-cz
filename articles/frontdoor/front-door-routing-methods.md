---
title: Služba front-dveří Azure – metody směrování provozu | Microsoft Docs
...
ms.locfileid: "78382101"
---
# <a name="front-door-routing-methods"></a>Metody směrování front-dveří

Služba front-dveří Azure podporuje různé metody směrování provozu k určení způsobu směrování provozu HTTP/HTTPS do různých koncových bodů služby. U každého z vašich klientských žádostí o připojení k frontě se použije nakonfigurovaná Metoda směrování, aby se zajistilo, že se požadavky předají do nejlepší instance back-endu. 
...
* **Spřažení relace:** Spřažení relace pro hostitele nebo domény front-endu můžete nakonfigurovat, pokud chcete, aby se následné požadavky uživatele odesílaly do stejného back-endu, dokud je uživatelská relace stále aktivní a instance back-endu stále hlásí stav v pořádku na základě sond stavu. 
...
## <a name = "priority"></a>Provoz založený na prioritách – směrování
...
Každé z back-endu ve fondu back-end v rámci konfigurace front-endu má vlastnost s názvem priority, která může být číslo mezi 1 a 5. U služby front-endu Azure nakonfigurujete prioritu back-endu explicitně pomocí této vlastnosti pro každý back-end. Tato vlastnost je hodnota v rozmezí od 1 do 5. Nižší hodnoty reprezentují vyšší prioritu. Back-endy můžou sdílet prioritní hodnoty.

## <a name = "weighted"></a>Vážená Metoda směrování provozu
...
Ve výchozím nastavení bez spřažení relací předávají přední dveře požadavky pocházející od stejného klienta k různým back-endu na základě konfigurace vyrovnávání zatížení, zejména při latencích jiné změny back-endu nebo v případě odlišných požadavků od stejného uživatele. rozdílně na jiné prostředí front-dveří. Pro některé stavové aplikace nebo v určitých jiných scénářích je však žádoucí, aby další požadavky od stejného uživatele přicházely do stejného back-endu, který zpracoval počáteční požadavek. Funkce spřažení relací na základě souborů cookie je užitečná v případě, že chcete zachovat uživatelskou relaci na stejném back-endu. Pomocí souborů cookie spravovaných přes dvířka může služba Azure front-endu směrovat následný provoz z uživatelské relace do stejného back-endu pro zpracování, dokud je back-end v pořádku a uživatelská relace nevypršela. 
