...
**Koncový bod** , který implementuje **ResourceType** , musí zpracovat požadavek a odpověď na nové rozhraní API v Azure. Když **se vytvoří** vlastní poskytovatel prostředků s prostředky, vytvoří se v Azure nová sada rozhraní API. V tomto případě vygeneruje příkaz **ResourceType** nové rozhraní API prostředků Azure pro `PUT`, `GET`a `DELETE` k provedení CRUD na jednom prostředku a také `GET` k načtení všech existujících prostředků:
...
Další informace o prostředcích "`Proxy`" najdete v tématu [Reference k vlastnímu proxy prostředku](proxy-resource-endpoint-reference.md) .

### <a name="proxy-cache-routing-type"></a>typ směrování mezipaměti proxy

Proxy "`Proxy, Cache`" **routingType** pouze `PUT` a `DELETE` metody požadavku na **koncový bod** zadaný ve vlastním poskytovateli prostředků. Vlastní poskytovatel prostředků automaticky vrátí `GET` požadavky na základě toho, co byl uložen v mezipaměti. Pokud je vlastní prostředek označený jako mezipaměť, vlastní poskytovatel prostředků také přidá/přepíše pole v odpovědi, aby rozhraní API splňovala požadavky na Azure. Kdy použít "`Proxy, Cache`":

- Vytvoření nového systému, který nemá žádné existující prostředky.
- Pracujte se stávajícím ekosystémem Azure.

Další informace o prostředcích "`Proxy, Cache`" najdete v tématu [vlastní odkaz na mezipaměť prostředků](proxy-cache-resource-endpoint-reference.md) .
...
Šablony Azure Resource Manager vyžadují, aby se `id`, `name`a `type` vrátily správně z koncového bodu pro příjem dat. Vrácená odpověď prostředku by měla být ve tvaru:
...
Parametr | Požaduje se | Popis
---|---|---
resourceTypeName | *Ano* | **Název** elementu **ResourceType** definovaného ve vlastním zprostředkovateli.
