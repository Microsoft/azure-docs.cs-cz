---
title: Získávání pravopisu zkontrolujte výsledky pomocí Bingu pravopisu zkontrolujte rozhraní API (kognitivní služeb pro Microsoft v Azure) | Microsoft Docs
description: Ukazuje, jak používat kontrola pravopisu Bing.
services: cognitive-services
author: v-jaswel
manager: kamrani
ms.assetid: 2575A80C-FC74-4631-AE5D-8101CF2591D3
ms.service: cognitive-services
ms.component: bing-spell-check
ms.topic: article
ms.date: 09/28/2017
ms.author: v-jaswel
ms.openlocfilehash: 4e4cdbb8a3d6ab01888d8f273083155c33eb06c1
ms.sourcegitcommit: 95d9a6acf29405a533db943b1688612980374272
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 06/23/2018
ms.locfileid: "35342692"
---
# <a name="build-a-web-page-spell-check-client"></a>Vytvoření klienta kontrola pravopisu webové stránky

V tomto kurzu využijeme webové stránky, která umožňuje uživatelům dotazu API kontrola pravopisu Bing.

V tomto kurzu získáte informace o následujících postupech:

> [!div class="checklist"]
> - Provést jednoduchý dotaz API kontrola pravopisu Bing
> - Zobrazí výsledky dotazu

## <a name="prerequisites"></a>Požadavky

Chcete-li postupovat podle kurzu, je třeba klíč předplatného pro API kontrola pravopisu Bing. Pokud nemáte, [zaregistrovat k bezplatné zkušební verzi](https://azure.microsoft.com/try/cognitive-services/?api=spellcheck-api).

## <a name="create-a-new-web-page"></a>Vytvořit novou webovou stránku

Otevřete textový editor. Vytvořte nový soubor s názvem, například spellcheck.html.

## <a name="add-html-header"></a>Přidat hlavičku HTML

Přidat informace ze záhlaví HTML a začít části skript následujícím způsobem.

```html
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8"> 
    <title>Bing Spell Check</title>

<style type="text/css">
    html, body, div, p, h1, h2 {font-family: Verdana, "Lucida Sans", sans-serif;}

    html, body, div, p  {font-weight: normal;}
    h1, h2 {font-weight: bold;}
    sup {font-weight: normal;}

    html, body, div, p  {font-size: 12px;}
    h1 {font-size: 20px;}
    h2 {font-size: 16px;}
    h1, h2 {clear: left;}

    img#logo {float: right;
</style>

<script type="text/javascript">
```

## <a name="getsubscriptionkey-function"></a>getSubscriptionKey – funkce

Funkce getSubscriptionKey vrací klíč rozhraní API kontrola pravopisu Bing. Je buď načte z místního úložiště (to znamená, že soubor cookie), nebo v případě potřeby vyzve uživatele k.

Začít funkci getSubscriptionKey a název souboru cookie deklarovat následujícím způsobem.

```html
getSubscriptionKey = function() {

    var COOKIE = "bing-spell-check-api-key";   // name used to store API key in key/value storage
```

FindCookie pomocné funkce vrátí hodnotu zadaného souboru cookie; Pokud není nalezen soubor cookie, vrátí prázdný řetězec.

```html
    function findCookie(name) {
        var cookies = document.cookie.split(";");
        for (var i = 0; i < cookies.length; i++) {
            var keyvalue = cookies[i].split("=");
            if (keyvalue[0].trim() === name) {
                return keyvalue[1];
            }
        }
        return "";
        }
```

Pomocné funkce getSubscriptionKeyCookie vyzve uživatele k hodnotě klíč rozhraní API kontrola pravopisu Bing a vrátí hodnotu klíče.

```html
    function getSubscriptionKeyCookie() {
        var key = findCookie(COOKIE);
        while (key.length !== 32) {
            key = prompt("Enter Bing Spell Check API subscription key:", "").trim();
            var expiry = new Date();
            expiry.setFullYear(expiry.getFullYear() + 2);
            document.cookie = COOKIE + "=" + key.trim() + "; expires=" + expiry.toUTCString();
        }
        return key;
    }
```

Pomocné funkce getSubscriptionKeyLocalStorage se nejprve pokusí se načíst klíč rozhraní API kontrola pravopisu Bing vyhledáním odpovídající soubor cookie. Pokud není nalezen soubor cookie, vyzve uživatele k hodnotě klíče. Pak vrátí hodnotu klíče.

```html
    function getSubscriptionKeyLocalStorage() {
        var key = localStorage.getItem(COOKIE) || "";
        while (key.length !== 32)
            key = prompt("Enter Bing Spell Check API subscription key:", "").trim();
        localStorage.setItem(COOKIE, key)
        return key;
    }
```

Pomocné funkce getSubscriptionKey přijímá jeden parametr, **zneplatnit**. Pokud **zneplatnit** je **true**, getSubscriptionKey odstraní soubor cookie, který obsahuje klíč rozhraní API kontrola pravopisu Bing. Pokud **zneplatnit** je **false**, getSubscriptionKey vrací hodnotu klíč rozhraní API kontrola pravopisu Bing.

```html
    function getSubscriptionKey(invalidate) {
        if (invalidate) {
            try {
                localStorage.removeItem(COOKIE);
            } catch (e) {
                document.cookie = COOKIE + "=";
            }
        } else {
            try {
                return getSubscriptionKeyLocalStorage();
            } catch (e) {
                return getSubscriptionKeyCookie();
            }
        }
    }
```

Vrátí jako výsledek funkce vnější getSubscriptionKey getSubscriptionKey pomocné funkce. Definice funkce vnější getSubscriptionKey zavřete.

```html
    return getSubscriptionKey;

}();
```

## <a name="helper-functions"></a>Podpůrné funkce

Před pomocné funkce vrátí zadaný text předem naformátované pomocí [před](https://www.w3schools.com/tags/tag_pre.asp) značky HTML.

```html
function pre(text) {
    return "<pre>" + text.replace(/&/g, "&amp;").replace(/</g, "&lt;") + "</pre>"
}
```

Funkce renderSearchResults zobrazí zadané výsledky z rozhraním API služby Bing pravopisu zkontrolovat, pomocí JSON velmi tisku.

```html
function renderSearchResults(results) {
    document.getElementById("results").innerHTML = pre(JSON.stringify(results, null, 2));
}
```

Funkce renderErrorMessage zobrazí zadanou chybovou zprávu a kód chyby.

```html
function renderErrorMessage(message, code) {
    if (code)
        document.getElementById("results").innerHTML = "<pre>Status " + code + ": " + message + "</pre>";
    else
        document.getElementById("results").innerHTML = "<pre>" + message + "</pre>";
}
```

## <a name="bingspellcheck-function"></a>bingSpellCheck – funkce

BingSpellCheck funkce je volána pokaždé, když uživatel zadá text do pole formuláře HTML.
Jak dlouho trvá dva parametry: obsah HTML formulář, pole a klíč rozhraní API kontrola pravopisu Bing.

```html
function bingSpellCheck(query, key) {
```

Zadejte koncový bod rozhraní API Bing pravopisu zkontrolujte a deklarace objektu XMLHttpRequest, které použijete k odesílání požadavků na koncový bod.

```html
    var endpoint = "https://api.cognitive.microsoft.com/bing/v7.0/spellcheck/";

    var request = new XMLHttpRequest();

    try {
        request.open("GET", endpoint + "?mode=proof&mkt=en-US&text=" + encodeURIComponent(query));
    }
    catch (e) {
        renderErrorMessage("Bad request");
        return false;
    }
```

Nastavte **Ocp-Apim-Subscription-Key** záhlaví na hodnotu klíč rozhraní API kontrola pravopisu Bing.

```html
    request.setRequestHeader("Ocp-Apim-Subscription-Key", key);
```

Zpracování odpovědi z koncového bodu. Pokud je stav 200 (OK), zobrazte výsledky; jinak zobrazte informace o chybě.

```html
    request.addEventListener("load", function() {
        if (this.status === 200) {
            renderSearchResults(JSON.parse(this.responseText));
        }
        else {
            if (this.status === 401) getSubscriptionKey(true);
            renderErrorMessage(this.statusText, this.status);
        }
    });
```

Možné chybové události z objektu XMLHttpRequest také zpracovávejte.

```html
    request.addEventListener("error", function() {
        renderErrorMessage("Network error");
    });

    request.addEventListener("abort", function() {
        renderErrorMessage("Request aborted");
    });
```

Odešlete požadavek. Zavřete funkci bingSpellCheck **skriptu** značky a **head** značky.

```html
    request.send();
    return false;
}
// --></script>

</head>
```

## <a name="html-body"></a>Text HTML

Pokud webová stránka načte, zajistěte, aby jsme klíč rozhraní API kontrola pravopisu Bing, výzvy pro uživatele pro něj v případě potřeby.

```html
<body onload="document.forms.bing.query.focus(); getSubscriptionKey();">
```

Zobrazení loga Bing.

```html
<img id="logo" align=base src="data:image/png;base64,">
```

Vytvoření formuláře HTML s textové pole. Zpracování **onsubmit** událostí a volání funkce bingSpellCheck, obsah textové pole a klíč rozhraní API kontrola pravopisu Bing předávání.

```html
<form name="bing" onsubmit="return bingSpellCheck(this.query.value, getSubscriptionKey())">
    <h2>Spell Check</h2>
    <input type="text" name="query" size="80" placeholder="Spell Check" autocomplete=off>
</form>
```

Přidání kódu HTML **div** značky, který používáme zobrazíte výsledky. JavaScript definovaného dříve odkazuje na tato **div** značky.

```html
<h2>Results</h2>
<div id="results">
<p>None yet.</p>

</div>

</body>
</html>
```

Uložte soubor.

## <a name="display-results"></a>Zobrazení výsledků

Otevřete webovou stránku v prohlížeči. Do příkazového řádku zadejte svůj klíč předplatného API kontrola pravopisu Bing. Zadejte dotaz (například "KR, wlrd!") v **kontrola pravopisu** textového pole a stiskněte klávesu **Enter**. Webová stránka se potom zobrazí výsledky dotazu.

```json
{
  "_type": "SpellCheck",
  "flaggedTokens": [
    {
      "offset": 0,
      "token": "Hollo",
      "type": "UnknownToken",
      "suggestions": [
        {
          "suggestion": "Hello",
          "score": 0.856629936217145
        },
        {
          "suggestion": "Hollow",
          "score": 0.816717853225633
        }
      ]
    },
    {
      "offset": 7,
      "token": "wlrd",
      "type": "UnknownToken",
      "suggestions": [
        {
          "suggestion": "world",
          "score": 0.856629936217145
        }
      ]
    }
  ]
}
```

## <a name="next-steps"></a>Další postup

> [!div class="nextstepaction"]
> [Požadavky na použití a zobrazení](../UseAndDisplayRequirements.md)
