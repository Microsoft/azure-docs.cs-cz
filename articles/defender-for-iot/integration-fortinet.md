...
## <a name="the-defender-for-iot-platform"></a>Defender pro platformu IoT
...
### <a name="stop-ransomware-before-it-damages-iot-and-ics-networks"></a>Zastavení ransomwarem před tím, než přeškodí sítě IoT a ICS
...
1. V levém podokně senzoru nebo místní konzoly pro správu vyberte **předat dál**.
    ...
    [:::image type="content" source="media/integration-fortinet/forwarding-view.png" alt-text="Prohlédněte si pravidla přesměrování v okně přesměrování.":::](media/integration-fortinet/forwarding-view.png#lightbox)
...
Nastavení pravidla brány firewall FortiGate blokujícího tento škodlivý zdroj:
...
## <a name="block-suspected-traffic-using-the-fortigate-firewall"></a>Zablokovat podezřelý provoz pomocí brány FortiGate firewall

1. V levém podokně vyberte **předat dál**.

    [:::image type="content" source="media/integration-fortinet/forwarding-view.png" alt-text="Možnost okna přesměrování ve snímači.":::](media/integration-fortinet/forwarding-view.png#lightbox)

1. Vyberte **vytvořit pravidla předávání** a definujte parametry pravidla.

    :::image type="content" source="media/integration-fortinet/new-rule.png" alt-text="Snímek obrazovky s oknem vytvořit pravidlo předávání":::
    ...
    | **Protokoly** | Ve výchozím nastavení jsou vybrány všechny protokoly.<br><br>Pokud chcete vybrat konkrétní protokol, vyberte **konkrétní** a vyberte protokol, pro který se toto pravidlo použije. |
    | **Motoru** | Ve výchozím nastavení jsou zapojeny všechny bezpečnostní moduly.<br><br>Pokud chcete vybrat konkrétní modul zabezpečení, pro který se toto pravidlo používá, vyberte **konkrétní** a vyberte modul. |
    | **Systémová oznámení** | *Online a offline* stav předávaného senzoru Tato možnost je k dispozici pouze v případě, že jste přihlášeni k místní konzole pro správu. |

1. Pokud chcete programu Defender pro IoT poslat pravidla firewallu na FortiGate, vyberte **Akce** a pak vyberte **Odeslat do Fortigate**.

    :::image type="content" source="media/integration-fortinet/fortigate.png" alt-text="Vytvořit okno pravidla předávání a vybrat odeslat do FortiGate":::

1. Konfigurace pravidla předávání FortiGate:

    :::image type="content" source="media/integration-fortinet/configure.png" alt-text="Konfigurace okna vytvořit předávací pravidlo":::

1. V podokně **Akce** nastavte následující parametry:

    | Parametr | Popis |
    |--|--|
    | Hostitel | Typ IP adresy serveru FortiGate |
    | Port | Typ portu serveru FortiGate |
    | Uživatelské jméno | Typ uživatelského jména serveru FortiGate |
    | Klíč rozhraní API | Zadejte klíč rozhraní API, který jste vytvořili v FortiGate. |
    | Příchozí rozhraní| Určete, jak se má blokování provádět:<br /><br />**Podle IP adresy**: vždy vytváří blokující zásady na panoramatu na základě IP adresy.<br /><br />**Podle plně kvalifikovaného názvu domény nebo IP adresy**: vytváří blokující zásady na panoramatu založené na plně kvalifikovaném názvu domény, jinak IP adresa.| 
    | Odchozí rozhraní |Nastavte e-mailovou adresu pro e-mail s oznámením zásad. <br /><br /> **Poznámka**: Ujistěte se, že jste nakonfigurovali poštovní server v XSense. Pokud není zadaná žádná e-mailová adresa, XSense nepošle e-mailovou zprávu s oznámením.| 
    |Konfigurace| Nastavte následující možnosti, které umožní blokování podezřelých zdrojů bránou firewall FortiGate: <br /><br />**Blokovat neplatné kódy funkcí**: porušení protokolu – neplatná hodnota pole porušující specifikaci protokolu ICS (potenciální zneužití)<br /><br />**Blokovat neoprávněné programování a aktualizace firmwaru pro PLC**: neautorizované změny PLC<br /><br />**Zablokovat neautorizovaný PLC**: zastavení PLC (výpadek)<br /><br />**Blokuje výstrahy týkající se malwaru**: blokování průmyslových pokusů o malware (Triton, NotPetya atd.). Můžete vybrat možnost **automatického blokování**. V takovém případě se blokování spustí automaticky a okamžitě.<br /><br />*Blokovat neoprávněné prohledávání*: neoprávněné prohledávání (potenciální rekognoskace)

1. Vyberte **Odeslat**.

## <a name="block-the-suspicious-source"></a>Zablokovat podezřelý zdroj

1. V podokně **výstrahy** vyberte výstrahu týkající se integrace Fortinet.
