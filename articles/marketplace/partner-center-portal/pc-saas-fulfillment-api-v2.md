...
|  `action`                | Akce prováděná na prostředku, `Unsubscribe`buď, `Suspend`, `Reinstate`nebo `ChangePlan`, `ChangeQuantity`,. `Transfer` |
|   |   |
...
|  autorizace     |  [Získat token nosiče webového tokenu JSON (Jwt)](https://docs.microsoft.com/azure/marketplace/cloud-partner-portal/saas-app/cpp-saas-registration#get-a-token-based-on-the-azure-ad-app). Například: "`Bearer <access_token>`". |
...
##### <a name="deletebr-httpsmarketplaceapimicrosoftcomapisaassubscriptionssubscriptionidapi-versionapiversion"></a>Odstranit<br> `https://marketplaceapi.microsoft.com/api/saas/subscriptions/<subscriptionId>?api-version=<ApiVersion>`
...
Umožňuje vydavateli sledovat stav zadané aktivované asynchronní operace ( `Subscribe`například, `Unsubscribe`, `ChangePlan`nebo `ChangeQuantity`).
