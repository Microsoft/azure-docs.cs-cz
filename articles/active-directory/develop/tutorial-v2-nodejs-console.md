...
Tady se `callApi` Metoda používá k vytvoření `GET` požadavku HTTP proti chráněnému prostředku, který vyžaduje přístupový token. Požadavek potom vrátí obsah volajícímu. Tato metoda přidá získaný token v *autorizační hlavičce protokolu HTTP*. Chráněným prostředkem je tady [koncový bod](/graph/api/user-list) Microsoft Graph API Users, který zobrazuje uživatele v tenantovi, ve kterém je tato aplikace zaregistrovaná.
