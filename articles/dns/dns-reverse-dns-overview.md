...
Například při vytváření reverzní zóny k záznamům hostitelů pro hostitele s IP adresami, které jsou v předponě 192.0.2.0/24, se název zóny vytvoří tak, že se izoluje síťová předpona adresy (192.0.2) a pak se přesměruje objednávka (2.0.192) a přidání přípony `.in-addr.arpa`.
...
V některých případech je rozsah IP adres přidělený organizaci menší než rozsah třídy C (/24). V takovém případě rozsah IP adres nespadá do hranice zóny v rámci hierarchie `.in-addr.arpa` zóny, a proto nemůže být delegovaný jako podřízená zóna.
