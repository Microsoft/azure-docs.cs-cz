...
5. V části **přihlašovací údaje správce** zadejte `https://dialpad.com/scim` na **adrese URL tenanta**. Zadejte hodnotu, kterou jste načetli a uložili dříve ze Storegate v **tajném tokenu**. Klikněte na **Test připojení** a ujistěte se, že se služba Azure AD může připojit k Storegate. Pokud se připojení nepovede, ujistěte se, že má váš účet Storegate oprávnění správce, a zkuste to znovu.
