...
- Instalace v existujícím clusteru s Helm grafy
  ...
  1. Nainstalujte doplněk pomocí grafu Helm. Nahraďte `<subscriptionId>` ID předplatného `<aks engine cluster resource group>` a skupinou prostředků, ve které se cluster AKS Engine Kubernetese pro správu používá samostatně.
...
Gatekeeper pod, _gatekeeper-Controller-Manager-0_je obvykle v oboru názvů `gatekeeper-system` nebo `kube-system` , ale může být v jiném oboru názvů v závislosti na tom, jak je nasazený.

Chcete-li zobrazit protokoly gatekeeper, `kubectl`použijte:
