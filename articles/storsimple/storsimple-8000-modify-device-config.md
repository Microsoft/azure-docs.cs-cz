...
![Popis zařízení v obecných nastaveních](./media/storsimple-8000-modify-device-config/modify-general-settings2.png)
 ...
 - **Primární server NTP** – konfigurace je povinná a je zadaná při použití Windows PowerShell pro StorSimple ke konfiguraci zařízení. Jako server NTP můžete zadat výchozí **time.Windows.com** Windows serveru. Konfiguraci primárního serveru NTP můžete zobrazit pomocí Azure Portal, k jeho změně ale musíte použít rozhraní Windows PowerShell. Pomocí rutiny `Set-HcsNTPClientServerAddress` upravte primární server NTP vašeho zařízení. Další informace najdete v syntaxi pro rutinu [set-HcsNTPClientServerAddress](https://technet.microsoft.com/library/dn688138.aspx) .
...
**Primární server DNS** – při počátečním nastavení použijte k prvnímu určení primárního serveru DNS Windows PowerShell pro StorSimple. Primární server DNS můžete překonfigurovat jenom přes rozhraní Windows PowerShell. Pomocí rutiny `Set-HcsDNSClientServerAddress` upravte primární server DNS vašeho zařízení. Další informace najdete v syntaxi pro rutinu [set-HcsDNSClientServerAddress](https://technet.microsoft.com/library/dn688138.aspx) .

**Sekundární server DNS** – Chcete-li změnit sekundární server DNS, použijte rutinu `Set-HcsDNSClientServerAddress` v rozhraní Windows PowerShell okna zařízení nebo **nastavení sítě** zařízení StorSimple v Azure Portal.
