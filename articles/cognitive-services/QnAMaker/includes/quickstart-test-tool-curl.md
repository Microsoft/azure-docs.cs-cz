...
1. Zkopírujte příkaz do upravitelného prostředí (například textového souboru), abyste mohli příkaz Upravit. Upravte hodnotu otázky následujícím způsobem, aby se metadata `service:qna_maker` používala jako filtr pro QnA sady.

    ```bash
    curl -X POST https://replace-with-your-resource-name.azurewebsites.net/qnamaker/knowledgebases/replace-with-your-knowledge-base-id/generateAnswer -H "Authorization: EndpointKey replace-with-your-endpoint-key" -H "Content-type: application/json" -d "{'top':30, 'question':'size','strictFilters': [{'name':'service','value':'qna_maker'}]}"
    ```

    Otázka je pouze jedno slovo, `size`, které může vracet jednu z obou QnA sad. `strictFilters` pole oznamuje odpověď, aby se snížila pouze na `qna_maker` odpovědi.
...
Informace o ladění vám pomůžou pochopit, jak byla zjištěna odpověď. I když je to užitečné, není nutné. Pokud chcete vygenerovat odpověď s ladicími informacemi, přidejte vlastnost `debug`:
...
Pokud chcete získat odpověď ze zkušební znalostní báze, použijte vlastnost text `isTest`.
...
1. V terminálu s povoleným oblým použijte od uživatele příkaz k ukončení konverzace robota, například `Thank you` jako otázku. Neexistují žádné další vlastnosti, které by bylo možné nastavit.
...
## <a name="use-curl-with-threshold-and-default-answer"></a>Použít oblý s prahovou hodnotou a výchozí odpovědí

Pro odpověď můžete požádat o minimální prahovou hodnotu. Pokud není prahová hodnota splněna, je vrácena výchozí odpověď.

1. Použijte následující příkaz složené instance, kde nahraďte vlastní název prostředku, ID znalostní báze a klíč koncového bodu, abyste požádali o odpověď na `size` s prahovou hodnotou 80% nebo vyšší. Znalostní báze by neměl tuto odpověď najít, protože skóre otázky je 71% a místo toho vrátí výchozí odpověď, kterou jste zadali při vytváření znalostní báze.
...
1. Přidejte vlastnost `threshold` a požádejte o odpověď, aby `size` s prahovou hodnotou 80% nebo vyšší. Znalostní báze by neměl tuto odpověď najít, protože skóre otázky je 71%. Výsledek vrátí výchozí odpověď, kterou jste zadali při vytváření znalostní báze.
