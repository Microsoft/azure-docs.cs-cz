...
## <a name="antimalware"></a>Antimalwarové
...
## <a name="encryption"></a>Šifr

Pro rozšířené zabezpečení virtuálních [počítačů s Windows](../articles/virtual-machines/windows/encrypt-disks.md) a virtuálních počítačů se systémem [Linux](../articles/virtual-machines/linux/disk-encryption-overview.md) můžou být virtuální disky v Azure šifrované. Virtuální disky na virtuálních počítačích s Windows jsou v klidovém stavu šifrované pomocí nástroje BitLocker. Virtuální disky na virtuálních počítačích se systémem Linux jsou v klidovém stavu zašifrované pomocí dm-crypt. 
...
Zásady přístupu trezoru klíčů udělují samostatně oprávnění klíčům, tajným klíčům a certifikátům. Můžete například dát uživateli přístup jenom k klíčům, ale nemáte oprávnění k tajným klíčům. Oprávnění pro přístup k klíčům nebo tajným klíčům nebo certifikátům však jsou na úrovni trezoru. [Zásady přístupu trezoru klíčů](../articles/key-vault/key-vault-secure-your-key-vault.md) nepodporují oprávnění na úrovni objektů.
...
Běžným problémem při sestavování cloudových aplikací je způsob, jak spravovat přihlašovací údaje v kódu pro ověřování ve službě Cloud Services. Zachování zabezpečení přihlašovacích údajů je důležitou úlohou. V ideálním případě se přihlašovací údaje nikdy nezobrazí na pracovních stanicích pro vývojáře a nejsou zaregistrovány do správy zdrojového kódu. Azure Key Vault poskytuje způsob bezpečného ukládání přihlašovacích údajů, tajných kódů a dalších klíčů, ale váš kód se musí ověřit, aby se Key Vaulty načetly. 

Tento problém řeší funkce spravované identity pro prostředky Azure v Azure Active Directory (Azure AD). Tato funkce poskytuje služby Azure s automaticky spravovanou identitou ve službě Azure AD. Identitu můžete použít k ověření pro libovolnou službu, která podporuje ověřování Azure AD, včetně Key Vault bez jakýchkoli přihlašovacích údajů v kódu.  Váš kód, který běží na virtuálním počítači, může požádat o token ze dvou koncových bodů, které jsou přístupné jenom z virtuálního počítače. Podrobnější informace o této službě najdete na stránce Přehled [spravovaných identit pro prostředky Azure](../articles/active-directory/managed-identities-azure-resources/overview.md) .   
