...
9. Vyberte svoje předplatné a skupinu prostředků a potom vyberte umístění trezoru. Až budete hotovi, klikněte na **Vytvořit**.
   ...
   > [!IMPORTANT]
   > Trezor se musí nacházet ve stejné oblasti jako databázového SQL serveru a musí používat stejnou skupinu prostředků jako databáze SQL serveru.
...
Použití [New-AzRecoveryServicesVault](/powershell/module/az.recoveryservices/new-azrecoveryservicesvault) vytvoření trezoru služby recovery services.
...
Použití [Set-AzSqlServerBackupLongTermRetentionVault](/powershell/module/az.sql/set-azsqlserverbackuplongtermretentionvault) k přidružení trezoru dříve vytvořeného recovery services s konkrétním serverem Azure SQL.
...
Zásada uchovávání informací stanoví, kde můžete nastavit dobu uchování zálohy databáze. Použít [Get-AzRecoveryServicesBackupRetentionPolicyObject](https://docs.microsoft.com/powershell/module/azurerm.recoveryservices.backup/get-Azrecoveryservicesbackupretentionpolicyobject) rutiny pro získání výchozí zásady uchovávání informací pro použití jako šablony pro vytváření zásad. V této šabloně je doba uchování nastavena na 2 roky. V dalším kroku spusťte [New-AzRecoveryServicesBackupProtectionPolicy](/powershell/module/azurerm.recoveryservices.backup/new-Azrecoveryservicesbackupprotectionpolicy) nakonec vytvořte zásadu.

> [!NOTE]
> Některé rutiny vyžadují nastavit kontext trezoru před spuštěním ([Set-AzRecoveryServicesVaultContext](/powershell/module/az.recoveryservices/set-azrecoveryservicesvaultcontext)), zobrazí se tato rutina v několika souvisejících fragmentech kódu. Můžete nastavit kontext, protože zásada je součástí trezoru. Můžete vytvořit více zásad uchovávání pro každý trezor a pak použít požadovanou zásadu u konkrétních databází.
...
Použití [Set-AzSqlDatabaseBackupLongTermRetentionPolicy](/powershell/module/az.sql/set-azsqldatabasebackuplongtermretentionpolicy) rutiny použijí nové zásady u konkrétní databáze.
...
Obnovení z dlouhodobě uchovávaných záloh používá [obnovení AzSqlDatabase](/powershell/module/az.sql/restore-azsqldatabase) rutiny.
