...
| **Atributy**              | Není k dispozici  |
| **Reference**              | Není k dispozici  |
| **Uvedené** | Šifry symetrických datových proudů, jako je například RC4, nesmí být použity. Místo šifry symetrických streamů by produkty měly používat blokové šifry, konkrétně AES s délkou klíče minimálně 128 bitů. |
...
| **Uvedené** | Algoritmy šifrování definují transformace dat, které nelze snadno vrátit neoprávněnými uživateli. SQL Server umožňuje správcům a vývojářům vybírat z několika algoritmů, včetně DES, Triple DES, TRIPLE_DES_3KEY, RC2, RC4, 128-bit RC4, DESX, 128 AES, 192-bit AES 256 a 16bitového AES. |
...
| **Atributy**              | Není k dispozici  |
| **Reference**              | Není k dispozici  |
| **Uvedené** | Ujistěte se, že zásady správy zařízení jsou zavedeny, které vyžadují použití PIN a umožňují vzdálené vymazání. |
...
| **Atributy**              | Není k dispozici  |
| **Reference**              | Není k dispozici  |
| **Uvedené** | Zajistěte, aby byly nastavené zásady správy zařízení, které vyžadují PIN/heslo nebo automatické uzamčení a šifrují všechna data (např. BitLocker). |
...
| **Uvedené** | Zajistěte, aby se při použití serveru identity převzaly podpisové klíče. Odkaz v části odkazy vysvětluje, jak by se to mělo naplánovat, aniž by to mělo za následek výpadkům aplikací, které se spoléhají na server identit. |
