---
title: Postup zálohování a obnovení serveru v Azure Database for MySQL
...
Při vytváření serveru pomocí příkazu `az mysql server create` se parametr `--geo-redundant-backup` rozhodne pro možnost redundance zálohy. Pokud `Enabled`, provedou se geograficky redundantní zálohy. Nebo pokud jsou pořízeny místně redundantní zálohy `Disabled`. 
