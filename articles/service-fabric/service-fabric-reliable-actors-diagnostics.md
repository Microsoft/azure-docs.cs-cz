...
V případě `Service Fabric Actor`kategorie jsou názvy instancí čítače v následujícím formátu:
...
Následuje příklad názvu instance čítače pro čítač, který patří do kategorie `Service Fabric Actor`:

`2740af29-78aa-44bc-a20b-7e60fb783264_635650083799324046`

V předchozím příkladu je `2740af29-78aa-44bc-a20b-7e60fb783264` řetězcové vyjádření Service Fabricho oddílu ID a `635650083799324046` je identifikátor 64, který je generován pro interní použití modulu runtime.

#### <a name="service-fabric-actor-method-category"></a>Kategorie metody Service Fabric objektu actor
V případě `Service Fabric Actor Method`kategorie jsou názvy instancí čítače v následujícím formátu:
...
Následuje příklad názvu instance čítače pro čítač, který patří do kategorie `Service Fabric Actor Method`:

`ivoicemailboxactor.leavemessageasync_2_89383d32-e57e-4a9b-a6ad-57c6792aa521_635650083804480486`

Ve výše uvedeném příkladu je `ivoicemailboxactor.leavemessageasync` název metody `2` je 32 identifikátor ID generovaný pro interní použití modulu runtime, `89383d32-e57e-4a9b-a6ad-57c6792aa521` je řetězcová reprezentace Service Fabric ID oddílu a `635650083804480486` je 64 bitová ID generovaná pro interní použití modulu runtime.
...
| --- | --- | --- | --- | --- |
| ActorMethodStart |7 |Podrobnosti |0x2 |Modul runtime objektů Actors se chystá vyvolat metodu objektu actor. |
| ActorMethodStop |8 |Podrobnosti |0x2 |Metoda objektu actor byla dokončena. To znamená, že bylo vráceno asynchronní volání modulu runtime do metody actor a úloha vrácená metodou actor byla dokončena. |
...
| Název události | ID události | Úroveň | Klíčové slovo | Popis |
| --- | --- | --- | --- | --- |
| ActorMethodCallsWaitingForLock |12 |Podrobnosti |0x8 |Tato událost je zapsána na začátku každého nového objektu actor. Obsahuje počet čekajících volání objektu actor, která čekají na získání zámku na základě objektu actor, který vynutil souběžnost založenou na funkci. |
...
| Název události | ID události | Úroveň | Klíčové slovo | Popis |
| --- | --- | --- | --- | --- |
| ActorSaveStateStart |10 |Podrobnosti |0x4 |Modul runtime objektů Actors se chystá uložit stav objektu actor. |
| ActorSaveStateStop |11 |Podrobnosti |0x4 |Modul runtime objektů actor dokončil ukládání stavu objektu actor. |
