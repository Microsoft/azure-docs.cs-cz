...
> [!NOTE]
> Vyhněte se použití proměnných v parametru `Name` `Get-AutomationVariable` v sadě Runbook nebo konfiguraci DSC. Použití tohoto parametru může zkomplikovat zjišťování závislostí mezi sadami Runbook a konfigurací DSC a proměnných automatizace v době návrhu.
...
> [!NOTE]
> Abyste mohli získat přístup k funkcím assetu, musíte naimportovat modul `automationassets` v horní části sady Runbook Pythonu.
...
Aktivita `Set-AutomationVariable` slouží k nastavení hodnoty proměnné automatizace v Runbooku PowerShellu nebo v konfiguraci DSC a `Get-AutomationVariable` ji načíst. V sadě Runbook nebo v konfiguraci DSC byste neměli používat rutiny `Set-AzAutomationVariable` a `Get-AzAutomationVariable` ani jejich ekvivalenty AzureRM modulu, protože jsou méně efektivní než aktivity pracovního postupu. 
...
Jediným způsobem, jak vytvořit novou proměnnou v rámci sady Runbook nebo konfigurace DSC, je použití rutiny `New-AzAutomationVariable`.
