...
Příklady vyžadují AZ PowerShell Module `Az.Storage (1.1.0 or greater)` a `Az.Resources (1.2.0 or greater)`. V okně PowerShellu spusťte `Get-Module -ListAvailable Az*` a najděte verzi. Pokud se nic nezobrazí nebo potřebujete upgrade, přečtěte si téma [Instalace modulu Azure PowerShell](/powershell/azure/install-az-ps).

> [!IMPORTANT]
> Použití této funkce Azure z PowerShellu vyžaduje, abyste měli nainstalovaný modul `Az`. Aktuální verze `AzTable` není kompatibilní s starším modulem AzureRM.
...
> [!IMPORTANT]
> V případě důvodů kompatibility názvů modulů stále probíhá publikování stejného modulu pod starým názvem `AzureRmStorageTables` v Galerie prostředí PowerShell. Tento dokument bude odkazovat pouze na nový název.
...
Chcete-li vytvořit tabulku, použijte rutinu [New-AzStorageTable](/powershell/module/az.storage/New-AzStorageTable) . V tomto příkladu je tabulka volána `pshtesttable`.
...
> [!IMPORTANT]
> Při práci s modulem PowerShellu pro **AzTable** je použití cloudové části povinné. Chcete-li získat odkaz na tento objekt, zavolejte příkaz **Get-AzTableTable** . Tento příkaz také vytvoří tabulku, pokud ještě neexistuje.
