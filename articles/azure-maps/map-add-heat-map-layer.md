...
* `radius`: Definuje pixel radius, ve kterém k vykreslení datových bodů. Poloměr je možné nastavit jako číslo s pevným nebo jako výraz. Nastavení poloměr jako výraz, který nastavuje poloměr podle úrovně přiblížení může mít za následek heat mapy, které pravděpodobně radius, který představuje konzistentní prostorových oblasti na mapě.
* `color`: Určuje, jak barevně zvýrazněné heat mapa. Palety barev přechodu se často používá pro heat mapy, ale stupňovitým barev, které jsou také užitečné, pokud mají být váš heat mapa vypadat více jako tvarování dat palety. Tyto barvy palety definovat barvy od minima k hodnotě maximální hustoty. Hodnoty barev pro heat mapy jsou určeny jako výraz na `heatmap-density` hodnotu. Barvy v indexu 0 ve výrazu přechodu nebo výchozí barvu barevné krok definuje barvu, oblasti, kde není žádná data nebo barvu pozadí. Mnoho chtít nastavit tuto hodnotu a je transparentní nebo poloprůhledného black. Tady jsou příklady výrazů barva;
