---
title: Použití Azure HDInsight Hadoop clusteru na 1 TB datové sady – proces týmu pro datové vědy
...
## <a name="dataset"></a>Popis datové sady Criteo
Criteo data jsou datovou sadou předpovědi kliknutí, která je přibližně 370GB komprimovaných souborů TSV s gzip (~ 1.3 TB Uncompressed), a to zahrnující více než 4 300 000 000 záznamů. Vychází ze 24 dnů od kliknutí na data, která jsou k dispozici v [Criteo](https://labs.criteo.com/downloads/download-terabyte-click-logs/). Pro usnadnění práce s odborníky přes data byla k dispozici data, která jsme mohli experimentovat.
...
**Definice:** možnost pro *interaktivní účely (centrum):* Toto je procento kliknutí v datech. V této Criteo datové sadě má centrum přibližně 3,3% nebo 0,033.
...
## <a name="setup"></a>Nastavení clusteru HDInsight Hadoop pro datové vědy
**Poznámka:** Obvykle se jedná o úlohu **správce** .
...
1. [Vytvoření účtu úložiště](../../storage/common/storage-quickstart-create-account.md): Tento účet úložiště se používá k ukládání dat v Azure Blob Storage. Zde jsou uložená data používaná v clusterech HDInsight.
2. [Přizpůsobení Azure HDInsight Hadoopch clusterů pro datové vědy](customize-hadoop-cluster.md): Tento krok vytvoří Azure HDInsight Hadoop cluster s 64. Anaconda Python 2,7 nainstalovanou na všech uzlech. Existují dva důležité kroky (popsané v tomto tématu) k dokončení přizpůsobení clusteru HDInsight.
   ...
   * Účet úložiště, který jste vytvořili v kroku 1, musíte propojit s clusterem HDInsight při jeho vytvoření. Tento účet úložiště se používá pro přístup k datům, která se dají zpracovat v rámci clusteru.
   * Po vytvoření musíte povolit vzdálený přístup k hlavnímu uzlu clusteru. Zapamatování přihlašovacích údajů vzdáleného přístupu, které tady zadáte (liší se od těch, které jsou zadané pro cluster při jeho vytvoření): k provedení následujících postupů budete potřebovat.
...
## <a name="getdata"></a>Získání a využití dat z veřejného zdroje
K datové sadě [Criteo](https://labs.criteo.com/downloads/download-terabyte-click-logs/) lze získat pøístup kliknutím na odkaz, přijetím podmínek použití a zadáním názvu. Snímek toho, co vypadá jako, je zobrazený tady:
...
Data se nachází ve veřejném umístění [úložiště objektů BLOB v Azure](../../storage/blobs/storage-dotnet-how-to-use-blobs.md) : wasb://criteo@azuremlsampleexperiments.blob.core.windows.net/raw/. "Wasb" odkazuje na umístění Azure Blob Storage.

1. Data v tomto veřejném úložišti objektů BLOB se skládají ze tří podsložek nekomprimovaných dat.

   1. Podsložka *nezpracované/počet/* obsahuje prvních 21 dnů od dne\_00 do dne\_20
   2. Podsložka *raw/vlak/* sestává z jednoho dne dat, dne\_21
   3. Podsložka *raw/test/* se skládá ze dvou dnů data, dne\_22 a dne\_23
2. Pro ty, kteří chtějí začít s nezpracovanými daty gzip, jsou k dispozici také v hlavní složce *raw/* as day_NN. gz, kde NN začíná od 00 do 23.
...
## <a name="hive-db-tables"></a>Vytvoření databáze a tabulek podregistru
...
Následující kód vytvoří databázi "Criteo" a pak vygeneruje 4 tabulky:

* *tabulka pro generování počtů* na základě dnů od dne\_00 do dne\_20,
* tabulka, která *se má použít jako datová sada vlaku* postavená na dni\_21 a
* dvě *tabulky, které se použijí jako testovací datové sady* postavené na den\_22 a den\_23.
...
[&#95;Podregistr&#95;Sample Create&#95;Criteo&#95;Database&#95;a&#95;Tables. HQL](https://github.com/Azure/Azure-MachineLearning-DataScience/blob/master/Misc/DataScienceProcess/DataScienceScripts/sample_hive_create_criteo_database_and_tables.hql) se tady zobrazí pro usnadnění:
...
Všechny tyto tabulky jsou externí, takže můžete jednoduše odkazovat na jejich umístění Azure Blob Storage (wasb).

**Existují dva způsoby, jak spustit libovolný dotaz z podregistru:**

1. **Pomocí příkazového řádku REPL podregistru**: první z nich je vydání příkazu "podregistr" a zkopírování a vložení dotazu do příkazového řádku REPL v podregistru. Uděláte to takto:

        cd %hive_home%\bin
        hive

     Nyní na příkazovém řádku REPL provede vyjmutí a vložení dotazu.
2. **Ukládání dotazů do souboru a spuštění příkazu**: druhým je uložit dotazy do souboru. HQL ([&#95;ukázkový&#95;podregistr Create&#95;Criteo&#95;Database&#95;and&#95;Tables. HQL](https://github.com/Azure/Azure-MachineLearning-DataScience/blob/master/Misc/DataScienceProcess/DataScienceScripts/sample_hive_create_criteo_database_and_tables.hql)) a pak vydejte následující příkaz pro spuštění dotazu:
...
## <a name="exploration"></a>Zkoumání dat v podregistru
...
Obsah [ukázkového typu&#95;&#95;podregistru&#95;ukázka&#95;ukázek&#95;tabulek. HQL](https://github.com/Azure/Azure-MachineLearning-DataScience/blob/master/Misc/DataScienceProcess/DataScienceScripts/sample_hive_count_train_table_examples.hql) se tady zobrazují:
...
Příkaz, který se má provést, je podobný jako ten, který je právě zobrazený (podívejte se na [vzorový&#95;podregistr&#95;Count&#95;&#95;Criteo test&#95;Day&#95;23&#95;Examples. HQL](https://github.com/Azure/Azure-MachineLearning-DataScience/blob/master/Misc/DataScienceProcess/DataScienceScripts/sample_hive_count_criteo_test_day_23_examples.hql)):
...
Distribuce popisků v datové sadě vlaků je zajímavá. Pokud to chcete zobrazit, zobrazte obsah [&#95;ukázkového&#95;podregistru&#95;Criteo&#95;&#95;popisek&#95;distribuce – tabulka. HQL](https://github.com/Azure/Azure-MachineLearning-DataScience/blob/master/Misc/DataScienceProcess/DataScienceScripts/sample_hive_criteo_label_distribution_train_table.hql):
...
Všimněte si, že procentuální hodnota pozitivních popisků je přibližně 3,3% (v souladu s původní datovou sadou).
...
Chcete-li zjistit, co vypadá, jak bude rozdělení číselných proměnných vypadat, můžete použít funkci "\_histogramu nativního" v podregistru. Tady je obsah [ukázkového&#95;&#95;Criteo&#95;histogramu&#95;s číslem. HQL](https://github.com/Azure/Azure-MachineLearning-DataScience/blob/master/Misc/DataScienceProcess/DataScienceScripts/sample_hive_criteo_histogram_numeric.hql):
...
Kombinovaný pohled – kombinace v podregistru slouží k vytvoření výstupu podobného SQL namísto normálního seznamu. Všimněte si, že v této tabulce odpovídá první sloupec centru bin a druhým až do frekvence přihrádek.
...
Také úroky s číselnými proměnnými je výpočet přibližných percentilů. Nativní "percentil\_v podregistru v tomto případě je to pro nás. Obsah [&#95;ukázkového podregistru&#95;Criteo&#95;přibližné&#95;percentily. HQL](https://github.com/Azure/Azure-MachineLearning-DataScience/blob/master/Misc/DataScienceProcess/DataScienceScripts/sample_hive_criteo_approximate_percentiles.hql) jsou:
...
Počet souběžných výskytů párů proměnných kategorií je také zajímavá. To se dá určit pomocí kódu v [ukázkovém&#95;&#95;podregistru&#95;Criteo spárované&#95;kategorií&#95;počty. HQL](https://github.com/Azure/Azure-MachineLearning-DataScience/blob/master/Misc/DataScienceProcess/DataScienceScripts/sample_hive_criteo_paired_categorical_counts.hql):
...
## <a name="downsample"></a>Debrat ukázku datových sad pro Azure Machine Learning
...
[&#95;Podregistr&#95;Sample&#95;Criteo převzorkuje&#95;zkušební&#95;den&#95;22&#95;DataSet. HQL](https://github.com/Azure/Azure-MachineLearning-DataScience/blob/master/Misc/DataScienceProcess/DataScienceScripts/sample_hive_criteo_downsample_test_day_22_dataset.hql) pro testovací data, den\_22:
...
Nakonec [&#95;podregistr&#95;Sample Criteo&#95;převzorkuje&#95;zkušební&#95;den&#95;23&#95;DataSet. HQL](https://github.com/Azure/Azure-MachineLearning-DataScience/blob/master/Misc/DataScienceProcess/DataScienceScripts/sample_hive_criteo_downsample_test_day_23_dataset.hql) pro testovací data, den\_23:
...
Před přechodem na Azure Machine Learning, která se týká tabulky Count, existuje finální důležitá součást. V další dílčí části je popsána tabulka Count v podrobnostech.

## <a name="count"></a>Stručná diskuze o tabulce Count
Jak jste viděli, několik proměnných kategorií má velmi vysokou dimenzionální hodnotu. V tomto návodu se jedná o výkonnou techniku [s názvem učení se](https://blogs.technet.com/b/machinelearning/archive/2015/02/17/big-learning-made-easy-with-counts.aspx) zakódováním těchto proměnných efektivním a robustním způsobem. Další informace o této technice najdete v zadaném odkazu.
...
Začněte tím, že vyberete **+ nový** -> **experiment** -> **prázdné experimenty**. Pak z **vyhledávacího** pole v levém horním rohu vyhledejte "Import dat". Přetažením modulu pro **Import dat** na plátno experimentu (střední část obrazovky) použijte modul pro přístup k datům.
...
2. V poli **dotaz databáze podregistru** jednoduchého příkazu SELECT * FROM <\_\_název databáze. tabulka\_\_název >-je dostatečně velká.
...
7. **Název účtu služby Azure Storage**: účet úložiště přidružený ke clusteru
8. **Klíč účtu úložiště Azure**: klíč účtu úložiště přidruženého ke clusteru.
...
### <a name="step2"></a>Krok 2: vytvoření jednoduchého experimentu v Azure Machine Learning k předvídání kliknutí/nekliknutí
...
Nyní prověřte klíčové komponenty tohoto experimentu. Nejprve přetáhněte náš uložený vlak a testovací datové sady na náš plátno experimentu.
...
![sestavování vlastností transformačního modulu](./media/hive-criteo-walkthrough/e0eqKtZ.png)
![sestavení pro počítání transformačních modulů](./media/hive-criteo-walkthrough/OdDN0vw.png)
...
Teď je zaměření na důležité místo pro transformaci našeho testu a testovacích dat před skutečným vygenerováním funkcí Count. Všimněte si, že se používají dva moduly pro **spuštění skriptu jazyka R** před tím, než se pro naše data použije transformace Count.
...
V tomto jednoduchém skriptu jazyka R se k nastavení množství vyvážení mezi kladnou a zápornou třídou používá "POS\_záporné\_poměr). To je důležité udělat, protože vylepšení nerovnováhy tříd má obvykle vliv na výkon při klasifikacích, kde je rozdělení třídy nakloněné (v tomto případě je to v tomto případě možné, že máte 3,3% pozitivní třídy a 96,7% negativní třídu).
...
### <a name="step3"></a>Krok 3: sestavování, výuka a určení skóre modelu
...
Pro experiment vyberte výchozí hodnoty. Všimněte si, že výchozí hodnoty jsou obvykle smysluplné a dobrý způsob, jak získat rychlé základní hodnoty pro výkon. Pokud se rozhodnete, že budete mít směrný plán, můžete vylepšit výkon pomocí úklidu parametrů.
...
Nakonec byste měli analyzovat výkon modelu. Pro dva problémy klasifikace třídy (binární) obvykle je vhodná míra AUC. Pokud to chcete vizualizovat, zapojte modul určení **skóre modelu** do modulu **vyhodnocení modelu** . Kliknutí na **vizualizaci** v modulu **vyhodnocení modelu** poskytuje obrázek podobný následujícímu:

![Zhodnotit model BDT modulu](./media/hive-criteo-walkthrough/0Tl0cdg.png)

V binárních problémech klasifikace (nebo dvou tříd) je dobrým měřítkem přesnosti předpovědi plocha pod křivkou (AUC). V následující části jsou uvedeny výsledky pomocí tohoto modelu v naší datové sadě testů. To získáte tak, že kliknete pravým tlačítkem na výstupní port modulu **vyhodnocení modelu** a pak **vizualizujete**.
...
Pokud to chcete provést, nejdřív uložte náš vycvičený model jako objekt trained model. To uděláte tak, že kliknete pravým tlačítkem na modul **vlakového modelu** a použijete možnost **Uložit jako vycvičený model** .
...
Jako zeroth krok, protože tabulka Count je rozsáhlá, pár řádků testovacích dat a generování výstupních dat s využitím funkcí Count. To může sloužit jako formát vstupních dat pro naši webovou službu. Toto je znázorněno zde:
...
Jako první se zobrazí, jak vypadá. Základní struktura je modul **bodového modelu** , který přijímá náš objekt trained model a několik řádků vstupních dat, která byla vygenerována v předchozích krocích pomocí modulu **Count Featurizer** . Pomocí příkazu Vybrat sloupce v datové sadě můžete vyznačit popisky s skóre a pravděpodobnosti skóre.
...
Kliknutím na odkaz **požadavek/odpověď** přejdete na stránku, která nám poskytuje předem zakonzervovaný kód v C#, Python a R. Tento kód se dá pohodlně použít k volání webové služby. Všimněte si, že klíč rozhraní API na této stránce se musí použít k ověřování.
...
Všimněte si, že výchozí klíč rozhraní API byl nahrazen náš klíč rozhraní API WebServices. Kliknutím na **Spustit** v této buňce v IPython poznámkovém bloku vyplníte následující odpověď:

![IPython odpověď](./media/hive-criteo-walkthrough/KSxmia2.png)

Pro dva příklady testů, které se týkají (v rozhraní JSON skriptu Pythonu), získáte odpovědi ve formě "popisků s skóre, pravděpodobnosti skóre". V tomto případě byly zvoleny výchozí hodnoty, které zakonzervovaný kód poskytuje (0 pro všechny číselné sloupce a řetězec "hodnota" pro všechny sloupce kategorií).

Tím se dokončí náš Názorný postup, který ukazuje, jak zpracovat rozsáhlou datovou sadu pomocí Azure Machine Learning. Začali jste s terabajty dat, vytvářeli předpovědní model a nasadili jste ho jako webovou službu v cloudu.
