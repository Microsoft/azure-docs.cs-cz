...
| [MsalServiceException](/dotnet/api/microsoft.identity.client.msalserviceexception?view=azure-dotnet) | AADSTS70002: Tělo žádosti musí obsahovat následující parametr: ' client_secret nebo client_assertion '.| K této výjimce může dojít, pokud nebyla vaše aplikace registrována jako veřejná klientská aplikace v Azure AD. V Azure Portal upravte manifest pro aplikaci a nastavte `allowPublicClient` `true`na. |
...
* *ClientAuthError:* Třída Error, která označuje problém s ověřením klienta. Většina chyb, které pocházejí z knihovny, bude ClientAuthErrors. Tyto chyby jsou jako volání metody přihlášení, když probíhá přihlášení, uživatelé ruší přihlášení a tak dále. 

* *ClientConfigurationError:* Třída Error rozšířící ClientAuthError vyvolala před provedením požadavků, pokud jsou zadané parametry konfigurace uživatele poškozené nebo chybí.
...
* *InteractionRequiredAuthError:* Třída Error rozšiřuje ServerError tak, aby představovala chyby serveru, které vyžadují interaktivní volání. Tato chyba je vyvolána, `acquireTokenSilent` Pokud je uživatel nutný k interakci se serverem za účelem poskytnutí pověření nebo souhlasu pro ověřování/autorizaci. Kódy chyb zahrnují "interaction_required", "login_required", "consent_required".
...
Vrátí se chyba, když se pokusíte použít neinteraktivní metodu získání tokenu (například `acquireTokenSilent`) a MSAL ho nedokázali v tichém režimu.

Možné důvody:

* Uživatel se musí přihlásit.
* uživatel musí souhlasit.
* uživatel musí projít prostředím Multi-Factor Authentication.
...
Pokud je server tokenu služby (STS) přetížený s příliš velkým počtem požadavků, vrátí chybu protokolu HTTP 429 s pomocným parametrem, kdy se můžete pokusit znovu za chvíli. Chybu lze číst z `Retry-After` pole Response.
