---
title: Základní hodnoty zabezpečení pro Azure synapse Analytics
description: Základní plán zabezpečení Azure pro synapse Analytics
author: msmbaldwin
ms.service: security
ms.topic: conceptual
ms.date: 06/22/2020
...
**Pokyny**: pomocí protokolu aktivit Azure můžete monitorovat konfigurace síťových prostředků a zjišťovat změny síťových prostředků, které se týkají vašeho fondu SQL Azure synapse. Vytvoří výstrahy v rámci Azure Monitor, které se aktivují, když budou provedeny změny v kritických síťových prostředcích.
...
**Pokyny**: zásady auditu je možné definovat pro určitou databázi nebo jako výchozí zásady serveru v Azure (které hostují SQL Database nebo Azure synapse). Zásady serveru se vztahují na všechny existující a nově vytvořené databáze na serveru.
...
**Doprovodné**materiály: Povolte auditování na úrovni serveru SQL Azure pro synapse fond SQL a vyberte umístění úložiště pro protokoly auditu (Azure Storage, Log Analytics nebo Event Hubs).
...
**Doprovodné**materiály: při ukládání protokolů souvisejících s vaším synapse fondem SQL v účtu úložiště, Log Analytics pracovním prostoru nebo centra událostí nastavte dobu uchování protokolu podle předpisů pro dodržování předpisů vaší organizace.
...
* [Jak nakonfigurovat vlastní výstrahy pro Azure SQL Database](https://docs.microsoft.com/azure/sql-database/sql-database-insights-alerts-portal?view=azps-1.4.0)
...
**Doprovodné**materiály: nepoužitelné; pro prostředky, které souvisejí s vaším synapse fondem SQL, je řešení proti malwaru spravované Microsoftem na podkladové platformě.
...
**Doprovodné**materiály: nepoužitelné; prostředky týkající se vašeho Synapseho fondu SQL nevytvářejí žádné protokoly DNS.
...
**Pokyny**: řízení přístupu na základě role v Azure (Azure RBAC) obsahuje předdefinované role, které se musí explicitně přiřadit a které jsou Queryable. Pomocí modulu Azure AD PowerShell můžete provádět dotazy ad hoc a zjišťovat účty, které jsou členy skupin pro správu.

Role můžete také konfigurovat na úrovni databáze a také vytvářet vlastní role pro uživatele, kteří přistupují k prostředkům Azure SQL Database. Role na úrovni serveru nejsou v současnosti pro Azure synapse SQL k dispozici.
...
* [Principy rolí na úrovni databáze](https://docs.microsoft.com/sql/relational-databases/security/authentication-access/database-level-roles?view=sql-server-ver15)
...
**Doprovodné**materiály: Azure Active Directory nemá koncept výchozích hesel. Při zřizování Azure SQL Database/serveru nebo synapse fondu SQL se doporučuje, abyste se rozhodli integrovat ověřování pomocí Azure Active Directory.
...
* [Pochopení SQL Server auditování](https://docs.microsoft.com/sql/relational-databases/security/auditing/sql-server-audit-database-engine?view=sql-server-ver15)
...
**Pokyny**: vytvořte správce Azure Active Directory (AD) pro server Azure SQL Database ve vašem synapse fondu SQL.
...
**Doprovodné**materiály: ve scénářích podpory, kde Microsoft potřebuje získat přístup k datům souvisejícím s Azure SQL Database ve vašem synapse fondu SQL, poskytuje Azure Customer Lockbox rozhraní pro kontrolu a schválení nebo zamítnutí požadavků na přístup k datům.
...
**Doprovodné**materiály: pro jakékoli Azure SQL Database ve fondu SQL synapse ukládání nebo zpracování citlivých informací označte databázi a související prostředky jako citlivé pomocí značek. Nakonfigurujte privátní propojení ve spojení s značkami služby skupiny zabezpečení sítě (NSG) na vašich instancích Azure SQL Database, abyste zabránili exfiltrace citlivých informací.
...
**Pokyny**: pomocí Azure Resource graphu můžete v rámci svých předplatných dotazovat a zjišťovat všechny prostředky, které souvisejí s vaším synapse fondem SQL. Ujistěte se, že máte ve svém tenantovi příslušná oprávnění (pro čtení) a že máte v rámci předplatných také výčet všech předplatných Azure i prostředků.
...
**Doprovodné**materiály: jakýkoliv prostředek, který souvisí s vaším synapse fondem SQL, který je potřeba pro obchodní operace, ale může pro organizaci zvýšit riziko, měli byste se izolovat v rámci svého vlastního virtuálního počítače nebo virtuální sítě a dostatečně zabezpečit pomocí Azure firewall nebo skupiny zabezpečení sítě.
...
**Doprovodné**materiály: využijte Azure Security Center k provádění kontrol standardních hodnot pro všechny prostředky, které se vztahují k vašemu synapse fondu SQL.
...
**Doprovodné**materiály: pravidelně testujte body obnovení, abyste měli jistotu, že jsou vaše snímky platné. Chcete-li obnovit existující fond SQL z bodu obnovení, můžete použít Azure Portal nebo PowerShell. Test obnovení zálohovaných klíčů spravovaných zákazníkem

* [Postup obnovení klíčů Azure Key Vault](https://docs.microsoft.com/powershell/module/azurerm.keyvault/restore-azurekeyvaultkey?view=azurermps-6.13.0)

* [Zálohování a obnovení ve fondu SQL Azure synapse](https://docs.microsoft.coms/azure/synapse-analytics/sql-data-warehouse/backup-and-restore)

* [Postup obnovení existujícího fondu SQL](https://docs.microsoft.com/azure/synapse-analytics/sql-data-warehouse/sql-data-warehouse-restore-active-paused-dw)
...
**Doprovodné**materiály: řiďte se prosím [pravidly zapojení Microsoftu](https://www.microsoft.com/msrc/pentest-rules-of-engagement?rtc=1) , abyste zajistili, že testy průniku nejsou v rozporu s zásadami Microsoftu.

Další informace o strategii Microsoftu a provádění testování průniku v reálném čase s využitím cloudové infrastruktury Microsoft spravovaného cloudu najdete [tady](https://gallery.technet.microsoft.com/Cloud-Red-Teaming-b837392e).
