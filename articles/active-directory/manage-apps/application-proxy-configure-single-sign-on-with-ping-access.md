...
Azure Proxy aplikace služby Active Directory a PingAccess společně společně poskytují zákazníkům Azure Active Directory přístup k i další aplikace. PingAccess rozšíří [stávající nabídky Proxy aplikace](application-proxy.md) zahrnout jednoho přístupového přihlášení do aplikace, které používají hlavičky pro ověřování.
...
Konektor Proxy aplikace je služba systému Windows Server, která přesměruje přenosy z vzdálení zaměstnanci k publikovaným aplikacím. Podrobné pokyny k instalaci, najdete v části [povolení Proxy aplikace v portálu Azure](application-proxy-enable.md).
...
3. Vyberte **stáhnout konektor** zahájíte stahování konektoru Proxy aplikace. Postupujte podle pokynů pro instalaci.
   ...
   ![Povolení Proxy aplikace a stáhnout konektor](./media/application-proxy-configure-single-sign-on-with-ping-access/install-connector.png)
...
10. Zvolte **na základě záhlaví přihlašování** z rozevírací nabídky. Vyberte **Uložit**.
   ...
   ![Vyberte na základě záhlaví přihlášení](./media/application-proxy-configure-single-sign-on-with-ping-access/sso-header.PNG)
...
12. Vyberte **Registrace aplikací**.
   ...
   ![Vyberte aplikaci registrace](./media/application-proxy-configure-single-sign-on-with-ping-access/app-registrations.png)
...
1. V okně nastavení vaší aplikace, vyberte **vlastnosti**. 
  ...
  ![Vyberte vlastnosti](./media/application-proxy-configure-single-sign-on-with-ping-access/properties.png)
...
- [Jak Azure AD Application Proxy poskytovat jednotné přihlašování?](application-proxy-single-sign-on.md)
