...
Otevřete prohlížeč a `http://localhost:3000`.
...
7. Přidejte vzdálené úložiště k klonu, který odkazuje na úložiště Git webové aplikace v App Service. Pro\>adresu URL klonu Git \<použijte adresu URL vrácenou v předchozím kroku. Spusťte následující příkaz v příkazovém okně.
...
* Z webové aplikace v Azure Portal v části **vývojové nástroje** vyberte **Konzola** a ověřte verze uzlů a npm pomocí `node -v` a `npm -v`.

* Pokud se zobrazí chyba týkající se nenalezení balíčku, možná jste spustili kroky mimo pořadí. Při nasazení lokality (s `git push`) služba App Service spouští `npm install`, která se spouští na základě aktuální verze uzlu, na kterém je nakonfigurované. Pokud se později v konfiguraci změní, budete muset provést nesmyslnou změnu kódu a znovu ho vložit.
