...
| cognitiveservices Account. Vision. spatialanalysis-personcrossingpolygon | Sleduje, když osoba v poli zobrazení kamery přeškrtne určený řádek. <br> Vygeneruje událost _personLineEvent_ , když osoba přeškrtne zónu a poskytne směrové informace. |
...
| cognitiveservices Account. Vision. spatialanalysis-personcrossingpolygon. livevideoanalytics | Sleduje, když osoba v poli zobrazení kamery přeškrtne určený řádek. <br> Vygeneruje událost _personLineEvent_ , když osoba přeškrtne zónu a poskytne směrové informace. |
...
> [!IMPORTANT]
> Modely AI pro počítačové zpracování obrazu zjišťují a hledají přítomnost lidí v video záběrech a výstupech pomocí ohraničujícího rámečku kolem lidského těla. Modely AI se nepokusí detekovat obličeje ani zjistit identity nebo demografické údaje jednotlivců.
...
| VIDEO_URL| Adresa URL služby RTSP pro zařízení kamery (příklad: `rtsp://username:password@url` ) Prostorová analýza podporuje kódovaný datový proud H. 264 buď prostřednictvím protokolu RTSP, http nebo MP4. |
...
| `threshold` | float| Pokud je spolehlivost modelů AI větší nebo rovna této hodnotě, dojde k odchozímu přenosu událostí. |
| `type` | řetězec| Pro **cognitiveservices Account. Vision. spatialanalysis-personcrossingpolygon** by to mělo být `enter` nebo `exit` .|
...
| `status` | řetězec| ' Enter ' nebo ' Exit '|
| `side` | int| Číslo strany mnohoúhelníku, kterou osoba přejíždí|
...
| `status` | řetězec | Označuje, zda je kalibrace kamery na základní rovinu dokončená.|
...
> [!IMPORTANT]
> Model AI detekuje osobu bez ohledu na to, jestli je někdo na kameře nebo pryč. Model AI nespouští detekci nebo rozpoznávání tváře a negeneruje žádné biometrické informace. 
...
| `points` |  – kolekce| Levý horní a dolní pravý bod, pokud je typ oblasti obdélník |
| `confidence` | float| Spolehlivost algoritmu|
| `centerGroundPoint` | 2 hodnoty float| `x`, `y` hodnoty se souřadnicemi odvozených umístění osoby na zemi ve stopách. `x` je vzdálenost od kamery kolmá k rovině obrazu kamery, která se nachází na desce ve stopách. `y` je vzdálenost od kamery rovnoběžná s rovinou obrazu, která je rozmístěná na desce ve stopách.|
