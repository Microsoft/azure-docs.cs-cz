...
1. Identita objektu zabezpečení je ověřena a je vrácen token OAuth 2,0.  Název prostředku pro vyžádání tokenu je `https://login.microsoftonline.com/{tenantID}`, kde `{tenantID}` odpovídá ID tenanta Azure Active Directory, ke kterému objekt služby patří.
