...
* Vývojový počítač se Java SE Development Kit 8 nebo novějším. Můžete si stáhnout Java 8 (LTS) JDK pro více platforem od [stažení Zulu Builds OpenJDK](https://www.azul.com/downloads/zulu-community/).
...
V textovém editoru otevřete na kopii sady Microsoft Azure IoT SDK pro jazyk Java, kterou jste si stáhli dříve, soubor *Azure-IoT-SDK-Java/Device/IoT-Device-Samples/PNP-Device-Sample/Temperature-Controller-Device-Sample/src/Main/Java/Samples/com/Microsoft/Azure/SDK/IoT/Device/TemperatureController. Java* .
...
* Voláním `initializeAndProvisionDevice` pro nastavení `dtmi:com:example:TemperatureController;2` ID modelu použijte DPS ke zřízení a registraci zařízení, vytvoření instance **DeviceClient** a připojení k aplikaci IoT Central. IoT Central používá ID modelu k identifikaci nebo generování šablony zařízení pro toto zařízení. Další informace najdete v tématu [přidružení zařízení k šabloně zařízení](../articles/iot-central/core/concepts-get-connected.md#associate-a-device-with-a-device-template).
* Vytvoří obslužné rutiny příkazů `getMaxMinReport` pro `reboot` příkazy a.
* Vytvoří obslužné rutiny aktualizace vlastností pro zapisovatelné `targetTemperature` Vlastnosti.
* Pošle počáteční hodnoty pro vlastnosti v rozhraní **informace o zařízení** a vlastnosti **paměti** a **sériového čísla** zařízení.
* Spustí vlákno k odeslání telemetrie teploty ze dvou termostatů a aktualizuje `maxTempSinceLastReboot` vlastnost každých pět sekund.
...
`sendTemperatureTelemetry`Metoda ukazuje, jak zařízení odesílá telemetrii teploty ze součásti do IoT Central. Tato metoda používá `PnpConvention` třídu k vytvoření zprávy:
...
`updateMaxTemperatureSinceLastReboot`Metoda pošle `maxTempSinceLastReboot` aktualizaci vlastnosti ze součásti do IoT Central. Tato metoda používá `PnpConvention` třídu k vytvoření opravy:
...
`TargetTemperatureUpdateCallback`Třída obsahuje `TwinPropertyCallBack` metodu pro zpracování aktualizací vlastností s možností zápisu do komponenty z IoT Central. Tato metoda používá `PnpConvention` třídu k vytvoření odpovědi:
...
`MethodCallback`Třída obsahuje `call` metodu pro zpracování příkazů komponenty volaných z IoT Central:
...
Ve Windows přejděte do kořenové složky sady Azure IoT SDK pro úložiště Java, které jste stáhli.
...
Pokud chcete spustit ukázkovou aplikaci, otevřete prostředí příkazového řádku a přejděte do složky *Azure-IoT-SDK-Java/Device/IoT-Device-Samples/PNP-Device-Sample/teplotní-Controller-Device-Sample* , která obsahuje složku *Src* s ukázkovým souborem *TemperatureController. Java* .
