...
Pomocí příkazu [AZ Key trezor Key Create](/cli/azure/keyvault/key?view=azure-cli-latest#az-keyvault-key-create) vytvořte KEK, který má klíčové operace nastavené na `import`. Poznamenejte si identifikátor klíče`kid`() vrácený z následujícího příkazu. (V `kid` [kroku 3](#step-3-generate-and-prepare-your-key-for-transfer)použijete hodnotu.)
