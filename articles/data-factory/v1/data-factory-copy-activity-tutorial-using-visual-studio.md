---
title: 'Kurz: Vytvoření kanálu s aktivitou kopírování pomocí sady Visual Studio '
...
2. Vytvořte v datové továrně vstupní a výstupní **datové sady**.  
    ...
    Podobně Azure SQL Database propojená služba Určuje připojovací řetězec, který Data Factory služba používá v době běhu pro připojení k Azure SQL Database. A výstupní datová sada tabulky SQL určuje tabulku v databázi, do které se kopírují data z úložiště objektů blob.
...
Odkazy propojených služeb Azure SQL Azure SQL Database k datové továrně. Data kopírovaná z úložiště objektů blob se ukládají do této databáze. V této databázi jste v rámci [požadavků](data-factory-copy-data-from-azure-blob-storage-to-sql-database.md) vytvořili tabulku emp.
...
3. V **AzureSqlLinkedService1.jssouborů** nahraďte `<servername>` ,, a `<databasename>` `<username@servername>` `<password>` názvy vašeho serveru, databáze, uživatelského účtu a hesla.    
...
Podobně Azure SQL Database propojená služba Určuje připojovací řetězec, který Data Factory služba používá v době běhu pro připojení k Azure SQL Database. A výstupní datová sada tabulky SQL (OutputDataset) určuje tabulku v databázi, do které se kopírují data z úložiště objektů blob. 
...
3. Nahraďte text JSON následujícím textem a uložte soubor **AzureBlobLocation1.json**. 
   ...
   ``` 
    ...
    | Vlastnost | Popis |
    |:--- |:--- |
    | typ | Vlastnost Type je nastavená na **azureblobu** , protože data se nacházejí v úložišti objektů BLOB anAzure. |
    | linkedServiceName | Odkazuje na **AzureStorageLinkedService** , který youcreated dříve. |
    | folderPath | Určuje **kontejner** objektů BLOB a **složku** thatcontains Input BLOBs. V tomto kurzu je adftutorial kontejnerem objektů BLOB andfolder je kořenová složka. | 
    | fileName | Tato vlastnost je nepovinná. Pokud tuto vlastnost vynecháte, filesfrom se všechny folderPath. V tomto kurzu se pro název souboru zadá **emp.txt** , takže se tento soubor vybral ke zpracování. |
    | format -> type |Vstupní soubor je ve formátu textu, proto používáme **TextFormat**. |
    | columnDelimiter | Sloupce ve vstupním souboru jsou oddělené **commacharacter ( `,` )**. |
    | frequency/interval | Frekvence je nastavená na **Hour (hodina** ) a interval je SETTO **1**, což znamená, že vstupní řezy jsou dostupné **každou hodinu**. V otherwords Služba Data Factory vyhledává vstupní data každou hodinu v RootFolder kontejneru objektů BLOB (**adftutorial**), který jste zadali. Vyhledává TheData v rámci kanálu od času spuštění a ukončení, ne před nebo po thesetimes.  |
    | external | Tato vlastnost je nastavena na **hodnotu true** , pokud data nejsou vygenerována kanálem bythis. Vstupní data v tomto kurzu jsou v souboru emp.txt, který negeneruje tento kanál, takže tuto vlastnost nastavíme na hodnotu true. |
...
V tomto kroku vytvoříte výstupní datovou sadu s názvem **OutputDataset**. Tato datová sada odkazuje na tabulku SQL v Azure SQL Database reprezentované **AzureSqlLinkedService1**. 
...
3. Nahraďte kód JSON následujícím kódem JSON a uložte **CopyActivity1.jsdo** souboru.
...
2. Vytvořili jste **propojené služby**:
   ...
   2. Propojená služba **Azure SQL** , která propojí vaši databázi, která obsahuje výstupní data. 
