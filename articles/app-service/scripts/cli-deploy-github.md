...
description: Zjistěte, jak pomocí azure cli automatizovat nasazení a správu aplikace App Service. Tato ukázka ukazuje, jak nasadit aplikaci z GitHubu.
...
---
# <a name="create-an-app-service-app-with-deployment-from-github-using-azure-cli"></a>Vytvoření aplikace služby App Service s nasazením z GitHubu pomocí rozhraní příkazového příkazu Azure

Tento ukázkový skript vytvoří aplikaci ve službě App Service s jeho související prostředky. Potom nasadí kód aplikace z veřejného úložiště GitHub (bez průběžného nasazení). Nasazení GitHubu s nepřetržitým nasazením najdete [v tématu Vytvoření aplikace s nepřetržitým nasazením z GitHubu](cli-continuous-deployment-github.md).
