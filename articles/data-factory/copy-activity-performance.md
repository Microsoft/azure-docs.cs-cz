...
4. **Tipy pro ladění výkonu a funkce optimalizace.** V některých případech se při spuštění aktivity kopírování v Azure Data Factory zobrazí zpráva "Tipy pro ladění výkonu" na stránce [monitorování aktivity kopírování](copy-activity-overview.md#monitor-visually), jak je znázorněno v následujícím příkladu. Zpráva vás upozorní na kritické body, které se identifikovaly pro daný běh kopírování. Také vám pomůže s tím, co se dá změnit, aby se zvýšila propustnost kopírování. Tipy pro ladění výkonu aktuálně poskytují návrhy jako:
   ...
   **Příklad: kopírování do Azure SQL Database s tipy pro ladění výkonu**
...
| Scénář kopírování | Výchozí DIUs určuje služba |
|:--- |:--- |
...
> [!NOTE]
> Nastavení DIUs větší než čtyři se v současnosti platí jenom při kopírování více souborů z Azure Blob/ADLS Gen1/ADLS Gen2/Amazon S3/Google Cloud Storage/Cloud FTP/cloudu nebo z cloudového úložiště relačních dat s povolenými možnostmi (včetně [Oracle ](connector-oracle.md#oracle-as-source)/[Netezza](connector-netezza.md#netezza-as-source)/[Teradata](connector-teradata.md#teradata-as-source)) do jakýchkoli dalších cloudových úložišť dat.
...
| Scénář kopírování | Výchozí počet paralelně kopírovaných kopií stanovený službou |
| --- | --- |
...
Pro řízení zatížení počítačů, které hostují vaše úložiště dat nebo pro optimalizaci výkonu kopírování, můžete přepsat výchozí hodnotu a zadat hodnotu pro vlastnost **parallelCopies** . Hodnota musí být celé číslo větší nebo rovno 1. V době běhu používá aktivita kopírování hodnotu, která je menší nebo rovna hodnotě, kterou jste nastavili.
