---
title: Ukázky Azure PowerShellu – Aktualizace cloudové služby Azure (s rozšířenou podporou)
description: Ukázkové skripty pro aktualizaci nasazení služby Azure Cloud Service (Rozšířená podpora)
ms.topic: sample
...
## <a name="add-an-extension-to-existing-cloud-service"></a>Přidání rozšíření do existující cloudové služby
```powershell
...
## <a name="remove-all-extensions-from-a-cloud-service"></a>Odebrat všechna rozšíření z cloudové služby
```powershell
# Get existing Cloud Service
$cloudService = Get-AzCloudService -ResourceGroup "ContosOrg" -CloudServiceName "ContosoCS"
...
## <a name="remove-the-remote-desktop-extension-from-cloud-service"></a>Odebrání rozšíření vzdálené plochy z cloudové služby
```powershell
# Get existing Cloud Service
$cloudService = Get-AzCloudService -ResourceGroup "ContosOrg" -CloudServiceName "ContosoCS"
...
## <a name="scale-out--scale-in-role-instances"></a>Role škálování na více instancí a škálování na více instancí
```powershell
# Get existing Cloud Service
$cloudService = Get-AzCloudService -ResourceGroup "ContosOrg" -CloudServiceName "ContosoCS"
