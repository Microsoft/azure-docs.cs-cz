...
Tato základní hodnota zabezpečení platí pro Linux Virtual Machines pokynů od [verze 1.0 služby Azure Security test](../../security/benchmarks/overview-v1.md) . Azure Security Benchmark poskytuje doporučení, jak můžete zabezpečit svá cloudová řešení v Azure. Obsah je seskupen podle **řízení zabezpečení** definovaného srovnávacím testem zabezpečení Azure a souvisejících pokynů, které se vztahují na Linux Virtual Machines. 

> [!NOTE]
> **Ovládací prvky** , které se nevztahují na Linux Virtual Machines nebo pro které je odpovědnost od Microsoftu, jsou vyloučené. Pokud chcete zjistit, jak Linux Virtual Machines kompletně mapuje srovnávací test zabezpečení Azure, přečtěte si **[celý soubor mapování standardních hodnot Linux Virtual Machines Security](https://github.com/MicrosoftDocs/SecurityBenchmarks/raw/master/Azure%20Offer%20Security%20Baselines/1.1/linux-virtual-machines-security-baseline-v1.1.xlsx)**.
...
- [Ukázky Azure Policy pro sítě](/azure/governance/policy/samples/built-in-policies#network)
...
- [Chcete-li zachytit data syslog pro monitorování, bude nutné povolit rozšíření Log Analytics.](https://docs.microsoft.com/azure/azure-monitor/vm/quick-collect-azurevm#enable-the-log-analytics-vm-extension)
...
- [Přehled agenta Log Analytics](/azure/azure-monitor/platform/log-analytics-agent)
...
- [Následující odkaz poskytuje doporučené bezpečnostní pokyny Microsoftu, které můžou sloužit jako seznam kritérií pro vybraný software o ohrožení zabezpečení.](/azure/virtual-machines/linux/security-recommendations)
...
**Doprovodné** materiály: Linux Virtual Machines a Azure Active Directory (Azure AD) nemají koncept výchozích hesel. Zákazník zodpovědný za aplikace třetích stran a služby Marketplace, které mohou používat výchozí hesla.
...
**Doprovodné** materiály: zákazníci, kteří používají jednotné přihlašování (SSO) s Azure Active Directory (Azure AD), nemůžou konfigurovat jednotlivé samostatné přihlašovací údaje pro službu. Použijte Azure Security Center doporučení pro správu identit a přístupu.
...
**Pokyny**: Konfigurace nastavení diagnostiky pro Azure Active Directory (Azure AD) pro odeslání protokolů auditu a protokolů přihlášení do pracovního prostoru Log Analytics K prohlížení protokolů a provádění dotazů na datech protokolu syslog z virtuálních počítačů Azure můžete také použít Azure Monitor.
...
- [Šifrování na straně serveru Azure Managed disks](../disk-encryption.md)
...
- [Doporučené bezpečnostní pokyny pro Microsoft](/azure/virtual-machines/linux/security-recommendations)
...
- [Řešení Update Management v Azure](/azure/automation/overview)

- [Správa aktualizací a oprav pro virtuální počítače Azure](/azure/automation/manage-updates-for-vm)
...
Kromě použití Change Tracking pro monitorování softwarových aplikací umožňují adaptivní řízení aplikací v Azure Security Center pomocí Machine Learning analyzovat aplikace běžící na vašich počítačích a vytvářet povolených z těchto inteligentních informací. Tato funkce značně zjednodušuje proces konfigurace a údržby zásad povolených aplikací, což vám umožní vyhnout se použití nežádoucího softwaru ve vašem prostředí. Můžete nakonfigurovat režim auditování nebo režim vymáhání. Režim auditování Audituje pouze aktivitu na chráněných virtuálních počítačích. Režim vynutilosti vynutil pravidla a zajišťuje, aby aplikace, které nejsou povolené ke spuštění, byly blokované.

- [Diagnostické rozšíření pro virtuální počítače se systémem Linux](https://docs.microsoft.com/azure/virtual-machines/extensions/diagnostics-linux?toc=/azure/azure-monitor/toc.json)
...
- [Porozumění Change Tracking](../../automation/change-tracking/overview.md)
...
- [Jak nahrát vlastní virtuální pevný disk virtuálního počítače do Azure](../windows/upload-generalized-managed.md)
...
**Doprovodné** materiály: Pokud používáte vlastní image (např. virtuální pevný disk), zajistěte, aby k nim měli přístup jenom autorizovaní uživatelé.
...
- [Jak nakonfigurovat Microsoft Antimalware pro Cloud Services a Virtual Machines](/azure/virtual-machines/linux/security-recommendations)
...
**Doprovodné** materiály: Zajistěte, aby bylo možné pravidelně provádět obnovování dat v rámci Azure Backup. V případě potřeby proveďte test obnovení obsahu do izolované virtuální sítě nebo předplatného. Zákazník testuje obnovení zálohovaných klíčů spravovaných zákazníkem.
...
**Pokyny**: při zálohování virtuálních počítačů Azure pomocí Azure Backup jsou virtuální počítače v klidovém stavu šifrované pomocí šifrování služby Storage (SSE). Azure Backup taky můžou zálohovat virtuální počítače Azure, které jsou zašifrované pomocí Azure Disk Encryption. Azure Disk Encryption se taky integruje s klíči šifrovacího klíče Azure Key Vault (KEK). Povolí Soft-Delete v Key Vault k ochraně klíčů proti náhodnému nebo škodlivému odstranění. 
