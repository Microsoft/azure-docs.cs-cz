...
1. Načtěte `TelemetryClient`instancinebo instanci z kontejneru vkládání závislostí (di) voláním `serviceProvider.GetRequiredService<TelemetryClient>();` nebo použitím injektáže konstruktoru. `ILogger` Tento krok spustí nastavení `TelemetryConfiguration` a automatické moduly kolekcí.
...
5. `Worker.cs` Upravte podle níže uvedeného příkladu.
...
2. `services.AddApplicationInsightsTelemetryWorkerService();` Přidejte`ConfigureServices()` do metody, jako v tomto příkladu:
...
Přidejte všechny nové `TelemetryInitializer` `DependencyInjection` kontejnery do kontejneru a sada SDK bude `TelemetryConfiguration`automaticky přidána do nástroje.
...
Získání instance `TelemetryClient` pomocí injektáže konstruktoru a volání požadované `TrackXXX()` metody. Nedoporučujeme vytvářet nové `TelemetryClient` instance. Instance `TelemetryClient` typu Singleton je již `DependencyInjection` v kontejneru zaregistrována, která sdílí `TelemetryConfiguration` se zbytkem telemetrie. Vytvoření nové `TelemetryClient` instance se doporučuje jenom v případě, že potřebuje konfiguraci, která je oddělená od zbytku telemetrie.
