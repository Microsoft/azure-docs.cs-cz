...
| **Postup** | Zabezpečte připojení AMQP nebo HTTP k centru událostí pomocí protokolu SSL/TLS. |
...
| **Atributy**              | –  |
| **Odkazy**              | –  |
| **Postup** | Ověřte oprávnění účtu služby a ověřte, že vlastní služby nebo stránky ASP.NET respektují zabezpečení CRM. |
...
| **Postup** | Služba IdentityServer standardně vyžaduje, aby všechna příchozí připojení mohla být v rámci protokolu HTTPS. Je nezbytně nutné, aby komunikace se IdentityServer prováděla pouze přes zabezpečené přenosy. Existují některé scénáře nasazení, jako je přesměrování zpracování TLS, kde je možné tento požadavek zmírnit. Další informace najdete na stránce věnované nasazení serveru identit v odkazech. |
...
| **Postup** | Povolení šifrování TLS zvyšuje zabezpečení dat přenášených mezi instancemi SQL Server a aplikací mezi sítěmi. |
...
| **Postup** | Pokud má aplikace i vazbu HTTPS i HTTP, klienti stále můžou k přístupu k webu používat protokol HTTP. Pokud tomu chcete zabránit, použijte filtr akcí, abyste zajistili, že požadavky na chráněná rozhraní API jsou vždycky přes protokol HTTPS.|
...
| **Postup** | Zabezpečte protokoly HTTP/AMQP nebo MQTT pomocí protokolu SSL/TLS. |
