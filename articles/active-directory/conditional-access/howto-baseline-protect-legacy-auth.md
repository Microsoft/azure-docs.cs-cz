---
title: Zásada standardních hodnot blokuje starší ověřování (Preview) – Azure Active Directory
description: Zásada podmíněného přístupu pro blokování starších protokolů ověřování
...
V současné době většina všech neúspěšných pokusů o přihlášení pocházela ze staršího ověřování. Starší verze ověřování nepodporuje službu Multi-Factor Authentication (MFA). I v případě, že máte ve svém adresáři povolené zásady vícefaktorového ověřování, může se špatný objekt actor ověřit pomocí starší verze protokolu a obejít MFA.

Nejlepším způsobem, jak ochránit váš účet před nebezpečnými požadavky na ověření, které udělaly starší protokoly, je blokovat tyto pokusy úplně. Abychom vám usnadnili zabezpečení svého prostředí, vytvořili jsme tuto základní zásadu k blokování starší verze ověřování.

**Blokovat starší verze ověřování** je [základní zásada](concept-baseline-protection.md) , která blokuje všechny požadavky na ověření provedené ze starších protokolů. K úspěšnému přihlášení pro všechny uživatele se musí použít moderní ověřování. Při použití ve spojení s ostatními základními základními zásadami budou všechny požadavky přicházející ze starších protokolů blokované a všechny uživatele budou vyžadovat MFA, kdykoli to bude potřeba. Tato zásada neblokuje Exchange ActiveSync.
...
### <a name="step-1-enable-modern-authentication-in-your-directory"></a>Krok 1: Povolit moderní ověřování v adresáři
...
Než budete moct začít s povolením moderního ověřování v místním prostředí, ujistěte se, že jste splnili požadavky.
Nyní jste připraveni povolit moderní ověřování v místním prostředí.
...
Zásady standardních **hodnot zásad: Zablokovat starší verze ověřování** (Preview) je předem nakonfigurovaná a v horní části se zobrazí, když v Azure Portal přejdete do okna podmíněný přístup. Toto nastavení se projeví až po úspěšném přihlášení, takže uživatelé budou mít i nadále možnost pokusit se používat starší verze ověřování.
