...
3. **[Vytvoření zkušebního uživatele FiscalNote](#create-a-fiscalnote-test-user)**  – Pokud chcete mít protějšek Britta Simon FiscalNote, který je propojený s Azure AD reprezentace uživatele.
...
3. Na **FiscalNote domény a adresy URL** části, proveďte následující kroky:
...
8. Na **FiscalNote konfigurace** klikněte na tlačítko **nakonfigurovat FiscalNote** otevřete **nakonfigurovat přihlašování** okna. Kopírovat **URL odhlašování SAML Entity ID a SAML jednotné přihlašování – adresa URL služby** z **Stručná referenční příručka oddílu.**

    ![Konfigurace FiscalNote](./media/fiscalnote-tutorial/tutorial_fiscalnote_configure.png) 

9. Ke konfiguraci jednotného přihlašování na **FiscalNote** straně, je nutné odeslat na stažený **Certificate(Raw), adresa URL odhlašování, SAML Entity ID a SAML jednotné přihlašování – adresa URL služby** k [FiscalNote tým podpory](mailto:support@fiscalnote.com). Nastavují tohoto nastavení můžete mít správně nastavené na obou stranách připojení SAML SSO.
