---
title: Spuštění více závislých služeb pomocí .NET Core a sady Visual Studio
...
ms.locfileid: "74279587"
---
# <a name="multi-service-development-with-azure-dev-spaces"></a>Vývoj více služeb pomocí Azure Dev Spaces
...
V této části vytvoříte druhou službu, `mywebapi`a `webfrontend` ji budete volat. Každá služba poběží v samostatném kontejneru. Ladění pak provedete v obou kontejnerech.
...
Teď napíšeme kód v projektu `webfrontend`, který vygeneruje požadavek do `mywebapi`. Přejděte do okna sady Visual Studio, ve kterém je projekt `webfrontend`. V souboru `HomeController.cs` *nahraďte* kód metody about následujícím kódem:
...
1. V souboru `Get(int id)`, který zpracovává požadavky GET `Controllers/ValuesController.cs`, nastavte zarážku v metodě `api/values/{id}`.
...
1. Zarážku v projektu **aktivujte kliknutím na odkaz**O aplikaci`webfrontend` v horní části stránky. 
