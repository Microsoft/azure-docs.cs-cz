...
- Vytvořte kontejner objektů blob a načtěte jeho identifikátor URI SAS pomocí postupu v článku [Správa prostředků služby Azure Blob Storage pomocí Průzkumníka služby Storage](../vs-azure-tools-storage-explorer-blobs.md#get-the-sas-for-a-blob-container). Při vytváření identifikátoru URI SAS nezapomeňte v okně zásad vybrat všechna oprávnění (čtení, zápis, odstranění, výpis). Tato podrobnosti poskytuje Azure Database Migration Service s přístupem k vašemu kontejneru účtu úložiště pro nahrávání záložních souborů používaných pro migraci databází do spravované instance SQL.

    > [!NOTE]
    > Při konfiguraci nastavení účtu úložiště během kroku [Konfigurace nastavení migrace](#configure-migration-settings) Azure Database Migration Service nepodporuje použití tokenu SAS na úrovni účtu.
...
1. Na obrazovce **Podrobnosti cíle migrace** zadejte podrobnosti o připojení pro cíl, což je předem zřízená spravovaná instance SQL, na kterou migrujete databázi **AdventureWorks2012** .
    ...
    Pokud jste ještě nezřídili spravovanou instanci SQL, vyberte [odkaz](../azure-sql/managed-instance/instance-create-quickstart.md) , který vám pomůžete zřídit instanci. I nadále můžete pokračovat v vytváření projektu a poté, když je SQL Managed instance připravena, vraťte se do tohoto konkrétního projektu a spusťte migraci.
...
1. Na obrazovce aktivity migrace výběrem **Aktualizovat** aktualizujte zobrazení.
   ...
   ![Snímek obrazovky zobrazující obrazovku aktivity migrace a tlačítko Aktualizovat](media/tutorial-sql-server-to-managed-instance/dms-monitor-migration1.png)
