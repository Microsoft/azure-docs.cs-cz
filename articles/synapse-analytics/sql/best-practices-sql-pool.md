---
title: Osvědčené postupy pro fondy SQL
...
Tento článek poskytuje kolekci osvědčených postupů, které vám pomůžou dosáhnout optimálního výkonu pro fondy SQL ve službě Azure synapse Analytics. Níže najdete základní pokyny a důležité oblasti, se kterými se můžete soustředit při sestavování řešení. V každé části se seznámíte s konceptem a potom odkazujete na podrobnější články, které se vztahují k konceptu podrobněji.

## <a name="sql-pools-loading"></a>Načítání fondů SQL

Pokyny k načítání pro fondy SQL najdete v tématu [pokyny k načítání dat](data-loading-best-practices.md).
...
Příliš mnoho oddílů může snížit efektivitu clusterovaných indexů columnstore, pokud má každý oddíl méně než 1 000 000 řádků. Fondy SQL automaticky rozdělí data do oddílů databáze 60. Takže pokud vytvoříte tabulku s 100 oddíly, výsledkem budou 6000 oddíly. Každá úloha je odlišná, takže nejlepším postupem je experimentovat s vytvářením oddílů, abyste viděli, co nejlépe vyhovuje vašim úlohám.  
...
- [ZMĚNIT TABULKU](/sql/t-sql/statements/alter-table-transact-sql?toc=/azure/synapse-analytics/toc.json&bc=/azure/synapse-analytics/breadcrumb/toc.json&view=azure-sqldw-latest)
...
Když dočasně vydáváte data na fondy SQL, tabulky haldy obvykle budou mít celkový proces rychleji.  Pokud načítáte data pouze pro přípravu před spuštěním více transformací, je načítání tabulky do tabulky haldy rychlejší než načítání dat do clusterované tabulky columnstore.  
...
Tabulky columnstore obecně nezadávají data do komprimovaného segmentu columnstore, dokud nebude existovat více než 1 000 000 řádků na tabulku. Každá tabulka fondů SQL je rozdělená na oddíly tabulky 60. V takovém případě tabulky columnstore nebudou využívat dotaz, pokud tabulka obsahuje více než 60 000 000 řádků.  
