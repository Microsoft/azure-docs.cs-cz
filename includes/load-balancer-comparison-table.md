...
| [Sonda stavu dolů chování](../articles/load-balancer/load-balancer-custom-probe-overview.md#probedown) | Připojení TCP zůstat naživu na instanci test dolů __a__ na všechny testy dolů. | Připojení TCP zůstat naživu na instanci test mimo provoz. Všechny koncové připojení protokolu TCP na všechny testy jsou vypnuté. |
...
| Ve výchozím nastavení zabezpečení | Použít výchozí příchozí uzavřené pro veřejné IP adresy a nástroje pro vyrovnávání zatížení koncové body a skupiny zabezpečení sítě na explicitní seznam povolených pro příjem provozu. | Výchozí otevřete skupinu zabezpečení sítě, které jsou volitelné. |
...
| [Odchozí pravidla](../articles/load-balancer/load-balancer-outbound-rules-overview.md) | Deklarativní odchozí NAT konfigurace, včetně které veřejnou IP adresu nebo předponu veřejné IP, Konfigurovatelný časový limit odchozí nečinnosti, vlastní SNAT přiřazování portů | Není k dispozici. |
