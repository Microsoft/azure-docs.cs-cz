...
Funkce obnovitelného odstranění je zpočátku dostupná prostřednictvím rozhraní [REST](/rest/api/keyvault/), [CLI](soft-delete-cli.md), [PowerShellu](soft-delete-powershell.md) a [rozhraní .NET/C#](/dotnet/api/microsoft.azure.keyvault?view=azure-dotnet) .
...
- Když je `--enable-purge-protection flag` Tato možnost povolená v samotném trezoru. V takovém případě bude Key Vault čekat na 90 dní od okamžiku, kdy byl původní tajný objekt označen pro odstranění, aby se objekt trvale odstranil.
