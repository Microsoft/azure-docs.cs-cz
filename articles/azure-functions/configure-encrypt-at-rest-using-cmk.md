...
Nejdřív [vytvořte účet Azure Storage](../storage/common/storage-account-create.md) a [Zašifrujte ho pomocí zákaznických spravovaných klíčů](../storage/common/storage-service-encryption.md#customer-managed-keys-with-azure-key-vault). Po vytvoření účtu úložiště použijte [Průzkumník služby Azure Storage](../vs-azure-tools-storage-manage-with-storage-explorer.md) k nahrání souborů balíčku.
...
Po nahrání souboru do úložiště objektů BLOB a zadání adresy URL SAS pro soubor nastavte nastavení aplikace `WEBSITE_RUN_FROM_PACKAGE` na adresu URL SAS. V následujícím příkladu se používá Azure CLI:
...
Nyní můžete nahradit hodnotu nastavení `WEBSITE_RUN_FROM_PACKAGE` aplikace pomocí Key Vault odkaz na adresu URL kódovanou pomocí SAS. Tím se zachovává adresa URL SAS zašifrovaná v Key Vault, která poskytuje další vrstvu zabezpečení.

1. K vytvoření instance Key Vault použijte následující [`az keyvault create`](/cli/azure/keyvault#az-keyvault-create) příkaz.       
...
1. Pomocí následujícího příkazu [`az keyvault secret set`](/cli/azure/keyvault/secret#az-keyvault-secret-set) přidejte svoji externí adresu URL jako tajný klíč do trezoru klíčů:   

    ```azurecli    
    az keyvault secret set --vault-name "Contoso-Vault" --name "external-url" --value "<SAS-URL>"    
    ```    

1.  Pomocí následujícího příkazu [`az webapp config appsettings set`](/cli/azure/webapp/config/appsettings#az-webapp-config-appsettings-set) vytvořte nastavení `WEBSITE_RUN_FROM_PACKAGE` aplikace s hodnotou jako Key Vault odkaz na externí adresu URL:

    ```azurecli    
    az webapp config appsettings set --settings WEBSITE_RUN_FROM_PACKAGE="@Microsoft.KeyVault(SecretUri=https://Contoso-Vault.vault.azure.net/secrets/external-url/<secret-version>"    
    ```

    `<secret-version>` bude ve výstupu předchozího příkazu `az keyvault secret set`.
...
1. Otočte klíč SAS tak, že přejdete na svůj účet úložiště v Azure Portal. V části **nastavení** > **přístupové klíče**klikněte na ikonu pro otočení klíče SAS.
...
- Spuštění vaší aplikace z balíčku pro nasazení zpřístupňuje `wwwroot/` jen pro čtení. Vaše aplikace při pokusu o zápis do tohoto adresáře obdrží chybu.
