...
Po instalaci podporované verze Azure PowerShell nebo rozhraní příkazového řádku Azure CLI můžete nasadit soubor bicep pomocí:
...
Bicep CLI můžete nainstalovat pomocí rozhraní příkazového řádku Azure, a to pomocí Azure PowerShell nebo ručně.

### <a name="use-azure-cli"></a>Použití Azure CLI

Pomocí příkazu AZ CLI verze 2.20.0 nebo novější nainstalují bicep CLI automaticky, když je spuštěný příkaz, který na něm závisí. Příkladem je `az deployment ... -f *.bicep` nebo `az bicep ...`.
...
> [!NOTE]
> AZ CLI nainstaluje samostatnou verzi rozhraní příkazového řádku bicep, která není v konfliktu s žádnou jinou instalací bicep, kterou máte k dispozici, a AZ CLI nepřidá bicep do vaší cesty.
...
Azure PowerShell ještě nemá schopnost nainstalovat rozhraní příkazového řádku bicep. Azure PowerShell (v 5.6.0 nebo novějším) předpokládá, že rozhraní příkazového řádku bicep je již nainstalováno a je k dispozici v cestě. Proveďte jednu z [metod ruční instalace](#install-manually). Po instalaci rozhraní příkazového řádku bicep se zavolá bicep CLI, kdykoli je to potřeba pro rutinu nasazení. Například, `New-AzResourceGroupDeployment ... -TemplateFile main.bicep`.
