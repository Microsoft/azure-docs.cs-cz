...
Vyhledá hodnotu `host` ve výstupu (IP adresa se `10.32.0.x`podobá) a hodnotu. `ports`


Nyní proveďte Terminálové připojení SSH (nikoli tunelové připojení) k plně kvalifikovanému názvu domény pro správu clusteru. Po připojení proveďte následující požadavek `host` a nahraďte správné hodnoty a: `ports`
...
> [!NOTE]
> Identifikátor URI je http:\//LOCALHOST/Marathon/v2/Apps/následovaný ID aplikace, která se má škálovat. Pokud používáte ukázku Nginx, která je zde uvedená, identifikátor URI by byl http:\//localhost/Marathon/v2/Apps/Nginx.
