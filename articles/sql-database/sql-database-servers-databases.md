---
title: Logické servery Azure SQL a izolovanými databázemi | Dokumentace Microsoftu
description: Další informace o logického serveru Azure SQL Database a koncepty izolované databáze a jejich prostředků.
...
> [!TIP]
> Platné skupiny a serveru názvy prostředků, najdete v části [pravidla a omezení pojmenování](https://docs.microsoft.com/azure/architecture/best-practices/naming-conventions).
>
...
## <a name="logical-servers-and-databases"></a>Logických serverů a databází

Na logickém serveru můžete vytvořit:

- Izolované databáze vytvořené v rámci [skupiny prostředků Azure](../azure-resource-manager/resource-group-overview.md) s [kombinovat sadu výpočetních a úložných kapacit](sql-database-service-tiers-dtu.md) nebo [nezávislé škálování výpočetní a úložnou kapacitu](sql-database-service-tiers-vcore.md). Azure SQL database je přidružená logického serveru Azure SQL Database, která je vytvořena v rámci konkrétní oblasti Azure.
- Databáze vytvořené v rámci [fond databází](sql-database-elastic-pool.md) v rámci [skupiny prostředků Azure](../azure-resource-manager/resource-group-overview.md) s [kombinovat sadu výpočetních a úložných prostředků (založený na DTU)](sql-database-service-tiers-dtu.md) nebo [nezávislé škálování výpočetní a úložnou kapacitu (založený na virtuálních jádrech)](sql-database-service-tiers-vcore.md) , která jsou sdílena mezi všemi databázemi ve fondu. Azure SQL database je přidružená logického serveru Azure SQL Database, která je vytvořena v rámci konkrétní oblasti Azure.

> [!IMPORTANT]
> SQL Database Managed Instance aktuálně ve verzi public preview, je [instanci systému SQL server](sql-database-managed-instance.md) (Managed Instance) vytvořených v rámci [skupiny prostředků Azure](../azure-resource-manager/resource-group-overview.md) s definovanou sadou výpočetních operací a úložiště prostředky pro všechny databáze v instanci serveru. Spravovaná instance obsahuje systémových a uživatelských databázích. Spravovaná Instance je navržená k umožnění databáze lift and shift na plně spravovaný model PaaS, bez nutnosti změny návrhu aplikace. Managed Instance poskytuje vysokou kompatibilitu s programovacím modelem v místním systému SQL Server a podporuje valné většiny funkcí SQL serveru a doprovodných nástrojů a služeb. Další informace najdete v tématu [SQL Database Managed Instance](sql-database-managed-instance.md). Zbývající část tohoto článku se nevztahují na Managed Instance.

## <a name="tds-and-tcpip-connections"></a>Připojení TDS a protokolu TCP/IP

Microsoft Azure SQL Database podporuje tabulková data služby stream (TDS) protokol klienta verzi 7.3 nebo novější a umožňuje jenom šifrovaná připojení TCP/IP.

## <a name="azure-sql-databases-protected-by-sql-database-firewall"></a>Azure SQL Database, které jsou chráněné bránou firewall databáze SQL

K ochraně vašich dat [bránu firewall služby SQL Database](sql-database-firewall-configure.md) brání veškerému přístupu k vašemu databázovému serveru, nebo žádnou z jeho databází z mimo připojení k serveru přímo prostřednictvím připojení k předplatnému Azure. Pokud chcete povolit další připojení, je nutné [vytvořte jedno nebo více pravidel brány firewall](sql-database-firewall-configure.md#creating-and-managing-firewall-rules). Vytváření a správa elastických fondů najdete v tématu [elastické fondy](sql-database-elastic-pool.md).
