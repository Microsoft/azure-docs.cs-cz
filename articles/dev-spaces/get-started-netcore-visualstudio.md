---
title: Vytvoření vývojového prostředí Kubernetes v cloudu pomocí .NET Core a sady Visual Studio | Microsoft Docs
...
- Vytvořit v Azure prostředí založené na Kubernetes, které je optimalizované pro vývoj
...
Služba Azure Dev Spaces umožňuje vytvářet vývojová prostředí založená na Kubernetes, která Azure plně spravuje a která jsou optimalizovaná pro vývoj. V projektu, který jste právě otevřeli, vyberte v rozevíracím okně nastavení spouštění **Azure Dev Spaces**, jak vidíte na následujícím obrázku.
...
Nejdřív uvidíte, že se přidala složka s názvem `charts` a v ní se vygeneroval [graf Helm](https://docs.helm.sh) pro vaši aplikaci. Tyto soubory slouží k nasazení vaší aplikace do vývojového prostředí.

Uvidíte, že se přidal soubor s názvem `Dockerfile`. Tento soubor obsahuje informace potřebné k zabalení vaší aplikace ve standardním formátu Dockeru. Vytvořil se také soubor `HeaderPropagation.cs`, o kterém se v tomto názorném návodu zmíníme později. 
...
Visual Studio bude komunikovat s vývojovým prostředím, aby se mohla aplikace sestavit a nasadit, a potom otevře prohlížeč se spuštěnou webovou aplikací. I když to vypadá, že je kontejner spuštěný místně, ve skutečnosti běží ve vývojovém prostředí v Azure. Adresa místního hostitele se zobrazuje proto, že služba Azure Dev Spaces vytvoří dočasný tunel SSH do kontejneru běžícího v Azure.
...
Kvůli úspoře času si stáhněme ukázkový kód z úložiště GitHub. Přejděte na https://github.com/Azure/dev-spaces a stáhněte si úložiště GitHub výběrem možnosti **Clone or Download** (Klonovat nebo stáhnout). Kód k této části je tady: `samples/dotnetcore/getting-started/mywebapi`

## <a name="run-mywebapi"></a>Spuštění *mywebapi*
1. Otevřete službu `mywebapi` projektu v *samostatném okně sady Visual Studio*.
1. Vyberte v rozevíracím okně nastavení spouštění **Azure Dev Spaces**, jak jste to udělali už dříve pro projekt `webfrontend`. Tentokrát místo vytváření nového vývojového prostředí vyberte prostředí, které jste už vytvořili. Stejně jako dříve nechte v rozevíracím seznamu Space (Prostor) vybranou položku `default` a klikněte na **OK**. V okně Výstup si můžete všimnout, že se ve vašem vývojovém prostředí začíná tato nová služba „připravovat“, aby se při spuštění ladění postup urychlil.
...
1. Poznamenejte si adresu URL koncového bodu, která se zobrazí v okně **Výstup** v podokně **Azure Dev Spaces for AKS** (Azure Dev Spaces pro AKS). Bude vypadat přibližně takto: http://localhost:\<portnumber\>. I když to vypadá, že je kontejner spuštěný místně, ve skutečnosti běží ve vývojovém prostředí v Azure.
...
Ve výše uvedeném příkladu kódu se používá také třída `HeaderPropagatingHttpClient`. Tato pomocná třída je soubor `HeaderPropagation.cs`, který se do projektu přidal, když jste ho nakonfigurovali pro používání Azure Dev Spaces. Třída `HeaderPropagatingHttpClient` je odvozená ze známé třídy `HttpClient` a přidává funkce potřebné k šíření určitých hlaviček z existujícího objektu ASP .NET HttpRequest do odchozího objektu HttpRequestMessage. Později se dozvíte, jak se tím v týmových scénářích zvyšuje produktivita vývojového prostředí.
...
1. V souboru `ValuesController.cs`, který zpracovává požadavky GET `api/values/{id}`, nastavte zarážku v metodě `Get(int id)`.
...
* Umožňuje týmu vývojářů pracovat ve stejném vývojovém prostředí.
...
### <a name="work-in-a-shared-development-environment"></a>Práce ve sdíleném vývojovém prostředí
Pomocí služby Azure Dev Spaces můžete nastavit *sdílené* vývojové prostředí v Azure. Každý vývojář se může zaměřit jenom na svoji část aplikace a může iterativně vyvíjet *kód před potvrzením* v prostředí, které už obsahuje všechny ostatní služby a cloudové prostředky, na kterých jsou příslušné scénáře závislé. Závislosti jsou vždycky aktuální a vývojáři pracují způsobem, který odpovídá produkčnímu prostředí.
...
Pomocí následujícího postupu se ujistěte, že služby `webfrontend` a `mywebapi` jsou spuštěné ve vašem vývojovém prostředí **a v prostoru `default`**.
...
Kdokoli, kdo otevře veřejnou adresu URL a přejde k webové aplikaci, vyvolá zapsanou cestu kódu, která spouští obě služby ve výchozím prostoru `default`. Předpokládejme teď, že chcete pokračovat ve vývoji `mywebapi` – jak to uděláte bez toho, že byste narušili práci jiných vývojářů, kteří používají toto vývojové prostředí? Vytvoříte si k tomu vlastní prostor.
...
7. Teď by se mělo zobrazit vaše vývojové prostředí a na stránce vlastností projektu by měl být vybraný nový prostor.

    ![](media/get-started-netcore-visualstudio/Settings2.png)

### <a name="update-code-for-mywebapi"></a>Aktualizace kódu pro *mywebapi*

1. V projektu `mywebapi` změňte kód v souboru `ValuesController.cs` v metodě `string Get(int id)` následujícím způsobem:
...
3. Stisknutím F5 spusťte službu `mywebapi`. Služba se spustí ve vývojovém prostředí s využitím vybraného prostoru, což v tomto případě je `scott`.

Následující diagram vám pomůže porozumět tomu, jak různé prostory fungují. Modrá cesta znázorňuje požadavek prostřednictvím prostoru `default`. Je to výchozí cesta, která se použije, když se žádný prostor nepřidá jako předpona k adrese URL. Zelená cesta znázorňuje požadavek prostřednictvím prostoru `scott`.
...
### <a name="test-code-running-in-the-scott-space"></a>Testování kódu spuštěného v prostoru `scott`
Pokud chcete otestovat novou verzi `mywebapi` ve spojení s projektem `webfrontend`, otevřete v prohlížeči adresu URL veřejného přístupového bodu pro `webfrontend` (například http://webfrontend-teamenv.123456abcdef.eastus.aksapp.io) a přejděte na stránku O aplikaci. Měla by se zobrazit původní zpráva: „Hello from webfrontend and Hello from mywebapi.“

Teď k adrese URL přidejte „scott.s.“. Adresa URL tedy bude vypadat nějak podobně jako http://scott.s.webfrontend-teamenv.123456abcdef.eastus.aksapp.io. Aktualizujte stránku v prohlížeči. Mělo by dojít k zastavení na zarážce, kterou jste nastavili v projektu `mywebapi`. Pokračujte kliknutím na F5. V prohlížeči by se teď měla zobrazit nová zpráva „Hello from webfrontend and mywebapi now says something new.“ Důvodem je skutečnost, že v prostoru `scott` je spuštěná cesta k aktualizovanému kódu v `mywebapi`.
