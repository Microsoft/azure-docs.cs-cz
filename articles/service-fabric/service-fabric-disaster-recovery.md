...
Řekněme, že služba má `InstanceCount` 5 a v současné době selhalo několik uzlů, na kterých se tyto instance nezdařily. Service Fabric odpoví automatickým vytvořením náhradních instancí na jiných uzlech. Bude pokračovat ve vytváření nahrazení až do chvíle, kdy se služba vrátí do požadovaného počtu instancí. Jako další příklad řekněme, že došlo k nestavové službě s `InstanceCount`-1, což znamená, že se spustí na všech platných uzlech clusteru. Řekněme, že některé z těchto instancí byly neúspěšné. V takovém případě Service Fabric oznámení, že služba není v požadovaném stavu, a pokusí se vytvořit instance v uzlech, kde chybí. 

U stavových služeb závisí situace na tom, jestli služba má trvalý stav nebo ne. Záleží taky na tom, kolik replik služba měla a kolik selhalo. Určení toho, jestli došlo k havárii pro stavovou službu a Správa se skládá ze tří fází:

1. Zjištění, zda došlo ke ztrátě kvora nebo ne
   - Ztráta kvora je pokaždé, když většina replik stavové služby je mimo provoz, včetně primární.
2. Zjištění, zda je ztráta kvora trvalá nebo ne
   - Ve většině případů jsou chyby přechodné. Procesy se restartují, uzly se restartují, virtuální počítače se znovu spouštějí a zamění se síťové oddíly. Někdy ale chyby jsou trvalé. 
     - U služeb bez trvalého stavu dojde k selhání kvora nebo většího počtu replik _okamžitě_ v trvalé ztrátě kvora. Když Service Fabric detekuje ztrátu kvora ve stavové netrvalé službě, okamžitě pokračuje krokem 3 tím, že deklaruje (možná) ztrátu dat. Pokračování na ztrátu dat dává smysl, protože Service Fabric ví, že není potřeba čekat na to, aby se repliky vrátily zpátky, protože i když byly obnoveny, by měly být prázdné.
     - U stavových trvalých služeb způsobí selhání kvora nebo většího počtu replik Service Fabric, aby se čekalo, že repliky vrátí a obnoví kvorum. Výsledkem je výpadek služby pro jakékoli _zápisy_ do ovlivněného oddílu (neboli sady replik) služby. Nicméně je možné, že čtení je stále možné se sníženými zárukami konzistence. Výchozí doba, po kterou Service Fabric čeká na obnovení kvora, je nekonečná, protože se jedná o (možná) událost ztráty dat a přináší další rizika. Přepsání výchozí hodnoty `QuorumLossWaitDuration` je možné, ale nedoporučujeme. Místo toho by se měly udělat všechny snahy o obnovení replik. To vyžaduje, aby se uzly, které jsou v nefunkčním stavu, přepnuly, a zajistili tak, že můžou znovu připojit jednotky, na kterých se uložil místní trvalý stav. Pokud je ztráta kvora způsobena selháním procesu, Service Fabric automaticky se pokusí znovu vytvořit procesy a restartovat repliky uvnitř těchto procesů. Pokud se tato chyba nezdařila, Service Fabric hlásí chyby stavu. Pokud je možné je vyřešit, repliky se většinou vrátí. Někdy se ale repliky nedají vrátit zpátky. Například jednotky se nezdařily, nebo počítače fyzicky zničí nějakým způsobem. V těchto případech teď máme trvalou událost ztráty kvora. Aby bylo možné Service Fabric, že přestane čekat na obnovení replik, musí správce clusteru určit, které oddíly a které služby jsou ovlivněny, a volat rozhraní API `Repair-ServiceFabricPartition -PartitionId` nebo `System.Fabric.FabricClient.ClusterManagementClient.RecoverPartitionAsync(Guid partitionId)`.  Toto rozhraní API umožňuje zadat ID oddílu, aby se přesunuly mimo QuorumLoss a do potenciální ztráty dataztrát.

   > [!NOTE]
   > Používání tohoto rozhraní API není _nikdy_ bezpečné než pro konkrétní oddíly cíleným způsobem. 
   >
