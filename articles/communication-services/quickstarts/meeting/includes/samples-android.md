---
title: Používání týmů komunikačních služeb Azure, které jsou vložené pro Android
description: V tomto přehledu se dozvíte, jak používat knihovnu pro Azure Communications Services pro Android.
author: palatter
ms.author: palatter
ms.date: 24/02/2021
ms.topic: conceptual
...
Přidejte `MeetingEventListener` do své třídy.
...
Nastavte `MeetingEventListener` v `this` .
...
Implementujte `onCallStateChanged` `onRemoteParticipantCountChanged` metody a.
...
Přidejte `MeetingIdentityProvider` do své třídy.
...
Nastavte `MeetingIdentityProvider` v `this` .
...
Namapujte každou `userMri` z odpovídajících miniatur.
