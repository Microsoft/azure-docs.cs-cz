...
1. Otevřete `res/layout/activity_main.xml` soubor a nahraďte následující definice obsahu:
    ...
    ```
2. Otevřete `res/values/strings.xml` soubor a nahraďte `send_button` definici následujícími řádky, které předefinují řetězec pro `send_button` a přidejte řetězce pro jiné ovládací prvky:
    ...
    ![Snímek obrazovky aplikace s poli pro uživatelské jméno, heslo, příjemce a zprávu a s tlačítky pro přihlášení a odesílání oznámení.][A1]
...
9. Pak přidejte následující metody pro zpracování události kliknutí na tlačítko pro **přihlášení** a odesílání nabízených oznámení.
    ...
    `login`Obslužná rutina pro tlačítko **Přihlásit** vygeneruje základní ověřovací token pomocí vstupního uživatelského jména a hesla (představuje všechny tokeny, které vaše schéma ověřování používá), potom používá `RegisterClient` k volání back-end pro registraci.
...
11. V třídě `MainActivity` následujícím způsobem aktualizujte metodu `sendNotificationButtonOnClick`, aby volala metodu `sendPush` s použitím systému oznámení platformy vybraného uživatelem.
    ...
    */
    ...
    ```
12. V `build.gradle` souboru přidejte následující řádek do `android` části za `buildTypes` sekcí.
