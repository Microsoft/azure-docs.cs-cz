...
1. [Nahrajte vstupní soubor](job-input-from-http-how-to.md) , jako je MP4, QuickTime nebo MOV, nebo jiný podporovaný formát souboru. Tento soubor se také označuje jako Mezzanine nebo zdrojový soubor. Seznam podporovaných formátů najdete v tématu [formáty podporované kodérem Standard](encode-media-encoder-standard-formats-reference.md).
...
1. Publikujte výstupní Asset, který obsahuje sadu MP4 s adaptivní přenosovou rychlostí. Publikujete vytvořením [lokátoru streamování](stream-streaming-locators-concept.md).
...
![Diagram pracovního postupu pro streamování na vyžádání s dynamickým balením](./media/encode-dynamic-packaging-concept/media-services-dynamic-packaging.svg)

Cesta ke stažení je k dispozici ve výše uvedeném obrázku, aby bylo vidět, že si můžete stáhnout soubor MP4 přímo prostřednictvím *koncového bodu streamování* (počátek) (zadáte [zásadu streamování](stream-streaming-policy-concept.md) ke stažení na lokátoru streamování).<br/>Dynamický balíček nemění soubor. Můžete volitelně použít rozhraní API služby Azure Blob Storage pro přístup k MP4 přímo pro progresivní stahování, pokud chcete obejít funkce *koncového bodu streamování* (Origin). 

### <a name="encode-to-adaptive-bitrate-mp4s"></a>Kódovat do adaptivní přenosové rychlosti rychlostmi

V následujících článcích se [dozvíte, jak zakódovat video pomocí Media Services](encode-concept.md):
...
Podívejte se na seznam [formátů a kodeků](encode-media-encoder-standard-formats-reference.md)Standard Encoder.
...
1. Vytvořte [živou událost](live-event-outputs-concept.md).
...
Informace o živém streamování v Media Services V3 najdete v tématu [Přehled živého streamování](stream-live-streaming-concept.md).
...
Chcete-li řídit počet běhů, formátů, přenosové rychlosti a časová období prezentace, která jsou odesílána přehrávačům, můžete použít dynamické filtrování pomocí nástroje Media Services Dynamic packageer. Další informace najdete v tématu [předběžné filtrování manifestů pomocí dynamického balíčku](filters-dynamic-manifest-concept.md).
