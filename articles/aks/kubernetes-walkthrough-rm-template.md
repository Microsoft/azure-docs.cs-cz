...
Chcete-li získat přístup k uzlům AKS, připojte se pomocí páru klíčů SSH. `ssh-keygen` Pomocí příkazu vygenerujte soubory veřejného a privátního klíče SSH. Ve výchozím nastavení se tyto soubory vytvoří v adresáři *~/.ssh* . Pokud v daném umístění existuje pár klíčů SSH se stejným názvem, tyto soubory se přepíší.
...
2. Vyberte nebo zadejte následující hodnoty.  
    ...
    * **Skupina prostředků**: Vyberte, že chcete **vytvořit novou** IP adresu. Zadejte jedinečný název pro skupinu prostředků, třeba *myResourceGroup*, a pak zvolte **OK**.
    ...
    * **Tajný kód klienta instančního objektu**: Zkopírujte a vložte *heslo* k instančnímu objektu z `az ad sp create-for-rbac` příkazu.
    * **Souhlasím s výše uvedenými podmínkami a ujednáními**: Zaškrtněte toto políčko pro vyjádření souhlasu.
