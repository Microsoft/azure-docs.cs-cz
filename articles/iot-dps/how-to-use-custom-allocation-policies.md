...
Například možná budete chtít ověřit certifikát, který zařízení používá během zřizování, a přiřadit zařízení ke službě IoT Hub na základě vlastnosti certifikátu. Možná budete mít informace uložené v databázi pro vaše zařízení a potřebujete zadat dotaz na databázi, abyste zjistili, ke kterému centru IoT Hub se má zařízení přiřadit.
...
- **-contoso-tstrsd-007**: Oddělení informačních zpráv společnosti Contoso
- **-contoso-hpsd-088**: Oddělení tepelného čerpadla společnosti Contoso
...
* Simulovat zařízení, abyste viděli, že jsou zřízené podle ukázkového kódu vlastní zásady přidělování
...
2. Pomocí Azure Cloud Shell můžete pomocí příkazu [AZ IoT Hub Create](/cli/azure/iot/hub#az-iot-hub-create) vytvořit **informační centrum společnosti Contoso** s oddělením IoT Hub. Centrum IoT se přidá do *skupiny contoso-US-Resource-Group*.
    ...
    Následující příklad vytvoří centrum IoT s názvem contoso- *-hub-1098* v umístění *eastus* . Musíte použít vlastní jedinečný název centra. Vytvořte vlastní příponu v názvu centra místo **1098**. Vzorový kód pro vlastní zásady přidělování vyžaduje `-toasters-` název centra.
...
3. Pomocí Azure Cloud Shell vytvořte pomocí příkazu [AZ IoT Hub Create](/cli/azure/iot/hub#az-iot-hub-create) vytvořit centrum IoT pro **tepelné pumpy společnosti Contoso** . Toto centrum IoT se taky přidá do *skupiny contoso-US-Resource-Group*.
    ...
    Následující příklad vytvoří centrum IoT s názvem *Contoso-heatpumps-hub-1098* v umístění *eastus* . Musíte použít vlastní jedinečný název centra. Vytvořte vlastní příponu v názvu centra místo **1098**. Vzorový kód pro vlastní zásady přidělování vyžaduje `-heatpumps-` název centra.
...
3. Do pole **Přidat skupinu**registrací zadejte následující informace a klikněte na tlačítko **Uložit** .
    ...
    **Název skupiny**: Zadejte **Contoso-Custom-alokovaný-Devices**.

    **Typ ověření identity**: Vyberte možnost **symetrický klíč**.

    **Automaticky vygenerovat klíče**: Toto zaškrtávací políčko by již mělo být zaškrtnuté.

    **Vyberte, jak chcete přiřadit zařízení k rozbočovačům**: Vyberte **vlastní (použijte funkci Azure Functions)** .
...
4. V části **Přidat skupinu**registrací klikněte na **odkaz propojit nové centrum IoT** a propojte si vaše nová centra IoT Hub. 
    ...
    Tento krok je nutné provést pro obě vaše centra IoT Hub.

    **Předplatné**: Pokud máte více předplatných, vyberte předplatné, ve kterém jste vytvořili centra IoT Hub.

    **Centrum IoT**: Vyberte jedno ze špičkových rozbočovačů, které jste vytvořili.
...
7. Na **Function App** vytvořit stránku, která se otevře, zadejte pro novou funkci následující nastavení a klikněte na **vytvořit**:
    ...
    **Skupina prostředků**: Pokud chcete zachovat všechny prostředky vytvořené v tomto článku, vyberte **použít existující** a **Contoso-US-Resource-Group** .
...
8. Zpátky na stránce **Přidat skupinu** registrací se ujistěte, že je vybraná nová aplikace Function App. Možná budete muset znovu vybrat předplatné, abyste mohli aktualizovat seznam aplikací funkcí.
...
9. V aplikaci Function App klikněte na vytvořit novou funkci.
...
K vygenerování klíče zařízení použijete **primární klíč** , který jste si předtím poznamenali, k výpočtu [HMAC-SHA256](https://wikipedia.org/wiki/HMAC) ID registrace zařízení pro každé zařízení a převod výsledku na Formát Base64. Další informace o vytváření odvozených klíčů zařízení pomocí skupin registrací najdete v části registrace skupin v tématu [ověření identity symetrického klíče](concepts-symmetric-key-attestation.md).
...
V této části připravíte vývojové prostředí použité k sestavení [Azure IoT C SDK](https://github.com/Azure/azure-iot-sdk-c). Sada SDK obsahuje vzorový kód pro simulované zařízení. Toto simulované zařízení se pokusí zřídit během spouštěcí sekvence zařízení.
...
V této části budete aktualizovat ukázku zřizování s názvem **prov\_dev\_Client\_Sample** v sadě Azure IoT C SDK, kterou jste si nastavili dříve. 
...
1. Chcete-li simulovat zařízení informačního zařízení, vyhledejte `prov_dev_set_symmetric_key_info()` volání v části **prov\_dev\_Client\_Sample. c** , který je přidán do komentáře.
    ...
    Odkomentujte volání funkce a nahraďte zástupné hodnoty (včetně lomených závorek) ID registrace informačního zařízení a odvozeného klíče zařízení, které jste předtím vygenerovali. Hodnota klíče **JC8F96eayuQwwz + PkE7IzjH2lIAjCUnAa61tDigBnSs =** uvedená níže je uvedena pouze jako příklad.
...
2. V nabídce sady Visual Studio vyberte **Ladit** > **Spustit bez ladění** a spusťte řešení. Po zobrazení výzvy k opětovnému sestavení projektu klikněte na **Ano** a před spuštěním projekt znovu sestavte.
    ...
    Následující výstup je příkladem simulovaného zařízení, které se úspěšně spouští a připojuje se k instanci služby zřizování, která se přiřadí ke službě IoT Hub pomocí vlastních zásad přidělování:
...
2. V nabídce sady Visual Studio vyberte **Ladit** > **Spustit bez ladění** a spusťte řešení. Po zobrazení výzvy k opětovnému sestavení projektu klikněte na **Ano** a před spuštěním projekt znovu sestavte.
    ...
    Následující výstup je příkladem simulovaného tepelného zařízení, které se úspěšně spouští a připojuje se k instanci služby zřizování, která se přiřadí ke službě IoT Hub v systému contoso pomocí vlastních zásad přidělování:
...
V následující tabulce jsou uvedené očekávané scénáře a kódy chyb výsledků, se kterými se můžete setkat. Tato tabulka vám umožní pomoct řešit problémy s chybami vlastního nastavení zásad přidělení s vaším Azure Functions.


| Scénář | Výsledek registrace ze služby zřizování | Zřizování výsledků sady SDK |
| -------- | --------------------------------------------- | ------------------------ |
| Webhook vrátí 200 OK s iotHubHostName nastavenou na platný název hostitele centra IoT Hub. | Stav výsledku: Přiřazená  | Sada SDK vrací PROV_DEVICE_RESULT_OK spolu s informacemi o centru |
| Webhook vrátí 200 OK s ' iotHubHostName ' obsažený v odpovědi, ale nastavte na prázdný řetězec nebo hodnotu null. | Stav výsledku: Selhalo<br><br> Kód chyby: CustomAllocationIotHubNotSpecified (400208) | SDK returns PROV_DEVICE_RESULT_HUB_NOT_SPECIFIED |
| Webhook vrátí 401 – Neautorizováno. | Stav výsledku: Selhalo<br><br>Kód chyby: CustomAllocationUnauthorizedAccess (400209) | Sada SDK vrací PROV_DEVICE_RESULT_UNAUTHORIZED |
...
| Webhook vrátí kód chyby > = 429. | Orchestrace DPS bude opakována kolikrát. Zásada opakování je aktuálně:<br><br>&nbsp;&nbsp;-Počet opakování: 10<br>&nbsp;&nbsp;-Počáteční interval: hodnotami<br>&nbsp;&nbsp;Zvětš devítky | Sada SDK bude ignorovat chybu a pošle do určeného času další zprávu o stavu Get. |
| Webhook vrátí jakýkoliv jiný stavový kód. | Stav výsledku: Selhalo<br><br>Kód chyby: CustomAllocationFailed (400207) | Sada SDK vrací PROV_DEVICE_RESULT_DEV_AUTH_ERROR |
