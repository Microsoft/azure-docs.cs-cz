...
Bing podporuje zvýrazňování přístupů, které označuje výrazy dotazu (nebo jiné výrazy, které Bing najde relevantní) v zobrazovaných řetězcích některých odpovědí. Například výsledek `name`webové stránky, `displayUrl`a `snippet` pole mohou obsahovat označené výrazy dotazu. 

Ve výchozím nastavení Bing neobsahuje značky zvýrazňování v řetězcích zobrazení. Chcete-li povolit značky, zahrňte `textDecorations` do žádosti parametr dotazu a nastavte jej na. `true`
...
* [Změnit velikost a oříznout miniatury](resize-and-crop-thumbnails.md)