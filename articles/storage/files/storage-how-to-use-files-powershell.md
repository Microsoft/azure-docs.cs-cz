...
Následující příklady ukazují, jak používat modul Azure PowerShell k manipulaci s sdílenou složkou Azure pomocí protokolu File REST. Parametr `-Context` slouží k načtení klíče účtu úložiště, který provede uvedené akce s sdílenou složkou. Pokud chcete načíst klíč účtu úložiště, musíte mít v účtu úložiště roli RBAC `Owner`.
...
I když je rutina `Start-AzStorageFileCopy` vhodná pro přenos souborů ad hoc mezi sdílenými složkami Azure, migrace a větší přesuny dat, doporučujeme `robocopy` ve Windows a `rsync` na macOS a Linux. `robocopy` a `rsync` pomocí protokolu SMB provádět přesuny dat místo rozhraní REST API.
...
Snímek sdílené složky můžete pro sdílenou složku vytvořit pomocí metody `Snapshot` v objektu prostředí PowerShell pro sdílenou složku, která se načte pomocí rutiny [Get-AzStorageShare](/powershell/module/az.storage/Get-AzStorageShare) . 
...
Obsah snímku sdílené složky můžete procházet tak, že do parametru `$snapshot` rutiny `-Share` předáte odkaz na snímek (`Get-AzStorageFile`).
...
Snímek sdílené složky můžete odstranit pomocí rutiny [Remove-AzStorageShare](/powershell/module/az.storage/Remove-AzStorageShare) s proměnnou obsahující `$snapshot` odkaz na parametr `-Share`.
