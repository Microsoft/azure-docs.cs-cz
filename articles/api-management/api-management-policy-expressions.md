...
Výrazy jednoho příkazu jsou uzavřeny `@(expression)`v, `expression` kde je příkaz výrazu ve C# správném formátu.
...
|System.Xml.Linq.XDocument|Vše s výjimkou: Načítání|
...
Proměnná s názvem `context` je implicitně dostupná ve všech [výrazech](api-management-policy-expressions.md#Syntax)zásad. Jeho členové poskytují informace, které jsou `\request`užitečné pro. `context` Všichni členové jsou jen pro čtení.
...
|<a id="ref-context-variables"></a>Kontext T Variables. GetValueOrDefault < T\>(proměnná: String, VýchozíHodnota: T)|proměnná: řetězec<br /><br /> Hodnot T<br /><br /> Vrátí přetypování hodnoty proměnné na `T` typ `defaultValue` nebo, pokud proměnná nebyla nalezena.<br /><br /> Tato metoda vyvolá výjimku, pokud zadaný typ neodpovídá skutečnému typu vrácené proměnné.|
...
|Jwt|Algoritmus: řetězec<br /><br /> Osoby IEnumerable < řetězec\><br /><br /> Podpory IReadOnlyDictionary < řetězec, řetězec [] ><br /><br /> ExpirationTime Hodnotu?<br /><br /> ID: řetězec<br /><br /> Vystavitel: řetězec<br /><br /> IssuedAt: Hodnotu?<br /><br /> NotBefore Hodnotu?<br /><br /> Předmět: řetězec<br /><br /> Typ: řetězec|
...
|Byte [] Encrypted (vstup: this Byte [], ALG: System. Security. Cryptography. SymmetricAlgorithm, klíč: Byte [], IV: Byte [])|vstup – prostý text, který se má zašifrovat<br /><br />ALG – šifrovací algoritmus<br /><br />šifrovací klíč klíče<br /><br />IV – vektor inicializace<br /><br />Vrátí šifrovaný prostý text.|
...
|Byte [] dešifrovat (vstup: this Byte [], ALG: System. Security. Cryptography. SymmetricAlgorithm, klíč: Byte [], IV: Byte [])|text Input-šifrováním, který se má dešifrovat<br /><br />ALG – šifrovací algoritmus<br /><br />šifrovací klíč klíče<br /><br />IV – vektor inicializace<br /><br />Vrátí prostý text.|
