...
Po spuštění emulátoru úložiště se zobrazí okno příkazového řádku. Pomocí tohoto okna konzoly můžete spustit a zastavit emulátor úložiště, vymazat data, získat stav a inicializovat emulátor. Další informace najdete v části referenční informace k [nástroji příkazového řádku emulátoru úložiště](#storage-emulator-command-line-tool-reference) dále v tomto článku.
...
Emulátor úložiště je ve výchozím nastavení nainstalován do `C:\Program Files (x86)\Microsoft SDKs\Azure\Storage Emulator`nástroje.

> [!TIP]
> K práci s prostředky emulátoru místního úložiště můžete použít [Průzkumník služby Microsoft Azure Storage](https://storageexplorer.com) . Po instalaci a spuštění emulátoru úložiště vyhledejte "(vývoj)" v části účty úložiště ve stromu Průzkumník služby Storagech prostředků.
>
...
1. V okně konzoly zadejte následující příkaz, kde `<SQLServerInstance>` je název instance SQL Server. Chcete-li použít LocalDB `(localdb)\MSSQLLocalDb` , zadejte jako instanci SQL Server.
...
Některé klientské knihovny pro Azure Storage, jako je například knihovna Xamarin, podporují pouze ověřování pomocí tokenu sdíleného přístupového podpisu (SAS). Token SAS můžete vytvořit pomocí nástroje podobného [Průzkumník služby Storage](https://storageexplorer.com/) nebo jiné aplikace, která podporuje ověřování pomocí sdíleného klíče.
...
2. Otevřete Azure PowerShell a spusťte následující příkazy, které nahradíte `CONTAINER_NAME` názvem, který zvolíte:
...
Ale emulátor úložiště, protože místní počítač neprovádí překlad názvů domén, název účtu je součástí cesty identifikátoru URI místo názvu hostitele. Pro prostředek v emulátoru úložiště použijte následující formát identifikátoru URI:
...
| --- | --- | --- | --- |
| **Start** |Spustí emulátor úložiště. |`AzureStorageEmulator.exe start [-inprocess]` |*-* InProcess: Spusťte emulátor v aktuálním procesu místo vytvoření nového procesu. |
...
| **Jejich** |Vymaže data ve všech službách zadaných v příkazovém řádku. |`AzureStorageEmulator.exe clear [blob] [table] [queue] [all]` |*blob*: Vymaže data objektu BLOB. <br/>*fronta*: Vymaže data ve frontě. <br/>*tabulka*: Vymaže data tabulky. <br/>*vše*: Vymaže všechna data ve všech službách. |
...
* Jak je popsáno v tématu [adresování prostředků v emulátoru úložiště](#addressing-resources-in-the-storage-emulator), se prostředky řeší odlišně v emulátoru úložiště a v účtu úložiště Azure. Důvodem je to, že překlad názvu domény je k dispozici v cloudu, ale ne v místním počítači.
...
* Získat rozdíl rozsahů stránek nefunguje mezi snímky zkopírovanými pomocí objektu BLOB přírůstkového kopírování.
...
* Emulátor úložiště podporuje hodnoty vlastností klíče oddílu a klíče řádku, které mají méně než 512 bajtů. Kromě toho celková velikost názvu účtu, názvu tabulky a názvů klíčových vlastností dohromady nesmí překročit 900 bajtů.
