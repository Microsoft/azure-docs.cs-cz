---
title: Příprava virtuálního pevného disku s Debian Linux v Azure
...
K dispozici jsou nástroje pro generování Debian VHD pro Azure, například skripty [Azure-Manage](https://github.com/credativ/azure-manage) z [credativ](https://www.credativ.com/). Toto je doporučený přístup a vytvoření obrázku od začátku. Pokud třeba chcete vytvořit virtuální pevný disk Debian 8, spusťte následující příkazy ke stažení nástroje `azure-manage` (a závislostí) a spusťte skript `azure_build_image`:
...
3. Pokud jste operační systém nainstalovali s použitím ISO, pak v `/etc/apt/source.list`Odkomentujte všechny řádky, které se týkají "`deb cdrom`".

4. Upravte soubor `/etc/default/grub` a upravte parametr **GRUB_CMDLINE_LINUX** následujícím způsobem, aby zahrnoval další parametry jádra pro Azure.
