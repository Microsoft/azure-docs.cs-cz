...
* **TimeToRun**: celkový čas spuštění tento Chaos před dokončením s úspěchem. Předtím, než bylo spuštěno dobu TimeToRun prostřednictvím rozhraní API StopChaos můžete zastavit Chaos.
...
* **WaitTimeBetweenIterations**: dobu čekat mezi iteracemi. To znamená množství času Chaos se pozastaví po provedení kruhové chyby a s dokončení odpovídající ověření stavu clusteru. Čím vyšší hodnota, nižší je míra vkládání průměrné selhání.
* **WaitTimeBetweenFaults**: dobu čekání mezi dvěma po sobě jdoucích chyb v jedné iterace. Čím vyšší hodnota, nižší současnému (nebo překrývají) chyb.
