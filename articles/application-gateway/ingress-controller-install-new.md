...
Vaše [Azure Cloud Shell](https://shell.azure.com/) už má všechny potřebné nástroje. Pokud se rozhodnete použít jiné prostředí, ujistěte se prosím, že jsou nainstalované následující nástroje příkazového řádku:
...
* `jq` – procesor JSON pro příkazový řádek: [pokyny k instalaci](https://stedolan.github.io/jq/download/)


## <a name="create-an-identity"></a>Vytvoření identity

Pomocí následujících kroků vytvořte [objekt instančního objektu služby](https://docs.microsoft.com/azure/active-directory/develop/app-objects-and-service-principals#service-principal-object)Azure Active Directory (AAD). Poznamenejte si hodnoty `appId`, `password`a `objectId` – ty se použijí v následujících krocích.
...
1. K získání `objectId` nového instančního objektu použijte `appId` z výstupu předchozího příkazu:
    ```bash
    objectId=$(az ad sp show --id $appId --query "objectId" -o tsv)
    ```
    Výstup tohoto příkazu je `objectId`, který se použije v šabloně Azure Resource Manager níže.
...
1. Vytvořte soubor parametrů, který bude použit v nasazení šablony Azure Resource Manager později.
    ...
    EOF
    ```
    Pokud chcete nasadit cluster s podporou **RBAC** , nastavte pole `aksEnabledRBAC` na `true`
...
S pokyny v předchozí části jsme vytvořili a nakonfigurovali nový cluster AKS a Application Gateway. Nyní jsme připraveni nasadit ukázkovou aplikaci a vstupní kontroler do naší nové infrastruktury Kubernetes.

### <a name="setup-kubernetes-credentials"></a>Nastavení přihlašovacích údajů Kubernetes
V následujících krocích potřebujeme příkaz Setup [kubectl](https://kubectl.docs.kubernetes.io/) , který použijeme pro připojení k našemu novému clusteru Kubernetes. [Cloud Shell](https://shell.azure.com/) už je `kubectl` nainstalované. K získání přihlašovacích údajů pro Kubernetes použijeme `az` CLI.
...
1. Použijte soubor `deployment-outputs.json` vytvořený výše a vytvořte následující proměnné.
    ```bash
...
1. Upravte nově staženou Helm-config. yaml a vyplňte oddíly `appgw` a `armAuth`.
    ```bash
   ...
   Hodnoty:
    ...
    - `armAuth.type`: `aadPodIdentity` nebo `servicePrincipal`
    ...
    - `armAuth.secretJSON`: je potřeba jenom v případě, že je zvolený tajný typ objektu služby (když je `armAuth.type` nastavená na `servicePrincipal`). 


   > [!NOTE]
   > `identityResourceID` a `identityClientID` jsou hodnoty, které byly vytvořeny během [vytváření kroků identity](https://github.com/Azure/application-gateway-kubernetes-ingress/blob/072626cb4e37f7b7a1b0c4578c38d1eadc3e8701/docs/setup/install-new.md#create-an-identity) , a lze je znovu získat pomocí následujícího příkazu:
   > ```bash
   > az identity show -g <resource-group> -n <identity-name>
   > ```
   > `<resource-group>` ve výše uvedeném příkazu je skupina prostředků vašeho Application Gateway. `<identity-name>` je název vytvořené identity. Všechny identity pro dané předplatné můžou být uvedené pomocí: `az identity list`
