...
- [Seznam služeb podporovaných nástrojem Azure Policy](/cli/azure/azure-services-the-azure-cli-can-manage)
...
- [Další informace o konfiguraci najdete v protokolech toku.](/cli/azure/azure-services-the-azure-cli-can-manage)
...
- [Nastavení diagnostiky pro cloudovou službu Azure (Classic) a virtuální počítače](/visualstudio/azure/vs-azure-tools-diagnostics-for-cloud-services-and-virtual-machines)
...
Pro další informace, rozhraní API pro klasický model nasazení poskytuje programový přístup k funkcím modelu nasazení Classic, který je k dispozici prostřednictvím Azure Portal. Sadu Azure SDK pro Python můžete použít ke správě účtů Azure Cloud Services a Azure Storage. Sada Azure SDK for Python zabalí rozhraní API modelu nasazení Classic, REST API. Všechny operace rozhraní API se provádí přes protokol TLS a vzájemně se ověřují pomocí certifikátů X. 509 v3. Služba pro správu je k dispozici v rámci služby spuštěné v Azure. Dá se taky získat přímo přes Internet z libovolné aplikace, která může poslat požadavek HTTPS a přijmout odpověď HTTPS.

- [Konfigurace TLS pro aplikaci v Azure](cloud-services-configure-ssl-certificate-portal.md)

- [Použití modelu nasazení Classic z Pythonu](cloud-services-python-how-to-use-service-management.md)
...
**Pokyny**: Azure Cloud Services nepodporuje šifrování v klidovém umístění. Důvodem je to, že Azure Cloud Services je navržený tak, aby byl bezstavový. Azure Cloud Services podporuje externí úložiště, například Azure Storage, které je ve výchozím nastavení zašifrované v klidovém stavu.  
