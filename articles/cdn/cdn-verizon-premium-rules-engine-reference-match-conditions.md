...
[Krajin](#country) | Identifikuje požadavky, které pocházejí ze zadaných zemí nebo oblastí.
...
Name | Účel
-----|--------
[IP adresa klienta](#client-ip-address) | Identifikuje požadavky, které pocházejí z konkrétní IP adresy.
...
- Identifikátor URI požadavku používá typ přístupového bodu obsahu (například/000001), který je definován v této podmínce shody:
  ...
  - Adresa URL pro CNAME Edge: Odpovídající konfigurace hraničního záznamu CNAME musí ukazovat na vybraný přístupový bod obsahu.
...
- Určete více IP adres nebo bloků IP adres tak, že každý z nich oddělujete jedním mezerou. Příklad:
  - **Příklad protokolu IPv4**: 1.2.3.4 10.20.30.40 odpovídá všem žádostem, které přicházejí buď z adresy 1.2.3.4 nebo 10.20.30.40.
...
- Syntaxe bloku IP adres je základní IP adresa, za kterou následuje lomítko a velikost předpony. Příklad:
  ...
  - **Příklad IPv6**: 1:2:3:/48 odpovídá všem požadavkům, které přicházejí z adres 1:2:3:0:0:0:0:0 až 1:2: ffff: ffff: ffff: ffff: ffff: FFFF.
- Kvůli způsobu, jakým jsou sledována nastavení mezipaměti, je tato podmínka shody nekompatibilní s následujícími funkcemi:
  - Vyplnit celou mezipaměť
  - Výchozí interní maximální stáří
  - Vynutit interní maximum – stáří
...
- Shoda se zástupnými znaky cesty URL: Nastavte [podmínku shody zástupných znaků cesty URL](#url-path-wildcard) do adresáře, který bude zabezpečen. 
...
- Shoda země: Nastavte podmínku shody země na požadovanou sadu zemí.
  - Povolil Nastavte podmínku shody země na, že se neshoduje s tím, že umožňuje přístup pouze k obsahu uloženému v umístění, které je definováno podmínkou pro porovnávání se zástupnými znaky cesty adresy URL.
  - Blokované Nastavte podmínku shody země na **shodu** s cílem zablokovat zadaným zemím přístup k obsahu uloženému v umístění, které je definováno podmínkou pro porovnávání se zástupnými znaky cesty adresy URL.

- Funkce odepření přístupu (403): Povolte [funkci odepřít přístup (403)](cdn-verizon-premium-rules-engine-reference-features.md#deny-access-403) pro replikaci části povolení nebo blokování funkce filtrování země.
...
Preferovaná deklarace DTD HTML | %{wurfl_cap_html_preferred_dtd} | Řetězec, který označuje upřednostňovanou definici typu dokumentu (DTD) mobilního zařízení pro obsah HTML. | žádný<br/>xhtml_basic<br/>html5
Vkládání obrázků | %{wurfl_cap_image_inlining} | Logická hodnota, která označuje, zda zařízení podporuje obrázky kódované v kódování Base64. | false
Je Android | %{wurfl_vcap_is_android} | Logická hodnota, která označuje, jestli zařízení používá operační systém Android. | true
Je IOS | %{wurfl_vcap_is_ios} | Logická hodnota, která indikuje, jestli zařízení používá iOS. | false
Is Smart TV | %{wurfl_cap_is_smarttv} | Logická hodnota, která indikuje, jestli je zařízení inteligentní TV. | false
...
Datum vydání | %{wurfl_cap_release_date} | Řetězec, který označuje rok a měsíc, kdy bylo zařízení přidáno do databáze WURFL.<br/><br/>Formát: `yyyy_mm` | 2013_december
...
- Pomocí možnosti **relativní k** určete, zda se porovnání adresy URL začíná před nebo za přístupovým bodem obsahu. Přístupový bod obsahu je část cesty, která se zobrazí mezi názvem hostitele CDN Verizon a relativní cestou k požadovanému prostředku (například/800001/CustomerOrigin). Identifikuje umístění podle typu serveru (například CDN nebo původ zákazníka) a čísla účtu zákazníka.
  ...
  - **Kořenová složka**: Označuje, že bod porovnání adresy URL začíná přímo za názvem hostitele CDN. 

  Například: http:\//WPC.0001.&lt; Doména&gt;800001/ **/myorigin/myFolder**/index.htm

  - **Počátek**: Označuje, že bod porovnání adresy URL začíná po přístupovém bodu obsahu (například/000001 nebo/800001/myorigin). Vzhledem k \*tomu, že se ve výchozím nastavení vytvoří název CNAME. azureedge.NET relativní ke zdrojovému adresáři Verizon CDN, Azure CDN uživatelé by měli použít **počáteční** hodnotu. 
...
- **Shody**: Vyžaduje adresu URL požadavku, aby obsahovala příponu souboru, která přesně odpovídá zadanému vzoru.
...
- **Relativní k** možnosti: Tato možnost určuje, zda se bod porovnání adresy URL začíná před nebo za přístupovým bodem obsahu.
...
/images/* /media/*      | Počátek         | Tento model odpovídá adresám CNAME CDN nebo Edge, jejichž relativní cesta začíná složkou "image" nebo "médium". <br />-CDN adresa URL: http\/:&lt; /WPC.0001. /800001/myorigin/images/Sales/event1.png&gt;domény<br />-Ukázka adresa URL CNAME hraničního okraje\/: http:/CDN.mydomain.com/images/Sales/event1.png
...
------|-------|--------
Uživatel  | Jana   | Tento model odpovídá, pokud je řetězec dotazu pro požadovanou adresu URL "? User = Jan".
Uživatel  | *     | Tento model odpovídá, pokud řetězec dotazu pro požadovanou adresu URL obsahuje parametr uživatele.
Email | Jana\* | Tento model se spáruje, když řetězec dotazu pro požadovanou adresu URL obsahuje parametr e-mailu, který začíná na Jana.
...
- **Shody**: Vyžaduje, aby požadavek obsahoval řetězec dotazu adresy URL, který odpovídá zadanému regulárnímu výrazu.
...
- Určete více hodnot tak, že každý z nich omezíte na jednu mezeru.
   ...
   Příklad: *Parametr1 = hodnota* *Hodnotab* *Parametr1 = ValueC & parametr2 = vracející*
...
\*user=\* \*optout=\* | Tento model se spáruje, když dotaz na adresu URL CDN obsahuje buď parametr User nebo OptOut.
