...
description: V tomto kurzu se dozvíte, jak zkopírovat data do zařízení Azure Data Box Heavy prostřednictvím služby kopírování dat
services: databox
author: alkohli
ms.service: databox
ms.subservice: heavy
...
---
# <a name="tutorial-use-the-data-copy-service-to-copy-data-into-azure-data-box-heavy-preview"></a>Kurz: Kopírování dat do Azure Data Box Heavy (preview) pomocí služby kopírování dat

Tento kurz popisuje, jak ingestovat data pomocí služby kopírování dat bez zprostředkujícího hostitele. Služba kopírování dat běží místně na Azure Data Box Heavy, připojí se k zařízení úložiště připojenému k síti (NAS) přes SMB a zkopíruje data do Data Box Heavy.
...
1. Dokončili jste tento kurz: [Nastavení Azure Data Box Heavy](data-box-heavy-deploy-set-up.md).
...
4. Jste připojeni k vysokorychlostní síti. Největší rychlosti kopírování je možné dosáhnout použitím dvou paralelních připojení 40 GbE (jedno na uzel). Pokud nemáte k dispozici připojení 40 GbE, doporučujeme použít alespoň dvě připojení 10 GbE (jedno na uzel). 
...
- Při kopírování dat se ujistěte, že velikost dat odpovídá omezením velikosti popsaným v článku [Azure storage and Data Box Heavy limits](data-box-heavy-limits.md).
- Pokud jsou data nahraná společností Data Box Heavy současně nahrána jinými aplikacemi mimo Data Box Heavy, může dojít k selhání úloh y nahrávání a poškození dat.
...
1. V místním webovém uživatelském rozhraní zařízení Data Box Heavy přejděte na **Spravovat** > **data kopírování**.
...
> [!IMPORTANT]
> Opakujte stejné pokyny pro kopírování dat do druhého uzlu na datové schránce Heavy.
...
Přejdete k dalšímu kurzu, kde se dozvíte, jak doložit zařízení Data Box Heavy zpět do Microsoftu.

> [!div class="nextstepaction"]
> [Odeslání zařízení Azure Data Box Heavy společnosti Microsoft](./data-box-heavy-deploy-picked-up.md)
