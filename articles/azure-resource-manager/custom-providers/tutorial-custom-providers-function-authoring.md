...
V závislosti na `x-ms-customproviders-requestpath` hlavičce můžete pro své úložiště vytvořit parametry *partitionKey* a *rowKey* , jak je znázorněno v následující tabulce:
...
> [!NOTE]
> Pokud nekopírujete kód přímo z tohoto kurzu, obsah odpovědi musí být platným kódem JSON, který nastaví hlavičku `Content-Type` na `application/json`.
...
U vlastních zprostředkovatelů se vlastní prostředek načte prostřednictvím požadavků GET. Vlastní *zprostředkovatel* nepřijímá text požadavku JSON. V případě požadavků GET koncový bod používá hlavičku `x-ms-customproviders-requestpath` k vrácení již vytvořeného prostředku.
...
U vlastních zprostředkovatelů se vlastní prostředek odebere přes požadavky na odstranění. Vlastní *zprostředkovatel* nepřijímá text požadavku JSON. Pro žádost o odstranění koncový bod používá hlavičku `x-ms-customproviders-requestpath` k odstranění již vytvořeného prostředku.
...
Aktualizovaná metoda **Run** teď zahrnuje vstupní vazbu *tableStorage* , kterou jste přidali pro Azure Table Storage. První část metody přečte hlavičku `x-ms-customproviders-requestpath` a používá `Microsoft.Azure.Management.ResourceManager.Fluent` knihovny k analýze hodnoty jako ID prostředku. Záhlaví `x-ms-customproviders-requestpath` odesílá vlastní zprostředkovatel a určuje cestu k příchozímu požadavku.
