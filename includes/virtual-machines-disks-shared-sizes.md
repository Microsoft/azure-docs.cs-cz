...
---
V současné době můžou sdílené disky povolit jenom disky s Ultra SSD a Premium. Různé velikosti disků mohou mít jiný `maxShares` limit, což není při nastavení `maxShares` hodnoty možné překročit. Pro Premium SSD jsou velikosti disků, které podporují sdílení svých disků, P15 a větší.
...
Minimální `maxShares` hodnota je 1, zatímco maximální `maxShares` hodnota je 5. Pro disky Ultra nejsou k dispozici žádná omezení velikosti, libovolná velikost Ultra disk může používat `maxShares`libovolnou hodnotu, až do a včetně maximální hodnoty.