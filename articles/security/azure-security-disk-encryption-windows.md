...
3. Po nasazení šablony, ověřte stav šifrování virtuálního počítače pomocí upřednostňované metody:
     ...
     -  Vyberte virtuální počítač a potom klikněte na **disky** pod **nastavení** záhlaví a ověřit stav šifrování na portálu. V grafu v části **šifrování**, zobrazí se, jestli je povolené. 
...
## <a name="enable-encryption-using-azure-ad-client-certificate-based-authentication"></a>Povolte šifrování pomocí služby Azure AD na základě ověření klientského certifikátu.
Ověření klientského certifikátu můžete použít s nebo bez certifikátu KEK. Tyto skripty vyžadují, aby [požadavky Azure Disk Encryption](azure-security-disk-encryption-prerequisites.md) se dokončí. Než začnete používat skripty prostředí PowerShell, byste už měli mít certifikát odeslán do trezoru klíčů a nasazené do virtuálního počítače. Pokud používáte KEK příliš, se klíč KEK by měl existovat. Další informace najdete v tématu [ověřování pomocí certifikátu pro službu Azure AD](azure-security-disk-encryption-prerequisites.md#bkmk_Cert) požadavky článku.
...
### <a name="enable-encryption-using-certificate-based-authentication-and-a-kek-with-azure-powershell"></a>Povolit šifrování pomocí ověřování na základě certifikátů a KEK pomocí Azure Powershellu
