...
> [!TIP]
> K dispozici je řada datových sad s modulem runtime R. Pokud chcete získat seznam nainstalovaných datových `library(help="datasets")` sad, zadejte na příkazovém řádku r.
...
2. Předpovídání zastavování vzdálenosti z těchto nových hodnot rychlosti.
   ...
   Vzhledem k tomu, že váš model je založen na algoritmu **rxLinMod** , který je součástí balíčku **RevoScaleR** , je místo obecné funkce jazyka R `predict` volána funkce [rxPredict](https://docs.microsoft.com/machine-learning-server/r-reference/revoscaler/rxpredict) .
...
> [!NOTE]
> V tomto příkladu skriptu je `str` funkce přidána během testovací fáze pro kontrolu schématu dat vrácených z R. Příkaz můžete odebrat později.
>
