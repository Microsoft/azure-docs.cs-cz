...
1. Nejdřív nakonfigurujeme ověřování/autorizaci, takže kliknete na název aplikace Function App (vedle ikony &lt;Z&gt; funkcí) a zobrazí se stránka s přehledem.
...
1. Nastavte položku Adresa URL odpovědi na tu, kterou jste si poznamenali, když jste nakonfigurovali redirect_uri udělení ověřovacího kódu v API Management dříve.
   ...
   Když je teď na `Echo API`povolená autorizace uživatele OAuth 2,0, Konzola pro vývojáře získá přístupový token pro uživatele před voláním rozhraní API.

1. Přejděte k libovolné operaci pod `Echo API` na portálu pro vývojáře a vyberte **vyzkoušet** , abyste vás přenesli do konzole pro vývojáře.
...
1. Po úspěšném přihlášení se do žádosti přidá hlavička `Authorization: Bearer` s přístupovým tokenem Azure AD B2C v kódování Base64. 
...
* Zvažte použití Graph API Azure AD k přiřazení vlastních deklarací identity a k ověření, jestli se v tokenu nacházejí, pomocí zásad API Management.
