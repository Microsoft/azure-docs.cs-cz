...
1. Vyberte **virtuálních počítačů** karta na portálu publikování. Do pole zobrazí zadejte název vaší nabídky. Název nabídky je obvykle název produktu nebo služby, který chcete prodávat na webu Azure Marketplace.
1. Vyberte **Vytvořit**.
...
1. **Přidejte skladovou jednotku.** SKU vyžaduje identifikátor, který se používá v adrese URL. Identifikátor musí být jedinečný ve vašem profilu publikování, ale neexistuje žádné riziko kolizí identifikátor s jinými vydavateli.
   ...
   >
   >
1. **Přidejte souhrnný popis skladové jednotky.** Souhrnný popis jsou viditelné pro zákazníky, abyste měli dělat snadno čitelný. Tyto informace není potřeba zamykat, dokud se fáze "Nepřejde k přípravě". Do té doby je můžete upravovat.
1. Pokud používáte skladové jednotky založené na Windows, přejděte na navrhované odkazy, kde získáte schválené verze Windows Serveru.
...
1. Na stránce základní image zvolte **vytvořit virtuální počítač** přejdete do nového [portálu Microsoft Azure][link-azure-portal].
    ...
    ![Kreslení][img-acom-1]
1. Přihlaste se k portálu pomocí účtu Microsoft a heslem pro předplatné Azure, které chcete použít.
1. Postupujte podle výzev a vytvořte virtuální počítač pomocí základní image, kterou jste vybrali. Budete muset poskytnout hostiteli name (název počítače), uživatelské jméno (registrovaný jako správce) a heslo pro virtuální počítač.

    ![Kreslení][img-portal-vm-create]
1. Vyberte velikost nasazovaného virtuálního počítače:
...
1. Vyberte **umístění** pro nasazení:
...
1. Klikněte na možnost **Vytvořit**. Virtuální počítač se začne nasazovat. Během několika minut budete mít úspěšné nasazení a můžete začít vytvářet image pro svoje skladové jednotky.
...
1. Otevře se okno Virtual machines. Ujistěte se, že běží virtuální počítač, který chcete propojit se službou a vyberte ho ze seznamu nasazených virtuálních počítačů.
1. Otevře se okno s popisem zvoleného virtuálního počítače. V horní části stránky, klikněte na tlačítko **připojit**.
1. Zobrazí se výzva k zadání uživatelského jména a hesla, které jste určili během zřizování.
...
1. Otevře se okno Virtual machines. Vyberte virtuální počítač, který jste nasadili.
1. Otevře se okno s popisem zvoleného virtuálního počítače.
1. Klikněte na **Vlastnosti**.
1. První část názvu domény je název služby. Názvem hostitele je název virtuálního počítače.

    ![Kreslení][img-portal-vm-rdp]
1. Rutina pro stažení souboru RDP pro vytvořený virtuální počítač na místní plochu správce je následujícím způsobem.
...
1. Přejděte na **Moje image**a potom vyberte image virtuálního počítače, ze kterého chcete virtuální počítač nasadit:
   ...
   1. Počet disků vám pomůže určit, jaký typ image nasazujete, protože většina imagí virtuálních počítačů mají více než jeden disk. Je však stále možné mít image virtuálního počítače s pouze jedním diskem operačního systému, který by pak měl **počet disků** nastavena na hodnotu 1.

      ![Kreslení][img-manage-vm-select]
1. Postupujte podle pokynů Průvodce vytvořením virtuálního počítače a zadejte virtuální počítač název, virtuální počítač velikost, umístění, uživatelské jméno a heslo.
...
1. Otevřete certifikační nástroj a klikněte **spustit nový Test** tlačítko.
1. Z **informace o testu** obrazovky, zadejte název pro testovací běh.
1. Zvolte, jestli váš virtuální počítač používá Linux nebo Windows. Podle toho, který systém zvolíte, vyberte další možnosti.
...
1. Pokud používáte ověřování pomocí hesla, zadejte název systému DNS (Domain Name), uživatelské jméno a heslo.
1. Pokud používáte ověřování souborem klíče, zadejte název DNS, uživatelské jméno a umístění privátního klíče.
...
1.  Průzkumník úložišť Microsoft
1.  Azure CLI
...
1. Stáhněte si [AzureStorageExplorer6Preview3.zip](https://azurestorageexplorer.codeplex.com/downloads/get/891668) a nainstalovat po rozbalení ho.

    ![Kreslení](media/marketplace-publishing-vm-image-creation/img5.2_02.png)

1. Po dokončení instalace, otevřete aplikaci.
1. Klikněte na tlačítko **přidat účet**.

    ![Kreslení](media/marketplace-publishing-vm-image-creation/img5.2_03.png)

1. Zadejte název účtu úložiště, klíč účtu úložiště a doména koncových bodů úložiště. Toto je účet úložiště ve vašem předplatném Azure, kde mají uchovávat vašeho virtuálního pevného disku na portálu Azure portal.

    ![Kreslení](media/marketplace-publishing-vm-image-creation/img5.2_04.png)

1. Průzkumník služby Azure Storage po připojení k vašemu účtu konkrétní úložiště, se začnou zobrazovat všechny obsahuje v rámci účtu úložiště. Vyberte kontejner, ve kterém jste zkopírovali soubor VHD disku operačního systému (také datové disky případě, že jsou použitelné pro váš scénář).

    ![Kreslení](media/marketplace-publishing-vm-image-creation/img5.2_05.png)

1. Po výběru kontejner objektů blob Azure Storage Explorer začne zobrazovat soubory v kontejneru. Vyberte soubor obrázku (VHD), který potřebuje k odeslání.

    ![Kreslení](media/marketplace-publishing-vm-image-creation/img5.2_06.png)

1.  Po výběru souboru VHD v kontejneru, klikněte na tlačítko **zabezpečení** kartu.

    ![Kreslení](media/marketplace-publishing-vm-image-creation/img5.2_07.png)

1.  V **zabezpečení kontejneru objektů Blob** dialogové okno pole, ponechte výchozí hodnoty na **úroveň přístupu** kartu a potom klikněte na tlačítko **sdílené přístupové podpisy** kartu.

    ![Kreslení](media/marketplace-publishing-vm-image-creation/img5.2_08.png)

1.  Podle následujících pokynů k vygenerování sdíleného přístupového podpisu URI pro VHD image:
...
1.  Aby bylo zajištěno, že generované sdílený přístup podpisu URI funguje, klikněte na tlačítko **otestovat v prohlížeči**. By se měl spustit proces stahování.

1.  Zkopírujte URI sdíleného přístupového podpisu. Je to identifikátor, který se má vložit do Portálu publikování.

1.  Opakujte kroky 6 až 10 pro každý virtuální pevný disk ve skladové Položce.
...
1.  Konfigurace Microsoft Azure Storage Exploreru k předplatnému s přihlášení ke svému účtu

    ![Kreslení](media/marketplace-publishing-vm-image-creation/img5.2_11.png)

1.  Přejděte do účtu úložiště a pak vyberte kontejner

1.  Vyberte **"Get sdílené složky přístupový podpis..."** s použitím pravým tlačítkem myši na sady **kontejneru**

    ![Kreslení](media/marketplace-publishing-vm-image-creation/img5.2_12.png)

1.  Čas spuštění aktualizace, čas vypršení platnosti a oprávnění podle následujících
...
1.  Zkopírujte URI sdíleného přístupového podpisu kontejneru
...
1.  Ujistěte se, že generované sdílený přístup podpisu URI funguje, že ji otestujte v prohlížeči. By se měl spustit proces stahování

1.  Zkopírujte URI sdíleného přístupového podpisu. Je to identifikátor, který se má vložit do Portálu publikování.

1.  Tyto kroky zopakujte pro každý virtuální pevný disk ve skladové jednotce.
...
1.  Po stažení, nainstalujte

1.  Vytvoření Powershellu (nebo jiného spustitelného souboru skriptu) soubor s následujícím kódem a uloží do místního prostředí
    ...
    c. **`<Permission Start Date>`**: Pokud chcete ochranu pro čas UTC, vyberte den před aktuálním datem. Například pokud je aktuální datum 25. října 2016, pak hodnota by měla být 10/25/2016. Pokud používáte Azure CLI 2.0 (příkaz az), zadejte datum a čas v počáteční a koncové datum, například: 10-25-2016T00:00:00Z.
...
1.  Otevřete editor Powershellu v režimu "Spustit jako správce" a otevřete soubor v kroku #3. Můžete použít libovolný editor skriptu, který je k dispozici na vašem operačním systému.

1.  Spusťte skript a poskytne vám adresa URL SAS pro úrovně přístupu ke kontejneru
...
1.  Teď získáte úrovni kontejneru adresy URL SAS a budete muset přidat název virtuálního pevného disku v ní.

    Adresa URL SAS úrovni kontejneru #

    `https://st20151.blob.core.windows.net/vhds?st=2016-10-25T07%3A00%3A00Z&se=2016-11-02T07%3A00%3A00Z&sp=rl&sv=2015-12-11&sr=c&sig=wnEw9RfVKeSmVgqDfsDvC9IHhis4x0fc9Hu%2FW4yvBxk%3D`

1.  Vložit název virtuálního pevného disku za název kontejneru v adrese URL SAS, jak je znázorněno níže `https://st20151.blob.core.windows.net/vhds/<VHDName>?st=2016-10-25T07%3A00%3A00Z&se=2016-11-02T07%3A00%3A00Z&sp=rl&sv=2015-12-11&sr=c&sig=wnEw9RfVKeSmVgqDfsDvC9IHhis4x0fc9Hu%2FW4yvBxk%3D`
...
1. Vyberte **imagí virtuálních počítačů** kartu.
1. Identifikátor uvedený v horní části stránky je ve skutečnosti identifikátor nabídky, nikoli identifikátor skladové jednotky.
1. Vyplňte vlastnosti z kategorie **SKU** oddílu.
1. V části **operační systém řady**, klikněte na typ operačního systému, které jsou spojené s operačním systémem virtuálního pevného disku.
1. V **operačního systému** pole, popište operační systém. Zvažte formát jako je řada operačních systémů, typ, verze a aktualizace. Příkladem je "Windows Server Datacenter 2014 R2."
1. Vyberte až šest doporučené velikosti virtuálního počítače. Jedná se o doporučení, která se zobrazí zákazníka v okně cenová úroveň na webu Azure Portal chvíli kdy se rozhodnou vaši image koupit a nasadit. **Jedná se jenom o doporučení. Zákazník se může vybrat jakoukoli velikost virtuálního počítače, které se vejdou disky určené ve vaší imagi.**
1. Zadejte verzi. Pole verze zapouzdřuje sémantickou verzi k identifikaci produktu a jeho aktualizace:
...
1. V **URL virtuálního pevného disku operačního systému** zadejte sdílený přístupový podpis pro operační systém virtuální pevný disk vytvořit identifikátor URI.
1. Pokud je této skladové jednotce přidružené datové disky, vyberte číslo logické jednotky (LUN), ke které byste chtěli tento datový disk připojit při nasazení.
1. V **URL virtuálního pevného disku LUN X** zadejte sdílený přístupový podpis pro první datový virtuální pevný disk vytvořit identifikátor URI.
