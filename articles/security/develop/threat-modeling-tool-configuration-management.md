...
| **IoT Cloud Gateway** | <ul><li>[Zajistěte, aby cloudová brána implementovala proces, který zachová firmware připojených zařízení v aktuálním stavu.](#cloud-firmware)</li></ul> |
...
## <a id="csp-js"></a>Implementace zásad zabezpečení obsahu (CSP) a zakázání vloženého JavaScriptu
...
## <a id="xss-filter"></a>Povolit filtr skriptování XSS v prohlížeči
...
## <a id="trace-deploy"></a>Aplikace ASP.NET musí před nasazením zakázat trasování a ladění.
...
| **Použitelné technologie** | Obecné |
| **Atributy**              | Není k dispozici  |
| **Odkazy**              | [Přehled ladění ASP.NET](https://msdn.microsoft.com/library/ms227556.aspx), [Přehled trasování ASP.NET](https://msdn.microsoft.com/library/bb386420.aspx), [postupy: Povolit trasování pro ASP.NET aplikaci](https://msdn.microsoft.com/library/0x5wc973.aspx), [postupy: Povolit ladění pro aplikace ASP.NET](https://msdn.microsoft.com/library/e8z01xdh(VS.80).aspx) |
...
## <a id="js-trusted"></a>Přístup k javascriptům třetích stran jenom z důvěryhodných zdrojů
...
| **Použitelné technologie** | Obecné |
| **Atributy**              | Není k dispozici  |
| **Odkazy**              | [OWASP kliknutí na tahák pro ochranu](https://www.owasp.org/index.php/Clickjacking_Defense_Cheat_Sheet)pomocí konektorů, [interní funkce IE – boj po kliknutí s X-frame-Options](https://blogs.msdn.microsoft.com/ieinternals/2010/03/30/combating-clickjacking-with-x-frame-options/) |
...
## <a id="cors-aspnet"></a>Zajistěte, aby byly povoleny pouze důvěryhodné zdroje, pokud je v ASP.NET webových aplikacích povoleno CORS.
...
## <a id="local-js"></a>Používat místně hostované nejnovější verze knihoven JavaScript
...
| **Použitelné technologie** | Obecné |
| **Atributy**              | Není k dispozici  |
| **Odkazy**              | [Část zabezpečení IE8 V: Komplexní ochrana](https://blogs.msdn.com/ie/archive/2008/07/02/ie8-security-part-v-comprehensive-protection.aspx), [typ MIME](https://en.wikipedia.org/wiki/Mime_type) |
...
## <a id="standard-finger"></a>Odebrání standardních hlaviček serveru na webech Windows Azure, aby nedocházelo k otiskům prstů
...
| **Kroky** | Systémy brány firewall zabraňují neoprávněnému přístupu k prostředkům počítače. Pokud chcete získat přístup k instanci databázového stroje SQL Server přes bránu firewall, musíte bránu firewall nakonfigurovat na počítači, na kterém běží SQL Server, aby se povolil přístup. |

## <a id="cors-api"></a>Ujistěte se, že je povolený jenom důvěryhodný původ, pokud je v rozhraní Web API pro ASP.NET povolené CORS.
...
**Přístup – 1** Povolování CORS s middlewarem: Pokud chcete povolit CORS pro celou aplikaci, přidejte middleware CORS do kanálu požadavků pomocí metody rozšíření UseCors. Pokud přidáte middleware CORS pomocí třídy CorsPolicyBuilder, můžete zadat zásady pro více zdrojů. Chcete-li to provést dvěma způsoby:
...
**Přístup 2** Povolování CORS v MVC: Vývojáři mohou případně použít MVC k aplikování konkrétní CORS na akci, na kontroler nebo globálně pro všechny řadiče.
...
## <a id="config-sensitive"></a>Šifrovat oddíly konfiguračních souborů webového rozhraní API, které obsahují citlivá data
...
## <a id="admin-strong"></a>Ujistěte se, že všechna rozhraní správce jsou zabezpečená pomocí silných přihlašovacích údajů.
...
## <a id="partition-iot"></a>Zašifrujte operační systém a další oddíly zařízení IoT pomocí bitových zámků.
...
| **Odkazy**              | [Přehled správy zařízení IoT Hub](https://azure.microsoft.com/documentation/articles/iot-hub-device-management-overview/), [jak aktualizovat firmware zařízení](https://docs.microsoft.com/azure/iot-hub/tutorial-firmware-update) |
...
## <a id="controls-policies"></a>Ujistěte se, že zařízení mají ovládací prvky zabezpečení koncového bodu nakonfigurované podle zásad pro organizace.
...
| **Kroky** | <p>Úložiště klíčů: Doporučuje se ukládat přístupové klíče Azure Storage do Azure Key Vault jako tajný kód a nechat aplikace získat klíč z trezoru klíčů. Tento postup je doporučený z následujících důvodů:</p><ul><li>Aplikace nikdy nebude mít klíč úložiště pevně zakódované v konfiguračním souboru. tím se odstraní tento způsob někoho, kdo získá přístup k klíčům bez konkrétního oprávnění.</li><li>Přístup ke klíčům lze ovládat pomocí Azure Active Directory. To znamená, že vlastník účtu může udělit přístup k několik aplikacím, které potřebují načíst klíče z Azure Key Vault. Ostatní aplikace nebudou mít přístup ke klíčům bez udělení oprávnění výslovně.</li><li>Opětovné generování klíče: Doporučuje se mít k dispozici proces pro opětovné vygenerování přístupových klíčů úložiště Azure z bezpečnostních důvodů. Podrobnosti o tom, proč a jak naplánovat vygenerování klíčů, najdete v článku Referenční příručka zabezpečení Azure Storage</li></ul>|

## <a id="cors-storage"></a>Ujistěte se, že je povolený jenom důvěryhodný původ, pokud je v Azure Storage povolené CORS.
...
## <a id="throttling"></a>Povolit funkci omezování služby WCF
...
| **Kroky** | <p>Omezení použití systémových prostředků by mohlo vést k vyčerpání prostředků a nakonec k odepření služby.</p><ul><li>**POŽADOVANÉ** Windows Communication Foundation (WCF) nabízí možnost omezit žádosti o služby. Povolení příliš velkého počtu žádostí klienta může naplavit systém a vyčerpat jeho prostředky. Na druhé straně může být povolení jenom malého počtu požadavků ke službě zabránit legitimním uživatelům v používání služby. Každá služba by měla být individuálně vyladěna a nakonfigurována tak, aby umožňovala odpovídající množství prostředků.</li><li>**Doporučení** Povolte funkci omezování služby WCF a nastavte limity vhodné pro vaši aplikaci.</li></ul>|
...
## <a id="info-metadata"></a>WCF – zpřístupnění informací prostřednictvím metadat
...
| **Atributy**              | Není k dispozici  |
| **Odkazy**              | [MSDN](https://msdn.microsoft.com/library/ff648500.aspx), [obohacení království](https://vulncat.fortify.com) |
| **Kroky** | Metadata můžou útočníkům pomáhat se systémem a naplánovat formu útoku. Služby WCF je možné nakonfigurovat tak, aby vystavily metadata. Metadata poskytují podrobné informace o popisu služby a neměly by se vysílat v produkčních prostředích. Vlastnosti třídy oddílu serviceMetadata definují, zda bude služba vystavovat metadata. `HttpGetEnabled`  /  `HttpsGetEnabled` | 
