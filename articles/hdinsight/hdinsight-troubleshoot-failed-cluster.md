...
[Řídicí panel uživatelského rozhraní Ambari](#view-cluster-configuration-settings-with-the-ambari-ui) (`https://<clustername>.azurehdinsight.net`) poskytuje přehled stavu clusteru, jako je například doba provozu, paměti, sítě a využití procesoru, HDFS, použití disku a tak dále. Pomocí části hostitelé v Ambari můžete zobrazit prostředky na úrovni hostitele. Můžete také zastavit a restartovat služby.
...
Pokud se server WebHCat stále nespustí, zkontrolujte zprávy o chybách v protokolu operací. Podrobnější informace najdete v souborech `stderr` a `stdout`, na které se odkazuje v uzlu.

#### <a name="webhcat-times-out"></a>WebHCat časový limit

Vyprší časový limit An HDInsight brány na odezvy, které trvá déle než dvě minuty, a vrátí `502 BadGateway`. WebHCat se dotazuje na úlohy PŘÍZe na stavech úloh a pokud by PŘÍZe trvala déle než dvě minuty, může to vytrvat i vypršení tohoto požadavku.

V takovém případě zkontrolujte následující protokoly v adresáři `/var/log/webhcat`:
...
> [!NOTE]  
> Každý `webhcat.log` se provádí za den a generuje soubory s názvem `webhcat.log.YYYY-MM-DD`. Vyberte příslušný soubor pro časový rozsah, který zkoumáte.
...
Když je WebHCat pod zatížením, s více než 10 otevřenými sokety trvá vytvoření nových připojení soketu déle, což může mít za následek časový limit. Chcete-li zobrazit seznam síťových připojení k WebHCat a z nich, použijte `netstat` pro aktuální aktivní hlavnímu uzlu:
...
1. Odeslání úlohy PŘÍZe může trvat dostatečně dlouho, aby se mohl zapříčinit časový limit.
    ...
    Pokud otevřete soubor protokolu `/var/log/webhcat/webhcat.log` a vyhledáte "úlohu zařazenou do fronty", může se zobrazit více položek, kde doba provádění je příliš dlouhá (> 2000 MS), s položkami, které zvyšují dobu čekání.
...
2. Zpracování PŘÍZe může trvat dlouhou dobu, což může způsobit vypršení časových limitů.
    ...
    * Seznam úloh, které jsou starší než sedm dní: JobHistoryServer v HDInsight se nakonfiguruje tak, aby uchovávala informace o dokončených úlohách po dobu sedmi dnů (`mapreduce.jobhistory.max-age-ms` hodnota). Při pokusu o výčet vyčištěných úloh dojde k vypršení časového limitu.

Diagnostikujte tyto problémy:

1. Určení rozsahu času UTC pro řešení potíží
2. Vyberte odpovídající soubory `webhcat.log`.
...
1. Stavový kód HTTP 500
    ...
    Ve většině případů, kdy WebHCat vrátí 500, obsahuje chybová zpráva podrobnosti o selhání. V opačném případě se podíváte na `webhcat.log` pro upozornění a chybové zprávy.

2. Selhání úlohy

    Můžou nastat případy, kdy interakce s WebHCat jsou úspěšné, ale úlohy selžou.

    Templeton shromažďuje výstup konzoly úloh jako `stderr` v `statusdir`, což je často užitečné při řešení potíží. `stderr` obsahuje identifikátor aplikace nitě pro vlastní dotaz.
