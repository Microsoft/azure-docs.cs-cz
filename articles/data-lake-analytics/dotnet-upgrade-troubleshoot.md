...
- Funkce XmlWriter vyvolá neplatné náhradní páry.
  - Pro aplikace, které cílí na .NET Framework 4.5.2 nebo předchozí verze, zapisuje neplatnou náhradní dvojici pomocí zpracování záložního použití výjimek, vždy výjimku vyvolá. Pro aplikace, které cílí na .NET Framework 4,6, vyvolá pokus o zápis neplatného náhradního páru `ArgumentException`.
...
- HtmlTextWriter nevykresluje `<br/>` element správně.
  - Počínaje .NET Framework 4,6, volání `HtmlTextWriter.RenderBeginTag()` a `HtmlTextWriter.RenderEndTag()` pomocí elementu `<BR />` bude správně vkládat pouze jeden `<BR />` (místo dvou).
  - Ovlivněné knihovny: System. Web
  - Navrhovaná akce: Ujistěte se, že vkládáte množství `<BR />`, které očekáváte, takže se v produkční úloze neobjeví žádné náhodné chování.

- Volání CreateDefaultAuthorizationContext s argumentem null se změnilo.
  - Implementace AuthorizationContext vrácená voláním `CreateDefaultAuthorizationContext(IList<IAuthorizationPolicy>)` s prázdným argumentem authorizationPolicies se změnila jeho implementace v .NET Framework 4,6.
...
- Algoritmem RSACng nyní správně načítá klíče RSA nestandardní velikosti klíče.
  - V .NET Framework verzích starších než 4.6.2 nemůžou zákazníci s nestandardními velikostmi klíčů pro certifikáty RSA přistupovat k těmto klíčům prostřednictvím metod rozšíření `GetRSAPublicKey()` a `GetRSAPrivateKey()`. Je vyvolána `CryptographicException` se zprávou "požadovaná velikost klíče není podporována". S .NET Framework 4.6.2 Tento problém byl vyřešen. Podobně `RSA.ImportParameters()` a `RSACng.ImportParameters()` nyní fungují s nestandardními velikostmi klíčů bez vyvolání `CryptographicException`.
...
- Volání konstruktorů hodnota ClaimsIdentity
  - Počínaje .NET Framework 4.6.2 dojde ke změně v tom, jak `T:System.Security.Claims.ClaimsIdentity` konstruktory s parametrem `T:System.Security.Principal.IIdentity` nastavili vlastnost `P:System.Security.Claims.ClaimsIdentify.Actor`. Pokud je argumentem `T:System.Security.Principal.IIdentity` `T:System.Security.Claims.ClaimsIdentity` objekt a vlastnost `P:System.Security.Claims.ClaimsIdentify.Actor` objektu `T:System.Security.Claims.ClaimsIdentity` není `null`, je vlastnost `P:System.Security.Claims.ClaimsIdentify.Actor` připojena pomocí metody `M:System.Security.Claims.ClaimsIdentity.Clone`. V rozhraní .NET 4.6.1 a starších verzích je vlastnost `P:System.Security.Claims.ClaimsIdentify.Actor` připojena jako stávající odkaz. Z důvodu této změny počínaje .NET Framework 4.6.2 není vlastnost `P:System.Security.Claims.ClaimsIdentify.Actor` nového objektu `T:System.Security.Claims.ClaimsIdentity` rovna hodnotě `P:System.Security.Claims.ClaimsIdentify.Actor` `T:System.Security.Principal.IIdentity` argumentu konstruktoru. V .NET Framework 4.6.1 a dřívějších verzích se rovná.
