...
Auditování pro [Azure SQL Database](sql-database-technical-overview.md) a [Azure synapse Analytics](../synapse-analytics/sql-data-warehouse/sql-data-warehouse-overview-what-is.md) podporuje zápis událostí databáze na [účet Azure Storage](../storage/common/storage-account-overview.md) za virtuální sítí a bránou firewall. 

Tento článek vysvětluje dva způsoby, jak nakonfigurovat Azure SQL Server a účet úložiště Azure pro tuto možnost. První používá Azure Portal, druhá používá REST.
...
Připojte se k [Azure Portal](https://portal.azure.com) k vašemu předplatnému. Přejděte do skupiny prostředků a na server služby Azure SQL Database.
...
|`<sql database server>`| Název serveru služby Azure SQL Database|
...
1. Zaregistrujte Azure SQL Database Server pomocí Azure Active Directory (Azure AD). Použijte buď PowerShell, nebo REST API.
...
2. Otevřete [Azure Portal](https://portal.azure.com). Přejděte na svůj účet úložiště. Vyhledejte **Access Control (IAM)** a klikněte na **Přidat přiřazení role**. Přiřazení role RBAC **Přispěvatel dat objektů BLOB úložiště** k Azure SQL Server hostování vaší databáze SQL Azure, kterou jste zaregistrovali v Azure Active Directory (Azure AD) jako v předchozím kroku.
...
3. Nakonfigurujte [zásady auditování objektů BLOB v Azure SQL serveru](/rest/api/sql/server%20auditing%20settings/createorupdate)bez zadání *storageAccountAccessKey*:
...
- [Pomocí PowerShellu vytvořte koncový bod služby virtuální sítě a potom pravidlo virtuální sítě pro Azure SQL Database.](sql-database-vnet-service-endpoint-rule-powershell.md)
