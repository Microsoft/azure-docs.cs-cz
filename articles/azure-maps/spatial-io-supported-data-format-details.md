...
| `atom:link`          | ano     | ano     |                                                                                                                            |
| `atom:name`          | ano     | ano     |                                                                                                                            |
| `BalloonStyle`       | partial | partial | `displayMode`není podporováno. Převedeno na `PopupTemplate`. Chcete-li zapisovat, `popupTemplate` přidejte vlastnost jako vlastnost funkce, pro kterou chcete zapisovat. |
| `begin`              | ano     | ano     |                                                                                                                            |
...
| `east`               | ano     | ano     |                                                                                                                            |
| `end`                | ano     | ano     |                                                                                                                            |
| `ExtendedData`       | ano     | ano     | Podporuje náhrady netypového `SimpleData` typu `Schema` `$[dataName]` `Data`nebo entit formuláře.                      |
...
| `heading`            | partial | ne      | Analyzovány `SimpleDataLayer`, ale nebyly vykreslovány. Zapisuje pouze, pokud jsou data uložena ve vlastnosti obrazce.                 |
| `hotSpot`            | ano     | partial | Zapisuje pouze, pokud jsou data uložena ve vlastnosti obrazce. Jednotky jsou výstupy jenom jako pixely.                         |
| `href`               | ano     | ano     |                                                                                                                            |
| `Icon`               | partial | partial | Analyzovány `SimpleDataLayer`, ale nebyly vykreslovány. Zapíše vlastnost Icon obrazce pouze v případě, že obsahuje data identifikátoru URI. Podporuje `href` se jenom. |
| `IconStyle`          | partial | partial | `icon`hodnoty `heading`, `colorMode`, a `hotspots` jsou analyzovány, ale nejsou vykreslovány`SimpleDataLayer`         |
...
| `Link`               | ano     | ne      | Pro síťová `href` propojení je podporována pouze vlastnost.                                                                   |
...
| `outline`            | ano     | ano     |                                                                                                                            |
| `overlayXY`          | ne      | ne      |                                                                                                                            |
| `Pair`               | partial | ne      | Podporován `StyleMap` je `normal` pouze styl ve stylu. `highlight`není podporováno.                                   |
| `phoneNumber`        | ano     | ano     |                                                                                                                            |
| `PhotoOverlay`       | ne      | ne      |                                                                                                                            |
| `Placemark`          | ano     | ano     |                                                                                                                            |
| `Point`              | ano     | ano     |                                                                                                                            |
...
| `south`              | ano     | ano     |                                                                                                                            |
| `Style`              | ano     | ano     |                                                                                                                            |
| `StyleMap`           | partial | ne      | Podporován je pouze normální styl v `StyleMap` rámci.                                                                        |
...
| `TimeStamp`          | ano     | ano     |                                                                                                                            |
| `value`              | ano     | ano     |                                                                                                                            |
| `viewRefreshMode`    | partial | ne      |  Pokud odkazujete na službu WMS, je podporována `onStop` pouze pro základní překryvy. Připojí `BBOX=[bboxWest],[bboxSouth],[bboxEast],[bboxNorth]` se k adrese URL a aktualizuje se, jak se přesune mapa.  |
| `visibility`         | ano     | ano     |                                                                                                                            |
| `west`               | ano     | ano     |                                                                                                                            |
| `when`               | ano     | ano     |                                                                                                                            |
| `width`              | ano     | ano     |                                                                                                                            |
...
| `atom:uri`               | ano     | ano   |                                                                                                |
| `geo:lat`                | ano     | ne    | Zapsáno `georss:point`jako.                                                                   |
| `geo:lon`                | ano     | ne    | Zapsáno `georss:point`jako.                                                                   |
| `geo:long`               | ano     | ne    | Zapsáno `georss:point`jako.                                                                   |
...
| `geourl:latitude`        | ano     | ne    | Zapsáno `georss:point`jako.                                                                   |
| `geourl:longitude`       | ano     | ne    | Zapsáno `georss:point`jako.                                                                   |
...
| `rss:author`             | ano     | partial | Zapsáno `atom:author`jako.                                                                 |
| `rss:category`           | ano     | partial | Zapsáno `atom:category`jako.                                                               |
| `rss:channel`            | ano     | ne    |                                                                                                |
| `rss:cloud`              | ano     | ne    |                                                                                                |
| `rss:comments`           | ano     | ne    |                                                                                                |
| `rss:copyright`          | ano     | partial | Zapsáno `atom:rights` jako obrazec if, který `rights` `properties` již neobsahuje vlastnost.       |
| `rss:description`        | ano     | partial | Zapsáno `atom:content` jako obrazec if, který `content` `properties` již neobsahuje vlastnost.      |
| `rss:docs`               | ano     | ne    |                                                                                                |
| `rss:enclosure`          | ano     | ne    |                                                                                                |
| `rss:generator`          | ano     | ne    |                                                                                                |
| `rss:guid`               | ano     | partial | Zapsáno `atom:id` jako obrazec `id` `properties` if, který již neobsahuje vlastnost.         |
| `rss:image`              | ano     | partial | Zapsáno `atom:logo` jako obrazec if, který `logo` `properties` již neobsahuje vlastnost.      |
| `rss:item`               | ano     | partial | Zapsáno `atom:entry`jako.                                                                  |
| `rss:language`           | ano     | ne    |                                                                                                |
| `rss:lastBuildDate`      | ano     | partial | Zapsáno `atom:updated` jako obrazec `updated` `properties` if, který již neobsahuje vlastnost.     |
| `rss:link`               | ano     | partial | Zapsáno `atom:link`jako.                                                                   |
| `rss:managingEditor`     | ano     | partial | Zapsáno `atom:contributor`jako.                                                            |
| `rss:pubDate`            | ano     | partial | Zapsáno `atom:published` jako obrazec if, který `published` `properties` již neobsahuje vlastnost.  |
| `rss:rating`             | ano     | ne    |                                                                                                |
| `rss:skipDays`           | ano     | ne    |                                                                                                |
| `rss:skipHours`          | ano     | ne    |                                                                                                |
| `rss:source`             | ano     | partial | Zapsáno `atom:source` jako obsahující `atom:link`.                                       |
| `rss:textInput`          | ano     | ne    |                                                                                                |
| `rss:title`              | ano     | partial | Zapsáno `atom:title`jako.                                                                  |
| `rss:ttl`                | ano     | ne    |                                                                                                |
...
| `gml:MultiCurve`        | ano  | ne    | Čte `gml:LineString` pouze členy. Zapsáno jako`gml.MultiLineString`                  |
...
| `gml:MultiSurface`      | ano  | ne    | Čte `gml:Polygon` pouze členy. Zapsáno jako`gml.MultiPolygon`                        |
| `gml:name`              | ano  | ano   |                                                                                        |
...
| `gpx_style:line`         | partial | partial | `color``opacity` `width` `lineCap`                                           |
...
| `gpxx:RouteExtension`    | partial | ne      | Všechny vlastnosti jsou čteny `properties`do. Používá `DisplayColor` se jenom.                     |
| `gpxx:TrackExtension`    | partial | ne      | Všechny vlastnosti jsou čteny `properties`do. Používá `DisplayColor` se jenom.                     |
| `gpxx:WaypointExtension` | partial | ne      | Všechny vlastnosti jsou čteny `properties`do. Používá `DisplayColor` se jenom.                     |
