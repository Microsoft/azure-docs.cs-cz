...
- **Podmíněně připravené pro Azure**: počítač se může v Azure spustit, ale nemusí mít plnou podporu Azure. Například počítač, na kterém běží starší verze Windows serveru, se v Azure nepodporuje. Než tyto počítače migrujete do Azure, musíte být opatrní a podle pokynů k nápravě navrhovaných v posouzení opravit problémy s připraveností.
...
1. Pro virtuální počítače VMware shromažďuje Azure Migrate zařízení ukázkový bod v reálném čase v intervalu 20 sekund. V případě virtuálních počítačů Hyper-V je ukázkový bod v reálném čase shromážděn v intervalu 30 sekund. U fyzických serverů se ukázkový bod v reálném čase shromažďuje každých 5 minut. 
...
4. Tato hodnota je vynásobena faktorem komfortu, aby získala efektivní data o využití výkonu pro jednotlivé metriky (využití procesoru, využití paměti, IOPS disku (čtení a zápis), propustnost disku (čtení a zápis) a propustnost sítě (v/v). zařízení shromažďuje.

Po určení efektivní hodnoty využití se velikost úložiště, sítě a výpočetního prostředí zpracuje následujícím způsobem.

> [!NOTE]
> V případě serverů přidaných prostřednictvím importu se údaje o výkonu poskytované uživatelem používají přímo pro správnou velikost.
...
**Změna velikosti sítě**: vyhodnocování serveru se pokouší najít virtuální počítač Azure, který může podporovat počet síťových adaptérů připojených k místnímu počítači a výkon vyžadovaný těmito síťovými adaptéry.
...
> [!NOTE]
> Pro importované servery se aktuálně nepodporuje zadání počtu síťových adaptérů.

**Výpočet velikosti**: po výpočtu požadavků na úložiště a síť vyhodnotí požadavky na procesor a paměť, aby se v Azure vyhledala vhodná velikost virtuálního počítače.
