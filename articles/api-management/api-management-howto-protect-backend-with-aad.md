...
4. Nakonfigurujte konzolu pro vývojáře, aby používala autorizaci uživatelů OAuth 2,0.
...
1. Pro nyní nechejte oddíl **URI pro přesměrování** prázdné.
...
Každá klientská aplikace, která volá rozhraní API, musí být registrovaná taky jako aplikace v Azure AD. V tomto příkladu je ukázková klientská aplikace konzole pro vývojáře na portálu API Management Developer Portal. Tady je postup, jak zaregistrovat jinou aplikaci ve službě Azure AD, která bude reprezentovat konzolu pro vývojáře.
...
2. V části **Přidat tajný klíč klienta**zadejte **Popis**. Zvolte, kdy má klíč vypršet, a vyberte **Přidat**.
...
2. Vyberte `client-app`a v seznamu stránek pro aplikaci přejdete na **oprávnění rozhraní API**.

3. Vyberte **Přidat oprávnění**.

4. V části **Vybrat rozhraní API**vyhledejte a vyberte `backend-app`.

5. V části **delegovaná oprávnění**vyberte požadovaná oprávnění pro `backend-app`.

6. Vyberte **Přidat oprávnění** . 

> [!NOTE]
> Pokud **Azure Active Directory** není uveden v části oprávnění k jiným aplikacím, vyberte **Přidat** a přidejte je ze seznamu.
...
4. V poli **Adresa URL registrační stránky klienta**zadejte hodnotu zástupného symbolu, například `http://localhost`. **Adresa URL stránky pro registraci klienta** odkazuje na stránku, kterou můžou uživatelé použít k vytvoření a konfiguraci vlastních účtů pro poskytovatele OAuth 2,0, kteří to podporují. V tomto příkladu uživatelé nevytvářejí a nekonfigurují vlastní účty, takže místo toho použijete zástupný symbol.

5. V případě **autorizačních typů udělte**vyberte **autorizační kód**.

6. Zadejte **adresu URL koncového bodu autorizace** a **adresu URL koncového bodu tokenu**. Tyto hodnoty načtěte ze stránky **koncových bodů** v TENANTOVI Azure AD. Přejděte na stránku **Registrace aplikací** znovu a vyberte **koncové body**.

7. Zkopírujte **koncový bod autorizace OAuth 2,0**a vložte ho do textového pole **Adresa URL koncového bodu autorizace** .

8. Zkopírujte **koncový bod tokenu OAuth 2,0**a vložte ho do textového pole **Adresa URL koncového bodu tokenu** . Kromě vložení do koncového bodu tokenu přidejte parametr body s názvem **Resource**. Pro hodnotu tohoto parametru použijte **ID aplikace** pro back-endové aplikace.

9. Dále zadejte pověření klienta. Toto jsou přihlašovací údaje klienta aplikace.

10. Pro **ID klienta**použijte **ID aplikace** pro klienta aplikace.

11. Pro **tajný klíč klienta**použijte klíč, který jste vytvořili dříve pro klienta aplikace. 

12. Hned za tajný klíč klienta je **redirect_url** pro typ udělení autorizačního kódu. Tuto adresu URL si poznamenejte.

13. Vyberte **Vytvořit**.

14. Vraťte se na stránku **Nastavení** klienta aplikace.

15. Vyberte **adresy URL odpovědi**a vložte **redirect_url** do prvního řádku. V tomto příkladu jste nahradili `https://localhost` adresu URL v prvním řádku.  
...
Když je teď v konzole povolená `Echo API`autorizace uživatelů OAuth 2,0, Konzola pro vývojáře získá přístupový token jménem uživatele před voláním rozhraní API.

1. Přejděte na libovolnou operaci `Echo API` na portálu pro vývojáře a vyberte **vyzkoušet**. Tím se vám zobrazí Konzola pro vývojáře.
...
V tomto okamžiku, když se uživatel pokusí uskutečnit volání z konzole pro vývojáře, se uživateli zobrazí výzva k přihlášení. Konzola pro vývojáře získá přístupový token jménem uživatele.

Ale co když někdo volá vaše rozhraní API bez tokenu nebo s neplatným tokenem? Rozhraní API můžete například pořád volat i v případě, že `Authorization` hlavičku odstraníte. Důvodem je, že API Management v tomto okamžiku neověřuje přístupový token. Jednoduše předá `Authorization` hlavičku back-endové rozhraní API.

Pomocí zásady [ověřit token JWT](api-management-access-restriction-policies.md#ValidateJWT) můžete předem autorizovat žádosti v API Management tím, že ověříte přístupové tokeny každého příchozího požadavku. Pokud požadavek nemá platný token, API Management ho zablokuje. Můžete například přidat následující zásadu do `<inbound>` části zásady v tématu. `Echo API` Kontroluje deklaraci identity cílové skupiny v přístupovém tokenu a pokud token není platný, vrátí chybovou zprávu. Informace o tom, jak nakonfigurovat zásady, najdete v tématu [nastavení nebo úprava zásad](set-edit-policies.md).
