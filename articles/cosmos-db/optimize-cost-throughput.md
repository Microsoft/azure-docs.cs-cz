...
1. Jak bylo zmíněno dříve, Azure Cosmos DB vám umožní kombinovat a porovnat výše uvedené dvě strategie, takže teď můžete mít v databázi Azure Cosmos nějaké kontejnery, které mohou sdílet propustnost zajištěnou v databázi, a také některé kontejnery ve stejné databázi. , což může mít vyhrazené množství zřízené propustnosti. 
...
[](https://docs.microsoft.com/dotnet/api/microsoft.azure.documents.client.retryoptions.maxretryattemptsonthrottledrequests?view=azure-dotnet)MaxRetryAttemptsOnThrottledRequests je nastavené na hodnotu 3, takže v tomto případě platí, že pokud je operace požadavku omezená na překročení rezervované propustnosti pro kolekci, operace požadavku se třikrát pokusí vyvoláním výjimky pro použití.  [MaxRetryWaitTimeInSeconds](https://docs.microsoft.com/dotnet/api/microsoft.azure.documents.client.retryoptions.maxretrywaittimeinseconds?view=azure-dotnet#Microsoft_Azure_Documents_Client_RetryOptions_MaxRetryWaitTimeInSeconds)  je nastavená na 60, takže pokud se v tomto případě kumulativní opakování počká v sekundách, protože první požadavek přesáhne 60 sekund, vyvolá se výjimka.
...
## <a name="partitioning-strategy-and-provisioned-throughput-costs"></a>Strategie vytváření oddílů a náklady na propustnost
...
2. Jednou z metod pro odhad množství rezervované propustnosti, kterou vaše aplikace vyžaduje, je zaznamenání poplatků za RU jednotek požadavků spojených se spouštěním typických operací s reprezentativním kontejnerem Azure Cosmos nebo databází, kterou používá vaše aplikace. pak odhadujte počet operací, které předpokládáte, aby se provedla Každá sekunda. Nezapomeňte měřit a zahrnovat i typické dotazy a jejich využití. Informace o tom, jak odhadnout náklady na dotazy pomocí kódu programu nebo pomocí portálu, najdete v tématu [optimalizace nákladů na dotazy](online-backup-and-restore.md). 
