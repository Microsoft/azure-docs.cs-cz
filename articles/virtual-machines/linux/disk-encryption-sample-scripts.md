...
|Parametr|Popis|Mandatory?|
|------|------|------|
|$resourceGroupName| Name of the resource group to which the KeyVault belongs to.  A new resource group with this name will be created if one doesn't exist.| Pravda|
|$keyVaultName|Name of the KeyVault in which encryption keys are to be placed. A new vault with this name will be created if one doesn't exist.| Pravda|
|$location|Location of the KeyVault. Make sure the KeyVault and VMs to be encrypted are in the same location. Seznam umístění získáte pomocí rutiny `Get-AzLocation`.|Pravda|
|$subscriptionId|Identifier of the Azure subscription to be used.  Své ID předplatného můžete získat pomocí rutiny `Get-AzSubscription`.|Pravda|
|$aadAppName|Name of the Azure AD application that will be used to write secrets to KeyVault. Pokud aplikace se zadaným názvem neexistuje, vytvoří se nová. If this app already exists, pass aadClientSecret parameter to the script.|Nepravda|
|$aadClientSecret|Client secret of the Azure AD application that was created earlier.|Nepravda|
|$keyEncryptionKeyName|Name of optional key encryption key in KeyVault. A new key with this name will be created if one doesn't exist.|Nepravda|
...
2. Change the crypt config in */etc/crypttab*. Mělo by to vypadat takto:
   ```
