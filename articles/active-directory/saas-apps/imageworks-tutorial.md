...
3. **[Vytvoření zkušebního uživatele OBRAZE](#create-a-image-works-test-user)**  – Pokud chcete mít protějšek Britta Simon v OBRAZE, který je propojený s Azure AD reprezentace uživatele.
...
3. Na **IMAGE FUNGUJE domény a adresy URL** části, proveďte následující kroky:
...
6. Na **konfigurace IMAGE FUNGUJE** klikněte na tlačítko **OBRAZE umožňuje nakonfigurovat** otevřete **nakonfigurovat přihlašování** okna. Kopírovat **odhlašování adresy URL, SAML Entity ID a SAML jednotné přihlašování služby** z **Stručná referenční příručka oddílu.**

    ![OBRÁZEK konfigurace FUNGUJE](./media/imageworks-tutorial/tutorial_imageworks_configure.png) 

7. Ke konfiguraci jednotného přihlašování na **OBRAZE** straně, je nutné odeslat na stažený **Certificate(Base64), adresa URL odhlašování, SAML Entity ID a SAML jednotné přihlašování – adresa URL služby** k [IMAGE FUNGUJE tým podpory](mailto:iw-sd-support@fujifilm.com). Nastavují tohoto nastavení můžete mít správně nastavené na obou stranách připojení SAML SSO.
