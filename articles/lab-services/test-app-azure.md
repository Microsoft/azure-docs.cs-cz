...
1. Na místním počítači použijte skript, který [uchovává přihlašovací údaje sdílené složky Azure v systému Windows v](../storage/files/storage-how-to-use-files-windows.md#persisting-azure-file-share-credentials-in-windows) části [použití sdílené složky Azure s Windows](../storage/files/storage-how-to-use-files-windows.md) . 
...
4. V rámci virtuálního počítače spusťte **Průzkumníka souborů**a vyberte **Tento počítač** , abyste našli svoji sdílenou složku.
    ...
    > [!NOTE]
    > Pokud nemůžete najít sdílenou složku na virtuálním počítači nebo na místním počítači, můžete ji z nějakého důvodu znovu připojit spuštěním `net use` příkazu. `net use` Příkaz najdete v průvodci **připojením** **sdílené složky** v Azure Portal.
