---
title: Migrace uživatelů se sociálními identitami
titleSuffix: Azure AD B2C
description: Přečtěte si základní koncepty týkající se migrace uživatelů se sociálními identitami do Azure AD B2C pomocí Graph API.
...
ms.locfileid: "74950116"
---
# <a name="azure-active-directory-b2c-migrate-users-with-social-identities"></a>Azure Active Directory B2C: Migrace uživatelů pomocí sociálních identit
Když plánujete migrovat poskytovatele identity na Azure AD B2C, budete možná muset taky migrovat uživatele se sociálními identitami. Tento článek vysvětluje, jak migrovat existující účty sociálních identit, jako jsou například účty Facebook, LinkedIn, Microsoft a Google, které vám Azure AD B2C. Tento článek platí také pro federované identity, ale tyto migrace jsou méně časté. Ve zbývající části tohoto článku Vezměte v úvahu cokoli, co platí pro účty sociálních sítí, které platí i pro jiné typy federovaných účtů.
...
* V Azure AD B2C se přihlašovací jména **místních účtů** (uživatelské jméno nebo e-mailová adresa) ukládají do kolekce `signInNames` v záznamu uživatele. `signInNames` obsahuje jeden nebo více `signInName` záznamů, které určují přihlašovací jména pro uživatele. Každé přihlašovací jméno musí být v rámci tenanta jedinečné.

* Identity **sociálních účtů** jsou uložené v kolekci `userIdentities`. Položka určuje `issuer` (název zprostředkovatele identity), jako je například facebook.com a `issuerUserId`, což je jedinečný identifikátor uživatele pro vystavitele. Atribut `userIdentities` obsahuje minimálně jeden záznam UserIdentity, který určuje typ účtu sociální sítě a jedinečný identifikátor uživatele od poskytovatele sociální identity.

* **Zkombinujte místní účet se sociální identitou**. Jak už jsme uvedli, přihlašovací jména místních účtů a identity sociálních účtů se ukládají do různých atributů. `signInNames` se používá pro místní účet, zatímco `userIdentities` se používá pro účty sociálních sítí. Jedním Azure AD B2C účtem může být jenom místní účet, jenom sociální účet, nebo kombinovat místní účet s jednou nebo více sociálními identitami v jednom záznamu uživatele. Toto chování umožňuje spravovat jediný účet, zatímco se uživatel může přihlásit pomocí přihlašovacích údajů místního účtu nebo pomocí sociálních identit.

* `UserIdentity` typ – obsahuje informace o identitě uživatele sociálního účtu v klientovi Azure AD B2C:
  * `issuer` řetězcové vyjádření zprostředkovatele identity, který vystavil identifikátor uživatele, například facebook.com.
  * `issuerUserId` jedinečného identifikátoru uživatele používaného poskytovatelem sociální identity ve formátu kódovaném v kódování Base64.
...
Uživatelský účet Azure AD B2C vytvoříte pomocí [Graph API](https://docs.microsoft.com/azure/active-directory-b2c/active-directory-b2c-devquickstarts-graph-dotnet).
Abyste mohli komunikovat s Graph API, musíte mít účet služby s oprávněními správce. Ve službě Azure AD zaregistrujete aplikaci a ověřování do služby Azure AD. Přihlašovací údaje aplikace jsou ID aplikace a tajný kód aplikace. Aplikace funguje samostatně, nikoli jako uživatel, pro volání Graph API. Postupujte podle pokynů v kroku 1 v článku [migrace uživatelů](https://docs.microsoft.com/azure/active-directory-b2c/active-directory-b2c-user-migration) .
...
* **userPrincipalName** – hlavní název uživatele (someuser@contoso.com). Hlavní název uživatele musí obsahovat jednu z ověřených domén pro tenanta. Pokud chcete zadat hlavní název uživatele (UPN), vygenerujte novou hodnotu GUID zřetězeně s `@` a vaším názvem tenanta.
...
Další informace najdete v tématu: [Graph API Reference](/previous-versions/azure/ad/graph/api/users-operations#CreateLocalAccountUser) .
...
Název vystavitele nebo název zprostředkovatele identity je nakonfigurovaný v zásadách. Pokud neznáte hodnotu, která se má zadat v `issuer`, postupujte podle následujících pokynů:
...
6. Vyhledá prvek `userIdentities` uvnitř návratového Azure AD B2C JSON.
7. Volitelné Je také možné, že budete chtít dekódovat `issuerUserId` hodnotu.
...
https://graph.windows.net/tenant-name.onmicrosoft.com/users/userObjectId **opravy**
...
[Stáhněte a spusťte ukázkovou aplikaci v2](https://github.com/Azure-Samples/active-directory-b2c-custom-policy-starterpack/tree/master/scenarios/aadb2c-user-migration). Ukázková aplikace V2 používá soubor JSON, který obsahuje fiktivní uživatelská data, jako jsou místní účet, účet sociální sítě a místní & sociální identity v rámci jednoho účtu.  Chcete-li upravit soubor JSON, otevřete `AADB2C.UserMigration.sln` řešení sady Visual Studio. V projektu `AADB2C.UserMigration` otevřete `UsersData.json` soubor. Soubor obsahuje seznam entit uživatele. Každá entita uživatele má následující vlastnosti:
