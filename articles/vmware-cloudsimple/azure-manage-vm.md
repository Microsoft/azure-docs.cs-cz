---
title: Řešení Azure VMware (AVS) – Správa virtuálních počítačů s privátním cloudem služby AVS v Azure
description: Popisuje, jak spravovat virtuální počítače s privátním cloudem funkce AVS v Azure Portal, včetně přidávání disků, změny kapacity virtuálních počítačů a přidávání síťových rozhraní.
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/16/2019
ms.topic: article
...
---
# <a name="manage-your-avs-private-cloud-virtual-machines-in-azure"></a>Správa virtuálních počítačů privátního cloudu služby AVS v Azure

Pro správu virtuálních počítačů, které jste [vytvořili pro privátní cloud služby AVS](azure-create-vm.md), se přihlásíte k [Azure Portal](https://portal.azure.com). Vyhledejte a vyberte virtuální (vyhledávání v části **všechny služby** nebo **Virtual Machines** v postranní nabídce).
...
| Řízení | Popis |
| ------------ | ------------- |
| Propojení | Připojte se k zadanému virtuálnímu počítači.  |
| Začátek | Spusťte zadaný virtuální počítač.  |
| Restartování | Vypněte a pak zapněte zadaný virtuální počítač.  |
| Zastavit | Vypněte konkrétní virtuální počítač.  |
| Capture | Zachyťte image zadaného virtuálního počítače, aby ji bylo možné použít jako image k vytvoření dalších virtuálních počítačů. Viz [Vytvoření spravované image zobecněného virtuálního počítače v Azure](../virtual-machines/windows/classic/capture-image.md).   |
| Přesunout | Přejděte na zadaný virtuální počítač.  |
| Odstranit | Odeberte zadaný virtuální počítač.  |
| Obnovit | Aktualizuje data v zobrazení.  |
...
| Network (Síť) | Provoz v síti a ze sítě (MB) v průběhu vybraného intervalu.  |
...
Chcete-li přidat rozhraní, klikněte na tlačítko **Přidat síťové rozhraní**. Proveďte konfiguraci každého z následujících nastavení tak, že zadáte nebo vyberete vloženou možnost. Klikněte na **Uložit**.
   ...
   | Network (Síť) | Vyberte ze seznamu konfigurovaných sítí v privátním cloudu služby AVS vSphere.  |
