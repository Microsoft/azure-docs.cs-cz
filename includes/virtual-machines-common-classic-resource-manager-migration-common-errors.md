...
| Migrace se pro nasazení {název_nasazení} v hostované službě {název_hostované_služby} nepovoluje, protože není v místním oboru. Naleznete na https:\//aka.ms/regionalscope pro přesunutí tohoto nasazení do místního oboru. | V roce 2014 byl pro Azure oznámen přesun síťových prostředků z oboru na úrovni serveru do místního oboru. Další [https://aka.ms/regionalscope](https://aka.ms/regionalscope) podrobnosti naleznete v části . K této chybě dochází v případě, že u migrovaného nasazení neproběhla operace aktualizace, která by ho automaticky přesunula do místního oboru. Nejlepším alternativním řešením je přidat do virtuálního počítače koncový bod nebo datový disk a potom opakovat migraci. <br> Viz [Postup nastavení koncových bodů na klasickém virtuálním počítači s Windows v Azure](/previous-versions/azure/virtual-machines/windows/classic/setup-endpoints#create-an-endpoint) nebo [Připojení datového disku k virtuálnímu počítači s Windows vytvořenému pomocí modelu nasazení Classic](../articles/virtual-machines/windows/classic/attach-disk.md).|
| Migrace není podporována pro virtuální síť {vnet-name}, protože má nasazení PaaS bez brány. | K této chybě dochází, pokud máte nasazení PaaS bez brány, jako je například aplikační brána nebo služby správy rozhraní API, které jsou připojeny k virtuální síti.|
