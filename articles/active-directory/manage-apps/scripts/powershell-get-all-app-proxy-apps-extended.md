...
description: Příklad PowerShellu, který obsahuje všechny aplikace proxy aplikací pro Azure Active Directory (Azure AD) spolu s ID aplikace (AppId), názvem (DisplayName), externí adresou URL (ExternalUrl), interní adresou URL (InternalUrl) a typem ověřování ( ExternalAuthenticationType).
...
Tento ukázkový skript PowerShellu obsahuje informace o všech aplikacích proxy aplikací pro Azure Active Directory (Azure AD), včetně ID aplikace (AppId), názvu (DisplayName), externí adresy URL (ExternalUrl), interní URL (InternalUrl) a typ ověřování (ExternalAuthenticationType).
