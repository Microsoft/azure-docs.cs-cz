...
Při vytvoření adresáře Azure AD B2C se v novém adresáři automaticky vytvoří `b2c-extensions-app. Do not modify. Used by AADB2C for storing user data.` aplikace s názvem. Tato aplikace, která se označuje jako **B2C-Extensions-App**, je v *Registrace aplikací*viditelná. Služba Azure AD B2C ji používá k ukládání informací o uživatelích a vlastních atributech. Pokud se aplikace odstraní, Azure AD B2C nebude správně fungovat a bude to mít vliv na vaše produkční prostředí.
...
1. Vydejte požadavek HTTP GET na adresu `https://graph.windows.net/myorganization/deletedApplications` URL s rozhraním API-Version = 1.6. Tato operace zobrazí seznam všech aplikací, které byly odstraněny během posledních 30 dnů.
1. Vyhledejte aplikaci v seznamu, kde název začíná na ' B2C-Extension-App ' a zkopírujte jeho `objectid` hodnotu vlastnosti.
1. Vydejte požadavek HTTP POST na adresu `https://graph.windows.net/myorganization/deletedApplications/{OBJECTID}/restore`URL. Nahraďte `{OBJECTID}` část adresy URL `objectid` z předchozího kroku.
