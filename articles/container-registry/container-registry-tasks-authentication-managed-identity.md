...
Pokud chcete povolit identitu přiřazenou systémem, předejte `--assign-identity` bez hodnoty nebo `assign-identity [system]`. Následující příklad příkazu vytvoří úlohu pro Linux z veřejného úložiště GitHub, které vytvoří bitovou kopii `hello-world` a povolí spravovanou identitu přiřazenou systémem:
...
Pokud chcete povolit uživatelsky přiřazenou identitu, předejte `--assign-identity` s hodnotou *ID prostředku* identity. Následující příklad příkazu vytvoří úlohu pro Linux z veřejného úložiště GitHub, které vytvoří bitovou kopii `hello-world` a umožní uživateli přiřazenou spravovanou identitu:
...
Chcete-li přidat pověření pro uživatelem přiřazenou identitu k ověření pomocí registru *targetregistry*, předejte `use-identity` s hodnotou *ID klienta* identity. Například:
