...
Funkce offline mobilní aplikace v Azure umožňují interakci s místní databází, když jste v offline scénáři. Chcete-li použít tyto funkce v aplikaci, inicializujte rozhraní [SyncContext][synccontext] to a local store. Then reference your table through the [IMobileServiceSyncTable][IMobileServiceSyncTable]. SQLite se používá jako místní úložiště na zařízení.
...
3. V Průzkumník řešení klikněte pravým tlačítkem na **odkazy** > **Přidat odkaz...** > univerzální **rozšíření** **Windows** > a pak povolit obě **SQLite pro Univerzální platforma Windows** i **Visual C++ 2015. Modul runtime pro aplikace Univerzální platforma Windows**.
...
Při prvním spuštění aplikace volá obslužná rutina události `OnNavigatedTo` `InitLocalStoreAsync`. Tato metoda zase volá `SyncAsync` pro synchronizaci místního úložiště s back-end databází. Aplikace se při spuštění pokusí o synchronizaci.
...
4. V aplikaci klikněte na zaškrtávací políčko vedle několika položek, abyste je mohli doplňovat v místním úložišti.
   ...
   `UpdateCheckedTodoItem` volá `SyncAsync` pro synchronizaci každé dokončené položky s back-endu mobilní aplikace. `SyncAsync` vyvolá volání push a Pull. Kdykoli však provedete přijetí **změn u tabulky, u které klient provedl změny, je nabízení vždy provedeno automaticky**. Tím zajistíte, že všechny tabulky v místním úložišti spolu s relacemi zůstanou konzistentní. Výsledkem tohoto chování může být neočekávané vložení.  Další informace o tomto chování najdete v tématu [Synchronizace offline dat v prostředí Azure Mobile Apps].
