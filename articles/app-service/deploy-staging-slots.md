...
Po vytvoření slotu nasazení není žádný obsah. Můžete nasadit na pozici v jiném úložišti větev nebo úplně jiném úložišti. Můžete také změnit konfiguraci slotu. Použijte Publikovat profil nebo nasazení přihlašovací údaje související s slot nasazení pro aktualizace obsahu.  Například můžete [publikovat tento slot s gitem](deploy-local-git.md).
...
Operace prohození někdy trvá nějakou dobu, například pokud má aplikace, kterou Prohodí zahřívání dlouhou dobu. Můžete získat další informace o operacích odkládacího souboru v [protokolu aktivit](../azure-monitor/platform/activity-logs-overview.md) v [webu Azure portal](https://portal.azure.com).
