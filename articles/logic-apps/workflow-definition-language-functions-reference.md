...
| <*jazyka*> | No | Řetězec | Národní prostředí, které se má použít `number.ToString(<format>, <locale>)`jako podporované. Pokud není zadaný, použije se výchozí hodnota `en-us`. |
|||||
