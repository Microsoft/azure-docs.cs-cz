---
author: erhopf
ms.service: cognitive-services
ms.topic: include
ms.date: 08/06/2019
ms.author: erhopf
ms.openlocfilehash: 40ef0840253411915d89677e63b2c7e73e27d978
ms.sourcegitcommit: 9ee0cbaf3a67f9c7442b79f5ae2e97a4dfc8227b
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 03/27/2020
ms.locfileid: "69906650"
---
## <a name="prerequisites"></a>Požadavky

K tomuto rychlému startu potřebujete:

* C# 7.1 nebo novější
* [Sada SDK rozhraní .NET](https://www.microsoft.com/net/learn/dotnet/hello-world-tutorial)
* [Json.NET balíček NuGet](https://www.nuget.org/packages/Newtonsoft.Json/)
* [Visual Studio](https://visualstudio.microsoft.com/downloads/), [Visual Studio Code](https://code.visualstudio.com/download)nebo váš oblíbený textový editor
* Předplatné Azure – [vytvořte si ho zdarma](https://azure.microsoft.com/free/)
