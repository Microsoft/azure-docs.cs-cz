...
* `$Location` identifikuje datové centrum, ve kterém zřizujeme obor názvů.
* `$CurrentNamespace` ukládá obor názvů odkazu, který načteme (nebo vytvoříte).

V samotném skriptu můžete `$Namespace` a `$Location` předat jako parametry.
