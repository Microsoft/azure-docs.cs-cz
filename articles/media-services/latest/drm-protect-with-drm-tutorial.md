...
Azure Media Services můžete použít k doručování datových proudů šifrovaných pomocí licencí Microsoft PlayReady, Google Widevine nebo Apple FairPlay. Podrobné vysvětlení najdete v tématu [Ochrana obsahu s dynamickým šifrováním](drm-content-protection-concept.md).
...
![AMS s videem chráněným technologií DRM v Azure Media Player](./media/drm-protect-with-drm-tutorial/ams_player.png)
...
* Přečíst si článek [Přehled ochrany obsahu](drm-content-protection-concept.md)
* Projděte si téma [návrh ochrany obsahu s více technologiemi DRM pomocí řízení přístupu](architecture-design-multi-drm-system.md).
...
Jak je uvedeno výše, objekt **Transformace** je předpis a [Úloha](transform-jobs-concept.md) je vlastní požadavek na službu Media Services, aby **transformaci** použila na daný vstupní videoobsah nebo zvukový obsah. **Úloha** určuje informace, jako je umístění vstupního videa a umístění výstupu.
...
* Konfigurace
    ...
    Licence [PlayReady](drm-playready-license-template-concept.md) a [Widevine](drm-widevine-license-template-concept.md) jsou nakonfigurované tak, že se dají doručit pomocí služby doručení licencí Media Services. I když Tato ukázková aplikace nekonfiguruje licenci [Fairplay](drm-fairplay-license-overview.md) , obsahuje metodu, kterou můžete použít ke konfiguraci Fairplay. Konfiguraci FairPlay můžete přidat jako jinou možnost.
...
1. Vytvořte [Lokátor streamování](stream-streaming-locators-concept.md).
...
> [!IMPORTANT]
> Pokud používáte vlastní [zásady streamování](stream-streaming-policy-concept.md), měli byste navrhnout určitou sadu takových zásad pro svůj účet Media Service a znovu je použít pro své StreamingLocators, kdykoli budete potřebovat stejné možnosti šifrování a protokoly. Počet záznamů StreamingPolicy je pro účty služby Media Service omezený kvótou. Neměli byste vytvářet novou StreamingPolicy pro každou StreamingLocator.
...
![Ochrana pomocí DRM](./media/drm-protect-with-drm-tutorial/playready_encrypted_url.png)
...
> [Ochrana pomocí AES-128](drm-playready-license-template-concept.md)
