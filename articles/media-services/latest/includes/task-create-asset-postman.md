---
author: IngridAtMicrosoft
ms.service: media-services
ms.topic: include
ms.date: 08/18/2020
ms.author: inhenkel
ms.custom: dotnet
ms.openlocfilehash: 3bcf10cc5605d00d89e1723f6c0e05e5bcce2cf0
ms.sourcegitcommit: 829d951d5c90442a38012daaf77e86046018e5b9
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 10/09/2020
ms.locfileid: "88653871"
---
<!--Create a media services asset with Postman-->

V této části se dozvíte, jak vytvořit nový prostředek pomocí metody post.

1. Vyberte **Assets**  ->  **assety vytvořit nebo aktualizovat Asset**.
2. Stiskněte **Odeslat**. Zobrazí se **odpověď** s informacemi o nově vytvořeném prostředku.

<!-- improved graphics needed here.  They have been omitted until this can be a work item in the backlog -->
