...
ms.locfileid: "68610573"
---
# <a name="quickstart-encrypt-a-windows-iaas-vm-with-azure-powershell"></a>Rychlý start: Šifrování virtuálního počítače s Windows IaaS pomocí Azure PowerShell
...
- Instalace nebo aktualizace na [nejnovější verzi modulu Azure PowerShell](/powershell/azure/install-az-ps)
    - Verze modulu AZ Module musí být 1.0.0 nebo vyšší. Použijte `Get-Module Az -ListAvailable | Select-Object -Property Name,Version,Path` pro kontrolu verze.
...
1. **Správce: Integrované skriptovací prostředí (ISE) v prostředí Windows PowerShell** okně klikněte na **Zobrazit** a pak klikněte na **Zobrazit podokno skriptu**.
...
1. **Správce: Integrované skriptovací prostředí (ISE) v prostředí Windows PowerShell** okně klikněte na **soubor** a potom na **otevřít**. Přejděte k souboru **ADEPrereqScript.ps1** a dvakrát na něj klikněte. Skript se otevře v podokně skriptu.
...
![Skript pro požadované součásti služby Azure Disk Encryption spuštěný v Integrovaném skriptovacím prostředí (ISE) v prostředí PowerShell](./media/quick-encrypt-vm-powershell/ade-prereq-script.PNG)
...
1. Zkopírujte skript do **správce: Integrované skriptovací prostředí (ISE) v prostředí Windows PowerShell** podokno skriptu a změnit horní tři proměnné. Skupina prostředků a umístění se musí shodovat s těmi, které jste použili pro [skript pro požadované součásti](#bkmk_PrereqScript).  
...
Teď, když jste vytvořili a nakonfigurovali Trezor klíčů a virtuální počítač, můžete disk zašifrovat pomocí rutiny **set-AzVmDiskEncryptionExtension** . 
