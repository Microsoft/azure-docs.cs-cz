...
V tomto článku se dozvíte, jak odebrat privátní koncové body později, ale dál používat prostředek pro rozpoznávání řeči.
...
> [!WARNING]
> Prostředek pro rozpoznávání řeči, který používá vlastní název domény, spolupracuje se službou Speech Services jiným způsobem.
> Je možné, že budete muset upravit kód aplikace tak, aby používal prostředek řeči s privátním koncovým bodem, a také použít prostředek pro rozpoznávání řeči _bez_ privátního koncového bodu.
> Oba scénáře mohou být potřeba, protože přepnutí na vlastní název domény není _vratné._
>
> Když zapnete vlastní název domény, operace se [nevratí](../cognitive-services-custom-subdomains.md#can-i-change-a-custom-domain-name). Jediným způsobem, jak přejít zpět k [regionálnímu názvu](../cognitive-services-custom-subdomains.md#is-there-a-list-of-regional-endpoints) , je vytvoření nového prostředku řeči.
>
...
Pokud chcete pro vybraný prostředek řeči zapnout vlastní název domény, použijte rutinu [set-AzCognitiveServicesAccount](/powershell/module/az.cognitiveservices/set-azcognitiveservicesaccount) .
...
```
## <a name="turn-on-a-custom-domain-name"></a>Zapnutí vlastního názvu domény

Pokud chcete použít vlastní název domény s vybraným prostředkem řeči, použijte příkaz [AZ cognitiveservices Account Account Update](/cli/azure/cognitiveservices/account#az_cognitiveservices_account_update) .
...
## <a name="turn-on-private-endpoints"></a>Zapnutí privátních koncových bodů

Doporučujeme používat [privátní ZÓNU DNS](../../dns/private-dns-overview.md) připojenou k virtuální síti s nezbytnými aktualizacemi privátních koncových bodů. Během procesu zřizování můžete vytvořit privátní zónu DNS. Pokud používáte vlastní server DNS, možná budete muset změnit také konfiguraci DNS.
...
K vytvoření soukromých koncových bodů použijte jeden z následujících článků.
Tyto články používají jako ukázkový prostředek webovou aplikaci, která umožňuje jejich zpřístupnění prostřednictvím soukromých koncových bodů.
...
Tuto kontrolu proveďte pouze v případě, že jste v části **síť** svého prostředku zapnuli možnost **všechny sítě** nebo **vybrané sítě a soukromé koncové body** . 
...
## <a name="adjust-an-application-to-use-a-speech-resource-with-a-private-endpoint"></a>Úprava aplikace tak, aby používala prostředek řeči s privátním koncovým bodem

Prostředek řeči s vlastní doménou spolupracuje se službou Speech Services jiným způsobem. To platí pro prostředek řeči s povoleným vlastním doménou s i bez privátních koncových bodů. Informace v této části se týkají obou scénářů.

Podle pokynů v této části upravte stávající aplikace a řešení tak, aby používala prostředek řeči s vlastním názvem domény a soukromým koncovým bodem zapnutým.

Prostředek řeči s vlastním názvem domény a soukromým koncovým bodem je zapnutý, používá jiný způsob, jak pracovat se službami Speech. V této části se dozvíte, jak použít takový prostředek s rozhraními REST API služby Speech Services a [sadou Speech SDK](speech-sdk.md).

> [!NOTE]
> Prostředek řeči bez privátních koncových bodů, který používá vlastní název domény, má také speciální způsob, jak pracovat se službou Speech Services.
> Tímto způsobem se liší od scénáře prostředku řeči, který používá privátní koncový bod. To je důležité vzít v úvahu, protože později se můžete rozhodnout odebrat privátní koncové body.
> Další informace najdete v tématu _Úprava aplikace pro použití zdroje řeči bez privátních koncových bodů_ dále v tomto článku.
...
Po zapnutí vlastní domény pro prostředek řeči (což je nutné pro privátní koncové body) bude tento prostředek používat následující vzor názvu DNS pro základní REST API koncový bod: <p/>`{your custom name}.cognitiveservices.azure.com`
...
Po zapnutí vlastního názvu domény pro prostředek řeči obvykle nahradíte název hostitele ve všech adresách URL žádostí s novým názvem hostitele vlastní domény. Všechny ostatní části žádosti (jako cesta `/speechtotext/v3.0/transcriptions` v předchozím příkladu) zůstávají stejné.
...
>
> Pomocí autorizačního tokenu a jeho předáním do zvláštního koncového bodu přes `Authorization` hlavičku budou fungovat *jenom* v případě, že jste zapnuli možnost přístup k **všem sítím** v části **síť** vašeho prostředku řeči. V ostatních případech se `Forbidden` `BadRequest` při pokusu o získání autorizačního tokenu zobrazí buď nebo.
...
Následující ekvivalentní adresa URL používá privátní koncový bod, ve kterém je vlastní název domény prostředku řeči `my-private-link-speech.cognitiveservices.azure.com` :
...
1. Určete adresu URL koncového bodu aplikace:
   ...
   - [Zapněte protokolování aplikace](how-to-use-logging.md) a spusťte ji pro protokolování aktivity.
...
## <a name="adjust-an-application-to-use-a-speech-resource-without-private-endpoints"></a>Úprava aplikace tak, aby používala prostředek řeči bez privátních koncových bodů
