...
|  |Sdílený klíč (klíč účtu úložiště)  |Sdílený přístupový podpis (SAS)  |Azure Active Directory (Azure AD)  |Místní služba Active Directory Domain Services (preview) |Anonymní veřejný přístup pro čtení  |
|---------|---------|---------|---------|---------|---------|
...
- **Ověřování Azure Active Directory Domain Services (Azure AD DS)** pro soubory Azure. Soubory Azure podporuje autorizaci založenou na identitách přes blok zpráv serveru (SMB) prostřednictvím služby Azure AD DS. RBAC můžete použít pro jemně odstupňovanou kontrolu nad klienta přístup k prostředkům Azure Files v účtu úložiště. Další informace o ověřování souborů Azure pomocí doménových služeb najdete v [přehledu](../files/storage-files-active-directory-overview.md).

- **Místní ověřování (preview) služby Active Directory Domain Services (AD DS nebo místní služby AD DS)** pro soubory Azure. Azure Files podporuje autorizaci založenou na identitě přes SMB prostřednictvím služby AD DS. Vaše prostředí služby AD DS se dá hostovat v místních počítačích nebo ve virtuálních počítačích Azure. Přístup SMB k souborům je podporován pomocí přihlašovacích údajů služby AD DS z počítačů spojených s doménou, ať už místně nebo v Azure. Můžete použít kombinaci RBAC pro řízení přístupu na úrovni sdílené složky a seznamy DACL NTFS pro vynucení oprávnění na úrovni adresáře nebo souboru. Další informace o ověřování souborů Azure pomocí doménových služeb najdete v [přehledu](../files/storage-files-active-directory-overview.md).
