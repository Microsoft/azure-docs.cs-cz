---
title: Převod virtuálního počítače se systémem Linux v Azure z nespravovaných disků na Managed disks – Azure Managed Disks | Microsoft Docs
...
1. Vypíše seznam všech virtuálních počítačů ve skupině dostupnosti pomocí výrazu [AZ VM Availability-set list](/cli/azure/vm/availability-set). V následujícím příkladu jsou uvedené všechny virtuální počítače ve skupině dostupnosti `myAvailabilitySet` s názvem ve skupině prostředků `myResourceGroup`s názvem:
