...
* `Connect-AzAccount`: ověřuje Azure PowerShell vašeho předplatného Azure.
...
* `Use-AzHDInsightCluster`: nastaví aktuální cluster, který se má použít pro příkaz `Invoke-AzHDInsightHiveJob`.
...
2. Otevřete nový příkazový řádek **Azure PowerShell** . Změňte adresáře na umístění souboru `hivejob.ps1` a potom spusťte skript pomocí následujícího příkazu:
...
4. Jak bylo zmíněno dříve, `Invoke-Hive` lze použít ke spuštění dotazu a čekání na odpověď. Pomocí následujícího skriptu můžete zjistit, jak funguje příkaz Invoke-podregistr:
   ...
   > [!NOTE]  
   > Pro delší dotazy HiveQL můžete použít rutinu Azure PowerShell **tady-Strings** nebo soubory skriptu HiveQL. Následující fragment kódu ukazuje, jak použít rutinu `Invoke-Hive` ke spuštění souboru skriptu HiveQL. Soubor skriptu HiveQL musí být nahrán do wasbs://.
   >
...
Pokud se po dokončení úlohy nevrátí žádné informace, podívejte se na protokoly chyb. Chcete-li zobrazit informace o chybě pro tuto úlohu, přidejte následující na konec souboru `hivejob.ps1`, uložte jej a potom jej spusťte znovu.
