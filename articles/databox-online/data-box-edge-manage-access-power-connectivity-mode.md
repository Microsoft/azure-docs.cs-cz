...
Měli byste mít `User` přístup k tenantovi služby Active Directory, protože potřebujete být schopni `Read all directory objects`. Nemůžete být uživatel typu Host, protože nemají oprávnění k `Read all directory objects`. Pokud jste Host, pak operace, jako je generace aktivačního klíče, vytvoření sdílené složky na zařízení Data Box Edge, vytvoření uživatele, konfigurace hraniční výpočetní role, resetování hesla zařízení selže.
