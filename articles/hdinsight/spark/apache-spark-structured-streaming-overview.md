...
Tyto soubory JSON jsou uložené v podsložce `temps` pod kontejnerem clusteru HDInsight.
...
Dále definujte cíl pro řádky, které jsou přidány do tabulky výsledků v rámci každého intervalu triggeru. Tento příklad pouze vytvoří výstup všech řádků do tabulky v paměti `temps` můžete později dotazovat pomocí SparkSQL. Úplný režim výstupu zajistí, že všechny řádky pro všechna okna budou ve výstupu pokaždé.
...
Při spuštění dotazu ve stejném SparkSession můžete spustit dotaz SparkSQL proti tabulce `temps`, kde jsou uloženy výsledky dotazu.
