---
title: Získávání výsledků pro automatické návrhy pomocí API pro automatické návrhy v Bingu (kognitivní služby Azure) | Microsoft Docs
description: Ukazuje, jak používat automatické návrhy v Bingu.
services: cognitive-services
author: v-jaswel
manager: kamrani
ms.assetid: 2575A80C-FC74-4631-AE5D-8101CF2591D3
ms.service: cognitive-services
ms.component: bing-autosuggest
ms.topic: article
ms.date: 09/28/2017
ms.author: v-jaswel
ms.openlocfilehash: f66af61ed06f4c6a7fe36cd44aa7781b71bb18ea
ms.sourcegitcommit: 95d9a6acf29405a533db943b1688612980374272
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 06/23/2018
ms.locfileid: "35343432"
---
# <a name="build-a-web-page-autosuggest-client"></a>Vytvoření klienta automatických návrhů webové stránky

V tomto kurzu využijeme webové stránky, která umožňuje uživatelům zpracovat dotaz rozhraní API pro automatické návrhy v Bingu.

V tomto kurzu získáte informace o následujících postupech:

> [!div class="checklist"]
> - Provést jednoduchý dotaz rozhraní API pro automatické návrhy v Bingu
> - Zobrazí výsledky dotazu

## <a name="prerequisites"></a>Požadavky

Chcete-li postupovat podle kurzu, musíte pro rozhraní API pro automatické návrhy v Bingu klíč předplatného. Pokud nemáte, [zaregistrovat k bezplatné zkušební verzi](https://azure.microsoft.com/try/cognitive-services/?api=autosuggest-api).

## <a name="create-a-new-web-page"></a>Vytvořit novou webovou stránku

Otevřete textový editor. Vytvořte nový soubor s názvem, například autosuggest.html.

## <a name="html-header"></a>Záhlaví HTML

Přidat informace ze záhlaví HTML a začít části skript následujícím způsobem.

```html
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Bing Autosuggest</title>

<style type="text/css">
    html, body, div, p, h1, h2 {font-family: Verdana, "Lucida Sans", sans-serif;}

    html, body, div, p  {font-weight: normal;}
    h1, h2 {font-weight: bold;}
    sup {font-weight: normal;}

    html, body, div, p  {font-size: 12px;}
    h1 {font-size: 20px;}
    h2 {font-size: 16px;}
    h1, h2 {clear: left;}

    img#logo {float: right;
</style>

<script type="text/javascript">
```

## <a name="getsubscriptionkey-function"></a>getSubscriptionKey – funkce

Funkce getSubscriptionKey vrací klíč rozhraní API pro automatické návrhy v Bingu. Je buď načte z místního úložiště (to znamená, že soubor cookie), nebo v případě potřeby vyzve uživatele k.

Začít funkci getSubscriptionKey a název souboru cookie deklarovat následujícím způsobem.

```html
getSubscriptionKey = function() {

    var COOKIE = "bing-autosuggest-api-key";   // name used to store API key in key/value storage
```

FindCookie pomocné funkce vrátí hodnotu zadaného souboru cookie; Pokud není nalezen soubor cookie, vrátí prázdný řetězec.

```html
    function findCookie(name) {
        var cookies = document.cookie.split(";");
        for (var i = 0; i < cookies.length; i++) {
            var keyvalue = cookies[i].split("=");
            if (keyvalue[0].trim() === name) {
                return keyvalue[1];
            }
        }
        return "";
        }
```

Pomocné funkce getSubscriptionKeyCookie vyzve uživatele k hodnotu klíče rozhraní API pro automatické návrhy v Bingu a vrátí hodnotu klíče.

```html
    function getSubscriptionKeyCookie() {
        var key = findCookie(COOKIE);
        while (key.length !== 32) {
            key = prompt("Enter Bing Autosuggest API subscription key:", "").trim();
            var expiry = new Date();
            expiry.setFullYear(expiry.getFullYear() + 2);
            document.cookie = COOKIE + "=" + key.trim() + "; expires=" + expiry.toUTCString();
        }
        return key;
    }
```

Pomocné funkce getSubscriptionKeyLocalStorage se nejprve pokusí se načíst klíč rozhraní API pro automatické návrhy v Bingu vyhledáním odpovídající soubor cookie. Pokud není nalezen soubor cookie, vyzve uživatele k hodnotě klíče. Pak vrátí hodnotu klíče.

```html
    function getSubscriptionKeyLocalStorage() {
        var key = localStorage.getItem(COOKIE) || "";
        while (key.length !== 32)
            key = prompt("Enter Bing Autosuggest API subscription key:", "").trim();
        localStorage.setItem(COOKIE, key)
        return key;
    }
```

Pomocné funkce getSubscriptionKey přijímá jeden parametr, **zneplatnit**. Pokud **zneplatnit** je **true**, getSubscriptionKey odstraní soubor cookie, který obsahuje klíč rozhraní API pro automatické návrhy v Bingu. Pokud **zneplatnit** je **false**, getSubscriptionKey vrátí hodnotu klíče rozhraní API pro automatické návrhy v Bingu.

```html
    function getSubscriptionKey(invalidate) {
        if (invalidate) {
            try {
                localStorage.removeItem(COOKIE);
            } catch (e) {
                document.cookie = COOKIE + "=";
            }
        } else {
            try {
                return getSubscriptionKeyLocalStorage();
            } catch (e) {
                return getSubscriptionKeyCookie();
            }
        }
    }
```

Vrátí jako výsledek funkce vnější getSubscriptionKey getSubscriptionKey pomocné funkce. Definice funkce vnější getSubscriptionKey zavřete.

```html
    return getSubscriptionKey;

}();
```

## <a name="helper-functions"></a>Podpůrné funkce

Před pomocné funkce vrátí zadaný text předem naformátované pomocí [před](https://www.w3schools.com/tags/tag_pre.asp) značky HTML.

```html
function pre(text) {
    return "<pre>" + text.replace(/&/g, "&amp;").replace(/</g, "&lt;") + "</pre>"
}
```

Funkce renderSearchResults zobrazí zadané výsledky z Bingu pro automatické návrhy v rozhraní API služby, pomocí JSON velmi tisku.

```html
function renderSearchResults(results) {
    document.getElementById("results").innerHTML = pre(JSON.stringify(results, null, 2));
}
```

Funkce renderErrorMessage zobrazí zadanou chybovou zprávu a kód chyby.

```html
function renderErrorMessage(message, code) {
    if (code)
        document.getElementById("results").innerHTML = "<pre>Status " + code + ": " + message + "</pre>";
    else
        document.getElementById("results").innerHTML = "<pre>" + message + "</pre>";
}
```

## <a name="bingautosuggest-function"></a>bingAutosuggest – funkce

BingAutosuggest funkce je volána pokaždé, když uživatel zadá text do pole formuláře HTML.
Jak dlouho trvá dva parametry: obsah HTML formulář, pole a klíč rozhraní API pro automatické návrhy v Bingu.

```html
function bingAutosuggest(query, key) {
```

Zadejte koncový bod rozhraní API pro automatické návrhy v Bingu a deklarace objektu XMLHttpRequest, které použijete k odesílání požadavků na koncový bod.

```html
    var endpoint = "https://api.cognitive.microsoft.com/bing/v7.0/Suggestions";

    var request = new XMLHttpRequest();

    try {
        request.open("GET", endpoint + "?q=" + encodeURIComponent(query));
    }
    catch (e) {
        renderErrorMessage("Bad request");
        return false;
    }
```

Nastavte **Ocp-Apim-Subscription-Key** záhlaví na hodnotu klíče rozhraní API pro automatické návrhy v Bingu.

```html
    request.setRequestHeader("Ocp-Apim-Subscription-Key", key);
```

Zpracování odpovědi z koncového bodu. Pokud je stav 200 (OK), zobrazte výsledky; jinak zobrazte informace o chybě.

```html
    request.addEventListener("load", function() {
        if (this.status === 200) {
            renderSearchResults(JSON.parse(this.responseText));
        }
        else {
            if (this.status === 401) getSubscriptionKey(true);
            renderErrorMessage(this.statusText, this.status);
        }
    });
```

Možné chybové události z objektu XMLHttpRequest také zpracovávejte.

```html
    request.addEventListener("error", function() {
        renderErrorMessage("Network error");
    });

    request.addEventListener("abort", function() {
        renderErrorMessage("Request aborted");
    });
```

Odešlete požadavek. Zavřete funkci bingAutosuggest **skriptu** značky a **head** značky.

```html
    request.send();
    return false;
}
// --></script>

</head>
```

## <a name="html-body"></a>Text HTML

Pokud webová stránka načte, zajistěte, aby jsme klíč rozhraní API pro automatické návrhy v Bingu pro něj výzvy pro uživatele, v případě potřeby.

```html
<body onload="document.forms.bing.query.focus(); getSubscriptionKey();">
```

Zobrazení loga Bing.

```html
<img id="logo" align=base src="data:image/png;base64,">
```

Vytvoření formuláře HTML s textové pole. Zpracování **oninput** událostí a volání funkce bingAutosuggest, obsah textové pole a klíč rozhraní API pro automatické návrhy v Bingu předávání.

```html
<form name="bing" oninput="return bingAutosuggest(this.query.value, getSubscriptionKey())">
    <h2>Autosuggest</h2>
    <input type="text" name="query" size="80" placeholder="Autosuggest" autocomplete=off>
</form>
```

Přidání kódu HTML **div** značky, který používáme zobrazíte výsledky. JavaScript definovaného dříve odkazuje na tato **div** značky.

```html
<h2>Results</h2>
<div id="results">
<p>None yet.</p>

</div>

</body>
</html>
```

Uložte soubor.

## <a name="display-results"></a>Zobrazení výsledků

Otevřete webovou stránku v prohlížeči. Do příkazového řádku zadejte svůj klíč rozhraní API pro automatické návrhy v Bingu předplatného. Potom zadejte dotaz (například "vedení") v **automatických návrhů** textové pole. Jak budete zadávat, webová stránka se automaticky aktualizuje zobrazení výsledků automatických návrhů.

```json
{
  "_type": "Suggestions",
  "queryContext": {
    "originalQuery": "sail"
  },
  "suggestionGroups": [
    {
      "name": "Web",
      "searchSuggestions": [
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=vheQSvKZylM3dlX_B9bQ8-hQEsEJo8zDD2y7H1nsBjE&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailor%2bbrinkley%2bcook%26FORM%3dUSBAPI&p=DevEx,5003.1",
          "displayText": "sailor brinkley cook",
          "query": "sailor brinkley cook",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=EStLqAfxGCa44Ur3jEMXBv-Qp-lXUSFJbkBfnUdKKDg&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailor%2bbrinkley%26FORM%3dUSBAPI&p=DevEx,5004.1",
          "displayText": "sailor brinkley",
          "query": "sailor brinkley",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=gvtP9TS9NwhajSapY2Se6y1eCbP2fq_GiP2n-cxi6OY&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailrite%26FORM%3dUSBAPI&p=DevEx,5005.1",
          "displayText": "sailrite",
          "query": "sailrite",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=c0QOA_j6swCZJy9FxqOwke2KslJE7ZRmMooGClAuCpY&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailboats%2bfor%2bsale%26FORM%3dUSBAPI&p=DevEx,5006.1",
          "displayText": "sailboats for sale",
          "query": "sailboats for sale",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=mnMdREUH20SepmHQH1zlh9Hy_w7jpOlZFm3KG2R_BoA&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailing%2banarchy%26FORM%3dUSBAPI&p=DevEx,5007.1",
          "displayText": "sailing anarchy",
          "query": "sailing anarchy",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=0udadFl0gCTKCp0QmzQTXS3_y08iO8FpwsoKPHPS6kw&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailboatdata%26FORM%3dUSBAPI&p=DevEx,5008.1",
          "displayText": "sailboatdata",
          "query": "sailboatdata",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=BTS0G6AakxntIl9rmbDXtk1n6rQpsZZ99aQ7ClE7dTY&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsail%2bsand%2bpoint%26FORM%3dUSBAPI&p=DevEx,5009.1",
          "displayText": "sail sand point",
          "query": "sail sand point",
          "searchKind": "WebSearch"
        },
        {
          "url": "https://www.bing.com/cr?IG=30C49D910FAE478288D54A8DBC5D66F1&CID=122B759B00966D02199C7E9001906C30&rd=1&h=quBMwmKlGwqC5wAU0K7n416plhWcR8zQCi7r-Fw9Y0w&v=1&r=https%3a%2f%2fwww.bing.com%2fsearch%3fq%3dsailflow%26FORM%3dUSBAPI&p=DevEx,5010.1",
          "displayText": "sailflow",
          "query": "sailflow",
          "searchKind": "WebSearch"
        }
      ]
    }
  ]
}
```

## <a name="next-steps"></a>Další postup

> [!div class="nextstepaction"]
> [Požadavky na použití a zobrazení](../UseAndDisplayRequirements.md)