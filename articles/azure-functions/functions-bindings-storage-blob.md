...
Následující příklad ukazuje [ C# funkci](functions-dotnet-class-library.md) , která při přidání nebo aktualizaci objektu blob do kontejneru `samples-workitems` zapisuje protokol.
...
Kód C# skriptu, který se váže k `Stream`:
...
Kód C# skriptu, který se váže k `CloudBlockBlob`:
...
* [BlobTriggerAttribute](https://github.com/Azure/azure-webjobs-sdk/blob/master/src/Microsoft.Azure.WebJobs.Extensions.Storage/Blobs/BlobTriggerAttribute.cs)
  ...
  Vlastnost `Connection` můžete nastavit tak, aby určovala použití účtu úložiště, jak je znázorněno v následujícím příkladu:
...
|---------|---------|----------------------|
|**textový** | není k dispozici | Musí být nastavené na `blobTrigger`. Tato vlastnost se nastaví automaticky při vytvoření triggeru v Azure Portal.|
|**směr** | není k dispozici | Musí být nastavené na `in`. Tato vlastnost se nastaví automaticky při vytvoření triggeru v Azure Portal. Výjimky jsou uvedeny v části [použití](#trigger---usage) . |
...
Přístup k datům objektů BLOB pomocí `context.bindings.<name from function.json>`.
...
Můžete zadat vzor názvu objektu BLOB ve vlastnosti `path` v *Function. JSON* nebo v konstruktoru atributu `BlobTrigger`. Vzor názvu může být [filtr nebo výraz vazby](./functions-bindings-expressions-patterns.md). V následujících částech jsou uvedeny příklady.
...
Pokud je objekt BLOB pojmenovaný *Original-Blob1. txt*, hodnoty proměnných `blobname` a `blobextension` v kódu funkce jsou *původní – Blob1* a *txt*.
...
Pokud je objekt BLOB pojmenovaný *@no__t -1-soundfile. mp3*, hodnota proměnné `name` v kódu funkce je *soundfile. mp3*.
...
* Aktivovaná funkce (*název aplikace &lt;function >* . POZVYHLEDAT. *&lt;function název >* ", například:" MyFunctionApp. Functions. CopyBlob ")
* Název kontejneru
...
* FunctionId (ve formátu *&lt;function název aplikace >* . POZVYHLEDAT. *@no__t název > 3function*)
...
V souboru *Function. JSON* se k určení názvu objektu BLOB ve vlastnostech `path` používá vlastnost metadata `queueTrigger`:
...
#### <a name="http-trigger-look-up-blob-name-from-query-string"></a>Aktivační událost HTTP, vyhledat název objektu BLOB z řetězce dotazu
 ...
 Následující příklad ukazuje funkci jazyka Java, která používá anotaci `HttpTrigger` pro příjem parametru obsahujícího název souboru v kontejneru úložiště objektů BLOB. Anotace `BlobInput` pak přečte soubor a předá jeho obsah funkci jako `byte[]`.
...
#### <a name="queue-trigger-receive-blob-name-from-queue-message"></a>Aktivační událost fronty, příjem názvu objektu BLOB ze zprávy ve frontě
 ...
 Následující příklad ukazuje funkci jazyka Java, která používá anotaci `QueueTrigger` pro příjem zprávy obsahující název souboru v kontejneru úložiště objektů BLOB. Anotace `BlobInput` pak přečte soubor a předá jeho obsah funkci jako `byte[]`.
...
V [knihovně modulu runtime Functions jazyka Java](/java/api/overview/azure/functions/runtime)použijte anotaci `@BlobInput` u parametrů, jejichž hodnota by pocházela z objektu BLOB.  Tuto poznámku lze použít s nativními typy Pojo, nebo s možnou hodnotou null pomocí `Optional<T>`.
...
Konstruktor atributu přebírá cestu k objektu BLOB a parametr `FileAccess` indikující čtení nebo zápis, jak je znázorněno v následujícím příkladu:
...
Atribut `StorageAccount` můžete použít k určení účtu úložiště na úrovni třídy, metody nebo parametru. Další informace najdete v tématu [Trigger – atributy](#trigger---attributes).
...
|Function. JSON – vlastnost | Vlastnost atributu |Popis|
|---------|---------|----------------------|
|**textový** | není k dispozici | Musí být nastavené na `blob`. |
|**směr** | není k dispozici | Musí být nastavené na `in`. Výjimky jsou uvedeny v části [použití](#input---usage) . |
|**Jméno** | není k dispozici | Název proměnné, která představuje objekt BLOB v kódu funkce.|
...
|není k dispozici | **Stoupit** | Určuje, zda budete číst nebo zapisovat. |
...
Přístup k datům objektu BLOB pomocí `context.bindings.<name from function.json>`.
...
#### <a name="http-trigger-using-outputbinding-java"></a>Aktivační událost HTTP s použitím OutputBinding (Java)
 ...
 Následující příklad ukazuje funkci jazyka Java, která používá anotaci `HttpTrigger` pro příjem parametru obsahujícího název souboru v kontejneru úložiště objektů BLOB. Anotace `BlobInput` pak přečte soubor a předá jeho obsah funkci jako `byte[]`. Poznámka `BlobOutput` se váže k `OutputBinding outputItem`, kterou funkce používá k zápisu obsahu vstupního objektu blob do konfigurovaného kontejneru úložiště.
...
#### <a name="queue-trigger-using-function-return-value-java"></a>Aktivační událost fronty pomocí návratové hodnoty funkce (Java)
 ...
 Následující příklad ukazuje funkci jazyka Java, která používá anotaci `QueueTrigger` pro příjem zprávy obsahující název souboru v kontejneru úložiště objektů BLOB. Anotace `BlobInput` pak přečte soubor a předá jeho obsah funkci jako `byte[]`. Poznámka `BlobOutput` se váže k vrácené hodnotě funkce, kterou modul runtime používá k zápisu obsahu vstupního objektu blob do konfigurovaného kontejneru úložiště.
...
```
 ...
 V [běhové knihovně funkcí jazyka Java](/java/api/overview/azure/functions/runtime)použijte v parametrech funkce anotaci `@BlobOutput`, jejíž hodnota by byla zapsána do objektu v úložišti objektů BLOB.  Typ parametru by měl být `OutputBinding<T>`, kde T je jakýkoliv nativní typ Java nebo POJO.
...
|---------|---------|----------------------|
|**textový** | není k dispozici | Musí být nastavené na `blob`. |
|**směr** | není k dispozici | Pro výstupní vazbu musí být nastavená na `out`. Výjimky jsou uvedeny v části [použití](#output---usage) . |
|**Jméno** | není k dispozici | Název proměnné, která představuje objekt BLOB v kódu funkce.  Nastavte na `$return` pro odkaz na návratovou hodnotu funkce.|
...
| Vazba |  Odkaz |
|---|---|
| Příznaky | [Kódy chyb objektů BLOB](https://docs.microsoft.com/rest/api/storageservices/fileservices/blob-service-error-codes) |
