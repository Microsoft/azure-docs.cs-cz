...
* Vyhněte se obecným cestám, jako je `c:\*.*` při nastavení cesty, protože by to vedlo k procházení příliš velkého počtu složek.
...
* Procesy démon systému Linux mohou zobrazovat změněný stav, i když došlo ke změně. Důvodem je způsob, jakým je zachyceno pole `SvcRunLevels`.
...
|Dotaz  |Popis  |
|---------|---------|
|ConfigurationChange <br>&#124;kde ConfigChangeType = = "Files" a FileSystemPath obsahuje "c:\\Windows\\system32\\Drivers\\"|Užitečné pro sledování změn souborů důležitých pro systém|
|ConfigurationChange <br>&#124;kde FieldsChanged obsahuje "FileContentChecksum" a FileSystemPath = = "c:\\Windows\\system32\\Drivers\\atd.\\hostitelé"|Užitečné pro sledování úprav konfiguračních souborů klíčů|
...
|ConfigurationChange <br>&#124;kde RegistryKey obsahuje @ "HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Services\\parametry SharedAccess\\Parameters\\FirewallPolicy"| Užitečné pro sledování změn nastavení brány firewall|
