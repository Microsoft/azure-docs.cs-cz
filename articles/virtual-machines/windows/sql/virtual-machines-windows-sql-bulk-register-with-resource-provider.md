...
Tento článek popisuje, jak hromadně zaregistrovat SQL Server virtuální počítač v Azure pomocí poskytovatele prostředků virtuálního počítače SQL pomocí rutiny `Register-SqlVMs` PowerShellu.
...
Při každém použití `Register-SqlVMs` rutiny je vygenerována sestava i soubor protokolu. 

### <a name="report"></a>Sestava

Sestava je vygenerována jako `.txt` soubor s `RegisterSqlVMScriptReport<Timestamp>.txt` názvem, kde časové razítko je čas spuštění rutiny. V sestavě jsou uvedeny následující podrobnosti:
