...
Tento příspěvek je zaměřený na vývojáře v jazyce C a C++, kteří se pokoušejí připojit ke službě Azure SQL DB. Rozdělí se na oddíly, takže můžete přejít na oddíl, který nejlépe zaznamená váš zájem.
...
Zkopírujte obsah řetězce **ODBC (včetně Node. js) [ověřování SQL]** . Tento řetězec používáme později pro připojení z našeho překladače příkazového řádku C++ ODBC. Tento řetězec poskytuje podrobnosti, jako jsou například ovladače, server a další parametry připojení databáze.
...
V tomto okamžiku jste nakonfigurovali databázi SQL Azure a jste připraveni se připojit z kódu jazyka C++.
...
Blahopřejeme! Úspěšně jste dokončili kurz a teď se můžete připojit ke službě Azure SQL DB z C++ na platformách Windows a Linux.
