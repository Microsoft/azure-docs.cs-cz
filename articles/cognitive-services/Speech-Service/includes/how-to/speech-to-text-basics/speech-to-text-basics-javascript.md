...
> [!TIP]
> Pokud cílíte na webový prohlížeč `<script>` a používáte značku; `sdk` předpona není potřeba. Předpona `sdk` je alias, který `import` používáme k pojmenování našeho nebo `require` modulu.
