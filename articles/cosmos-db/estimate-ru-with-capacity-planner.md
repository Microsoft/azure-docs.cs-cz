...
|**Vstup**  |**Popis**  |
|---------|---------|
| Rozhraní API |Vyberte typ rozhraní API vašeho účtu. |
...
|Celkem dat uložených v transakčním úložišti |Celková odhadovaná data uložená (GB) v transakčním úložišti v jedné oblasti.|
|Celkem dat uložených v analytickém úložišti | Tato možnost se **zobrazí, pokud zapnete** možnost **použít analytické úložiště** . Představuje celkové odhadované množství uložených dat (GB) v analytickém úložišti v jedné oblasti.  |
|Velikost položky|Odhadovaná velikost datové položky (např. dokumentu) v rozsahu od 1 KB do 2 MB. |
|Počet čtení za sekundu na oblast|Počet očekávaných operací čtení za sekundu |
|Počet vytvoření za sekundu na oblast|Počet očekávaných operací vytvoření za sekundu |
|Počet aktualizací za sekundu na oblast|Počet očekávaných operací aktualizace za sekundu |
|Odstraní za sekundu na oblast.|Počet očekávaných operací odstranění za sekundu |

Po vyplnění požadovaných podrobností vyberte **Vypočítat**. Karta **odhad nákladů** zobrazuje celkové náklady na úložiště a zřízenou propustnost. Kliknutím na odkaz **Zobrazit podrobnosti** na této kartě můžete získat rozpis propustnosti požadované pro různé žádosti CRUD. Pokaždé, když změníte hodnotu libovolného pole, vyberte **Vypočítat** a přepočítejte odhadované náklady.

:::image type="content" source="./media/estimate-ru-with-capacity-planner/basic-mode.png" alt-text="Základní režim plánovače kapacity" border="true":::

## <a name="estimate-provisioned-throughput-and-cost-using-advanced-mode"></a><a id="advanced-mode"></a>Odhad zajištěné propustnosti a nákladů pomocí rozšířeného režimu
...
:::image type="content" source="./media/estimate-ru-with-capacity-planner/advanced-mode.png" alt-text="Rozšířený režim plánovače kapacity" border="true":::
