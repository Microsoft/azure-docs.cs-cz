...
Uzamknout zásady neměnnosti (přidat `-Force` pro zavření výzvy):
...
Odeberte odemčenou zásadu neměnnosti (přidejte `-Force` pro zavření výzvy):
...
[Ukládání důležitých podnikových dat objektů BLOB s neměnném úložištěm](storage-blob-immutable-storage.md)
