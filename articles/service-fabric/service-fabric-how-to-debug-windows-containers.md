...
1. Aby bylo možné podporovat překlad DNS mezi kontejnery, budete muset nastavit místní cluster pro vývoj pomocí názvu počítače. Tyto kroky jsou nezbytné také v případě, že chcete adresovat služby prostřednictvím reverzního proxy serveru.
   ...
   2. Přejděte do složky pro instalaci clusteru sady SDK, obvykle `C:\Program Files\Microsoft SDKs\Service Fabric\ClusterSetup`.
   3. Spusťte skript `DevClusterSetup.ps1`
      ...
      > [!NOTE]
      > Pomocí `-CreateOneNodeCluster` můžete nastavit cluster s jedním uzlem. Ve výchozím nastavení se vytvoří místní cluster s pěti uzly.
      >
