...
>- **Jako zdroj**, v Průzkumníku dat -> přístup, přidělit nejméně **číst + provést** oprávnění k seznamu a zkopírujte soubory ve složce/podsložek, nebo **čtení** zkopírovat jeden soubor; a zvolit oprávnění Přidat jako **přístupová oprávnění a výchozí položka oprávnění**. Žádné požadavky na řízení přístupu na úrovni účtu (IAM).
>- **Jako jímku**, v Průzkumníku dat -> přístup, přidělit nejméně **zapisovat + provést** oprávnění k vytváření podřízených položek ve složce a chcete přidat jako **přístupová oprávnění a výchozí položka oprávnění**. Pokud používáte prostředí Azure IR kopírování (zdroj a jímka mají v cloudu), řízení přístupu (IAM), udělit alespoň **čtečky** role, aby bylo možné umožnit detekci Data Lake Store oblasti služby Data Factory. Pokud chcete se vyhnout této role IAM explicitně [vytvořit prostředí Azure IR](create-azure-integration-runtime.md#create-azure-ir) umístěním systému Data Lake Store a přidružení ve Data Lake Store propojenou službu, jako v následujícím příkladu.
