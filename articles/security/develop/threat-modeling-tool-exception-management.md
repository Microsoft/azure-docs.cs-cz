...
| **Webové rozhraní API** | <ul><li>[Zajistěte, aby ve webovém rozhraní API ASP.NET bylo provedeno správné zpracování výjimek.](#exception)</li></ul> |
...
## <a id="servicedebug"></a>WCF – Nezahrnovat uzel serviceDebug do konfiguračního souboru
...
## <a id="servicemetadata"></a>WCF – Nezahrnovat uzel oddílu serviceMetadata do konfiguračního souboru
...
## <a id="exception"></a>Zajistěte, aby ve webovém rozhraní API ASP.NET bylo provedeno správné zpracování výjimek.
...
| **Atributy**              | Není k dispozici  |
| **Odkazy**              | [Zpracování výjimek ve webovém rozhraní api ASP.NET](https://www.asp.net/web-api/overview/error-handling/exception-handling), [ověřování modelu ve webovém rozhraní API ASP.NET](https://www.asp.net/web-api/overview/formats-and-model-binding/model-validation-in-aspnet-web-api) |
...
### <a name="example"></a>Příklad
Chcete-li řídit stavový kód vrácený rozhraním `HttpResponseException` API, lze použít jak je znázorněno níže: 
```csharp
...
### <a name="example"></a>Příklad
Pro další kontrolu na reakci na výjimku lze použít `HttpResponseMessage` třídu, jak je znázorněno níže: 
```csharp
public Product GetProduct(int id)
{
    Product item = repository.Get(id);
...
### <a name="example"></a>Příklad
Tady je filtr, který převádí `NotImplementedException` výjimky na stavový kód `501, Not Implemented`http: 
```csharp
...
```
### <a name="example"></a>Příklad
Chcete-li použít filtr na všechny akce na `controller`, přidejte filtr jako atribut `controller` do třídy: 
...
### <a name="example"></a>Příklad
Pokud chcete filtr použít globálně na všechny řadiče webového rozhraní API, přidejte do `GlobalConfiguration.Configuration.Filters` kolekce instanci filtru. Filtry výjimek v této kolekci platí pro všechny akce kontroleru webového rozhraní API. 
```csharp
...
## <a id="messages"></a>Nezveřejňujte podrobnosti zabezpečení v chybových zprávách.
...
## <a id="default"></a>Implementace výchozí stránky pro zpracování chyb
...
| **Kroky** | <p>Když aplikace ASP.NET selže a dojde k chybě interního serveru HTTP/1. x 500 nebo konfigurace funkce (například filtrování požadavků) brání zobrazení stránky, bude vygenerována chybová zpráva. Správci mohou zvolit, zda má aplikace zobrazit popisnou zprávu pro klienta, podrobnou chybovou zprávu klientovi nebo podrobnou chybovou zprávu pouze pro localhost. `<customErrors>` Značka v souboru Web. config má tři režimy:</p><ul><li>**Pnete** Určuje, že jsou povoleny vlastní chyby. Pokud není zadán žádný atribut defaultRedirect, zobrazí se uživatelům Obecná chyba. Vlastní chyby se zobrazí vzdáleným klientům a místnímu hostiteli.</li><li>**Zaokrouhl** Určuje, jestli jsou vlastní chyby zakázané. Podrobné chyby ASP.NET se zobrazí vzdáleným klientům a místnímu hostiteli.</li><li>**RemoteOnly** Určuje, že se vlastní chyby zobrazují pouze vzdáleným klientům a že se chyby ASP.NET zobrazují na místním hostiteli. Toto je výchozí hodnota.</li></ul><p>Otevřete soubor pro aplikaci nebo web a ujistěte se, že značka má buď `<customErrors mode="RemoteOnly" />` nebo `<customErrors mode="On" />` definován. `web.config`</p>|

## <a id="deployment"></a>Nastavení metody nasazení na maloobchodní prodej ve službě IIS
...
| **Kroky** | <p>`<deployment retail>` Přepínač je určený pro použití v produkčních serverech služby IIS. Tento přepínač slouží k tomu, aby aplikace běžely s nejlepším možným výkonem a s nejnižšími možnými bezpečnostními informacemi, a to zakázáním schopnosti aplikace generovat výstup trasování na stránce a zakázat možnost zobrazení podrobných chybových zpráv. koncoví uživatelé a zakáže přepínač ladění.</p><p>V průběhu aktivního vývoje jsou často povoleny časy, přepínače a možnosti, které jsou zaměřené na vývojáře, jako je například trasování a ladění požadavků na neúspěšné zpracování. Doporučuje se, aby byla metoda nasazení na jakémkoli provozním serveru nastavená na Retail. Otevřete soubor Machine. config a ujistěte se, `<deployment retail="true" />` že zůstane nastavené na true.</p>|

## <a id="fail"></a>Výjimky by měly selhat bez obav
...
| **Použitelné technologie** | Obecné |
| **Atributy**              | Není k dispozici  |
| **Odkazy**              | [Neúspěšné zabezpečení](https://www.owasp.org/index.php/Fail_securely) |
