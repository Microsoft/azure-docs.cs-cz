...
Spuštění `az network watcher packet-capture show-status` rutina načte stav paketu zachycení aktuálně spuštěné, nebo pokud byla dokončena.

```azurecli
az network watcher packet-capture show-status --name packetCaptureName --location {networkWatcherLocation}
```

V následujícím příkladu je výstup z `az network watcher packet-capture show-status` rutiny. V následujícím příkladu je při zaznamenávání zastavena, s StopReason TimeExceeded. 
