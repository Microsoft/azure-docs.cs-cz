---
title: Sledovat, monitorovat a analyzovat běhy
...
* Spuštění běhu a jeho procesu protokolování
    ...
    1. Připojte konfiguraci pracovního prostoru ke složce, která obsahuje váš školicí skript. Nahraďte `myworkspace` pracovním prostorem Azure Machine Learning. Nahraďte `myresourcegroup` skupinou prostředků Azure, která obsahuje váš pracovní prostor:
        ...
        Další informace najdete v tématu [AZ ml složka připojit](/cli/azure/ml/folder?preserve-view=true&view=azure-cli-latest#az_ml_folder_attach).
    ...
    2. Chcete-li spustit spuštění, použijte následující příkaz. Při použití tohoto příkazu zadejte název souboru RunConfig (text před \* . RunConfig, pokud se díváte na systém souborů) s parametrem-c.
        ...
        Další informace najdete v tématu [AZ ml Run odeslání-Script](/cli/azure/ml/run?preserve-view=true&view=azure-cli-latest#az_ml_run_submit-script).
...
* Monitoruje stav spuštění.
    ...
    * Chcete-li zobrazit seznam spuštění experimentu, použijte následující příkaz. Nahraďte `experiment` názvem experimentu:
        ...
        Další informace najdete v tématu [AZ ml experiment list](/cli/azure/ml/experiment?preserve-view=true&view=azure-cli-latest#az_ml_experiment_list).
    ...
    * Chcete-li zobrazit informace o konkrétním spuštění, použijte následující příkaz. Nahraďte `runid` ID běhu:
        ...
        Další informace najdete v tématu [AZ ml Run show](/cli/azure/ml/run?preserve-view=true&view=azure-cli-latest#az_ml_run_show).
    
    
    # <a name="studio"></a>[Studio](#tab/azure-studio)
    
    ---
## <a name="custom-view"></a>Vlastní zobrazení 
...
:::image type="content" source="media/how-to-track-monitor-analyze-runs/custom-views.gif" alt-text="Snímek obrazovky: Vytvoření vlastního zobrazení":::
...
:::image type="content" source="media/how-to-track-monitor-analyze-runs/run-description.gif" alt-text="Snímek obrazovky: vytvoření popisu spuštění"::: 
...
* Přidat vlastnosti a značky
    ...
    Další informace najdete v tématu [AZ ml Run Update](/cli/azure/ml/run?preserve-view=true&view=azure-cli-latest#az_ml_run_update).
    ...
    :::image type="content" source="media/how-to-track-monitor-analyze-runs/run-tags.gif" alt-text="Snímek obrazovky: Přidání, úprava nebo odstranění značek run":::
...
Další informace najdete v tématu [AZ ml Run Cancel](/cli/azure/ml/run?preserve-view=true&view=azure-cli-latest#az_ml_run_cancel).
...
Pole můžete použít `Run.parent` pro přístup ke spuštění, které spustilo aktuální podřízené spuštění. Běžným případem použití `Run.parent` je kombinace výsledků protokolu na jednom místě. Podřízené běhy se spouštějí asynchronně a není nijak zaručeno řazení ani synchronizace nad rámec možnosti nadřazeného objektu, aby čekal na dokončení jeho podřízených běhů.
...
1. Vyberte **nastavení diagnostiky** a pak vyberte **+ Přidat nastavení diagnostiky**.
    ...
    ![Snímek obrazovky s nastavením diagnostiky pro e-mailové oznámení](./media/how-to-track-monitor-analyze-runs/diagnostic-setting.png)
...
1. V nastavení diagnostiky 
    ...
    ![Místo uložení e-mailových oznámení](./media/how-to-track-monitor-analyze-runs/log-location.png)

1. Na kartě **protokoly** přidejte **nové pravidlo výstrahy**. 

    ![Nové pravidlo výstrahy](./media/how-to-track-monitor-analyze-runs/new-alert-rule.png)
...
* Informace o tom, jak zaznamenat metriky pro vaše experimenty, najdete v tématu [metriky protokolu během školicích běhů](how-to-log-view-metrics.md).
