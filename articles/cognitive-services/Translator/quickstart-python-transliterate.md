...
V současné době je pro službu Translator Text dostupný jeden koncový bod, který je nastavený jako `base_url`. `path` nastaví trasu `transliterate` a určuje, že chceme cílit na rozhraní API verze 3.
