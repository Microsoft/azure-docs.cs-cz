---
title: Předdefinované definice zásad pro Azure Kognitivní hledání
description: Seznam Azure Policy integrovaných definic zásad pro Azure Kognitivní hledání. Tyto integrované definice zásad poskytují běžné přístupy ke správě prostředků Azure.
ms.date: 07/10/2020
ms.topic: sample
author: HeidiSteen
ms.author: heidist
ms.service: search
ms.custom: subject-policy-samples
ms.openlocfilehash: 5e4a12af83fd81134b930103cd28377bf4185fa3
ms.sourcegitcommit: 0b2367b4a9171cac4a706ae9f516e108e25db30c
ms.translationtype: MT
ms.contentlocale: cs-CZ
ms.lasthandoff: 07/11/2020
ms.locfileid: "86274288"
---
# <a name="azure-policy-built-in-definitions-for-azure-cognitive-search"></a>Azure Policy předdefinované definice pro Azure Kognitivní hledání

Tato stránka je indexem [Azure Policy](../governance/policy/overview.md) integrovaných definic zásad pro Azure kognitivní hledání. Další Azure Policy integrovaných pro jiné služby najdete v tématu [Azure Policy předdefinované definice](../governance/policy/samples/built-in-policies.md).

Název každé z vestavěných definic zásad odkazuje na definici zásady v th Azure Portal. Pomocí odkazu ve sloupci **verze** Zobrazte zdroj v [úložišti Azure Policy GitHubu](https://github.com/Azure/azure-policy).

## <a name="azure-cognitive-search"></a>Azure Cognitive Search

[!INCLUDE [azure-policy-samples-rp-search](../../includes/policy/samples/byrp/microsoft.search.md)]

## <a name="next-steps"></a>Další kroky

- Projděte si předdefinované možnosti v [úložišti služby Azure Policy na GitHubu](https://github.com/Azure/azure-policy).
- Projděte si [strukturu definic Azure Policy](../governance/policy/concepts/definition-structure.md).
- Projděte si [Vysvětlení efektů zásad](../governance/policy/concepts/effects.md).
