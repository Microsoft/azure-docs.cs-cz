...
| Kód chyby | Zpráva |
| ---------- | ------- |
| `AADB2C90002` | Prostředek{0}CORS vrátil hodnotu 404, která nebyla nalezena. |
| `AADB2C90006` | Identifikátor URI{0}přesměrování zadaný v požadavku není zaregistrován pro ID{1}klienta. |
| `AADB2C90007` | Aplikace přidružená k ID{0}klienta nemá žádné registrované identifikátory URI pro přesměrování. |
| `AADB2C90008` | Požadavek neobsahuje parametr ID klienta. |
| `AADB2C90010` | Požadavek neobsahuje parametr oboru. |
| `AADB2C90011` | ID{0}klienta uvedené v žádosti neodpovídá ID{1}klienta zaregistrovanému v zásadě. |
| `AADB2C90012` | Obor{0}zadaný v požadavku není podporován. |
| `AADB2C90013` | Požadovaný typ{0}odpovědi zadaný v požadavku není podporován. |
| `AADB2C90014` | Požadovaný režim{0}odpovědi zadaný v požadavku není podporován. |
| `AADB2C90016` | Požadovaný typ{0}kontrolního výrazu klienta se neshoduje s očekávaným typem{1}. |
| `AADB2C90017` | Kontrolní výraz klienta zadaný v požadavku je neplatný:{0} |
| `AADB2C90018` | ID{0}klienta zadané v žádosti není zaregistrované v tenantovi{1}. |
| `AADB2C90019` | Kontejner klíčů s ID{0}v tenantovi{1}nemá platný klíč. Důvod: {2}. |
...
| `AADB2C90023` | {0}Profil neobsahuje požadovaný klíč{1}metadat. |
...
| `AADB2C90031` | {0}Zásada neurčuje výchozí cestu uživatele. Ujistěte se, že zásada nebo její rodiče určují výchozí cestu uživatele jako součást oddílu předávající strany. |
...
| `AADB2C90040` | Cesta{0}uživatele neobsahuje krok odeslat deklarace. |
...
| `AADB2C90044` | Překladač{1}deklarací identity{0}nepodporuje deklaraci identity. |
...
| `AADB2C90055` | Obor{0}zadaný v požadavku musí určovat prostředek,https://example.com/calendar.readtřeba. |
...
| `AADB2C90067` | Identifikátor URI{0}přesměrování po odhlášení má neplatný formát. Zadejte adresu URL založenou na protokolu HTTPShttps://example.com/return, jako je ' ' nebo pro nativní klienty použijte identifikátor URI nativního klienta IETF: IETF: WG: OAuth: 2.0: OOB '. |
| `AADB2C90068` | Poskytnutá aplikace s IDENTIFIKÁTORem{0}není platná pro tuto službu. Použijte prosím aplikaci vytvořenou prostřednictvím portálu B2C a zkuste to znovu. |
| `AADB2C90075` | Výměna{0}deklarací identity zadaná v kroku{1}vrátila chybovou odpověď HTTP s kódem{2}a důvodem.{3} |
...
| `AADB2C90080` | Platnost poskytnutého udělení vypršela. Proveďte prosím znovu ověření a zkuste to znovu. Aktuální čas: {0}, čas vydání grantu {1}:, přidělení posuvných oken čas {2}vypršení platnosti:. |
...
| `AADB2C90092` | Poskytnutá aplikace s ID{0}je pro tenanta{1}zakázaná. Povolte prosím aplikaci a zkuste to znovu. |
...
| `AADB2C90120` | Parametr{0}maximálního stáří zadaný v požadavku je neplatný. Maximální stáří musí být celé číslo v rozmezí{1}od do{2}(včetně). |
| `AADB2C90122` | Vstup pro{0}objekt přijatý v požadavku nedokázal ověřit požadavek HTTP. Ujistěte se, že vstup neobsahuje znaky jako < nebo &. |
...
| `AADB2C90188` | Technický profil{0}SAML určuje PartnerEntity adresu URL, ale načtení metadat z{1}důvodu{2}je neúspěšné. |
| `AADB2C90194` | {0}Deklarace identity zadaná pro token nosiče není v dostupných deklaracích k dispozici. Dostupné deklarace identity{1}. |
...
| `AADB2C90220` | Kontejner klíčů v tenantovi{0}s identifikátorem{1}úložiště existuje, ale neobsahuje platný certifikát. Je možné, že vypršela platnost certifikátu nebo že se váš certifikát může v budoucnu aktivovat (NBF). |
...
| `AADB2C90242` | Technický profil{0}SAML určuje PartnerEntity CDATA, který nelze načíst z důvodu{1}. |
...
| `AADB2C90255` | Výměna deklarací identity zadaná v technickém profilu{0}se nedokončila podle očekávání. Možná budete chtít zkusit spustit relaci od začátku. |
| `AADB2C90261` | Výměna{0}deklarací identity zadaná v kroku{1}vrátila chybovou odpověď HTTP, kterou nebylo možné analyzovat. |
...
| `AADB2C90276` | Požadavek není v souladu s nastavením ovládacího prvku{0}' ': '{1}' v technicalProfile '{2}' pro zásadu{3}' ' klienta{4}' '. |
| `AADB2C90277` | Krok{0}orchestrace uživatelsky navýšení cesty{1}pro zásadu{2}neobsahuje odkaz na definici obsahu. |
| `AADB2C90279` | Zadané ID{0}klienta se neshoduje s ID klienta, které vydalo udělené oprávnění. |
| `AADB2C90284` | Aplikaci s identifikátorem{0}se nepodařilo udělit souhlas a nelze ji použít pro místní účty. |
| `AADB2C90285` | Aplikace s identifikátorem{0}nebyla nalezena. |
| `AADB2C90288` | UserJourney s ID{0}, na které se odkazuje TechnicalProfile{1}pro aktualizaci uplatnění tokenu pro tenanta{2}, neexistuje v zásadě{3}ani v žádné z jeho základních zásad. |
...
| `AADB2C99006` | Aplikace rozšíření s ID{0}aplikace se nedá najít Azure AD B2C. Další informace https://go.microsoft.com/fwlink/?linkid=851224 najdete na stránce. |
| `AADB2C99011` | Hodnota{0}metadat nebyla zadána v TechnicalProfile{1}v zásadě.{2} |
| `AADB2C99013` | Zadaná kombinace grant_type [{0}] a token_type [{1}] není podporována. |
| `AADB2C99015` | {0}U profilu v zásadě{1}v tenantovi{2}chybí všechny InputClaims vyžadované pro tok přihlašovacích údajů k heslu vlastníka prostředku. |
