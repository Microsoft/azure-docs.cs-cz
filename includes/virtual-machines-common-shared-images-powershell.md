...
Zobrazí se seznam imagí, které jsou k dispozici ve skupině prostředků pomocí [Get-AzureRmImage](/powershell/module/AzureRM.Compute/get-azurermimage). Jakmile budete znát název bitové kopie a jaký prostředek seskupit je v, můžete použít `Get-AzureRmImage` znovu k získání objektu image a uložte ji do proměnné pro pozdější použití. Tento příklad načte obrázek s názvem *myImage* ze skupiny prostředků "myResourceGroup" a přiřadí ji do proměnné *$managedImage*. 
...
Galerie obrázků je primární prostředek, který používá k povolení sdílení imagí. Galerie názvy musí být jedinečné v rámci vašeho předplatného. Vytvoření galerie obrázků s využitím [New-AzureRmGallery](/powershell/module/AzureRM.Compute/new-azurermgallery). Následující příklad vytvoří galerii s názvem *myGallery* v *myGalleryRG* skupinu prostředků.
...
Vytvoření s použitím definice image Galerie [New-AzureRmGalleryImageDefinition](/powershell/module/azurerm.compute/new-azurermgalleryimageversion). V tomto příkladu je název image z Galerie *myGalleryImage*.
...
Vytvoření image verze ze spravované image pomocí [New-AzureRmGalleryImageVersion](/powershell/module/AzureRM.Compute/new-azurermgalleryimageversion) . V tomto příkladu je verze image *1.0.0* a se replikuje do obou *střed USA – západ* a *střed USA – jih* datových centrech.
