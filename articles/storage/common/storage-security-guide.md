...
Pokud chcete mít zabezpečený komunikační kanál, měli byste při volání rozhraní REST API nebo přístupu k objektům v úložišti vždycky používat protokol HTTPS. **Sdílené přístupové podpisy**, které se dají použít k delegování přístupu k Azure Storage objektům, zahrnují taky možnost určit, že při použití sdílených přístupových podpisů se dá použít jenom protokol HTTPS, což zajistí, že kdokoli odesílající odkazy s tokeny SAS bude používat správný protokol.
...
* **AllowedOrigins** Tím se dozvíte, které nevyhovující domény můžou vyžádat a přijímat data ze služby úložiště. To říká, že contoso.com i fabrikam.com můžou požadovat data z Blob Storage pro určitý účet úložiště. Můžete ji také nastavit na zástupný znak (\*), aby všechny domény mohly získávat požadavky.
