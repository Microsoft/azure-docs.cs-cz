...
V tomto článku se dozvíte, jak nasadit aplikaci duálního zásobníku (IPv4 + IPv6) v Azure, která zahrnuje virtuální síť s duálním zásobníkem a podsíť, standardní interní Load Balancer s front-end konfiguracemi Dual (IPv4 + IPv6), virtuální počítače se síťovými kartami se dvěma IP adresami. konfigurace, skupina zabezpečení sítě a veřejné IP adresy.
...
- @No__t-0 se zadává jako IPv6.
- Argument `-PublicIpAddress` buď byl vynechán, nebo byl nahrazen řetězcem `-PrivateIpAddress`. Všimněte si, že soukromá adresa musí být v rozsahu ADRESního prostoru podsítě, ve kterém bude nasazen interní nástroj pro vyrovnávání zatížení. Pokud je hodnota static `-PrivateIpAddress` vynechána, bude v podsíti, ve které je nasazen interní nástroj pro vyrovnávání zatížení, vybrána další bezplatná adresa IPv6.
- Podsíť duálního zásobníku, ve které bude nasazen interní nástroj pro vyrovnávání zatížení, je určena parametrem `-Subnet` nebo `-SubnetId`.
