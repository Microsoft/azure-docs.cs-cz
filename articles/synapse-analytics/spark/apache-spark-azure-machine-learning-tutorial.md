...
3. V závislosti na velikosti vašeho fondu Spark může být nezpracovaná data příliš velká nebo mohou trvat příliš dlouho, než budou fungovat. Tato data můžete filtrovat dolů na menší hodnotu pomocí ```start_date``` ```end_date``` filtrů a. Tím se aplikuje filtr, který vrací měsíc dat. Až budete mít filtrovaný datový rámec, spustíte také ```describe()``` funkci na novém dataframe pro zobrazení souhrnných statistik pro každé pole. 
...
4. Dále vygenerujte funkce z datové sady výběrem sady sloupců a vytvořením různých časových funkcí z pole Datum vyzvednutí. Vyfiltrujte odlehlé hodnoty, které byly zjištěny v předchozím kroku, a pak odeberte poslední několik sloupců, protože nejsou potřebné pro školení.
...
```
Po dokončení experimentu vrátí výstup podrobnosti o dokončených iteracích. Pro každou iteraci vidíte typ modelu, dobu trvání běhu a přesnost školení. **Nejlepší** pole sleduje nejlepší průběžné školení na základě typu metriky.

![Snímek výstupu modelu](./media/azure-machine-learning-spark-notebook/model-output.png)

> [!NOTE]
> Po odeslání automatizovaného experimentu strojového učení se spustí různé iterace a typy modelů. Tento běh obvykle trvá 60-90 minut. 
...
1. Po přizpůsobení modelu lineární regrese teď musíte určit, jak dobře model odpovídá datům. Chcete-li to provést, vykreslíte skutečné hodnoty jízdného s předpokládaným výstupem. Kromě toho je třeba vypočítat míru na čtverci, abyste pochopili, jak se data zavřou na pevně danou regresní čáru.
   ...
   Z výsledků vidíte, že účty měr R-kvadrát pro 95 procent odchylky. Tato hodnota je také ověřena skutečným sezobrazovaným v porovnání s pozorovaným vykreslením. Další odchylku, která je zavedená regresní modelem, se blíže k datovým bodům přiblíží vložená regresní čára.  
...
Nakonec můžete k výsledkům iterací získat přístup tak, že v pracovním prostoru Azure Machine Learning do experimentu. Tady se můžete Dig do dalších podrobností o stavu běhu, pokusu o modelech a dalších metrikách modelu. 
