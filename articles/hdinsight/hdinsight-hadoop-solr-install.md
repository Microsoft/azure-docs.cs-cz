---
title: Použití akce skriptu k nainstalovat Solr clusteru Hadoop – Azure | Dokumentace Microsoftu
...
ms.locfileid: "37952871"
---
# <a name="install-and-use-solr-on-windows-based-hdinsight-clusters"></a>Instalace a použití Solru na clusterech HDInsight se systémem Windows

Naučte se přizpůsobení clusteru HDInsight se systémem Windows s využitím akcí skriptů Solr a použití Solru na prohledávat data.

> [!IMPORTANT]
> Kroky v tomto dokumentu fungovat jenom s clustery HDInsight se systémem Windows. HDInsight je dostupná jenom ve Windows pro verze nižší než HDInsight 3.4. HDInsight od verze 3.4 výše používá výhradně operační systém Linux. Další informace najdete v tématu [Vyřazení prostředí HDInsight ve Windows](hdinsight-component-versioning.md#hdinsight-windows-retirement). Informace o použití Solr v clusteru se systémem Linux najdete v tématu [instalace a použití Solru na clusterech HDinsight Hadoop (Linux)](hdinsight-hadoop-solr-install-linux.md).
...
* [Instalace a použití Solru na clusterech HDinsight Hadoop (Linux)](hdinsight-hadoop-solr-install-linux.md)
...
4. **Doporučeno: Zálohování indexovaná data z Solr do Azure Blob storage přidružené ke clusteru HDInsight**. Dobrým postupem je měli zálohovat indexovaná data z uzlů clusteru Solr do úložiště objektů Blob v Azure. Proveďte následující postup:
...
Zobrazit [HDInsight přizpůsobit clustery pomocí akce skriptu](hdinsight-hadoop-customize-cluster.md#call-scripts-using-azure-powershell).  Vzorek ukazuje, jak nainstalovat Spark pomocí Azure Powershellu. Je třeba přizpůsobit skript, který chcete použít [ https://hdiconfigactions.blob.core.windows.net/solrconfigactionv01/solr-installer-v01.ps1 ](https://hdiconfigactions.blob.core.windows.net/solrconfigactionv01/solr-installer-v01.ps1).

## <a name="install-solr-using-net-sdk"></a>Nainstalovat Solr pomocí sady .NET SDK
Zobrazit [HDInsight přizpůsobit clustery pomocí akce skriptu](hdinsight-hadoop-customize-cluster.md#call-scripts-using-azure-powershell). Vzorek ukazuje, jak nainstalovat Spark pomocí sady .NET SDK. Je třeba přizpůsobit skript, který chcete použít [ https://hdiconfigactions.blob.core.windows.net/solrconfigactionv01/solr-installer-v01.ps1 ](https://hdiconfigactions.blob.core.windows.net/solrconfigactionv01/solr-installer-v01.ps1).
...
* [Clustery HDInsight nainstalovat Giraph](hdinsight-hadoop-giraph-install.md): akce skriptu vzorku o instalaci Giraph.
