...
> [!NOTE]  
>  Považuje se za špatný bezpečnostní postup pro předávání citlivých dat, například `api-key` v identifikátoru URI požadavku. Z tohoto důvodu Azure Kognitivní hledání akceptuje klíč dotazu jenom jako `api-key` v řetězci dotazu. měli byste se tomu vyhnout, pokud by obsah vašeho indexu neměl být veřejně dostupný. Jako obecné pravidlo doporučujeme předat `api-key` jako hlavičku požadavku.  
...
3. Vyberte službu a na stránce Přehled klikněte na **nastavení** >**klíče** . zobrazí se klíče pro správu a dotazy.
...
> [!Note]
> Příklad kódu zobrazujícího použití klíče dotazu najdete v [dotazování indexu služby Azure kognitivní hledání v C# ](search-query-dotnet.md).
...
1. Na stránce **nastavení** >**klíče** zkopírujte sekundární klíč.
