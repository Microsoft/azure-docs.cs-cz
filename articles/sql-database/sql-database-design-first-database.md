...
1. Na stránce **databáze SQL** vyberte *yourDatabase* . Otevře se stránka s přehledem pro vaši databázi, na které se zobrazí plně kvalifikovaný **název serveru** (například `contosodatabaseserver01.database.windows.net`), který poskytuje možnosti pro další konfiguraci.
...
K navázání připojení k vaší izolované databázi použijte [SQL Server Management Studio](/sql/ssms/sql-server-management-studio-ssms) .
...
2. V dialogovém okně **Připojení k serveru** zadejte následující informace:
   ...
   | **Hlas** | Účet správce serveru | Jedná se o účet, který jste zadali při vytváření serveru. |
...
> [Návrh relační databáze v jedné databázi v Azure SQL Database C# a ADO.NET](sql-database-design-first-database-csharp.md)
