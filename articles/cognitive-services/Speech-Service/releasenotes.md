...
- **JavaScript**: snazší nastavení pro hlasové asistenty a roboty [`BotFrameworkConfig`](https://docs.microsoft.com/javascript/api/microsoft-cognitiveservices-speech-sdk/botframeworkconfig) , které teď má `fromHost()` a metody pro `fromEndpoint()` vytváření, které zjednodušují použití vlastních umístění služby a ruční nastavení vlastností. Také jsme standardizovani volitelnou specifikaci `botId` pro použití nevýchozího robota v rámci konfiguračních továren.
