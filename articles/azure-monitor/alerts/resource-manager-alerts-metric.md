...
V tomto článku najdete ukázky použití [šablon Azure Resource Manager](../../azure-resource-manager/templates/template-syntax.md) ke konfiguraci [pravidel upozornění metrik](../alerts/alerts-metric-near-real-time.md) v Azure monitor. Každá ukázka obsahuje soubor šablony a soubor parametrů s ukázkovými hodnotami, které se mají poskytnout šabloně.

[!INCLUDE [azure-monitor-samples](../../../includes/azure-monitor-resource-manager-samples.md)]

Seznam prostředků, které se dají používat s pravidly upozornění metrik, najdete [v tématu Podporované prostředky pro výstrahy metrik v Azure monitor](../alerts/alerts-metric-near-real-time.md) . Vysvětlení schématu a vlastností pro pravidlo výstrahy je k dispozici na stránce [Upozornění na metriky – vytvořit nebo aktualizovat](/rest/api/monitor/metricalerts/createorupdate).

> [!NOTE]
> Šablona prostředků pro vytváření výstrah metrik pro typ prostředku: pracovní prostor Azure Log Analytics (tj.) `Microsoft.OperationalInsights/workspaces` vyžaduje další kroky. Podrobnosti najdete v tématu [Výstraha metriky pro protokoly – šablona prostředků](../alerts/alerts-metric-logs.md#resource-template-for-metric-alerts-for-logs).
...
## <a name="single-criteria-static-threshold"></a>Jednoduchá kritéria, statická prahová hodnota
Následující ukázka vytvoří pravidlo upozornění metriky s použitím jednoho kritéria a statické prahové hodnoty.
...
## <a name="single-criteria-dynamic-threshold"></a>Jednoduchá kritéria, dynamická prahová hodnota
...
## <a name="multiple-criteria-static-threshold"></a>Více kritérií, statická prahová hodnota
Výstrahy metrik podporují upozorňování na multidimenzionální metriky a až 5 kritérií na pravidlo výstrahy. Následující ukázka vytvoří pravidlo upozornění metriky pro multidimenzionální metriky a určí více kritérií.

Následující omezení platí při použití dimenzí v pravidle výstrahy, které obsahuje více kritérií:
- V rámci každého kritéria můžete vybrat jenom jednu hodnotu na dimenzi.
- \*Jako hodnotu dimenze nelze použít "".
- Pokud metriky, které jsou konfigurovány v různých kritériích, podporují stejnou dimenzi, pak musí být nakonfigurovaná hodnota dimenze explicitně nastavena stejným způsobem pro všechny metriky v příslušných kritériích.
...
## <a name="multiple-dimensions-static-threshold"></a>Více dimenzí, statická prahová hodnota
Jedno pravidlo výstrahy může sledovat několik časových řad metrik najednou, což má za následek méně pravidel upozornění, která se mají spravovat. Následující ukázka vytvoří pravidlo statické výstrahy metriky pro multidimenzionální metriky.

V této ukázce pravidlo výstrahy monitoruje kombinace hodnot dimenzí dimenzí **ResponseType** a **ApiName** pro metriku **transakcí** :
...
## <a name="multiple-dimensions-dynamic-thresholds"></a>Více dimenzí, dynamické prahové hodnoty
Jedno pravidlo výstrahy s jedním dynamickými prahovými hodnotami může vytvořit přizpůsobené prahové hodnoty pro stovky časových řad (dokonce i různých typů), což vede k menšímu počtu pravidel upozornění, která se mají spravovat. Následující ukázka vytvoří pravidlo upozornění metriky dynamických prahových hodnot pro dimenzionální metriky.
...
## <a name="custom-metric-static-threshold"></a>Vlastní metrika, statická prahová hodnota
...
Azure Monitor podporuje monitorování více prostředků stejného typu s jedním pravidlem upozornění na metriky pro prostředky, které existují ve stejné oblasti Azure. Tato funkce je momentálně podporovaná jenom ve veřejném cloudu Azure a jenom pro virtuální počítače, databáze SQL serveru, elastické fondy SQL serveru a Data Box Edgeá zařízení. Tato funkce je také k dispozici pouze pro metriky platforem a není podporována pro vlastní metriky.
...
Tato ukázka vytvoří pravidlo upozornění na metriky dynamických prahových hodnot, které monitoruje procentuální hodnotu procesoru pro všechny virtuální počítače v jedné oblasti Azure v jedné nebo více skupinách prostředků.
...
Tato ukázka vytvoří pravidlo upozornění na metriku statické prahové hodnoty, které monitoruje procentuální hodnotu procesoru pro všechny virtuální počítače v jedné oblasti Azure v rámci předplatného.
...
Tato ukázka vytvoří pravidlo upozornění na metriku dynamických prahových hodnot, které monitoruje procentuální hodnotu procesoru pro všechny virtuální počítače (v jedné oblasti Azure) v předplatném.
...
Tato ukázka vytvoří pravidlo upozornění na metriku statické prahové hodnoty, které monitoruje procentuální hodnotu procesoru pro seznam virtuálních počítačů v jedné oblasti Azure v rámci předplatného.
...
Tato ukázka vytvoří pravidlo upozornění na metriku dynamických prahových hodnot, které monitoruje procentuální hodnotu procesoru pro seznam virtuálních počítačů v jedné oblasti Azure v rámci předplatného.
