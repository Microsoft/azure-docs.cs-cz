...
K protokolům kontejneru se dá dostat pomocí [Service Fabric Explorer](service-fabric-visualizing-your-cluster.md).  Ve webovém prohlížeči otevřete Service Fabric Explorer z koncového bodu správy clusteru, a to tak, že `http://mycluster.region.cloudapp.azure.com:19080/Explorer`přejdete na.  
...
Ve stromovém zobrazení Najděte balíček kódu na uzlu *_lnxvm_0* rozbalením **uzlů**>**_lnxvm_0**>**Fabric:/**>**azurevotfrontPkg**>**Code Packages**>**Code Code Package Code.**  Pak vyberte možnost **protokoly kontejnerů** a zobrazte tak protokoly kontejnerů.
...
Nastavení **RunInteractive** odpovídá `--interactive` Docker a `tty` [Flags](https://docs.docker.com/engine/reference/commandline/run/#options). Pokud je toto nastavení v souboru manifestu nastaveno na hodnotu true, budou tyto příznaky použity ke spuštění kontejneru.  
