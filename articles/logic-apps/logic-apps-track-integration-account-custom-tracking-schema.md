...
| source | Ano | Řetězec nebo JToken | Pokud se jedná `Microsoft.Logic/workflows`o typ zdroje, musí zdrojové informace postupovat podle tohoto schématu. Pokud je `custom`typ zdroje, je schéma JToken. |
...
| repeatItemScopeName | Ano | Řetězec | Opakovat název položky, pokud je akce uvnitř smyčky `foreach`or `until` |
| repeatItemIndex | Ano | Integer | Označuje, že akce je uvnitř smyčky `foreach` nebo `until` a je opakujícím se číslem indexu položky. |
| trackingId | Ne | Řetězec | ID sledování ke korelaci zpráv |
| correlationId | Ne | Řetězec | ID korelace ke korelaci zpráv |
| ID žádosti klienta | Ne | Řetězec | Klient může tuto vlastnost naplnit tak, aby koreluje zprávy. |
