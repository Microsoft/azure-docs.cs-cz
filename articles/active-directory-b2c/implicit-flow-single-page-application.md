...
| response_type | Ano | Musí zahrnovat `id_token` pro přihlášení OpenID Connect. Může také zahrnovat typ `token`odpovědi. Pokud použijete `token`aplikaci, může ihned získat přístupový token z autorizačního koncového bodu, aniž by se musel zasílat druhý požadavek na koncový bod autorizace.  Použijete-li `token` typ odpovědi, `scope` parametr musí obsahovat obor, který určuje, který prostředek má vydávat token pro. |
...
* Ověřte deklarace `iat` identity `exp` a, abyste zajistili, že platnost tokenu ID nevypršela.
...
| response_type |Požaduje se |Musí zahrnovat `id_token` pro přihlášení OpenID Connect.  Může také zahrnovat typ `token`odpovědi. Pokud zde použijete `token` tuto možnost, může aplikace získat přístupový token z autorizačního koncového bodu, aniž by se musela provést druhá žádost o autorizaci koncového bodu. Použijete-li `token` typ odpovědi, `scope` parametr musí obsahovat obor, který určuje, který prostředek má vydávat token pro. |
...
| response_mode |Doporučené |Určuje metodu, která se používá k odeslání výsledného tokenu zpátky do vaší aplikace. Pro implicitní tok použijte `fragment`. Lze zadat dva další režimy, `query` `form_post`ale nefungují v implicitním toku. |
...
| výzv |Požaduje se |Chcete-li aktualizovat a získat tokeny ve skrytém `prompt=none` prvku IFRAME, použijte k zajištění toho, že prvek IFRAME nebude na přihlašovací stránce zablokovaný, a okamžitě se vrátí. |
...
| domain_hint |Požaduje se |Může být `consumers` nebo `organizations`.  Pokud chcete aktualizovat a získat tokeny ve skrytém prvku IFRAME `domain_hint` , uveďte hodnotu v žádosti.  Extrahujte `tid` deklaraci identity z tokenu ID dřívějšího přihlášení a určete, která hodnota se má použít ( `profile` obor se vyžaduje pro získání `tid` deklarace). Pokud je `tid` `9188040d-6c67-4c5b-b112-36a304b66dad`hodnota deklarace identity, použijte `domain_hint=consumers`.  V opačném `domain_hint=organizations`případě použijte. |
...
Tokeny ID a přístupové tokeny po krátké době vyprší. Vaše aplikace musí být připravená aktualizovat tyto tokeny pravidelně.  Pokud chcete aktualizovat libovolný typ tokenu, proveďte stejný skrytý požadavek IFRAME, který jsme použili v předchozím příkladu, a to `prompt=none` pomocí parametru pro řízení kroků Azure AD.  Chcete-li získat `id_token` novou hodnotu, nezapomeňte použít `response_type=id_token` a `scope=openid`a `nonce` parametr.
