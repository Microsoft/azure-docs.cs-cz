---
title: Úložiště dotazů – databáze Azure pro MariaDB
description: Přečtěte si o funkci Úložiště dotazů v Azure Database for MariaDB, která vám pomůže sledovat výkon v čase.
author: ajlam
ms.author: andrela
ms.service: mariadb
ms.topic: conceptual
...
---
# <a name="monitor-azure-database-for-mariadb-performance-with-query-store"></a>Monitorování výkonu Azure Database pro výkon MariaDB pomocí úložiště dotazů

**Platí pro:** Databáze Azure pro MariaDB 10.2

Funkce Úložiště dotazů v Azure Database for Mariadb poskytuje způsob, jak sledovat výkon dotazů v čase. Úložiště dotazů zjednodušuje řešení potíží s výkonem tím, že vám pomůže rychle najít nejdéle spuštěné a nejvíce náročné dotazy. Úložiště dotazů automaticky zachytí historii dotazů a statistik za běhu a zachová je pro vaši kontrolu. Odděluje data podle časových oken, takže můžete zobrazit vzory využití databáze. Data pro všechny uživatele, databáze a dotazy jsou uložena v databázi **schématu mySQL** v instanci Azure Database for MariaDB.
...
1. Přihlaste se na portál Azure a vyberte si azure databáze pro mariadb server.
...
- Úložiště statistiky za běhu pro uchování informací statistiky spuštění dotazu.
- Statistiky čekání úložiště pro trvalé čekání statistiky informací.
...
Pomocí [portálu Azure](howto-server-parameters.md) získáte nebo nastavíte jinou hodnotu parametru.

## <a name="views-and-functions"></a>Pohledy a funkce

Zobrazení a správa úložiště dotazů pomocí následujících zobrazení a funkcí. Kdokoli ve [veřejné roli select privilege](howto-create-users.md#create-additional-admin-users) může tato zobrazení zobrazit v úložišti dotazů pomocí těchto zobrazení. Tato zobrazení jsou k dispozici pouze v databázi **mysql.**
...
- Pokud má server MariaDB `default_transaction_read_only` parametr zapnutý, úložiště dotazů nemůže zachytit data.
...
- Wait statistiky používá vzorek ti zachytit zlomek událostí. Frekvenci lze upravit pomocí `query_store_wait_sampling_frequency`parametru .
