...
[Vytvořte proměnné prostředí](https://docs.microsoft.com/azure/cognitive-services/cognitive-services-apis-create-account#configure-an-environment-variable-for-authentication)s názvem `QNAMAKER_AUTHORING_KEY` a `QNAMAKER_ENDPOINT`. Soubor [. env. Sample](https://github.com/Azure-Samples/cognitive-services-quickstart-code/blob/master/javascript/QnAMaker/sdk/.env.sample) můžete zkopírovat do `.env` a použít proměnné prostředí v tomto souboru.
...
Soubor `package.json` vaší aplikace se aktualizuje pomocí závislostí. `dotenv` je volitelná a používá se k tomu, aby bylo možné nastavit proměnné prostředí v textovém souboru. Nekontrolujte `.env` do správy zdrojového kódu.
...
Když se metoda Create vrátí, předejte vrácené ID operace do metody [wait_for_operation](#get-status-of-an-operation) k dotazování na stav. Metoda wait_for_operation vrátí po dokončení operace. Analyzuje hodnotu hlavičky `resourceLocation` vrácené operace za účelem získání nového ID znalostní báze.
