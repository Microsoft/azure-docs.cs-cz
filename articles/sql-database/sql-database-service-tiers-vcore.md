...
Úrovně služby jsou rozlišené pomocí rozsah úrovně výkonu, vysoká dostupnost, izolaci chyb, typy úložiště a vstupně-výstupních operací. Zákazník musí zvlášť nakonfigurovat požadované období úložiště a uchovávání dat pro zálohování. Pomocí modelu vCore izolované databáze a elastické fondy mají nárok až na 30 procent spoření s [zvýhodněné hybridní využití Azure pro SQL Server](../virtual-machines/windows/hybrid-use-benefit-licensing.md).
...
|Compute|1 až 80 vCore, Gen4 i Gen5 |1 až 80 vCore, Gen4 i Gen5|
...
- Když konfigurujete velikost databáze (velikost MDF), 30 % dalšího úložiště, je automaticky přidaná kvůli podpoře LDF
- Můžete vybrat libovolné velikosti databáze mezi 10 GB a podporované maximum
...
Přidělení úložiště pro zálohování databáze podporují místo v době obnovení (PITR) a dlouhá období uchovávání dat (LTR) schopnosti služby SQL Database. Toto úložiště je přidělena odděleně pro každou databázi a účtovat jako dvě samostatné na databázi poplatky. 

- **PITR**: jednotlivé databáze, zálohy se zkopírují do úložiště RA-GRS se automaticky. Velikost úložiště zvyšuje dynamicky vytvářené nových záloh.  Úložiště využívané týdenními úplnými zálohami, denními rozdílovými zálohami a zálohami protokolů transakcí se kopíruje každých 5 minut. Spotřeba úložiště závisí na četnosti změn databáze a dobu uchování. Můžete nakonfigurovat samostatný uchovávají pro každou databázi mezi 7 až 35 dnů. Minimální objem úložiště rovna 1 x velikosti dat je k dispozici bez dalších poplatků. Pro většinu databází je tato částka dostatečná k uložení zálohy za 7 dní.
