---
title: SQL Database Managed Instance zabezpečení Azure pomocí přihlášení Azure AD | Dokumentace Microsoftu
...
Azure SQL Database Managed Instance poskytuje téměř všechny funkce zabezpečení, které nejnovější SQL serveru v místním, že má databázový stroj (Enterprise Edition):
...
> [!div class="checklist"]
> - Vytvoření přihlášení Azure Active Directory (AD) pro spravované instance
...
> [!NOTE]
> Azure AD přihlášení pro SQL Database Managed Instance je v **ve verzi public preview**.

Další informace najdete v tématu [přehled Azure SQL Database Managed Instance](sql-database-managed-instance-index.yml) a [možnosti](sql-database-managed-instance.md) článků.
...
- Přístup k Azure SQL Database Managed Instance a [zřízení správce Azure AD pro spravovanou instanci](sql-database-aad-authentication-configure.md#provision-an-azure-active-directory-administrator-for-your-managed-instance). Další informace naleznete v tématu:
    - [Připojení aplikace k Azure SQL Database Managed Instance](sql-database-managed-instance-connect-app.md) 
    - [Architektura připojení spravované instance Azure SQL Database](sql-database-managed-instance-connectivity-architecture.md)
    - [Konfigurace a Správa služby Azure Active Directory ověřování pomocí SQL](sql-database-aad-authentication-configure.md)

## <a name="limiting-access-to-your-managed-instance"></a>Omezení přístupu k Managed Instance

Spravovaná instance je přístupný pouze prostřednictvím privátní IP adresu. Neexistují žádné koncové body služby, které jsou k dispozici pro připojení k Managed Instance, z, mimo síť Managed Instance. Mnohem jako izolované prostředí v místním systému SQL Server, aplikacím nebo uživatelům potřebují přístup k síti spravované Instance (VNet) předtím, než je možné navázat připojení. Další informace najdete v tématu v následujícím článku [vaši aplikaci do Azure SQL Database Managed Instance připojit](sql-database-managed-instance-connect-app.md).

> [!NOTE] 
> Protože Managed instance je přístupný pouze uvnitř jeho virtuální síti, [pravidel brány firewall SQL Database](sql-database-firewall-configure.md) nebudou použity. Spravované instance má vlastní [integrované firewall](sql-database-managed-instance-management-endpoint-verify-built-in-firewall.md).
...
Při prvním přihlášení Azure AD musí být vytvořena standardní účet serveru SQL Server (jiné než azure AD), který je `sysadmin`. Naleznete v následujících článcích pro připojení k Managed Instance příklady:

- [Rychlé zprovoznění: Konfigurace virtuálního počítače Azure pro připojení k Azure SQL Database Managed Instance](sql-database-managed-instance-configure-vm.md)
- [Rychlé zprovoznění: Konfigurace připojení typu point-to-site k Azure SQL Database Managed Instance z místní](sql-database-managed-instance-configure-p2s.md)

> [!IMPORTANT]
> Správce služby Azure AD používané k nastavení Managed Instance nelze použít k vytvoření přihlášení Azure AD v rámci Managed Instance. Musíte vytvořit pomocí účtu systému SQL Server, který je první přihlášení k Azure AD `sysadmin`. Jedná se o dočasné omezení, která bude odebrána po zavedení všeobecné dostupnosti budou přihlašovací údaje Azure AD Pokud se pokusíte vytvořit přihlášení pomocí účtu správce Azure AD se zobrazí následující chyba: `Msg 15247, Level 16, State 1, Line 1 User does not have permission to perform this action.`

1. Přihlaste se k Managed Instance pomocí standardní účet systému SQL Server (jiné než azure AD), který je `sysadmin`s použitím [SQL Server Management Studio](sql-database-managed-instance-configure-p2s.md#use-ssms-to-connect-to-the-managed-instance).
...
## <a name="granting-permissions-to-allow-the-creation-of-managed-instance-logins"></a>Udělení oprávnění k vytvoření Managed Instance přihlášení
...
1. Přihlaste se znovu do Managed Instance, nebo použít existující připojení s SQL objekt zabezpečení, který je `sysadmin`.
...
1. Připojení k serveru Managed Instance pomocí přihlášení Azure AD pomocí aplikace SQL Server Management Studio. Zadejte název serveru Managed Instance. Ověřování v aplikaci SSMS existují tři možnosti, jak vybírat při přihlášení pomocí účtu Azure AD:
...
1. V okně dotazu vytvořte přihlašovací údaje pro jiný účet služby Azure AD pomocí následující syntaxi:
    ...
    Spuštěním následujícího příkazu T-SQL. Federované Azure AD účty jsou nahrazení Managed Instance pro místní Windows přihlášení a uživatelů.
...
1. Vytvořit databázi do Managed Instance pomocí [CREATE DATABASE](/sql/t-sql/statements/create-database-transact-sql?view=azuresqldb-mi-current) syntaxe. Tato databáze se použije k testování přihlášení uživatelů v další části.
    1. V **Průzkumník objektů**, klikněte pravým tlačítkem na server a zvolte **nový dotaz**.
...
1. Jako test Přihlaste se ke Managed Instance pomocí nově vytvořeného přihlášení nebo skupiny. Otevřít nové připojení k Managed Instance a použít nové přihlašovací údaje při ověřování.
...
> [!NOTE]
> Azure AD uživatele typu Host jsou podporovány pro Managed Instance přihlášení pouze v případě přidána jako součást skupiny Azure AD. Uživatel typu Host Azure AD je účet, který je pozvaný do služby Azure AD, který patří Managed Instance, z jiné služby Azure AD. Například joe@contoso.com (účet Azure AD) nebo steve@outlook.com (účet MSA) můžete přidat do skupiny v Azure AD aadsqlmi. Jakmile uživatelé se přidají do skupiny, je možné vytvořit přihlašovací údaje v Managed Instance **hlavní** databáze pro skupinu pomocí **CREATE LOGIN** syntaxe. Uživatelé typu Host, kteří jsou členy této skupiny můžou připojit k Managed Instance pomocí své aktuální přihlašovací údaje (například joe@contoso.com nebo steve@outlook.com).
...
1. Přihlaste se k vašemu Managed Instance pomocí `sysadmin` účtu pomocí SQL Server Management Studio.
1. V **Průzkumník objektů**, klikněte pravým tlačítkem na server a zvolte **nový dotaz**.
...
1. Vytvořit nové připojení k Managed Instance pomocí uživatele, který byl přidán do `db_datareader` role.
...
## <a name="using-cross-database-queries-in-managed-instances"></a>V Managed instance pomocí mezidatabázové dotazy
...
1. Přihlaste se k Managed Instance pomocí SQL Server Management Studio jako člen skupiny Azure AD _mygroup_. Otevřete nové okno dotazu a spusťte příkaz SELECT mezidatabázové:
...
Přečtěte si následující [funkce zabezpečení funkce Managed Instance](sql-database-managed-instance.md#azure-sql-database-security-features) článku s úplným seznamem způsoby, jak zabezpečit vaši databázi. Jsou popsány následující funkce zabezpečení:
...
Úplný přehled funkcí Azure SQL Database Managed Instance naleznete v tématu:
