---
title: Řešení potíží s samoobslužným resetem hesla – Azure Active Directory
...
ms.locfileid: "68879171"
---
# <a name="troubleshoot-self-service-password-reset"></a>Řešení potíží s samoobslužným resetováním hesla

Máte potíže se službou Azure Active Directory (Azure AD) Samoobslužné resetování hesla (SSPR)? Následující informace vám můžou pomohou znovu pracovat.
...
| SsprNotEnabledInUserPolicy = 11 | Je nám líto, ale heslo teď nemůžete resetovat, protože správce nenakonfiguroval resetování hesla pro vaši organizaci. Neexistuje žádná další akce, kterou můžete použít k vyřešení této situace. Obraťte se na správce a požádejte ho o konfiguraci resetování hesla. Další informace o konfiguraci resetování hesel najdete v [tématu rychlý Start: Samoobslužné resetování](https://docs.microsoft.com/azure/active-directory/active-directory-passwords-getting-started)hesla služby Azure AD. | SSPR_0011: Vaše organizace nedefinovala zásady pro resetování hesla. Kontaktujte prosím správce a požádejte ho, aby definoval zásady pro resetování hesla. |
...
| Když se uživatel pokusí resetovat heslo nebo odemknout účet se zapnutým zpětným zápisem hesla, operace se nezdařila. <br> <br> Kromě toho se v protokolu událostí Azure AD Connect, který obsahuje, zobrazí událost: "Synchronizační modul vrátil chybu HR = 800700CE, zpráva = název souboru nebo přípona je příliš dlouhá" po výskytu operace odemknutí. | Vyhledejte účet služby Active Directory pro Azure AD Connect a resetujte heslo, aby neobsahovalo více než 256 znaků. Pak otevřete **synchronizační službu** z nabídky **Start** . Přejděte ke **konektorům** a najděte **konektor služby Active Directory**. Vyberte ji a pak vyberte **vlastnosti**. Přejděte na stránku **přihlašovací údaje** a zadejte nové heslo. Kliknutím na **tlačítko OK** stránku zavřete. |
...
| 33004| ADPermissionsError| Tato událost označuje, že účet služby Agent pro správu služby Active Directory (ADMA) nemá v daném účtu příslušná oprávnění k nastavení nového hesla. Ujistěte se, že účet ADMA v doménové struktuře uživatele má oprávnění resetovat a změnit heslo pro všechny objekty v doménové struktuře. Další informace o tom, jak nastavit oprávnění, najdete v kroku 4: Nastavte příslušná oprávnění služby Active Directory.|
...
* **Obecný popis chyby**: Jaká je chyba? Jak se jednalo o chování, které se zaznamenalo? Jak můžeme chybu reprodukována? Poskytněte co nejvíce podrobností.
...
* **Kód podpory**: Jaký byl kód podpory, který se vygeneroval, když uživatel viděl chybu?
...
* **Datum, čas a časové pásmo**: Zahrňte přesné datum a čas v časovém pásmu, ve kterém došlo k chybě.
* **ID uživatele**: Kdo byl uživatel, který viděl tuto chybu? Příkladem je *contoso.com\@uživatele*.
...
* **Protokol událostí aplikace**: Pokud používáte zpětný zápis hesla a chyba je ve vaší místní infrastruktuře, zahrňte do Azure AD Connect serveru kopii protokolu událostí aplikace.
