...
> [!NOTE] 
> Od března 2021 můžete také používat značky služby místo explicitních rozsahů IP adres v [trasách definovaných uživatelem](./virtual-networks-udr-overview.md). Tato funkce je v současné době ve verzi Public Preview. 
...
| **AzureAPIForFHIR** | Rozhraní Azure API pro FHIR (rychlé provázání prostředků pro interoperabilitu zdravotní péče)<br/><br/> *Poznámka: Tato značka se v tuto chvíli nedá nakonfigurovat přes Azure Portal.*| Odchozí | No | No |
...
| **AzureMonitor** | Log Analytics, Application Insights, AzMon a vlastní metriky (koncové body 4Gigový).<br/><br/>*Poznámka:* Pro Log Analytics se vyžaduje také značka **úložiště** . Pokud se používají agenti Linux, vyžaduje se také značka **GuestAndHybridManagement** . | Odchozí | No | Yes |
