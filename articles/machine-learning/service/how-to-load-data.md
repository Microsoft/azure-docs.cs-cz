...
ms.locfileid: "46989681"
---
#<a name="load-data-with-the-azure-machine-learning-data-prep-sdk"></a>Načtení dat pomocí sady SDK pro Azure Machine Learning Data Prep

[Sady SDK služby Azure Machine Learning Data Prep](https://docs.microsoft.com/python/api/overview/azure/dataprep?view=azure-dataprep-py) vám umožní načíst různé druhy vstupní data. Můžete určit typ datového souboru a jeho parametry nebo pomocí funkce Inteligentní čtení sady SDK automaticky rozpoznat typ souboru.

## <a name="read-lines"></a>Načíst řádky
...
## <a name="read-csv"></a>Čtení sdíleného svazku clusteru
...
## <a name="read-excel"></a>Aplikace Excel pro čtení
Obsahuje sadu SDK pro Azure Machine Learning Data Prep `read_excel` funkce načíst soubory aplikace Excel.
```
...
Načetli jste první tabulky Excelového souboru. Stejného výsledku dosáhnout explicitním zadáním názvu na listu, které chcete načíst. Pokud chcete místo toho zatížení druhý list, můžete zadat jeho název jako argument.
```
...
Jak je vidět, došlo v tabulce v druhé tabulce záhlaví a tři prázdné řádky. Budete muset upravit argumenty funkce odpovídajícím způsobem.
```
...
## <a name="read-fixed-width-files"></a>Čtení souborů s pevnou šířkou
Pro soubory s pevnou šířkou můžete zadat seznam posunutí. První sloupec je vždy považován za spuštění na posunu 0.
...
Pokud neexistují žádné hlavičky v souborech, je vhodné na první řádek považovat za data. Předáním `PromoteHeadersMode.NONE` argument – klíčové slovo záhlaví, můžete vyhnout detekci záhlaví a získat správná data.
...
## <a name="read-sql"></a>Přečtěte si SQL
...
Chcete-li čtení dat z SQL serveru, vytvořte objekt zdroje dat, který obsahuje informace o připojení.
...
Jakmile vytvoříte objekt zdroje dat, můžete přejít k číst data.
```
...
## <a name="read-from-azure-data-lake-storage"></a>Čtení ze služby Azure Data Lake Storage
...
Můžete použít rozhraní Azure CLI k vytvoření instančního objektu služby a odpovídající certifikát. Tento konkrétní instanční objekt je nakonfigurovaný jako čtečka otisku prstů, kde je její obor omezit na jenom účet úložiště Azure Data Lake "dpreptestfiles"
```
...
##### <a name="configure-an-azure-data-lake-storage-account-for-the-service-principal"></a>Konfigurace účtu Azure Data Lake Storage pro instanční objekt
Ke konfiguraci seznamu ACL pro systém souborů Azure Data Lake Storage, použijte ID objektu uživatele nebo instanční objekt v tomto příkladu:
```
az ad sp show --id "8dd38f34-1fcb-4ff9-accd-7cd60b757174" --query objectId
```
##### <a name="configure-read-and-execute-access-for-the-azure-data-lake-storage-file-system"></a>Konfiguraci pro čtení a provádění přístup pro systém souborů Azure Data Lake Storage.
Protože základní model řízení přístupu HDFS nepodporuje dědičnosti, musíte nakonfigurovat seznam ACL pro soubory a složky jednotlivě.
```
