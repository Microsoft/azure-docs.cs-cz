...
|Možnost|Popis|
|-|-|
| Vytvoření bezplatného App Service spravovaného certifikátu (Preview) | Privátní certifikát, který se dá snadno použít, pokud potřebujete zabezpečení `www` [vlastní domény](app-service-web-tutorial-custom-domain.md) nebo domény, které nejsou ve holém prostředí, v App Service. |
...
- Jenom bezplatný certifikát: namapujte subdoménu (například `www.contoso.com`) tak, aby App Service se [záznamem CNAME](app-service-web-tutorial-custom-domain.md#map-a-cname-record).
...
K zabezpečení vlastní domény ve vazbě SSL má certifikát další požadavky:
...
Bezplatný App Service spravovaný certifikát představuje řešení pro zabezpečení vlastního názvu DNS v App Service. Jedná se o plně funkční certifikát SSL, který se spravuje App Service a automaticky se obnovuje. Bezplatný certifikát přináší následující omezení:
...
V levém navigačním panelu aplikace vyberte **Nastavení TLS/SSL** > **certifikáty privátních klíčů (. pfx)**  > **Import App Service Certificate**.
...
V levém navigačním panelu aplikace vyberte **Nastavení TLS/SSL** > **certifikáty privátních klíčů (. pfx)**  > **nahrát certifikát**.
...
Po nahrání certifikátu zkopírujte kryptografický otisk certifikátu a podívejte [se na zpřístupněný certifikát](configure-ssl-certificate-in-code.md#load-the-certificate).
...
- [Exportovat certifikát](#export-certificate)
