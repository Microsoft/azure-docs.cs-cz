...
V rámci `BreakSentence()`třídy vytvořte funkci s názvem. `Program` Tato funkce přijímá čtyři argumenty: `subscriptionKey`, `host`, `route` `inputText`a.
...
Všimněte si, že v `Main`, deklarujete `subscriptionKey`, `host` `route`, a text, který chcete vyhodnotit `breakSentenceText`.
