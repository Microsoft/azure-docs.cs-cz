...
Vytvořte nový projekt Go pomocí svého oblíbeného IDE / editoru nebo nové složky na ploše. Potom zkopírujte tento fragment kódu do projektu nebo `dictionaryLookup.go`složky v souboru s názvem .
...
Pojďme vytvořit funkci získat alternativní překlady. Tato funkce bude trvat jeden argument, váš klíč předplatného Translator Text.
...
Dále vytvoříme adresu URL. Adresa URL je `Parse()` sestavena pomocí metod a. `Query()` Všimněte si, že parametry `Add()` jsou přidány s metodou. V této ukázce překládáme z angličtiny do španělštiny.

Zkopírujte tento `altTranslations` kód do funkce.
...
Dále vytvořte anonymní strukturu pro tělo požadavku a zakódujte jej jako JSON s `json.Marshal()`. Přidejte tento `altTranslations` kód do funkce.
...
Přidejte tento `altTranslations` kód do funkce dekódovat odpověď JSON a potom formátovat a vytisknout výsledek.
