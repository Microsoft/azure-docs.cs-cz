---
title: Správa koncových bodů virtuální sítě – portál Azure – databáze Azure pro MySQL
description: Vytváření a správa koncových bodů a pravidel služby Azure Database pro virtuální síť MySQL pomocí portálu Azure
author: bolzmj
ms.author: mbolz
ms.service: mysql
ms.topic: conceptual
...
ms.locfileid: "80063539"
---
# <a name="create-and-manage-azure-database-for-mysql-vnet-service-endpoints-and-vnet-rules-by-using-the-azure-portal"></a>Vytvoření a správa koncových bodů služby Azure Database for MySQL VNet a pravidel virtuální sítě pomocí portálu Azure
Pravidla a koncové body služeb virtuální sítě rozšiřují privátní adresní prostor virtuální sítě na server Azure Database for MySQL. Přehled koncových bodů služby Azure Database for MySQL VNet, včetně omezení, najdete v [tématu Azure Database for MySQL Server Virtuální síť koncových bodů](concepts-data-access-and-security-vnet.md). Koncové body služby Virtuální sítě jsou dostupné ve všech podporovaných oblastech pro Azure Database for MySQL.
...
1. Na stránce MySQL server v záhlaví Nastavení klikněte na **Zabezpečení připojení** a otevřete podokno Zabezpečení připojení pro Azure Database for MySQL. 

2. Ujistěte se, že povolit přístup ke službám Azure ovládací prvek je nastavena na **VYPNUTO**.

> [!Important]
> Pokud ponecháte ovládací prvek nastavený na ZAPNUTO, váš server Azure MySQL Database přijímá komunikaci z libovolné podsítě. Ponechání ovládacího prvku nastaveného na zapnuto může být nadměrný přístup z hlediska zabezpečení. Funkce koncového bodu služby Virtuální síť Microsoft Azure v koordinaci s funkcí pravidla virtuální sítě v Azure Database for MySQL může společně snížit plochu zabezpečení.
...
- Podobně můžete skriptovat [povolit koncové body služby virtuální sítě a vytvořit pravidlo virtuální sítě pro Azure Database for MySQL pomocí rozhraní příkazového příkazu Azure](howto-manage-vnet-using-cli.md).
- Nápovědu k připojení k azure databázi pro mySQL server najdete v tématu [Knihovny připojení pro Azure Database for MySQL.](./concepts-connection-libraries.md)
