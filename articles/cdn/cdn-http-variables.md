...
| Hodnota hlavičky žádosti | %{http_RequestHeader} | Vrátí hodnotu odpovídající hlavičce požadavku identifikovaného termínem RequestHeader. <br /><br />Pokud název hlavičky požadavku obsahuje spojovník (například User-Agent), nahraďte ho podtržítkem (například User_Agent).| Použití ukázkového souboru:% {http_Connection}<br /><br />Ukázková hodnota: Udržování připojení | 
...
| Identifikátor URI žádosti (relativní bez řetězce dotazu) | % {URI} | Určuje relativní cestu k požadovanému obsahu. <br /><br/>Informace o klíči:<br />– Tato relativní cesta vyloučí řetězec dotazu.<br />– Tato relativní cesta odráží přepsání adresy URL. Adresa URL bude přepsána za následující podmínky:<br />  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-Funkce přepsání adresy URL: Tato funkce přepíše relativní cestu definovanou v identifikátoru URI požadavku.<br />    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;– Adresa URL CNAME Edge: Tento typ požadavku se přepíše na odpovídající adresu URL CDN. |/800001/corigin/rewrittendir/foo.js |
| Identifikátor URI žádosti | % {Request} | Popisuje požadavek. <br />Syntaktick &lt;Protokol HTTP&gt; relativní cesty&gt; &lt; kmetodě&lt;http&gt; | ZÍSKAT/marketing/foo.js? LoggedIn = true HTTP/1.1 |
| Hodnota hlavičky odpovědi | % {resp_&lt;ResponseHeader&gt;} | Vrátí hodnotu odpovídající hlavičce Response identifikovaného &lt;termínem ResponseHeader.&gt; <br /><br />Pokud název hlavičky odpovědi obsahuje pomlčku (například User-Agent), nahraďte ji podtržítkem (například User_Agent). | Použití ukázkového souboru:% {resp_Content_Length}<br /><br />Ukázková hodnota: 100 |
...
- Hodnota přiřazená k posunu určuje počáteční znak podřetězce:
     ...
     - Kladný Počáteční znak podřetězce je vypočítán z prvního znaku v řetězci.
     - Vynulujte Počátečním znakem podřetězce je první znak v řetězci.
     - Příznivé Počáteční znak podřetězce je vypočítán z posledního znaku v řetězci.

- Délka podřetězce je určena *délkou* :

     - Vynechán Vynechání délky období umožňuje, aby dílčí řetězec zahrnoval všechny znaky mezi počátečním znakem a koncem řetězce.
     - Kladný Určuje délku podřetězce od počátečního znaku vpravo.
     - Příznivé Určuje délku podřetězce od počátečního znaku vlevo.
...
| Syntaxe | Action |
| ------ | ------ |
| %{Variable#Pattern} | Odebere text, pokud je zadaný vzor nalezen na začátku hodnoty proměnné. |
| %{Variable%Pattern} | Odebere text, pokud je zadaný vzor nalezen na konci hodnoty proměnné. |
...
- Případ zástupného symbolu vzoru (například $1) lze upravit pomocí následujících příznaků:
     ...
     - L: Malá a velká písmena rozšířené hodnoty.
...
- Před vzorem je nutné zadat operátor. Zadaný operátor určuje chování zachycení vzoru:
     ...
     - `=`: Indikuje, že se musí zachytit a přepsat všechny výskyty zadaného vzoru.
     - `^`: Určuje, že bude zachycen pouze text, který začíná zadaným vzorem.
     - `$`: Určuje, že bude zachytávání pouze text, který končí zadaným vzorem.
