...
Můžete najít další informace o zprávy typu cloud zařízení v [zasílání zpráv část Příručka vývojáře pro IoT Hub](iot-hub-devguide-messaging.md).

Na konci tohoto článku spustíte dvě Swift projekty iOS:

* **Ukázka zařízení**, stejnou aplikaci vytvořenou v [odesílání telemetrických dat ze zařízení do služby IoT hub](quickstart-send-telemetry-ios.md), který se připojí ke službě IoT hub a přijímá zprávy typu cloud zařízení.
...
Toto je ukázková zařízení, který vytvoříte v následujícím článku [odesílání telemetrických dat ze zařízení do služby IoT hub](quickstart-send-telemetry-ios.md). Pokud už máte, systém, můžete tuto část přeskočit.
...
1. Načtení připojovacího řetězce pro vaše zařízení. Můžete zkopírovat tento řetězec z [webu Azure portal](https://portal.azure.com) v okně podrobností o zařízení je nebo načtěte pomocí následujícího příkazu rozhraní příkazového řádku:
...
1. Načtení připojovacího řetězce služby pro službu IoT hub. Můžete zkopírovat tento řetězec z [webu Azure portal](https://portal.azure.com) z **iothubowner** zásad v **zásady sdíleného přístupu** okně je nebo načtěte pomocí následujícího příkazu rozhraní příkazového řádku:  
...
2. V **ukázka klienta služby IOT hub** aplikaci spuštěnou v service s Simulovaná zařízení, zadejte ID zařízení IoT, který chcete odeslat zprávu. 
...
Příklady kompletní řešení začátku do konce, které používají služby IoT Hub najdete v tématu [akcelerátory řešení Azure IoT](https://azure.microsoft.com/documentation/suites/iot-suite/) dokumentaci.
