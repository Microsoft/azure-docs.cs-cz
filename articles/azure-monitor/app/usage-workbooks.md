...
* Kolik uživatelů navštívilo určitou sadu stránek na webu, ale ne některé jiné sady stránek? To může být užitečné pochopit, jestli máte clustery uživatelů, kteří používají různé podmnožiny funkcí vaší lokality (použijte `join` operátor s `kind=leftanti` modifikátorem v [jazyce dotazů Kusto](/azure/kusto/query/)).
