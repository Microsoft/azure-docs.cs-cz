...
Pokud chcete porovnat BYOL s průběžnými platbami a licencováním pro SQL Server na virtuálním počítači Azure, přečtěte si téma Začínáme [s SQL Servermi virtuálními počítači](sql-server-on-azure-vm-iaas-what-is-overview.md#get-started-with-sql-server-vms).
...
FILESTREAM není podporován pro cluster s podporou převzetí služeb při selhání se sdílenou složkou Premium. Pokud chcete použít FILESTREAM, nasaďte cluster pomocí [prostory úložiště s přímým přístupem](failover-cluster-instance-storage-spaces-direct-manually-configure.md).
...
- Doménový uživatelský účet pro spuštění služby SQL Server a který můžete použít k přihlášení k virtuálnímu počítači při připojování sdílené složky. 
...
1. Až Azure vytvoří vaše virtuální počítače, připojí se ke každému z nich pomocí protokol RDP (Remote Desktop Protocol) (RDP).
...
1. Pokud používáte jednu z imagí virtuálních počítačů založených na SQL Server, odeberte instanci SQL Server.
   ...
   1. Odeberte všechny funkce ve **službě databázového stroje**. Neodstraňujte **sdílené funkce**. Uvidíte něco podobného jako na následujícím snímku obrazovky:
        ...
        ![Výběr funkcí](./media/failover-cluster-instance-premium-file-share-manually-configure/03-remove-features.png)
...
1. V rozevíracím seznamu vyberte písmeno jednotky, které chcete použít, a potom zkopírujte oba bloky kódu do poznámkového bloku.
   ...
   :::image type="content" source="media/manually-configure-failover-cluster-instance-premium-file-share/premium-file-storage-commands.png" alt-text="Kopírování příkazů PowerShellu z portálu pro připojení ke sdílené složce":::
...
Teď nakonfigurujete cluster s podporou převzetí služeb při selhání. V této části provedete následující kroky:
...
1. V části **Výběr testu**vyberte všechny testy s výjimkou **úložiště** a **prostory úložiště s přímým přístupem**, jak je znázorněno zde:
   ...
   :::image type="content" source="media/manually-configure-failover-cluster-instance-premium-file-share/cluster-validation.png" alt-text="Výběr testů pro ověření clusteru":::
...
### <a name="create-a-cloud-witness-win-2016-and-later"></a>Vytvoření určujícího cloudu (Win 2016 a novější)
...
Pro Windows Server 2016 a novější Nakonfigurujte cluster tak, aby používal diskovou kopii cloudu, kterou jste právě vytvořili. Postupujte podle všech kroků a [nakonfigurujte určující disk kvora v uživatelském rozhraní](https://technet.microsoft.com/windows-server-docs/failover-clustering/deploy-cloud-witness#to-configure-cloud-witness-as-a-quorum-witness).
...
:::image type="content" source="media/manually-configure-failover-cluster-instance-premium-file-share/test-cluster-failover.png" alt-text="Testování převzetí služeb při selhání clusteru přesunutím základního prostředku do ostatních uzlů":::
...
1. Vyberte **nový SQL Server instalace clusteru s podporou převzetí služeb při selhání**. Podle pokynů v průvodci nainstalujte SQL Server FCI.
   ...
   :::image type="content" source="media/manually-configure-failover-cluster-instance-premium-file-share/use-file-share-as-data-directories.png" alt-text="Použít sdílení souborů jako datové adresáře SQL":::
