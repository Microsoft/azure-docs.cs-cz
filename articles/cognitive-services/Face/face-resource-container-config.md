...
Nastavení `ApiKey` Určuje klíč prostředku Azure, který se používá ke sledování fakturačních informací pro kontejner. Je nutné zadat hodnotu pro ApiKey a hodnota musí být platným klíčem pro prostředek _Cognitive Services_ zadaný pro nastavení [`Billing`](#billing-configuration-setting) konfigurace.
...
Konfigurační nastavení v části `CloudAI` poskytují specifické možnosti kontejneru, které jsou pro váš kontejner jedinečné. Následující nastavení a objekty jsou podporovány pro kontejner obličeje v části `CloudAI`

| Název | Data type | Popis |
|------|-----------|-------------|
| `Storage` | Objekt | Scénář úložiště používaný kontejnerem obličeje. Další informace o scénářích úložiště a souvisejících nastaveních pro objekt `Storage` najdete v tématu [nastavení scénáře úložiště](#storage-scenario-settings) . |
...
Scénáře úložiště a přidružená nastavení konfigurace jsou spravovány objektem `Storage` v části `CloudAI` konfigurace. V `Storage`m objektu jsou k dispozici následující konfigurační nastavení:
...
| `ConnectionStringOfAzureStorage` | Řetězec | Připojovací řetězec pro prostředek Azure Storage používaný kontejnerem.<br/>Toto nastavení platí pouze v případě, že je pro nastavení konfigurace `StorageScenario` zadána `Azure`. |
| `ConnectionStringOfCosmosMongo` | Řetězec | Připojovací řetězec MongoDB pro prostředek Azure Cosmos DB používaný kontejnerem.<br/>Toto nastavení platí pouze v případě, že je pro nastavení konfigurace `StorageScenario` zadána `Azure`. |
...
> [!IMPORTANT]
> Aby bylo možné spustit kontejner, musí být zadány možnosti `Eula`, `Billing`a `ApiKey`. v opačném případě se kontejner nespustí.  Další informace najdete v tématu [fakturace](face-how-to-install-containers.md#billing).
