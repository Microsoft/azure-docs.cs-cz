...
2. Výběrem příkazu **Execute (spustit** ) vložte nový řádek do tabulky `Product`. Zobrazí **Messages** se podokno zprávy **(počet ovlivněných řádků: 1)** .
...
2. Výběrem příkazu **Spustit** odstraňte zadaný řádek v `Product` tabulce. Zobrazí **Messages** se podokno zprávy **(počet ovlivněných řádků: 1)** .
