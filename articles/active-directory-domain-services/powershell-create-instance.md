...
* Abyste mohli vytvořit požadované prostředky Azure služba AD DS, potřebujete oprávnění *přispěvatele* v předplatném Azure.
  ...
  > [!IMPORTANT]
  > I když je modul PowerShell **AZ. ADDomainServices** ve verzi Preview, musíte ho nainstalovat samostatně pomocí `Install-Module` rutiny.
...
Pomocí vytvořené skupiny *správci řadiče domény AAD* Získejte ID objektu požadovaného uživatele pomocí rutiny [Get-AzureADUser][Get-AzureADUser] a potom přidejte uživatele do skupiny pomocí rutiny [Add-AzureADGroupMember][Add-AzureADGroupMember] .
