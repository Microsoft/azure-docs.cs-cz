...
## <a name="on-premises-virtualizationhost-servers"></a>Místní hostitel nebo virtualizace serverů
...
--- | ---
Nastavení počítače | Počítače, které se replikují do Azure, musí splňovat [požadavky pro Azure](#failed-over-azure-vm-requirements).
...
Adresáře | Tyto adresáře (Pokud nastavený jako samostatné oddíly/souboru systems) musí být na stejném disku operačního systému na zdrojovém serveru: / (kořenová), / Boot, USR, /usr/local, /var, etc.</br></br> / Boot by měla být na oddílu disku a nesmí být svazek LVM<br/><br/>
Požadavky na volné místo| 2 GB v oddílu/root <br/><br/> 250 MB v instalační složce
XFSv5 | Funkce XFSv5 v systémech souborů s XFS, jako je například metadata kontrolního součtu, jsou podporovány z verze služby Mobility 9.10 a vyšší. Nástroje xfs_info zkontrolujte tzv XFS pro oddíl. Pokud ftype nastavena na hodnotu 1, funkce XFSv5 jsou používány.
...
**Architektura operačního systému hosta** | 64 bitů | Kontrola selže, pokud není podporován. 
**Velikost disku s operačním systémem** | Až 2 048 GB | Kontrola selže, pokud není podporován. 
**Počet disků s operačním systémem** | 1 | Kontrola selže, pokud není podporován.  
**Počet datových disků** | 64 nebo méně | Kontrola selže, pokud není podporován.  
**Velikost datového virtuálního pevného disku** | Až 4095 GB | Kontrola selže, pokud není podporován. 
...
**Disk FC** | Nepodporuje se. | Kontrola selže, pokud není podporován. 
**Formát pevného disku** | VHD <br/><br/> VHDX | VHDX se aktuálně nepodporuje v Azure, ale Site Recovery automaticky převede VHDX na virtuální pevný disk po převzetí služeb při selhání. Pokud žádnou zpět na místní, virtuální počítače nadále používat formát VHDX.
...
**Typ virtuálního počítače** | Generace 1 a generace 2 (jenom Windows) |  Virtuální počítače 2. generace, musí mít základní disk operačního systému (včetně nebo dvě datové svazky naformátované jako VHDX) a menší než 300 GB místa na disku 
Virtuální počítače s Linuxem generace 2 nejsou podporované. 
