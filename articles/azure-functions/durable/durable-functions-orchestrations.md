...
* Identifikátory instance nesmí začínat na `@`.
* Identifikátory instance nesmí obsahovat `/`, `\`, `#`ani `?` znaky.
...
Durable Functions používá zdrojové události transparentně. Na pozadí poskytuje operátor `await` (C#) nebo `yield` (JavaScript) ve funkci Orchestrator kontrolu nad vláknem nástroje Orchestrator zpátky do objektu Dispatcher dispečera rozhraní úloh. Dispečer pak potvrdí všechny nové akce, které naplánovala funkce Orchestrator (například volání jedné nebo více podřízených funkcí nebo naplánování trvalého časovače) do úložiště. Transparentní akce potvrzení připojí k historii spuštění instance Orchestration. Historie je uložená v tabulce úložiště. Akce potvrzení pak přidá zprávy do fronty, aby naplánovala skutečnou práci. V tomto okamžiku může být funkce Orchestrator uvolněna z paměti.
...
3. Zařazování zpráv do fronty pro samotný nástroj Orchestrator &mdash; například odolné zprávy časovače.
...
| PartitionKey (InstanceId)                     | Typ             | Časové razítko               | Vstup | Název             | Výsledek                                                    | Status |
|----------------------------------|-----------------------|----------|--------------------------|-------|------------------|-----------------------------------------------------------|
| eaee885b | ExecutionStarted      | 2017-05-05T18:45:28.852Z | platnost  | E1_HelloSequence |                                                           |                     |
...
* **EventType**: představuje typ události. Může být jeden z následujících typů:
  * **OrchestrationStarted**: funkce Orchestrator obnovila z await nebo je spuštěná poprvé. Sloupec `Timestamp` slouží k naplnění deterministické hodnoty pro rozhraní API `CurrentUtcDateTime` (.NET) a `currentUtcDateTime` (JavaScript).
  * **ExecutionStarted**: funkce Orchestrator zahájila první spuštění. Tato událost také obsahuje vstup funkce ve sloupci `Input`.
  * **TaskScheduled**: naplánovala se funkce Activity. Název funkce aktivity je zachycen ve sloupci `Name`.
  * **TaskCompleted**: byla dokončena funkce aktivity. Výsledek funkce je ve sloupci `Result`.
  ...
  * **EventRaised**: do instance Orchestration byla odeslána externí událost. Sloupec `Name` zachycuje název události a `Input` sloupec zachycuje datovou část události.
  * **OrchestratorCompleted**: očekává se funkce Orchestrator.
  * **ContinueAsNew**: funkce Orchestrator byla dokončena a restartována s novým stavem. Sloupec `Result` obsahuje hodnotu, která se používá jako vstup v instanci restartu.
  * **ExecutionCompleted**: funkce Orchestrator se úspěšně dokončila (nebo selhala). Výstupy funkce nebo podrobnosti o chybě jsou uloženy ve sloupci `Result`.
...
Orchestrace můžou naplánovat *trvalé časovače* pro implementaci zpoždění nebo nastavení zpracování časového limitu u asynchronních akcí. Používejte odolné časovače ve funkcích nástroje Orchestrator namísto `Thread.Sleep` aC#`Task.Delay` () nebo `setTimeout()` a `setInterval()` (JavaScript).
...
Funkce nástroje Orchestrator mohou používat funkce pro zpracování chyb v programovacím jazyce. Existující vzory, jako je `try`/`catch`, jsou podporovány v kódu orchestrace.
...
> [!NOTE]
> Pokud ve funkci Orchestrator dojde k neošetřené výjimce, instance Orchestration se dokončí ve stavu `Failed`. Instanci orchestrace nelze opakovat, jakmile se nezdařila.
...
`LockAsync` získá trvalá zámky a vrátí `IDisposable`, který po Dispose ukončí oddíl kritické. Tento `IDisposable` výsledek lze použít společně s blokem `using` pro získání syntaktické reprezentace důležité části. Když funkce Orchestrator vstoupí do kritické části, může tento blok kódu spustit pouze jedna instance. Všechny ostatní instance, které se pokoušejí zadat kritickou část, budou zablokovány, dokud předchozí instance neukončí kritickou část.
