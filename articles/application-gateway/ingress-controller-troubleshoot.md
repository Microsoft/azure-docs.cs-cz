...
Následující postup předpokládá:
  ...
  - Už jste ověření Application Gateway ve virtuální síti sdílené s vaším clusterem AKS.
...
Zkopírujte a vložte všechny řádky z výše uvedeného skriptu do [Azure Cloud Shell](https://shell.azure.com/). Ujistěte se prosím, že je zkopírovaný celý příkaz – počínaje `cat` a včetně posledního `EOF`.

![vyrovnat](./media/application-gateway-ingress-controller-troubleshooting/tsg--apply-config.png)
...
Jedna z lusků bude AGIC. `kubectl get pods` se zobrazí seznam lusků, z nichž jedna začíná "příchozí – Azure". Získejte všechny protokoly, které jsou pod `kubectl logs <name-of-ingress-controller-pod>`, a ověřte tak, že máme úspěšné nasazení. Úspěšné nasazení by do protokolu přidalo následující řádky:
```
...
Případně můžete z [Cloud Shell](https://shell.azure.com/) načíst pouze řádky indikující úspěšnou Application Gateway konfiguraci pomocí `kubectl logs <ingress-azure-....> | grep 'Applied App Gateway config in'`, kde `<ingress-azure....>` by měl být přesný název AGIC pod.

Application Gateway bude použita následující konfigurace:

- Naslouchací proces:](./media/application-gateway-ingress-controller-troubleshooting/tsg--listeners.png) naslouchacího procesu ![
...
Nakonec můžeme použít příkaz `cURL` v rámci [Cloud Shell](https://shell.azure.com/) k navázání připojení HTTP k nově nasazené aplikaci:

1. Pomocí `kubectl get ingress` získat veřejnou IP adresu Application Gateway
2. Použít `curl -I -H 'test.agic.contoso.com' <publitc-ip-address-from-previous-command>`

![Podů](./media/application-gateway-ingress-controller-troubleshooting/tsg--curl.png)

Výsledek `HTTP/1.1 200 OK` označuje, že systém Application Gateway + AKS + AGIC pracuje podle očekávání.
...
Aby funkce AGIC fungovala podle očekávání, musí být na tomto místě:
  1. AKS musí mít jednu nebo více zdravých **lusků**.
     Ověřte to z [Cloud Shell](https://shell.azure.com/) s `kubectl get pods -o wide --show-labels` Pokud máte pod `apsnetapp`, váš výstup může vypadat takto:
     ```bash
  ...
  2. Jedna nebo více **služeb**, odkazující na lusky přes párové popisky `selector`.
     Ověřit tento [Cloud Shell](https://shell.azure.com/) s využitím `kubectl get services -o wide`
     ```bash
  ...
  3. Příchozí s poznámkou `kubernetes.io/ingress.class: azure/application-gateway`s odkazem na službu výše ověřte tuto [Cloud Shell](https://shell.azure.com/) pomocí `kubectl get ingress -o wide --show-labels`
     ```bash
  ...
  4. Zobrazit anotace příchozího přenosu dat výše: `kubectl get ingress aspnetapp -o yaml` (nahraďte `aspnetapp` názvem vašeho příchozího přenosu dat).
     ```bash
     ...
     U prostředku příchozího přenosu dat musí být poznámení s `kubernetes.io/ingress.class: azure/application-gateway`.
...
* AGIC pod by měl být v oboru názvů `default` (viz `NAMESPACE`sloupců). V pořádku by se `Running` ve sloupci `STATUS`. Měl by existovat alespoň jeden AGIC pod.
...
* Je vaše [příchozí](https://kubernetes.io/docs/concepts/services-networking/ingress/) Poznámka s poznámkami: `kubernetes.io/ingress.class: azure/application-gateway`? AGIC se bude sledovat jenom u prostředků Kubernetes příchozího přenosu dat, které mají tuto poznámku.
...
| Podrobností | Popis |
|-----------|-------------|
|  1\. místo        | Výchozí úroveň protokolu; zobrazuje podrobnosti o spuštění, upozornění a chyby. |
...
Úrovně podrobností lze nastavit pomocí proměnné `verbosityLevel` v souboru [Helm-config. yaml](#sample-helm-config-file) . Zvýšit úroveň podrobností na `5` a získat tak konfiguraci JSON, která se odešle do [ARM](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-overview):
  - Přidat `verbosityLevel: 5` na sebe sebe v [Helm-config. yaml](#sample-helm-config-file) a znovu nainstalovat
  - získání protokolů pomocí `kubectl logs <pod-name>`
