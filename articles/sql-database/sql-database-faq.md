...
## <a name="whatis-the-new-vcore-based-purchasing-model-for-azure-sql-database"></a>Whatis nový na základě vCore nákupní model pro Azure SQL Database?

Nový model nákupu je kromě existujícího modelu na základě DTU. Modelu na základě vCore je navržená tak získali zákazníkům flexibilitu, řízení, průhlednost a snadný způsob, jak převede místní požadavky úloh do cloudu. Také to umožňuje zákazníkům škálování výpočetní kapacity a úložiště podle jejich potřeb pracovního vytížení. Izolované databáze a možnosti elastického fondu pomocí modelu vCore jsou také vhodné pro až na 30 procent úspor s [výhody použití hybridní Azure pro systém SQL Server](../virtual-machines/windows/hybrid-use-benefit-licensing.md). V tématu [úrovních služeb](sql-database-service-tiers.md) podrobnosti o nákupu modelu na základě DTU a na základě vCore nákupní model.
...
## <a name="is-moving-to-the-vcore-based-model-required"></a>Přechází na modelu na základě vCore vyžaduje?
...
DTU a na základě vCore modely bude existovat vedle sebe. Jsme se spouští na základě vCore modelu v reakci na požadavky zákazníků další transparentnosti kolem jejich prostředky databáze a schopnost škálování výpočetní kapacity a úložiště samostatně. Model na základě vCore taky umožňuje dalším úsporám plynoucím pro zákazníky s active Software Assurance prostřednictvím Benefit hybridní Azure pro systém SQL Server.

## <a name="how-should-i-choose-between-the-dtu-based-purchasing-model-vs-the-vcore-based-purchasing-model"></a>Jak mám zvolit mezi vs na základě DTU nákupu modelu na základě vCore nákupní model? 
Jednotky transakcí databáze (DTU) je založena na kombinaci měření procesoru, paměti, čte a zapisuje. Úrovně výkonu na základě DTU představuje předkonfigurované sady prostředků disku různé úrovně výkonu aplikace. Zákazníci, kteří nechcete starat o základních prostředků a raději jednoduchost předkonfigurované sady při platícího pevná každý měsíc možná modelu na základě DTU vhodnější pro své potřeby. Pro zákazníky, kteří potřebují další aspekty příslušných prostředků nebo potřebujete škálování je nezávisle na nástroji k dosažení optimálního výkonu, ale modelu na základě vCore bude nejlepší volbou.  Kromě toho pokud zákazník má aktivní Software Assurance (SA) pro systém SQL Server, mohou využívat své stávající investice a uložit až 30 % [výhody použití hybridní Azure pro systém SQL Server](../virtual-machines/windows/hybrid-use-benefit-licensing.md).  Možnosti v rámci každé nákupu modely poskytují výhody plně spravované služby, jako je automatické zálohování, softwarové aktualizace a opravy. 

## <a name="what-is-the-azure-hybrid-benefit-for-sql-server"></a>Jaké jsou výhody hybridní Azure pro systém SQL Server? 
[Výhody použití hybridní Azure pro systém SQL Server](../virtual-machines/windows/hybrid-use-benefit-licensing.md) pomáhá maximalizovat hodnotu z vaše aktuální licencování investice a urychlit jejich migraci do cloudu. Výhody založené na Azure, která umožňuje licencí systému SQL Server pomocí programu Software Assurance věnovat snížené (dále jen "základní míra") v databázi SQL Azure hybridní využívat pro SQL Server je. Azure Benefit hybridní pro SQL Server je k dispozici ve verzi public preview na základě vCore nákupu modelu pro databáze SQL izolované databáze i elastické fondy. Může použít tuto výhodu, i když je aktivní verze SKU, ale Všimněte si, že základní míra platí od okamžiku, vyberte ho v portálu Azure. Žádný kredit bude vydaná dodatečně.
...
V současné době nabízíme zákazníci SQL serveru s licencí programu Software Assurance mobility výhody, které umožňuje opětovné přiřazení jejich licencí pro servery třetích stran, které jsou sdílené. Tuto výhodu lze použít v Azure IaaS a AWS EC2.
...
Nové úrovně služeb nabízejí jednoduchý online převod metodu, která je podobná existující proces upgradu databáze z Standard na úroveň služeb Premium a naopak. Tento převod lze inicializovat pomocí portálu, ARM, prostředí PowerShell, rozhraní příkazového řádku Azure nebo T-SQL. V tématu [spravovat izolované databáze](sql-database-single-database-resources.md) a [spravovat elastické fondy](sql-database-elastic-pool.md).
...
Na základě faktur SQL Database na předvídatelný hodinové sazby [zakoupení modelu](sql-database-service-tiers.md). Skutečné využití je počítaný a poměrně každou hodinu, takže vaše faktura může zobrazovat zlomků jednu hodinu. Například pokud databáze existuje 12 hodin za měsíc, vaše faktura zobrazuje využití 0,5 dní. 
...
Podrobnosti najdete v tématu [SQL Database – ceny](https://azure.microsoft.com/pricing/details/sql-database/) a [úrovních služeb](sql-database-service-tiers.md).
...
- Je-li vytvořit instanci spravované a odstranit ji později pět minut, se vám budou účtovat pro jednu databázi hodinu.
