---
title: Šifrování dat pomocí klíče spravovaného zákazníkem – Azure Database for MySQL
description: Šifrování dat Azure Database for MySQL pomocí klíče spravovaného zákazníkem umožňuje přenést vlastní klíč (BYOK) pro ochranu dat v klidovém stavu. To také umožňuje organizacím provádět oddělení povinností při správě klíčů a dat.
author: kummanish
ms.author: manishku
ms.service: mysql
ms.topic: conceptual
...
![Diagram, který znázorňuje přehled přineste si vlastní klíč](media/concepts-data-access-and-security-data-encryption/mysqloverview.png)

Aby server MySQL používal klíče spravované zákazníkem uložené v trezoru klíčů k šifrování sady DEK, poskytuje správce trezoru klíčů serveru následující přístupová práva:
...
* Trezor klíčů a Azure Database for MySQL musí patřit ke stejnému tenantovi Azure Active Directory (Azure AD). Meziklientský trezor klíčů a interakce serveru nejsou podporovány. Přesunutí prostředků poté vyžaduje překonfigurovat šifrování dat.
...
* Udělte databázi Azure pro MySQL přístup k trezoru klíčů s oprávněními get, wrapKey a unwrapKey pomocí jeho jedinečné spravované identity. Na webu Azure Portal se jedinečná identita automaticky vytvoří, když je na MySQL povoleno šifrování dat. Podrobné podrobné pokyny při používání webu Azure Portal najdete v článku [Konfigurace šifrování dat pro MySQL.](howto-data-encryption-portal.md)
...
* Ujistěte se, že trezor klíčů a Databáze Azure pro MySQL jsou umístěny ve stejné oblasti, abyste zajistili rychlejší přístup pro operace dek obtékání a rozbalování.
...
* Spusťte proces vytváření obnovy nebo čtení replik z hlavní databáze Azure pro MySQL.
...
Zjistěte, jak [nastavit šifrování dat pomocí klientem spravovaného klíče pro databázi Azure pro MySQL pomocí portálu Azure](howto-data-encryption-portal.md).
