...
> [!TIP]
> Při přidávání vrstev do mapy pomocí `map.layers.add` metody může být ID nebo instance existující vrstvy předána jako druhý parametr. To by znamenalo, že mapa pro vložení nové vrstvy se přidá pod existující vrstvu. V kromě pro předávání v ID vrstvy tato metoda také podporuje následující hodnoty.
>
