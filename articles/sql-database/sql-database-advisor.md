---
title: Doporučení pro výkon služby Database Advisor pro databáze s jednou a ve fondu
description: Azure SQL Database poskytuje doporučení pro databáze s jednou a ve fondu, které můžou zlepšit výkon dotazů ve službě Azure SQL Database.
...
ms.locfileid: "82096567"
---
# <a name="database-advisor-performance-recommendations-for-single-and-pooled-databases"></a>Database Advisor doporučení k výkonu pro databáze s jednou a fondem

Azure SQL Database se učí a přizpůsobí vaší aplikaci. Pro databáze typu Single a Pool SQL Database má mnoho poradců databáze, které poskytují přizpůsobená doporučení, která vám umožní maximalizovat výkon. Tyto informační zpravodaje budou průběžně hodnotit a analyzovat historii využití a poskytovat doporučení na základě vzorců úloh, které pomůžou zvýšit výkon.
...
- Dlaždice **doporučení** obsahuje rozpis doporučení pro ladění pro vaši databázi (pokud existuje více), zobrazí se v nich více hlavních tří doporučení. Kliknutím na tuto dlaždici přejdete na **[Možnosti doporučení pro výkon](sql-database-advisor-portal.md#viewing-recommendations)**.
...
- Dlaždice **dotazy databáze** zobrazuje souhrn výkonu dotazů pro vaši databázi (celkové využití DTU a nejčastější dotazy týkající se využívání prostředků). Kliknutím na tuto dlaždici přejdete na **[Query Performance Insight](sql-database-query-performance.md)**.

## <a name="performance-recommendation-options"></a>Možnosti doporučení výkonu

K dispozici jsou možnosti doporučení výkonu pro databáze s jednou a fondem v Azure SQL Database:
...
Pokud chcete použít doporučení pro výkon, přečtěte si téma [použití doporučení](sql-database-advisor-portal.md#applying-recommendations). Stav doporučení zobrazíte v tématu [monitorování operací](sql-database-advisor-portal.md#monitoring-operations).
