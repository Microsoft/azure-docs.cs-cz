...
> [!TIP]
> Můžete se také rozhodnout povolit jednotné přihlašování založené na SAML pro SpaceIQ podle pokynů uvedených v [kurzu SpaceIQ jednotného přihlašování](https://docs.microsoft.com/azure/active-directory/saas-apps/spaceiq-tutorial). Jednotné přihlašování se dá nakonfigurovat nezávisle na automatickém zřizování uživatelů, i když se tyto dvě funkce vzájemně přidávají.
...
5. V části **přihlašovací údaje správce** zadejte `https://api.spaceiq.com/scim` na **adrese URL tenanta**. Zadejte hodnotu **SCIM tokenu ověřování** získanou dříve v **tajném tokenu**. Klikněte na **Test připojení** a ujistěte se, že se služba Azure AD může připojit k SpaceIQ. Pokud se připojení nepovede, ujistěte se, že má váš účet SpaceIQ oprávnění správce, a zkuste to znovu.
