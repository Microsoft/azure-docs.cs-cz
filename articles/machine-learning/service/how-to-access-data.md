...
Proměnná `script_params` je slovník obsahující parametry entry_script. Použijte ho k předání do úložiště dat a popište, jak budou data dostupná na výpočetním cíli. Další informace získáte v našem uceleném [kurzu](tutorial-train-models-with-aml.md).
...
```
Pokud dáváte přednost použití objektu RunConfig pro školení, je nutné nastavit objekt [DataReference](https://docs.microsoft.com/en-us/python/api/azureml-core/azureml.data.data_reference.datareference?view=azure-ml-py) . 
