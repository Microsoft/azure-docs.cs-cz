...
V tomto kurzu se dozvíte, jak nakonfigurovat nastavení týkající se zařízení Azure Stack Edge pomocí GPU s integrovaným grafickým procesorem. Pomocí místního webového uživatelského rozhraní můžete nastavit název zařízení, server aktualizací a časový server.
...
* Pro vaše fyzické zařízení:
    ...
    - V zařízení jste nakonfigurovali síť a povolili a nakonfigurovali výpočetní síť, jak je podrobně popsáno v tomto [kurzu: konfigurace sítě pro Azure Stack Edge pomocí GPU](azure-stack-edge-gpu-deploy-configure-network-compute-web-proxy.md).
...
1. Na dlaždici **instalace zařízení** v části **zařízení**vyberte **Konfigurovat**.

    ![Stránka místního webového uživatelského rozhraní (zařízení)](./media/azure-stack-edge-gpu-deploy-set-up-device-update-time/device-1.png)

    Na stránce **zařízení** proveďte následující kroky:
...
1. Na dlaždici **instalace zařízení** pro možnost **aktualizovat**vyberte **Konfigurovat**. Teď můžete nakonfigurovat umístění, ze kterého se mají stahovat aktualizace pro vaše zařízení.  

    ![Místní webové uživatelské rozhraní – stránka aktualizace serveru](./media/azure-stack-edge-gpu-deploy-set-up-device-update-time/update-1.png)
...
2. Vyberte **Použít**.
3. Po dokončení konfigurace serveru aktualizací se vraťte na **začátek**.
...
1. Na dlaždici **instalace zařízení** vyberte **čas**. Můžete vybrat časové pásmo a primární a sekundární servery NTP pro vaše zařízení.  
    ...
    ![Stránka čas místního webového uživatelského rozhraní](./media/azure-stack-edge-gpu-deploy-set-up-device-update-time/time-1.png)

2. Na stránce **čas** postupujte takto:
...
3. Po použití nastavení se **vraťte na začátek**.
...
Informace o tom, jak nakonfigurovat certifikáty pro zařízení Azure Stack Edge, najdete v těchto tématech:
