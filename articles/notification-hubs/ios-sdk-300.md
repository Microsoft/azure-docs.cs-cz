...
10. V souboru projektu **AppDelegate. h** přidejte následující `import` příkaz:

    ```objc
    #import "Constants.h"
    ```

11. Ve stejném souboru **AppDelegate. m** nahraďte veškerý kód za `didFinishLaunchingWithOptions` následujícím kódem:
...
2. Také v **soubor viewcontroller. h** přidejte po deklaraci následující deklaraci vlastnosti `@interface` :
