...
Pokud chcete použít generování schématu, zahrňte do `inference-schema` souboru závislostí Open Source balíček verze 1.1.0 nebo novější. Další informace o tomto balíčku najdete v tématu [https://github.com/Azure/InferenceSchema](https://github.com/Azure/InferenceSchema) . Aby bylo možné generovat vyhovující automatické spotřebě webových služeb, musí mít funkce běhu skriptu bodování () tento tvar rozhraní API:
* První parametr typu "StandardPythonParameterType", pojmenované vstupy, vnořené obsahující PandasDataframeParameterTypes.
* Nepovinný druhý parametr typu "StandardPythonParameterType" s názvem GlobalParameter, který není vnořený.
* Vrátí slovník typu "StandardPythonParameterType", který je možná vnořený obsahující PandasDataFrameParameterTypes.
...
Následující příklad ukazuje, jak definovat tvar rozhraní API podle pokynů výše. Tato metoda je podporována pro využívání nasazené webové služby od Power BI. (Další[informace o tom, jak využívat webovou službu z Power BI](https://docs.microsoft.com/power-bi/service-machine-learning-integration).)
