...
MSAL.NET používá adresy URL k vyvolání zprostředkovatele a pak vrátí odpověď zprostředkovatele do vaší aplikace. Pokud chcete dokončit operaci round trip, zaregistrujte do souboru `Info.plist` schéma adresy URL pro vaši aplikaci.
...
Podpora zprostředkovatele je povolená na základě PublicClientApplication. Ve výchozím nastavení je zakázaný. Při vytváření `IPublicClientApplication` prostřednictvím `PublicClientApplicationBuilder`použijte parametr `WithBroker()` (ve výchozím nastavení nastaven na hodnotu true).
...
Toho lze dosáhnout směrováním výsledku do `SetAuthenticationContinuationEventArgs(int requestCode, Result resultCode, Intent data)` přepsáním metody OnActivityResult (), jak je znázorněno níže.
...
K tomu je nutné zadat aktivitu (obvykle MainActivity) do `WithParentActivityOrWindow(object parent)` jako nadřazený objekt. 
...
Poslední část identifikátoru URI, `hgbUYHVBYUTvuvT&Y6tr554365466=`, je podpis, se kterým je APK podepsán, kódování Base64.
