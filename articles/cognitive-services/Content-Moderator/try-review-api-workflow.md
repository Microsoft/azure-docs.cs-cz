...
1. Upravte pole **Text žádosti** , aby se ZADAL požadavek JSON s podrobnostmi pro **Popis** a **typ** (`Image` nebo `Text`).
...
Vyberte **Poslat**. Pokud je operace úspěšná, bude **stav odpovědi** `200 OK` a v poli **obsah odpovědi** se zobrazí `true`.
...
Vyberte **Poslat**. Pokud je operace úspěšná, bude **stav odpovědi** `200 OK` a v poli **obsah odpovědi** se zobrazí pracovní postup ve formátu JSON, jako je následující:
