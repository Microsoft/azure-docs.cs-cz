...
- [Nákupní model založený](sql-database-service-tiers-vcore.md) na vCoreech umožňuje zvolit počet virtuální jádra, velikost paměti a množství a rychlost úložiště. Nákupní model založený na vCoreech vám také umožní použít [Zvýhodněné hybridní využití Azure SQL Server](https://azure.microsoft.com/pricing/hybrid-benefit/) k získání úspory nákladů. Další informace o Zvýhodněné hybridní využití Azure najdete v tématu [Nejčastější dotazy](#sql-database-frequently-asked-questions-faq).
...
- **Azure Storage**: Archivace obrovského množství telemetrie za malou cenu
- **Centrum událostí Azure**: Integrace SQL Database telemetrie s vlastními řešeními monitorování nebo aktivními kanály
...
Zotavení po havárii předpokládá, že závažná událost bude geograficky lokalizovaná, aby měla další počítač/sadu počítačů s kopií vašich dat.  V SQL Server můžete k získání této funkce použít skupiny dostupnosti Always On spuštěné v asynchronním režimu.  Rychlost světle potíží obvykle znamená, že uživatelé nechtějí čekat na replikaci, která je daleko před potvrzením transakce, takže při neplánovaných převzetí služeb při selhání může dojít ke ztrátě dat.
...
Efektivní správa velkého počtu databází může být nemožná i se všemi dostupnými nástroji a sestavami, které SQL Database a Azure Portal nabízí. Místo ručního monitorování a optimalizace databáze můžete zvážit delegování některých akcí monitorování a optimalizace do služby SQL Database pomocí [automatické optimalizace](sql-database-automatic-tuning.md). SQL Database automaticky aplikuje doporučení, testy a ověří každé z jeho akcí optimalizace, aby se zajistilo, že bude výkon neustále zlepšovat. Díky tomu se služba SQL Database může automaticky přizpůsobit vaší úloze řízeným a bezpečným způsobem. Automatická optimalizace znamená, že se výkon vaší databáze pečlivě monitoruje a porovnává před a po každé akci optimalizace, a pokud se výkon nezlepší, akce optimalizace se vrátí zpět.
