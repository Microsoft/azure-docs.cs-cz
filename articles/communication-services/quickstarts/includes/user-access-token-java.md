...
Pokud jste nastavili spravovanou identitu, najdete informace v tématu [použití spravovaných identit](../managed-identity.md). ke inicializaci a ověřování pomocí služby identity můžete použít spravovanou identitu.
```java
...
Alternativně můžete pomocí metody ' createUserWithToken ' vytvořit novou položku v adresáři s jedinečným `Id` a vydáním přístupového tokenu.
