...
| Jak je nakonfigurován nebo nainstalovat?      |  Nainstalovaná s podporou GPU   |
| Jak používat nebo ji spustit      | Jako knihovna Pythonu (2,7 a 3,5), balíček R a nástroj příkazového řádku na cestě (`C:\dsvm\tools\xgboost\bin\xgboost.exe` pro Windows a `/dsvm/tools/xgboost/xgboost` pro Linux)    |
...
| Jak používat nebo ji spustit      | Jako nástroj uživatelského rozhraní. V systému Windows spusťte příkazový řádek, spusťte R a potom v jazyce R spusťte `rattle()`příkaz. V systému Linux se připojte pomocí X2Go, spusťte terminál, spusťte R a potom v jazyce R spusťte příkaz `rattle()`. |
...
K dispozici je několik dalších knihoven strojového učení v DSVMs, jako je `scikit-learn` oblíbený balíček, který je součástí distribuce Pythonu Anaconda pro DSVMs. Podívejte se na seznam balíčků dostupných v Pythonu, R a Helena, spuštěním Správce příslušných balíčků.
