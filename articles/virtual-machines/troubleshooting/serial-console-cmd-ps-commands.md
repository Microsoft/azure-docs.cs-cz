...
Po znaménku rovná se vyžaduje mezera. Mezi možné počáteční hodnoty patří `boot`, `system`, `auto`, `demand`, `disabled`, `delayed-auto`.
...
Další informace o `netsh`získáte [kliknutím sem](https://docs.microsoft.com/windows-server/networking/technologies/netsh/netsh-contexts).
...
### <a name="enable-windows-firewall"></a>Zapnutí brány Windows Firewall
...
Virtuální počítače Azure vytvořené z generalizované image budou mít účet místního správce přejmenovaný na název zadaný při zřizování virtuálních počítačů. Proto se obvykle nebude `Administrator`.
...
Změňte `/c:10` na požadovaný počet událostí, které se mají vrátit, nebo ho přesuňte, aby se vracely všechny události, které filtr odpovídají.
...
`sort /r` seřadí sestupně podle data instalace a usnadňuje tak zobrazení, co bylo nedávno nainstalováno. Použijte `<spacebar>` k přechodu na další stránku výstupu nebo `<enter>` na jeden řádek.
### <a name="uninstall-an-application"></a>Odinstalace aplikace
`wmic path win32_product where name="<name>" call uninstall`

U aplikace, kterou chcete odebrat, nahraďte `<name>` názvem vráceným ve výše uvedeném příkazu.
...
Cesta při použití `/restore` musí být nadřazenou složkou složky, kterou jste zadali při použití `/save`. V tomto příkladu je `\RSA` nadřazeným adresářem `\MachineKeys`, který je určený v předchozím příkladu `/save`.
...
Přidání `/f` vynutí, aby se spuštěné aplikace zavřely bez upozornění uživatele.
...
`Get-Service` lze použít, ale nezahrnuje přihlašovací účet služby. `Get-WmiObject win32-service`.
### <a name="set-service-logon-account"></a>Nastavení přihlašovacího účtu služby
...
`Set-service` pro typ spouštění přijmout `Automatic`, `Manual`nebo `Disabled`.
### <a name="set-service-dependencies"></a>Nastavit závislosti služby
...
`Get-NetAdapter` je k dispozici v roce 2012 +, pro 2008 R2 use `Get-WmiObject`.
### <a name="show-ip-properties"></a>Zobrazit vlastnosti IP adresy
...
`Get-NetAdapter` je k dispozici v roce 2012 +. Pro 2008 R2 použijte `Get-WmiObject`. Virtuální počítače Azure by měly být vždy nakonfigurované v hostovaném operačním systému, aby k získání IP adresy používaly protokol DHCP. Nastavení statické IP adresy Azure pořád používá protokol DHCP k přidělení IP adresy virtuálnímu počítači.
### <a name="ping"></a>Ping
`test-netconnection`

> [!NOTE]
> Rutina s průběhem zápisu nemusí s tímto příkazem fungovat. Jako zmírnění můžete v prostředí PowerShell spustit `$ProgressPreference = "SilentlyContinue"`, abyste mohli vypnout indikátor průběhu.

nebo

`get-wmiobject Win32_PingStatus -Filter 'Address="8.8.8.8"' | format-table -autosize IPV4Address,ReplySize,ResponseTime`

`Test-Netconnection` bez parametrů se pokusí `internetbeacon.msedge.net`testovat přes test. Je k dispozici v roce 2012 +. Pro 2008 R2 použijte `Get-WmiObject` jako v druhém příkladu.
### <a name="port-ping"></a>Příkazy pro odeslání portu
...
`Test-NetConnection` je k dispozici v roce 2012 +. Pro 2008 R2 použijte `Net.Sockets.TcpClient`
### <a name="test-dns-name-resolution"></a>Testování překladu názvů DNS
...
`Resolve-DnsName` je k dispozici v roce 2012 +. Pro 2008 R2 použijte `System.Net.DNS`.
...
`Get-NetFirewallPortFilter` je k dispozici v roce 2012 +. Pro 2008 R2 použijte objekt COM `hnetcfg.fwpolicy2`.
### <a name="disable-windows-firewall"></a>Zakázat bránu Windows Firewall
`Set-NetFirewallProfile -Profile Domain,Public,Private -Enabled False`

`Set-NetFirewallProfile` je k dispozici v roce 2012 +. Pro 2008 R2 použijte `netsh advfirewall`, jak je odkazováno v části CMD výše.
## <a name="manage-users-and-groups"></a>Správa uživatelů a skupin
### <a name="create-local-user-account"></a>Vytvořit místní uživatelský účet
`new-localuser <name>`
### <a name="verify-user-account-is-enabled"></a>Ověřte, že je povolený uživatelský účet.
...
`Get-LocalUser` je k dispozici v roce 2012 +. Pro 2008 R2 použijte `Get-WmiObject`. V tomto příkladu se zobrazuje integrovaný účet místního správce, který má vždycky `S-1-5-21-*-500`SID. Virtuální počítače Azure vytvořené z generalizované image budou mít účet místního správce přejmenovaný na název zadaný při zřizování virtuálních počítačů. Proto se obvykle nebude `Administrator`.
### <a name="add-local-user-to-local-group"></a>Přidat místního uživatele do místní skupiny
`add-localgroupmember -group Administrators -member <username>`
### <a name="enable-local-user-account"></a>Povolit místní uživatelský účet
`get-localuser | where {$_.SID -like "S-1-5-21-*-500"} | enable-localuser`

Tento příklad povoluje integrovaný účet místního správce, který má vždycky `S-1-5-21-*-500`SID. Virtuální počítače Azure vytvořené z generalizované image budou mít účet místního správce přejmenovaný na název zadaný při zřizování virtuálních počítačů. Proto se obvykle nebude `Administrator`.
### <a name="view-user-account-properties"></a>Zobrazení vlastností uživatelského účtu
...
`Get-LocalUser` je k dispozici v roce 2012 +. Pro 2008 R2 použijte `Get-WmiObject`. V tomto příkladu se zobrazuje integrovaný účet místního správce, který má vždycky `S-1-5-21-*-500`SID.
### <a name="view-local-groups"></a>Zobrazit místní skupiny
`(get-localgroup).name | sort` `(get-wmiobject win32_group).Name | sort`

`Get-LocalUser` je k dispozici v roce 2012 +. Pro 2008 R2 použijte `Get-WmiObject`.
## <a name="manage-the-windows-event-log"></a>Správa protokolu událostí systému Windows
### <a name="query-event-log-errors"></a>Chyby protokolu událostí dotazu
...
Tento příklad vytvoří složku `c:\bin` a potom stáhne a extrahuje sadu nástrojů na webu Sysinternals do `c:\bin`.
## <a name="miscellaneous-tasks"></a>Různé úlohy
### <a name="show-os-version"></a>Zobrazit verzi operačního systému
...
Přidání `-force` vynutí, aby se spuštěné aplikace zavřely bez upozornění uživatele.
