...
- **ApplicationId**: Zadejte jedinečný identifikátor aplikace AAD, kterou vytvoříte se správnými oprávněními pro generování kanálů v ADF. Další informace najdete v tématu [Vytvoření aplikace a instančního objektu služby AAD pomocí Azure Portal](https://docs.microsoft.com/azure/active-directory/develop/howto-create-service-principal-portal) článku.
...
- **ResourceGroup**: Zadejte název skupiny prostředků Azure, ve které se vytvořil ADF.

- **DataFactory**:  Zadejte název vašeho ADF, ve kterém se generují jedinečné kanály se spuštěnou aktivitou balíčku SSIS v nich založené na hodnotách možností, které jsou k dispozici při vyvolání **AzureDTExec**.

- **IRName**: Zadejte název Azure-SSIS IR do ADF, na kterém se při vyvolání **AzureDTExec** spustí balíčky zadané v cestě UNC (Universal Naming Convention).

- **PackageAccessDomain**: Zadejte přihlašovací údaje domény pro přístup k balíčkům v cestě UNC zadané při vyvolání **AzureDTExec**.

- **PackageAccessUserName**:  Zadejte přihlašovací údaje uživatelského jména pro přístup k balíčkům v cestě UNC zadané při volání **AzureDTExec**.
...
- **LogLevel**:  Zadejte vybraný rozsah protokolování z předdefinovaných možností**úplného podrobného**/**výkonu** s **hodnotou null**//pro spuštění balíčku na Azure-SSIS IR.

- **LogAccessDomain**: Zadejte přihlašovací údaje domény pro přístup do složky protokolu v cestě UNC při zápisu souborů protokolu, které jsou požadovány, když je zadán parametr **logPath** a **LogLevel** není **null**.

- **LogAccessUserName**: Zadejte přihlašovací údaje uživatelského jména pro přístup ke složce protokolů v cestě UNC při zápisu souborů protokolu, které jsou povinné, když je zadaný **logPath** a **LogLevel** není **null**.

- **LogAccessPassword**: Zadejte přihlašovací údaje hesla pro přístup ke složce protokolů v cestě UNC při zápisu souborů protokolu, které jsou povinné, když je zadaný **logPath** a **LogLevel** není **null**.

- **PipelineNameHashStrLen**: Zadejte délku řetězců hash, které mají být generovány z hodnot možností, které poskytnete při volání **AzureDTExec**.  Řetězce se použijí k vytvoření jedinečných názvů pro kanály ADF, na kterých běží vaše balíčky na Azure-SSIS IR.  Postačuje délka 32 znaků.
...
**AzureDTExec** můžete vyvolat na příkazovém řádku a zadat relevantní hodnoty pro konkrétní možnosti ve scénáři použití, například:
...
- **/Set**: Přepíše konfiguraci parametru, proměnné, vlastnosti, kontejneru, zprostředkovatele protokolů, enumerátoru foreach nebo připojení v balíčku.  Tuto možnost lze zadat vícekrát.  Jako hodnotu této možnosti můžete zadat následující: `property_path;value`, `\package.variables[counter].Value;1` například přepsat hodnotu `counter` proměnné jako 1.  Průvodce konfigurací balíčku můžete použít k vyhledání, zkopírování a vložení hodnoty `property_path` pro položky v balíčku, jejichž hodnota se má přepsat. Další informace najdete v dokumentaci [Průvodce konfigurací balíčku](https://docs.microsoft.com/sql/integration-services/package-configuration-wizard-ui-reference?view=sql-server-2014) .

- **/De [šifrovaný]** : Nastaví dešifrovací heslo balíčku, který je nakonfigurovaný s úrovní ochrany **EncryptAllWithPassword**/**EncryptSensitiveWithPassword** .

## <a name="next-steps"></a>Další kroky
Po vygenerování a spuštění jedinečných kanálů s SSIS aktivitou balíčku v nich, které se budou spouštět po vyvolání **AzureDTExec**, se dají upravit nebo znovu spustit na portálu ADF. Další informace najdete v článku [spuštění balíčků SSIS jako aktivity ADF](https://docs.microsoft.com/azure/data-factory/how-to-invoke-ssis-package-ssis-activity) .
