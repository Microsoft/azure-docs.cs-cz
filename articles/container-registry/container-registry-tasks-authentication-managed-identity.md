...
### <a name="1-optional-create-a-user-assigned-identity"></a>1. Volitelné Vytvoření uživatelsky přiřazené identity
...
### <a name="2-enable-identity-on-an-acr-task"></a>2. Povolení identity u ACR úlohy
...
Pokud chcete povolit identitu přiřazenou systémem, předejte `--assign-identity` bez hodnoty nebo. `assign-identity [system]` Následující příkaz vytvoří úlohu Linux z veřejného úložiště GitHub, které vytvoří `hello-world` Image pomocí triggeru potvrzení Git a pomocí spravované identity přiřazené systémem:
...
Pokud chcete povolit uživatelsky přiřazenou identitu, `--assign-identity` předejte jí hodnotu *ID prostředku* identity. Následující příkaz vytvoří úlohu Linux z veřejného úložiště GitHub, které vytvoří `hello-world` Image pomocí triggeru potvrzení Git a pomocí spravované identity přiřazené uživatelem:
...
### <a name="3-grant-the-identity-permissions-to-access-other-azure-resources"></a>3. Udělení oprávnění identity pro přístup k jiným prostředkům Azure
...
### <a name="4-optional-add-credentials-to-the-task"></a>4. Volitelné Přidat pověření k úkolu

Pokud úloha vyžádá nebo nahraje image do jiného služby Azure Container Registry, přidejte přihlašovací údaje k úloze pro ověření identity. Spuštěním příkazu [AZ ACR Task Credential Add přidejte][az-acr-task-credential-add] do úlohy přihlašovací `--use-identity` údaje identity a předejte parametr. 
