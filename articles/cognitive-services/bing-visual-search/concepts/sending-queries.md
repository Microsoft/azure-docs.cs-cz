...
Následují parametry dotazu, které váš požadavek musí obsahovat. Měli byste aspoň zadat parametr `mkt` dotazu:
...
Následují hlavičky dotazu, které váš požadavek musí obsahovat. Záhlaví `Content-Type` a `Ocp-Apim-Subscription-Key` jsou jediná požadovaná záhlaví, ale měli byste také `User-Agent`zahrnout, `X-MSEdge-ClientID` `X-MSEdge-ClientIP`, a. `X-Search-Location`
...
Pokud image obsahuje rozpoznanou entitu, například kulturní známou nebo oblíbenou osobu, místo nebo věc, může jedna z značek zahrnovat přehled entit. Pole `mainEntity` a `data` jsou k dispozici pouze v `enableEntityData` případě, že `Content-Type` atribut v hlavičce je `true`nastaven na hodnotu.
