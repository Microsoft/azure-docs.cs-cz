...
Následující tabulka popisuje vlastnosti konfigurace vazby, které jste nastavili v souboru *Function. JSON* . Pro knihovny tříd jazyka C# nejsou k dispozici žádné vlastnosti atributů, které by odpovídaly těmto vlastnostem *Functions. JSON* .

|Vlastnost  |Popis  |
|---------|---------|
| **textový** |Musí být nastaven na `http`hodnotu. |
| **direction** | Musí být nastaven na `out`hodnotu. |
| **Jméno** | Název proměnné použitý v kódu funkce pro odpověď nebo `$return` pro použití návratové hodnoty. |

## <a name="usage"></a>Využití

K odeslání odpovědi HTTP použijte standardní vzory odezvy jazyka. V jazyce C# nebo C#, nastavte návratový typ `IActionResult` funkce nebo. `Task<IActionResult>` V jazyce C# není atribut návratové hodnoty požadován.
...
|---------|---------|---------| 
| customHeaders|Žádná|Umožňuje nastavit vlastní hlavičky v odpovědi HTTP. Předchozí příklad přidá `X-Content-Type-Options` hlavičku k odpovědi, aby nedocházelo ke sledování obsahu typu obsahu. |
