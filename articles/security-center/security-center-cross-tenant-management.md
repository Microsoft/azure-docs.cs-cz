...
- Bezpečnostní týmy organizací s více klienty můžou zobrazit a spravovat stav zabezpečení z jednoho místa.
  ...
  ![Správa mezi klienty](./media/security-center-cross-tenant-management/cross-tenant-security-center.png)
...
- **Spravovat zásady zabezpečení**: Z jednoho zobrazení spravujte stav zabezpečení mnoha prostředků pomocí [zásad](tutorial-security-policy.md), provedete akce s doporučeními zabezpečení a shromáždíte a spravujete data související se zabezpečením. Například na následujícím obrázku jsou čtyři adresáře klientů (Doug. Lora vytvořené, Microsoft, SSO, Wilde Company) a každý tenant má předplatná. Vybraná předplatná z různých tenantů jsou uvedená v seznamu správy zásad.

     ![Správa zásad mezi klienty](./media/security-center-cross-tenant-management/cross-tenant-policy.png)

- **Vylepšete zabezpečené skóre a stav dodržování předpisů**: Díky viditelnosti mezi klienty můžete zobrazit celkové stav zabezpečení všech vašich klientů a kde a jak nejlépe vylepšit [bezpečné skóre](security-center-secure-score.md) a [dodržování předpisů stav](security-center-compliance-dashboard.md) pro každý z nich, jak je vidět na následujícím obrázku.

     ![Bezpečnostní skóre](./media/security-center-cross-tenant-management/cross-tenant-secure-score.png)

- **Opravit doporučení**: Monitorujte a opravte [doporučení](security-center-recommendations.md) pro mnoho prostředků z různých tenantů najednou (jak je vidět na následujícím obrázku). Pak můžete okamžitě řešit ohrožení zabezpečení, která představují nejvyšší riziko ve všech klientech.

  ![Správa doporučení mezi klienty](./media/security-center-cross-tenant-management/cross-tenant-recommendation.png)
