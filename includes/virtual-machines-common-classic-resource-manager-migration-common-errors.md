...
| Migrace se pro nasazení {název_nasazení} v hostované službě {název_hostované_služby} nepovoluje, protože není v místním oboru. Naleznete http://aka.ms/regionalscope pro přesun toto nasazení na místní obor. | V roce 2014 byl pro Azure oznámen přesun síťových prostředků z oboru na úrovni serveru do místního oboru. V tématu [http://aka.ms/regionalscope] Další podrobnosti (http://aka.ms/regionalscope). K této chybě dochází v případě, že u migrovaného nasazení neproběhla operace aktualizace, která by ho automaticky přesunula do místního oboru. Nejlepším alternativním řešením je přidat do virtuálního počítače koncový bod nebo datový disk a potom opakovat migraci. <br> Viz [Postup nastavení koncových bodů na klasickém virtuálním počítači s Windows v Azure](../articles/virtual-machines/windows/classic/setup-endpoints.md#create-an-endpoint) nebo [Připojení datového disku k virtuálnímu počítači s Windows vytvořenému pomocí modelu nasazení Classic](../articles/virtual-machines/windows/classic/attach-disk.md).|
| Migrace není podporovaná pro virtuální síť {název virtuální sítě}, protože obsahuje nasazení PaaS není brány. | K této chybě dojde, když máte nasazení PaaS není brány jako aplikační bránou nebo API Management services, které jsou připojené k virtuální síti.|
