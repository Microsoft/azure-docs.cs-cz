---
title: Připojit data Cisco k Azure Sentinel Preview | Microsoft Docs
...
ms.locfileid: "68679331"
---
# <a name="connect-your-cisco-asa-appliance"></a>Připojení zařízení Cisco ASA 
...
Můžete připojit Azure Sentinel k jakémukoli zařízení Cisco ASA. Cisco ASA je nativně integrovaný s Azure Sentinel pro příjem dat, takže i když vaše zařízení Cisco neukládá protokoly jako CEF, Azure Sentinel je ingestuje stejným způsobem jako v případě, že zpracovává protokoly CEF. Integrace s Azure Sentinel umožňuje snadno spouštět analýzy a dotazy napříč daty souboru protokolu z Cisco ASA. 

> [!NOTE]
> Data budou uložená v geografickém umístění pracovního prostoru, na kterém běží Azure Sentinel.

## <a name="step-1-connect-your-cisco-asa-appliance-using-an-agent"></a>Krok 1: Připojení zařízení Cisco ASA pomocí agenta

Pokud chcete připojit zařízení Cisco ASA ke službě Azure Sentinel, musíte agenta nasadit na vyhrazený počítač (virtuální počítač nebo místně), aby podporoval komunikaci mezi zařízením a službou Azure Sentinel. 
...
Postup zobrazení síťového diagramu obou možností najdete v tématu [připojení zdrojů dat](connect-data-sources.md).

### <a name="deploy-the-agent-on-your-machine"></a>Nasazení agenta na svém počítači

1. Na portálu Sentinel Azure klikněte na **datové konektory** a vyberte **Cisco ASA** a pak **otevřete stránku konektor**. 
...
## <a name="step-2-forward-cisco-asa-logs-to-the-syslog-agent"></a>Krok 2: Předejte protokoly Cisco ASA do agenta syslog.
...
Přejděte na [Odeslat zprávy syslog na externí server SYSLOG](https://aka.ms/asi-syslog-cisco-forwarding)a postupujte podle pokynů pro nastavení připojení. Po zobrazení výzvy použijte tyto parametry:
- Nastavte **port** na 514 nebo na port, který jste nastavili v agentovi.
- Nastavte **syslog_ip** na IP adresu agenta.
- Nastavte **Protokolovací zařízení** na zařízení, které jste nastavili v agentovi. Ve výchozím nastavení agent nastaví zařízení na 4.

Pokud chcete použít příslušné schéma v Log Analytics pro události Cisco, vyhledejte `CommonSecurityLog`.
