...
1. Po stažení všechny soubory .zip rozzipujte.
...
1. Vytvořte nový kontejner **data-files**, ve kterém se budou ukládat datové soubory projektu **BikeShare**.

1. Nahrajte datové soubory. Soubor `BostonWeather.csv` nahrajte do složky `weather`. Soubory dat o jízdách nahrajte do složky `tripdata`.
...
1. Data programu __Hubway__ jsou uspořádána do souborů podle roků a měsíců. Například soubor s názvem `201501-hubway-tripdata.zip` obsahuje soubor .csv s daty z ledna 2015. Data obsahují následující pole, přičemž každý řádek představuje jednu jízdu na kole:
...
1. Vytvořte nový projekt služby Machine Learning. Vyberte tlačítko **+** na stránce **Projects** (Projekty) nebo vyberte **File** (Soubor) > **New** (Nový).
...
1. Přidejte zdroj dat. Vyberte ikonu **+** a pak vyberte **Add Data Source** (Přidat zdroj dat).
...
1. **File Selection** (Výběr souboru): Přidejte data o počasí. Procházením vyberte soubor `BostonWeather.csv`, který jste dříve nahráli do úložiště objektů blob. Vyberte **Další**.

   ![Výběr souboru s vybraným souborem BostonWeather.csv](media/tutorial-bikeshare-dataprep/azureblobpickweatherdatafile.png)

1. **File Details** (Podrobnosti souboru): Zkontrolujte rozpoznané schéma souboru. Aplikace Machine Learning Workbench analyzuje data v souboru a odvodí z nich schéma, které se má použít.
...
1. **Data Types** (Datové typy): Zkontrolujte automaticky rozpoznané datové typy. Aplikace Machine Learning Workbench analyzuje data v souboru a odvodí z nich datové typy, které se mají použít.
...
1. **Sampling** (Vzorkování): Vyberte **Edit** (Upravit) a vytvořte schéma vzorkování. Vyberte nový přidaný řádek __Top 10000__ (Nejvyšších 10 000) a pak vyberte __Edit__ (Upravit). V poli __Sample Strategy__ (Strategie vzorku) nastavte hodnotu **Full File** (Celý soubor) a pak vyberte **Apply** (Použít).
...
1. **Path Column** (Sloupec s cestou): Část __Path Column__ (Sloupec s cestou) použijte k zahrnutí úplné cesty k souboru jako sloupce v importovaných datech. Vyberte __Do Not Include Path Column__ (Nezahrnovat sloupec s cestou).
...
1. **Finish** (Dokončit): Vytváření zdroje dat dokončíte výběrem možnosti **Finish** (Dokončit).
...
1. Po zobrazení výzvy zadejte název balíčku pro přípravu dat, například **BikeShare Data Prep**. 

1. Pokračujte výběrem tlačítka __OK__.

   ![Dialogové okno Prepare (Příprava)](media/tutorial-bikeshare-dataprep/dataprepdialog.png)

1. V části __Data Preparation__ (Příprava dat) na kartě __Data__ se zobrazí nový balíček s názvem **BikeShare Data Prep**. 

   Výběrem této položky balíček zobrazíte. 

1. Výběrem tlačítka **>>** rozbalte seznam __Dataflows__ (Toky dat) a zobrazte toky dat, které balíček obsahuje. V tomto příkladu je to jediný tok dat __BostonWeather__.
...
1. Pro účely tohoto kurzu vyberte buňku obsahující hodnotu `FM-15`. Pak nastavte filtr na **equals** (rovná se).  Teď budou data filtrována tak, aby vracela jen řádky, které mají v poli __REPORTTYPE__ (Typ zprávy) hodnotu `FM-15`.
...
1. Klikněte pravým tlačítkem na jedno z vybraných záhlaví sloupců a pak vyberte **Convert Field Type to Numeric** (Převést typ pole na číselný). Tato možnost převede datový typ sloupců na číselný.

   ![Převod více sloupců na číselný datový typ](media/tutorial-bikeshare-dataprep/weatherconverttonumeric.png)

1. Odfiltrování chybových hodnot U některých sloupců můžou při převodu datového typu vzniknout určité problémy. Tyto problémy jsou označeny červenou barvou na panelu __Data Quality Bar__ (Panel kvality dat) příslušného sloupce.
...
1. Abyste odebrali zbývající řádky s chybami v ostatních sloupcích, opakujte tento postup filtrování u sloupců **HOURLYRelativeHumidity** (Hodinová relativní vlhkost) a **HOURLYWindSpeed** (Hodinová rychlost větru).
...
1. Aplikace Machine Learning Workbench automaticky identifikuje smysluplný oddělovač a vytvoří dva sloupce rozdělením dat na hodnoty data a času. 

1. Výběrem tlačítka __OK__ přijměte výsledky operace rozdělení.
...
1. V tomto novém sloupci vyberte první prázdnou buňku. Uveďte příklad požadovaného časového rozmezí tím, že do nového sloupce zadáte **12AM-2AM** (0:00–2:00), a pak stisknete Enter.
...
1. Pokud chcete zkontrolovat navrhované změny, vyberte odkaz **Review next suggested row** (Zkontrolovat další navržený řádek). Buňka, kterou byste měli zkontrolovat a v případě potřeby opravit, je na obrazovce zvýrazněna.
...
1. Tím se vrátíte do zobrazení mřížky dat __BostonWeather__. Mřížka teď obsahuje tři sloupce, které jste přidali v předchozích krocích.
...
1. Sloupec přejmenujte dvojím kliknutím na záhlaví sloupce a zadáním názvu **Hour Range** (Hodinové rozmezí). Stisknutím klávesy Enter změnu uložte.

   ![Přejmenování sloupce](media/tutorial-bikeshare-dataprep/weatherhourrangecolumnrename.png)

1. Abyste odvodili rozmezí data a času, vyberte současně sloupce **Date\_1** (Datum_1) a **Hour Range** (Hodinové rozmezí), klikněte pravým tlačítkem a pak vyberte **Derive Column by Example** (Odvodit sloupec podle příkladu).
...
1. Počkejte, než se stav změní z **Analyzing Data** (Analyzují se data) na **Review next suggested row** (Zkontrolovat další navržený řádek). Tato změna může trvat několik sekund. Výběrem odkazu stavu přejděte na navrhovaný řádek. 
...
1. Výběrem tlačítka **OK** transformaci přijměte.
...
1. Abyste mohli sloupec přejmenovat, poklikejte na jeho záhlaví. Změňte název na **Date Hour Range** (Hodinové rozmezí k datu) a pak stiskněte Enter.

1. Vyberte současně sloupce **DATE** (Datum), **DATE\_1** (Datum_1), **DATE\_2** (Datum_2) a **Hour Range** (Hodinové rozmezí). Klikněte pravým tlačítkem a pak vyberte **Remove column** (Odebrat sloupec).
...
1. Do textového pole, které se zobrazí, zadejte následující kód. Pokud jste používali stejné názvy sloupců, měl by kód fungovat bez úprav. V jazyce Python vytváříte jednoduchou logiku normalizace minimálních a maximálních hodnot.
...
1. Výběrem tlačítka __OK__ skript použijte. Číselné sloupce v mřížce teď obsahují hodnoty v rozmezí 0 až 1.
...
1. Po importu dat začněte připravovat data výběrem možnosti __Prepare__ (Příprava). Vyberte existující balíček **BikeShare Data Prep.dprep** a potom vyberte __OK__.
...
1. Po načtení mřížky rozbalte položku __DATAFLOWS__ (Toky dat). Teď máte dva toky dat: **BostonWeather** a **201701-hubway-tripdata**. Vyberte položku **201701-hubway-tripdata**.
...
1. Mapovou vizualizaci můžete maximalizovat výběrem ikony **Maximize** (Maximalizovat). Pokud chcete mapu přizpůsobit velikosti okna, vyberte ikonu **E** v levé horní části vizualizace.

    ![Obrázek maximalizované mapy](media/tutorial-bikeshare-dataprep/maximizedmap.png)

1. Výběrem tlačítka **Minimize** (Minimalizovat) se vrátíte do zobrazení mřížky.
...
1. V dialogovém okně __Add Column (Script)__ (Přidat sloupec pomocí skriptu) použijte následující hodnoty:
...
1. Výběrem tlačítka __OK__ sloupec **logtripduration** (Logaritmus trvání jízdy) přidejte.

1. Klikněte na sloupec pravým tlačítkem a vyberte **Histogram**.
...
1. V dialogovém okně __Edit__ (Upravit) použijte následující hodnoty:
...
1. Výběrem tlačítka __OK__ filtr použijte.
...
1. Pokud chcete haló efekt odebrat, vyberte __Edit__ (Upravit) a zrušte zaškrtnutí políčka __Show halo__ (Zobrazit haló efekt).

    ![Možnosti histogramu](media/tutorial-bikeshare-dataprep/uncheckhalo.png)

1. Výběrem **OK** zakažte haló efekt. Potom histogram minimalizujte.
...
1. Do prvního řádku zadejte jako příklad hodnotu **Jan 01, 2017 12AM-2AM** (1. ledna 2017, 0:00–2:00).
...
1. Počkejte, než aplikace vypočítá hodnoty pro všechny řádky. Tento proces může trvat několik sekund. Po dokončení analýzy zkontrolujte data pomocí odkazu __Review next suggested row__ (Zkontrolovat další navržený řádek).
...
1. Výběrem tlačítka **OK** transformaci přijměte. Přejmenujte nově vytvořený sloupec na **Date Hour Range** (Hodinové rozmezí k datu).

    ![Přejmenovaný sloupec](media/tutorial-bikeshare-dataprep/tripdatasummarize.png)

1. Klikněte pravým tlačítkem na záhlaví sloupce **starttime** (Čas začátku jízdy) a vyberte **Remove column** (Odebrat sloupec).

1. Data můžete sumarizovat výběrem možnosti __Summarize__ (Sumarizovat) v nabídce __Transform__ (Transformace). Transformaci vytvoříte pomocí následujících kroků:
...
1. Výběrem tlačítka **OK** přijměte výsledek sumarizace.
...
1. V části __Tables__ (Tabulky) vyberte **BostonWeather** jako **Left** (Levý) tok dat a **201701-hubway-tripdata** jako **Right** (Pravý) tok dat. Pokračujte výběrem tlačítka **Next** (Další).

    ![Výběry tabulek](media/tutorial-bikeshare-dataprep/jointableselection.png)

1. V části __Key Columns__ (Klíčové sloupce) vyberte u obou tabulek sloupec **Date Hour Range** (Hodinové rozmezí k datu) a pak vyberte __Next__ (Další).

    ![Výběry klíčových sloupců](media/tutorial-bikeshare-dataprep/joinkeyselection.png)

1. V části __Join Type__ (Typ spojení) vyberte jako typ spojení možnost __Matching rows__ (Odpovídající řádky) a pak vyberte __Finish__ (Dokončit).
...
1. Když chcete vytvořit sloupec, který bude obsahovat časový interval z příslušného řádku, klikněte pravým tlačítkem na sloupec **Date Hour Range** (Hodinové rozmezí k datu) a vyberte **Derive Column by Example** (Odvodit sloupec podle příkladu). U řádku, který obsahuje hodnotu **Jan 01, 2017 12AM-2AM** (1. ledna 2017, 0:00–2:00), použijte hodnotu **12AM-2AM** (0:00–2:00). Stiskněte Enter a pak vyberte **OK**. Přejmenujte tento sloupec na **Period** (Časový interval).

    ![Sloupec Period (Časový interval)](media/tutorial-bikeshare-dataprep/featurehourrange.png)

1. Pokud chcete odebrat sloupce **Date Hour Range** (Hodinové rozmezí k datu) a **r_Date Hour Range** (r_Hodinové rozmezí k datu), stiskněte klávesu Ctrl (na Macu klávesu Command ⌘) a pak vyberte jednotlivá záhlaví sloupců. Klikněte pravým tlačítkem a vyberte **Remove column** (Odebrat sloupec).
...
1. Aktualizujte skript v jazyce Python v souboru `BikeShare Data Prep.py` pomocí následujícího kódu:
...
1. Nahraďte `Your Azure Storage blob path` cestou k výstupnímu souboru, který se má vytvořit. Nahraďte proměnné `blobfolder` a `csvfiles`.
...
1. Pomocí příkazového řádku se přihlaste k Azure. 
...
1. Vytvořte konfiguraci spuštění služby HDInsight. Potřebujete název vašeho clusteru a heslo uživatele `sshuser`.
...
1. V rozevíracím seznamu vyberte **hdinsight**, aby se váš skript spustil v clusteru HDInsight.

1. Vyberte **Run** (Spustit). Skript se odešle jako úloha. Po zapsání souboru do zadaného umístění ve vašem kontejneru úložiště se stav úlohy změní na __Completed__(Dokončeno).
...
1. Vyberte ikonu složky, abyste si mohli prohlédnout soubory v projektu. Rozbalte adresář __aml\_config__ a pak vyberte soubor `hdinsight.runconfig`.

    ![Umístění souboru hdinsight.runconfig](media/tutorial-bikeshare-dataprep/hdinsightsubstitutedatasources.png) 

1. Výběrem tlačítka **Edit** (Upravit) otevřete soubor ve Visual Studio Code.

1. Na konec souboru `hdinsight.runconfig` přidejte následující řádky a potom soubor uložte výběrem ikony disku.
...
1. Pro výstup trénovacích dat použijte složku `traindata`.
