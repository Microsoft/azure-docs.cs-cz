...
Pro cílovou datovou sadu musí být nastaven vlastnost `timeseries`, a to zadáním sloupce časového razítka buď ze sloupce v datech, nebo z virtuálního sloupce odvozeného ze vzor cesty souborů. To lze provést prostřednictvím sady Python SDK nebo Azure Machine Learning studia. Pro přidání `timeseries`ho vlastností do datové sady je nutné zadat sloupec, který představuje časové razítko "jemného zrnitku". Pokud jsou vaše data rozdělená do struktury složek s časovými informacemi, jako je například {rrrr/MM/DD}, můžete vytvořit virtuální sloupec pomocí nastavení vzoru cesty a nastavit ho jako časové razítko "hrubých", aby se zlepšila důležitost funkcí časové řady. 

#### <a name="python-sdk"></a>Python SDK

Metoda [`with_timestamp_columns()`](https://docs.microsoft.com/python/api/azureml-core/azureml.data.tabulardataset?view=azure-ml-py#with-timestamp-columns-fine-grain-timestamp--coarse-grain-timestamp-none--validate-false-) třídy [`Dataset`](https://docs.microsoft.com/python/api/azureml-core/azureml.data.tabulardataset?view=azure-ml-py#with-timestamp-columns-fine-grain-timestamp--coarse-grain-timestamp-none--validate-false-) definuje sloupec časového razítka pro datovou sadu. 
...
[Formát ![ho oddílu](./media/how-to-monitor-datasets/partition-format.png)](media/how-to-monitor-datasets/partition-format-expand.png)
...
Úplný příklad nastavení `timeseries` datové sady a detektoru pro přenos dat najdete na našem [ukázkovém poznámkovém bloku](https://aka.ms/datadrift-notebook).
...
Následující obrázek představuje příklad grafů zobrazených Azure Machine Learning ve službě disNOAA **Studio, které** vyplývají z výplně [integrovaných dat Surface](https://azure.microsoft.com/services/open-datasets/catalog/noaa-integrated-surface-data/). Data byla Navzorkovaná na `stationName contains 'FLORIDA'`, přičemž se jako základní datová sada a všechna 2019 data použitá jako cíl používají leden 2019.
...
Tyto metriky lze také načíst v sadě Python SDK prostřednictvím metody `get_metrics()` u objektu `DataDriftDetector`. 
...
[Přehled ![Azure Portal](./media/how-to-monitor-datasets/ap-overview.png)](media/how-to-monitor-datasets/ap-overview-expanded.png)
