...
| Vyžadovat přiřazení uživatele  | Ve výchozím nastavení mají uživatelé přístup k podnikovým aplikacím bez jejich přiřazení. Pokud však aplikace zveřejňuje role nebo pokud chcete, aby se aplikace zobrazila na přístupovém panelu uživatele, vyžadovat přiřazení uživatele. (Další informace najdete v tématu [pokyny pro vývojáře při integraci aplikací](developer-guidance-for-integrating-applications.md).)  | 
| Nasazení přístupového panelu Moje aplikace do vašich uživatelů | [Přístupový panel](end-user-experiences.md) na adrese `https://myapps.microsoft.com` je webový portál, který poskytuje uživatelům jediný vstupní bod pro své přiřazené cloudové aplikace. Při přidání dalších možností, jako je Správa skupin a Samoobslužné resetování hesla, si uživatelé můžou najít na přístupovém panelu. Viz [Plánování nasazení přístupového panelu](access-panel-deployment-plan.md).
