---
title: Získejte podrobnosti pomocí nástroje pro vytváření snímků konzistentního pro aplikace Azure pro Azure NetApp Files | Microsoft Docs
description: Poskytuje návod pro spuštění příkazu Details pro nástroj snímek konzistentní vzhledem k aplikacím Azure, který můžete použít s Azure NetApp Files.
...
ms.locfileid: "104865294"
---
# <a name="obtain-details-using-azure-application-consistent-snapshot-tool-preview"></a>Získání podrobností pomocí nástroje pro vytváření snímků konzistentního vzhledem k aplikacím Azure (Preview)

V tomto článku najdete návod, jak spustit příkaz s podrobnostmi o nástroji pro snímek konzistentní vzhledem k aplikacím Azure, který můžete použít s Azure NetApp Files.

## <a name="introduction"></a>Úvod

`azacsnap -c details`Příkaz zobrazí seznam podrobností snímků (název svazku, název snímku, čas vytvoření, komentáře, velikost snímku) uložený na všech svazcích konfiguračního souboru.  Výstup konzoly lze vložit do tabulky pro další analýzu.

Kvůli informacím uloženým v systémech back-end se výstup příkazu mírně liší při spuštění ve **velké instanci Azure** a **Azure NetApp Files**.

## <a name="command-options"></a>Možnosti příkazu

`-c details`Příkaz má následující možnosti:

- `--details snapshots` Obsahuje seznam základních podrobností o snímcích pro každý svazek, který byl nakonfigurován. Tento příkaz lze spustit na primárním serveru nebo na serveru v umístění pro zotavení po havárii. Příkaz poskytuje následující informace rozdělené podle každého svazku obsahujícího snímky:
...
- `--details replication` Poskytuje základní podrobnosti o stavu replikace z produkční lokality do lokality pro zotavení po havárii. Příkaz poskytuje výstup, který umožňuje monitorování, aby bylo zajištěno, že proběhne replikace.  Zobrazuje velikost položek, které se replikují. Poskytuje také pokyny v případě, že replikace trvá příliš dlouho nebo pokud je odkaz mimo provoz.

- `[--configfile <config filename>]` je volitelný parametr, který umožňuje názvy vlastních konfiguračních souborů.

### <a name="output-of-the-azacsnap--c-details---details-snapshots-command"></a>Výstup `azacsnap -c details --details snapshots` příkazu

Níže uvedený příklad byl proveden na **velké instanci Azure** a výstup byl zkrácen pro zkrácení.  Také si všimněte, že některé řádky byly pravděpodobně zabaleny, aby odpovídaly výstupu.
...
> [!NOTE]
> Tento příklad ukazuje výstup pro snímky spouštěné pomocí předchozí verze (v 4.3) a snímků pořízených s nejnovější verzí (5,0).

Následující příklad obsahuje výstup pro **Azure NetApp Files**. Poznamenejte si omezené informace pomocí tohoto příkazu, protože tyto informace služba nezveřejňuje.
...
### <a name="output-of-the-azacsnap--c-details---details-replication-command"></a>Výstup `azacsnap -c details --details replication` příkazu

Tento příkaz zkontroluje stav replikace úložiště z primární lokality do umístění DR a *musí* se spustit na **serveru lokality Dr**. Tento příkaz kontroluje **pouze** svazky v konfiguračním souboru.

> [!NOTE]
> Tato možnost je dostupná jenom v systémech HLI ( **Azure large instance** ).
...
Následující příklad má porušený stav replikace a v tomto scénáři by pravděpodobně aktivace programu DR způsobila nekompletní data na webu DR.

> [!NOTE]
> Všimněte si upozornění v následujícím příkladu o replikaci přerušenou pro dva svazky.
...
Tento příklad má úspěšnou replikaci mezi primární lokalitou a lokalitou DR, takže tyto systémy jsou připravené na podporu scénáře zotavení po havárii.
