...
## <a name ="enablebgp"></a>Povolení protokolu BGP pro bránu VPN
...
#### <a name="Login"></a>1. Připojte se k předplatnému
...
Vytvořte bránu virtuální sítě pro virtuální síť TestVNet1. Protokol BGP vyžaduje bránu sítě VPN založenou na trasách. K nastavení čísla autonomního systému (ASN) pro virtuální sítě testvnet1 potřebujete také další parametr `-Asn`. Vytvoření brány může trvat až (45 minut). 

Pokud tento příkaz spustíte pomocí parametru `--no-wait`, nezobrazí se žádná zpětná vazba ani výstup. Parametr `--no-wait` umožňuje vytvořit bránu na pozadí. Neznamená to, že se Brána VPN vytvoří hned.
...
Spusťte následující příkaz a podívejte se na část `bgpSettings` v horní části výstupu:
...
V tomto kroku vytvoříte připojení z virtuální sítě testvnet1 k site5. Je nutné zadat parametr `--enable-bgp` pro povolení protokolu BGP pro toto připojení. 
...
Zkopírujte hodnoty po `"id":` do textového editoru, jako je například Poznámkový blok, abyste je mohli snadno vložit při vytváření připojení. 
...
## <a name ="v2vbgp"></a>Navázání připojení typu VNet-to-VNet k protokolu BGP
...
V tomto příkladu virtuální sítě patří do stejného předplatného. Mezi různými předplatnými můžete nastavit připojení VNet-to-VNet. Další informace najdete v tématu [Konfigurace připojení typu VNet-to-VNet](vpn-gateway-howto-vnet-vnet-cli.md). Ujistěte se, že při vytváření připojení k povolení protokolu BGP přidáte `-EnableBgp $True`.
...
V tomto kroku vytvoříte připojení z virtuální sítě testvnet1 k site5. Chcete-li pro toto připojení povolit protokol BGP, je nutné zadat parametr `--enable-bgp`.
