...
Chyby a známé problémy najdete v tématu [známé problémy](sql-database-release-notes.md#known-issues).

## <a name="new-features"></a>Nové funkce

**Kde můžu najít nejnovější funkce a funkce ve verzi Public Preview?**

Nové funkce a verze Preview najdete v [poznámkách k verzi](sql-database-release-notes.md?tabs=managed-instance).
...
Výchozí zóna DNS spravované instance *. Database.Windows.NET* může být změněna. 
...
**Můžu se ke spravované instanci připojit pomocí IP adresy?**

Připojení ke spravované instanci pomocí IP adresy není podporováno. Název hostitele spravované instance se mapuje na nástroj pro vyrovnávání zatížení před virtuálním clusterem spravované instance. Protože jeden virtuální cluster by mohl hostovat více připojení ke spravovaným instancím, nebylo možné směrovat do správné spravované instance bez zadání jeho názvu.

Další informace o architektuře virtuálních clusterů spravované instance najdete v tématu [Architektura připojení virtuálních clusterů](sql-database-managed-instance-connectivity-architecture.md#virtual-cluster-connectivity-architecture).
