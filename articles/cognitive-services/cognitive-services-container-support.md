...
ms.locfileid: "51634982"
---
# <a name="container-support-in-azure-cognitive-services"></a>Podpora kontejnerů ve službě Azure Cognitive Services

Podpora kontejnerů ve službě Azure Cognitive Services umožňuje vývojářům používat stejná bohatá rozhraní API, které jsou dostupné v Azure, ale Díky flexibilitě, který je součástí [kontejnery Dockeru](https://www.docker.com/what-container). Podpora kontejnerů je aktuálně dostupná ve verzi preview pro podmnožinu služeb Azure Cognitive Services, včetně částí [pro počítačové zpracování obrazu](Computer-vision/Home.md), [pro rozpoznávání tváře](Face/Overview.md), a [rozhraní Text Analytics](text-analytics/overview.md).
...
Podívejte se na to [krátkém videu](https://azure.microsoft.com/resources/videos/containers-support-of-cognitive-services) ukázku.

[Pro počítačové zpracování obrazu](Computer-vision/Home.md), [pro rozpoznávání tváře](Face/Overview.md), a [rozhraní Text Analytics](text-analytics/overview.md) služby jsou dostupné na [Microsoft Azure](https://azure.microsoft.com). Přihlaste se [webu Azure portal](https://portal.azure.com/) k vytvoření a prozkoumáte prostředky Azure pro tyto služby.

## <a name="features-and-benefits"></a>Funkce a výhody

- **Kontrola nad daty**: umožňují zákazníkům pomocí služeb Cognitive Services s úplnou kontrolou nad svými daty.  To je nezbytné pro zákazníky, které nelze odesílat data do cloudu, ale přístup k technologie služeb Cognitive Services. Podpora konzistence v hybridním prostředí – napříč daty, správu, identity a zabezpečení.
...
- **Přenosné architektura**: Povolit vytváření architektury přenosné aplikace, která se dá nasadit v cloudu, místní a hraniční. Kontejnery je také možné nasadit přímo do [Azure Kubernetes Service](/azure/aks/), [Azure Container Instances](/azure/container-instances/), nebo [Kubernetes](https://kubernetes.io/) clusteru nasadí do [Azure Zásobník](/azure/azure-stack/). Další informace najdete v tématu [nasazení Kubernetes pro Azure Stack](/azure/azure-stack/user/azure-stack-solution-template-kubernetes-deploy).
- **Vysoká propustnost / nízká latence**: poskytují zákazníkům možnost škálování tím, že služby Cognitive Services spustit ve službě Azure Kubernetes Service fyzicky blízko jejich aplikační logiku a data pro vysokou propustností a nízkou latenci.
