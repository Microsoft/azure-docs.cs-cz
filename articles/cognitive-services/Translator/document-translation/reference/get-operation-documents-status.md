---
title: Stav dokumentů pro operaci získání překladu dokumentů
titleSuffix: Azure Cognitive Services
description: Metoda get Documents status Returns vrátí stav všech dokumentů v požadavku na překlad dokumentu Batch.
...
ms.locfileid: "105612993"
---
# <a name="document-translation-get-operation-documents-status"></a>Překlad dokumentu: stav získání dokumentů operací

Metoda get Documents status Returns vrátí stav všech dokumentů v požadavku na překlad dokumentu Batch.

Dokumenty zahrnuté v odpovědi jsou seřazené podle ID dokumentu v sestupném pořadí. Pokud počet dokumentů v odpovědi překročí limit stránkování, použije se stránkování na straně serveru. Stránkované odpovědi označují částečný výsledek a v odpovědi obsahují token pro pokračování. Absence tokenu pokračování znamená, že nejsou k dispozici žádné další stránky.

parametry dotazu $top a $skip lze použít k určení počtu výsledků, které se mají vrátit, a posunutí kolekce. Server respektuje hodnoty určené klientem. Nicméně klienti musí být připraveni zpracovat odpovědi, které obsahují jinou velikost stránky nebo obsahují token pro pokračování.
...
|400|Neplatný požadavek Ověřte vstupní parametry.|
...
## <a name="get-operation-documents-status-response"></a>Odpověď na stav operace Get Documents

### <a name="successful-get-operation-documents-status-response"></a>Úspěšná operace Get – odpověď stavu dokumentů

Následující informace jsou vráceny v úspěšné odpovědi.

|Název|Typ|Description|
|--- |--- |--- |
|@nextLink|řetězec|Adresa URL další stránky Hodnota null, pokud nejsou k dispozici žádné další stránky.|
|hodnota|DocumentStatusDetail []|Podrobný stav jednotlivých dokumentů uvedených níže.|
|hodnota. cesta|řetězec|Umístění dokumentu nebo složky.|
|Value. createdDateTimeUtc|řetězec|Datum a čas vytvoření operace|
|Value. lastActionDateTimeUt|řetězec|Datum a čas, kdy se stav operace aktualizoval.|
|Value. status|status|Seznam možných stavů pro úlohu nebo dokument.<ul><li>Zrušeno</li><li>Ruší</li><li>Neúspěšný</li><li>NotStarted</li><li>Spuštěno</li><li>Úspěšný</li><li>ValidationFailed</li></ul>|
|value.to|řetězec|Do jazyka.|
|hodnota. průběh|řetězec|Průběh překladu, je-li k dispozici.|
|value.id|řetězec|ID dokumentu|
