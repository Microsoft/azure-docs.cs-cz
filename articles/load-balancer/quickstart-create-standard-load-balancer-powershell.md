...
```
> [!NOTE]
> Výše uvedený příkaz umožňuje odchozí připojení pro prostředky ve fondu back-end nástroje pro vyrovnávání zatížení. Pro pokročilou konfiguraci odchozího připojení, přečtěte si téma **[odchozí připojení v Azure](load-balancer-outbound-connections.md)** a **[nakonfigurujte pravidla vyrovnávání zatížení a odchozí pravidla v Standard Load BALANCER pomocí Azure CLI](configure-load-balancer-outbound-cli.md)**.
...
Pro přístup k virtuálním počítačům pomocí připojení RDP budete potřebovat veřejné IP adresy pro virtuální počítače. 

Pomocí [New-AzPublicIpAddress](/powershell/module/az.network/new-azpublicipaddress) vytvořte standardní veřejné IP adresy pro:
...
Vytvořte skupinu zabezpečení sítě pomocí [New-AzNetworkSecurityRuleConfig](/powershell/module/az.network/new-aznetworksecurityruleconfig):

* S názvem **myNSGRuleRDP**.
* Popis **Povolení protokolu RDP**.
* Přístup k **Povolení**.
* Protokol **TCP**.
* Směr **příchozí**.
* Priorita **1000**.
...
2. Pomocí veřejných IP adres virtuálních počítačů Vytvořte připojení ke vzdálené ploše pomocí **myVM1**, **myVM2**a **myVM3** .
...
Pokud už je nepotřebujete, můžete k odebrání skupiny prostředků, nástroje pro vyrovnávání zatížení a všech prostředků použít příkaz [Remove-AzResourceGroup](/powershell/module/az.resources/remove-azresourcegroup) . 
