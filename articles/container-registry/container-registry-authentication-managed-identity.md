...
V tomto článku se předpokládá, že máte v registru uloženou image kontejneru `aci-helloworld:v1`. V příkladech se používá název registru pro *myContainerRegistry*. Nahraďte vlastními názvy registru a imagí v pozdějších krocích.
...
Nejdřív pomocí identity, kterou jste nakonfigurovali na VIRTUÁLNÍm počítači, proveďte ověření v Azure CLI pomocí [AZ Login][az-login]. V případě `<userID>`nahraďte ID identity, kterou jste získali v předchozím kroku. 

```azurecli
az login --identity --username <userID>
```

Pak proveďte ověření v registru pomocí [AZ ACR Login][az-acr-login]. Při použití tohoto příkazu rozhraní příkazového řádku používá token služby Active Directory vytvořený při spuštění `az login` k bezproblémovému ověření relace pomocí registru kontejnerů. (V závislosti na nastavení virtuálního počítače možná budete muset spustit tento příkaz a příkazy Docker s `sudo`.)

```azurecli
az acr login --name myContainerRegistry
```

Měla by se zobrazit `Login succeeded` zpráva. Pak můžete spouštět příkazy `docker` bez zadání přihlašovacích údajů. Například spusťte [Docker Pull][docker-pull] , aby se načetla `aci-helloworld:v1` image, a určete název přihlašovacího serveru vašeho registru. Název přihlašovacího serveru se skládá z názvu registru kontejneru (všech malých písmen) následovaných `.azurecr.io`, například `mycontainerregistry.azurecr.io`.
...
Pomocí příkazu [AZ VM show][az-vm-show] nastavte proměnnou na hodnotu `principalId` (ID instančního objektu) identity virtuálního počítače, který chcete použít v pozdějších krocích.
