---
title: Vytvoření, změna nebo odstranění virtuální sítě Azure | Dokumentace Microsoftu
...
3. Pro virtuální síť, kterou jste vybrali, jsou uvedeny následující nastavení:
    ...
    - **Adresní prostor**: jsou uvedeny adresní prostory, které jsou přiřazeny k virtuální síti. Informace o přidávání a odebírání rozsah adres do adresního prostoru, proveďte kroky v [přidat nebo odebrat rozsah adres](#add-or-remove-an-address-range).
    ...
    - **Podsítě**: Zobrazí seznam podsítí, které existují v rámci virtuální sítě. Zjistěte, jak přidat a odebrat podsíť, najdete v článku [Správa podsítí](virtual-network-manage-subnet.md).
    ...
    - **Vlastnosti**: nastavení virtuální sítě, včetně ID prostředku virtuální sítě a předplatné Azure, které se zobrazí.
...
4. Vyberte jednu z následujících možností:
    ...
    - **Vlastní**: můžete přidat jeden nebo více serverů, až do limitu Azure pro virtuální síť. Další informace o omezeních server DNS, najdete v článku [omezení Azure](../azure-subscription-service-limits.md?toc=%2fazure%2fvirtual-network%2ftoc.json#virtual-networking-limits-classic). Máte následující možnosti:
        - **Přidat adresu**: Přidá tento server do seznamu serverů DNS virtuální sítě. Tato možnost také zaregistruje DNS server s využitím Azure. Pokud už zaregistrovaný DNS server s využitím Azure, můžete vybrat tento server DNS v seznamu.
