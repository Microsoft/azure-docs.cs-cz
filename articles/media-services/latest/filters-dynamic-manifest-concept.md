...
Azure Media Services podporuje protokoly HLS, MPEG POMLČKy a Smooth Streaming. V rámci [dynamického balení](encode-dynamic-packaging-concept.md)se dynamicky generují manifesty klienta streamování (HLS hlavní seznam testů, pomlčka multimediální prezentace [MPD] a Smooth Streaming) na základě selektor formátu v adrese URL. Další informace najdete v tématu Delivery Protocols ( [běžné pracovní postupy na vyžádání](encode-dynamic-packaging-concept.md#to-prepare-your-source-files-for-delivery)).
...
V následujícím příkladu byl kodér použit ke kódování Mezzanine prostředku do sedmi verzí rychlostmi videa ISO (od 180p do 1080p). Kódovaný prostředek se dá [dynamicky balit](encode-dynamic-packaging-concept.md) do libovolného z následujících protokolů streamování: HLS, MPEG pomlčka a hladký.
