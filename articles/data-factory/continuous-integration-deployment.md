---
title: Průběžnou integraci a nasazení v Azure Data Factory | Microsoft Docs
description: Další informace o použití průběžnou integraci a nasazení pro přesun kanálů služby Data Factory z jednoho prostředí (vývoj, testovací, produkční) do jiné.
...
Pak přejděte do okna objekt pro vytváření dat testovací a produkční objekt pro vytváření dat a vyberte **šablony Import ARM**.

![](media/continuous-integration-deployment/continuous-integration-image2.png)

Tím přejdete na portál Azure, kde můžete importovat vyexportované šablony. Vyberte **sestavení vlastní šablonu v editoru** a potom **načíst soubor** a vyberte vygenerované šablony Resource Manageru. Zadejte nastavení a data factory a celý kanálu je naimportována v provozním prostředí.
...
## <a name="continuous-integration-lifecycle"></a>Životní cyklus průběžnou integraci
Tady je celý životní cyklus průběžnou integraci a nasazení, které můžete použít po povolení integrace služby VSTS GITU v uživatelském rozhraní objektu pro vytváření dat:

1.  Nastaví objekt pro vytváření dat vývoj pomocí služby VSTS, ve kterém můžete všechny vývojářům vytvářet prostředky pro vytváření dat jako kanály, datové sady a tak dále.

2.  Vývojáři pak můžete upravit prostředkům, například kanály. Podle svého jejich změny, můžete vybrat **ladění** zobrazíte, jak se kanál spustí s poslední změny.

3.  Jakmile se jejich změny spokojeni vývojáři, mohou vytvářet žádosti o přijetí změn z jejich větve hlavní větve (nebo větev spolupráce) Chcete-li získat jejich změny zkontrolovány uživatelem partnerské uzly.

4.  Jakmile jsou změny v hlavní větve, můžete publikovat k objektu pro vytváření vývoj výběrem **publikovat**.

5.  Když týmem je připraven k podpoře změny objektu pro vytváření testovací a produkční objekt pro vytváření, se můžete exportovat šablony Resource Manageru z hlavní pobočky nebo z jiných větve v případě, že jejich hlavní větve zálohuje za provozu vývoj Data Factory.

6.  Vyexportované šablony Resource Manageru můžete nasadit s jiným parametrem soubory k objektu pro vytváření testovací a produkční objekt pro vytváření.

## <a name="automate-continuous-integration-with-vsts-releases"></a>Automatizovat průběžnou integraci s verzemi služby VSTS

Tady jsou kroky při nastavení vydání služby VSTS, abyste mohli automatizovat nasazení objektu pro vytváření dat do několika prostředí.

![Diagram průběžnou integraci s služby VSTS](media/continuous-integration-deployment/continuous-integration-image12.png)

### <a name="requirements"></a>Požadavky

-   Předplatné Azure propojené s Team Foundation Server nebo služby VSTS pomocí [ *koncový bod služby Azure Resource Manager*](https://docs.microsoft.com/vsts/build-release/concepts/library/service-endpoints#sep-azure-rm).

-   Objekt pro vytváření dat s Gitem služby VSTS nakonfigurované.

-   [Azure Key Vault](https://azure.microsoft.com/services/key-vault/) obsahující těchto tajných klíčů.

### <a name="set-up-a-vsts-release"></a>Nastavení služby VSTS verze

1.  Přejděte na stránku služby VSTS ve stejném projektu jako konfigurovaných pomocí služby Data Factory.

2.  Kliknutím na horní nabídce **sestavení a verze** &gt; **verze** &gt; **vytvořit verze definice**.
...
7.  Přidáte úloha nasazení Azure Resource Manager:
    ...
    b.  V úloze pro nasazení vyberte předplatné, skupinu prostředků a umístění pro cílový objekt pro vytváření dat a v případě potřeby zadejte přihlašovací údaje.
    ...
    f.  Vyberte **...** vedle položky **přepsat parametry šablony** pole a zadejte informace pro cíl služby Data Factory. Přihlašovací údaje, které pocházejí z trezoru klíčů, použijte stejný název pro tajný klíč v následujícím formátu: název tajného klíče za předpokladu, že je `cred1`, zadejte `"$(cred1)"` (mezi uvozovky).
...
Nasazení může selhat, pokud se pokusíte aktualizovat active aktivační události. Aktualizace active aktivačních událostí, musíte ručně je zastavit a spustit je po dokončení nasazení. Pro tento účel, můžete přidat úkol aplikace Azure Powershell, jak je znázorněno v následujícím příkladu:

1.  Na kartě úlohy verze služby VSTS vyhledejte **prostředí Azure Powershell** a přidejte ji.

2.  Zvolte **Azure Resource Manager** jako připojení typu a vybrat své předplatné.
...
Můžete podobný postup a použít podobný kód (s `Start-AzureRmDataFactoryV2Trigger` funkce) k restartu aktivačních událostí po nasazení.

## <a name="sample-template-and-script"></a>Ukázka šablony a skriptu
Zde jsou dvou vzorcích, které vám pomůže začít pracovat s průběžnou integraci a nasazení pro vytváření dat:

-   Ukázka šablonu nasazení, který můžete importovat do služby VSTS.
-   Ukázkový skript aktivační události před nasazením zastavit a restartovat aktivuje později. Tento skript také obsahuje kód, který odstranit prostředky, které byly odebrány.

Zde je ukázka šablony nasazení, který můžete importovat do služby VSTS.
...
Tady je ukázkový skript zastavit aktivační události před nasazením a pak restartujte aktivační události:
