...
| source | Ano | Řetězec nebo JToken | Pokud je typ zdroje `Microsoft.Logic/workflows`, musí zdrojové informace postupovat podle tohoto schématu. Pokud je typ zdroje `custom`, je schéma JToken. |
...
| repeatItemScopeName | Ano | Řetězec | Opakovat název položky, je-li akce uvnitř `foreach`nebo smyčka `until` |
| repeatItemIndex | Ano | Integer | Označuje, že akce je uvnitř `foreach` nebo `until` smyčky a je číslo indexu opakované položky. |
| trackingId | Ne | Řetězec | ID sledování ke korelaci zpráv |
| correlationId | Ne | Řetězec | ID korelace ke korelaci zpráv |
| clientRequestId | Ne | Řetězec | Klient může tuto vlastnost naplnit tak, aby koreluje zprávy. |
| eventLevel | Ano | Řetězec | Úroveň události |
| eventTime | Ano | Datum a čas | Čas události ve formátu UTC: *RRRR-MM-DDThh: mm: ss. 00000Z* |
| recordType | Ano | Řetězec | Typ záznamu sledování s touto povolenou hodnotou: `custom` |
| record | Ano | JToken | Vlastní typ záznamu pouze ve formátu JToken |
|||||
