---
title: Řešení potíží s nabízenou instalací služby mobility pomocí Azure Site Recovery f
...
* Azure Site Recovery vyžaduje k provedení nabízené instalace **kořenový** účet nebo uživatelský účet s **oprávněními správce** . V opačném případě bude nabízená instalace na zdrojovém počítači blokovaná.
  ...
  * Pro Linux (**chyba 95108**) musíte zvolit kořenový účet pro úspěšnou instalaci agenta mobility. Navíc by měly být spuštěny služby SFTP. Postup povolení subsystému SFTP a ověřování hesla v souboru sshd_config:
    ...
    2. Přejít na/etc/ssh/sshd_config soubor, Najděte řádek, který začíná na PasswordAuthentication.
...
Uživatelský účet vybraný během povolování replikace byl zakázán. Pokud chcete uživatelský účet povolit, přečtěte [si tento článek nebo](https://aka.ms/enable_login_user) spusťte následující příkaz, který nahradí textové *uživatelské* jméno skutečným uživatelským jménem.
`net user 'username' /active:yes`

### <a name="credentials-locked-out-due-to-multiple-failed-login-attempts-errorid-95520"></a>Přihlašovací údaje byly uzamčeny z důvodu více nezdařených pokusů o přihlášení (ErrorID: 95520).
...
* Pro **virtuální počítače se systémem Linux**navíc
  ...
  * Služby SFTP by měly být spuštěné. Pokud chcete povolit podsystém SFTP a ověřování hesla v souboru sshd_config,
    ...
    * Přejít na/etc/ssh/sshd_config soubor, Najděte řádek, který začíná na PasswordAuthentication.
...
1. Vyhledejte UUID zařízení spuštěním příkazu "blkid \<název zařízení >". Příklad:<br>
   ```
   blkid /dev/sda1
...
K tomuto problému dochází hlavně v případě, že antivirový software blokuje operace služby Azure Site Recovery Services. Řešení tohoto problému:
...
K této chybě dojde, pokud je Azure Site Recovery databáze služby poskytovatele VSS [uzamčena](https://msdn.microsoft.com/library/ms833798.aspx). Pokuste se ručně nainstalovat VSS na zdrojový počítač spuštěním následujícího příkazového řádku.
...
2. Postup při obejít instalaci stínové kopie svazku:
   ...
   2. Přidejte Azure Site Recovery instalační skripty poskytovatele služby VSS **nMageVSSProvider_Install** a **InMageVSSProvider_Uninstall. cmd** , aby se vždy úspěšně zdařily, a to přidáním následujících řádků:
...
V protokolu, který se nachází na konfiguračním serveru C:\ProgramData\ASRSetupLogs\UploadedLogs\<> UA_InstallLogFile. log, se zobrazí následující výjimka:
