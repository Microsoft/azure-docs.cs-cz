...
description: Úvod k zálohování SQL Server databází pomocí Azure Backup Server
author: kasinh
manager: vvithal
ms.service: backup
ms.topic: conceptual
...
8. Klikněte na **Další**
    ...
    MABS zobrazuje celkový dostupný prostor úložiště a možné využití místa na disku.

    ![Přidělení disku](./media/backup-azure-backup-sql/pg-storage.png)

    Ve výchozím nastavení MABS vytvoří jeden svazek na zdroj dat (SQL Server databázi), který se používá pro prvotní záložní kopii. Pomocí tohoto přístupu omezuje Správce logických disků (LDM) MABS ochranu na 300 zdrojů dat (SQL Server databází). Toto omezení můžete obejít tak, že vyberete možnost **společně najít data ve fondu úložiště DPM**, možnost. Pokud použijete tuto možnost, MABS použije jeden svazek pro více zdrojů dat, což umožňuje MABS chránit až 2000 databází SQL.

    Pokud je vybraná možnost **automaticky zvětšit svazky** , může MABS při zvětšování produkčních dat brát v úvahu zvýšený svazek zálohy. Pokud možnost **automaticky zvětšit svazky** není VYBRÁNA, MABS omezí úložiště zálohování používané pro zdroje dat ve skupině ochrany.
...
10. Vyberte, kdy chcete spustit kontrolu konzistence, a klikněte na tlačítko **Další**.
    ...
    MABS může provést kontrolu konzistence a ověřit integritu bodu zálohování. Vypočítá kontrolní součet záložního souboru na provozním serveru (SQL Server počítač v tomto scénáři) a zálohovaná data pro tento soubor na adrese MABS. V případě konfliktu se předpokládá, že zálohovaný soubor v MABS je poškozený. MABS vystaví zálohovaná data odesláním bloků odpovídajících neshodě kontrolního součtu. Vzhledem k tomu, že je kontrola konzistence náročná na výkon, mají správci možnost naplánovat kontrolu konzistence nebo ji spustit automaticky.
