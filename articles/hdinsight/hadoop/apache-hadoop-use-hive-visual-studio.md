...
* Sadu [Visual Studio](https://visualstudio.microsoft.com/vs/). Kroky v tomto článku používají Visual Studio 2019.
...
5. Otevřete soubor **script. HQL** , který je vytvořen pomocí tohoto projektu, a vložte následující příkazy HiveQL:
    ...
    * `SELECT`: vybere počet všech řádků, ve kterých `t4` sloupec obsahuje hodnotu `[ERROR]`. Tento příkaz vrátí hodnotu `3`, protože tři řádky obsahují tuto hodnotu.

    * `INPUT__FILE__NAME LIKE '%.log'`: instruuje podregistr, aby vracel jenom data ze souborů, které končí. log. Tato klauzule omezí hledání na soubor *Sample. log* , který obsahuje data.
...
7. Vyberte **výstup úlohy** a zobrazte výstup této úlohy. Zobrazuje `[ERROR] 3`, což je hodnota vrácená tímto dotazem.

### <a name="additional-example"></a>Další příklad

Následující příklad spoléhá na tabulku `log4jLogs` vytvořenou v předchozím postupu, [vytvoří aplikaci podregistru](#create-a-hive-application).
...
2. Zadejte následující dotaz na podregistr:
    ...
    * `CREATE TABLE IF NOT EXISTS`: vytvoří tabulku, pokud ještě neexistuje. Protože klíčové slovo `EXTERNAL` není použito, tento příkaz vytvoří interní tabulku. Interní tabulky jsou uloženy v datovém skladu podregistru a jsou spravovány podregistru.
...
4. Pokud chcete ověřit, že úloha vytvořila tabulku, přejít na **Průzkumník serveru** a rozbalte **Azure** > **HDInsight**. Rozbalte svůj cluster HDInsight a potom rozbalte položku **databáze podregistru** > **výchozí**. Zobrazí **se tabulka chyb a** tabulka **log4jLogs** .
