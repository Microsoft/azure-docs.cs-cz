...
Během instalace se zobrazí výzva `Modify profile to update your $PATH and enable shell/tab completion now? (Y/n)`. Pokud odpovíte `y` a `/etc/rc.conf` zadáte `a path to an rc file to update`jako, může se tento problém `ERROR: [Errno 13] Permission denied`vyhovět. Chcete-li tento problém vyřešit, měli byste tomuto souboru `etc/rc.conf`udělit právo k zápisu pro aktuálního uživatele.
