...
`Set-AzureRmVMDiagnosticsExtension` Příkaz slouží k přidání rozšíření Azure Diagnostics do existujícího virtuálního počítače. Viz také [použití Powershellu k povolení diagnostiky Azure v rámci virtuálního počítače se systémem Windows](ps-extensions-diagnostics.md).
```powershell
