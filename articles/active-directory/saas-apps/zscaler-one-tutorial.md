...
1. Do vyhledávacího pole zadejte **Zscalerem jeden**.

    ![Vytváří se testovací uživatele služby Azure AD](./media/zscaler-one-tutorial/tutorial_zscalerone_search.png)

1. Na panelu výsledků vyberte **Zscalerem jeden**a potom klikněte na tlačítko **přidat** tlačítko pro přidání aplikace.
...
1. **[Vytváří se Zscalerem jednoho testovacího uživatele](#creating-a-zscaler-one-test-user)**  – Pokud chcete mít protějšek Britta Simon Zscalerem jeden, který je propojený s Azure AD reprezentace uživatele.
...
1. Na **Zscalerem jedné domény a adresy URL** části, proveďte následující kroky:
...
1. Na **Zscalerem jednu konfiguraci** klikněte na tlačítko **nakonfigurovat jeden Zscalerem** otevřete **nakonfigurovat přihlašování** okna. Kopírovat **SAML jednotné přihlašování – adresa URL služby** z **Stručná referenční příručka oddílu.**

    ![Konfigurace jednotného přihlašování](./media/zscaler-one-tutorial/tutorial_zscalerone_configure.png) 

1. V jiných webových okno prohlížeče Přihlaste se k webu Zscalerem jeden společnosti jako správce.
...
1. V seznamu aplikací vyberte **Zscalerem jeden**.
