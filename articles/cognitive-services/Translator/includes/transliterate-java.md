...
Otevřete `Transliterate.java` a přidejte tyto příkazy importu:
...
Přidejte tyto `Transliterate` řádky do třídy. Nejprve klíč předplatného a koncový bod jsou čteny z proměnných prostředí. Pak si všimnete, že `api-version`spolu s , dva další parametry byly připojeny k `url`. Tyto parametry se používají k nastavení vstupního jazyka a skriptů pro přepis. V této ukázce je`jpan`nastavena na`latn`japonštinu ( ) a latinku ( ). 
...
Přidejte tento `Transliterate` řádek do třídy `OkHttpClient`k vytvoření instance :
...
Dále sestavíme požadavek POST. Nebojte se změnit text pro přepis.
