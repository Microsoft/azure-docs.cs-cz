...
description: Přečtěte si o fondech spravované instance Azure SQL (Preview), což je funkce, která poskytuje pohodlný a cenově výhodný způsob migrace menších SQL Server databází do cloudu ve velkém měřítku a správě více spravovaných instancí SQL.
...
Následující diagram znázorňuje fond instancí s více spravovanými instancemi SQL nasazenými v podsíti virtuální sítě.
...
Fondy instancí mají podobnou architekturu pro běžné (*samostatné*) spravované instance. Aby se podporovaly [nasazení v rámci virtuálních sítí Azure (virtuální sítě)](../../virtual-network/virtual-network-for-azure-services.md)   a poskytovaly izolaci a zabezpečení pro zákazníky, využívají fondy instancí také [virtuální clustery](connectivity-architecture-overview.md#high-level-connectivity-architecture). Virtuální clustery reprezentují vyhrazenou sadu izolovaných virtuálních počítačů nasazených v podsíti virtuální sítě zákazníka.
...
- Spravované instance SQL v rámci fondu mají vyhrazené PROCESORy a paměť RAM, takže agregovaný počet virtuální jádra napříč všemi instancemi musí být menší nebo roven počtu virtuální jádra přidělených fondu.
- Všechna [omezení na úrovni instance](resource-limits.md#service-tier-characteristics) se vztahují na instance vytvořené v rámci fondu.
...
Spravované instance SQL vytvořené ve fondech podporují stejné [úrovně kompatibility a funkce podporované v rámci jedné spravované instance](sql-managed-instance-paas-overview.md#sql-features-supported).

Každá spravovaná instance SQL nasazená ve fondu má samostatnou instanci SQL agenta.
...
Pokud máte problémy související s jednotlivými spravovanými instancemi SQL nebo databázemi v rámci fondu, měli byste vytvořit pravidelný lístek podpory pro spravované instance Azure SQL.
