...
Související článek: [Předání parametru uloženým procedurám](#scenario-6-pass-parameters-to-u-sql-script)
...
3. Nastavte **soubory** `*.usql` v **konfiguraci enumerátoru** tak, aby kontejner smyčky zachytává `.usql`pouze soubory končící na.
...
2. Nastavte **připojení** k souboru připojení, které odkazuje na objekty souborů vrácené z kontejneru smyčky foreach.
   ...
   1. Zvolit **připojení \<New... >** nastavení připojení.
   ...
   4. V okně **vlastnosti** rozbalte **výrazy**a nastavte **ConnectionString** na proměnnou definovanou v `@[User::FileName]`kontejneru smyčky foreach, například.
...
5. Použijte **výrazy** k dynamickému nastavení názvu úlohy U-SQL:
    ...
    2. Nastavte hodnotu pro JobName na proměnnou definovanou v kontejneru smyčky foreach, `@[User::FileName]`například.
...
Postup je podobný [scénáři 2: Použijte v Azure Data Lake Store](#scenario-2-use-u-sql-files-in-azure-data-lake-store)soubory U-SQL. Změňte úlohu systému souborů Azure Data Lake Store na úlohu stažení objektu BLOB v Azure. [Přečtěte si další informace o úloze stažení objektu BLOB v Azure](https://docs.microsoft.com/sql/integration-services/control-flow/azure-blob-download-task?view=sql-server-2017).
...
- [Spouštění balíčků SSIS v Azure](https://docs.microsoft.com/azure/data-factory/how-to-invoke-ssis-package-ssis-activity)
