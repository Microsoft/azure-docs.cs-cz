...
| Stav | Popis | Non-browser apps affected | Browser apps affected | Modern authentication affected |
|:---:|:---:|:---:|:--:|:--:|
| Zakázáno |The default state for a new user not enrolled in Azure MFA. |Ne |Ne |Ne |
| Povoleno |The user has been enrolled in Azure MFA, but has not registered. They receive a prompt to register the next time they sign in. |Ne.  They continue to work until the registration process is completed. | Ano. After the session expires, Azure MFA registration is required.| Ano. After the access token expires, Azure MFA registration is required. |
| Vynuceno |The user has been enrolled and has completed the registration process for Azure MFA. |Ano. Apps require app passwords. |Ano. Azure MFA is required at login. | Ano. Azure MFA is required at login. |
