...
> [!IMPORTANT]
> V tomto rychlém startu se používá rozhraní API pro rozpoznávání formulářů v 1.0. Pokud je vaše předplatné ve `West US 2` nebo `West Europe` oblasti, měli byste místo toho postupovat podle pokynů v části[rychlý Start pro rozhraní API v 2,0](./curl-train-extract.md) .
...
Všimněte si `"modelId"` hodnoty. Budete ho potřebovat v následujících krocích.
...
1. Nahraďte `<modelID>` číslem ID, které jste obdrželi v předchozí části.
...
> [Referenční dokumentace REST API](https://westus2.dev.cognitive.microsoft.com/docs/services/form-recognizer-api/operations/AnalyzeWithCustomModel)