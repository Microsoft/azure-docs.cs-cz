...
Tento článek se zaměřuje na přizpůsobení Azure AD B2C uživatelského rozhraní s *dynamickým obsahem* pomocí vlastních zásad. Informace o tom, jak začít, najdete v tématu [přizpůsobení uživatelského rozhraní ve vlastních zásadách](custom-policy-ui-customization.md).

>[!NOTE]
>Článek Azure AD B2C, [Konfigurace přizpůsobení uživatelského rozhraní ve vlastních zásadách](custom-policy-ui-customization.md), popisuje následující základy:
...
V článku [Konfigurace přizpůsobení uživatelského rozhraní ve vlastní zásadě](custom-policy-ui-customization.md) nahrajete soubory HTML5 do úložiště objektů BLOB v Azure. Tyto soubory HTML5 jsou statické a vykreslují pro každý požadavek stejný obsah HTML.
...
3. Pojmenujte aplikaci (například *contoso. AADB2C. UI*) a pak vyberte **OK**.
    ...
    ![Vytvořit nový projekt sady Visual Studio](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-create-project1.png)

4. Vyberte šablonu **webové aplikace** .

5. Nastavte ověřování na **bez ověřování**.

    ![Vybrat šablonu webové aplikace](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-create-project2.png)
...
1. Klikněte pravým tlačítkem na položku zobrazení nebo domovskou složku a **přidejte** > **novou položku**.
    ...
    ![Přidat položku nabídky nová položka v aplikaci Visual Studio](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-view1.png)
...
5. Vyberte **Přidat**.
    ...
    ![Dialogové okno Přidat novou položku v aplikaci Visual Studio se zvýrazněnou stránkou zobrazení MVC](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-view2.png)
...
10. Zkopírujte obsah souboru a vložte jej pod definici rozložení. Váš kód by měl vypadat takto:
    ...
    ![Sjednocený soubor. cshtml po přidání jazyka HTML5](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-edit-view1.png)
...
![IMG – element s vlastní hodnotou background_background_image src](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-static-background.png)
...
1. Otevřete **souboru controllers\homecontroller.cs**a přidejte následující metodu:
    ...
    ![Změna kontroleru pro vykreslení zobrazení](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-controller-view.png)
...
1. V **Průzkumník řešení**klikněte pravým tlačítkem myši na projekt **contoso. AADB2C. UI** a pak vyberte **publikovat**.
    ...
    ![Publikování do Microsoft Azure App Service](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish1.png)

2. Vyberte dlaždici **Microsoft Azure App Service** a pak vyberte **publikovat**.

    ![Vytvořit novou Microsoft Azure App Service](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish2.png)
...
4. Výběrem možnosti **Vytvořit** spustíte vytváření prostředků Azure.
    ...
    ![Zadat App Service vlastnosti](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish3.png)
...
1. V [Azure Portal](https://portal.azure.com/)vyberte **App Services**a pak vyberte název vaší aplikace API.
    ...
    ![V Azure Portal vyberte aplikace API.](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS1.png)

2. V části **Nastavení** v části **rozhraní API** vyberte **CORS**.

    ![Položka nabídky CORS zvýrazněná v nabídce App Service v Azure Portal](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS2.png)
...
4. Vyberte **Uložit**.
    ...
    ![Stránka nastavení CORS se zvýrazněnou hvězdičkou v povolených zdrojích](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS3.png)
...
6. Změňte hodnotu `LoadUri` z _~/tenant/default/Unified_ na _https://< APP_NAME >. azurewebsites. NET/Home/Unified_.
    ...
    ![Příklad fragmentu XML s zvýrazněným elementem LoadUri](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-content-definition.png)
...
2. Otevřete **B2C_1A_signup_signin**, vlastní zásady předávající strany (RP), které jste nahráli, a pak vyberte **Spustit nyní**.
    ...
    ![Vaše zásady registrace nebo přihlašování](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo1.png)
...
2. Vyhledejte prvek `<img>` s ID `background_background_image`a nahraďte hodnotu `src` `@ViewData["background"]`.
    ...
    ![IMG – element s zvýrazněnou hodnotou src ](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-dynamic-background.png)
...
5. Přidejte parametr řetězce dotazu _campaignId_ k identifikátoru URI. Přidejte například `&campaignId=hawaii`, jak je znázorněno na následujícím obrázku:
    ...
    ![Identifikátor URI s vybraným parametrem řetězce dotazu campaignId](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-campaignId-param.png)

6. Pokud chcete zobrazit obrázek na pozadí Havajské ostrovy, vyberte **ENTER** .

    ![Přihlašovací stránka pro přihlášení s vlastním pozadím ve havajské imagi](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo2.png)

7. Změňte hodnotu na *Tokio*a pak vyberte **ENTER**.
    V prohlížeči se zobrazí pozadí v Brně.

    ![Přihlašovací stránka pro přihlášení s vlastním pozadím v neřízeném obrázku](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo3.png)
