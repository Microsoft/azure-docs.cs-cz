---
title: Řešení potíží s kódy chyb pro rozšíření Azure MFA NPS – Azure Active Directory
...
Pokud narazíte na chyby s rozšířením NPS pro Azure Multi-Factor Authentication, použijte tento článek k rychlejšímu dosažení řešení. Protokoly rozšíření NPS se nacházejí v Prohlížeč událostí v části **vlastní zobrazení** > **role** > serveru**zásady sítě a přístup** na serveru, na kterém je nainstalovaná přípona NPS.
...
| **USERNAME_CANONICALIZATION_ERROR** | Ověřte, zda se uživatel nachází v místní instanci služby Active Directory a zda má služba NPS oprávnění pro přístup k tomuto adresáři. Pokud používáte vztahy důvěryhodnosti mezi doménovými strukturami, požádejte o další pomoc [podporu](#contact-microsoft-support) . |
...
| ---------- | ------------- | --------------------- |
| **ALTERNATE_LOGIN_ID_ERROR** | Chyba: nepovedlo se userObjectSidovat vyhledávání | Ověřte, zda uživatel existuje v místní instanci služby Active Directory. Pokud používáte vztahy důvěryhodnosti mezi doménovými strukturami, požádejte o další pomoc [podporu](#contact-microsoft-support) . |
| **ALTERNATE_LOGIN_ID_ERROR** | Chyba: Nepovedlo se LoginIdovat alternativní vyhledávání | Ověřte, že je LDAP_ALTERNATE_LOGINID_ATTRIBUTE nastaven na [platný atribut služby Active Directory](https://msdn.microsoft.com/library/ms675090(v=vs.85).aspx). <br><br> Je-li LDAP_FORCE_GLOBAL_CATALOG nastaven na hodnotu true nebo je-li pro LDAP_LOOKUP_FORESTS nakonfigurována neprázdná hodnota, ověřte, zda jste nakonfigurovali globální katalog a zda je do něj přidán atribut AlternateLoginId. <br><br> Pokud je LDAP_LOOKUP_FORESTS nakonfigurovaný s neprázdnou hodnotou, ověřte, jestli je hodnota správná. Pokud existuje více než jeden název doménové struktury, názvy musí být odděleny středníkem, ne mezerami. <br><br> Pokud tyto kroky problém nevyřeší, požádejte o další pomoc [podporu](#contact-microsoft-support) . |
...
V některých případech mohou uživatelé získat zprávy od služby Multi-Factor Authentication, protože jejich žádost o ověření se nezdařila. Tyto chyby nejsou v produktu konfigurace, ale jedná se o úmyslné výstrahy s vysvětlením, proč byla žádost o ověření zamítnuta.
