...
* Visual Studio Tools for Xamarin, ve Visual Studiu 2017 nebo novějším nebo Visual Studio pro Mac. Pokyny najdete na [stránce věnované instalaci Xamarinu][Install Xamarin].

* (Volitelné) K sestavení aplikace pro iOS se vyžaduje Mac s prostředím Xcode 9.0 nebo novějším. Visual Studio pro Mac se dá použít k vývoji aplikací pro iOS, nebo visual studio 2017 nebo novější se dá použít (pokud je Mac dostupný v síti).
...
1. Přejděte na [portál Azure](https://portal.azure.com/) a přejděte na mobilní aplikaci, kterou jste vytvořili. Na `Overview` okně vyhledejte adresu URL, která je veřejným koncovým bodem pro vaši mobilní aplikaci. Příklad – název webu pro název aplikace "test123" bude https://test123.azurewebsites.net.

2. Otevřete `Constants.cs` soubor v této složce - xamarin.forms/ZUMOAPPNAME. Název aplikace `ZUMOAPPNAME`je .

3. Ve `Constants.cs` třídě `ZUMOAPPURL` nahraďte proměnnou veřejným koncovým bodem výše.
...
1. Klikněte pravým tlačítkem myši na projekt iOS a potom vyberte **Nastavit jako spouštěcí projekt**.
