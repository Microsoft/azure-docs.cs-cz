...
Azure s víc uzly systémy zásobníku, které používají model fakturace založený na spotřebě hlásit data o využití do Azure pro účely fakturace.  Operátoři Azure stacku měli nakonfigurovat svou instanci služby Azure Stack data využívání na Azure.
...
Pokud jste zaregistrovali služby Azure Stack pomocí s předplatným Enterprise Agreement, můžete zobrazit využití a poplatků [portál EA](https://ea.azure.com/). Využití služby Azure Stack se zahrnou rozšířené soubory ke stažení, společně s Azure použití části sestavy na portálu EA. 

## <a name="view-usage--other-subscriptions"></a>Zobrazit využití – další předplatná

Pokud jste zaregistrovali služby Azure Stack pomocí libovolného jiné předplatné typu, například předplatné průběžnými platbami, můžete zobrazit využití a poplatků v centru účtů Azure. Přihlaste se k [centra účtů Azure](https://account.windowsazure.com/Subscriptions) jako Azure účtu správce a vyberte předplatné Azure, který jste použili k registraci Azure Stack. Můžete zobrazit data využití služby Azure Stack, velikost účtovat pro každou použitých prostředků, jak je znázorněno na následujícím obrázku:

   ![fakturační toku](media/azure-stack-usage-reporting/pricing-details.png)

Pro Azure Stack Development Kit prostředky služby Azure Stack se neúčtují, cena je zobrazena jako 0.00 $.

## <a name="which-azure-stack-deployments-are-charged"></a>Které nasazení Azure Stack se účtují?

Využití prostředků je zdarma pro Azure Stack Development Kit. Zatímco několikauzlovými systémy Azure Stack, se účtují úloh virtuálních počítačů, služeb úložiště a App Services.
...
Pro Azure Stack multinode předplatná se smlouvou Enterprise (EA) a zprostředkovatele kryptografických služeb se podporují. Pro Azure Stack Development Kit podporu vytváření sestav dat využití předplatných sady Enterprise Agreement (EA), s průběžnými platbami, CSP a MSDN.
