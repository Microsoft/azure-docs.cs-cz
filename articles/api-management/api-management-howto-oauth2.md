...
## <a name="step1"> </a>Konfigurace autorizačního serveru OAuth 2,0 v API Management
...
4. V další části formuláře jsou **typy udělení autorizace**, **Adresa URL koncového bodu autorizace**a nastavení **metody autorizační žádosti** .
    ...
    Zadejte **adresu URL koncového bodu autorizace**. V případě Azure Active Directory bude tato adresa URL podobná následující adrese URL, kde `<tenant_id>` nahrazuje ID vašeho tenanta Azure AD.
...
5. Pak je nutné zadat **adresu URL koncového bodu tokenu**, **metody ověřování klienta**, **metodu odeslání přístupového tokenu** a **výchozí obor** .
    ...
    U Azure Active Directory serveru OAuth 2,0 bude **Adresa URL koncového bodu tokenu** mít následující formát, ve kterém má `<TenantID>` formát `yourapp.onmicrosoft.com`.
...
## <a name="step2"> </a>Konfigurace rozhraní API pro použití autorizace uživatele OAuth 2,0
...
## <a name="step3"> </a>Starší verze portálu pro vývojáře – testování autorizace uživatele OAuth 2,0
...
Jakmile se přihlásíte, **hlavičky žádosti** se naplní hlavičkou `Authorization : Bearer`, která požadavek autorizuje.
