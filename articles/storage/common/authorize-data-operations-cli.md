...
Příkazy rozhraní příkazového řádku Azure pro čtení a zápis dat objektů BLOB a front obsahují volitelný parametr `--auth-mode`. Zadáním tohoto parametru označíte, jak má být datová operace autorizována:

- Nastavte parametr `--auth-mode`, který se má `login` přihlašovat pomocí objektu zabezpečení služby Azure AD (doporučeno).
- Nastavte parametr `--auth-mode` na hodnotu starší `key`, abyste se pokusili načíst přístupový klíč účtu, který se má použít pro autorizaci. Pokud parametr `--auth-mode` vynecháte, Azure CLI se také pokusí načíst přístupový klíč.

Pokud chcete použít parametr `--auth-mode`, ujistěte se, že máte nainstalované rozhraní příkazového řádku Azure CLI verze 2.0.46 nebo novější. Spusťte `az --version` a ověřte nainstalovanou verzi.

> [!IMPORTANT]
> Pokud parametr `--auth-mode` vynecháte nebo ho nastavíte na `key`, pokusí se rozhraní příkazového řádku Azure CLI použít pro autorizaci přístupový klíč k účtu. V takovém případě společnost Microsoft doporučuje zadat přístupový klíč buď v příkazu, nebo v proměnné prostředí **AZURE_STORAGE_KEY** . Další informace o proměnných prostředí naleznete v části s názvem [nastavení proměnných prostředí pro parametry autorizace](#set-environment-variables-for-authorization-parameters).
>
...
1. Pomocí příkazu [AZ Storage Container Create](/cli/azure/storage/container#az-storage-container-create) použijte s parametrem `--auth-mode` nastavenou na `login` k vytvoření kontejneru pomocí vašich přihlašovacích údajů Azure AD. Nezapomeňte nahradit zástupné hodnoty v lomených závorkách vlastními hodnotami:
...
Následující příklad ukazuje, jak vytvořit kontejner pomocí přístupového klíče účtu. Zadejte klíč účtu a zadejte parametr `--auth-mode` s hodnotou `key`:
