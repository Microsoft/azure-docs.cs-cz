...
Tyto požadavky se vztahují i na [přímý přístup do cloudu](#direct-share-access-vs-azure-file-sync) (přes virtuální počítač nebo službu Azure), než na synchronizace souborů Azure. Pokud plánujete použít Synchronizace souborů Azure jenom na těchto sdílených složkách, je v pořádku seskupení několika do jednoho účtu úložiště Azure. Zvažte také, že budete chtít aplikaci najímat a přesunout do cloudu, která by pak měla přímý přístup ke sdílené složce. Nebo můžete začít používat službu v Azure, která by mohla mít k dispozici vyšší počet vstupně-výstupních operací a propustnosti.
...
Na konci fáze 3 budete spouštět úlohy služby transformace dat ze svazků StorSimple do sdílených složek Azure. Nyní se můžete soustředit na buď nastavení Synchronizace souborů Azure pro sdílenou složku (po dokončení úloh migrace pro sdílenou složku), nebo nasměrováním přístupu ke sdílení pro vaše informační pracovníky a aplikace do sdílené složky Azure.
...
V této fázi jste v StorSimple Data Manager vytvořili a spustili více úloh služby transformace dat. Tyto úlohy migrovali soubory a složky do sdílených složek Azure. Nasadili jste taky Synchronizace souborů Azure nebo připravil vaše síťové a úložné účty pro přímý přístup do sdílené složky.
