...
![Diagram pro Azure Resource Manager](media/azure-stack-edge-gpu-connect-resource-manager/edge-device-flow.svg)
...
1. Spusťte PowerShell verze 5,0. Musíte mít PowerShell verze 5,0. PowerShell Core se nepodporuje. Pokud chcete zjistit verzi PowerShellu v systému, spusťte následující rutinu:
...
3. Pro referenci použijte následující obrázek. Uložte soubor **hostitelů** .
    ...
    ![soubor hostitelů v programu Poznámkový blok](media/azure-stack-edge-gpu-connect-resource-manager/hosts-file-notepad.png)
...
1. K ověření, že je název koncového bodu vyřešený, můžete použít nástroj příkazového řádku ping.exe. Po zadání IP adresy vrátí příkaz příkazu k zadání názvu hostitele TCP/IP počítače, který chcete \' znovu sledovat.
    ...
    ![Příkaz testovat v příkazovém řádku](media/azure-stack-edge-gpu-connect-resource-manager/ping-command-prompt.png)
...
2. Zavoláním rozhraní API místního zařízení ověřte připojení k Azure Resource Manageru. 
    ...
    2. Můžete se připojit prostřednictvím `login-AzureRMAccount` příkazu nebo pomocí `Connect-AzureRMAccount` příkazu. 
        ...
        1. Pokud se chcete přihlásit, zadejte následující příkaz. ID tenanta v této instanci je pevně kódované – c0257de7-538f-415c-993A-1b87a031879d. Použijte následující uživatelské jméno a heslo.
            ...
            - **Heslo**  -  [Nastavte heslo pro Azure Resource Manager](azure-stack-edge-gpu-set-azure-resource-manager-password.md) a použijte toto heslo pro přihlášení. 
