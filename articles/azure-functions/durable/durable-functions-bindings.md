...
Při psaní funkcí nástroje Orchestrator ve skriptovacích jazycích (například skriptování v jazyce JavaScript nebo C#) je aktivační událost Orchestration definována následujícím objektem JSON v `bindings` poli *funkce Function. JSON* :
...
* **vstupy** – funkce orchestrace .NET podporují jenom `DurableOrchestrationContext` jako typ parametru. Deserializace vstupů přímo v signatuře funkce není podporována. Kód musí pro načtení `GetInput<T>` vstupních funkcí nástroje Orchestrator `getInput` použít metodu (.NET) nebo (JavaScript). Tyto vstupy musí být typy serializovatelných hodnot JSON.
...
> [!NOTE]
> Orchestrace JavaScriptu by měla `return`použít. Knihovna `durable-functions` se postará o volání `context.done` metody.
...
Pokud používáte aplikaci Visual Studio, je aktivační událost aktivity nakonfigurována pomocí atributu `ActivityTriggerAttribute` rozhraní .NET.
...
Pokud pro vývoj používáte skriptovací jazyky (například soubory *. csx* nebo *. js* ), aktivační událost orchestrace je definována následujícím objektem JSON v `bindings` poli *Function. JSON*:
...
Alternativně mohou funkce .NET navazovat `IAsyncCollector<T>` vazby `T` na `StartOrchestrationArgs` Where `JObject`je nebo.

Další informace o těchto operacích najdete v dokumentaci `IDurableOrchestrationClient` k rozhraní API.
...
> [!NOTE]
> Předchozí kód je pro Durable Functions 2. x. Pro Durable Functions 1. x je nutné použít typ `DurableOrchestrationClient` parametru místo. `IDurableOrchestrationClient` Další informace o rozdílech mezi verzemi najdete v článku o [Durable Functions verzích](durable-functions-versions.md) .
...
* **GetInput\<TInput> ()**: Získá vstup pro aktuální operaci. Parametr `TInput` typu musí být primitivního typu nebo typu JSON, který lze serializovat.
...
`IDurableEntityContext` Objekt předaný do funkce entity lze použít pomocí vlastnosti `Entity.Current` Async-Local. Tento přístup je vhodný při použití programovacího modelu založeného na třídě.
